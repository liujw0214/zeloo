...
_SESSION_OWNERSHIP_UNAVAILABLE = "Zeloo could not safely reserve this session. Try again."
...
def _other_runtime_lease_guard(session_id: str, session: dict):
    ...
    try:
        from zeloo_cli.active_sessions import active_session_liveness_guard, release_active_session_liveness_guard
    except Exception as exc:
...
def _transfer_active_session_slot(sid: str, session: dict, *, new_session_id: str) -> bool:
    ...
    if lease is None:
        return True
    try:
        from zeloo_cli.active_sessions import transfer_active_session
...
def _pop_session_by_id(sid: str) -> dict | None:
    ...
    with _sessions_lock:
        ...
        if session is not None:
            ...
            home = str(Path(session.get("profile_home") or get_zeloo_home()).resolve())
    ...
    for sid, session in clientless:
        ...
        with _session_resume_lock, _sessions_lock:
            ...
            if current.get("close_on_disconnect"):
                ...
                # `Zeloo --tui` keeps real _stdio. UNLESS another window (pop-out viewer) still shows the session:
