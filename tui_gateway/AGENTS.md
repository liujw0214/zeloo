...
activate with `Zeloo --tui` or `ZELOO_TUI=1`. `tui_gateway` is ALSO the backend the Desktop app
...
themselves come from `zeloo_cli/commands.py` (`zeloo_cli/AGENTS.md`).
...
npm run dev       # watch mode (rebuilds Zeloo-ink + tsx --watch)
npm start         # production
npm run build     # full build (Zeloo-ink + tsc)
