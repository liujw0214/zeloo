"""Cross-process mutual exclusion for in-flight Zeloo updates.
...
MARKER_NAME = ".Zeloo-update-in-progress"

# Set by an orchestrating updater (Tauri `Zeloo-setup --update`) to its own pid before
# spawning `Zeloo update` as a child stage; the parent holds the marker for its whole run,
...
HANDOFF_PID_ENV = "ZELOO_UPDATE_HANDOFF_PID"
...
# Tauri updater (UPDATE_EXIT_CONCURRENT in update.rs) to show "Zeloo is still running".
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Zeloo home (never the context-local profile override): the Rust
    updater resolves ``$ZELOO_HOME`` or the platform default and the desktop pins that same
    ...
    return get_process_zeloo_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``Zeloo update`` as a (grand)child, so a live marker
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Zeloo update is already running (PID {holder.pid}, "
