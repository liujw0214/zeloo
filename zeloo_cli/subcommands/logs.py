...
def build_logs_parser(subparsers, *, cmd_logs: Callable) -> None:
    ...
    logs_parser = subparsers.add_parser(
        "logs", help="View and filter Zeloo log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log / gui.log / desktop.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    Zeloo logs                    Show last 50 lines of agent.log
    Zeloo logs -f                 Follow agent.log in real time
    Zeloo logs errors             Show last 50 lines of errors.log
    Zeloo logs gateway -n 100     Show last 100 lines of gateway.log
    Zeloo logs gui -f             Follow gui.log in real time
    Zeloo logs desktop -f         Follow desktop.log (Electron app boot/backend)
    Zeloo logs --level WARNING    Only show WARNING and above
    Zeloo logs --session abc123   Filter by session ID
    Zeloo logs --component tools  Only show tool-related lines
    Zeloo logs --since 1h         Lines from the last hour
    Zeloo logs --since 30m -f     Follow, starting from 30 min ago
    Zeloo logs list               List available log files with sizes
""")
