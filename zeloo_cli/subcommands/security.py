"""``Zeloo security`` subcommand parser."""
...
def build_security_parser(subparsers, *, cmd_security: Callable) -> None:
    ...
    security_parser = subparsers.add_parser(
        ...
        description="On-demand vulnerability scan against OSV.dev. Covers the Zeloo "
            "venv (installed PyPI dists), Python deps declared by plugins under "
            "~/.Zeloo/plugins/, and pinned npx/uvx MCP servers in config.yaml. "
    ...
    audit_parser.add_argument(
        "--skip-venv", action="store_true", help="Skip scanning the Zeloo Python venv")
    audit_parser.add_argument(
