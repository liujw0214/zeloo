...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Zeloo Agent. Use 'Zeloo debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    Zeloo debug share              Upload debug report (asks for confirmation)
    Zeloo debug share --yes        Skip confirmation (for scripts/CI)
    Zeloo debug share --lines 500  Include more log lines
    Zeloo debug share --expire 30  Keep dpaste.com fallback pastes for 30 days
    Zeloo debug share --local      Print report locally (no upload)
    Zeloo debug share --no-redact  Disable upload-time secret redaction
    Zeloo debug share --nous       Upload to Nous-internal storage (private)
    Zeloo debug delete <url>       Delete a previously uploaded paste
""")
    ...
    delete_parser = debug_sub.add_parser(
        "delete", help="Delete a paste uploaded by 'Zeloo debug share'")
