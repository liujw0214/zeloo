"""``Zeloo pause`` / ``Zeloo resume`` — the global emergency stop.

``pause`` writes the ESTOP sentinel at ``$ZELOO_HOME/ESTOP``; cron, kanban and new gateway
...
def cmd_pause(args: argparse.Namespace) -> int:
    ...
    state = get_state() or {}
    verb = "Still paused" if already else "Zeloo paused"
    ...
    print(
        ...
        "    In-flight work keeps running. Run `Zeloo resume` to lift the pause.")
    return 0
...
def cmd_resume(args: argparse.Namespace) -> int:
    ...
    if disengage():
        print("▶️  Zeloo resumed — dispatch picks up on the next tick.")
    else:
        print(f"Zeloo is not paused (no sentinel at {sentinel_path()}).")
    return 0
