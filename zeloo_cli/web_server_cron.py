...
from zeloo_cli.web_models import CronJobCreate
...
def _default_multiplex_profile_allowlist() -> "list[str] | None":
    ...
    cfg_path = get_default_zeloo_root() / "config.yaml"
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from zeloo_cli.web_server_profiles import _fallback_profile_dicts
...
def _cron_default_profile() -> str:
    ...
    profile's job into ~/.Zeloo. ``custom`` (ZELOO_HOME outside the profiles tree) has no
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, ZELOO_HOME)."""
    from zeloo_cli import profiles as profiles_mod
...
def _cron_store_scope(home: Path):
    """Point ZELOO_HOME and the cron.jobs store at one profile's home for the block.
...
def _gateway_fire_endpoint(profile: str, home: Path) -> str:
    ...
    if multiplex and profile != "default":
        from zeloo_constants import get_default_zeloo_root
        listener_profile, listener_home = "default", get_default_zeloo_root()
        _log.info(
    ...
    try:
        ...
        # expansion) — never a raw yaml.safe_load (tests/zeloo_cli/test_config_read_guard.py).
        from zeloo_constants import reset_zeloo_home_override, set_zeloo_home_override
        token = set_zeloo_home_override(str(listener_home))
        try:
