...
DISTINCT from ``zeloo_cli.config.is_managed()`` / ``ZELOO_MANAGED`` (a coarse package-manager
...
_DEFAULT_MANAGED_DIR = Path("/etc/Zeloo")
...
def _under_pytest() -> bool:
    """True inside the test suite: ignore the system ``/etc/Zeloo`` so a real managed scope on a
    dev/CI box can't leak policy into the suite. An explicit ``ZELOO_MANAGED_DIR`` still wins."""
...
def get_managed_dir() -> Optional[Path]:
    ...
    Priority: ``$ZELOO_MANAGED_DIR`` (IT-only bootstrap override; never persisted to any .env;
    honored only when non-empty AND the directory exists), then ``/etc/Zeloo`` when it exists.
    A missing directory resolves to None — the common case, so it must be cheap + side-effect-free.
    """
    override = os.environ.get("ZELOO_MANAGED_DIR", "").strip()
    if override:
        p = Path(override)
    elif _under_pytest():
        return None
...
def apply_managed_overlay(config: dict) -> dict:
    ...
    try:
        ...
        from zeloo_cli.config import _deep_merge, _expand_env_vars, _normalize_root_model_keys
