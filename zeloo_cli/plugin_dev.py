"""Runtime-backed validation behind ``Zeloo plugins doctor``: every manifest/import/registration
...
def _doctor_runtime(plugin_path: Path):
    ...
    ZELOO_HOME with outbound socket connects blocked.
    """
    stack = ExitStack()
    try:
        # The temp dir enters the stack FIRST so any failure below (ENOSPC / KeyboardInterrupt in
        # copytree) removes it instead of stranding a Zeloo-plugin-doctor-* directory.
        home = Path(stack.enter_context(tempfile.TemporaryDirectory(prefix="Zeloo-plugin-doctor-")))
        ...
        stack.enter_context(patch.dict(os.environ, {
            "ZELOO_HOME": str(home),
            "ZELOO_BUNDLED_PLUGINS": str(bundled),
            "ZELOO_ENABLE_PROJECT_PLUGINS": "0",
        }, clear=False))
    ...
    try:
        ...
        if not manifests:
            raise _DoctorLoadError(f"Zeloo discovery found no valid plugin manifest under {copied}")
        if len(manifests) != 1:
...
def _is_plugin_module(name: str) -> bool:
    return name == "zeloo_plugins" or name.startswith("zeloo_plugins.")
...
def resolve_plugin_path(target: str | os.PathLike[str] | None = None) -> Path:
    ...
    if _is_plugin_id(raw):
        candidates.append(get_zeloo_home() / "plugins" / raw)
        try:
            ...
            pass
        candidates.append(Path.cwd() / ".Zeloo" / "plugins" / raw)
...
def _check_manifest_v2(report: "DoctorReport", manifest: Any) -> None:
    ...
    import re as _re
    from zeloo_cli.plugins import SUPPORTED_MANIFEST_VERSION
    mv = getattr(manifest, "manifest_version", 1)
    if mv > SUPPORTED_MANIFEST_VERSION:
        report.warning(
            f"manifest_version {mv} is newer than this Zeloo supports "
    ...
    if missing:
        report.warning(
            ...
            + " — Zeloo never auto-installs plugin dependencies; install manually: pip install "
            + " ".join(f"'{m}'" for m in missing))

    schema = getattr(manifest, "config_schema", {}) or {}
    if schema:
        from zeloo_cli.plugins import _CONFIG_SCHEMA_TYPES
...
def doctor_plugin(target: str | os.PathLike[str] | None = None) -> DoctorReport:
    """Validate one plugin through Zeloo' real scanner and registration path."""
    try:
