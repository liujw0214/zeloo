...
``zeloo_cli.models.<name>`` stays the stable import/monkeypatch surface:
...
from zeloo_cli.urllib_security import open_credentialed_url
from zeloo_cli.models_catalog_static import (
    CANONICAL_PROVIDERS,
    OPENROUTER_MODELS,
...
from zeloo_cli.models_reasoning_caps import (
...
_ZELOO_USER_AGENT = f"Zeloo-cli/{_ZELOO_VERSION}"
...
def _custom_provider_ssl_context(base_url: str):
    ...
    if not base_url:
        return None
    try:
        from zeloo_cli.config import get_custom_provider_tls_settings
...
def _openrouter_catalog_disk_ttl() -> float:
    ...
    from zeloo_cli.model_catalog import refresh_interval_seconds
...
def _openrouter_catalog_disk_path() -> Path:
    ...
    return get_zeloo_home() / "cache" / "openrouter_curated_catalog.json"
...
def _nous_recommended_disk_path() -> "Path":
    from zeloo_constants import get_zeloo_home
    return get_zeloo_home() / "cache" / "nous_recommended_cache.json"
...
def _resolve_nous_portal_url() -> str:
    ...
    try:
        from zeloo_cli.auth import DEFAULT_NOUS_PORTAL_URL, get_provider_auth_state
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    try:
        from zeloo_cli.model_catalog import get_default_model_from_cache
...
def recommended_nous_default_model() -> dict[str, Any]:
    ...
    sign-in completion in ``zeloo_cli.anon_auth`` so both land on the same model.
    """
    from zeloo_cli import models_pricing as mp
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    """True when ``supported_parameters`` advertises ``tools`` (Zeloo-agent is tool-calling-first).
...
def fetch_openrouter_models(
    ...
    try:
        from zeloo_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id, _ in fallback:
        ...
        if live_item is None or not _openrouter_model_supports_tools(live_item):
            continue
        # Hide models that don't advertise tool-calling support — Zeloo-agent requires it and surfacing
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    try:
        from zeloo_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    from zeloo_constants import AI_GATEWAY_BASE_URL
...
def _configured_custom_provider_ids() -> set[str]:
    ...
    try:
        ...
        from zeloo_cli.providers import custom_provider_slug
...
def _provider_has_credentials(pid: str) -> bool:
    try:
        from zeloo_cli.auth import get_auth_status, has_usable_secret
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    derived from :data:`CANONICAL_PROVIDERS` (shared with ``Zeloo model`` and ``/model``)."""
...
def _resolve_static_model_alias(
    ...
    try:
        from zeloo_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def detect_provider_for_model(
    ...
    from zeloo_cli.models_detect import (
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``zeloo_cli.auth.resolve_provider()`` to resolve it from credentials."""
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Zeloo should expose the /fast toggle for this model."""
...
def _first_exchangeable_copilot_token(raw_tokens) -> str:
    ...
    from zeloo_cli.copilot_auth import exchange_copilot_token, validate_copilot_token
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    def _pool_token() -> str:
        from zeloo_cli.auth import read_credential_pool
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from zeloo_cli.model_search import model_alias_canonical
...
def _codex_catalog(normalized: str, force_refresh: bool) -> list[str]:
    from zeloo_cli.codex_models import get_codex_model_ids
...
def _nous_catalog(normalized: str, force_refresh: bool) -> Optional[list[str]]:
    try:
        from zeloo_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
    ...
    # Portal models still surface without a Zeloo release.
...
def _openai_catalog(normalized: str, force_refresh: bool) -> Optional[list[str]]:
    ...
    # entries, so intersect with the curated agentic catalog so ``/model`` matches ``Zeloo model``.
    ...
    # pure-catalog helper (no extra live fetch) so we only accept models Zeloo actually ships. (#46850)
    ...
    from zeloo_cli.providers import is_official_openai_host
...
# every authed provider's /v1/models). One JSON file at $ZELOO_HOME/provider_models_cache.json;
...
def _provider_models_cache_path() -> Path:
    from zeloo_constants import get_zeloo_home
    return get_zeloo_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    try:
        ...
        for rel in ("auth.json", "credentials.json"):
            _mtime_part(rel, get_zeloo_home() / rel)
    except Exception:
        pass
...
def clear_provider_models_cache(provider: Optional[str] = None) -> None:
    ...
    ``/model --refresh`` and ``Zeloo model --refresh``."""
    try:
...
def opencode_zen_free_headers() -> dict:
    ...
    try:
        from zeloo_cli import __version__ as _v
    except Exception:
        _v = "0"
    return {
        "Authorization": "",
...
def probe_api_models(
    ...
    headers: dict[str, str] = {"User-Agent": _ZELOO_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"Zeloo-agent/{_ZELOO_VERSION}"
    ...
    if isinstance(request_headers, dict):
        ...
        from zeloo_cli.config import normalize_extra_headers
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=Zeloo"
...
def _fetch_ai_gateway_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    headers = {"Authorization": f"Bearer {api_key}", "User-Agent": _ZELOO_USER_AGENT}
    try:
...
def cached_fetch_api_models(
    ...
    from zeloo_cli.model_switch_providers import _NativePickerModelList
...
_PLUGIN_COMPAT_LAZY = {
    'LMStudioLoadResult': ('zeloo_cli.models_local', 'LMStudioLoadResult'),
    'PROVIDER_GROUPS': ('zeloo_cli.models_catalog_static', 'PROVIDER_GROUPS'),
    'ProviderEntry': ('zeloo_cli.models_catalog_static', 'ProviderEntry'),
    'atomic_json_write': ('utils', 'atomic_json_write'),
    'base_url_host_matches': ('utils', 'base_url_host_matches'),
    'compute_sale_discount': ('zeloo_cli.models_pricing', 'compute_sale_discount'),
    'ensure_lmstudio_model_loaded': ('zeloo_cli.models_local', 'ensure_lmstudio_model_loaded'),
    'fetch_ai_gateway_pricing': ('zeloo_cli.models_pricing', 'fetch_ai_gateway_pricing'),
    'fetch_lmstudio_models': ('zeloo_cli.models_local', 'fetch_lmstudio_models'),
    'fetch_models_with_pricing': ('zeloo_cli.models_pricing', 'fetch_models_with_pricing'),
    'fetch_ollama_local_models': ('zeloo_cli.models_local', 'fetch_ollama_local_models'),
    'get_cached_nous_inference_base_url': ('zeloo_cli.models_pricing', 'get_cached_nous_inference_base_url'),
    'get_pricing_for_provider': ('zeloo_cli.models_pricing', 'get_pricing_for_provider'),
    'group_providers': ('zeloo_cli.models_catalog_static', 'group_providers'),
    'lmstudio_model_reasoning_options': ('zeloo_cli.models_local', 'lmstudio_model_reasoning_options'),
    'nous_catalog_url': ('zeloo_cli.models_reasoning_caps', 'nous_catalog_url'),
    'nous_model_reasoning_capabilities': ('zeloo_cli.models_reasoning_caps', 'nous_model_reasoning_capabilities'),
    'nous_policy_allowed_ids': ('zeloo_cli.models_pricing', 'nous_policy_allowed_ids'),
    'ollama_model_supports_thinking': ('zeloo_cli.models_local', 'ollama_model_supports_thinking'),
    'openrouter_model_reasoning_capabilities': ('zeloo_cli.models_reasoning_caps', 'openrouter_model_reasoning_capabilities'),
    'parse_openrouter_reasoning_capabilities': ('zeloo_cli.models_reasoning_caps', 'parse_openrouter_reasoning_capabilities'),
    'peek_cached_pricing': ('zeloo_cli.models_pricing', 'peek_cached_pricing'),
    'pricing_cache_scope': ('zeloo_cli.models_pricing', 'pricing_cache_scope'),
    'probe_lmstudio_models': ('zeloo_cli.models_local', 'probe_lmstudio_models'),
    'probe_ollama_local_models': ('zeloo_cli.models_local', 'probe_ollama_local_models'),
    'provider_group_for_slug': ('zeloo_cli.models_catalog_static', 'provider_group_for_slug'),
    'refresh_reasoning_caps_async': ('zeloo_cli.models_reasoning_caps', 'refresh_reasoning_caps_async'),
    'restrict_to_nous_policy': ('zeloo_cli.models_pricing', 'restrict_to_nous_policy'),
    'should_use_ollama_native_catalog': ('zeloo_cli.models_local', 'should_use_ollama_native_catalog'),
    'url_origin': ('zeloo_cli.urllib_security', 'url_origin'),
    'validate_requested_model': ('zeloo_cli.models_validate', 'validate_requested_model'),
    'warm_nous_reasoning_caps_async': ('zeloo_cli.models_reasoning_caps', 'warm_nous_reasoning_caps_async'),
    'warm_openrouter_reasoning_caps_async': ('zeloo_cli.models_reasoning_caps', 'warm_openrouter_reasoning_caps_async'),
}
