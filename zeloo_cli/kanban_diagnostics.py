...
def _log_hint_action(task_id: str) -> DiagnosticAction:
    cmd = f"Zeloo kanban log {task_id}"
...
_TRIAGE_SLOTS = {
    ...
    False: (
        ...
        "`Zeloo kanban specify`, which uses auxiliary.triage_specifier.",
    ),
}
...
def _rule_triage_aux_unavailable(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    actions = [_cli_hint(
        f"Configure {primary_slot}", f"Zeloo config set {primary_slot}.provider auto", suggested=True,
    )]
    if not fallback_explicit and not main_visible:
        actions.append(_cli_hint(
            f"Or configure fallback {fallback_slot}", f"Zeloo config set {fallback_slot}.provider auto",
        ))
    if not auto_decompose:
        cmd = f"Zeloo kanban specify {task_id}"
...
def _rule_repeated_failures(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    if most_recent_outcome == "spawn_failed" and assignee and assignee != "default":
        ...
        doctor, auth = f"Zeloo -p {assignee} doctor", f"Zeloo -p {assignee} auth"
...
def _rule_review_dependency_deadlock(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    if task_id:
        actions.append(_cli_hint(
            "Complete the finished implementation phase", f"Zeloo kanban complete {task_id}",
            suggested=True,
        ))
    if task_id and child_ids:
        actions.append(_cli_hint(
            "Or unlink the incorrectly gated reviewer", f"Zeloo kanban unlink {task_id} {child_ids[0]}",
        ))
...
def _rule_block_unblock_cycling(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    actions: list[DiagnosticAction] = []
    if task_id:
        cmd = f"Zeloo kanban events {task_id}"
...
def _rule_stranded_in_ready(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    actions = [
        ...
        _cli_hint("Check dispatcher status", "Zeloo kanban diagnostics"),
    ]
    return [Diagnostic(
