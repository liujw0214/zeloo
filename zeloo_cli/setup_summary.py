...
resolved through the module object so test patches on ``zeloo_cli.setup.<name>`` take effect."""

import logging
from tools import tool_backend_helpers
from zeloo_cli import nous_subscription
...
_TTS_SUMMARY_ROWS = {
    ...
    "neutts": ("NeuTTS", "neutts", "run 'Zeloo setup tts'"),
    "kittentts": ("KittenTTS", "kittentts", "run 'Zeloo setup tts'")}
...
_STT_SUMMARY_DEFAULT = ("Local Whisper", "faster_whisper", "run 'Zeloo tools' → Speech-to-Text")
...
_EDIT_WIZARD_ROWS = (
    ("Zeloo setup", "          Re-run the full wizard"), ("Zeloo setup model", "    Change model/provider"),
    ("Zeloo setup terminal", " Change terminal backend"), ("Zeloo setup gateway", "  Configure messaging"),
    ("Zeloo setup tools", "    Configure tool providers"))
_EDIT_CONFIG_ROWS = (
    ("Zeloo config", "         View current settings"), ("Zeloo config edit", "    Open config in your editor"),
    ("Zeloo config set <key> <value>", ""))
_READY_ROWS = (
    ("Zeloo", "              Start chatting"), ("Zeloo gateway", "      Start messaging gateway"),
    ("Zeloo doctor", "       Check for issues"))
...
def _vision_row(config, feats):
    ...
    try:
        ...
        ok = False
    return ("Vision (image analysis)", ok, None if ok else "run 'Zeloo setup' to configure")
...
def _video_gen_row(config, feats):
    # Opt-in via `Zeloo tools` → Video Generation. Only show the row when a plugin reports
...
def _modal_row(config, feats):
    ...
    if _setup.cfg_get(config, "terminal", "backend") == "modal":
        ...
        return ("Modal Execution", False, "run 'Zeloo setup terminal'")
...
def _spotify_row(config, feats):
    # OAuth via Zeloo auth spotify — check auth.json, not env vars
    try:
...
def _print_setup_summary(config: dict, zeloo_home):
    ...
    try:
        ...
        print()
        _setup.print_warning("No inference provider is configured — Zeloo cannot chat yet.")
        _setup._info("  Finish this one step with either of:",
              "    Zeloo model            (pick any provider/model)",
              "    Zeloo setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if available_count < len(tool_status):
        _setup.print_warning("Some tools are disabled. Run 'Zeloo setup tools' to configure them,")
    ...
    for label, value in (("Settings:", f"  {_setup.get_config_path()}"), ("API Keys:", f"  {_setup.get_env_path()}"),
                         ("Data:", f"      {zeloo_home}/cron/, sessions/, logs/")):
...
import zeloo_cli.setup as _setup  # noqa: E402  (bottom: zeloo_cli.setup imports this module)
