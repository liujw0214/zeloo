...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    values: Dict[str, Any] = {}
    for path in (zeloo_home / f"{name}.json", zeloo_home / name / "config.json"):
    ...
    # Holographic stores under plugins.Zeloo-memory-store.
    plugins_cfg = cfg.get("plugins")
    if name == "holographic" and isinstance(plugins_cfg, dict):
        holographic_cfg = plugins_cfg.get("Zeloo-memory-store")
