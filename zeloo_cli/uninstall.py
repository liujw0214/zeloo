...
def _strip_zeloo_path_lines(content: str) -> str:
    """Drop the ``# Zeloo Agent`` marker (+ its PATH line) and any Zeloo PATH line; squash blank runs."""
    new_lines = []
    skip_next = False
    for line in content.split('\n'):
        if '# Zeloo Agent' in line or '# Zeloo-agent' in line:
            skip_next = True
            continue
        if skip_next and ('Zeloo' in line.lower() and 'PATH' in line):
            skip_next = False
            continue
        skip_next = False
        if 'Zeloo' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
            continue
...
def remove_path_from_shell_configs():
    """Remove Zeloo PATH entries from shell configuration files."""
    removed_from = []
    for config_path in (c for c in (Path.home() / n for n in _SHELL_RC_NAMES) if c.exists()):
        try:
            content = config_path.read_text(encoding="utf-8")
            new_content = _strip_zeloo_path_lines(content)
...
def remove_wrapper_script():
    """Remove the Zeloo wrapper script if it exists."""
    def _unlink_ours(wrapper: Path) -> bool:
        # Only our wrapper (contains a zeloo_cli / Zeloo-agent reference).
        content = wrapper.read_text(encoding="utf-8")
        return _unlink_if('zeloo_cli' in content or 'Zeloo-agent' in content, wrapper)

    candidates = (
        bin_dir / name
        for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin"))
        for name in ("Zeloo", "Zeloo-acp", "Zeloo-agent"))
...
def remove_node_symlinks(zeloo_home: Path) -> list:
    ...
    node_dir = (zeloo_home / "node").resolve()
...
def uninstall_gateway_service():
    ...
    try:
        from zeloo_cli.gateway import kill_gateway_processes, find_gateway_pids
...
def _remove_launchd_gateway() -> bool:
    ...
    from zeloo_cli.gateway import get_launchd_plist_path
...
def _remove_windows_gateway() -> bool:
    ...
    from zeloo_cli import gateway_windows as gw
...
# ZELOO_HOME / ZELOO_GIT_BASH_PATH (HKCU\Environment), User-scope PATH entries
# (%LOCALAPPDATA%\Zeloo\git\{cmd,bin,usr\bin}, ...\Zeloo\node), PortableGit + Node copies
...
def _zeloo_path_markers(zeloo_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Prefixes identifying Zeloo-owned User-PATH entries (prefix match sweeps git\cmd, git\bin,
    ...
    root = str(zeloo_home).rstrip("\\/")
    subs = ("Zeloo-agent", "git", "node", "venv") + (("bin",) if include_managed_bin else ())
    return [f"{root}\\{sub}" for sub in subs]


def remove_path_from_windows_registry(zeloo_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip Zeloo-owned entries from User-scope PATH in the registry (see ``_zeloo_path_markers``)."""
    markers = tuple(m.lower() for m in _zeloo_path_markers(zeloo_home, include_managed_bin=include_managed_bin))
...
def remove_zeloo_env_vars_windows() -> list[str]:
    """Delete ZELOO_HOME and ZELOO_GIT_BASH_PATH from User-scope env vars."""
    def edit(winreg, key, removed):
        for name in ("ZELOO_HOME", "ZELOO_GIT_BASH_PATH"):
            try:
...
def remove_portable_tooling_windows(zeloo_home: Path) -> list[Path]:
    """Delete the PortableGit / Node / gateway-service dirs the Windows installer created under
    ``zeloo_home`` (isolated from any system Git / Node, so nothing else breaks)."""
    targets = (zeloo_home / sub for sub in ("git", "node", "gateway-service"))
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``Zeloo`` launchers install.ps1 staged in the managed ``bin`` dir: every mode
    ...
    try:
        ...
        from zeloo_cli._install_repair import _WINDOWS_BIN_LAUNCHERS
        from zeloo_constants import get_default_zeloo_root
        bin_dir = get_default_zeloo_root() / "bin"
    except Exception as e:
...
def _is_default_zeloo_home(zeloo_home: Path) -> bool:
    """Return True when ``zeloo_home`` points at the default (non-profile) root."""
    try:
        from zeloo_constants import get_default_zeloo_root
        return zeloo_home.resolve() == get_default_zeloo_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    out to ``Zeloo -p <name> gateway stop|uninstall`` because service names / unit paths derive
    from the current ZELOO_HOME and can't be switched in-process."""
    name = profile.name
    log_info(f"Uninstalling profile '{name}'...")

    # 1. Gateway service, via `python -m zeloo_cli.main` (the `Zeloo` wrapper may be half-gone).
    zeloo_invocation = [sys.executable, "-m", "zeloo_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                zeloo_invocation + ["gateway", subcmd], capture_output=True, text=True,
    ...
    # 3. Wipe the profile's ZELOO_HOME directory.
    _rmtree_step(profile.path, indent="  ", fully=False)


def run_gui_uninstall(args):
    """``Zeloo uninstall --gui``: remove the desktop app's built artifacts, packaged bundle
    (best-effort) and Electron userData — never config/sessions/.env, the agent or its venv."""
    from zeloo_cli.gui_uninstall import agent_is_installed, gui_install_summary, uninstall_gui
    zeloo_home = get_zeloo_home()
    summary = gui_install_summary(zeloo_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    _print_box("│         ⚕ Zeloo Chat GUI Uninstaller                  │", Colors.MAGENTA)
    print()

    if not summary["gui_installed"]:
        print("No Zeloo Chat GUI installation was found.")
        print(f"  Checked: {zeloo_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Zeloo agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(zeloo_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Zeloo agent at {zeloo_home / 'Zeloo-agent'}")
        print(f"  • Your config, sessions, and secrets under {zeloo_home}")
        print()
    ...
    print()
    uninstall_gui(zeloo_home)

    print()
    _print_box("│            ✓ Chat GUI Uninstalled!                      │", Colors.GREEN)
    print()
    print("The Zeloo agent is still installed. Run 'Zeloo' to use the CLI,")
    print("or 'Zeloo uninstall' to remove the agent too.")
    print()


def run_uninstall(args):
    """Interactive/``--yes`` uninstall: full removes code and ``~/.Zeloo/``; keep-data removes only
    ...
    if bool(getattr(args, "dry_run", False)):
        _print_uninstall_dry_run(
            project_root=project_root, zeloo_home=zeloo_home, full_uninstall=full_flag)
        return

    # Named profiles (only when uninstalling from the default root) are offered for cleanup too,
    # instead of leaving zombie ZELOO_HOMEs and systemd units behind.
    is_default_profile = _is_default_zeloo_home(zeloo_home)
    ...
    print()
    _print_box("│            ⚕ Zeloo Agent Uninstaller                  │", Colors.MAGENTA)
    print()

    # Show what will be affected
    print(color("Current Installation:", Colors.CYAN, Colors.BOLD))
    print(f"  Code:    {project_root}")
    print(f"  Config:  {zeloo_home / 'config.yaml'}")
    print(f"  Secrets: {zeloo_home / '.env'}")
    print(f"  Data:    {zeloo_home / 'cron/'}, {zeloo_home / 'sessions/'}, {zeloo_home / 'logs/'}")
    print()
    ...
    # services, alias wrappers, ZELOO_HOME dirs) — otherwise they leave zombie services behind.
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Zeloo data!", Colors.RED, Colors.BOLD))
        print(color("   Including: configs, API keys, sessions, scheduled jobs, logs", Colors.RED))
        if remove_profiles:
            print(color(f"   Plus {n_profiles} profile(s): {profile_names}", Colors.RED))
    else:
        print("This will remove the Zeloo code but keep your configuration and data.")
    ...
    _perform_uninstall(
        project_root=project_root, zeloo_home=zeloo_home, full_uninstall=full_uninstall,
        remove_profiles=remove_profiles, named_profiles=named_profiles)


def _print_uninstall_dry_run(*, project_root: Path, zeloo_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Zeloo PATH entries from shell configs / Windows User PATH")
    print("  • Zeloo wrapper scripts and Zeloo-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if not full_uninstall:
        print(f"  • Keep Zeloo config/data: {zeloo_home}")
    else:
        print(f"  • Zeloo config/data: {zeloo_home}")
        profiles = _discover_named_profiles() if _is_default_zeloo_home(zeloo_home) else []
        if profiles:
...
def _perform_uninstall(
    *,
    ...
    the checkout -> (Windows) PortableGit/Node -> optionally ``$ZELOO_HOME`` and named profiles."""
    print()
    ...
    # 2-3b. PATH entries, wrapper, Windows launchers, node symlinks. Windows: zeloo_home is
    #    %VAR%-expanded because install.ps1 writes literal C:\Users\<u>\...; Zeloo\bin (launchers +
    ...
    windows = _is_windows()
    sweep_managed_bin = windows and full_uninstall and _is_default_zeloo_home(zeloo_home)
    for on_this_platform, label, remove, success_fmt, none_msg in (
        ...
        (windows, "Removing PATH entries from Windows User environment...",
         lambda: remove_path_from_windows_registry(
             Path(os.path.expandvars(str(zeloo_home))), include_managed_bin=sweep_managed_bin),
         "Removed from User PATH: {}", "No Zeloo-owned PATH entries in User environment"),
        (windows, "Removing ZELOO_HOME / ZELOO_GIT_BASH_PATH User env vars...",
         remove_zeloo_env_vars_windows, "Removed User env var: {}", "No Zeloo-set User env vars to remove"),
        (True, "Removing Zeloo command...", remove_wrapper_script, "Removed {}", "No wrapper script found"),
        (windows, "Removing Windows Zeloo launchers...",
         remove_windows_bin_launchers, "Removed {}", "No Windows Zeloo launchers found"),
        (True, "Removing Zeloo-managed node/npm/npx symlinks...",
         lambda: remove_node_symlinks(zeloo_home), "Removed {}", "No Zeloo-managed node/npm/npx symlinks found"),
    ):
        if on_this_platform:
    ...
    #     .env (safe in keep-data mode); the packaged app + Electron userData live OUTSIDE ZELOO_HOME.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from zeloo_cli.gui_uninstall import uninstall_gui
        if not uninstall_gui(zeloo_home):
    ...
    if windows:
        _remove_step(
            ...
            lambda: remove_portable_tooling_windows(zeloo_home), "Removed {}",
            "No Windows installer artifacts to remove")

    # 5. Optionally remove ~/.Zeloo/ data directory (and named profiles)
    if full_uninstall:
        ...
        _rmtree_step(zeloo_home)
    else:
        log_info(f"Keeping configuration and data in {zeloo_home}")
    ...
    print()
    print("Thank you for using Zeloo Agent! ⚕")
    print()


_REINSTALL_HINT = {
    True: "  iex (irm https://Zeloo-agent.nousresearch.com/install.ps1)",
    False: "  curl -fsSL https://Zeloo-agent.nousresearch.com/install.sh | bash"}
...
def main(argv=None) -> int:
    """``python -m zeloo_cli.uninstall --mode <gui|lite|full>``. Imports only stdlib +
    ``zeloo_constants`` + ``zeloo_cli.colors``, so it runs under a bare system Python (no venv)."""
    import argparse
    parser = argparse.ArgumentParser(prog="python -m zeloo_cli.uninstall")
    parser.add_argument(
