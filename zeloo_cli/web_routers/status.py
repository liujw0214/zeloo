...
Extracted from ``zeloo_cli.web_server``; app state and helpers are late-bound through
:mod:`zeloo_cli.web_deps` (cycle-safe, monkeypatch-friendly).
"""
...
from zeloo_cli.web_deps import LateState, late
from zeloo_cli.web_server_gateway import _display_system_platform
...
from zeloo_cli.config import get_config_path, get_env_path
from zeloo_cli.web_models import CuratorPause, LearningNodeRef, LearningNodeEdit, DebugShareRequest
from zeloo_cli.web_routers._common import scoped_to_thread
...
_collect_profile_gateway_topology_cached = late("_collect_profile_gateway_topology_cached", "zeloo_cli.web_server_gateway")
_config_profile_scope = late("_config_profile_scope", "zeloo_cli.web_server_profiles")
_dashboard_local_update_managed_externally = late("_dashboard_local_update_managed_externally", "zeloo_cli.web_server_files")
_load_configured_gateway_platforms = late("_load_configured_gateway_platforms", "zeloo_cli.web_server_gateway")
_probe_gateway_health = late("_probe_gateway_health", "zeloo_cli.web_server_gateway")
_require_token = late("_require_token")
_resolve_profile_dir = late("_resolve_profile_dir", "zeloo_cli.web_server_profiles")
_resolve_restart_drain_timeout = late("_resolve_restart_drain_timeout", "zeloo_cli.web_server_lifecycle")
_spawn_zeloo_action = late("_spawn_zeloo_action", "zeloo_cli.web_server_gateway")
_ssh_runtime_intact = late("_ssh_runtime_intact")
app = LateState("app")  # the FastAPI instance (app.state.*)
check_config_version = late("check_config_version", "zeloo_cli.config")
get_zeloo_home = late("get_zeloo_home", "zeloo_cli.config")
...
load_config = late("load_config", "zeloo_cli.config")
read_runtime_status = late("read_runtime_status", "gateway.status")
_open_session_db_for_profile = late("_open_session_db_for_profile", "zeloo_cli.web_server_sessions")
...
def _count_status_active_sessions() -> int:
    ...
    /api/status never routinely writes to state.db while another Zeloo process uses it."""
    from zeloo_state import _default_db_path
...
async def get_ssh_ownership(request: Request):
    from zeloo_cli.web_server import _SSH_OWNER_NONCE
...
# status) are written to the per-profile home, not the process-level ZELOO_HOME (see issue #69143). Plain
...
def _auth_gate_status() -> Dict[str, Any]:
    ...
    auth_flows: list[str] = []
    try:
        from zeloo_cli.dashboard_auth import (
...
def _nous_session_validity() -> str:
    ...
    try:
        from zeloo_cli.auth import get_nous_session_validity
...
async def _component_health(gateway: Dict[str, Any]) -> Dict[str, Any]:
    ...
    from zeloo_cli.web_server import DASHBOARD_HEALTH
    ...
    try:
        ...
        storage_check = await run_in_threadpool(_probe_state_db, get_zeloo_home())
...
async def _advisory_pressure(status: Dict[str, Any], home: Path) -> None:
    ...
    try:
        from zeloo_state import SessionDB as _SDB
...
async def get_status(profile: Optional[str] = None):
    ...
    """
    from zeloo_cli.web_server import _GATEWAY_HEALTH_URL
    status_scope = None
    ...
    try:
        ...
        # Off-loop: on a cold Windows install the first import of zeloo_cli.gateway blocks
        ...
        status = {
            ...
            "can_update_ZELOO": not _dashboard_local_update_managed_externally(),
        ...
        await _advisory_pressure(status, profile_dir if profile_dir else get_zeloo_home())

        # Profile NAMES and ``gateway_mode`` are low-sensitivity product surface (Zeloo Cloud
        ...
        if not auth["auth_required"]:
            status.update({
                "zeloo_home": str(get_zeloo_home()), "config_path": str(get_config_path()),
...
async def get_system_stats():
    ...
    disk/uptime when available). Non-sensitive: no env values, no paths beyond Zeloo home."""
    ...
    info: Dict[str, Any] = {
        **_display_system_platform(
        ...
        "zeloo_version": __version__, "cpu_count": os.cpu_count()}

    def _disk():
        du = psutil.disk_usage(str(get_zeloo_home()))
...
def _spawn_action(argv: list, name: str, prefix: str) -> dict:
    """Spawn a background ``Zeloo <argv>`` action; a spawn failure is ``500 "<prefix>: <exc>"``."""
    try:
        proc = _spawn_zeloo_action(argv, name)
    except Exception as exc:
...
def _get_portal_status_sync():
    ...
    auth: Dict[str, Any] = {}
    try:
        from zeloo_cli.auth import get_nous_auth_status_local
    ...
    features = []
    try:
        from zeloo_cli.nous_subscription import get_nous_subscription_features
...
async def get_logs(
    ...
    from zeloo_cli.logs import _read_tail, LOG_FILES
    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_zeloo_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from zeloo_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
