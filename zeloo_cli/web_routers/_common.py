...
late-binding seam in :mod:`zeloo_cli.web_deps` (web_server owns helpers/state;
...
log = logging.getLogger("zeloo_cli.web_server")
...
def spawn_profile_action(
    ...
    """Spawn a background ``Zeloo -p <profile> <argv>`` action; a spawn
    failure is logged and becomes ``500 "<prefix>: <exc>"``."""
    with http_failure(log_msg, 500, prefix):
        proc = _spawn_zeloo_action(_profile_cli_args(profile) + argv, name)
...
# instead of a 500. The file is never quarantined or renamed from here — that is `Zeloo doctor`'s job.
...
CORRUPT_STORE_DETAIL = {
    ...
    "message": "state.db corrupt — run `Zeloo doctor` (then `Zeloo doctor --fix` or `Zeloo sessions repair`).",
}
...
def corrupt_store_as_status(db_path):
    ...
    from zeloo_state_errors import is_malformed_db_error
    ...
    try:
        ...
        if last is None or now - last >= _CORRUPT_STORE_WARN_INTERVAL_S:
            ...
            log.warning("state.db at %s is corrupt (%s); dashboard reads return a status payload until it is "
                        "repaired — run `Zeloo doctor`", db_path, exc)
        else:
