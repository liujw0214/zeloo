"""Gateway restart/drain, Zeloo update and background-action status dashboard routes.

Extracted from ``zeloo_cli.web_server``; helpers/state that tests monkeypatch on
...
from zeloo_cli.config import format_docker_update_message, recommended_update_command_for_method
from zeloo_cli.web_deps import LateState, late
from zeloo_cli.web_server_gateway import _ACTION_LOG_FILES
from zeloo_cli.web_routers._common import http_failure
...
detect_install_method = late("detect_install_method", "zeloo_cli.config")
get_zeloo_home = late("get_zeloo_home", "zeloo_cli.config")
_ACTION_COMMANDS = LateState("_ACTION_COMMANDS", "zeloo_cli.web_server_gateway")
_ACTION_IDS = LateState("_ACTION_IDS", "zeloo_cli.web_server_gateway")
_ACTION_PROCS = LateState("_ACTION_PROCS", "zeloo_cli.web_server_gateway")
_ACTION_RESULTS = LateState("_ACTION_RESULTS", "zeloo_cli.web_server_gateway")
...
def _server_path(name: str) -> Path:
    ...
    import zeloo_cli.web_server_gateway as gw
...
_UPDATE_ACTION_COMPLETED_RE = re.compile(r"^=== Zeloo-update completed ([0-9a-f]{32}) ===$")

_MANAGED_EXTERNALLY_MESSAGE = "Zeloo updates are managed outside this dashboard in containerized environments."
...
def _durable_completed_update_action_id(lines: List[str]) -> Optional[str]:
    ...
    for index, line in enumerate(lines):
        if line.startswith("=== Zeloo update started "):
            last_start = index
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``Zeloo gateway restart`` in the background."""
...
def _update_refused(error: str, message: str, update_command: str) -> Dict[str, Any]:
    _record_completed_action("Zeloo-update", message, exit_code=1)
    return {
        "ok": False, "pid": None, "name": "Zeloo-update", "error": error, "message": message,
        "update_command": update_command,
    }


@router.post("/api/Zeloo/update")
async def update_ZELOO():
    """Kick off ``Zeloo update`` in the background."""
    ...
    existing = _ACTION_PROCS.get("Zeloo-update")
    if existing is not None and existing.poll() is None:
        response = {"ok": True, "pid": existing.pid, "name": "Zeloo-update", "already_running": True}
        action_id = _ACTION_IDS.get("Zeloo-update")
        if action_id:
            response["action_id"] = action_id
        return response

    action_id = secrets.token_hex(16)
    with http_failure("Failed to spawn Zeloo update", 500, "Failed to start update"):
        proc = _spawn_zeloo_action(["update"], "Zeloo-update", env_overrides={"ZELOO_ACTION_ID": action_id})
    return {"ok": True, "pid": proc.pid, "name": "Zeloo-update", "action_id": action_id}


_NON_APPLYABLE_MESSAGES = {
    "docker": format_docker_update_message,
    "apt": lambda: "Zeloo is managed by Termux APT; run `pkg upgrade Zeloo-agent`.",
}


@router.get("/api/Zeloo/update/check")
async def check_zeloo_update(force: bool = False):
    """Report whether a Zeloo update is available, without applying it.
    ...
    try:
        from zeloo_cli.banner import check_for_updates, upstream_commits_behind

        if force:
            with contextlib.suppress(OSError):
                (get_zeloo_home() / ".update_check").unlink()
...
async def get_action_status(name: str, lines: int = 200):
    ...
    if name == "Zeloo-update":
        ...
        if durable_update_action_id:
            marker = f"=== Zeloo-update completed {durable_update_action_id} ==="
...
def _latest_update_receipt_summary() -> Optional[Dict[str, Any]]:
    """Compact summary of the latest receipt (written by EVERY ``Zeloo update`` run,
    incl. refused/failed), or None; never raises. Steps/skips stay in the full endpoint.

    Phase-1 bullet 3 (#91277): the receipt (written by EVERY ``Zeloo update`` run since #91283, including
...
@status_router.get("/api/Zeloo/update/receipt")
async def get_update_receipt():
    ...
    receipt = _read_latest_receipt()
    if not receipt:
        raise HTTPException(status_code=404, detail="No update receipt found (no `Zeloo update` run recorded).")
