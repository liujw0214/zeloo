...
from zeloo_cli.web_models import ModelAssignment, MoaConfigPayload, MoaModelSlot
...
def get_recommended_default_model(provider: str = ""):
    ...
    ``Zeloo model``'s curation so GUI onboarding lands on a sensible default.
    ...
    try:
        ...
        from zeloo_cli.models import pick_silent_default_model
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    with http_failure("PUT /api/model/moa failed", 500, detail="Failed to save MoA config"):
        from zeloo_cli.moa_config import normalize_moa_config, validate_moa_payload
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    ``~/.Zeloo/config.yaml`` — applies to **new** sessions only; a running chat
