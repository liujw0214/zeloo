"""Top-level argparse construction for the Zeloo CLI.

Only the top-level parser and the ``chat`` subparser live here. Every other subparser (model,
gateway, sessions, …) is built by ``zeloo_cli/subcommands/<group>.py`` and wired in
...
# (it sets ``ZELOO_HOME`` and strips itself from ``sys.argv``), so it isn't on the parser.
...
# live parser fails. The derived path is authoritative; tests/zeloo_cli/
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    argparse surface — the drift that made ``Zeloo --reasoning high chat …`` misread ``high`` as
...
def _inherited_flag(parser, *args, **kwargs):
    """``parser.add_argument`` + tag the Action ``inherit_on_relaunch`` for ``zeloo_cli.relaunch``."""
...
_EPILOGUE = """
Examples:
    Zeloo                        Start interactive chat
    Zeloo chat -q "Hello"        Single query mode
    Zeloo --tui                  Launch the modern TUI (or set display.interface: tui)
    Zeloo --cli                  Force the classic REPL (overrides display.interface: tui)
    Zeloo -c                     Resume the most recent session
    Zeloo -c "my project"        Resume a session by name (latest in lineage)
    Zeloo --resume <session_id>  Resume a specific session by ID
    Zeloo --resume latest        Resume the most recent session (same as -c)
    Zeloo --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    Zeloo setup                  Run setup wizard
    Zeloo logout                 Clear stored authentication
    Zeloo auth add <provider>    Add a pooled credential
    Zeloo auth list              List pooled credentials
    Zeloo auth remove <p> <t>    Remove pooled credential by index, id, or label
    Zeloo auth reset <p> [t]     Clear exhaustion status for a provider, or one credential
    Zeloo auth priority <p> <t> <n>  Move a pooled credential to priority n (0 = tried first)
    Zeloo auth refresh <p> [t]   Refresh a pooled OAuth credential and clear its cooldown
    Zeloo model                  Select default model
    Zeloo fallback [list]        Show fallback provider chain
    Zeloo fallback add           Add a fallback provider (same picker as `Zeloo model`)
    Zeloo fallback remove        Remove a fallback provider from the chain
    Zeloo config                 View configuration
    Zeloo config edit            Edit config in $EDITOR
    Zeloo config set model gpt-4 Set a config value
    Zeloo gateway                Run messaging gateway
    Zeloo -s Zeloo-agent-dev,github-auth
    Zeloo -w                     Start in isolated git worktree
    Zeloo gateway install        Install gateway background service
    Zeloo sessions list          List past sessions
    Zeloo sessions browse        Interactive session picker
    Zeloo sessions rename ID T   Rename/title a session
    Zeloo logs                   View agent.log (last 50 lines)
    Zeloo logs -f                Follow agent.log in real time
    Zeloo logs errors            View errors.log
    Zeloo logs --since 1h        Lines from the last hour
    Zeloo debug share             Upload debug report for support
    Zeloo console                Open the safe Zeloo command console
    Zeloo update                 Update to latest version
    Zeloo dashboard              Start web UI dashboard (port 9119)
    Zeloo dashboard --stop       Stop running dashboard processes
    Zeloo dashboard --status     List running dashboard processes
...
def _add_top_level_flags(parser: argparse.ArgumentParser) -> None:
    ...
    inherited(parser, "-m", "--model", default=None, help=(
        ...
        "Applies to -z/--oneshot and --tui. Also settable via ZELOO_INFERENCE_MODEL env var."))
    inherited(parser, "--provider", default=None, help=(
        "Provider override for this invocation (e.g. openrouter, anthropic). "
        "Applies to -z/--oneshot and --tui. The persistent provider lives in config.yaml "
        "under model.provider — use `Zeloo setup` or edit the file to change it."))
    ...
    inherited(parser, "--accept-hooks", action="store_true", default=False, help=(
        ...
        "without a TTY prompt.  Equivalent to ZELOO_ACCEPT_HOOKS=1 or "
    ...
    inherited(parser, "--ignore-user-config", action="store_true", default=False,
              help="Ignore ~/.Zeloo/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)")
...
def _build_chat_parser(subparsers) -> argparse.ArgumentParser:
    ...
    ``Zeloo -m foo chat`` argparse first sets ``args.model`` from the top-level parser, then
    ...
    unless the flag is actually passed after the subcommand (tests/zeloo_cli/
    test_argparse_flag_propagation.py).
    """
    chat_parser = subparsers.add_parser(
        "chat", help="Interactive chat with the agent",
        description="Start an interactive chat session with Zeloo Agent")
    ...
    inherited(chat_parser, "--accept-hooks", action="store_true", default=SUPPRESS, help=(
        "Auto-approve any unseen shell hooks declared in config.yaml "
        "without a TTY prompt (see also ZELOO_ACCEPT_HOOKS env var and "
    ...
    inherited(chat_parser, "--ignore-user-config", action="store_true", default=SUPPRESS,
              help="Ignore ~/.Zeloo/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.")
    ...
    inherited(chat_parser, "--safe-mode", action="store_true", default=SUPPRESS,
              help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Zeloo itself.")
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="Zeloo", description="Zeloo Agent - AI assistant with tool-calling capabilities",
