...
Split out of :mod:`zeloo_cli.plugins`. Names that tests patch on the origin
...
ENTRY_POINTS_GROUP = "zeloo_agent.plugins"
ENTRY_POINT_CAPABILITIES_GROUP = "zeloo_agent.plugin_capabilities"
...
def discover_entrypoint_manifests() -> List["PluginManifest"]:
    ...
    ``zeloo_agent.plugin_capabilities`` group (``<plugin-id>.<capability-id>`` entries pointing at the same
...
def collect_directory_manifests() -> List[PluginManifest]:
    ...
    from zeloo_cli import plugins as _origin  # patched names resolve through the origin
    manifests: List[PluginManifest] = []
    ...
    user_dir = get_zeloo_home() / "plugins"
    logger.debug("Scanning user plugins: %s", user_dir)
    _scan("user", user_dir, "user")
    if _origin._env_enabled("ZELOO_ENABLE_PROJECT_PLUGINS"):
        project_dir = Path.cwd() / ".Zeloo" / "plugins"
        logger.debug("Scanning project plugins: %s", project_dir)
        _scan("project", project_dir, "project")
    else:
        logger.debug("Project plugins disabled (set ZELOO_ENABLE_PROJECT_PLUGINS=1 to enable)")
    return manifests
...
def gate_manifest(
    ...
    if names & LEGACY_RELAY_PLUGIN_KEYS:
        error = (
            "removed — Relay lifecycle is owned by Zeloo core; configure "
            f"{RELAY_PLUGINS_CONFIG_ENV} instead"
        )
        return _placeholder(error, logging.WARNING, "Refusing to load removed Zeloo Relay plugin '%s'; %s", error)
    if names & disabled:
    ...
    if manifest.source == "bundled":
        ...
        # Bundled platforms register LAZILY: eagerly importing ~20 heavy SDKs added seconds to every `Zeloo`
    ...
    if enabled is None or not names & enabled:
        return _placeholder(
            f"not enabled in config (run `Zeloo plugins enable {lookup_key}` to activate)", logging.DEBUG,
