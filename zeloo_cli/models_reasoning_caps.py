...
Split out of ``zeloo_cli.models``. OpenRouter and
...
``zeloo_cli.models`` — tests reset them there — and are read/written by attribute name.
...
# that must never block on HTTP — so without a disk copy, `Zeloo -p`, a cron job, or a freshly
...
def _reasoning_caps_disk_path() -> Path:
    ...
    return get_zeloo_home() / "cache" / "reasoning_caps.json"
...
def _fetch_reasoning_caps_catalog(url: str, timeout: float) -> Optional[Caps]:
    ...
    m = _origin()
    try:
        req = urllib.request.Request(url, headers={"Accept": "application/json", "User-Agent": m._ZELOO_USER_AGENT})
...
class _CapsSource:
    """One catalog's cache slots on ``zeloo_cli.models`` plus how to name its URL.
