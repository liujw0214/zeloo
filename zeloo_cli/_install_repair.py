...
Callers: ``_early_recovery.recover_if_needed`` (stdlib-only, runs BEFORE ``zeloo_cli.main``'s
third-party imports so a pending update completes while no native extension is mapped yet) and
``zeloo_cli.main_install_repair._recover_core_update_marker_locked`` (the post-import recovery path).
...
from zeloo_cli import _early_recovery as _er
...
def _is_termux_env(env: dict | None = None) -> bool:
    """Stdlib Termux probe (zeloo_cli.main's version lives behind imports)."""
...
def _stdout_to_stderr():
    ...
    ``Zeloo acp`` speaks JSON-RPC on stdout; an inherited-fd install child writing there would
...
def _venv_scripts_dir(root: Path) -> Path | None:
    """Project venv Scripts/bin dir, when present (zeloo_constants is stdlib-only)."""
    # zeloo_constants is stdlib-only, so the canonical layout helpers are safe to use from this
...
#: Zeloo root's ``bin``, next to uv.exe). Keep in lockstep with scripts/install.ps1.
_WINDOWS_BIN_LAUNCHERS = ("Zeloo", "Zeloo-acp")
...
def _default_zeloo_root() -> Path | None:
    """The DEFAULT Zeloo root (not ``get_zeloo_home()``, which under ``Zeloo -p <name>`` is
    ...
    try:
        return Path(get_default_zeloo_root())
    except Exception:
        return None
...
def ensure_windows_bin_launchers(
    ...
    r"""Re-stage the Windows ``Zeloo`` launchers when they vanish.

    On Windows, ``Zeloo`` resolves through launchers derived from the venv console scripts — never
    ...
    The canonical launcher home is the managed binary dir — the default Zeloo root's ``bin``
    (``%LOCALAPPDATA%\\Zeloo\\bin``, next to the managed uv) — which lives OUTSIDE the git checkout so no
    git operation can ever touch it. It is a per-machine dir shared by every profile: ``get_zeloo_home()``
    would point inside ``profiles\\<name>`` under ``Zeloo -p``, so the anchor here is
    :func:`zeloo_constants.get_default_zeloo_root`. See #83797.
    """
    if windows is None:
        windows = _is_windows()
    if not windows:
        return []
    root = Path(root)
    home = _default_zeloo_root()
    if home is None:
        return []

    targets: list[Path] = []
    # Runs at every zeloo_cli.main process start, so the healthy path must stay a few stat calls.
    ...
    if restored:
        ...
        with contextlib.suppress(OSError, ValueError):
            print("  ✓ Restored Zeloo launcher(s): " + ", ".join(restored), file=sys.stderr)
    return restored
...
def migrate_windows_bin_path(
    ...
    """One-time PATH migration to the ``ZELOO_HOME\\bin`` launcher layout (``Zeloo update`` tail).
    ...
    *read_user_path*/*write_user_path* are injectable for tests.
    ...
    if kept != entries:
        try:
            ...
            return False
        with contextlib.suppress(OSError, ValueError):
            print(f"  ✓ Zeloo launchers now resolve from {home_bin} "
...
def _quarantine_running_zeloo_exe(
    ...
    """Rename live Zeloo*.exe shims aside so the installer can rewrite them.
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    ...
    (#75584). Warnings land on stderr — this module runs in the early-recovery path and ``Zeloo acp``
...
def _run_install_cmd(cmd: list[str], *, env: dict | None, root: Path) -> None:
    ...
    failed: list[str] = []
    moved = _quarantine_running_zeloo_exe(scripts_dir, failed_out=failed) if scripts_dir else []
    if failed:
    ...
    try:
        ...
        # the shims renamed aside and `Zeloo` gone from PATH. Restore only renames back when the
