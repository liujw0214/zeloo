"""Zeloo webhook — manage dynamic webhook subscriptions from the CLI."""
...
def _subscriptions_path() -> Path:
    ...
    return get_zeloo_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: Zeloo gateway run
"""


def webhook_command(args):
    """Entry point for 'Zeloo webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)
    if not sub:
        print("Usage: Zeloo webhook {subscribe|list|remove|test}")
        print("Run 'Zeloo webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (Zeloo gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: Zeloo webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from Zeloo webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (Zeloo gateway run)")
