"""``Zeloo plugins validate`` — admission checks for a plugin directory.
...
scratch child process (with a throwaway ``ZELOO_HOME``), so a crashing or
...
_PROBE_TIMEOUT = 30
_PROBE_SENTINEL = "ZELOO_VALIDATE_JSON:"
...
def _requires_zeloo_spec_valid(spec: str) -> bool:
    """Strictly validate a ``requires_ZELOO`` spec.

    Unlike :func:`zeloo_cli.plugins._version_satisfies` (permissive at load
    ...
    """
    from zeloo_cli.plugins import _VERSION_COMPARATOR_RE, _version_tuple
...
def _check_requires_ZELOO(report: ValidationReport, manifest: dict) -> None:
    spec = str(manifest.get("requires_ZELOO") or "").strip()
    if not spec:
        report.add("requires_ZELOO", True, "not declared")
        return
    if _requires_zeloo_spec_valid(spec):
        report.add("requires_ZELOO", True, f"spec {spec!r} parses")
    else:
        report.add(
            "requires_ZELOO",
            False,
            f"requires_ZELOO spec {spec!r} does not parse "
...
# from Zeloo so a hostile plugin only sees a bare interpreter.
_PROBE_SCRIPT = r"""
...
def _run_capability_probe(plugin_dir: Path) -> Tuple[Optional[dict], str]:
    ...
    *error* is a human-readable failure description otherwise.
    """
    with tempfile.TemporaryDirectory(prefix="Zeloo-validate-") as scratch:
        env = dict(os.environ)
        env["ZELOO_HOME"] = scratch
        try:
...
def validate_plugin_dir(plugin_dir: Path) -> ValidationReport:
    ...
    _check_requires_ZELOO(report, manifest)
