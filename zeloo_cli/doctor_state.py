"""ZELOO_HOME state checks for Zeloo doctor: directories, memory files, state.db health, skills hub, memory provider, profiles.
...
from zeloo_cli.sizefmt import format_bytes as _human_bytes
from zeloo_state_common import FTS_STORAGE_VERSION
...
def _doctor_memory_config(zeloo_home: Path | None = None) -> dict:
    ...
    try:
        ...
        config_path = (zeloo_home if zeloo_home is not None else ZELOO_HOME) / "config.yaml"
...
def _render_state_db_stats(stats: dict, holders=None) -> list:
    ...
    if isinstance(deferral, dict):
        ...
        if deferral.get("futile"):
            ...
            lines.append(("warn", f"state.db FTS repair is blocked after {deferral.get('attempts') or '?'} deferral(s) "
                          ...
                          "'Zeloo sessions optimize-storage' with every holder stopped)"))
    ...
    if logical is not None and logical > STATE_DB_SIZE_WARN_BYTES:
        ...
        if stats.get("fts_rebuild_pending") or stale_trigram:
            detail += "; run 'Zeloo sessions optimize-storage' offline (with the gateway stopped) to compact FTS storage"
...
def _memory_store_flags(zeloo_home: Path) -> tuple:
    from tools.memory_tool import get_builtin_memory_store_flags
    return get_builtin_memory_store_flags({"memory": _doctor_memory_config(zeloo_home)})


@doctor_check()
def _check_directory_structure(should_fix: bool, f: Finding) -> None:
    """ZELOO_HOME, expected subdirs, SOUL.md, and the enabled built-in memory files."""
    from zeloo_cli.doctor import ZELOO_HOME, _DHH
    zeloo_home = ZELOO_HOME
    ensure_dir(f, should_fix, zeloo_home, f"{_DHH} directory exists", f"Created {_DHH} directory", f"{_DHH} not found")
    _memory_enabled, _user_profile_enabled = _memory_store_flags(zeloo_home)
    ...
    for subdir_name in ["cron", "sessions", "logs", "skills"] + (["memories"] if memory_on else []):
        ensure_dir(f, should_fix, zeloo_home / subdir_name, f"{_DHH}/{subdir_name}/ exists",
                   f"Created {_DHH}/{subdir_name}/", f"{_DHH}/{subdir_name}/ not found")
    # SOUL.md persona file
    soul_path = zeloo_home / "SOUL.md"
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Zeloo a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text("# Zeloo Agent Persona\n\n<!-- Edit this file to customize how Zeloo communicates. -->\n\n"
                                 "You are Zeloo, a helpful AI assistant.\n", encoding="utf-8")
    ...
    memories_dir = zeloo_home / "memories"
    if not memory_on:
...
_STATE_DB_REPAIRS = {
    "fts": ("Repaired state.db FTS write health",
            ...
            "state.db FTS write corruption — run 'Zeloo doctor --fix' (or 'Zeloo sessions repair') to rebuild the FTS index"),
    "schema": ("Repaired state.db schema ({count} sessions recovered)",
               ...
               "state.db schema malformed — run 'Zeloo doctor --fix' (or 'Zeloo sessions repair') to recover hidden sessions"),
}
_STATE_DB_STRUCTURAL_ISSUE = (
    ...
    "cannot repair it. Stop the gateway, then run 'Zeloo {profile_arg}sessions recover --source {db_path} "
    "--inspect-only' and, if it reports recoverable, 'Zeloo {profile_arg}sessions recover --source {db_path} "
...
def _repair_state_db(f: Finding, should_fix: bool, state_db_path: Path, kind: str) -> None:
    ...
    if kind == "structural":
        from zeloo_constants import profile_cli_selector
    ...
    from zeloo_state_repair import repair_state_db_schema
...
def _state_db_health(f: Finding, should_fix: bool, state_db_path: Path, _DHH: str) -> None:
    ...
    try:
        ...
        from zeloo_state_repair import _db_opens_cleanly, state_db_has_structural_damage
        ...
        from zeloo_state import is_malformed_db_error
...
def _state_db_stats(issues: list, state_db_path: Path) -> None:
    ...
    with warn_on_error("state.db stats unavailable ({e})", "", report=lambda t, _d: check_info(t)):
        from zeloo_state_dbfile import collect_state_db_stats, count_db_holders
        ...
        for _kind, _text, _detail in rows:
            ...
            if "auto_prune" in _detail:
                issues.append("state.db is large — enable sessions.auto_prune in config.yaml"
                              + (" and run 'Zeloo sessions optimize-storage' offline (gateway stopped)" if "optimize-storage" in _detail else ""))
...
def _state_db_wal(f: Finding, should_fix: bool, state_db_path: Path) -> None:
    ...
    with warn_on_error(""):
        ...
        if size > 50 * 1024 * 1024:  # 50 MB
            ...
            if not should_fix:
                return f.issues.append("Large WAL file — run 'Zeloo doctor --fix' to checkpoint")
            ...
            from zeloo_state_holders import live_writer_holds_db
            from zeloo_state_repair import _connect_repair_durable
...
def _check_state_db(should_fix: bool, f: Finding) -> None:
    ...
    from zeloo_cli.doctor import ZELOO_HOME, _DHH
    state_db_path = ZELOO_HOME / "state.db"
...
def _check_skills_hub(should_fix: bool, f: Finding) -> None:
    from zeloo_cli.doctor import ZELOO_HOME, _DHH
    hub_dir = ZELOO_HOME / "skills" / ".hub"
    if check_bool(hub_dir.exists(), "Skills Hub directory exists", ("Skills Hub directory not initialized", "(run: Zeloo skills list)")):
...
def _memory_provider_honcho(issues: list) -> None:
    ...
    if not cfg_path.exists():
        ...
        check_bool(hcfg.api_key or hcfg.base_url,
                   ...
                   ("Honcho config not found", "run: Zeloo memory setup"))
    elif not hcfg.enabled:
        check_info(f"Honcho disabled (set enabled: true in {cfg_path} to activate)")
    elif not (hcfg.api_key or hcfg.base_url):
        _fail_and_issue("Honcho API key or base URL not set", "run: Zeloo memory setup",
                        "No Honcho API key — run 'Zeloo memory setup'", issues)
    else:
...
def _memory_provider_mem0(issues: list) -> None:
    ...
    if mem0_cfg.get("api_key", ""):
        ...
        _fail_and_issue("Mem0 API key not set", "(set MEM0_API_KEY in .env or run Zeloo memory setup)",
...
def _memory_provider_generic(name: str) -> None:
    ...
    if _provider and _provider.is_available():
        ...
        check_warn(f"{name} configured but not available", "run: Zeloo memory status")
    else:
        check_warn(f"{name} plugin not found", "run: Zeloo memory setup")


@doctor_check()
def _check_memory_provider(should_fix: bool, f: Finding) -> None:
    from zeloo_cli.doctor import ZELOO_HOME
    name = _doctor_memory_config(ZELOO_HOME).get("provider", "")
    if not name:
...
def _check_profiles(should_fix: bool, f: Finding) -> None:
    from zeloo_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
    import re as _re
    ...
    if wrapper_dir.is_dir():
        for wrapper in wrapper_dir.iterdir():
            if not wrapper.is_file():
                continue
            with warn_on_error(""):
                _m = _re.search(r"Zeloo -p (\S+)", wrapper.read_text(encoding="utf-8"))
