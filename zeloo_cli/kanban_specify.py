...
``Zeloo kanban specify [task_id | --all]`` asks the auxiliary LLM for a
tightened title + concrete body for a Triage task, then flips it
``triage -> todo`` via ``kanban_db.specify_triage_task``.

Mirrors ``zeloo_cli/goals.py``: same aux-client pattern, same "empty config
...
ZELOO_KANBAN_SPECIFY_MAX_TOKENS = max(1500, env_int("ZELOO_KANBAN_SPECIFY_MAX_TOKENS", 6000))

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Zeloo Agent board.
...
def _profile_author(default: str = "specifier") -> str:
    """Mirror of ``zeloo_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return os.environ.get("ZELOO_PROFILE") or os.environ.get("USER") or default
...
def specify_task(
    ...
    *,
    ...
    raw, reason = _call_aux(
        ...
        max_tokens=ZELOO_KANBAN_SPECIFY_MAX_TOKENS, timeout=timeout or 120,
    )
    if raw is None:
        return SpecifyOutcome(task_id, False, reason)
    raw = raw.strip()
