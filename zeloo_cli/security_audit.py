"""On-demand supply-chain audit for Zeloo Agent installs.
...
def _discover_plugins(zeloo_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.Zeloo/plugins``. Plugins typically don't install
...
def _discover_components(
    *, skip_venv: bool = False, skip_plugins: bool = False, skip_mcp: bool = False, zeloo_home: Optional[Path] = None
) -> list[Component]:
    """Discover all scannable components across the enabled sources."""
    home = zeloo_home or Path(get_zeloo_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `Zeloo security audit`."""
    ...
    components = _discover_components(zeloo_home=home, **skips)
    ...
    try:
        findings = run_audit(zeloo_home=home, components=components)
