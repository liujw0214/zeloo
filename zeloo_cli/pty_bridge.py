"""PTY bridge for `Zeloo dashboard` chat tab.
...
``zeloo_cli.web_server``. POSIX-only: depends on ``fcntl``, ``termios`` and ``ptyprocess`` (native
...
# ui-tui/packages/Zeloo-ink/src/ink/termio/host.ts — keep the two in sync.
PTY_HOST_ENV = "ZELOO_PTY_HOST"
        ...
        if not _PTY_AVAILABLE:
            if sys.platform.startswith("win"):
                raise PtyUnavailableError("Pseudo-terminals are unavailable on this platform. "
                                          "Zeloo Agent supports Windows only via WSL.")
