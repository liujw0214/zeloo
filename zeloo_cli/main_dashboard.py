...
def _find_stale_dashboard_pids(*, exclude_pids: set[int] | None = None) -> list[int]:
    ...
    from zeloo_cli.dashboard_procs import _scan_dashboard_processes
...
def _parse_dashboard_runtime(command: str) -> tuple[str, str, int] | None:
    ...
    for candidate in ("dashboard", "serve"):
        patterns = (f"Zeloo {candidate}", f"zeloo_cli.main {candidate}", f"zeloo_cli/main.py {candidate}")
...
def _restart_managed_dashboard_service(reason: str, unit: str = _DASHBOARD_SYSTEMD_UNIT) -> bool:
    ...
    # User manager first (Zeloo installs Linux services in the user scope by
...
def _get_systemd_service_for_pid(pid: int) -> str | None:
    """The systemd service unit name *pid* belongs to (``Zeloo-serve.service``), or None.
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Respawn manually-started dashboards after ``Zeloo update``, detached, logging to
    ...
    log_path = get_zeloo_home() / "logs" / "dashboard-restart.log"
...
class _UpdateOutputStream:
    """stdout/stderr wrapper for ``Zeloo update``: mirrors to ``logs/update.log`` and, once the
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    try:
        ...
        # zeloo_cli.config.get_zeloo_home to simulate setup failure.
        from zeloo_cli.config import get_zeloo_home as _get_zeloo_home
        logs_dir = _get_zeloo_home() / "logs"
        logs_dir.mkdir(parents=True, exist_ok=True)
        log_file = open(logs_dir / "update.log", "a", buffering=1, encoding="utf-8")

        import datetime as _dt

        log_file.write(f"\n=== Zeloo update started {_dt.datetime.now().isoformat(timespec='seconds')} ===\n")
...
def _report_dashboard_status() -> int:
    ...
    if not live:
        print("No Zeloo dashboard or serve processes running.")
        return 0

    print(f"{len(live)} Zeloo dashboard/serve process(es) running:")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    try:
        from zeloo_cli.web_server import should_require_dashboard_auth
    ...
    print("    [2] OAuth via Nous Portal (run `Zeloo dashboard register`)\n    [3] Cancel\n")
    ...
    if choice == "2":
        ...
        print(
            ...
            "ZELOO_DASHBOARD_OAUTH_CLIENT_ID into ~/.Zeloo/.env for you.\n"
    ...
    try:
        ...
        from zeloo_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
        cfg = load_config()
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from zeloo_cli.windows_ssh_runtime import read_token
    ...
    # The Desktop client writes the token under the account's $HOME/.Zeloo/
    # desktop-ssh, independent of ZELOO_HOME and the active profile. Anchor
    # validation there, NOT get_zeloo_home(): a non-default profile or a Docker
    # /opt/data root re-homes get_zeloo_home() and would reject every token.
    # See #69551.
    token_root = Path.home() / ".Zeloo" / "desktop-ssh"
    try:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    A standalone ``Zeloo dashboard`` inheriting that ``ZELOO_WEB_DIST`` would
...
def _route_named_profile_dashboard(
    ...
    (ZELOO_DESKTOP=1) stay per-profile. Returns normally when no routing applies.
    """
    try:
    ...
    if (
        ...
        or os.environ.get("ZELOO_DESKTOP") == "1"
    ):
        return
    ...
    reexec_argv = [
        sys.executable, "-m", "zeloo_cli.main",
        "-p", "default",
    ...
    # ZELOO_HOME is pinned to the machine root below — the factory must not
    # re-inject a profile home.
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=False)
    # Pin the child to the machine ROOT, resolved explicitly rather than by
    # dropping ZELOO_HOME: in the Docker layout the root is /opt/data, and an
    # unset ZELOO_HOME would fall back to $HOME/.Zeloo = /opt/data/.Zeloo — an
    # empty auto-seeded home with only the default profile and no install stamp.
    # get_default_zeloo_root() strips a trailing profiles/<name> for both layouts.
    try:
        from zeloo_constants import get_default_zeloo_root
        env["ZELOO_HOME"] = str(get_default_zeloo_root())
    except Exception:
        env.pop("ZELOO_HOME", None)  # prior behaviour rather than blocking the reroute
...
def _resolve_dashboard_web_dist(args, _headless_backend: bool) -> None:
    ...
    ``serve`` sets ZELOO_SERVE_HEADLESS so mount_spa() stays off. Otherwise build
    unless ZELOO_WEB_DIST / --skip-build promise a dist — then verify index.html
    (else the server serves 404s). --skip-build on the default location gets ONE
    recovery build; a caller-managed ZELOO_WEB_DIST can't be populated.
    """
    from zeloo_cli.main import PROJECT_ROOT
    from zeloo_cli.main_web_build import _build_web_ui
    skip_build = getattr(args, "skip_build", False)
    if _headless_backend:
        os.environ["ZELOO_SERVE_HEADLESS"] = "1"  # set before web_server import
    elif "ZELOO_WEB_DIST" not in os.environ and not skip_build:
        ...
        _dist_root = (
            ...
            Path(os.environ["ZELOO_WEB_DIST"])
            if "ZELOO_WEB_DIST" in os.environ
            else PROJECT_ROOT / "zeloo_cli" / "web_dist"
        )
        if not (_dist_root / "index.html").exists():
            # Only the default dist location is recoverable (desktop launches with
            # --build-mode skip after a wipe of web_dist); a custom ZELOO_WEB_DIST
            ...
            _recoverable = "ZELOO_WEB_DIST" not in os.environ
            if _recoverable:
        ...
        # ZELOO_WEB_DIST without --skip-build: the env var points at a
        # caller-managed dist, so validate it like the --skip-build branch.
        # ZELOO_WEB_DIST is set without --skip-build: the build is skipped (the env var points at a
        ...
        # path).
        _dist_root = Path(os.environ["ZELOO_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ ZELOO_WEB_DIST is set but no web dist found at: {_dist_root}")
            print(_PRE_BUILD_HINT)
            print("  Or unset ZELOO_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # web_server reads ZELOO_WEB_DIST raw at import (no expanduser), so a
        # validated "~/dist" would otherwise pass here and still 404 there.
        os.environ["ZELOO_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from ZELOO_WEB_DIST: {_dist_root}")
