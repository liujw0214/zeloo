...
from zeloo_cli.dashboard_auth import list_session_providers
...
from zeloo_cli.dashboard_auth.prefix import prefix_from_request
from zeloo_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
from zeloo_cli.dashboard_auth.request_utils import (
...
_GATE_PUBLIC_PREFIXES: tuple[str, ...] = (
    ...
    # Dashboard plugin static assets — loaded by the SPA via <script src> and
    # <link href>, which cannot attach auth headers. The serving endpoint
    # already constrains by suffix allowlist and 404s unknown paths.
    "/dashboard-plugins/")
...
def _safe_next_target(request: Request) -> str:
    ...
    SPA's ``sessionStorage["Zeloo.lastLocation"]``."""
...
_PLUGIN_COMPAT_LAZY = {
    'DashboardAuthProvider': ('zeloo_cli.dashboard_auth.base', 'DashboardAuthProvider'),
}
