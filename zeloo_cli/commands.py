"""Slash command registry for the Zeloo CLI and gateway.
...
from zeloo_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Key in ``zeloo_cli.slash_exec.EXECUTORS`` (a string, not a callable: keeps this module
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of Zeloo config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Zeloo works on across turns until achieved", "Session",
    ...
    CommandDef("plan", "Write a markdown implementation plan to .Zeloo/plans/ without executing anything", "Session",
               args_hint="[task]"),
    ...
    CommandDef("wake", "Toggle the 'Hey Zeloo' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]", subcommands=("on", "off", "status")),
    CommandDef("busy", "Control how messages behave while Zeloo is working", "Configuration",
    ...
    CommandDef("reload-skills", "Re-scan ~/.Zeloo/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Zeloo Agent to the latest version", "Info",
               busy_policy="dispatch", desktop="terminal"),
    CommandDef("version", "Show Zeloo Agent version", "Info", aliases=("v",),
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``Zeloo skills list`` and the agent's ``/skill-name`` dispatch but
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'discord_skill_commands_by_category': ('zeloo_cli.commands_platforms', 'discord_skill_commands_by_category'),
    'slack_app_manifest': ('zeloo_cli.commands_platforms', 'slack_app_manifest'),
    'slack_native_slashes': ('zeloo_cli.commands_platforms', 'slack_native_slashes'),
    'slack_subcommand_map': ('zeloo_cli.commands_platforms', 'slack_subcommand_map'),
    'telegram_bot_commands': ('zeloo_cli.commands_platforms', 'telegram_bot_commands'),
    'telegram_menu_commands': ('zeloo_cli.commands_platforms', 'telegram_menu_commands'),
    'telegram_menu_max_commands': ('zeloo_cli.commands_platforms', 'telegram_menu_max_commands'),
}
