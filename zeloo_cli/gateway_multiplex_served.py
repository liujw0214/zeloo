...
guess (``Zeloo -p coder ...`` loads coder's ``.env``, so an env-only opt-in on the default profile
...
def live_default_gateway_pid() -> Optional[int]:
    ...
    rec = _read_pid_record(get_default_zeloo_root() / "gateway.pid")
...
def recorded_served_profiles(default_root: Optional[Path] = None) -> Optional[list[str]]:
    ...
    if live_default_gateway_pid() is None:
        return None
    runtime = read_runtime_status((default_root or get_default_zeloo_root()) / "gateway_state.json")
