"""Directory initialization and storage diagnostics for the active Zeloo home."""
...
def _ensure_directory(path: Path, *, create: bool, secure: bool) -> None:
    from zeloo_cli.config import _secure_dir
    ...
    try:
        ...
        raise HomeInitializationError(
            f"Cannot initialize Zeloo directory {path}: {exc}. "
            ...
            "Zeloo has not replaced the link or created its missing target."
        ) from exc


def initialize_home(home: Path, subdirs: tuple[str, ...], ensured: set[str]) -> None:
    from zeloo_cli.config import _ensure_default_soul_md, is_managed
    ...
    try:
        ...
        try:
            ...
            raise HomeInitializationError(
                f"Cannot initialize Zeloo home {home}: {exc}. "
...
def config_load_issue(exc: Exception):
    from zeloo_cli.config import ConfigIssue

    if isinstance(exc, (HomeInitializationError, OSError)):
        return ConfigIssue(
            "error", f"Zeloo storage is unavailable: {exc}",
            "Check the reported path, link target, mount and permissions; keep config.yaml unchanged.",
        )
    return ConfigIssue("error", "Could not load config.yaml", "Run 'Zeloo setup' to create a valid config")
