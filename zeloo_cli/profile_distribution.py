"""Profile distributions — shareable, packaged Zeloo profiles via git.
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            zeloo_requires=_str(data, "zeloo_requires"), author=_str(data, "author"),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        optional = (
            ("description", self.description), ("zeloo_requires", self.zeloo_requires),
...
def check_zeloo_requires(spec: str, current_version: str) -> None:
    ...
    if not _VERSION_OPS[op](_parse_semver(current_version), _parse_semver(target)):
        raise DistributionError(f"This distribution requires Zeloo {op}{target}, but you have {current_version}.")
    ...
    *workdir* (``.git`` removed); a local directory is used in place."""
    ...
    if _looks_like_git_url(src_str):
        ...
        missing = (
            ...
            "This repository is not a Zeloo profile distribution."
        )
...
def plan_install(source: str, workdir: Path, override_name: Optional[str] = None) -> InstallPlan:
    ...
    from zeloo_cli.profiles import _canon_valid, get_profile_dir
    from zeloo_cli import __version__ as zeloo_version
    staged, provenance = _stage_source(source, workdir)
    _reject_distribution_symlinks(staged)
    manifest = read_manifest(staged)
    if manifest is None:
        raise DistributionError(
            f"No {MANIFEST_FILENAME} found at the distribution root — this source is not a Zeloo distribution."
        )
    check_zeloo_requires(manifest.zeloo_requires, zeloo_version)  # fail fast
    canon = _canon_valid(override_name or manifest.name)
    if canon == "default":
        raise DistributionError(
            "Cannot install a distribution as 'default' — that is the built-in "
            "root profile (~/.Zeloo).  Pass --name <name> to install under a new profile."
        )
    manifest.name = canon
...
def _bootstrap_user_dirs(target: Path) -> None:
    ...
    from zeloo_cli.profiles import _PROFILE_DIRS
...
def install_distribution(
    ...
    from zeloo_cli.profiles import check_alias_collision, create_wrapper_script
    with tempfile.TemporaryDirectory(prefix="zeloo_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `Zeloo profile update` to upgrade in place, or pass --force to overwrite."
            )
...
def _existing_profile(profile_name: str) -> Tuple[str, Path]:
    ...
    from zeloo_cli.profiles import _existing_profile_dir
...
def update_distribution(profile_name: str, force_config: bool = False) -> InstallPlan:
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`Zeloo profile install <source> --name {canon} --force`."
        )
    with tempfile.TemporaryDirectory(prefix="zeloo_dist_update_") as tmp:
