...
# to the checkout (NOT in ZELOO_HOME): __pycache__ is per-checkout state shared
# by every profile.
...
def _record_bytecode_fingerprint() -> None:
    ...
    from zeloo_cli.main import PROJECT_ROOT, _read_git_revision_fingerprint
    try:
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    Update-time clears can't close the stale-bytecode class: ``Zeloo update`` runs
    ...
    pull inside ``Zeloo update``, a manual ``git pull``, a ZIP update, a file-sync restore) while
    ...
    """
    from zeloo_cli.main import PROJECT_ROOT, _clear_bytecode_cache, _read_git_revision_fingerprint
    try:
        fingerprint = _read_git_revision_fingerprint(PROJECT_ROOT)
        if not fingerprint:
...
def _web_dist_dir(web_dir: Path) -> Path:
    """Vite outputs to ``zeloo_cli/web_dist/`` (vite.config.ts outDir), NOT ``web/dist/``."""
    return _web_project_root(web_dir) / "zeloo_cli" / "web_dist"
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    Content hash, NOT mtime: ``git checkout`` / ``Zeloo update`` rewrite source
...
def _web_ui_stamp_path() -> Path:
    """Path of the web UI build stamp under $ZELOO_HOME."""
    from zeloo_constants import get_zeloo_home
    return get_zeloo_home() / "web-ui-build-stamp.json"
...
def _run_with_idle_timeout(
    ...
    half-state with the ``Zeloo`` launcher present but ``zeloo_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    """``PYTHON=`` env for node-gyp on NixOS (bare PATH lookup fails outside nix-shell): the Zeloo
...
def _run_npm_install_deterministic(
    ...
    run_env["PATH"] = with_zeloo_node_path(run_env)["PATH"]
...
def _web_npm_install_context(web_dir: Path) -> tuple[Path, tuple[str, ...]]:
    ...
    closure as ``Zeloo update``'s ``_update_node_dependencies()`` (ui-tui + web +
    ...
    # When running from the workspace root, this must name the SAME closure as `Zeloo update`'s
...
def _report_web_build_failure(step: str, result: subprocess.CompletedProcess, *, fatal: bool) -> bool:
    ...
    _console_print(f"  {'✗' if fatal else '⚠'} Web UI {step} failed" + ("" if fatal else " (Zeloo web will not be available)"))
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    soft warning (used by ``Zeloo web``). Returns True when the build succeeded
    ...
    if not npm:
        ...
        return not fatal
    build_env = _npm_lifecycle_env(with_zeloo_node_path())
