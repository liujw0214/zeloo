"""Migrate Zeloo MCP server config and Codex's installed curated plugins into ~/.codex/config.toml.
"""
...
MIGRATION_END_MARKER = "# end Zeloo-agent managed section"
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Zeloo config.")
...
# Zeloo MCP keys codex understands (transport stdio/http, timeouts, general). Any other key is
# dropped with a warning: ``sampling`` has no codex equivalent; the rest are unknown Zeloo keys.
_KNOWN_ZELOO_KEYS = {
...
# (Zeloo key, codex key, skip note) — timeouts are emitted as floats or skipped when non-numeric.
_TIMEOUT_KEYS = (
...
def _translate_one_server(name: str, zeloo_cfg: dict) -> tuple[Optional[dict], list[str]]:
    """Translate one Zeloo MCP server config to codex's inline-table dict.

    Returns ``(codex_entry, skipped_keys)``; ``codex_entry`` is None when the config is unusable.
    stdio (``command``) wins over ``url`` when both are set. Zeloo' ``transport: sse`` hint is
    informational only — codex auto-negotiates. ``enabled`` is emitted only when explicitly false
    (codex defaults to true).
    """
    if not isinstance(zeloo_cfg, dict):
        return None, []
    skipped: list[str] = []
    out: dict[str, Any] = {}
    if zeloo_cfg.get("command"):
        if zeloo_cfg.get("url"):
            skipped.append("url (both command and url set; preferring stdio)")
        out["command"] = str(zeloo_cfg["command"])
        if zeloo_cfg.get("args"):
            out["args"] = [str(a) for a in zeloo_cfg["args"]]
        if zeloo_cfg.get("env"):
            out["env"] = _str_map(zeloo_cfg["env"])
        if zeloo_cfg.get("cwd"):
            out["cwd"] = str(zeloo_cfg["cwd"])
    elif zeloo_cfg.get("url"):
        out["url"] = str(zeloo_cfg["url"])
        if zeloo_cfg.get("headers"):
            out["http_headers"] = _str_map(zeloo_cfg["headers"])
        if zeloo_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]
    for zeloo_key, codex_key, note in _TIMEOUT_KEYS:
        if zeloo_key in zeloo_cfg:
            try:
                out[codex_key] = float(zeloo_cfg[zeloo_key])
            except (TypeError, ValueError):
                skipped.append(note)
    if zeloo_cfg.get("enabled") is False:
        out["enabled"] = False
    for key in zeloo_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            skipped.append(f"{key} (no codex equivalent)")
        elif key not in _KNOWN_ZELOO_KEYS:
            skipped.append(f"{key} (unknown Zeloo key)")
    return out, skipped
...
# env-var passthrough (ZELOO_HOME, PYTHONPATH) could carry one in pathological cases.
_TOML_ESCAPES = (
    ("\\", "\\\\"), ('"', '\\"'), ("\b", "\\b"), ("\t", "\\t"),
    ("\n", "\\n"), ("\f", "\\f"), ("\r", "\\r"))
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out += ["# (no MCP servers, plugins, or permissions configured by Zeloo)", MIGRATION_END_MARKER]
        return "\n".join(out) + "\n"
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(codex_home=str(codex_home) if codex_home else None) as client:
            client.initialize(client_name="Zeloo-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    Such dirs are reaped between sessions; a ZELOO_HOME pointing there burned into
...
def _build_zeloo_tools_mcp_entry() -> dict:
    """Codex stdio entry launching Zeloo' own tool surface as an MCP server (browser/web/
    delegate_task/vision/memory/skills call-backs).

    ZELOO_HOME passes through only IF SET, read from os.environ (not get_zeloo_home()): when
    unset the codex subprocess must inherit its launcher's runtime ZELOO_HOME (systemd, gateway,
    kanban), not a migrate-time default burned into config.toml that pins the wrong profile. The
    pytest-tempdir guard keeps a sibling test's monkeypatched ZELOO_HOME out of the user's real
    config. PYTHONPATH passes through so a worktree-launched Zeloo finds the branch's modules.
    """
    import sys
    env: dict[str, str] = {}
    # ZELOO_HOME passes through IF SET so the MCP subprocess sees the same config / auth / sessions DB as
    # the parent CLI. Read from os.environ (not get_zeloo_home()) on purpose: when the env var is unset we
    # want codex's subprocess to inherit whatever ZELOO_HOME its launcher sets at runtime (systemd unit,
    # gateway, kanban dispatcher, custom shell), rather than burning the migrate-time resolved default into
    # config.toml — that would override the launcher's ZELOO_HOME and pin the subprocess to the wrong
    # profile. The pytest-tempdir guard below catches the issue #26250 Bug C scenario: a sibling test's
    # monkeypatch.setenv("ZELOO_HOME", tmp_path) would otherwise leak a transient pytest tempdir into the
    # user's real ~/.codex/config.toml and silently brick codex once the tempdir is GC'd.
    zeloo_home = os.environ.get("ZELOO_HOME") or ""
    if zeloo_home and not _looks_like_test_tempdir(zeloo_home):
        env["ZELOO_HOME"] = zeloo_home
    if os.environ.get("PYTHONPATH"):
        env["PYTHONPATH"] = os.environ["PYTHONPATH"]
    # Quiet mode + redaction defaults so the MCP wire stays clean.
    env["ZELOO_QUIET"] = "1"
    env["ZELOO_REDACT_SECRETS"] = env.get("ZELOO_REDACT_SECRETS", "true")
    return {
...
def migrate(
    zeloo_config: dict, *, codex_home: Optional[Path] = None, dry_run: bool = False,
    discover_plugins: bool = True, default_permission_profile: Optional[str] = ":workspace",
    expose_zeloo_tools: bool = True) -> MigrationReport:
    """Translate Zeloo mcp_servers config + Codex curated plugins into ~/.codex/config.toml.
    ...
    approval prompt on every write. ``expose_zeloo_tools`` registers Zeloo' own tool surface
    (agent/transports/zeloo_tools_mcp_server.py, launched on demand by codex over stdio) as an MCP
    ...
    zeloo_servers = (zeloo_config or {}).get("mcp_servers") or {}
    if not isinstance(zeloo_servers, dict):
        report.errors.append("mcp_servers in Zeloo config is not a dict; cannot migrate.")
        return report
    translated: dict[str, dict] = {}
    for raw_name, cfg in zeloo_servers.items():
        name = str(raw_name)
    ...
    if expose_zeloo_tools:
        translated["Zeloo-tools"] = _build_zeloo_tools_mcp_entry()
        if "Zeloo-tools" not in report.migrated:
            report.migrated.append("Zeloo-tools")
