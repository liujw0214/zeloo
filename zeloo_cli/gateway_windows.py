...
_TASK_NAME_DEFAULT = "ZELOO_Gateway"
_TASK_DESCRIPTION = "Zeloo Agent Gateway - Messaging Platform Integration"
_TASK_LOGON_DELAY = "PT30S"
_TASK_RESTART_INTERVAL = "PT1M"
_TASK_RESTART_COUNT = 999

_GATEWAY_ENV = (("PYTHONIOENCODING", "utf-8"), ("ZELOO_GATEWAY_DETACHED", "1"), ("ZELOO_SUPERVISED_CHILD", "1"))
...
def _preserve_zeloo_home_path(path: str | Path) -> str:
    r"""Render Zeloo-owned paths under the configured ZELOO_HOME spelling.

    ``%LOCALAPPDATA%\Zeloo`` may be a symlink/junction to another drive; launcher files must not
    bake in the resolved target for paths under ZELOO_HOME.
    """
    candidate = Path(path)
    try:
        home = _zeloo_home()
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Zeloo profile."""
    from zeloo_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "zeloo_cli.main", *_current_profile_cli_args(), "gateway", command, *(extra_args or [])]
...
def _launch_elevated_install(force: bool = False, *, start_now: bool | None = None, start_on_login: bool | None = None) -> bool:
    ...
    overrides = {"ZELOO_GATEWAY_ELEVATED_HANDOFF": "1"}
    extra_args = ["--elevated-handoff"]
    if force:
        extra_args.append("--force")
    for choice, env_key, flag in (
        (start_now, "ZELOO_GATEWAY_INSTALL_START_NOW", "start-now"),
        (start_on_login, "ZELOO_GATEWAY_INSTALL_START_ON_LOGIN", "start-on-login"),
    ):
...
def get_task_name() -> str:
    ...
    _assert_windows()
    from zeloo_cli.gateway import _profile_suffix  # local: avoids circular init during boot
...
def get_task_script_path() -> Path:
    """The generated ``gateway.cmd`` wrapper under ``<ZELOO_HOME>/gateway-service/`` (per-profile
    installs stay self-contained); the VBS launcher lives beside it."""
    _assert_windows()
    script_dir = _zeloo_home() / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    """Stable cwd for detached/startup runs: anchor at ZELOO_HOME when it exists (mirrors the POSIX
...
def _gateway_run_argv(python_exe: str, profile_arg: str) -> list[str]:
    """``python -m zeloo_cli.main [--profile X] gateway run`` — shared by every launcher renderer."""
    argv = [python_exe, "-m", "zeloo_cli.main"]
    if profile_arg:
...
def _launcher_settings() -> tuple[str, str, str, str]:
    """Return (python_path, working_dir, zeloo_home, profile_arg) for generated launchers."""
    from zeloo_cli.gateway import PROJECT_ROOT, _profile_arg, get_python_path  # avoid circular init

    zeloo_home = str(_zeloo_home())
    return (
        _preserve_zeloo_home_path(get_python_path()),
        _stable_gateway_working_dir(PROJECT_ROOT),
        zeloo_home,
        _profile_arg(zeloo_home),
    )


def _launcher_pythonpath_entries(extra_pythonpath: list[str]) -> list[str]:
    return [
        _preserve_zeloo_home_path(Path(__file__).resolve().parent.parent),
        *[_preserve_zeloo_home_path(entry) for entry in extra_pythonpath],
    ]


def _build_gateway_cmd_script(python_path: str, working_dir: str, zeloo_home: str, profile_arg: str) -> str:
    ...
    lines = [
        ...
        f'set "ZELOO_HOME={zeloo_home}"',
        *[f'set "{k}={v}"' for k, v in _GATEWAY_ENV],
        # VIRTUAL_ENV lets the gateway's own python detection find the venv.
        f'set "VIRTUAL_ENV={_preserve_zeloo_home_path(venv_dir)}"',
...
def _build_gateway_vbs_script(python_path: str, working_dir: str, zeloo_home: str, profile_arg: str) -> str:
    ...
    lines = [
        ...
        f"env.Item({q('ZELOO_HOME')}) = {q(zeloo_home)}",
        *[f"env.Item({q(k)}) = {q(v)}" for k, v in _GATEWAY_ENV],
        f"env.Item({q('VIRTUAL_ENV')}) = {q(_preserve_zeloo_home_path(venv_dir))}",
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    _assert_windows()
    from zeloo_cli.gateway import PROJECT_ROOT

    python_path, working_dir, zeloo_home, profile_arg = _launcher_settings()
    python_exe, venv_dir, extra_pythonpath = _resolve_detached_python(python_path)
    env_overlay = {"ZELOO_HOME": zeloo_home, **dict(_GATEWAY_ENV), "VIRTUAL_ENV": _preserve_zeloo_home_path(venv_dir)}
    _prepend_pythonpath(env_overlay, [_preserve_zeloo_home_path(p) for p in (PROJECT_ROOT, *extra_pythonpath)])
...
def windowless_gateway_restart_spec(run_argv: list[str]) -> tuple[list[str], str, dict[str, str]]:
    ...
    interpreter via ``_resolve_detached_python`` and supplies the stable cwd + env overlay (ZELOO_HOME,
    ...
    try:
        zeloo_home = str(_zeloo_home().resolve())
    except Exception:
        zeloo_home = ""
    env_overlay: dict[str, str] = {"PYTHONIOENCODING": "utf-8", "ZELOO_GATEWAY_DETACHED": "1", "VIRTUAL_ENV": str(venv_dir)}
    if zeloo_home:
        env_overlay["ZELOO_HOME"] = zeloo_home
...
def _prompt_install_choices(start_now: bool | None = None, start_on_login: bool | None = None) -> tuple[bool, bool]:
    ...
    if start_now is None:
        start_now = _install_choice_from_env("ZELOO_GATEWAY_INSTALL_START_NOW")
    if start_on_login is None:
        start_on_login = _install_choice_from_env("ZELOO_GATEWAY_INSTALL_START_ON_LOGIN")
    if start_now is not None and start_on_login is not None:
        return start_now, start_on_login

    from zeloo_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    if running_pids or start_now:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"Zeloo {profile_arg} gateway start" if profile_arg else "Zeloo gateway start"
...
def _offer_elevated_install(headline: str, force: bool, start_now: bool, start_on_login: bool) -> bool:
    ...
    if prompt_yes_no("  Open the UAC prompt now?", False):
        if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
            print("✓ Launched elevated Zeloo gateway install prompt.")
            if start_now:
                print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
            else:
                print("  Approve the Windows UAC prompt, then run: Zeloo gateway status")
            return True
...
def install(
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: Zeloo gateway start")
        return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: Zeloo gateway start")
        _print_next_steps()
        return
...
def _start_attestation_path() -> Path:
    return _zeloo_home().joinpath(*_START_ATTESTATION_RELATIVE)
...
def _attested_pid_exited_cleanly(pid: int) -> bool:
    ...
    try:
        ...
        data = json.loads(get_lifecycle_sentinel_path(_zeloo_home()).read_text(encoding="utf-8"))
    except Exception:
        return False
...
def check_start_attestation(current_pids: list[int] | None = None) -> str | None:
    ...
    lines = [
        ...
        "shutdown record.",
        "  This usually means the shell that ran `Zeloo gateway start` was inside "
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        print(f"  Check the log for startup errors:\n    type {_zeloo_home()}\\logs\\gateway.log\n    type {_zeloo_home()}\\logs\\gateway-stdio.log")
...
def _print_next_steps() -> None:
    ...
    print(f"  type {_zeloo_home()}\\logs\\gateway.log       # View logs")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_gateway_command("uninstall"):
                    print("✓ Launched elevated Zeloo gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: Zeloo gateway status")
                    return
...
def start() -> None:
    ...
    if not is_task_registered() and not is_startup_entry_installed():
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: Zeloo gateway install")
            return
        install(force=False)
        if not is_task_registered() and not is_startup_entry_installed():
            print("⚠ Gateway install did not complete in this process.")
            print("  If a UAC prompt opened, approve it, then run: Zeloo gateway start")
            return
...
def restart() -> None:
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `Zeloo gateway status`."
        )
