"""Bounded product contract for the first Zeloo shared-metrics slice."""
...
SCHEMA_KEY = "Zeloo.metrics.schema_version"
SCHEMA_VERSION = "Zeloo.metrics.event.v2"
MODEL_CALL_SCOPE = "Zeloo.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "Zeloo.task_run"
TOOL_CALL_SCOPE = "Zeloo.tool_call"
CLIENT_ACTIVE_MARK = "Zeloo.client.active"
TOOL_APPROVAL_MARK = "Zeloo.tool_approval"
SKILL_LIFECYCLE_MARK = "Zeloo.skill.lifecycle"
SKILL_LOAD_MARK = "Zeloo.skill.load"
SUBSCRIBER_NAME = "Zeloo.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "Zeloo.client.active"
LEGACY_MODEL_CALL_METRIC = "Zeloo.model_call.count"
MODEL_ROUTE_METRIC = "Zeloo.model_route.count"
TASK_STARTED_METRIC = "Zeloo.task_run.started"
TASK_FINISHED_METRIC = "Zeloo.task_run.finished"
TOOL_CALL_METRIC = "Zeloo.tool_call.count"
TOOL_APPROVAL_METRIC = "Zeloo.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "Zeloo.skill.lifecycle.count"
SKILL_LOAD_METRIC = "Zeloo.skill.load.count"
...
CLIENT_RESOURCE_KEYS = frozenset({"architecture", "zeloo_version", "install_method", "os_family"})
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Zeloo installation method."""
    normalized = _norm(value)
    return _allowlisted("nixos" if normalized == "nix" else normalized, CLIENT_INSTALL_METHODS)


def client_resource(
    zeloo_version: Any, *, os_name: Any, architecture: Any, install_method: Any
) -> dict[str, str]:
    """Build the bounded client resource attached to aggregate packages."""
    version = str(zeloo_version or "").strip()
    return {
        "architecture": client_architecture(architecture),
        "zeloo_version": version if 0 < len(version) <= 64 else "unknown",
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("zeloo_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Zeloo logical scope."""
    metadata = _relay_metadata(
        event, RUNTIME_SCHEMA_KEY, RUNTIME_SCHEMA_VERSION, "Zeloo.call_role"
    )
    call_role = (metadata or {}).get("Zeloo.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from zeloo_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Zeloo terminal state to bounded (outcome, end_reason, termination)."""
...
_TOOL_CATEGORY_PREFIXES = (
    ...
    (("discord", "email", "feishu", "Zeloo-yuanbao", "slack", "sms"), "communication"),
)


def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Zeloo registry toolset metadata to a low-cardinality category."""
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Zeloo tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Zeloo."""
