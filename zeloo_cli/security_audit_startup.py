...
logger = logging.getLogger("Zeloo.security_audit")
...
def _running_as_root() -> Optional[str]:
    return None if not _is_root() else (
        ...
        "prompt-injection or exposed endpoint is a full host compromise. Run Zeloo as an unprivileged user "
...
def _in_container() -> bool:
    ...
    if os.path.exists("/.dockerenv"):
        return True
    if os.environ.get("ZELOO_DESKTOP_CHILD_PID"):
...
def _container_no_volume_mount(zeloo_home: Optional[Path]) -> Optional[str]:
    ...
    if _path_is_mounted(zeloo_home):  # any error propagates to run_security_audit (= no finding)
        return None
    return (f"Running in a container but the data dir ({zeloo_home}) is NOT on a persistent volume mount — "
            "sessions, memory, skills, and API keys are ephemeral and lost on container restart. Mount a host "
            "volume over the ZELOO_HOME data directory.")
...
def run_security_audit(*, zeloo_home: Optional[Path] = None, config: Optional[dict] = None) -> list[str]:
    ...
    """
    findings: list[str] = []
    for check in (_running_as_root, _ssh_password_auth_enabled, lambda: _container_no_volume_mount(zeloo_home),
...
def log_startup_security_warnings(
    *, zeloo_home: Optional[Path] = None, config: Optional[dict] = None, force: bool = False
) -> list[str]:
    ...
    if _AUDIT_RAN and not force:
        return []
    _AUDIT_RAN = True
    findings = run_security_audit(zeloo_home=zeloo_home, config=config)
    if findings:
