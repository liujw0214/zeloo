...
# the CLI's copy/exit/clear bindings also claim ``a-c``/``a-d``/``a-l`` (Zeloo-ink reports Alt as
...
# ``a-l`` via the action-modifier lookup, and Zeloo-ink reports Alt as ``key.meta`` on many terminals.
...
def normalize_voice_record_key_for_prompt_toolkit(raw: Any) -> str:
    ...
    Zeloo-ink rejects them; a bare key is refused by the TUI parser.
...
def _debug(msg: str) -> None:
    """ZELOO_VOICE_DEBUG=1 breadcrumb on stderr (the TUI gateway shows it as a gateway.stderr
    ...
    if os.environ.get("ZELOO_VOICE_DEBUG", "").strip() == "1":
...
def _speak_whole_file(text: str) -> None:
    ...
    os.makedirs(os.path.join(tempfile.gettempdir(), "zeloo_voice"), exist_ok=True)
    mp3_path = os.path.join(tempfile.gettempdir(), "zeloo_voice", f"tts_{time.strftime('%Y%m%d_%H%M%S')}.mp3")
