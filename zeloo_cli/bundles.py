"""Implementation of the ``Zeloo bundles`` CLI subcommand."""
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  Zeloo bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``Zeloo bundles`` argparse tree (called from zeloo_cli/main.py, which owns the
    top-level subparser)."""
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `Zeloo bundles list`")
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``Zeloo bundles <subcommand>`` to the right handler."""
