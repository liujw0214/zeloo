...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or Zeloo -c.",
    ...
    "Ctrl+Z suspends Zeloo to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "Zeloo -c resumes your most recent CLI session. Zeloo -c \"project name\" resumes by title.",
    "Zeloo -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "Zeloo -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "Zeloo chat -t web,terminal enables only specific toolsets for a focused session.",
    "Zeloo chat -s github-pr-workflow preloads a skill at launch.",
    "Zeloo chat -q \"query\" runs a single non-interactive query and exits.",
    "Zeloo chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "Zeloo chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "Zeloo --yolo bypasses all dangerous command approval prompts for the entire session.",
    "Zeloo chat --source telegram tags the session for filtering in Zeloo sessions list.",
    "Zeloo -p work chat runs under a specific profile without changing your default.",
    # --- CLI Subcommands ---
    "Zeloo doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "Zeloo dump outputs a compact setup summary — great for bug reports.",
    "Zeloo config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "Zeloo config edit opens config.yaml in your default editor.",
    "Zeloo config check scans for missing or stale configuration options.",
    "Zeloo sessions browse opens an interactive session picker with search.",
    "Zeloo sessions stats shows session counts by platform and database size.",
    "Zeloo sessions prune --older-than 30 cleans up old sessions.",
    "Zeloo skills search react --source skills-sh searches the skills.sh public directory.",
    "Zeloo skills check scans installed hub skills for upstream updates.",
    "Zeloo skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "Zeloo skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "Zeloo mcp add github --command npx adds MCP servers from the command line.",
    "Zeloo mcp serve runs Zeloo itself as an MCP server for other agents.",
    "Zeloo auth add lets you add multiple API keys for credential pool rotation.",
    "Zeloo completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "Zeloo logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "Zeloo backup creates a zip backup of your entire Zeloo home directory.",
    "Zeloo profile create coder creates an isolated profile that becomes its own command.",
    "Zeloo profile create work --clone copies your current config and keys to a new profile.",
    "Zeloo update syncs new bundled skills to ALL profiles automatically.",
    "Zeloo gateway install sets up Zeloo as a system service (systemd/launchd).",
    "Zeloo memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "Zeloo webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: Zeloo tools disables unused tools, Zeloo skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "Zeloo models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    # --- Configuration ---
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as Zeloo -w).",
    ...
    "ZELOO_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Zeloo tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'Zeloo profile create coder' creates the 'coder' command.",
    "Zeloo profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Zeloo prints a resume command with session ID and stats.",
    "Zeloo sessions export backup.jsonl exports all sessions for backup or analysis.",
    "Zeloo -r SESSION_ID resumes any specific past session by its ID.",
    # --- Memory ---
    ...
    "Zeloo skills install official/security/1password installs optional skills from the repo.",
    ...
    "/plan writes a markdown implementation plan to .Zeloo/plans/ without executing anything.",
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: Zeloo cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Zeloo runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "Zeloo gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Zeloo via a local macOS server.",
    ...
    "Context files (.Zeloo.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Zeloo loads project context from .Zeloo.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "Zeloo mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "Zeloo mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "Zeloo mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — Zeloo tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.Zeloo/checkpoints/ — your project's .git is never touched.",
    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "Zeloo chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "Zeloo plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "Zeloo acp runs Zeloo as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "ZELOO_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "Zeloo auth add nous or Zeloo auth add openai-codex sets up OAuth-based providers.",
    ...
    "Zeloo status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.Zeloo/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Zeloo your own.",
    ...
    "Before a gateway session reset, Zeloo auto-flushes important facts to memory in the background.",
    ...
    "execute_code helpers require explicit imports: from zeloo_tools import json_parse, shell_quote, retry.",
    ...
    "The skills audit log at ~/.Zeloo/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope Zeloo state via ZELOO_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "ZELOO_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in ZELOO_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.Zeloo/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'Zeloo profile create ls' would be rejected.",
    "Zeloo profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "Zeloo claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "ZELOO_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via Zeloo tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.Zeloo/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.Zeloo/skills/.hub/quarantine/ holds skills pending security review.",
    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Zeloo auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Zeloo config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Zeloo is working.',
    ...
    '/reload reloads ~/.Zeloo/.env into the running session — pick up new API keys without restarting.',
    ...
    "ZELOO_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.Zeloo/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.Zeloo/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',
    # --- Curator ---
    'Zeloo curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "Zeloo curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'Zeloo curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',
    # --- Credential Pools & Routing ---
    'Zeloo auth reset <provider> [target] clears cooldowns and exhaustion flags on a whole credential pool, or on one credential by index, id, or label.',
    ...
    # --- TUI & Dashboard ---
    'ZELOO_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "ZELOO_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'Zeloo dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'Zeloo dashboard embeds the full Zeloo TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.Zeloo/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.Zeloo/dashboard-plugins/ — no npm build required.',
    ...
    'ZELOO_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs concise/all/error/off).',
    'ZELOO_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'ZELOO_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'ZELOO_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.Zeloo/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.Zeloo/skills/ so drop-in skills appear without restarting the session.',
    ...
    'Zeloo -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'Zeloo chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'Zeloo chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'Zeloo chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "Zeloo chat --source tool tags programmatic chats so they don't clutter Zeloo sessions list.",
    'Zeloo dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'Zeloo sessions rename <ID> "new title" renames any past session; Zeloo sessions delete <ID> removes one.',
    'Zeloo import restores a full Zeloo backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'Zeloo fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'Zeloo pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'Zeloo setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'Zeloo status --deep runs the full health sweep across every component; plain Zeloo status is the quick view.',
    # --- Agent Behavior Env Vars ---
    'ZELOO_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'ZELOO_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.Zeloo/plugins/ — trust-gated by design.',
    "ZELOO_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'ZELOO_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'ZELOO_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'ZELOO_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'ZELOO_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'ZELOO_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'ZELOO_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',
    # --- Gateway Behavior Env Vars ---
    'ZELOO_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'ZELOO_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'ZELOO_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'ZELOO_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Zeloo block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from Zeloo sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.Zeloo/dashboard-plugins/.',
]
...
# Task-oriented example prompts for the empty composer. Kept generic — Zeloo is
