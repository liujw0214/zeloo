...
DEFAULT_API_URL = "https://setup.Zeloo-agent.nousresearch.com"
TELEGRAM_ONBOARDING_URL_ENV = "TELEGRAM_ONBOARDING_URL"
DEFAULT_BOT_NAME = "Zeloo Agent"
...
def auto_setup_telegram_bot_result(
    api_url: str | None = None, manager_bot: str = "ZELOOSetupBot",
    ...
    print(f"\n  Contacting Zeloo Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Zeloo Telegram onboarding service.\n"
...
DEFAULT_MANAGER_BOT = "ZELOOSetupBot"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``ZELOO_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``ZELOO_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"ZELOO_{generate_username_slug()}_bot"
