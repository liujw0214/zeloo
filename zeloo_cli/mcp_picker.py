"""MCP picker — interactive `Zeloo mcp picker` (also the default `Zeloo mcp`)."""
...
from zeloo_cli.colors import Colors
from zeloo_cli.cli_output import prompt_yes_no
from zeloo_cli.curses_ui import curses_single_select
from zeloo_cli.mcp_catalog import (
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    _say(
        ...
        "Start a new Zeloo session for changes to take effect."
    )
...
def _configure_tools(name: str) -> None:
    ...
    from zeloo_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    """Plain-text catalog dump: `Zeloo mcp catalog` output and the non-curses fallback."""
    print()
    if not rows:
    ...
    print()
    _say("  Install: Zeloo mcp install <name>    Picker: Zeloo mcp", Colors.DIM)
    # Manifest-version warnings: the user's Zeloo is too old to install everything listed.
    future = [d for d in catalog_diagnostics() if d[1] == "future_manifest"]
    if future:
        print()
        for name, _, _msg in future:
            _say(f"  ⚠ '{name}' requires a newer Zeloo — run `Zeloo update` to install this entry.", Colors.YELLOW)
        print()
    print()


def show_catalog() -> None:
    """`Zeloo mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`Zeloo mcp picker` (and default `Zeloo mcp`) — interactive selector; re-renders after each
    action until ESC/q."""
    while True:
        rows = _build_rows()
...
def install_by_name(identifier: str) -> int:
    """`Zeloo mcp install <name>` — non-interactive entry-point."""
    from zeloo_cli.mcp_catalog import get_entry

    entry = get_entry(identifier)
    if entry is None:
        _say(
            f"  ✗ '{identifier}' is not in the catalog. Run `Zeloo mcp catalog` to see available entries.",
            Colors.RED,
        )
        return 1
