...
Mixed into :class:`zeloo_cli.plugins.PluginManager`. ``_resolve_hook_callback_timeout`` stays on
...
from zeloo_cli.middleware import OBSERVER_SCHEMA_VERSION
...
PLUGIN_SECTIONS_START = "<!-- Zeloo-plugin-sections:start -->"
PLUGIN_SECTIONS_END = "<!-- Zeloo-plugin-sections:end -->"
...
def format_system_prompt_section(section_id: str, content: str) -> str:
    ...
    return (
        ...
        f"<!-- Zeloo-plugin-section-chars:{len(content)} -->\n\n{content}")
...
# Reserved event namespace prefix — only core may publish ``Zeloo:<event>``.
ZELOO_EVENT_NAMESPACE = "Zeloo"
...
class PluginDispatchMixin:
    ...
    def invoke_hook(self, hook_name: str, **kwargs: Any) -> List[Any]:
        ...
        """
        from zeloo_cli.plugins import _resolve_hook_callback_timeout
    ...
    def _run_hook_callback_bounded(
        ...
        thread = threading.Thread(target=_runner, name=f"Zeloo-hook-{callback_name}"[:40], daemon=True)
        try:
            thread.start()
    ...
    def _ensure_event_worker_locked(self) -> None:
        ...
        worker = threading.Thread(
            target=self._event_worker_loop, args=(self._event_queue,), name="Zeloo-plugin-events",
            daemon=True,
        )
    ...
    def _deliver_event(self, item: _QueuedPluginEvent) -> None:
        ...
        from zeloo_cli.plugins import resolve_plugin_command_result
        with self._event_lock:
