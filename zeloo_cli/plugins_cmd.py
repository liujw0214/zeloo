"""``Zeloo plugins`` CLI subcommand — install, update, remove, and list plugins."""
...
from zeloo_cli._subprocess_compat import noninteractive_git_env
from zeloo_cli.cli_output import line_input
from zeloo_cli.config import cfg_get
from zeloo_cli.plugin_capabilities import _child_dict
...
def _plugins_dir() -> Path:
    ...
    plugins = get_zeloo_home() / "plugins"
...
def _read_manifest(plugin_dir: Path) -> dict:
    ...
    if manifest_file is None:
        if not _has_portable_manifest(plugin_dir):
            return {}
        try:
            from zeloo_cli.agent_plugins import read_agent_plugin_manifest
...
def _missing_env_specs(manifest: dict) -> list[dict]:
    ...
    normalised to dicts; nameless entries dropped) whose variable is unset in ``~/.Zeloo/.env``."""
    env_specs = [
...
def _print_python_dependencies(manifest: dict, console) -> None:
    """Print declared ``python_dependencies`` with an install hint — Zeloo never auto-installs
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  Skipped (you can set these later in {display_zeloo_home()}/.env)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]✓[/green] Saved to {display_zeloo_home()}/.env")
        else:
            console.print(f"  [dim]  Skipped (set {name} in {display_zeloo_home()}/.env later)[/dim]")
...
def _install_metadata_path() -> Path:
    return get_zeloo_home() / "plugins" / ".install-metadata.json"
...
def _check_manifest_version(manifest: dict, plugin_name: str) -> None:
    ...
    if mv_int > _SUPPORTED_MANIFEST_VERSION:
        ...
        raise PluginOperationError(
            ...
            f"Run {recommended_update_command()} to update Zeloo.",
        ) from None
    ...
    with tempfile.TemporaryDirectory(prefix=".install-", dir=plugins_dir) as tmp:
        ...
        if target.exists() and not force:
            raise PluginOperationError(
                ...
                f"or run `Zeloo plugins update {plugin_name}`.")
...
def cmd_install(
    ...
    a ``.Zeloo-catalog.json`` sidecar; URLs/shorthand are flagged as custom (unreviewed) sources. Every
    ...
    *enable* None prompts "Enable now? [y/N]"; True/False skip the prompt.
    """
    from zeloo_cli import plugins_cmd_catalog as catalog
    console = _console()
    entry = None
    if catalog.looks_like_catalog_name(identifier):
        ...
        console.print("[yellow]Warning:[/yellow] custom (unreviewed) source — not from the Zeloo catalog.")
    if allow_removed:
        console.print(
    ...
    if not _looks_like_plugin_dir(target):
        console.print(
            ...
            f"plugin.json, or __init__.py. It may not be a valid Zeloo plugin.")
    ...
    if enable:
        ...
        console.print(
            ...
            f"Run `Zeloo plugins enable {installed_name}` to activate.[/dim]")
    ...
    console.print("[dim]  Zeloo gateway restart[/dim]")
    console.print()
...
def cmd_update(name: str) -> None:
    ...
    try:
        output = _pull_plugin_update(
            ...
            lambda rec: (
                ...
                f"`Zeloo plugins install {escape(str(rec.get('source', '<source>')))} --force "
    ...
    if declared_caps:
        from zeloo_cli.plugin_capabilities import declared_set_changed, pending_capabilities
...
def _rescan_after_update(target: Path, name: str, console) -> None:
    ...
    if scan_result.verdict == "dangerous":
        ...
        console.print(
            ...
            f"findings, then re-enable with `Zeloo plugins enable {name}` "
            f"if you trust them.")
...
def ensure_basic_auth_plugin_enabled_in_config(cfg: dict) -> bool:
    ...
    ``Zeloo setup`` / ``Zeloo plugins disable basic`` can park it there while
...
def cmd_enable(name: str, allow_tool_override: Optional[bool] = None) -> None:
    ...
    """
    from zeloo_cli.relay_plugin_cutover import LEGACY_RELAY_PLUGIN_KEYS, RELAY_PLUGINS_CONFIG_ENV
    console = _console()
    ...
    def _refuse_legacy_relay(plugin: str) -> None:
        if plugin in LEGACY_RELAY_PLUGIN_KEYS:
            _fail(console, (
                ...
                f"by Zeloo core; configure {RELAY_PLUGINS_CONFIG_ENV} instead."))
...
def _declared_capabilities_from_manifest(manifest: dict, plugin_name: str = "?") -> list:
    ...
    from zeloo_cli.plugin_capabilities import parse_declared_capabilities
...
def _declared_capabilities_for_key(key: str) -> list:
    ...
    if entry[3] == "entrypoint":
        from zeloo_cli.plugins import discover_entrypoint_manifests
...
def _run_capability_consent(console, plugin_id: str, declared: list, *, context: str = "install") -> bool:
    ...
    """
    from zeloo_cli.plugin_capabilities import CAPABILITY_REGISTRY, pending_capabilities, record_consent
    ...
    if not _is_tty():
        console.print(
            ...
            f"`Zeloo plugins capabilities {plugin_id}` to review and "
            f"`Zeloo plugins enable {plugin_id}` to grant interactively.")
        return False
    ...
    console.print(
        ...
        f"`Zeloo plugins enable {plugin_id}` to grant later.[/dim]")
    return False


def cmd_capabilities(name: Optional[str] = None) -> None:
    """``Zeloo plugins capabilities [<id>]`` — declared vs granted."""
    from zeloo_cli.plugin_capabilities import (
        CAPABILITY_REGISTRY,
...
def _resolve_tool_override_grant(console, key: str, allow_tool_override: Optional[bool]) -> None:
    ...
    if allow_tool_override:
        ...
        console.print(
            ...
            f"`Zeloo plugins enable {key} --allow-tool-override` to grant "
            "this later.[/dim]")
...
def _discover_all_plugins() -> list:
    ...
    from zeloo_cli.plugins import discover_entrypoint_manifests, get_bundled_plugins_dir
...
def _filter_plugin_entries(entries: list, args: Any, enabled: set, disabled: set) -> list:
    """Apply ``Zeloo plugins list`` CLI filters."""
    filtered = entries
...
def cmd_list(args: Any | None = None) -> None:
    ...
    if not entries:
        ...
        console.print("[dim]Install with:[/dim] Zeloo plugins install owner/repo")
        return
    ...
    for line in removed_lines:
        console.print(line)
    console.print()
    console.print("[dim]Compact view:[/dim] Zeloo plugins list --plain --no-bundled")
    console.print("[dim]Interactive toggle:[/dim] Zeloo plugins")
    console.print("[dim]Enable/disable:[/dim] Zeloo plugins enable/disable <name>")
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    try:
        ...
        pass
    try:
        from zeloo_cli.plugins import discover_plugins, get_plugin_context_engine
        discover_plugins()
...
def cmd_show(name: str) -> None:
    ...
    if match is None:
        ...
        _fail(console, "[dim]List installed plugins:[/dim] Zeloo plugins list")
...
def _run_composite_ui(curses, plugin_keys, plugin_labels, plugin_selected, disabled, categories, console):
    ...
    from zeloo_cli.curses_ui import _addnstr, flush_stdin
...
def dashboard_install_plugin(
    ...
    if catalog_name:
        ...
        if entry is None:
            return {"ok": False, "error": f"'{catalog_name}' is not in the Zeloo plugin catalog."}
        identifier = entry.install_identifier
    else:
        warnings.append("Custom (unreviewed) source — not from the Zeloo catalog.")
    try:
...
def _get_plugin_toolset_key(name: str) -> Optional[str]:
    ...
    def _from_manifest_on_disk() -> Optional[str]:
        from zeloo_cli.plugins import get_bundled_plugins_dir
        return next((
...
def _user_installed_plugin_dir(name: str) -> Optional[Path]:
    """Resolved path under ``~/.Zeloo/plugins/<name>`` if it exists."""
    try:
...
def dashboard_update_user_plugin(name: str) -> dict[str, Any]:
    """``git pull`` inside ``~/.Zeloo/plugins/<name>``."""
    from zeloo_cli import plugins_cmd_catalog as catalog
    ...
    try:
        ...
        msg = _pull_plugin_update(
            ...
            lambda rec: (
                ...
                f"run `Zeloo plugins install {rec.get('source', '<source>')} --force "
...
def _run_plugin_git(
    ...
    if auth_url:
        from zeloo_cli.git_credentials import with_git_auth
...
def _autostash_dirty_tree(git_exe: str, target: Path) -> tuple[bool, str]:
    ...
    push = _run_plugin_git(
        git_exe, target, "stash", "push", "--include-untracked", "-m", "Zeloo-plugin-update-autostash")
...
def _git_pull_plugin_dir(target: Path) -> tuple[bool, str]:
    ...
    marketplace now succeeds when its checkout has local changes"), and the same autostash approach ``Zeloo
...
def dashboard_remove_user_plugin(name: str) -> dict[str, Any]:
    """Delete a plugin tree under ``~/.Zeloo/plugins/`` only."""
    plugins_dir = _plugins_dir()
...
def cmd_plugin_doctor(target: str = ".", *, ci: bool = False) -> None:
    ...
    from zeloo_cli.plugin_dev import doctor_plugin
...
def _catalog():
    from zeloo_cli import plugins_cmd_catalog
    return plugins_cmd_catalog


def _action_pack(args):
    from zeloo_cli.plugin_packs import pack_command
    pack_command(args)


def cmd_compat(args: Any | None = None) -> None:
    """``Zeloo plugins compat`` — which installed plugins import paths scheduled for removal, and where."""
    import sys
    from pathlib import Path
    from zeloo_cli.plugin_compat import (
...
def plugins_command(args) -> None:
    """Dispatch Zeloo plugins subcommands."""
