"""Shell completion script generation for Zeloo CLI. Walks the live argparse parser tree, so
    ...
    for cmd, info in subcommands:
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_zeloo_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;")
            ...
            cases.append(
                ...
                f"            ;;")
    cases_str = "\n".join(cases)
    return f"""# Zeloo Agent bash completion
# Add to ~/.bashrc:
#   eval "$(Zeloo completion bash)"

_zeloo_profiles() {{
    local profiles_dir="$HOME/.Zeloo/profiles"
...
_zeloo_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_zeloo_profiles)" -- "$cur"))
        return
    fi
...
complete -F _zeloo_completion Zeloo
"""
    ...
    for cmd, info in subcommands:
        ...
        if cmd == "profile":
            ...
            sub_cases.append(
                ...
                f"                    ;;")
    sub_cases_str = "\n".join(sub_cases)
    return f"""#compdef Zeloo
# Zeloo Agent zsh completion
# Add to ~/.zshrc:
...
_zeloo_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.Zeloo/profiles" ]]; then
        profiles+=($HOME/.Zeloo/profiles/*(N/:t))
    fi
...
_ZELOO() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_zeloo_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'Zeloo command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Zeloo Agent fish completion",
        "# Add to your config:",
        "#   Zeloo completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __zeloo_profiles",
        "    echo default",
        "    if test -d $HOME/.Zeloo/profiles",
        "        for d in $HOME/.Zeloo/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c Zeloo -f -s p -l profile"
        " -d 'Profile name' -xa '(__zeloo_profiles)'",
        "",
        "# Top-level subcommands"]
    for cmd, info in subcommands:
        lines.append(
            f"complete -c Zeloo -f "
        ...
        if cmd == "profile":  # profile names for the actions that take one
            for action in sorted(_PROFILE_NAME_ACTIONS):
                lines.append(
                    ...
                    f"-a '(__zeloo_profiles)' -d 'Profile name'")
    lines.append("")
    return "\n".join(lines)
