"""Baked-in build metadata for Zeloo Agent.

Source installs report their git revision live via ``git rev-parse`` (``zeloo_cli/dump.py``,
``zeloo_cli/banner.py``). Inside the published Docker image ``.dockerignore`` excludes ``.git``,
so those callsites fall back to the ``.zeloo_build_sha`` file written by the Dockerfile.
"""
...
_BUILD_SHA_FILE = Path(__file__).parent.parent / ".zeloo_build_sha"
...
def get_code_identity(refresh: bool = False) -> dict:
    ...
    ``.zeloo_build_sha`` for Docker images, else unknown. Cached per process — code identity
...
def get_build_sha(short: int = 8) -> Optional[str]:
    """Baked-in build SHA (Dockerfile ``ZELOO_GIT_SHA`` build-arg, full 40-char hash on one
