# zeloo_cli/ + cli.py — CLI, slash commands, config, skins, updater, profiles

Applies on top of the root `AGENTS.md`. Long-form: `website/docs/developer-guide/cli-internals.md`.

## CLI architecture

`cli.py` holds `ZELOOCLI` (REPL loop, config, slash dispatch); behaviour lives in mixins
`zeloo_cli/cli_commands_mixin.py`, `cli_stream_mixin.py`, `cli_status_bar_mixin.py`,
...
`ZELOOCLI._SLASH_DISPATCH` (`canonical -> (method name, pass_arg)`), falling back to a
`_handle_<name>_command` method by naming convention. **There is no `elif` ladder — do not add one.**
Skill slash commands (`agent/skill_commands.py`) scan `~/.Zeloo/skills/` and inject as a **user
message**, never into the system prompt (prompt caching).

Rules: all interactive menu-pickers use curses (`zeloo_cli/curses_ui.py`; example
`zeloo_cli/tools_config.py`). Never emit `\033[K` (ANSI erase-to-EOL) in spinner/display code —
...
## Slash command registry (`zeloo_cli/commands.py`)
...
`zeloo_cli/goal_command.py::dispatch_goal_command` owns `/goal` parsing and manager
...
## Config system (`zeloo_cli/config.py`)
...
- **Three loaders — know which you're in:** `load_cli_config()` (CLI, `cli.py`); `load_config()`
  (`Zeloo tools/setup`, most subcommands, `zeloo_cli/config.py`, merges `DEFAULT_CONFIG`); raw
...
## Skin engine (`zeloo_cli/skin_engine.py`)
...
`load_skin(name)` (user `~/.Zeloo/skins/*.yaml` → built-ins → default; missing values inherit
...
## Update pipeline (`Zeloo update`) — transactional; every stage guards a real field failure
...
- **Plan** (`update_inventory.py`, `Zeloo update --plan`): read-only inventory — install kind, all
...
- **Restart-per-kind**: systemd and launchd restarts are FLEET-WIDE (every `Zeloo-gateway*` unit /
  `ai.Zeloo.gateway*` LaunchAgent), drain-first (SIGUSR1), with per-unit/per-label failure
...
- **Report**: every run writes a machine-readable receipt to `~/.Zeloo/logs/update_receipts/`
...
`_apply_profile_override()` in `zeloo_cli/main.py` sets `ZELOO_HOME` before any module import, so
every `get_zeloo_home()` scopes to the active profile (rules in root). Profiles are independent
...
## Nous free tier (`zeloo_cli/anon_auth.py`)
...
which never contains a raw exception, a URL or a `Zeloo` verb) and `.copy_terminal`, so no caller
...
contextvars. `upgrade_guest` (`Zeloo auth upgrade`), the CLI `/login` handler and the desktop
