"""``Zeloo profile`` command — one handler per action, dispatched by ``PROFILE_ACTIONS``.

Imports from ``zeloo_cli.profiles`` stay lazy (inside each handler) so tests can monkeypatch
...
def _render_distribution_plan(plan) -> None:
    ...
    from zeloo_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.zeloo_requires:
        print(f"  Requires: Zeloo {mf.zeloo_requires}")
...
def _profile_status(args):
    """Bare ``Zeloo profile`` — show current profile status."""
    from zeloo_constants import display_zeloo_home
    from zeloo_cli.profiles import format_profile_label, get_active_profile_name, list_profiles
    profile_name = get_active_profile_name()
    dhh = display_zeloo_home()
    ...
    if current is not None:
        ...
        if p.alias_path:
            print(f"Alias:          {p.alias_name or p.name} → Zeloo -p {p.name}")
    print()
...
def _profile_use(args):
    from zeloo_cli.profiles import set_active_profile
    name = args.profile_name
    try:
        set_active_profile(name)
        print("Switched to: default (~/.Zeloo)" if name == "default" else f"Switched to: {name}")
...
def _profile_create(args):
    ...
    if not no_alias:
        ...
        if collision:
            ...
            print(f"  Choose a custom alias:  Zeloo profile alias {name} --name <custom>")
            print(f"  Or access via flag:     Zeloo -p {name} chat")
        else:
...
def _profile_delete(args):
    from zeloo_cli.profiles import delete_profile
    try:
...
def _describe_target_dir(name: str) -> Path:
    """Profile dir for ``describe``: ``default`` maps to the CURRENT home (get_zeloo_home),
    everything else to its named directory."""
    from zeloo_cli import profiles as _profiles_mod
    if _profiles_mod.normalize_profile_name(name) == "default":
        from zeloo_constants import get_zeloo_home as _hh
        return Path(_hh())
...
def _profile_describe(args):
    ...
    from zeloo_cli import profile_describer as _pd
    if all_flag:
...
def _profile_show(args):
    ...
    if dist_name:
        ...
        print(f"  (run `Zeloo profile info {name}` for full manifest)")
    if alias_name:
        print(f"Alias:   {alias_name} → Zeloo -p {name}  ({_wrapper_path(alias_name)})")
    print()
...
def _profile_rename(args):
    from zeloo_cli.profiles import normalize_profile_name, rename_profile
    try:
...
def _profile_export(args):
    from zeloo_cli.profiles import export_profile, get_profile_export_path
    name = args.profile_name
    try:
...
def _profile_install(args):
    import tempfile
    from zeloo_cli.profile_distribution import DistributionError, install_distribution, plan_install
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="zeloo_dist_preview_") as tmp:
        ...
        if plan.has_cron:
            print(
                ...
                f"  Review them with:  Zeloo -p {plan.manifest.name} cron list"
            )
        print(f"\n  Use with:      Zeloo -p {plan.manifest.name} chat")
    except (DistributionError, ValueError) as e:
        _die(f"Error: {e}")


def _profile_update(args):
    from zeloo_cli.profile_distribution import DistributionError, read_manifest, update_distribution
    from zeloo_cli.profiles import get_profile_dir, normalize_profile_name
    try:
        ...
        if current is None:
            _die(
                ...
                "Only profiles installed via `Zeloo profile install` can be updated."
            )
        ...
        if plan.has_cron:
            print(f"  Cron files were refreshed.  Review with:  Zeloo -p {plan.manifest.name} cron list")
    except (DistributionError, ValueError) as e:
        _die(f"Error: {e}")
...
def _profile_info(args):
    from zeloo_cli.profile_distribution import describe_distribution, DistributionError
    try:
...
# Order mirrors the original if/elif chain; None = bare ``Zeloo profile``.
PROFILE_ACTIONS = {
