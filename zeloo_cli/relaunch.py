"""Unified self-relaunch for Zeloo CLI: preserves inherited flags (--tui, --dev, --profile, --model…)
across process replacement so ``Zeloo sessions browse`` / post-setup relaunch keep the user's mode."""
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched Zeloo."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_zeloo_bin() -> Optional[str]:
    """Zeloo entry point: ``sys.argv[0]`` if a real executable, else ``which Zeloo``, else ``None``
    (caller falls back to ``python -m zeloo_cli.main``)."""
    argv0 = sys.argv[0]
    ...
    if not argv0.startswith("-") and os.path.isfile(argv0):
        ...
        if os.access(abs_path, os.X_OK) and not (_is_windows and _is_python_script(abs_path)):
            return abs_path
    return shutil.which("Zeloo") or None


def build_relaunch_argv(
    extra_args: Sequence[str], *, preserve_inherited: bool = True, original_argv: Optional[Sequence[str]] = None
) -> list[str]:
    """Construct an argv list for replacing the current process with Zeloo."""
    bin_path = resolve_zeloo_bin()
    argv = [bin_path] if bin_path else [sys.executable, "-m", "zeloo_cli.main"]
...
def relaunch(
    ...
    """Replace the current process with a fresh Zeloo invocation.
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # Raw ``[Errno 8] Exec format error`` is cryptic; usual causes are ``Zeloo`` not on
            # PATH yet (install hasn't propagated User PATH into this shell) or a stale shim.
            print(
                f"\nZELOO relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run Zeloo.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
