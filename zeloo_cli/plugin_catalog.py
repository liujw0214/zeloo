"""Plugin catalog — curated, Nous-approved Zeloo plugins shipped with the repo.
...
an installed Zeloo sees new entries and removals without updating. Any fetch failure falls back to the
...
LIVE_CATALOG_URL = "https://Zeloo-agent.nousresearch.com/docs/api/plugin-catalog.json"
...
class PluginCatalogEntry:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "maintainer": self.maintainer, "tier": self.tier, "requires_ZELOO": self.requires_ZELOO,
...
def entry_from_mapping(data: Any, label: str) -> Optional[PluginCatalogEntry]:
    ...
    return PluginCatalogEntry(
        ...
        requires_ZELOO=str(data.get("requires_ZELOO") or "").strip(),
...
def _live_cache_path() -> Path:
    ...
    return get_zeloo_home() / "cache" / "plugin-catalog.json"
...
def fetch_live_catalog(*, force: bool = False) -> Optional[Dict[str, Any]]:
    ...
    ``ZELOO_HOME/cache`` for :data:`LIVE_CATALOG_TTL_SECONDS`. ``None`` on ANY failure — callers fall
...
def entry_capability_summary(entry: PluginCatalogEntry) -> str:
    ...
    if entry.requires_ZELOO:
        bits.append(f"Requires Zeloo {entry.requires_ZELOO}.")
    return " ".join(bits)
