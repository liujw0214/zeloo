...
Split out of :mod:`zeloo_cli.plugins`; validation warns and never fails a load.
"""
...
from zeloo_cli.plugin_capabilities import parse_declared_capabilities as _parse_declared_capabilities
...
# and continue loading. ``capabilities``/``emits``/``listens``/``Zeloo``/``depends`` are reserved.
...
_KNOWN_MANIFEST_FIELDS: Set[str] = {
    ...
    "license", "homepage", "tags", "capabilities", "emits", "listens", "Zeloo", "depends",
    "requires_ZELOO",
}

# Highest manifest schema version this Zeloo understands.
...
def _plugins_debug() -> bool:
    from zeloo_cli import plugins as _origin
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this Zeloo "
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `Zeloo plugins enable %s` if it is installed.",
                    k, dep_id, dep_id,
                )
            elif resolved == k:
...
class PluginManifest:
    ...
    kind: str = "standalone"
    # Path-derived registry key used by plugins.enabled/disabled and `Zeloo plugins list`: ``disk-cleanup``
    # for a flat plugin, ``image_gen/openai`` for a category plugin. Empty -> name.
    key: str = ""
    # Zeloo version requirement (``">=0.19"``, comma-separated clauses allowed). Unsatisfied plugins are
    # recorded with an error and skipped before import — see ``requires_zeloo_error``.
    requires_ZELOO: str = ""
    portable: bool = False
    skill_namespace: str = ""
    ...
    # VALIDATED AND SURFACED ONLY — Zeloo never auto-installs these (isolation design for the install seam
...
# ── requires_ZELOO version gate ─────────────────────────────────────────────
_VERSION_COMPARATOR_RE = re.compile(r"^\s*(>=|<=|==|!=|>|<)\s*(.+?)\s*$")


def running_zeloo_version() -> str:
    """Installed ``Zeloo-agent`` distribution version, else ``zeloo_cli.__version__`` (source checkout)."""
    try:
        return importlib.metadata.version("Zeloo-agent")
    except Exception:
...
def requires_zeloo_error(manifest: "PluginManifest") -> Optional[str]:
    """Load-blocking reason when the manifest's ``requires_ZELOO`` rejects the running version."""
    if not manifest.requires_ZELOO:
        return None
    current = running_zeloo_version()
    if version_satisfies(manifest.requires_ZELOO, current):
        return None
    return f"requires Zeloo {manifest.requires_ZELOO}, running {current}"
...
def parse_manifest_file(
    ...
    try:
        ...
        return PluginManifest(
            ...
            kind=kind, key=key, requires_ZELOO=str(data.get("requires_ZELOO") or "").strip(),
