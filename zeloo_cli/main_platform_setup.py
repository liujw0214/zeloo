...
from zeloo_cli.model_setup_flows_common import _say
...
def _whatsapp_choose_mode(get_env_value, save_env_value):
    """Step 1 of ``Zeloo whatsapp``: ``"bot"`` / ``"self-chat"``, or None when cancelled."""
    current_mode = get_env_value("WHATSAPP_MODE") or ""
    if current_mode:
        mode_label = "separate bot number" if current_mode == "bot" else "personal number (self-chat)"
        print(f"\n✓ Mode: {mode_label}")
        return current_mode
    _say("", "How will you use WhatsApp with Zeloo?", "",
...
def _whatsapp_allowed_users(wa_mode: str, get_env_value, save_env_value) -> None:
    """Step 3 of ``Zeloo whatsapp``: show / set WHATSAPP_ALLOWED_USERS."""
...
def _whatsapp_install_bridge(bridge_dir) -> bool:
    """Step 4 of ``Zeloo whatsapp``: ``npm install`` the bridge when needed. False = stop."""
    ...
    try:
        result = subprocess.run(
            ...
            encoding="utf-8", errors="replace", env=with_zeloo_node_path())
...
def cmd_whatsapp(args):
    ...
    from zeloo_cli.main import _require_tty, get_zeloo_home
    ...
    # every `Zeloo gateway` would pay a 30s bridge timeout + indefinite retries. Set only after
    ...
    session_dir = get_zeloo_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if _yes_no("\n  Re-pair? This will clear the existing session. [y/N] "):
            ...
            _say("\n✓ WhatsApp is configured and paired!", "  Start the gateway with: Zeloo gateway")
            return
    ...
    _say("", "  Or install as a service: Zeloo gateway install")


def cmd_whatsapp_cloud(args):
    """Set up WhatsApp Business Cloud API (official Meta integration) — complementary to the
    ``Zeloo whatsapp`` Baileys bridge wizard. See ``zeloo_cli/setup_whatsapp_cloud.py``."""
    from zeloo_cli.main import _require_tty
    _require_tty("whatsapp-cloud")
    from zeloo_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def _sync_toggle(args, sub: str) -> int:
    ...
    if not is_curation_eligible(skill):
        _err(f"'{skill}' is not sync-eligible (bundled, hub-installed, "
             ...
             f"skills under ~/.Zeloo/skills/ can sync.")
        return 1
...
def _sync_status(ssc) -> int:
    ...
    if status.get("org_available"):
        ...
        if modified:
            _err(f"  {len(modified)} with local edits not yet shared: "
                 f"{', '.join(modified)}\n"
                 f"  Share them back with `Zeloo sync propose <skill>`. "
    ...
    if not status.get("logged_in"):
        ...
        _err("\nSync feature is off for this instance (set ZELOO_SYNC_ENABLED=1 "
             "or config.yaml sync.enabled: true). Sync is inert.")
    elif not status.get("base_url"):
        _err("\nNo sync base URL configured (config.yaml sync.base_url or ZELOO_SYNC_BASE_URL). Sync is inert.")
    return 0
...
_SYNC_GATED = {
    "pull": _sync_pull,
    "push": lambda ssc, identity: ssc.push_skills(identity=identity, message="Zeloo sync push"),
    "now": lambda ssc, identity: {"pull": ssc.pull_skills(identity=identity),
                                  "push": ssc.push_skills(identity=identity, message="Zeloo sync now")}}
...
def cmd_sync(args):
    ...
    if not ssc.resolve_sync_base_url():
        _err("sync inert: no sync base URL configured (config.yaml sync.base_url or ZELOO_SYNC_BASE_URL).")
        return 1
...
def cmd_slack(args):
    """``Zeloo slack <subcommand>``; ``manifest`` prints or writes a Slack app manifest with
    every gateway command registered as a first-class slash."""
    sub = getattr(args, "slack_command", None)
    if sub in {None, ""}:
        _err("usage: Zeloo slack <subcommand>\n"
             "\n"
             "subcommands:\n"
             "  manifest   Generate a Slack app manifest with every gateway\n"
             "             command registered as a native slash\n"
             "\n"
             "Run `Zeloo slack manifest -h` for details.")
        return 1

    if sub == "manifest":
        from zeloo_cli.slack_cli import slack_manifest_command
