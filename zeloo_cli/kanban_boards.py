"""``Zeloo kanban boards …`` — board directories, the ``current`` pointer and ``board.json``.
...
from zeloo_cli.kanban_output import _err, _fmt_counts, _json_out


def _dispatch_boards(args: argparse.Namespace) -> int:
    """``Zeloo kanban boards <action>`` — filesystem-only, so it works before ``kanban init``."""
...
def _cmd_boards_list(args: argparse.Namespace) -> int:
    ...
    if _json_out(args, boards):
        return 0
    if not boards:
        print("(no boards — create one with `Zeloo kanban boards create <slug>`)")
        return 0
    ...
    if len(boards) > 1:
        print("Switch boards with `Zeloo kanban boards switch <slug>`.")
    return 0
...
def _cmd_boards_create(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "switch", False):
        ...
        print(f"  Use `Zeloo kanban boards switch {meta['slug']}` to make it current.")
    return 0
...
def _cmd_boards_export(args: argparse.Namespace) -> int:
    ...
    print(f"Exported board {res['board']!r} → {res['archive']}\n"
          ...
          "Import it with `Zeloo kanban boards import <archive>`.")
    return 0
...
def _cmd_boards_import(args: argparse.Namespace) -> int:
    ...
    if res["activated"]:
        ...
        print(f"  Switch to it with `Zeloo kanban boards switch {res['board']}`.")
    return 0
