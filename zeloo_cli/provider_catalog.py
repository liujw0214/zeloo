...
The provider list shown by ``Zeloo model`` (CLI/TUI) and the desktop Settings → Providers tabs
...
come from :data:`zeloo_cli.auth.PROVIDER_REGISTRY` (credential truth); ``display_name`` /
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `Zeloo model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """One descriptor per provider in the ``Zeloo model`` universe (:data:`CANONICAL_PROVIDERS`,
    ...
    PROVIDER_REGISTRY = _safe_import("zeloo_cli.auth", "PROVIDER_REGISTRY", {})
    OPTIONAL_ENV_VARS = _safe_import("zeloo_cli.config", "OPTIONAL_ENV_VARS", {})
    ...
    ZELOO_OVERLAYS = _safe_import("zeloo_cli.providers", "ZELOO_OVERLAYS", {})
    try:
