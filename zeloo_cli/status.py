...
from zeloo_cli.auth import AuthError, resolve_provider
from zeloo_cli.colors import Colors, color
from zeloo_cli.config import get_env_path, get_env_value, get_zeloo_home, load_config
from zeloo_cli.models import provider_label
from zeloo_cli.runtime_provider import resolve_requested_provider
from zeloo_cli.vercel_auth import describe_vercel_auth
from zeloo_cli.status_auth import (  # renderers wired into _SECTIONS below
    _render_api_keys, _render_apikey_providers, _render_auth_providers, _render_nous_gateway)
from zeloo_constants import OPENROUTER_MODELS_URL
...
def _estop_status_line():
    """One-line pause banner for `Zeloo status`, or None when not paused."""
    try:
    ...
    return f"⏸️  PAUSED (global emergency stop{f' — reason: {reason}' if reason else ''}; `Zeloo resume` to lift)"
...
def _render_terminal(ctx):
    ...
    if terminal_env in _TERMINAL_ENV_ROWS:
        ...
        _kv_flag("SDK:", importlib.util.find_spec("vercel") is not None, "installed",
                 "missing (install: pip install 'Zeloo-agent[vercel]')")
        ...
        # (fail-soft — never break `Zeloo status`).
        try:
...
def _render_gateway(ctx):
    ...
    try:
        ...
        from zeloo_cli.gateway_multiplex_served import multiplexer_served_secondaries
        ...
        if not snapshot.running and named_profile_served_by_running_multiplexer():
            ...
            _kv("Manage with:", "Zeloo gateway status   # from the default profile")
            return
...
def _render_cron(ctx):
    ...
    jobs_file = get_zeloo_home() / "cron" / "jobs.json"
...
def _render_sessions(ctx):
    ...
    if gateway_rows:
        ...
        if freshest > 0:
            from zeloo_cli.timefmt import relative_time
            print(f"  Last activity:{relative_time(freshest):>13}")
    elif not (sessions_file := get_zeloo_home() / "sessions" / "sessions.json").exists():
        _kv("Active:", 0)
    else:
        try:
    ...
    try:
        from zeloo_cli.active_sessions import (
...
def _render_footer(ctx):
    _banner(("─" * 60, "  Run 'Zeloo doctor' for detailed diagnostics", "  Run 'Zeloo setup' to configure"),
            Colors.DIM)
    print()


# Print order of `Zeloo status`; each renderer takes the shared _StatusContext.
_SECTIONS = (
...
def show_status(args):
    """Show status of all Zeloo Agent components."""
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_nous_portal_account_info': ('zeloo_cli.nous_account', 'get_nous_portal_account_info'),
