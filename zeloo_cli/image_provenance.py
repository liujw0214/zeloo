"""Image-authored deployment provenance for immutable Zeloo runtimes.

The image bakes ``/etc/Zeloo/image-provenance.json`` outside ``$ZELOO_HOME`` and the checkout, so a
...
IMAGE_PROVENANCE_PATH = Path("/etc/Zeloo/image-provenance.json")
