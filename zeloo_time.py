"""Timezone-aware clock for Zeloo.

``now()`` returns a tz-aware datetime in the user's configured IANA timezone. Resolution order:
``ZELOO_TIMEZONE`` env var, then ``timezone`` in ``~/.Zeloo/config.yaml``, else server-local
...
# ZELOO_HOME, so one unkeyed global would leak the first profile's timezone into later
...
def _env_timezone() -> str:
    """``ZELOO_TIMEZONE`` when it may speak for the active profile. Under the multiplexed
    ...
    if is_multiplex_active():
        return ""
    return os.getenv("ZELOO_TIMEZONE", "").strip()
