...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *Zeloo Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    ...
    /opt/Zeloo) is mirrored to ZELOO_HOME so npm install works."""
    import shutil
    from zeloo_constants import get_zeloo_home
    install_bridge = Path(__file__).resolve().parents[2] / "scripts" / "whatsapp-bridge"
    zeloo_home_bridge = get_zeloo_home() / "scripts" / "whatsapp-bridge"
    try:
        ...
        pass
    if zeloo_home_bridge.exists():
        return zeloo_home_bridge
    try:
        zeloo_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(install_bridge, zeloo_home_bridge, dirs_exist_ok=False)
        return zeloo_home_bridge
    except Exception:
        return install_bridge
