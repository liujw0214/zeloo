...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.Zeloo to the active profile home."""
    ...
    if raw == "~/.Zeloo" or raw.startswith("~/.Zeloo/"):
        return zeloo_home / raw[len("~/.Zeloo/"):]
    path = Path(raw).expanduser()
    return path if path.is_absolute() else zeloo_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script; must live under ZELOO_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from zeloo_constants import get_zeloo_home
    scripts_root = (get_zeloo_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.Zeloo" or raw_text.startswith("~/.Zeloo/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        Non-zero exit, empty/``[SILENT]`` stdout, or a ``[SILENT]``/``__zeloo_ignore__`` flag drops the
        ...
        if not isinstance(transformed, dict):
            ...
            return False, None
        silenced = transformed.get("[SILENT]") is True or transformed.get("__zeloo_ignore__") is True
