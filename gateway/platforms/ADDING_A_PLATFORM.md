...
There are two ways to add a platform to the Zeloo gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.Zeloo/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Zeloo code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `Zeloo gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `zeloo_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Zeloo core.
...
"Zeloo-your-platform": {
    ...
    "tools": _ZELOO_CORE_TOOLS,
    "includes": []
},
```

And add it to the `Zeloo-gateway` composite:

```python
"Zeloo-gateway": {
    "includes": [..., "Zeloo-your-platform"]
}
```
...
## 12. Status Display (`zeloo_cli/status.py`)
...
## 13. Gateway Setup Wizard (`zeloo_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ zeloo_cli/ toolsets.py \
  --include="*.py" -l | sort -u
