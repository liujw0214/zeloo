"""Relay/connector support package for the Zeloo gateway.
...
def relay_explicitly_disabled() -> bool:
    ...
    try:
        cfg = read_yaml_layers(get_zeloo_home())
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        try:
            from zeloo_cli.skin_engine import get_active_skin  # late import: boot-safe
        ...
        # prefix every reply "**Zeloo Agent:**" and shadow the connector's
        # linked-owner fallback, which actually disambiguates. Only a customized
        # name is forwarded.
        if value == "Zeloo Agent":
            value = ""
...
def _resolve_relay_identity_token() -> str:
    ...
    Canonical resolver shared by runtime self-provision and ``Zeloo gateway enroll``.
    ...
    if not token_url:
        from zeloo_cli.auth import resolve_nous_access_token
...
def self_provision_relay() -> bool:
    ...
    ``~/.Zeloo/.env``), so a hosted container re-provisions every boot; the
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'Zeloo'}"
