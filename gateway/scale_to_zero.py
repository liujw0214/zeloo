...
SCALE_TO_ZERO_ENV = "ZELOO_SCALE_TO_ZERO"  # stamped by NAS when the Labs toggle is on
...
def dashboard_client_heartbeat_path(zeloo_home: Optional[os.PathLike | str] = None):
    """Path of the dashboard-client liveness marker under ZELOO_HOME."""
    if zeloo_home is None:
        from zeloo_constants import get_zeloo_home
        zeloo_home = get_zeloo_home()
    return Path(zeloo_home) / DASHBOARD_CLIENT_HEARTBEAT_REL
