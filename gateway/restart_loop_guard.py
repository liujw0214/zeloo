...
Defenses 1-2 (``_ZELOO_GATEWAY`` guard on ``Zeloo gateway stop|restart`` /
...
persisted to ``<ZELOO_HOME>/gateway/restart_loop.json`` and CHAIN while gaps stay
...
def _state_path():
    return get_zeloo_home() / "gateway" / "restart_loop.json"
