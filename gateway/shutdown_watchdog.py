...
``<ZELOO_HOME>/state/gateway.heartbeat`` so supervisors can tell "process alive" from "loop
...
def _process_zeloo_home() -> Path:
    """ZELOO_HOME for process-level identity files (ignore profile overrides)."""
    val = os.environ.get("ZELOO_HOME", "").strip()
    return Path(val) if val else get_zeloo_home()


def _home(home: Optional[Path]) -> Path:
    return home if home is not None else _process_zeloo_home()
...
def get_loop_tick_socket_path(home: Optional[Path] = None, pid: Optional[int] = None) -> Path:
    """``<ZELOO_HOME>/state/gateway.loop-tick.<pid>.sock`` — PID-suffixed so a stale node from a
