...
Built on gateway startup, refreshed every 5 min, saved to ~/.Zeloo/channel_directory.json.
...
def _directory_path() -> Path:
    return DIRECTORY_PATH or get_zeloo_home() / "channel_directory.json"


def _aliases_path() -> Path:
    return CHANNEL_ALIASES_PATH or get_zeloo_home() / "channel_aliases.json"
...
def _build_from_sessions_json(platform_name: str) -> List[Dict[str, str]]:
    ...
    sessions_path = get_zeloo_home() / "sessions" / "sessions.json"
...
def format_directory_for_display(platforms: Optional[Dict[str, Any]] = None) -> str:
    ...
    ``platforms`` overrides the on-disk directory (``Zeloo send --list`` merges in
