...
class PlatformEntry:
    ...
    required_env: list = field(default_factory=list)  # ``Zeloo setup`` display
    ...
    plugin_name: str = ""  # owning manifest so ``Zeloo gateway setup`` can auto-enable it
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved ZELOO_HOME and overlay the
        ...
        # `Zeloo` invocation, so the import happens only when a lookup asks for it.
