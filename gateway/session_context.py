"""Session-scoped context variables for the Zeloo gateway.

Replaces the old ``os.environ``-based ``ZELOO_SESSION_*`` state with task-local ``ContextVar``s
...
_SESSION_VARS = (
    ...
    "ZELOO_SESSION_PLATFORM", "ZELOO_SESSION_SOURCE", "ZELOO_SESSION_CHAT_ID",
    "ZELOO_SESSION_CHAT_TYPE", "ZELOO_SESSION_CHAT_NAME", "ZELOO_SESSION_THREAD_ID",
    "ZELOO_SESSION_USER_ID", "ZELOO_SESSION_USER_ID_ALT", "ZELOO_SESSION_USER_NAME",
    "ZELOO_SESSION_SCOPE_ID", "ZELOO_SESSION_KEY", "ZELOO_SESSION_ID",
    "ZELOO_UI_SESSION_ID", "ZELOO_SESSION_MESSAGE_ID", "ZELOO_SESSION_PROFILE",
    "ZELOO_BROWSER_CONTROL_PRINCIPAL", "ZELOO_BROWSER_CONTROL_TRANSPORT_FAMILY",
    "ZELOO_CRON_SESSION", "ZELOO_SESSION_PARENT_CHAT_ID",
))
...
_SESSION_ASYNC_DELIVERY = ContextVar("ZELOO_SESSION_ASYNC_DELIVERY", default=_UNSET)
...
_SESSION_HISTORY_DELIVERY = ContextVar("ZELOO_SESSION_HISTORY_DELIVERY", default=_UNSET)

# Cron auto-delivery vars, set per-job in run_job() so concurrent jobs don't clobber.
_CRON_AUTO_DELIVER_PLATFORM = ContextVar("ZELOO_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID = ContextVar("ZELOO_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID = ContextVar("ZELOO_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)
...
def set_current_session_id(session_id: str) -> None:
    """Synchronize ``ZELOO_SESSION_ID`` across ContextVar and ``os.environ`` (tools read it
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session var by legacy ``ZELOO_SESSION_*`` name; drop-in for os.getenv.  The
...
# Surfaces that are not a human chat channel (gateway binds ZELOO_SESSION_PLATFORM, CLI/TUI/
# desktop bind ZELOO_SESSION_SOURCE, so both are consulted).  Default-deny: an unrecognized
...
def session_is_messaging_surface() -> bool:
    ...
    ``ZELOO_PLATFORM``, then the session platform, then the session source)."""
    platform = os.getenv("ZELOO_PLATFORM") or get_session_env("ZELOO_SESSION_PLATFORM", "")
    idents = (platform, get_session_env("ZELOO_SESSION_SOURCE", ""))
...
def declare_stateless_channel() -> None:
    ...
    See NousResearch/Zeloo-agent#53027 and #63142.
    """
...
def async_delivery_supported() -> bool:
    ...
    (``ZELOO_KANBAN_TASK``: one-shot subprocesses whose parent disappears after the turn)."""
