...
failed approvals, chmod 0600 data files, codes never logged. Storage: ~/.Zeloo/pairing/
"""
...
# path would freeze the ZELOO_HOME/profile context at gateway boot, ignoring later
# context-local overrides, so the gateway and ``Zeloo pairing`` CLI wrote different
...
# computing the path eagerly freezes it to whatever ZELOO_HOME/profile context existed at that exact import
# moment for the rest of the process's lifetime -- even if a context-local override (see
# zeloo_constants.set_zeloo_home_override) is established afterward. A freshly-started, short-lived
# process (e.g. the ``Zeloo pairing`` CLI) re-imports this module later with the final environment already
...
# (NousResearch/Zeloo-agent#93449). ``_default_pairing_dir()`` below resolves this fresh on every call in
...
def _default_pairing_dir() -> Path:
    return PAIRING_DIR if PAIRING_DIR is not None else get_zeloo_dir("platforms/pairing", "pairing")
...
def _write_allowlist_env(env_var: str, ids: list) -> None:
    ...
    with contextlib.suppress(Exception):
        from zeloo_cli.config import save_env_value, remove_env_value
...
def _load_json_file(path: Path) -> dict:
    ...
    try:
        ...
        try:
            ...
            # exec` runs as root and writes the file, then the gateway process — running as `Zeloo` after
        ...
        logger.warning(
            ...
            "If you ran `docker exec <container> Zeloo pairing approve ...` as root, "
            "re-run with `docker exec -u Zeloo <container> ...` and "
            "chown the existing file to the Zeloo user, or restart the "
...
class PairingStore:
    ...
    profile's ZELOO_HOME exactly as ``Zeloo -p <name> pairing ...`` does, so multiplex
    ...
    def __init__(self, profile: Optional[str] = None):
        ...
        self._dir = get_zeloo_dir("platforms/pairing", "pairing", home=profile_home) if profile else _default_pairing_dir()
