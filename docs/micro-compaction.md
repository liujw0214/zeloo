...
has to be thrown away or summarized. Zeloo has always done this in one batch:
...
Zeloo folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Zeloo's normal session flush is
...
With `micro_compact` unset or `false` Zeloo behaves exactly as it always has:
...
Defaults to `$ZELOO_HOME/logs/agent.log`. It reports passes, outcome mix, net
