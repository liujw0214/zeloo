...
or change arbitrary Zeloo configuration.
...
small host fact requested by the RFC. Settings writes use Zeloo'
...
`ZELOO_HOME`, so one globally loaded plugin context follows multiplexed
...
```text
<ZELOO_HOME>/plugin-data/<plugin-data-namespace>/state.json
```
...
The implementation is covered with real temporary-Zeloo-home tests for:
...
- [Issue #64227](https://github.com/NousResearch/Zeloo-agent/issues/64227)
- [RFC PR #58542](https://github.com/NousResearch/Zeloo-agent/pull/58542) by Topher Ross
