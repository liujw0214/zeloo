...
Zeloo supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.Zeloo/config.yaml`:
...
Or set the env var: `ZELOO_KANBAN_DISPATCH_IN_GATEWAY=false`
