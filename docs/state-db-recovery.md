...
`Zeloo doctor` reports the explicit repair command.
...
repaired or restored file. Do not run `Zeloo doctor --fix` while the gateway
is still up. Next steps:

```bash
Zeloo gateway stop
ZELOO_HOME="$HOME/.Zeloo" Zeloo sessions recover --source "$HOME/.Zeloo/state.db" --inspect-only
# if recoverable:
ZELOO_HOME="$HOME/.Zeloo" Zeloo sessions recover --source "$HOME/.Zeloo/state.db" --output "$HOME/recovered-state.db"
```
...
```bash
Zeloo gateway stop
ZELOO_HOME="$HOME/.Zeloo" Zeloo sessions repair --check-only
ZELOO_HOME="$HOME/.Zeloo" Zeloo sessions repair
```
