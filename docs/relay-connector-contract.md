...
This document is the formal interface between the **Zeloo gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `Zeloo gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`Zeloo gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `Zeloo` CLI.
