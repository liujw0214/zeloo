...
The canonical transcript store is SQLite via `SessionDB` (from `zeloo_state`). The
...
Counts consecutive restarts via a JSON file (`{ZELOO_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `Zeloo update`, `Zeloo gateway restart`,
or `/restart`.
