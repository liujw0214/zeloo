...
When running Zeloo inside Docker, the default `network_mode: host` gives the
...
The Zeloo [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   ZELOO_UID=$(id -u) ZELOO_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=Zeloo,Zeloo-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://Zeloo-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Zeloo trust model and vulnerability reporting
