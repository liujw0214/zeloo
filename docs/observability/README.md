# Zeloo Observer Hooks

Zeloo observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Zeloo' planner, model providers, memory, tool registry,
...
Zeloo also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "Zeloo.observer.v1"
```

Hook callbacks are fail-open. Zeloo catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Zeloo only builds
...
The native NeMo Relay SDK integration maps Zeloo session, turn, LLM, and tool
