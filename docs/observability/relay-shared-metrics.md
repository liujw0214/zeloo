...
Zeloo includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Zeloo and does not require a Zeloo observability plugin. Zeloo remains
importable without Relay on other native targets. Those targets use an
explicit reduced-capability no-op host:
Zeloo execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `Zeloo-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

> [!WARNING]
> This removes the Zeloo `observability/nemo_relay` plugin. Existing users
...
> `plugins.toml` selected with `ZELOO_NEMO_RELAY_PLUGINS_TOML`. The legacy
> `ZELOO_NEMO_RELAY_ATOF_*` and `ZELOO_NEMO_RELAY_ATIF_*` variables no
> longer activate exporters. Without the new variable, Zeloo does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

Zeloo requires NeMo Relay 0.8.3 or later within the 0.8 release line. That
line provides the provider-codec and canonical tool-result contracts Zeloo
uses for managed provider and tool calls.

## Runtime Dependency and Data Boundary

Zeloo installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Zeloo process so configured interceptors can
...
Collection remains off unless Zeloo policy enables it:
...
opt-in. Set `ZELOO_NEMO_RELAY_PLUGINS_TOML` to a selected `plugins.toml` to
activate configured middleware, exporters, or dynamic plugins. When the
variable is unset, Zeloo does not invoke Relay's plugin initializer, so Relay
...
selected file cannot be loaded, Zeloo reports the error and does not invoke
...
retain the same `session_id` and add `Zeloo.session.segment` plus
`Zeloo.session.segment_reason` (`compaction` or `max_turns`).

## Process-Wide Plugin Policy and Profile Isolation

Relay plugin configuration is a process-level deployment choice, not a Zeloo
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that Zeloo process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, Zeloo logs:
...
profile's Relay scope stack. Native dynamic plugins are loaded into the Zeloo
process and share the same policy boundary.

Run profiles in separate Zeloo processes when they require different trust
...
Zeloo core owns one Relay host and one isolated Relay session scope per Zeloo
...
marks do not require Zeloo plugin registration. Shared-metrics marks must
...
```text
Zeloo turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Zeloo shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Zeloo sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Zeloo used for the logical call, such as
...
New calls use `Zeloo.model_route.count`. The previous
`Zeloo.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `Zeloo.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; Zeloo
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Zeloo processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `Zeloo.task_run`, parented to
the owning Zeloo session. The start counter contains only bounded execution
...
provider attempts for the same Zeloo API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Zeloo tool-call
...
cancellations. Active task ownership follows the task ID if Zeloo rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`Zeloo.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. Zeloo
...
tool or plugin names. Zeloo does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `Zeloo.tool_approval` marks and
...
Successful skill mutations emit `Zeloo.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `Zeloo.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. Zeloo derives reuse and patch-generation
...
```text
$ZELOO_HOME/telemetry/shared_metrics/metrics.sqlite3
$ZELOO_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the Zeloo version,
...
schema field name, its current scope is one `ZELOO_HOME`, so it is more
...
Deleting `$ZELOO_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `ZELOO_HOME`. To reset it, stop Zeloo
processes and remove `$ZELOO_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real Zeloo CLI turn against the deterministic local model server:
...
`$ZELOO_HOME/telemetry/shared_metrics/outbox/` to the Zeloo telemetry ingest
...
Removing `$ZELOO_HOME/telemetry/shared_metrics` still resets local identity,
