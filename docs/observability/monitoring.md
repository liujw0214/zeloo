...
Zeloo gateway daemon, exported over OTLP/HTTP to an operator-configured
...
capture is a separate plane served by Zeloo's native NeMo Relay SDK
...
| --- | --- | --- |
| Gateway gauges | `/v1/metrics` | `Zeloo.gateway.up/state/busy/drainable/active_agents/background_work/background_delegations/restart_requested`, `Zeloo.platform.up/degraded` with bounded `error_code` attributes |
...
`Zeloo.gateway.active_agents`, `Zeloo.gateway.background_work`, and
`Zeloo.gateway.background_delegations` are complementary. `active_agents`
...
```bash
Zeloo monitoring status
```

The OpenTelemetry SDK is an optional extra (`pip install 'Zeloo-agent[otlp]'`),
...
a plain `Zeloo gateway run`: the exporter lives in the gateway process, so
...
`Zeloo.gateway.up`, `Zeloo.platform.up`, and `Zeloo.platform.degraded`.
...
absent_over_time(zeloo_gateway_up[5m])

# Locally owned bridge is explicitly down.
zeloo_platform_up == 0

# Scheduler thread is stale even though the gateway may still be alive.
zeloo_cron_scheduler_heartbeat_age_seconds > 180

# Ticker loops but has not completed a successful tick recently.
zeloo_cron_scheduler_last_success_age_seconds > 300

# One or more jobs are beyond their existing scheduler grace window.
zeloo_cron_jobs_overdue > 0

# Catch-up counter increased, proving at least one stale occurrence was
# collapsed and run once after a delay.
increase(zeloo_cron_scheduler_catch_up_occurrences[15m]) > 0
```

Cron execution lifecycle records arrive as `Zeloo.cron_execution` spans.
Alert or derive events from bounded attributes such as:

```text
Zeloo.status = failed|unknown
Zeloo.delivery_outcome = failed|not_configured
Zeloo.error_class = auth_failed|rate_limited|timeout|network_error|
...
3. a cron lifecycle feed keyed only by opaque `Zeloo.job_key`;
...
Zeloo Agent-owned Relay transport health remains in scope. A separate gateway
...
python scripts/observability/otel_capture_collector.py \
  --host 127.0.0.1 --port 4318 --log /tmp/zeloo_otel_capture.jsonl
...
python scripts/observability/gateway_health_export_probe.py \
  ...
  --log /tmp/zeloo_otel_capture.jsonl --wait 8
...
`Zeloo.error_class = ...` list in this file's alert section and the enum's unit
...
```bash
Zeloo monitoring status                 # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
...
The `Zeloo monitoring` CLI intentionally exposes `status` only. This first
release covers only Zeloo Agent-owned service-health and operational-diagnostic
signals, including Zeloo Agent-owned Relay transport health. Team Gateway's
