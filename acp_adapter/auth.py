"""ACP auth helpers — detect and advertise Zeloo authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "Zeloo-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Zeloo runtime provider, or None if unavailable.
...
def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Zeloo.

    The ACP registry requires at least one usable auth method in the initial
    handshake. A fresh Zed install may have no Zeloo credentials yet, so the
    ...
    if provider:
        methods.append(AuthMethodAgent(
            ...
            description=f"Authenticate Zeloo using the currently configured {provider} runtime credentials.",
        ))
    methods.append(TerminalAuthMethod(
        id=TERMINAL_SETUP_AUTH_METHOD_ID, name="Configure Zeloo provider", type="terminal", args=["--setup"],
        description=("Open Zeloo' interactive model/provider setup in a terminal. "
                     "Use this when Zeloo has not been configured on this machine yet."),
    ))
    return methods
...
def has_provider() -> bool:
    """Return True if Zeloo can resolve any runtime provider credentials."""
