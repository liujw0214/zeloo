"""ACP session manager — maps ACP sessions to Zeloo AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.Zeloo/state.db``) so they
...
from zeloo_constants import get_zeloo_home, translate_cwd_for_wsl_backend, windows_path_to_wsl
...
def _translate_acp_cwd(cwd: str) -> str:
    ...
    when Zeloo runs in WSL so agents, tools, and persisted sessions agree; no-op elsewhere."""
...
def _expand_acp_enabled_toolsets(toolsets: List[str] | None = None,
    ...
    names = [n for n in (toolsets or ["Zeloo-acp"]) if n]
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Zeloo agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Zeloo AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        the runtime provider config. ``db``: SessionDB; default lazily opens ``~/.Zeloo/state.db``."""
    ...
    def _get_db(self):
        ...
        minimal test env). ``ZELOO_HOME`` is resolved here, not via the import-time
        ``DEFAULT_DB_PATH``, so test fixtures that change the env var later are honoured."""
        if self._db_instance is None:
            try:
                from zeloo_state import SessionDB
                self._db_instance = SessionDB(db_path=get_zeloo_home() / "state.db")
            except Exception:
    ...
    def _make_agent(self, *, session_id: str, cwd: str, model: str | None = None,
        ...
        kwargs = {
            ...
            "enabled_toolsets": _expand_acp_enabled_toolsets(["Zeloo-acp"], mcp_server_names=configured_mcp_servers),
        ...
        # picked up by ZELOOACPAgent._schedule_mcp_late_refresh.
        try:
