"""CLI entry point for the Zeloo-agent ACP adapter.

Loads ``~/.Zeloo/.env``, routes logging to stderr (stdout is reserved for ACP
JSON-RPC), and starts the ACP agent server.

Usage::

    python -m acp_adapter.entry   # or: Zeloo acp / Zeloo-acp
"""

# IMPORTANT: zeloo_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See zeloo_bootstrap.py for full rationale.
try:
    import zeloo_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Partial ``Zeloo update`` (git-reset landed, ``uv pip install -e .`` did not):
    # UTF-8 stdio setup is skipped on Windows; POSIX is unaffected.
    pass
else:
    # Stop a ``utils/``/``proxy/``/``ui/`` package in the launch cwd from shadowing Zeloo modules.
...
def _load_env() -> None:
    """Load .env from ZELOO_HOME (default ``~/.Zeloo``)."""
    from zeloo_cli.env_loader import load_zeloo_dotenv

    zeloo_home = get_zeloo_home()
    loaded = load_zeloo_dotenv(zeloo_home=zeloo_home)
    log = logging.getLogger(__name__)
    for env_file in loaded or ():
        log.info("Loaded env from %s", env_file)
    if not loaded:
        log.info("No .env found at %s, using system env", zeloo_home / ".env")


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="Zeloo-acp", description="Run Zeloo Agent as an ACP stdio server.")
    parser.add_argument("--version", action="store_true", help="Print Zeloo version and exit")
    parser.add_argument("--check", action="store_true", help="Verify ACP dependencies and adapter imports, then exit")
    parser.add_argument("--setup", action="store_true",
                        help="Run interactive Zeloo provider/model setup for ACP terminal auth")
    parser.add_argument("--setup-browser", action="store_true",
                        help="Install agent-browser + Playwright Chromium into ~/.Zeloo/node/ "
...
def _run_check() -> None:
    ...
    from acp_adapter.server import ZELOOACPAgent  # noqa: F401

    print("Zeloo ACP check OK")


def _run_setup() -> None:
    from zeloo_cli.main import main as zeloo_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "Zeloo", "model"]
        zeloo_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting Zeloo-agent ACP adapter")
    ...
    import acp
    from .server import ZELOOACPAgent
    ...
    if os.environ.get("ZELOO_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
