# Zeloo Agent - Development Guide

Instructions for AI coding assistants and developers working on the Zeloo-agent codebase.
...
Zeloo is a personal AI agent that runs the same agent core across a CLI, a messaging
...
Read the balance right: Zeloo ships a **lot**. Most merges are bug fixes to reported
...
- **Expand reach at the edges.** New adapters, channels, providers, models, desktop/TUI/
  ...
  the existing setup/config UX (`Zeloo tools`, `Zeloo setup`, auto-install) rather than
...
- **E2E validation, not just green unit mocks.** Anything touching resolution chains, config
  ...
  real path with real imports against a temp `ZELOO_HOME`. Mocks hide integration bugs.
...
- **New `ZELOO_*` env vars for non-secret config.** `.env` is for secrets only. Behavioral
...
- **Outbound telemetry / usage attribution without opt-in gating.** No analytics,
  ...
  (config gate + setup prompt + `Zeloo tools` toggle) exists. Park behind a label.
...
- **Third-party products integrated into the core tree.** Observability backends, vendor
  ...
  backend we don't own. Ship as a **standalone plugin repo** (`~/.Zeloo/plugins/` or pip
...
2. **CLI command + skill** — config/state/infra expressible as shell commands; the agent runs
   `Zeloo <subcommand>` guided by a skill. Default for subscriptions, scheduled tasks,
   service setup (`Zeloo webhook`, `Zeloo cron`, `Zeloo tools`).
...
4. **Plugin** — third-party/niche/user-specific; lives in `~/.Zeloo/plugins/` or a pip
...
drive a locally spawned backend, one over SSH, one behind URL + token, or Zeloo Cloud, and
only the first two carry `ZELOO_DESKTOP=1`. An env-keyed gate is a silent no-op on the other
topologies — the tool is stripped from the schema while the platform hint tells the model it
is "inside the Zeloo desktop app". The pattern:

- **The toolset is the surface gate.** Keep such tools off `_ZELOO_CORE_TOOLS` and in a named
...
- **Ask which identity you mean.** `ZELOO_DESKTOP=1` legitimately means "this backend was
  spawned by the app" (cron ticker, web-dist handling). It does NOT mean "a GUI is watching";
  the embedded terminal pane (`Zeloo --tui` against that backend) is the counterexample.
...
```
`scripts/run_tests.sh` probes `.venv`, then `venv`, then `$HOME/.Zeloo/Zeloo-agent/venv`
...
├── toolsets.py           # TOOLSETS dict, _ZELOO_CORE_TOOLS
├── cli.py                # ZELOOCLI (REPL, slash dispatch) + zeloo_cli/cli_*_mixin.py
├── zeloo_state.py       # SessionDB facade; zeloo_state_*.py siblings
├── zeloo_constants.py   # get_zeloo_home(), display_zeloo_home() — profile-aware paths
├── zeloo_logging.py     # agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # turn_*.py loop phases, providers, memory, compression, prompt builder
├── zeloo_cli/           # CLI subcommands, setup, config, plugins loader, skins, updater
...
├── ui-tui/               # Ink (React) terminal UI — `Zeloo --tui`
...
**User state:** `~/.Zeloo/config.yaml` (settings), `~/.Zeloo/.env` (secrets only),
`~/.Zeloo/logs/` (`agent.log` INFO+, `errors.log` WARNING+, `gateway.log`); all
profile-aware via `get_zeloo_home()`. Browse logs with `Zeloo logs [--follow] [--level] [--session]`.
...
families: `zeloo_state.py` (21), `gateway/run.py` (15), `tools/mcp_tool.py` (15),
`zeloo_cli/kanban.py` (14), `zeloo_cli/web_server.py` (13 + 24 routers), `zeloo_cli/auth.py`
(12), `tools/browser_tool.py` (11), `cli.py` (12 `zeloo_cli/cli_*_mixin.py`), `run_agent.py`
...
- **Compat pointers are OFF LIMITS in-tree.** Old import paths kept alive for external plugins
  ...
  `-W error::zeloo_cli.plugin_compat.ZELOOPluginCompatWarning` catches them in the suite.
...
- **Never infer process identity from argv substrings** (`"serve" in cmdline`) — the bug class
  ...
  `zeloo_cli.update_cmd._zeloo_holder_subcommand`; flag sets are DERIVED from the parser
  (`_holder_value_flags()`), never hand-written; match FULL cmdlines and truncate only for
  display. Details: `zeloo_cli/AGENTS.md`.
- **Never hardcode `~/.Zeloo`.** `get_zeloo_home()` for code paths, `display_zeloo_home()`
  for user-facing text (both from `zeloo_constants`). Hardcoding breaks profiles (5 bugs in
  PR #3575). Module-level constants are fine — they cache after `_apply_profile_override()`
  sets `ZELOO_HOME`. Profile operations themselves are HOME-anchored
  (`_get_profiles_root()` = `Path.home()/.Zeloo/profiles`) so `Zeloo -p x profile list`
  sees all profiles — intentional, not a bug.
- **Argparse alias dispatch:** `add_parser("list", aliases=["ls"])` sets `dest` to the literal
  the user typed (`"ls"`). Dispatch must accept both (caught PTY-testing `Zeloo webhook ls`).
- **Don't wire in dead code without E2E validation.** Unshipped code was dead for a reason;
  E2E the real resolution chain with real imports against a temp `ZELOO_HOME` first.
...
vars unset, `TZ=UTC`, `LANG=C.UTF-8`, `ZELOO_HOME` → temp dir, and per-file subprocess
...
- **Flake policy:** a failing FILE is retried once in a fresh subprocess (`--file-retries`;
  `ZELOO_TEST_FILE_RETRIES=0` disables). Pass-on-retry is green but printed under `⚠ FLAKY`
...
- **Tests must not write to `~/.Zeloo/`.** The autouse `_isolate_zeloo_home` fixture in
  `tests/conftest.py` redirects `ZELOO_HOME`; never hardcode `~/.Zeloo/` in tests. Profile
  tests also mock `Path.home()` so `_get_profiles_root()` / `_get_default_zeloo_home()` stay
  in the temp dir (pattern: `tests/zeloo_cli/test_profiles.py`):
  ```python
  @pytest.fixture
  def profile_env(tmp_path, monkeypatch):
      home = tmp_path / ".Zeloo"; home.mkdir()
      monkeypatch.setattr(Path, "home", lambda: tmp_path)
      monkeypatch.setenv("ZELOO_HOME", str(home))
      return home
  ```
  Tests that `patch.object(Path, "home", ...)` must ALSO set `ZELOO_HOME` — code reads the
  env var, not `Path.home()/.Zeloo`.
...
`tests/zeloo_cli/test_venv_holder_windows_live.py` on a real `windows-latest` runner (real
...
| `cli.py`, `zeloo_cli/`, `main.py` | `zeloo_cli/AGENTS.md` | CLI mixins, `_SLASH_DISPATCH`, slash registry, config system + loaders, skins, `Zeloo update` pipeline, profiles / multiplex |
...
| `plugins/`, `zeloo_cli/plugins*.py` | `plugins/AGENTS.md` | Plugin kinds, native compat contract, in-tree policy, Sep-2026 compat window |
| `tui_gateway/`, `ui-tui/` | `tui_gateway/AGENTS.md` | Process model, JSON-RPC transport, key surfaces, slash flow, dev commands |
| `web/`, `zeloo_cli/web_routers/` | `web/AGENTS.md` | Dashboard embeds the real TUI; what React may and may not rebuild |
...
`Zeloo-agent-dev` skill, not here.
