...
//!
//! Driven when the installer is launched as `Zeloo-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Zeloo desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `Zeloo update`
//!      or repair bootstrap can race locked files),
//!   2. run `Zeloo update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in zeloo_cli/main.py),
//!   3. run `Zeloo desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `Zeloo update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_ZELOO) and
...
/// `Zeloo update` exit code meaning "another Zeloo process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// zeloo_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `Zeloo update`'s own guard decide.
...
///
/// The marker is also the cross-process update lock: `Zeloo update` claims
/// the same file (see `zeloo_cli/update_lock.py`) so a dashboard-spawned
...
/// overwriting — the pre-fix clobber is what let a dashboard `Zeloo update`
...
/// UPDATE_MARKER_MAX_AGE_SECONDS in zeloo_cli/update_lock.py — all three read
...
/// a dashboard-spawned `Zeloo update`) still blocks.
...
/// the marker name our PID — because a `Zeloo update` child that
...
async fn run_update(app: AppHandle) -> Result<()> {
    let zeloo_home = crate::paths::zeloo_home();
    let install_root = zeloo_home.join("Zeloo-agent");
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_ZELOO`
    ...
    //
    // The same marker is the cross-process update lock (zeloo_cli/
    // update_lock.py claims it too), so a live foreign owner means another
    // updater — most often a dashboard-spawned `Zeloo update` — is already
    ...
    let _update_marker = match UpdateMarkerGuard::acquire(
        ...
        Err(owner) => {
            ...
            let msg = format!(
                "Another Zeloo update is already running (PID {}, started {} ago). \
    ...
    let Zeloo = resolve_ZELOO(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the Zeloo CLI under {}. Is Zeloo installed? \
    ...
    // async on Windows. If it still holds the venv shim, `Zeloo update`
    ...
    // ---- stage 2: Zeloo update -----------------------------------------
    // Pass --branch so `Zeloo update` targets the branch this installer was
    ...
    // without --branch, `Zeloo update` switches the checkout to `main` (a
    ...
    // --force skips `Zeloo update`'s Windows running-exe guard (which would
    ...
    // time `Zeloo update` runs there is no legitimate Zeloo.exe to protect,
    // and the guard would only produce a false "Zeloo is still running" stop.
    //
    ...
    // probe only checks the Zeloo.exe shim and app.asar, so an external venv
    ...
    // below surfaces the correct "close all Zeloo windows" message.
    ...
    // Retry-once for the update-boundary crash. `Zeloo update` lazily imports
    ...
    // second `Zeloo update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Zeloo" state
    // a retry can't fix.
    ...
    // checkout predating the ZELOO_UPDATE_HANDOFF_PID env fix (8c76fe19f)
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Zeloo is still running. Close all Zeloo windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "Zeloo update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::zeloo_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 3: Zeloo desktop --build-only ----------------------------
    // `Zeloo update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `Zeloo update` already does above, and `Zeloo update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `Zeloo desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Zeloo manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_zeloo_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for Zeloo to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            ...
            // Zeloo.exe, so an image-name kill would tear down the app itself.
            emit_log(
                app,
                Some(stage),
                LogStream::Stdout,
                &format!(
                    "[handoff] Zeloo still holding install files ({}); locating backend shims…",
                    format_locked_paths(&locked)
                ),
            );
            let shim = venv_ZELOO(install_root);
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_ZELOO(install_root)];
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("Zeloo.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("Zeloo.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Find processes running the exact `venv\Scripts\Zeloo.exe` shim for this
/// installation. Windows image names are case-insensitive and the desktop is
/// also Zeloo.exe, so matching by image name alone is unsafe.
#[cfg(windows)]
...
/// Spawn `Zeloo <args>` from `cwd`, stream stdout/stderr as Log events on the
...
async fn run_streamed(
    ...
    // the same rule about pipe EOF: `Zeloo update` is precisely the shape that
...
/// Path to the venv Zeloo shim under an install root, regardless of existence.
fn venv_ZELOO(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("Zeloo.exe")
    } else {
        install_root.join("venv").join("bin").join("Zeloo")
    }
}

/// Resolve the Zeloo CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `Zeloo` on PATH.
fn resolve_ZELOO(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_ZELOO(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "Zeloo.exe" } else { "Zeloo" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    let zeloo_home = crate::paths::zeloo_home();
    let mut envs = vec![(
        "ZELOO_HOME".to_string(),
        zeloo_home.as_os_str().to_os_string(),
    )];
    // `Zeloo update` is a Python CLI writing to a pipe here, so CPython
    ...
    // `Zeloo update` child claims that SAME lock (zeloo_cli/update_lock.py).
    ...
    // refuses its parent's marker with exit 2 ("Zeloo is still running")
    // and no number of retries can ever succeed. Keep the variable name in
    // sync with HANDOFF_PID_ENV in zeloo_cli/update_lock.py.
    envs.push((
        "ZELOO_UPDATE_HANDOFF_PID".to_string(),
        OsString::from(std::process::id().to_string()),
    ));
    if let Some(path) = path_with_prepended_entries(&[
        zeloo_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_zeloo_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Zeloo.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.Zeloo-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.Zeloo-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::zeloo_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_zeloo_is_under_install_root() {
        let root = Path::new("/x/Zeloo-agent");
        let shim = venv_ZELOO(root);
    ...
    fn update_child_env_forces_unbuffered_python() {
        let envs = update_child_env(Path::new("/x/Zeloo-agent"));
        assert!(
            envs.iter()
    ...
    fn update_child_env_names_our_pid_for_the_lock_handoff() {
        let envs = update_child_env(Path::new("/x/Zeloo-agent"));
        assert!(
            envs.iter().any(|(k, v)| k == "ZELOO_UPDATE_HANDOFF_PID"
                && v.to_str() == Some(std::process::id().to_string().as_str())),
            "the Zeloo update child claims the same marker we hold; without our pid \
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_ZELOO(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/Zeloo-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn same_windows_path_accepts_case_only_difference() {
        assert!(same_windows_path(
            Path::new(r"C:\Users\tester\.Zeloo\Zeloo-agent\venv\scripts\Zeloo.EXE"),
            Path::new(r"c:\users\tester\.Zeloo\Zeloo-agent\venv\Scripts\Zeloo.exe"),
        ));
    }
    ...
    fn same_windows_path_rejects_desktop_binary() {
        assert!(!same_windows_path(
            Path::new(r"C:\Users\tester\.Zeloo\Zeloo-agent\apps\desktop\Zeloo.exe"),
            Path::new(r"C:\Users\tester\.Zeloo\Zeloo-agent\venv\Scripts\Zeloo.exe"),
        ));
    }
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".Zeloo-update-in-progress");
    ...
    fn acquire_refuses_while_a_live_updater_owns_the_marker() {
        ...
        // dashboard `Zeloo update` and install-mode bootstrap mutate one tree
    ...
    fn acquire_adopts_a_marker_prewritten_with_our_own_pid() {
        // #74761: desktop writeUpdateMarker(ZELOOHome, child.pid) races ahead
    ...
    // checkout's `Zeloo update` reads it as a live foreign update and exits
    ...
    fn missing_or_garbage_marker_never_heals() {
        ...
        let garbage = dir.join(".Zeloo-update-in-progress");
    ...
    fn heal_end_to_end_marker_lifecycle() {
        ...
        // own exit 2, e.g. a genuinely still-running Zeloo, stays terminal).
        assert!(!should_heal_self_marker_refusal(
            Some(UPDATE_EXIT_CONCURRENT),
            &marker
        ));
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Zeloo.app"]),
            Some(PathBuf::from("/Applications/Zeloo.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Zeloo.app");
        let tmp = base.join("Zeloo.app.Zeloo-update-new");
        let old = base.join("Zeloo.app.Zeloo-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Zeloo.app");
        let tmp = base.join("Zeloo.app.Zeloo-update-new"); // intentionally absent
        let old = base.join("Zeloo.app.Zeloo-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Zeloo.app");
        let tmp = base.join("Zeloo.app.Zeloo-update-new"); // absent
        let old = base.join("Zeloo.app.Zeloo-update-old");
        write_marker(&target, "OLD");
