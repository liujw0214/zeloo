...
/// read side does not see EOF until the last of them closes it. `Zeloo update`
...
/// stranded with it.
///
/// Same bound `Invoke-ZELOOStep` grew in `scripts/desktop-update/windows.ps1`
...
///
/// `zeloo_home_override` propagates to the child as $ZELOO_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, zeloo_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = zeloo_home_override {
        cmd.env("ZELOO_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, zeloo_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = zeloo_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_zeloo_home() {
    ...
    /// child. `Zeloo update` is this shape -- the Electron build alone is
    /// megabytes.
    #[cfg(unix)]
    #[tokio::test]
