...
//!
//! Mirrors `zeloo_constants.get_zeloo_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\Zeloo
//!   macOS:   ~/.Zeloo
//!   Linux:   ~/.Zeloo  (override via $ZELOO_HOME)
//!
//! NOTE (macOS): Python's get_zeloo_home(), scripts/install.sh, and the
//! Electron desktop's resolveZELOOHome() ALL use ~/.Zeloo on macOS — there
...
/// Returns the canonical Zeloo home directory, respecting $ZELOO_HOME if set.
pub fn zeloo_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("ZELOO_HOME") {
    ...
    {
        // %LOCALAPPDATA%\Zeloo — matches scripts/install.ps1's $ZELOOHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("Zeloo");
        }
    }

    // macOS + Linux + fallback: ~/.Zeloo (matches Python get_zeloo_home(),
    // install.sh, and the Electron desktop's resolveZELOOHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".Zeloo");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    zeloo_home().join("bootstrap-cache")
}
...
/// ZELOO_HOME so it survives repo checkout deletion (unlike anything under
/// Zeloo-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\Zeloo\Zeloo-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_ZELOO`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under ZELOO_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves ZELOO_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    zeloo_home().join(".Zeloo-update-in-progress")
}
...
pub fn copy_self_to_zeloo_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to ZELOO_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_ZELOO_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".Zeloo-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under ZELOO_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[Zeloo-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("ZELOO_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_zeloo_home() -> String {
    zeloo_home().to_string_lossy().into_owned()
}
