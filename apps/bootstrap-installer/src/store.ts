...
export const $ZELOOHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.Zeloo/logs/bootstrap-installer.log')
    $ZELOOHome.set('~/.Zeloo')
  ...
  try {
    const [logPath, ZELOOHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_zeloo_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $ZELOOHome.set(ZELOOHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Zeloo" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Zeloo-Setup.exe --update);
...
export async function launchZELOODesktop(): Promise<void> {
  ...
  await invoke('launch_zeloo_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Zeloo repository' },
