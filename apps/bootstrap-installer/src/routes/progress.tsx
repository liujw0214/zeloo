...
export default function ProgressScreen({ bootstrap }: ProgressProps) {
  ...
  const title = bootstrap.status === 'completed' ? 'Done' : isUpdate ? 'Updating Zeloo' : 'Setting up Zeloo Agent'

  const description = isUpdate
    ? 'Zeloo is updating to the latest version — this only takes a moment.'
    : 'This is a one-time setup. The Zeloo installer is downloading dependencies and configuring your machine. Subsequent launches will skip this step.'

  const pct = Math.round(progress.fraction * 100)

  return (
    <div className="Zeloo-fade-in flex h-full flex-col">
