...
<p align="center">
  <a href="https://github.com/NousResearch/Zeloo-agent/releases"><img src="https://img.shields.io/badge/Download-macOS%20%C2%B7%20Windows%20%C2%B7%20Linux-FFD700?style=for-the-badge" alt="Download"></a>
...
**The native desktop app for [Zeloo Agent](../../README.md) — the self-improving AI agent from [Nous Research](https://nousresearch.com).** Same agent, same skills, same memory as the CLI and gateway, in a polished native window — chat with streaming tool output, side-by-side previews, a file browser, voice, and settings, no terminal required. Available for **macOS, Windows, and Linux**.

<table>
<tr><td><b>Chat with the full agent</b></td><td>Streaming responses, live tool activity, structured tool summaries, and the same conversation history as every other Zeloo surface.</td></tr>
<tr><td><b>Side-by-side previews</b></td><td>Render web pages, files, and tool outputs in a right-hand pane while you keep chatting.</td></tr>
<tr><td><b>File browser</b></td><td>Explore and preview the working directory without leaving the app.</td></tr>
<tr><td><b>Voice</b></td><td>Talk to Zeloo and hear it back.</td></tr>
...
### Install with Zeloo (recommended)

Already have the Zeloo CLI? Just run:

```bash
Zeloo desktop
```

It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. If Desktop cannot find a usable runtime or saved remote connection, first launch lets you connect to an existing Zeloo gateway or install Zeloo locally. Local onboarding then walks you through choosing a provider and model.

### Prebuilt installers

Prebuilt installers are built and distributed via [the Zeloo Desktop website.](https://Zeloo-agent.nousresearch.com/).
...
```bash
# throwaway ZELOO_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the Zeloo Agent runtime into `ZELOO_HOME`
(`~/.Zeloo`, or `%LOCALAPPDATA%\Zeloo` on Windows), using the same layout as a
CLI install.
...
- **Zeloo Agent** runs as a headless `Zeloo serve` process and exposes the
...
1. `ZELOO_DESKTOP_ZELOO_ROOT`
2. the current source checkout during development
3. a completed managed install
4. `ZELOO_DESKTOP_ZELOO`, or `Zeloo` on `PATH`
5. a system Python that can import the Zeloo runtime
...
Desktop supports a managed local backend, explicit remote gateways, and Zeloo
...
screen offers **Connect to existing Zeloo** before starting the local installer.
...
terminal commands, and file operations run against the remote Zeloo host, not
...
transport- or Zeloo-managed header names such as `Authorization`, `Cookie`,
`Host`, `Origin`, `Referer`, and `X-Zeloo-Session-Token`.
...
Boot logs land in `ZELOO_HOME/logs/desktop.log` (includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure.
...
```powershell
# Force a clean first-launch setup
Remove-Item "$env:LOCALAPPDATA\Zeloo\Zeloo-agent\.Zeloo-bootstrap-complete"
# Rebuild a broken Python venv
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\Zeloo\Zeloo-agent\venv"
```

> The default Zeloo home on Windows is `%LOCALAPPDATA%\Zeloo`. Set the `ZELOO_HOME` env var if you've relocated it.
...
- 📖 [Documentation](https://Zeloo-agent.nousresearch.com/docs/)
