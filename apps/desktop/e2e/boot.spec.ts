 ...
 * packaged binary) with a real `Zeloo serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → Zeloo serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Zeloo title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('Zeloo')
  })
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { ZELOODesktop?: Record<string, unknown> }).ZELOODesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → Zeloo serve is spawned → WS connects → config
