...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'Zeloo E2E'], { cwd: repo })
...
function configureRepoCwd(ZELOOHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(ZELOOHome, mockUrl)
  fs.appendFileSync(path.join(ZELOOHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(ZELOOHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.ZELOOHome, mock.url, repo)
