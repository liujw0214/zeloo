...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  fs.appendFileSync(
    path.join(sandbox.ZELOOHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.ZELOOHome)
  const builder = await RealSessionBuilder.start(sandbox.ZELOOHome)
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.ZELOOHome, mock.url)
  fs.appendFileSync(path.join(sandbox.ZELOOHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
