/**
 * Shared E2E fixtures for the Zeloo desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → Zeloo serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `Zeloo desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `Zeloo-e2e-${prefix}-${Math.random()}`))
  const ZELOOHome = path.join(root, 'Zeloo-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(ZELOOHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(ZELOOHome, 'config.yaml')
 ...
 */
export function writeEnvFile(ZELOOHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(ZELOOHome, '.env')
 ...
 */
function writeEmptyConfig(ZELOOHome: string): void {
  const configPath = path.join(ZELOOHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - ZELOO_HOME → sandbox Zeloo-home (isolated config/sessions)
 *  - ZELOO_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - ZELOO_DESKTOP_IGNORE_EXISTING=1 → don't pick up `Zeloo` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - ZELOO_DESKTOP_ZELOO_ROOT → REPO_ROOT (dev checkout resolution)
 *  - ZELOO_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    ZELOO_HOME: sandbox.ZELOOHome,
    ZELOO_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    ZELOO_DESKTOP_IGNORE_EXISTING: '1',
    ZELOO_DESKTOP_ZELOO_ROOT: REPO_ROOT,
    ZELOO_DESKTOP_APP_NAME: `ZELOOE2E-${Date.now()}`,
    ...
    ZELOO_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated ZELOO_HOME + userData
 * @param env      - the process environment (already has ZELOO_HOME etc.)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.ZELOOHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via ZELOO_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`Zeloo serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.ZELOOHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { ZELOO_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Zeloo backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Zeloo.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Zeloo.app', 'Contents', 'MacOS', 'Zeloo')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'Zeloo')
}
 ...
 * progress without spawning a real Zeloo backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * ZELOO_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    ZELOO_DESKTOP_BOOT_FAKE: '1',
    ZELOO_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + Zeloo-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).ZELOO_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).ZELOO_DESKTOP_ZELOO
  delete (env as Record<string, string | undefined>).ZELOO_DESKTOP_ZELOO_ROOT
