...
test.beforeAll(async () => {
  scratch = await fs.mkdtemp(path.join(os.tmpdir(), 'Zeloo-glass-'))
...
test('glass tabs fade the label beneath the close button without repainting the field', async ({ page }) => {
  ...
  await page.evaluate(() => {
    document.documentElement.setAttribute('data-Zeloo-glass-scope', 'sidebar')
  ...
  await page.evaluate(() => document.documentElement.removeAttribute('data-Zeloo-glass'))
