...
/**
 * E2E smoke tests for the packaged Zeloo desktop app.
 *
 ...
 * (credential stripping, isolated ZELOO_HOME + userData, unique app name).
 *
...
test('window opens with the Zeloo title', async () => {
...
test('HUD composer remains fully inside the transparent window', async () => {
  ...
  await fixture!.page.evaluate(() =>
    (window as typeof window & {
      ZELOODesktop?: { hud?: { open: (options: { sessionId: null }) => Promise<void> } }
    }).ZELOODesktop?.hud?.open({ sessionId: null })
  )
