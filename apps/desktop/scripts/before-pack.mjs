 ...
 * product name (`Zeloo`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Zeloo'
 *
 * This is a hard failure with no obvious cause for the user — `Zeloo desktop`
 ...
 * (the mac-unpacked Zeloo.app bundle) and Windows (win-unpacked), so we
 ...
 * Zeloo.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `Zeloo desktop --build-only` (zeloo_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Zeloo.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Zeloo'}.exe`
