 ...
 *
 * Stamps the Zeloo icon + identity onto the packed Windows Zeloo.exe via
 * rcedit (delegated to set-exe-identity.mjs). This runs for EVERY packed build
 * — first install, `Zeloo desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Zeloo')
 */
...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Zeloo'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Zeloo.exe keeps the stock Electron icon`)
  }
}
