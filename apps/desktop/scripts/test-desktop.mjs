...
// no longer bundles the Zeloo Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Zeloo.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Zeloo'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Zeloo.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Zeloo'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default ZELOO_HOME for non-sandboxed runs -- matches main.ts's
// resolveZELOOHome(). On Windows it's %LOCALAPPDATA%\Zeloo; elsewhere
// it's ~/.Zeloo. The fresh-install sandbox launchFresh() sets its own
// ZELOO_HOME and never touches this.
const DEFAULT_ZELOO_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'Zeloo')
  }
  return path.join(os.homedir(), '.Zeloo')
})()
const VENV_ROOT = path.join(DEFAULT_ZELOO_HOME, 'Zeloo-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'Zeloo-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.ZELOO_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Zeloo-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Zeloo-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Zeloo-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Zeloo-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.ZELOO_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.ZELOO_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const ZELOOHome = path.join(sandbox, 'Zeloo-home')
  ...
  env.ZELOO_DESKTOP_CWD = cwd
  env.ZELOO_DESKTOP_IGNORE_EXISTING = '1'
  env.ZELOO_DESKTOP_TEST_MODE = 'fresh-install'
  env.ZELOO_DESKTOP_USER_DATA_DIR = userDataDir
  env.ZELOO_HOME = ZELOOHome
  delete env.ZELOO_DESKTOP_ZELOO
  delete env.ZELOO_DESKTOP_ZELOO_ROOT
  ...
  console.log(`  ZELOO_HOME: ${ZELOOHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(ZELOOHome, 'Zeloo-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Zeloo Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of zeloo_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'Zeloo-agent', 'zeloo_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Zeloo
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + ZELOO_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  ZELOO_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
