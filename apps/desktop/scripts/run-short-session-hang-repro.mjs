...
const UPSTREAM_URL = 'https://github.com/NousResearch/Zeloo-agent.git'
...
const LEGACY_STORAGE_KEY = 'Zeloo.desktop.inflightTurnJournal.v1'
const LEGACY_MIGRATION_KEY = 'Zeloo.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.ZELOODesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `Zeloo-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const ZELOOHome = join(sandbox, 'Zeloo-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(ZELOOHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(ZELOOHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        ZELOO_DESKTOP_APP_NAME: `ZELOOShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        ZELOO_DESKTOP_ZELOO_ROOT: target.targetRoot,
        ZELOO_DESKTOP_IGNORE_EXISTING: '1',
        ZELOO_DESKTOP_USER_DATA_DIR: userData,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'Zeloo-short-session-ab-'))
  const prepared = []
