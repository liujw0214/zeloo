import { ZELOOApi } from '@/api/client'
import type {
  ZELOOConnection,
  ZELOOReadDirResult,
  ZELOOReadFileTextResult,
  ZELOOSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: ZELOOConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: ZELOOConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<ZELOOReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<ZELOOReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<ZELOOReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<ZELOOReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.ZELOODesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: ZELOOSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
