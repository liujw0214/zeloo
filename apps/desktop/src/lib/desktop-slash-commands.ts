...
/** Why a known Zeloo command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Zeloo profile', surface: action('profile') },
  {
    name: '/skin',
...
function isKnownZELOOSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Zeloo' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownZELOOSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with Zeloo and were never asked for. It is
