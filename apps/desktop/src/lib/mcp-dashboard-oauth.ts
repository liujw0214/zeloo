...
const UPDATE_BACKEND = 'Update the Zeloo backend to support Desktop MCP OAuth callbacks.'
...
export async function completeMcpDesktopOAuth({
  ...
  const bridge = window.ZELOODesktop.mcpOauth

  // A legacy null connection can resolve to a remote registry primary.
  if (!bridge && scope.connectionId !== 'local') {
    throw new Error('Update Zeloo Desktop to support MCP OAuth callbacks.')
  }
  ...
  try {
    ...
    await window.ZELOODesktop.openExternal(authUrl)
    let pollFailures = 0
