...
beforeEach(() => {
  ...
  document.querySelector('Zeloo-watch')?.remove()
  delete (window as unknown as Record<string, unknown>).__ZELOOWatch
})

/** The overlay lives in a closed shadow root, so a test can only see the host. */
const host = () => document.querySelector('Zeloo-watch')
...
const drawn = () =>
  (window as unknown as { __ZELOOWatch: { parts: Record<string, HTMLElement> & { shadow: ShadowRoot } } })
    .__ZELOOWatch.parts
...
describe('watchInPage', () => {
  it('mounts one host on documentElement and reuses it across stages', () => {
    ...
    expect(document.querySelectorAll('Zeloo-watch')).toHaveLength(1)
  ...
  it('rebuilds its chrome when the injected source changes underneath it', () => {
    ...
    const stamp = (value: number) => {
      ;(window as unknown as Record<string, unknown>).__ZELOOWatchTag = value
    }
