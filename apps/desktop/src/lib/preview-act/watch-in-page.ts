...
export function watchInPage(doc: Document, holder: WatchHolder, stage: WatchStage, label?: string): void {
  const win = doc.defaultView as (Window & { __ZELOOWatch?: Record<string, unknown> }) | null
  ...
  const busy = 25
  const state = win.__ZELOOWatch || (win.__ZELOOWatch = {})
  ...
  const build = () => {
    ...
    const stale = doc.querySelectorAll('Zeloo-watch')

    for (let i = 0; i < stale.length; i++) {
      stale[i].remove()
    }

    const host = doc.createElement('Zeloo-watch')
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __ZELOOWatchTag?: number }).__ZELOOWatchTag
