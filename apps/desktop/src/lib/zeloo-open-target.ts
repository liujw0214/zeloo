 ...
 * `Zeloo://` deep links and `host.navigate('/path?…')`.
 *
 ...
 *  - `Zeloo://index-network/intent/1` → `/index-network/intent/1` (plugin-scoped)
 *  - `Zeloo://open/my-page?item=x` → `/my-page?item=x` (generic open)
 *  - `/my-page?item=x` / `#/my-page?item=x` (hash-router paths)
 */

export type ZELOOOpenTarget = string | { href: string } | { path: string; params?: Record<string, string> }

const ZELOO_PROTOCOL = 'Zeloo:'
...
export function normalizeZELOOOpenString(raw: string): string | null {
  ...
  if (trimmed.startsWith('Zeloo://') || trimmed.startsWith(`${ZELOO_PROTOCOL}//`)) {
    try {
      const url = new URL(trimmed)
      const host = url.hostname || ''
      const rest = decodeURIComponent((url.pathname || '').replace(/^\//, ''))

      // Zeloo://open/<path>?… → /<path>?…
      if (host === 'open') {
        if (!rest) {
          return null
        }
      ...
      // Zeloo://index-network/intent/1 → /index-network/intent/1
...
export function resolveZELOOOpenPath(target: ZELOOOpenTarget | null | undefined): string | null {
  if (target == null) {
    return null
  }

  if (typeof target === 'string') {
    return normalizeZELOOOpenString(target)
  }
  ...
  if ('href' in target && typeof target.href === 'string') {
    return normalizeZELOOOpenString(target.href)
  }

  if ('path' in target && typeof target.path === 'string') {
    const base = normalizeZELOOOpenString(target.path)
 ...
 * (`Zeloo://<kind>/<name>?…` → kind/name/params).
 */
export function pathFromZELOODeepLink(kind: string, name: string, params: Record<string, string> = {}): string | null {
  if (!kind || !name) {
    return null
  }

  if (kind === 'open') {
    return resolveZELOOOpenPath({ path: `/${name.replace(/^\//, '')}`, params })
  }
  ...
  return resolveZELOOOpenPath({ path: `/${kind}/${name.replace(/^\//, '')}`, params })
}

/** Convenience for `Zeloo://open/<name>?…` payloads. */
export function pathFromOpenDeepLink(name: string, params: Record<string, string> = {}): string | null {
  return pathFromZELOODeepLink('open', name, params)
}
