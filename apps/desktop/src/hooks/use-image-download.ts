...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'Zeloo:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.ZELOODesktop?.saveImageFromUrl) {
        if (await window.ZELOODesktop.saveImageFromUrl(src)) {
