import type { GatewayWsUrlResult } from '@Zeloo/shared'
import type { TranslucencyState } from '@Zeloo/shared/translucency'

import type { ZELOONotification } from '../electron/notification-types'
...
declare global {
  interface Window {
    ZELOODesktop: {
      ...
      // Resume this session in the user's own terminal emulator (`Zeloo --tui
      ...
      // Zeloo Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new ZELOO_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: ZELOOApiRequest) => Promise<T>
      notify: (payload: ZELOONotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<ZELOOReadFileTextResult>
      ...
      readPluginSource?: (filePath: string) => Promise<ZELOOReadFileTextResult>
      selectPaths: (options?: ZELOOSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<ZELOOPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<ZELOOPreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<ZELOOPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: ZELOOActiveWork) => void
      setTitleBarTheme?: (payload: ZELOOTitleBarTheme) => void
       ...
       *  (ZELOO_GUEST_ONBOARDING=1 or --guest-onboarding). Read-only fact the
      ...
      readDir: (path: string) => Promise<ZELOOReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<ZELOO_HOME>/desktop-plugins),
      ...
      /** LOCAL `<ZELOO_HOME>/logs` (profile-aware) — error card "Open Logs". */
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<ZELOOGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<ZELOOGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<ZELOOGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<ZELOORepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: ZELOOReviewScope, baseRef?: null | string) => Promise<ZELOOReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: ZELOOReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<ZELOOReviewShipInfo>
          ...
          prList: (repoPath: string, branches: string[], numbers?: number[]) => Promise<ZELOORepoPullRequests>
          ...
          fetchPrComment: (repoPath: string, url: string) => Promise<ZELOOPrComment | null>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: ZELOOTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<ZELOOTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: ZELOOWindowState) => void) => () => void
      ...
      onPreviewFileChanged: (callback: (payload: ZELOOPreviewFileChanged) => void) => () => void
...
export interface ZELOOTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface ZELOOTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `Zeloo update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  ZELOORoot?: string
...
export interface ZELOOConnection {
  baseUrl: string
  darwinMajor?: number
...
export interface ZELOOTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface ZELOOActiveWork {
  count: number
  titles: string[]
}

export interface ZELOOWindowState {
  darwinMajor?: number
  isFullscreen: boolean
  isMinimized?: boolean
  isVisible?: boolean
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Zeloo Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Zeloo Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected Zeloo Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // the Zeloo gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered Zeloo Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface ZELOOApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface ZELOOPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface ZELOOReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface ZELOOPreviewWatch {
  id: string
  path: string
}
...
export interface ZELOOGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface ZELOOGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
  isRemote: boolean
...
// preselect it.
export interface ZELOOGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface ZELOORepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface ZELOORepoStatus {
  branch: null | string
...
// turn began.
export type ZELOOReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface ZELOOReviewFile {
  path: string
  added: number
  removed: number
...
export interface ZELOOReviewList {
  files: ZELOOReviewFile[]
...
export interface ZELOOReviewPr {
  url: string
  state: string
  number: number
}
...
export interface ZELOOBranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface ZELOORepoPullRequests {
  ghReady: boolean
  prs: ZELOOBranchPullRequest[]
}
...
export interface ZELOOPrComment {
  author: string
  body: string
  diffHunk: string
  kind: 'issue' | 'review'
...
export interface ZELOOReviewShipInfo {
  ghReady: boolean
  pr: ZELOOReviewPr | null
}

export interface ZELOOReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface ZELOOReadDirResult {
  entries: ZELOOReadDirEntry[]
  error?: string
}

export interface ZELOOPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface ZELOOSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
