...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Zeloo doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in zeloo_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `Zeloo model` uses — instead of
...
export type ZELOOConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `Zeloo model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// non-desktop) Zeloo simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (Zeloo.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors zeloo_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Zeloo, 'hub' = installed. */
  provenance?: 'agent' | 'bundled' | 'hub'
}

/** One entry of the built-in optional-skills catalog (optional-skills/ in the
 *  repo) — official skills that ship with Zeloo but install on demand. */
...
export interface ToolProvider {
   ...
   *  CLI `Zeloo tools` active-provider detection). */
  is_active: boolean
 ...
 *  not Zeloo); elsewhere, driver health from `cua-driver doctor`. `null`
...
export interface ActionStatusResponse {
  ...
  pid: number | null
  running: boolean
  /** Zeloo-update only: durable completion identity recovered from update.log. */
  action_id?: string
  /** Zeloo-update only: summary of the durable update receipt (#91277 bullet 3) —
...
/** Shape of `GET /api/Zeloo/update/check` — the backend's own update state.
