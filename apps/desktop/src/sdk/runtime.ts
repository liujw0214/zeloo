 ...
 * plugins resolve `@Zeloo/plugin-sdk` through the vite alias; RUNTIME-loaded
...
// `sdk/runtime` — so a module-scope `{ __ZELOO_PLUGIN_SDK__: sdk, … }` is
...
// `Object.keys(GLOBALS.__ZELOO_PLUGIN_SDK__)` threw
...
function pluginNamespaces() {
  return {
    __ZELOO_PLUGIN_SDK__: sdk,
    __ZELOO_REACT__: React,
    __ZELOO_REACT_JSX__: jsxRuntime,
    __ZELOO_REACT_JSX_DEV__: jsxDevRuntime
  }
}
...
export function sdkImportMap(): Record<string, string> {
  cached ??= {
    '@Zeloo/plugin-sdk': shimUrl('__ZELOO_PLUGIN_SDK__'),
    'react/jsx-dev-runtime': shimUrl('__ZELOO_REACT_JSX_DEV__'),
    'react/jsx-runtime': shimUrl('__ZELOO_REACT_JSX__'),
    react: shimUrl('__ZELOO_REACT__')
  }
