...
vi.mock('@/Zeloo', () => ({ deleteProfile: vi.fn(), getLogs: vi.fn(), getStatus: vi.fn(), ZELOOApi: vi.fn() }))
...
const { deleteProfile, ZELOOApi } = await import('@/Zeloo')
...
describe('connection-aware plugin host APIs', () => {
  it('retires a profile gateway before deleting it', async () => {
    ...
    // profile's backend, re-creating its ZELOO_HOME (#94235).
  ...
  it('refreshes the profile inventory before asking Electron for routes', async () => {
    ...
    ;(window as unknown as { ZELOODesktop: unknown }).ZELOODesktop = { getProfileRoutes }
  ...
  it('reads and hides persisted sessions through the source primary without activating the profile', async () => {
    ...
    expect(ZELOOApi).toHaveBeenNthCalledWith(
      1,
    ...
    expect(ZELOOApi).toHaveBeenNthCalledWith(2, {
      connectionId: 'source-a',
      path: '/api/sessions/bot-chat',
      method: 'PATCH',
      body: { hidden: true, profile: 'backend-worker' }
    })
    expect(vi.mocked(ZELOOApi).mock.calls.every(([request]) => !('profile' in request))).toBe(true)
