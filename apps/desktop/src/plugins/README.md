...
Drop a `<name>/plugin.{ts,tsx}` here that default-exports a `ZELOOPlugin` and
...
`$ZELOO_HOME/desktop-plugins/<name>/plugin.js` (the disk door) — see the
`Zeloo-desktop-plugins` skill.
