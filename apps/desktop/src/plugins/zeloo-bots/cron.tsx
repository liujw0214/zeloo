/**
 * Routines: the Zeloo cron jobs scoped to the bot you're chatting with — the
...
// Zeloo -p <bot> chat delegation wrapper so the run reaches that bot's
...
export function routinePrompt(
  ...
  return (
    ...
    `Zeloo -p ${shellQuote(bot)} chat -c ${shellQuote(`Routine: ${title}`)} -q ${shellQuote(`[Scheduled routine] ${instruction}`)}\n\n` +
...
// Zeloo-native schedule string; Advanced exposes it raw.
...
/** Compose the Zeloo schedule string from picker state. */
