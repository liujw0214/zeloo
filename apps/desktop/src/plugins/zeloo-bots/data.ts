...
import { atom, host, queryClient, useQuery, useValue } from '@Zeloo/plugin-sdk'
...
export const BOT_ATTENTION_HINTS: Record<string, string> = {
  ...
  missing_config: 'Provider not configured — run Zeloo model',
 ...
 *  protocol into the system prompt itself (Zeloo-agent bot_mode_probe).
...
export function useRoster() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Multi-source desktops (Zeloo-agent #86875) also expose the union
 ...
 *  callable alias is 'Zeloo' — the mention middleware resolves it back to
...
export function botHandle(name: string, bot?: Partial<RosterRow> | null): string {
  ...
  return (name || '').trim().toLowerCase() === 'default' ? 'Zeloo' : name
}

/** Taggable @-forms derived from a bot's friendly names — the core profile
 *  display name (`Zeloo profile rename`) and the Bot Mode title. Free text
 ...
 *  (researchbuddy). Reserved tokens are dropped so a bot renamed "Zeloo"
 *  can never hijack the primary profile's @Zeloo alias. */
...
export function botFriendlyNames(bot: Partial<RosterRow> | null | undefined): Array<null | string | undefined> {
  ...
  return [bot?.ui_meta?.['Zeloo-bots']?.title, localTitle, aliasTitle, bot?.title, bot?.display_name]
}
...
export function newBotChat(bot: RosterRow) {
  if (typeof host.newChat !== 'function') {
    host.notify?.({
      kind: 'error',
      message:
        getPluginCtx()?.i18n?.t('bot.openAnotherChatUnsupported') ?? 'Update Zeloo Desktop to open another Bot chat.'
    })
...
export function resolveRosterMentions(
  ...
  for (const match of prose.matchAll(/(^|\s)@([a-z0-9][a-z0-9_-]*)/gi)) {
    ...
    if (token === 'Zeloo') {
      token = byForm.has('Zeloo') ? 'Zeloo' : token
    }
