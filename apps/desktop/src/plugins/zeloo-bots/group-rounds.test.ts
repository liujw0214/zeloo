...
describe('routing', () => {
  ...
  it('resolves @Zeloo to the default member', async () => {
    const { rounds } = await loadRoom()
    ...
    const parsed = rounds.parseGroupChatMentions('@Zeloo take a look', members)
...
describe('turn prompt', () => {
  it('addresses the default profile as @Zeloo', async () => {
    const { rounds } = await loadRoom()
    ...
    expect(own).toMatch(/You are @Zeloo,/)
    ...
    expect(peer).toMatch(/group chat with @Zeloo/)
  })
