...
vi.mock('@Zeloo/plugin-sdk', () => ({ host: hostMock, LruCache: UnboundedCache }))

vi.mock('./data', () => ({
  botHandle: (name: string) => (name === 'default' ? 'Zeloo' : name),
...
describe('the roster loop pushes the OTHER connections’ agents', () => {
  it('gives each gateway a union roster that excludes its own agents', async () => {
    ...
    expect(syncs[1].params.agents).toEqual([
      expect.objectContaining({ connection_id: 'a', handle: 'Zeloo', profile: 'default' })
    ])
