...
import { host, LruCache } from '@Zeloo/plugin-sdk'
...
// to (local, remote URL, SSH, Zeloo Cloud, docker) must be able to find
...
// zeloo_cli/config_defaults.py and tools/bot_relay.py and fails if the
  ...
  try {
    ...
    return profiles
      .map(profile => ({
        ...
        title: String(profile?.ui_meta?.['Zeloo-bots']?.title || profile?.display_name || ''),
