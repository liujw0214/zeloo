...
import { forgetSessionUnread, host, queryClient } from '@Zeloo/plugin-sdk'
...
export function mergeServerMeta(roster: RosterRow[], fetchedAt = 0) {
  ...
  for (const bot of roster) {
    const server = bot.ui_meta?.['Zeloo-bots']
...
/** Permanently delete a bot's Zeloo profile, then remove plugin-local state
 ...
 * directory (Zeloo-agent#52279). That is the "can't delete a bot" error. */
