...
export function displayName(bot: Partial<RosterRow>, meta?: BotMeta | null): string {
  ...
  // "Zeloo". Annotated active rows carry sourceScoped too, and keying this
  ...
  // Core-profile display name (profile.yaml, set via `Zeloo profile rename
  ...
  // never generic "Zeloo" or a hostname-derived label.
  if (alias) {
  ...
  // that reads like nobody bothered. Present it as Zeloo (the agent it is)
