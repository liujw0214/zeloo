...
import { atom, host } from '@Zeloo/plugin-sdk'
...
const GROUP_CHAT_SYNC_META_KEY = 'Zeloo-bots-groups'
...
/** The v3 envelope stored under the default profile's `Zeloo-bots-groups`
 ...
 *  clients (Zeloo Go, headless backends) see rooms regardless of which
 ...
 *  renamed bot never shows up as its raw profile id or a stale "Zeloo"
 *  (community report, Aug 21 2026: renamed default still read "Zeloo is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as Zeloo (matching displayName and the
 *  @Zeloo handle) so the main agent never loses its name in rooms. */
export function groupSpeakerLabel(name?: null | string) {
  ...
  // Core profile display_name (`Zeloo profile rename …` / dashboard) from
  ...
  return trimmed.toLowerCase() === 'default' ? 'Zeloo' : trimmed
}
