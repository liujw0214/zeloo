...
vi.mock('@Zeloo/plugin-sdk', async () => {
...
vi.mock('./shared', () => ({ getPluginCtx: () => null, ID: 'Zeloo-bots' }))
...
describe('the @handle a bot answers to', () => {
  it('presents the primary profile as @Zeloo — "default" never surfaces in the UI', () => {
    expect(botHandle('default')).toBe('Zeloo')
...
describe('renamed bots stay taggable', () => {
  // Discord report, Aug 2026: renaming a bot — Bot Mode title or `Zeloo
  ...
  it('drops reserved tokens so a rename cannot hijack a built-in tag', () => {
    expect(mentionNameForms('Zeloo')).toEqual([])
  ...
  it('inserts the renamed slug, falling back to the profile handle', () => {
    ...
    expect(botMentionTag(row({ name: 'default' }))).toBe('Zeloo')
    // display_name (`Zeloo profile rename`) drives the tag too.
...
describe('resolving @mentions against the roster', () => {
  ...
  it('never treats @Zeloo in your own chat as a handoff to yourself', () => {
    expect(resolveRosterMentions('@Zeloo do it', roster, { connectionId: 'local', name: 'default' })).toEqual([])
    // From ANOTHER bot's chat the same tag is a real handoff.
    expect(
      resolveRosterMentions('@Zeloo do it', roster, { connectionId: 'mac-mini', name: 'dixie' }).map(bot => bot.name)
    ).toEqual(['default'])
  })
...
describe('roster search narrows without re-ranking', () => {
  ...
  it('matches profile handles and preserves roster order', () => {
    ...
    expect(filterBots(roster, meta, '@Zeloo').map(bot => bot.name)).toEqual(['default'])
...
describe('needs-attention badge (#93091 item 3)', () => {
  ...
  it('classifies the raw gateway error strings current backends emit', () => {
    ...
    expect(reasonFor('No LLM provider configured. Run Zeloo model to pick one.')).toBe('missing_config')
