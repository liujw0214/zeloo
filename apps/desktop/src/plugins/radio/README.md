...
Bundled Zeloo Desktop plugin, **off by default**. Enable **Radio** in
...
`plugin.js` imports only `@Zeloo/plugin-sdk`, `react`, and
...
`$ZELOO_HOME/desktop-plugins/radio/plugin.js` for development. Do not install
that duplicate alongside the bundled version. Layout styles use Zeloo theme
...
Stream availability, regional restrictions, and metadata vary. Zeloo is not
