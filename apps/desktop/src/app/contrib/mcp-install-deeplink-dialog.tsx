...
import { getZELOOConfigRecord, saveMcpServers } from '@/Zeloo'
...
import { setZELOOConfigCache } from '../hooks/use-config-record'

/**
 * Explicit-confirm gate for `Zeloo://mcp/install` deep links. The payload is
 ...
 * confirming lets Zeloo spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getZELOOConfigRecord())
      ...
      setZELOOConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))
