...
import { getLatestSessionMessages } from '@/Zeloo'
...
import { useZELOOConfig } from '../session/hooks/use-Zeloo-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  // Without this pin, the owner ladder sends session.create to Zeloo-setup
  ...
  const { refreshZELOOConfig, sttEnabled, voiceMaxRecordingSeconds } = useZELOOConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('Zeloo:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('Zeloo:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshZELOOConfig(true)
    void refreshActiveProfile()
    resetProjectTreeState()
  }, [gatewayScope, refreshCurrentModel, refreshZELOOConfig])
  ...
  const configRecord = useZELOOConfigRecord()
