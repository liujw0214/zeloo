...
const ZELOO_PATHS_MIME = 'application/x-Zeloo-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(ZELOO_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(ZELOO_PATHS_MIME)
  ...
  const getPath = window.ZELOODesktop?.getPathForFile
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.ZELOODesktop?.terminal
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (Zeloo --tui, vim) grab the mouse, so a plain drag
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.ZELOODesktop?.readClipboard?.()) ?? ''
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[Zeloo-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.ZELOODesktop?.terminal?.write(sessionId, `${command}\r`)
