...
import type { ZELOOReadDirEntry, ZELOOReadDirResult } from '@/global'
...
export type ProjectTreeEntry = ZELOOReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: ZELOOReadDirEntry) {
...
async function filterIgnored(entries: ZELOOReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<ZELOOReadDirResult> {
  if (!window.ZELOODesktop) {
