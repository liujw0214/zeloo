...
describe('PluginsTab', () => {
  ...
  it('renders a unified package as ONE row with a Desktop switch and an Agent switch', () => {
    $pluginRecords.set({
      media: { id: 'media', name: 'Media Studio', kind: 'disk', status: 'loaded', packageName: 'Zeloo-media-studio' }
    })
    $agentPlugins.set([
      {
        description: '',
        key: 'Zeloo-media-studio',
        name: 'Zeloo-media-studio',
        source: 'git',
        status: 'disabled',
        version: '1'
      }
  ...
  it('offers "Install here" for a desktop half whose agent half is not in the selected profile', async () => {
    $pluginRecords.set({
      media: {
        ...
        kind: 'disk',
        status: 'loaded',
        packageName: 'Zeloo-media-studio',
        packageOrigin: { repo: 'https://github.com/NousResearch/Zeloo-media-studio.git', sha: 'abc' }
      }
    })
    ...
    await waitFor(() => {
      expect($pluginInstallRequest.get()).toMatchObject({
        legacyHint: 'agent',
        profile: 'workbot',
        repo: 'https://github.com/NousResearch/Zeloo-media-studio.git',
        sha: 'abc'
      })
    })
  })
  ...
  it('opens the dual-target install modal from a catalog pick message', async () => {
    ...
    window.dispatchEvent(
      new MessageEvent('message', {
        ...
        origin: 'https://Zeloo-agent.nousresearch.com'
      })
    )
