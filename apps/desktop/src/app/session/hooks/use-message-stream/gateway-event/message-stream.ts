import type { BillingBlock } from '@Zeloo/shared'
 ...
 * billing-specific toast — never the generic "Zeloo error" — with a smart CTA
  ...
  if (event.type === 'message.complete') {
    ...
    const failure =
      payload?.status === 'error'
        ? {
            error: coerceGatewayText(payload.error).trim() || finalText || 'Zeloo reported an error',
