import { registryBackendScopeKey } from '@Zeloo/shared'
...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  const { activeSessionIdRef, compactedTurnRef, refreshZELOOConfig, sessionStateByRuntimeIdRef } = deps
  ...
  // refreshZELOOConfig — two REST calls (config + defaults) per event, per
