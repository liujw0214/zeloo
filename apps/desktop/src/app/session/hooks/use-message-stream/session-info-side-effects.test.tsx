...
let refreshZELOOConfig: ReturnType<typeof vi.fn<() => Promise<void>>>
...
beforeEach(() => {
  sessionStates = null
  refreshZELOOConfig = vi.fn<() => Promise<void>>(async () => undefined)
...
describe('session.info config refetch gating', () => {
  it('coalesces active-session bursts into one trailing config fetch', async () => {
    ...
    expect(refreshZELOOConfig).not.toHaveBeenCalled()
