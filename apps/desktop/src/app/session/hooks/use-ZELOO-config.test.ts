...
import { getZELOOConfig } from '@/Zeloo'
...
import { useZELOOConfig } from './use-Zeloo-config'

vi.mock('@/Zeloo', () => ({
  getZELOOConfig: vi.fn(),
  getZELOOConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'Zeloo.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getZELOOConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getZELOOConfig>>)

describe('useZELOOConfig refreshZELOOConfig', () => {
  beforeEach(() => {
  ...
  // `agent.reasoning_effort: high` to Zeloo' built-in medium.
  it('publishes the profile default effort even when a manual pick blocks the composer reseed', async () => {
    ...
    const { result } = renderHook(() => useZELOOConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshZELOOConfig()
    })
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useZELOOConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getZELOOConfig>>>()
    vi.mocked(getZELOOConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useZELOOConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshZELOOConfig(true)
    })
    expect(getZELOOConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getZELOOConfig>>)
  ...
  it('does not publish config after its switch loses ownership', async () => {
    const staleConfig = deferred<Awaited<ReturnType<typeof getZELOOConfig>>>()
    vi.mocked(getZELOOConfig).mockReturnValueOnce(staleConfig.promise)
    const { result } = renderHook(() => useZELOOConfig({ activeSessionIdRef: { current: null } }))
    let ownsSwitch = true

    let refresh!: Promise<void>
    act(() => {
      refresh = result.current.refreshZELOOConfig(false, () => ownsSwitch)
    })
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getZELOOConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getZELOOConfig>>>()
    vi.mocked(getZELOOConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshZELOOConfig(true)
      refreshC = result.current.refreshZELOOConfig(true)
    })
