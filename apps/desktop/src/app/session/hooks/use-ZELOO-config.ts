...
import { getZELOOConfig, getZELOOConfigDefaults } from '@/Zeloo'
...
interface ZELOOConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useZELOOConfig({ activeSessionIdRef }: ZELOOConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshZELOOConfig = useCallback(
    async (force = false, shouldPublish: () => boolean = () => true) => {
      ...
      try {
        const [config, defaults] = await Promise.all([getZELOOConfig(), getZELOOConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying Zeloo' built-in medium over the user's config.
        if (!canPublish()) {
          return
        }
  ...
  return { refreshZELOOConfig, sttEnabled, voiceMaxRecordingSeconds }
}
