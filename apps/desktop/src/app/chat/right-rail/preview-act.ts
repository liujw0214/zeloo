...
const preamble = () => `  var w = window;
  ...
  w.__ZELOOActHolder = w.__ZELOOActHolder || {};
  w.__ZELOOAct = ${actEngineSource()};
  w.__ZELOOWatch_fn = (${watchInPage.toString()});
  w.__ZELOOWatchTag = ${WATCH_TAG};
  var holder = w.__ZELOOActHolder;
  var act = function (a) { return w.__ZELOOAct(document, holder, a); };
  var watch = function (stage, label) {
    try { w.__ZELOOWatch_fn(document, holder, stage, label); } catch (err) {}
  };
...
${preamble()}
  ...
  document.addEventListener('pointerdown', function (e) {
    w.__ZELOOHit = { tag: e.target ? e.target.tagName : '?', trusted: e.isTrusted === true };
...
function hitNote(hit?: { tag: string; trusted: boolean } | null): string | undefined {
  return hit && hit.tag === 'Zeloo-WATCH' ? 'The action overlay intercepted the click instead of the page.' : undefined
}
