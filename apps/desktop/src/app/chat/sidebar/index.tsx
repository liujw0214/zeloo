...
import { searchSessions, type SessionInfo, type SessionSearchResult } from '@/Zeloo'
...
// highlight markers (sqlite snippet() delimiters — see zeloo_state_search.py).
...
export function ChatSidebar({
  ...
  useEffect(() => {
    ...
    window.addEventListener('Zeloo:new-session-shortcut', onShortcut)

    return () => {
      window.removeEventListener('Zeloo:new-session-shortcut', onShortcut)
      clearTimeout(timeout)
    }
  }, [])
  ...
  // A turn can `git worktree add/remove` in the terminal (e.g. you ask Zeloo to
