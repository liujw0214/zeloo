...
import type { SessionInfo } from '@/Zeloo'
...
// zero Zeloo sessions, and those arrive continuously — so once the user dragged
...
export function useRepoWorktreeMap(
  repoPaths: string[],
  enabled: boolean
): [Record<string, ZELOOGitWorktree[]>, boolean] {
  const [map, setMap] = useState<Record<string, ZELOOGitWorktree[]>>({})
