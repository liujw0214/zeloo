...
import type { DesktopBootProgress, ZELOOConnection } from '@/global'
import { ZELOOGateway } from '@/Zeloo'
...
export function primaryRuntimeConnectionId(connection: Pick<ZELOOConnection, 'connectionId' | 'mode'>): null | string {
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.ZELOODesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: ZELOOGateway | null) => void
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: ZELOOConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Zeloo...". Once the
    ...
    const attemptReconnect = async (manual?: { profile: string; activationEpoch: number }) => {
      ...
      try {
        ...
        // "Starting Zeloo…". The probe is a no-op for a healthy or local backend.
        ...
        const conn = await withTimeout(
          ...
          RECONNECT_ATTEMPT_TIMEOUT_MS,
          'Timed out reconnecting to Zeloo backend'
        )
        ...
        // as an opaque "Could not connect to Zeloo gateway". resolveGatewayWsUrl
        ...
        if (!manual || (isActivePrimary() && gatewayActivationEpoch() === manual.activationEpoch)) {
          reconcileBusyStatesOnReconnect()
          await callbacksRef.current.refreshZELOOConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshZELOOConfig(false, ownsSwitch).catch(() => undefined),
    ...
    const onBootProgress = (payload: DesktopBootProgress) => {
      ...
      // Soft switch / post-boot startZELOO re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new ZELOOGateway()
    ...
    const offGatewayReconnect = registerGatewayReconnect(async () => {
      ...
      if (!isActivePrimary()) {
        ...
        if (!(await ensureActiveGatewayOpen())) {
          throw new Error('Zeloo gateway is not connected')
        }
    ...
    async function boot() {
      ...
      try {
        ...
        // reaches the recovery affordance instead of hanging "Starting Zeloo…".
        const wsUrl = await withTimeout(
          resolveGatewayWsUrl(desktop, conn),
          RECONNECT_ATTEMPT_TIMEOUT_MS,
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshZELOOConfig(),
          ...
          // retry it), not brick boot behind the "Zeloo couldn't start"
