import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@Zeloo/shared'
...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as ZELOOGateway | null
  const gatewayRef = useRef<ZELOOGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.ZELOODesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<ZELOOGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as ZELOOGateway | null
      }),
    []
  )
