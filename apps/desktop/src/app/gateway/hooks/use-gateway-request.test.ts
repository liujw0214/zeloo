...
vi.mock('@/Zeloo', async importOriginal => {
  const actual = await importOriginal<typeof ZELOOModule>()

  class FakeZELOOGateway {
  ...
  return { ...actual, ZELOOGateway: FakeZELOOGateway }
})
...
const fakeGateway = { connectionState: 'open' } as unknown as ZELOOGateway
...
async function activateRemoteGateway() {
  ...
  setPrimaryGateway(primary as unknown as ZELOOGateway, 'default')
  $gateway.set(primary as unknown as ZELOOGateway)
...
describe('useGatewayRequest', () => {
  ...
  it('surfaces a missing optional scoped mint bridge without falling back to a stale ticket or local lookup', async () => {
    const { desktop, gateway } = await activateRemoteGateway()
    Reflect.deleteProperty(window.ZELOODesktop, 'getGatewayWsUrlFor')
  ...
  it('rejects instead of hanging forever when the reconnect getConnection() wedges (#93454)', async () => {
    ...
    } as unknown as ZELOOGateway

    const getConnection = vi.fn(() => new Promise(() => undefined))

    ;(window as unknown as { ZELOODesktop: unknown }).ZELOODesktop = { getConnection }
    $gateway.set(dropped)
