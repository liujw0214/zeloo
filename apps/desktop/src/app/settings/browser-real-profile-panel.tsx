...
import { type ProfileScope, saveZELOOConfigRecord } from '@/Zeloo'
 ...
 * the default browser's profile (cookies/logins) into a Zeloo-owned copy;
...
export function BrowserRealProfilePanel({ profile }: BrowserRealProfilePanelProps) {
  ...
  const { data: config } = useZELOOConfigRecord(profile)
  ...
  const toggle = useCallback(
    async (on: boolean) => {
      ...
      try {
        await saveZELOOConfigRecord(next, profile)
        notify({
          kind: 'info',
