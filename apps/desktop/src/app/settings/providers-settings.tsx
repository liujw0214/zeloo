...
import { disconnectOAuthProvider, listOAuthProviders } from '@/Zeloo'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/Zeloo'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.ZELOODesktop?.terminal)
...
//      in zeloo_cli/provider_catalog.py) — the SAME provider identity
//      `Zeloo model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Zeloo can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Zeloo can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Zeloo never deletes creds another tool owns behind a silent API call.
