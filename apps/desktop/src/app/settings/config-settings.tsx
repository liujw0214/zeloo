...
import { getElevenLabsVoices, getZELOOConfigSchema, saveZELOOConfig } from '@/Zeloo'
...
import type { ConfigFieldSchema, ZELOOConfigRecord } from '@/types/Zeloo'

import { ZELOOConfigCacheWriter, useZELOOConfigRecord } from '../hooks/use-config-record'
...
function ConfigSettingsInner({
  ...
  const [config, setConfig] = useState<ZELOOConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useZELOOConfigRecord(scopeProfile)
  ...
  const writeConfigCache = useMemo(() => ZELOOConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const {
    ...
    queryKey:
      scopeProfile == null ? ['Zeloo-config-schema'] : ['Zeloo-config-schema', normalizeProfileKey(scopeProfile)],
    queryFn: () => getZELOOConfigSchema(scopeProfile),
    staleTime: 5 * 60 * 1000
  })
  ...
  // never touched — possibly changed out-of-band by `Zeloo config set`
  // while this page sat open — is never resent with its stale value.
  const configBaselineRef = useRef<ZELOOConfigRecord | null>(null)
    ...
    const t = window.setTimeout(() => {
      ...
      saveQueueRef.current = saveQueueRef.current.then(async () => {
        try {
          ...
          const result = await saveZELOOConfig(patch, scopeProfile)
  ...
  const applyConfig = (next: ZELOOConfigRecord) => {
    saveVersionRef.current += 1
    setConfig(next)
    setSaveVersion(saveVersionRef.current)
  }

  const updateConfig = (next: ZELOOConfigRecord) => {
