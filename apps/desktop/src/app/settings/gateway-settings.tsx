...
// Zeloo Cloud discovery lifecycle for the cloud-mode panel.
...
export function GatewaySettings({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const setKeychainEncryption = async (on: boolean) => {
    ...
    try {
      const res = await window.ZELOODesktop.setSecretStorageEncryption(on)
  ...
  useEffect(() => {
    if (state.mode !== 'ssh' || !window.ZELOODesktop?.sshConfigHosts) {
      return
    }
  ...
  const payload = (allowPlainTextToken?: boolean) => ({
    ...
    sshRemoteZELOOPath: state.sshRemoteZELOOPath.trim(),
  ...
  const performSave = async (apply: boolean, allowPlainTextToken: boolean) => {
    ...
    try {
      const next = apply
        ? await window.ZELOODesktop.applyConnectionConfig(payload(allowPlainTextToken))
        : await window.ZELOODesktop.saveConnectionConfig(payload(allowPlainTextToken))
      ...
      const errors = {
        ...
        'Zeloo-not-found': g.sshErrNotInstalled,
  ...
  const signIn = async () => {
    ...
    try {
      ...
      const saved = await window.ZELOODesktop.saveConnectionConfig({
        mode: state.mode,
      ...
      const result = await window.ZELOODesktop.oauthLoginConnectionConfig(trimmedUrl)
      ...
      if (result.connected) {
        const refreshed = await window.ZELOODesktop.getConnectionConfig(null)
  ...
  const signOut = async () => {
    ...
    try {
      await window.ZELOODesktop.oauthLogoutConnectionConfig(trimmedUrl)
      const refreshed = await window.ZELOODesktop.getConnectionConfig(null)
  ...
  const resolveSshHost = async (host: string) => {
    if (!host || !window.ZELOODesktop?.sshResolveHost) {
      return
    }
    ...
    try {
      const resolved = await window.ZELOODesktop.sshResolveHost(host)
  ...
  const testSsh = async () => {
    ...
    try {
      const result = await window.ZELOODesktop.testConnectionConfig(payload())
  ...
  const testRemote = async () => {
    ...
    try {
      const result = await window.ZELOODesktop.testConnectionConfig({
        mode: 'remote',
        remoteAuthMode: authMode,
        remoteToken: authMode === 'token' ? remoteToken.trim() || undefined : undefined,
        remoteUrl: trimmedUrl
  ...
  if (!window.ZELOODesktop?.getConnectionConfig) {
  ...
  return (
    <SettingsContent bare={embedded}>
      ...
      {/* Zeloo Cloud panel: one portal sign-in, then a discovered-agent picker
          ...
          <ListRow
            action={
              <Input
                ...
                placeholder="https://gateway.example.com/Zeloo"
        ...
        <div className="mt-5 grid gap-1">
          ...
          <ListRow
            action={
              <Input
                className={cn('h-8 font-mono', CONTROL_TEXT)}
                onChange={event => setState(current => ({ ...current, sshRemoteZELOOPath: event.target.value }))}
                placeholder={g.sshZELOOPathPlaceholder}
                value={state.sshRemoteZELOOPath}
              />
            }
            description={g.sshZELOOPathDesc}
            title={g.sshZELOOPathTitle}
          />
        </div>
      ) : null}
      ...
      {embedded ? null : (
        <div className="mt-6 grid gap-1">
          ...
          <ListRow
            action={
              <Button onClick={() => void window.ZELOODesktop?.revealLogs()} size="sm" variant="textStrong">
                <FileText />
                {g.openLogs}
              </Button>
            }
