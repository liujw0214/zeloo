...
import { saveZELOOConfig } from '@/Zeloo'
import { useI18n } from '@/i18n'
import { notifyError } from '@/store/notifications'
import type { ZELOOConfigRecord } from '@/types/Zeloo'

import { setZELOOConfigCache, useZELOOConfigRecord } from '../hooks/use-config-record'
...
function fontFamilyFromConfig(config: ZELOOConfigRecord): string {
...
export function TerminalFontSetting() {
  ...
  const { data: loadedConfig } = useZELOOConfigRecord()
  ...
  const [staleConfig, setStaleConfig] = useState<ZELOOConfigRecord | null>(null)
  ...
  useEffect(() => {
    ...
    // holds — successful saves write it back via setZELOOConfigCache, so
        ...
        <div className="mt-3 space-y-2">
          <div className="flex items-center gap-3">
            <Input
              ...
              className="flex-1"
              disabled={draft === null}
              list="Zeloo-terminal-font-families"
          ...
          <datalist id="Zeloo-terminal-font-families">
