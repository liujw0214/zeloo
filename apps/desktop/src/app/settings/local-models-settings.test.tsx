...
import type { LocalCatalogModel, LocalHardware, LocalModelsStatus, LocalRuntimeJob } from '@/types/Zeloo'
...
import * as Zeloo from '@/Zeloo'

const mocked = vi.mocked(Zeloo)
...
describe('BrowseSection', () => {
  ...
  it('searches HF after a pause and shows fit-priced files on demand', async () => {
    ...
    try {
      vi.mocked(Zeloo.searchHFModels).mockResolvedValue({
        hits: [{ downloads: 872724, gated: false, likes: 47, repo: 'unsloth/Qwen3.8-27B-GGUF', updated: '2026-08-18' }]
      })
      vi.mocked(Zeloo.listHFRepoFiles).mockResolvedValue({
        files: [
      ...
      expect(Zeloo.searchHFModels).not.toHaveBeenCalled()
      await act(async () => {
        await vi.advanceTimersByTimeAsync(400)
      })
      expect(Zeloo.searchHFModels).toHaveBeenCalledWith('qwen')
      ...
      vi.mocked(Zeloo.downloadBrowsedModel).mockResolvedValue({ job_id: 'j1', model_id: 'Qwen3.8-27B-Q4_K_M' })
      fireEvent.click(q4Btn)
      await act(async () => {
        await vi.runOnlyPendingTimersAsync()
      })
      expect(Zeloo.downloadBrowsedModel).toHaveBeenCalledWith('unsloth/Qwen3.8-27B-GGUF', ['Qwen3.8-27B-Q4_K_M.gguf'])
    } finally {
      vi.useRealTimers()
    }
  })
...
describe('added-by-you rows', () => {
  it('staged models outside the catalog get the full action set', async () => {
    vi.mocked(Zeloo.getLocalModelsStatus).mockResolvedValue({
      ...BASE_STATUS,
      loaded_models: { 'Zeloo-4.3-36B-Q5_K_M': 'loaded' },
      models: [{ id: 'Zeloo-4.3-36B-Q5_K_M', size_bytes: 25 * 2 ** 30, size_label: '25.0 GB' }],
      placement: {
        'Zeloo-4.3-36B-Q5_K_M': {
    ...
    vi.mocked(Zeloo.getLocalCatalog).mockResolvedValue({ models: [] })

    renderPane()
    await screen.findByText('Zeloo-4.3-36B-Q5_K_M')
