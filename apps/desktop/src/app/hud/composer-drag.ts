...
function setWorkspaceTransfer(transferring: boolean): void {
  window.ZELOODesktop?.hud?.setWorkspaceTransfer?.(transferring)
}

function moveHud(state: PressState): void {
  window.ZELOODesktop?.hud?.moveBy?.({
    width: state.originW,
...
function armGrab(state: PressState, workspaceTransfer: boolean): void {
  ...
  window.ZELOODesktop?.hud?.beginMove?.()
}
 ...
 * (see Zeloo:hud:move-by — a transparent frameless window drifts wider on
...
export function useHudComposerDrag(
  ...
  const reset = useCallback(() => {
    ...
    if (state) {
      if (state.armed) {
        window.ZELOODesktop?.hud?.endMove?.()
      }
