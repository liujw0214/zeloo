...
function sessionsBotsTree() {
  ...
  registerPane('Zeloo-bots:pane', { placement: 'left', hideOnly: true })
  registerPane('workspace', { placement: 'main', uncloseable: true })
  $layoutTree.set(
    split('row', [
      group(['sessions', 'Zeloo-bots:pane'], { active: 'sessions', id: 'g-side' }),
...
describe('hide-only strip tabs', () => {
  it('hides and shows a chrome tab, keeping the pane in the tree', () => {
    ...
    expect(setStripTabHidden('Zeloo-bots:pane', true)).toBe(true)
    expect($hiddenTreePanes.get()).toContain('Zeloo-bots:pane')
    expect($hiddenStripTabs.get()).toContain('Zeloo-bots:pane')
    // Hidden, not dismissed: the pane stays in the layout tree.
    expect(allPaneIds($layoutTree.get()!)).toContain('Zeloo-bots:pane')

    expect(setStripTabHidden('Zeloo-bots:pane', false)).toBe(true)
    expect($hiddenTreePanes.get()).not.toContain('Zeloo-bots:pane')
    expect($hiddenStripTabs.get()).not.toContain('Zeloo-bots:pane')
  })

  it('refuses to hide the zone last visible tab', () => {
    sessionsBotsTree()
    setStripTabHidden('Zeloo-bots:pane', true)
  ...
  it('persists hides and clears them on reveal', () => {
    ...
    const persisted = JSON.parse(window.localStorage.getItem('Zeloo.desktop.hiddenStripTabs.v1') ?? '[]')

    expect(persisted).toContain('Zeloo-bots:pane')
    ...
    revealTreePane('Zeloo-bots:pane')
    expect($hiddenTreePanes.get()).not.toContain('Zeloo-bots:pane')
    expect(window.localStorage.getItem('Zeloo.desktop.hiddenStripTabs.v1')).toBeNull()
  })
  ...
  it('lists the zone hide-only tabs with live hidden state for the menu', () => {
    ...
    expect(hideOnlyZoneTabs('g-side')).toEqual([
      ...
      { hidden: true, id: 'Zeloo-bots:pane', title: 'Zeloo-bots:pane' }
    ])
  ...
  it('excludes hide-only tabs from every close verb', () => {
    ...
    $layoutTree.set(group(['sessions', 'Zeloo-bots:pane', 'session-tile:x'], { active: 'sessions', id: 'g-mixed' }))
