...
describe('Sessions/Bots strip — #91223', () => {
  beforeEach(() => {
    ...
    registerPane('Zeloo-bots:pane', { hideOnly: true, placement: 'left' }, 'Bots')
  ...
  it('double-clicking the Sessions tab leaves the strip and both chips', () => {
    ...
    expect(tabEl('Zeloo-bots:pane')).toBeTruthy()
  ...
  it('double-clicking the Bots tab leaves the strip too', () => {
    ...
    tap(tabEl('Zeloo-bots:pane')!)
    doubleTap(tabEl('Zeloo-bots:pane')!)
...
describe('docked tool tile — collapsing keeps the restore chip', () => {
  beforeEach(() => {
    registerPane('workspace', { placement: 'main', uncloseable: true }, 'Chat')
    registerPane('Zeloo-bots:routines', { placement: 'main', width: '250px' }, 'Cronjobs')
    $layoutTree.set(
      split('row', [
        group(['workspace'], { active: 'workspace', id: 'g-main' }),
        group(['Zeloo-bots:routines'], { active: 'Zeloo-bots:routines', id: 'g-routines' })
      ])
    )
  })
  ...
  it('clicking the active tab does not collapse the tile or drop its label', () => {
    ...
    expect(tabEl('Zeloo-bots:routines')).toBeTruthy()
    tap(tabEl('Zeloo-bots:routines')!)

    expect(zoneAt(1).minimized).toBeFalsy()
    expect(tabEl('Zeloo-bots:routines')).toBeTruthy()
    expect(tabEl('Zeloo-bots:routines')?.textContent).toMatch(/cronjobs/i)
  })
  ...
  it('collapsing via the store does not dismiss the pane from the tree', () => {
    collapseTreePane('Zeloo-bots:routines')

    expect(zoneAt(1).minimized).toBe(true)
    expect($dismissedPanes.get().has('Zeloo-bots:routines')).toBe(false)
    expect(zoneAt(1).panes).toContain('Zeloo-bots:routines')
