...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Zeloo gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>, overrides: Record<string, unknown> = {}) {
  const original = window.ZELOODesktop
  Object.defineProperty(window, 'ZELOODesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config, ...overrides }
  })

  return () => Object.defineProperty(window, 'ZELOODesktop', { configurable: true, value: original })
}
