...
const ZELOO_REF_TYPES = WIRE_REFERENCE_KINDS
type ZELOORefType = (typeof ZELOO_REF_TYPES)[number]
...
const ZELOO_DIRECTIVE_RE = referenceRe()
...
export const ZELOODirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(ZELOO_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...ZELOODirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Zeloo directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.ZELOODesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.ZELOODesktop?.readFileDataUrl(id)
