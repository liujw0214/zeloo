...
import { formatRefValue, ZELOODirectiveFormatter } from './directive-text'
...
describe('ZELOODirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = ZELOODirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = ZELOODirectiveFormatter.parse('@file:src/main.tsx the entry point')
  ...
  it('parses session links with profile/id values', () => {
    const segments = ZELOODirectiveFormatter.parse('see @session:work/20260101_abc123 next')
...
describe('inline skill references', () => {
  const skills = (text: string) =>
    [...ZELOODirectiveFormatter.parse(text)]
  ...
  it('keeps the surrounding prose as text around the chip', () => {
    const segments = ZELOODirectiveFormatter.parse('tidy this with /clean thanks')
  ...
  it('parses a skill chip alongside an @ reference', () => {
    const mentions = [...ZELOODirectiveFormatter.parse('run /clean on @file:`src/a.ts`')].filter(
