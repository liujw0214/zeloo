 ...
 * own design.
 *
 * WIDGETS TALK BACK OFF-SCREEN. `window.Zeloo.send(prompt)` (or declarative
 * `data-Zeloo-send` on any clickable element) routes the prompt through the
...
const SIZE_MESSAGE_TYPE = 'Zeloo-inline-preview-size'
const INTENT_MESSAGE_TYPE = 'Zeloo-inline-preview-intent'
...
/** The script that gives the widget its ONE voice: `Zeloo.send(prompt)`.
 ...
 *  wires `data-Zeloo-send` so declarative HTML works with zero script:
 *  `<button data-Zeloo-send="get-price eth">ETH</button>`. */
export function intentScript(token: string): string {
  return (
    ...
    ')},"*");return true}' +
    'window.Zeloo={send:send};' +
    'addEventListener("click",function(e){var el=e.target&&e.target.closest?' +
    'e.target.closest("[data-Zeloo-send]"):null;' +
    'if(el)send(el.getAttribute("data-Zeloo-send")||"")},true)})()</script>'
  )
}
...
export function InlinePreviewDirective({
  ...
  // URL connection — including a same-machine `Zeloo serve` — renders live.)
