...
const { getProfiles } = await import('@/Zeloo')
...
const remoteConn = (over: Partial<ZELOOConnection> = {}): ZELOOConnection =>
  ({ baseUrl: 'https://Zeloo-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as ZELOOConnection
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { ZELOODesktop: { getConnection } })
