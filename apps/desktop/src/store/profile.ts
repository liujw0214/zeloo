import { LOCAL_CONNECTION_ID, registryBackendScopeKey } from '@Zeloo/shared'
import { atom, batch, computed } from 'nanostores'

import type { ZELOOConnection } from '@/global'
import { getProfiles, ZELOOApi, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/Zeloo'
...
import type { ProfileInfo } from '@/types/Zeloo'
...
// /api/profiles/active `current`). "default" is the root ~/.Zeloo. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'Zeloo.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'Zeloo.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  ...
  try {
    const res = await ZELOOApi<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new ZELOO_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.ZELOODesktop.profile.set(name)
}
...
async function resolveConnectionForProfile(profile: string): Promise<ZELOOConnection | null> {
  const getConnection = window.ZELOODesktop?.getConnection
...
export async function ensureGatewayProfile(profile: string | null | undefined): Promise<void> {
  ...
  // #89206 split-brain — observed live as atom 'default' over a Zeloo-setup
...
// previous descriptor.
async function resolveConnectionForAgent(connectionId: string, profile: string): Promise<ZELOOConnection | null> {
  const getConnectionFor = window.ZELOODesktop?.getConnectionFor
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'Zeloo.desktop.showAllProfiles'
...
export function selectProfile(name: string): void {
  ...
  void Promise.all([activateOnCurrentSource(target), shouldRememberStartupProfile])
    .then(([, shouldRemember]) => {
      if (shouldRemember) {
        return window.ZELOODesktop?.profile?.remember(target)
      }
...
async function isLocalDesktopProfile(target: string): Promise<boolean> {
  const getConnectionConfig = window.ZELOODesktop?.getConnectionConfig
...
// Switch to the default (root ~/.Zeloo) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.ZELOODesktop?.touchBackend?.(target).catch(() => undefined)
}
