...
describe('selectProfile startup preference (#79886)', () => {
  ...
  it('does not replace the local startup preference for a profile SSH override', async () => {
    ...
    selectProfile('macmini-Zeloo')

    await vi.waitFor(() => expect(ensureGatewayForProfile).toHaveBeenCalledWith('macmini-Zeloo'))
    await vi.waitFor(() => expect(getConnection).toHaveBeenCalledWith('macmini-Zeloo'))
