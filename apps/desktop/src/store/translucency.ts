 ...
 * `@Zeloo/shared/translucency`, which owns the mapping both this store and the
 * main process read.
 *
 ...
 * material underneath the web contents to read (see the `[data-Zeloo-glass]`
 * block in styles.css).
 */
...
} from '@Zeloo/shared/translucency'
 ...
 * sets `ZELOODesktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.ZELOODesktop?.glassSupported === 'boolean'
    ? window.ZELOODesktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.ZELOODesktop?.translucencySupported === 'boolean'
    ? window.ZELOODesktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-Zeloo-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-Zeloo-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-Zeloo-glass-on', glassLive)
  root.toggleAttribute('data-Zeloo-glass', glassOn)
  root.toggleAttribute('data-Zeloo-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-Zeloo-glass-scope', scope)
  } else {
    root.removeAttribute('data-Zeloo-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  $translucency.subscribe(state => {
    ...
    window.ZELOODesktop?.setTranslucency?.(state)
  })
