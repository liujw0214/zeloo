...
export function setZoomPercent(percent: number): void {
  window.ZELOODesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.ZELOODesktop?.zoom) {
  void window.ZELOODesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.ZELOODesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
