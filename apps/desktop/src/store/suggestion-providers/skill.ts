 ...
 *  Working in `~/www/Zeloo-agent`, the draft says "Zeloo-agent" constantly
 *  — a "Use skill: Zeloo-agent" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`Zeloo-agent-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/Zeloo-agent-dev`, `plugin:skill`). */
