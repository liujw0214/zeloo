 ...
 *
 * The archive is the CLI's own `Zeloo profile export` tar.gz (config, skills,
...
import { exportProfileArchive, importProfileArchive } from '@/Zeloo'
...
import type { ProfileDesktopOverlay } from '@/types/Zeloo'
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'Zeloo profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.ZELOODesktop?.selectSavePath
...
export async function runImportProfileFlow(): Promise<null | string> {
  const paths = await window.ZELOODesktop?.selectPaths?.({
