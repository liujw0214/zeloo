...
import type { ZELOOReviewFile, ZELOOReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Zeloo' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'Zeloo.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'Zeloo.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'Zeloo.desktop.reviewTreeMode'
const SELECTED_KEY = 'Zeloo.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<ZELOOReviewFile[]>([])
...
export const $reviewShipInfo = atom<ZELOOReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['ZELOODesktop']>['git']>['review']>
...
export async function selectReviewFile(file: ZELOOReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly ZELOOReviewFile[], path: string): ZELOOReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.ZELOODesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.ZELOODesktop?.openExternal?.(url)
    }
