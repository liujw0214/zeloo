...
import type { ZELOOGitWorktree, ZELOORepoStatus } from '@/global'
...
const EMPTY_WORKTREES: ZELOOGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, ZELOORepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, ZELOOGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<ZELOORepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<ZELOOGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<ZELOORepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<ZELOOGitWorktree[]>>()
const $noRepoStatus = atom<ZELOORepoStatus | null>(null)
const $noWorktrees = atom<ZELOOGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<ZELOORepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<ZELOOGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: ZELOORepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: ZELOOGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<ZELOORepoStatus | null>
  seq: number
  target: string
}
