...
/**
 * Feature store for backend (agent) plugins — the native Zeloo plugins plus
 ...
 * primitives `Zeloo plugins` and the dashboard use, so all surfaces agree on
...
export interface AgentPluginRow {
  ...
  /** Agent Plugins v1 package (portable skills/MCP format) vs native Zeloo. */
  portable?: boolean
 ...
 *  ZELOO_HOME. Always refetches (it's a cheap local disk scan on the
...
export async function installAgentPlugin(
  ...
  opts: {
    ...
    ref?: string
    /** Target profile's ZELOO_HOME (null/undefined = backend launch profile). */
    profile?: string | null
  }
