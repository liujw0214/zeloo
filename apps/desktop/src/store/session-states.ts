...
import { type GatewayEvent, LOCAL_CONNECTION_ID, registryBackendScopeKey } from '@Zeloo/shared'
...
const TILES_KEY = 'Zeloo.desktop.sessionTiles.v2'
const LEGACY_TILES_KEY = 'Zeloo.desktop.sessionTiles.v1'
...
const BOT_CHAT_SCOPE_KEY = 'Zeloo.desktop.botChatSessions.v1'
 ...
 *  (Zeloo-agent#90102). The tile bucket is a Local Storage cache, and a
 ...
 * restore re-dials the profile's backend, whose ensure_zeloo_home() re-creates
 * the profile directory the delete just removed (Zeloo-agent#94235). Same
...
export function dropTilesForProfile(
  ...
  const ownerMatches = (owner: SessionProfileRoute | undefined): boolean => {
    ...
    // would orphan a live conversation (Zeloo-agent#94235). Tiles persisted
...
// Dev hook for automation (mirrors __ZELOO_LAYOUT_TREE__).
if ((import.meta.env.DEV || import.meta.env.VITE_PERF_PROBE === '1') && typeof window !== 'undefined') {
  ;(window as unknown as Record<string, unknown>).__ZELOO_SESSION_TILES__ = {
