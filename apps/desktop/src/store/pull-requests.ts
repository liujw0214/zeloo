...
import type { ZELOOBranchPullRequest } from '@/global'
import { scanSessionPullRequests, type SessionInfo } from '@/Zeloo'
...
export const $pullRequestsByBranch = atom<Record<string, ZELOOBranchPullRequest>>({})
...
export const $prBranchBySession = persistentAtom<Record<string, string>>(
  'Zeloo.desktop.prBranchBySession',
  {},
  Codecs.stringRecord
)
...
const $prScannedSessions = persistentAtom<string[]>('Zeloo.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: ZELOOBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }
