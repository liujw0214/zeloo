 ...
 * composer with the reply scrolling above it, so Zeloo can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.ZELOODesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.ZELOODesktop?.hud
...
export function resetHudLayout(): void {
  void window.ZELOODesktop?.hud?.resetLayout?.()
}
...
export const reportHudSession = (sessionId: null | string): void => window.ZELOODesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.ZELOODesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
