...
const agentConn = (over: Partial<ZELOOConnection> = {}): ZELOOConnection =>
  ({ baseUrl: 'https://homelab.invalid', mode: 'remote', profile: 'research', ...over }) as ZELOOConnection

const localConn = (over: Partial<ZELOOConnection> = {}): ZELOOConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as ZELOOConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<ZELOOConnection>>()

const getConnectionFor =
  vi.fn<(payload: { connectionId?: null | string; profile?: null | string }) => Promise<ZELOOConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { ZELOODesktop: { getConnection, getConnectionFor }, localStorage })
...
describe('ensureGatewayAgent commit hook (beforeActivate) — the Sessions switcher door (#93937)', () => {
  ...
  it('a descriptor lookup that never answers is bounded, fails open, and releases the mutex', async () => {
    ...
    try {
      getConnectionFor.mockImplementationOnce(() => new Promise<ZELOOConnection>(() => undefined))
