 ...
 *
 * Local models ship on main behind `--local` (either `Zeloo desktop --local`
 * or the flag on Zeloo.exe itself). The flag is strict: without it the GUI
...
export const $localModelsEnabled = atom<boolean>(
  typeof window !== 'undefined' && window.ZELOODesktop?.localModelsEnabled === true
)
