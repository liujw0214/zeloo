...
export function canOpenSessionWindow(): boolean {
  return typeof window !== 'undefined' && typeof window.ZELOODesktop?.openSessionWindow === 'function'
}

// True when the shell can open a full peer app window (⌘⇧N / "New Window").
export function canOpenNewWindow(): boolean {
  return typeof window !== 'undefined' && typeof window.ZELOODesktop?.openWindow === 'function'
}

// True when the shell can pop the in-app Browser into its own OS window.
export function canOpenBrowserWindow(): boolean {
  return typeof window !== 'undefined' && typeof window.ZELOODesktop?.openBrowserWindow === 'function'
}
...
export function canOpenSessionInTerminal(): boolean {
  return typeof window !== 'undefined' && typeof window.ZELOODesktop?.openSessionInTerminal === 'function'
}
...
export async function openSessionInNewWindow(sessionId: string, opts?: { watch?: boolean }): Promise<void> {
  ...
  await runWindowOpen(
    () => window.ZELOODesktop.openSessionWindow(sessionId, { ...opts, profile }),
...
export async function openNewWindow(): Promise<void> {
  ...
  await runWindowOpen(() => window.ZELOODesktop.openWindow(), 'Could not open a new window')
}
...
export async function openBrowserInNewWindow(tabId: string): Promise<boolean> {
  ...
  return runWindowOpen(() => window.ZELOODesktop.openBrowserWindow(tabId), 'Could not pop out browser')
}
...
export async function openSessionInTerminal(
  ...
  await runWindowOpen(
    () => window.ZELOODesktop.openSessionInTerminal(sessionId, opts),
