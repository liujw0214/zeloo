...
import type { ZELOOGitBaseBranch, ZELOOGitBranch } from '@/global'
import { getZELOOConfig, ZELOOApi, type ZELOOGateway } from '@/Zeloo'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/Zeloo'
...
const PROJECT_SCOPE_KEY = 'Zeloo.desktop.projectScope'
...
async function activeProjectsContext(profile = projectProfile()): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== normalizeProfileKey($activeGatewayProfile.get())) {
    throw new Error('Active Zeloo profile changed while connecting')
  }
...
async function refreshProjectTreeAcrossProfiles(): Promise<void> {
  ...
  try {
    const res = await ZELOOApi<ProjectTreePayload>({
...
const repoScanStates = new WeakMap<ZELOOGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<ZELOOGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  if (isDesktopFsRemoteMode()) {
    ...
    // with `scan: true` — added in #81723) so repos with zero Zeloo
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getZELOOConfig(context.profile))
...
// repo, returning where Zeloo should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<ZELOOGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<ZELOOGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.ZELOODesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.ZELOODesktop?.writeClipboard?.(path)
  }
}
