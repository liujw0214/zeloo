 ...
 * app's bounds and stays visible while Zeloo is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'Zeloo.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'Zeloo.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.ZELOODesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.ZELOODesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.ZELOODesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.ZELOODesktop?.petOverlay?.close()
}
