/**
 * Live skin sync from the Zeloo backend.
 *
 * The backend resolves the active skin (built-in or `$ZELOO_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Zeloo
 * authoring/activating a skin from a prompt, or `/skin` elsewhere) repaints.
 */

import type { ZELOOSkin } from '@Zeloo/shared/skin'
...
const BACKEND_THEMES_KEY = 'Zeloo-desktop-backend-themes-v1'
 ...
 */
export function ingestBackendSkin(skin: ZELOOSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as ZELOOSkin)
