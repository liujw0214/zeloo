...
test('redactSecrets scrubs the spawn-time session token env var', () => {
  const line = 'setsid env ZELOO_DASHBOARD_SESSION_TOKEN=abc123deadbeef ZELOO_DESKTOP=1 Zeloo dashboard'
  const out = redactSecrets(line)
  assert.ok(!out.includes('abc123deadbeef'))
  assert.match(out, /ZELOO_DASHBOARD_SESSION_TOKEN=<redacted>/)
  // non-secret env vars are preserved
  assert.match(out, /ZELOO_DESKTOP=1/)
})
...
test('redactSecrets scrubs Authorization and X-Zeloo-Session-Token headers', () => {
  ...
  assert.match(redactSecrets('X-Zeloo-Session-Token: hdr_888'), /X-Zeloo-Session-Token: ?<redacted>/)
  assert.ok(!redactSecrets('X-Zeloo-Session-Token: hdr_888').includes('hdr_888'))
})
...
test('controlSocketPath default base stays under sun_path even with the temp-listener suffix', () => {
  ...
  const p = controlSocketPath('Zeloo', 'remote-build-server', 22) // no baseDir → default
...
test('buildExecArgs ends with host then the remote command', () => {
  ...
  const args = buildExecArgs(conn, 'command -v Zeloo', 15000)
  assert.equal(args[args.length - 1], 'command -v Zeloo')
...
test('close() removes the control socket when -O exit fails', async () => {
  const dir = path.join(os.tmpdir(), `Zeloo-ssh-close-${process.pid}-${Date.now()}`)
...
test('open() creates the control-socket directory if it does not exist', async () => {
  const dir = path.join(os.tmpdir(), `Zeloo-ssh-test-${process.pid}-${Date.now()}`)
