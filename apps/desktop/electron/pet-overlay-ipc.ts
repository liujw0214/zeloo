...
export function registerPetOverlayIpc({
  ...
  ipcMain.handle('Zeloo:pet-overlay:open', async (_event, request) => {
  ...
  ipcMain.handle('Zeloo:pet-overlay:close', async () => {
    closePetOverlay()
  ...
  ipcMain.on('Zeloo:pet-overlay:set-bounds', (_event, bounds) => {
  ...
  ipcMain.on('Zeloo:pet-overlay:ignore-mouse', (_event, ignore) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('Zeloo:pet-overlay:set-focusable', (_event, focusable) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('Zeloo:pet-overlay:state', (_event, payload) => {
    const petOverlayWindow = getPetOverlayWindow()

    if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
      petOverlayWindow.webContents.send('Zeloo:pet-overlay:state', payload)
    }
  })
  // Overlay → main renderer: control messages (pop back in, composer submit).
  ipcMain.on('Zeloo:pet-overlay:control', (_event, payload) => {
    const mainWindow = getMainWindow()
    ...
    mainWindow.webContents.send('Zeloo:pet-overlay:control', payload)
  })
}
