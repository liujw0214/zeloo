 ...
 *   - the browser sees only a minimal "return to Zeloo" page.
 */
...
const DONE_HTML =
  ...
  '<p>You can close this window and return to Zeloo.</p>' +
...
export function registerMcpOauthCallbackIpc() {
  ...
  ipcMain.handle('Zeloo:mcp-oauth:listen', async () => {
  ...
  ipcMain.handle('Zeloo:mcp-oauth:wait', async (_event, id, timeoutMs) => {
  ...
  ipcMain.handle('Zeloo:mcp-oauth:cancel', (_event, id) => {
    dispose(String(id || ''))
