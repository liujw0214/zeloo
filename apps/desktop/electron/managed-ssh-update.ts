 ...
 * wire protocol:
 *
 *   gate dials -> drain every captured scope -> detached `Zeloo update`
...
function buildPosixManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const home = validateRemoteValue(target.ZELOOHome, 'Zeloo home')
  const ZELOOPath = validateRemoteValue(target.ZELOOPath, 'launcher path')
  ...
  const launcherWord = expandRemotePath(ZELOOPath)

  const updateCommand =
    `env ZELOO_HOME=${homeWord} ` +
    `ZELOO_UPDATE_CORRELATION_ID=${shq(correlation)} ` +
    'ZELOO_UPDATE_ORIGIN_PROFILE=default ' +
    `ZELOO_UPDATE_ORIGIN_HOME=${homeWord} ` +
    `ZELOO_UPDATE_OUTPUT_PATH=${outputWord} ` +
...
function buildWindowsManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const wrapper = [
    ...
    `$env:ZELOO_HOME=${psLiteral(home)}`,
    `$env:ZELOO_UPDATE_CORRELATION_ID=${psLiteral(correlation)}`,
    '$env:ZELOO_UPDATE_ORIGIN_PROFILE="default"',
    `$env:ZELOO_UPDATE_ORIGIN_HOME=${psLiteral(home)}`,
    `$env:ZELOO_UPDATE_OUTPUT_PATH=${psLiteral(outputPath)}`,
    ...
    `$env:ZELOO_UPDATE_WINDOWS_DETACHED=${psLiteral(correlation)}`,
    `$env:ZELOO_UPDATE_TAURI_OUTCOME_PATH=${psLiteral(statusPath)}`,
    `$env:ZELOO_UPDATE_TAURI_READY_PATH=${psLiteral(readyPath)}`,
    ...
    `& ${psLiteral(ZELOOPath)} update --yes *>> ${psLiteral(outputPath)}`,
    '$rc=$LASTEXITCODE',
...
marker_path=install_root/'.Zeloo-update-in-progress'
...
async function runManagedSshUpdate<TScope extends ManagedSshScope>(
  ...
  return {
    ...
    message:
      outcome === 'updated'
        ? 'Remote Zeloo updated and every managed SSH profile is ready.'
        : restoreOk
