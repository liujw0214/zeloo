 ...
 * `iter_zeloo_node_dirs()` in zeloo_constants.py, which the Electron main
 * process cannot import.
 */
function ZELOOManagedNodePathEntries(
  ZELOOHome,
  ...
  const root = pathModule.join(ZELOOHome, 'node')
...
function buildDesktopBackendPath({
  ...
  const ZELOONodeDirs = ZELOOManagedNodePathEntries(ZELOOHome, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([ZELOONodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function normalizeZELOOHomeRoot(ZELOOHome, { pathModule = pathModuleForPlatform(process.platform) }: any = {}) {
  if (!ZELOOHome) {
    return ZELOOHome
  }

  const resolved = pathModule.resolve(String(ZELOOHome))
...
function buildDesktopBackendEnv({
  ...
  return {
    ...
    // cp1252, ...). zeloo_bootstrap sets this inside the child too, but only
