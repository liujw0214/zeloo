...
function sandbox(tag: string) {
  const home = fs.mkdtempSync(path.join(os.tmpdir(), `Zeloo-handoff-marker-${tag}-`))
  const installRoot = path.join(home, 'Zeloo-agent')
...
function markerStartedAt(home: string): number {
  const [, startedAt] = fs.readFileSync(path.join(home, '.Zeloo-update-in-progress'), 'utf8').split('\n')
...
function runPosix(installRoot: string, startedAt?: string) {
  ...
  if (startedAt === undefined) {
    delete env.ZELOO_UPDATE_STARTED_AT
  } else {
    env.ZELOO_UPDATE_STARTED_AT = startedAt
  }
...
function assertScriptHandoff(run: (installRoot: string, startedAt?: string) => ReturnType<typeof spawnSync>) {
  ...
  fs.writeFileSync(path.join(refreshed.home, '.Zeloo-update-in-progress'), '999999\n1\n')
