 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Zeloo dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Zeloo install on the remote (login-shell probe),
 ...
 *     `ZELOO_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForZELOO in.
 *
 * The minted ZELOO_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const REMOTE_LOCK_DIR = '~/.Zeloo/desktop-ssh'
...
// Resolve the remote Zeloo executable. An EXPLICIT path is honored strictly
...
async function locateZELOO(ssh, remoteZELOOPath) {
  const resolveLauncher = async (candidate: string) => {
    // Return the candidate path directly. The Zeloo binary or wrapper script
    ...
    //     the Zeloo version, and
    //   - capability probing: `<python> serve --help` failed entirely.
    // See https://github.com/NousResearch/Zeloo-agent/issues/74411
    return candidate
  }
  ...
  if (remoteZELOOPath) {
    if (await isExecutable(remoteZELOOPath)) {
      return resolveLauncher(remoteZELOOPath)
    }

    const err: any = new Error(
      `The Zeloo path you set is not an executable on the remote host: "${remoteZELOOPath}". ` +
        'Check the path (it must be the full path to the `Zeloo` binary on the remote, e.g. ' +
        '~/Zeloo-agent/.venv/bin/Zeloo), or clear it to auto-detect.'
    )

    err.kind = 'Zeloo-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v Zeloo')}`)).trim()
  ...
  candidates.push('~/.local/bin/Zeloo')
  candidates.push('/usr/local/bin/Zeloo')
  candidates.push('~/.Zeloo/Zeloo-agent/venv/bin/Zeloo')
  ...
  const err: any = new Error(
    'Zeloo is not installed on the remote host (could not find a `Zeloo` executable). ' +
      'Install it on the remote with:  curl -fsSL https://Zeloo-agent.nousresearch.com/install.sh | sh  ' +
      '— or set the Zeloo path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<Zeloo> --version`,
// e.g. "Zeloo Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH Zeloo a
// connection uses, so a stale/unexpected install is visible.
async function probeZELOOVersion(ssh, ZELOOPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(ZELOOPath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Zeloo Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The ZELOO_HOME the remote dashboard will use (explicit env wins, else
// ~/.Zeloo). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteZELOOHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${ZELOO_HOME:-$HOME/.Zeloo}"')).trim().split('\n').pop()

    return out || '~/.Zeloo'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote Zeloo home.')
...
marker=home/'.Zeloo-update-in-progress'
try:
 ...
 * bytes; it never imports or executes code from the changing Zeloo checkout.
 ...
 */
async function assertRemoteInstallUpdateClear(ssh, ZELOOHome) {
  const home = assertSafeRemoteHome(ZELOOHome)
  let observation = ''
  ...
  try {
    ...
    const error: any = new Error('Could not prove that the remote Zeloo install is clear for SSH startup.')
  ...
  const error: any = new Error(
    live
      ? `Remote Zeloo update process ${live[1]} is still running; SSH startup is paused.`
      : 'The remote Zeloo update marker is unreadable or malformed; refusing SSH startup.'
  )
...
async function listRemoteZELOOProfiles(ssh) {
  const home = assertSafeRemoteHome(await probeRemoteZELOOHome(ssh))
  ...
  try {
    ...
    const error: any = new Error('Could not list remote Zeloo profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote Zeloo home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'ZELOOPath', 'ZELOOHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(
  ...
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !ZELOOPath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(ZELOOPath)})\n` +
      // The installer-facing launcher is intentionally preserved for invocation
      // (#74411), but it may `exec python <install-dir>/Zeloo`, leaving neither
      // launcher nor ZELOO_HOME-derived entrypoint in argv. The ownership-scoped
      // token path + random nonce + exact profile below are the alternative proof.
      `zeloo_home=os.path.expanduser(${shq(ZELOOHome)}) if ${shq(ZELOOHome)} else ""\n` +
      'expected_entries={expected}\n' +
      'if zeloo_home:\n' +
      ' expected_entries.add(os.path.join(zeloo_home,"Zeloo-agent","venv","bin","Zeloo"))\n' +
...
function buildOwnedStaleTerminationCommand(lock, ownershipId) {
  ...
  const expectedPath = expandRemotePath(lock.ZELOOPath)
  const expectedHome = lock.ZELOOHome ? expandRemotePath(lock.ZELOOHome) : "''"
  ...
  const executableMatch = lock.ZELOOHome
...
function lockMatchesManagedUpdateScope(lock, expected) {
  return Boolean(
    ...
    lock.ZELOOPath === expected.ZELOOPath &&
    lock.ZELOOHome === expected.ZELOOHome
  )
}
...
expected_path=os.path.expanduser(${py(lock.ZELOOPath)})
zeloo_home=os.path.expanduser(${py(lock.ZELOOHome)})
expected_entries={expected_path,os.path.join(zeloo_home,"Zeloo-agent","venv","bin","Zeloo")}
...
// execing Zeloo. mutexPath is expandRemotePath() output — a complete shell
...
result=None
try:
 result=subprocess.run(["sh","-c",payload,"Zeloo-update-mutex",str(fd)],pass_fds=(fd,),check=False)
finally:
 os.close(fd)
...
function buildSpawnCommand(ZELOOPath, profile, opts: any = {}) {
  const Zeloo = expandRemotePath(ZELOOPath)
  ...
  const marker = expandRemotePath(`${remoteInstallRoot(opts.ZELOOHome || '~/.Zeloo')}/.Zeloo-update-in-progress`)

  const updateMutex = expandRemotePath(
    `${remoteInstallRoot(opts.ZELOOHome || '~/.Zeloo')}/.Zeloo-update-in-progress.mutex`
  )
  ...
  const dashCmd =
    ...
    `exec env ZELOO_DESKTOP=1${opts.guestOnboarding === true ? ' ZELOO_GUEST_ONBOARDING=1' : ''} ${Zeloo} ${profileArgs}${subCmd}`

  const detachedShell = `eval "exec $1>&-"; ${dashCmd} </dev/null >> ${logPath} 2>&1 & echo $!`
  const detachedSpawn = `child=$("$(command -v setsid || echo nohup)" sh -c ${shq(detachedShell)} Zeloo-update-child "$1" & echo $!)`
...
async function remoteSupportsSshOwnership(ssh, ZELOOPath) {
  const Zeloo = expandRemotePath(ZELOOPath)

  const out = await ssh.exec(
    `help="$(${Zeloo} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(
  ...
  if (!(await remoteSupportsSshOwnership(ssh, ZELOOPath))) {
    const err: any = new Error(
      'The remote Zeloo install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Zeloo on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    ...
    out = await ssh.exec(
      buildSpawnCommand(ZELOOPath, profile, {
        spawnNonce,
        tokenFilePath,
        logPath,
        ZELOOHome,
 ...
 * opened SshConnection, forward/pickLocalPort/waitForZELOO, a token-gated
...
async function connect(deps) {
  ...
  const ZELOOHome = await probeRemoteZELOOHome(ssh)
  await assertRemoteInstallUpdateClear(ssh, ZELOOHome)
  const ZELOOPath = await locateZELOO(ssh, remoteZELOOPath)
  log(`located Zeloo at ${ZELOOPath}`)
  const ZELOOVersion = await probeZELOOVersion(ssh, ZELOOPath)

  if (ZELOOVersion) {
    log(`remote Zeloo version: ${ZELOOVersion}`)
  }
  ...
  if (isLockfileSkew(lock)) {
    ...
    const error: any = new Error(
      `The remote ownership record ${lpath} does not match this Zeloo Desktop build (${lock.reason}). ` +
  ...
  if (lock) {
    ...
    const reusable =
      ...
      lock.ZELOOPath === ZELOOPath &&
      lock.ZELOOHome === ZELOOHome
  ...
  const spawned = await spawnRemoteDashboard(ssh, {
    ...
    ZELOOHome,
    guestOnboarding,
    assertInstallClear: () => assertRemoteInstallUpdateClear(ssh, ZELOOHome)
  })
  ...
  try {
    ...
    assertBootstrapNotSuperseded(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForZELOO(baseUrl, spawnToken)
    assertBootstrapNotSuperseded(signal)
