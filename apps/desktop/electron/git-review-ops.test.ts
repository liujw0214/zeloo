...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'Zeloo-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'Zeloo-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Zeloo Test'], { cwd: dir })
