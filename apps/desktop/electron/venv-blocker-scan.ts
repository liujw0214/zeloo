...
const SCAN_MODULE = 'zeloo_cli._scan_venv_blockers'
 ...
 * preview servers. Unknown Python/Zeloo processes are deliberately ignored.
 */
export async function stopSafeVenvBlockers(
  ...
  for (const process of result.processes) {
    ...
    try {
      await execFn(
        pythonPath,
        ['-m', 'zeloo_cli._scan_venv_blockers', '--terminate-safe', String(process.pid), String(process.createTime)],
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another Zeloo process is using this installation.',
    '',
...
export function formatProbeFailedMessage(error?: string): string {
  ...
  return (
    'Update aborted: Desktop could not verify the Zeloo installation is free.' +
    timeoutDetail +
    '\n\n' +
    'Close other Zeloo windows and terminals, then retry.  If the problem\n' +
    'persists, run `zeloo update` in a terminal for detailed diagnostics.'
  )
}
