...
// hardening, ZELOO_HOME resolution, and the git binary stay injected.
...
export function registerFsIpc({
  ...
  ipcMain.handle('Zeloo:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

  ipcMain.handle('Zeloo:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

  // Reveal a path in the OS file manager (Finder / Explorer / Files).
  ipcMain.handle('Zeloo:fs:reveal', async (_event, targetPath) => {
  ...
  ipcMain.handle('Zeloo:fs:openDir', async (_event, dirPath) => {
  ...
  // The LOCAL Desktop runtime-plugin root: `<ZELOO_HOME>/desktop-plugins`,
  // resolved from the main-process ZELOO_HOME (see resolveZELOOHome) — NOT from
  // the connected backend. A remote backend reports its own `zeloo_home` over
  ...
  async function localPluginsRoot(dirName: string): Promise<string> {
    ...
    const base = profile && profile !== 'default' ? path.join(ZELOOHome, 'profiles', profile) : ZELOOHome
  ...
  async function desktopPluginsRoot(): Promise<string> {
    const root = await ensureDir(path.join(ZELOOHome, DESKTOP_PLUGINS_DIR))
    await migrateProfileScopedDesktopPlugins(ZELOOHome, root)
    await reconcileUnifiedDesktopHalves(ZELOOHome, root)

    return root
  }

  ipcMain.handle('Zeloo:fs:desktopPluginsRoot', async () => desktopPluginsRoot())
  ...
  ipcMain.handle('Zeloo:fs:reconcileDesktopPlugins', async () => {
    const root = await ensureDir(path.join(ZELOOHome, DESKTOP_PLUGINS_DIR))

    return reconcileUnifiedDesktopHalves(ZELOOHome, root)
  })

  // The LOCAL logs root (`<ZELOO_HOME>/logs`, profile-aware) — the error
  // card's "Open Logs" action reveals agent.log/gateway.log without the user
  // knowing where ZELOO_HOME lives. Same Electron-local resolution as the
  // plugin roots: valid in every connection mode, created on demand.
  ipcMain.handle('Zeloo:fs:logsRoot', async () => localPluginsRoot('logs'))

  ipcMain.handle('Zeloo:plugin:probe', async (_event, payload) => {
  ...
  ipcMain.handle('Zeloo:plugin:installDesktop', async (_event, payload) => {
    const identifier = String(payload?.identifier || payload?.repo || '').trim()
  ...
  ipcMain.handle('Zeloo:fs:rename', async (_event, targetPath, newName) => {
  ...
  ipcMain.handle('Zeloo:fs:writeText', async (_event, filePath, content) => {
  ...
  ipcMain.handle('Zeloo:fs:trash', async (_event, targetPath) => {
    const target = String(targetPath || '').trim()
