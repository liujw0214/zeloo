// Unit tests for the pure Windows `Zeloo` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvZELOOCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `Zeloo` shim shadows
//      the real Zeloo.cmd/Zeloo.exe.
...
//   3. resolveVenvZELOOCommand() — must probe the venv python via
//      canImportZELOOCli() before trusting it, or a broken venv gets
...
test('chooseUpdaterArgs: gentle --update when both updater runtime files exist', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvZELOO: true, hasVenvPython: true }, 'main'), [
    '--update',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: marker-only install uses --repair when the venv is gone', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvZELOO: false, hasVenvPython: false }, 'main'),
    ['--repair', '--branch', 'main']
  )
})

test('chooseUpdaterArgs: partial updater runtimes use --repair', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvZELOO: false, hasVenvPython: true }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvZELOO: true, hasVenvPython: false }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: passes the branch through unchanged in both modes', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvZELOO: true, hasVenvPython: true }, 'release/1.2'),
    ['--update', '--branch', 'release/1.2']
  )
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvZELOO: false, hasVenvPython: false }, 'release/1.2'),
    ['--repair', '--branch', 'release/1.2']
  )
})

function makeDeps(overrides: Partial<Parameters<typeof resolveVenvZELOOCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportZELOOCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    ZELOOHome: '/fake/Zeloo-home',
...
test('resolveVenvZELOOCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvZELOOCommand('/root/venv/Scripts/Zeloo.exe', [], deps), null)
})

test('resolveVenvZELOOCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvZELOOCommand('/root/venv/Scripts/Zeloo.cmd', [], deps), null)
})

test('resolveVenvZELOOCommand: returns null when the basename is not Zeloo/Zeloo.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvZELOOCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvZELOOCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvZELOOCommand('/root/venv/bin/Zeloo.exe', [], deps), null)
})

test('resolveVenvZELOOCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvZELOOCommand('/root/venv/Scripts/Zeloo.exe', [], deps), null)
})

test('resolveVenvZELOOCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportZELOOCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvZELOOCommand('/root/venv/Scripts/Zeloo.exe', ['serve'], deps)
...
test('resolveVenvZELOOCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvZELOOCommand('/root/venv/Scripts/Zeloo.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'zeloo_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvZELOOCommand: is case-insensitive on Zeloo.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvZELOOCommand('/root/venv/Scripts/Zeloo.EXE', [], deps))
  assert.ok(resolveVenvZELOOCommand('/root/venv/SCRIPTS/Zeloo.exe', [], deps))
})
