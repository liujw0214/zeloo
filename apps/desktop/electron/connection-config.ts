 ...
 *     `X-Zeloo-Session-Token` header; WS uses `?token=`.
...
// zeloo_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - zeloo_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - zeloo_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / Zeloo #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-zeloo_session_at', '__Secure-zeloo_session_at', 'zeloo_session_at']
const RT_COOKIE_VARIANTS = ['__Host-zeloo_session_rt', '__Secure-zeloo_session_rt', 'zeloo_session_rt']

// The Nous portal (NAS) does NOT use Zeloo gateway session cookies — it is a
...
// Keep this aligned with zeloo_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['Zeloo', 'test', 'tmp', 'root', 'sudo'])
...
function gatewayTicketFailure(error, authMessage, transportMessage) {
  ...
  if (needsOauthLogin) {
    ...
    // change until the user signs in, so tag it the way startZELOO latches
...
/** Which Zeloo profile the remote SSH dashboard should actually run as.
...
// Zeloo Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteZELOOPath = String(entry.remoteZELOOPath || '').trim()

  if (remoteZELOOPath) {
    out.remoteZELOOPath = remoteZELOOPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote Zeloo installation. Preserve an explicit mapping
  // when it is a valid Zeloo profile identifier; otherwise fall back to the
...
function localPrimaryRequestScope(opts: ProfileRouteOptions): boolean | null {
  ...
  // spawned them: `_spawn_zeloo_action` registers the (often dynamic, e.g.
 ...
 *     `--profile`, so its `ZELOO_HOME` scopes it.
 *
...
function resolveProfileBackendRoute(profile, opts: ProfileRouteOptions = {}): ProfileBackendRoute {
  ...
  if (scopedProfile === primaryProfile) {
    ...
    // the wire: the dashboard's process ZELOO_HOME can belong to a different
 ...
 * `Zeloo:api` handler — both routes reach a backend whose namespace is the
...
function translateSelfProfileQuery(path, profile, backendProfile) {
  ...
  try {
    parsed = new URL(rawPath, 'http://Zeloo.local')
  } catch {
    return path
  }
...
/**
 * Resolve the two decisions made by the `Zeloo:api` IPC handler from the same
...
/**
 * True if any cookie in `cookies` is a Zeloo session ACCESS-token cookie
 ...
 * the Zeloo gateway session cookies, so cloud sign-in / discovery liveness
