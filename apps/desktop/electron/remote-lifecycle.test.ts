...
function fakeSsh(rules: any[] = []) {
  ...
  return {
    ...
    async exec(cmd) {
      ...
      if (cmd.includes('.Zeloo-update-in-progress') && !cmd.includes('marker_clear()') && !/setsid|nohup/.test(cmd)) {
        return 'CLEAR'
      }
...
test('POSIX relaunch gate refuses live and uncertain install markers without executing Zeloo', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = {
      async exec(command) {
        ...
        if (command.includes('ZELOO_HOME')) {
          return '/home/alice/.Zeloo\n'
        }

        if (command.includes('.Zeloo-update-in-progress')) {
          return observation
        }
...
test('POSIX relaunch gate permits absent/dead markers and normalizes named-profile homes install-wide', async () => {
  ...
  await assertRemoteInstallUpdateClear(ssh, '/home/alice/.Zeloo/profiles/research')
  assert.match(commands[0], /home\.parent\.name/)
  assert.match(commands[0], /profiles/)
  assert.match(commands[0], /\.Zeloo-update-in-progress/)
})
...
test('listRemoteZELOOProfiles inventories Mini-style profile dirs without spawning a dashboard', async () => {
  const ssh = fakeSsh([
    [/ZELOO_HOME/, '/Users/zillajr/.Zeloo\n'],
    [/ls -1/, 'bob\ndixie\ngoose\nrambo\nbob.rollback-old\n']
  ])

  assert.deepEqual(await listRemoteZELOOProfiles(ssh), ['default', 'bob', 'dixie', 'goose', 'rambo'])
  assert.equal(
...
test('listRemoteZELOOProfiles rejects a hostile ZELOO_HOME', async () => {
  const ssh = fakeSsh([[/ZELOO_HOME/, '/tmp/x; echo pwned\n']])

  await assert.rejects(
    () => listRemoteZELOOProfiles(ssh),
    (err: any) => {
...
test('locateZELOO prefers the explicit profile path when executable', async () => {
  const ssh = fakeSsh([[/\[ -x .*\/opt\/Zeloo/, 'OK']])
  assert.equal(await locateZELOO(ssh, '/opt/Zeloo'), '/opt/Zeloo')
})

test('locateZELOO throws (no silent fallback) when an EXPLICIT path is not executable', async () => {
  // command -v WOULD find a different install, but an explicit path must not
  // silently fall back to it — that is the "connected to the wrong Zeloo" bug.
  const ssh = fakeSsh([
    [/command -v Zeloo/, '/home/u/.local/bin/Zeloo\n'],
    [/\[ -x .*\.local\/bin\/Zeloo/, 'OK']
  ])

  await assert.rejects(
    () => locateZELOO(ssh, '/bad/path/Zeloo'),
    (err: any) => {
      assert.equal(err.kind, 'Zeloo-not-found')
      assert.match(err.message, /\/bad\/path\/Zeloo/)
...
test('locateZELOO falls back to the login-shell command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v Zeloo/, '/home/u/.local/bin/Zeloo\n'],
    [/\[ -x .*\.local\/bin\/Zeloo/, 'OK']
  ])

  assert.equal(await locateZELOO(ssh, ''), '/home/u/.local/bin/Zeloo')
})

test('locateZELOO preserves an installer wrapper instead of resolving its interpreter', async () => {
  // install.sh venv mode writes: exec "$ZELOO_BIN" "$ZELOO_ENTRYPOINT" "$@",
  // where $ZELOO_BIN is the venv python. The old canonicalization returned
  ...
  const ssh = fakeSsh([
    ...
    [/python3 -c/, '/home/u/.Zeloo/Zeloo-agent/venv/bin/python\n']
  ])

  assert.equal(await locateZELOO(ssh, ''), '/home/u/.local/bin/Zeloo')
  assert.ok(
    !ssh.calls.some(cmd => cmd.includes('python3 -c')),
    'locateZELOO must not shell out to a python3 parser to rewrite the launcher'
  )
})

test('locateZELOO returns an explicit remoteZELOOPath unchanged', async () => {
  // The override half of #74411: an explicit remoteZELOOPath pointing at a
  // wrapper was also canonicalized to its interpreter, so overriding to
  // ~/.local/bin/Zeloo changed nothing for affected users.
  const ssh = fakeSsh([
    [/\[ -x .*\.local\/bin\/Zeloo/, 'OK'],
    [/python3 -c/, '/home/u/.Zeloo/Zeloo-agent/venv/bin/python\n']
  ])

  assert.equal(await locateZELOO(ssh, '~/.local/bin/Zeloo'), '~/.local/bin/Zeloo')
  assert.ok(!ssh.calls.some(cmd => cmd.includes('python3 -c')), 'an explicit remoteZELOOPath must never be rewritten')
})

test('locateZELOO falls back to ~/.local/bin/Zeloo when the login-shell probe misses', async () => {
  ...
  assert.equal(await locateZELOO(ssh, ''), '~/.local/bin/Zeloo')
})

test('locateZELOO tries the conventional venv path last', async () => {
  const ssh = fakeSsh([[/\[ -x .*venv\/bin\/Zeloo/, 'OK']])
  assert.equal(await locateZELOO(ssh, ''), '~/.Zeloo/Zeloo-agent/venv/bin/Zeloo')
})

test('locateZELOO throws a Zeloo-not-found error with an install hint', async () => {
...
test('locateZELOO uses a login shell for the command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v Zeloo/, '/x/Zeloo'],
    [/\[ -x/, 'OK']
  ])

  await locateZELOO(ssh, '')
  assert.ok(
...
test('ownership paths are isolated by ownership ID and spawn nonce', () => {
  assert.equal(ownershipDirectory(OWNERSHIP_ID), `~/.Zeloo/desktop-ssh/${OWNERSHIP_ID}`)
  assert.equal(lockfilePath(OWNERSHIP_ID), `~/.Zeloo/desktop-ssh/${OWNERSHIP_ID}/backend.lock.json`)
  assert.equal(spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE), `~/.Zeloo/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.log`)
})
...
test('metadata and process proof transport failures remain indeterminate', async () => {
  ...
  await assert.rejects(
    () => pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, failure]]), 5, SPAWN_NONCE, '/x/Zeloo'),
    (error: any) => error.kind === 'transient-transport-error'
  )
})

test('pidIsOurDashboard requires the exact serve ownership nonce', async () => {
  const ours = `/x/Zeloo serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'OWNED\n']]), 5, SPAWN_NONCE, '/x/Zeloo'), true)
  assert.equal(
...
test('pidIsOurDashboard accepts the venv entrypoint an installer wrapper execs into', async () => {
  ...
  assert.equal(
    await pidIsOurDashboard(ssh, 5, SPAWN_NONCE, '~/.local/bin/Zeloo', '/Users/cd9c/.Zeloo', OWNERSHIP_ID, 'ops'),
    true
  )
  assert.match(ownershipProbe, /Zeloo-agent.*venv.*bin.*Zeloo/)
...
test.skipIf(process.platform === 'win32')(
  ...
  async () => {
    const temp = await mkdtemp(path.join(os.tmpdir(), 'Zeloo wrapper ownership '))
    ...
    const entrypoint = path.join(installDir, 'Zeloo')
    const launcher = path.join(temp, 'Zeloo launcher')
    ...
    try {
      ...
      assert.equal(
        await pidIsOurDashboard(ssh, child.pid, SPAWN_NONCE, launcher, '/unrelated/Zeloo-home', OWNERSHIP_ID, 'ops'),
        true
      )
      assert.equal(
        await pidIsOurDashboard(
...
test('buildSpawnCommand is headless serve, detached, token not in argv', () => {
  const cmd = buildSpawnCommand('/x/Zeloo', 'work', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  ...
  assert.ok(!cmd.includes('ZELOO_DASHBOARD_SESSION_TOKEN'), 'token env var must not appear')
})
...
test('buildSpawnCommand atomically reserves the ownership slot through spawn and lock publication', () => {
  const cmd = buildSpawnCommand('/x/Zeloo', 'work', {
    ZELOOHome: '~/.Zeloo',
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    ownershipId: OWNERSHIP_ID,
  ...
  assert.ok(cmd.includes('.Zeloo-update-in-progress.mutex'))
  assert.match(cmd, /fcntl\.flock\(fd,fcntl\.LOCK_EX\)/)
  assert.match(cmd, /os\.O_CLOEXEC/)
  assert.match(
    cmd,
    /subprocess\.run\(\["sh","-c",payload,"Zeloo-update-mutex",str\(fd\)\],pass_fds=\(fd,\),check=False\)/
  )
  assert.doesNotMatch(cmd, /os\.set_inheritable\(fd,True\)/)
  assert.match(cmd, /Zeloo-update-child "\$1"/)
...
test.skipIf(process.platform === 'win32')('detached backend does not inherit the update mutex descriptor', async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), 'Zeloo-update-mutex-'))
  const ZELOOPath = path.join(directory, 'Zeloo')
...
for fd in /proc/$$/fd/*; do
  ...
  case "$target" in
    *Zeloo-update-in-progress.mutex) printf '%s\\n' "$target" >> ${reportPath} ;;
  esac
done
`,
      { mode: 0o700 }
    ...
    const command = buildSpawnCommand(ZELOOPath, '', {
      ZELOOHome: path.join(directory, 'home'),
      logPath
    })
...
test('spawnRemoteDashboard always spawns serve (legacy dashboard path removed)', async () => {
  ...
  await spawnRemoteDashboard(ssh, { ZELOOPath: '/x/Zeloo', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID })
...
test('READY_RE accepts both serve and dashboard sentinels', () => {
  assert.equal(READY_RE.exec('ZELOO_BACKEND_READY port=4321')?.[1], '4321')
  assert.equal(READY_RE.exec('ZELOO_DASHBOARD_READY port=8765')?.[1], '8765')
  // The remote log is `>> log 2>&1`, so a stderr chunk without a newline can be spliced onto the sentinel.
  assert.equal(READY_RE.exec('INFO  Started server process [4711]ZELOO_BACKEND_READY port=65238')?.[1], '65238')
})
...
test('spawnRemoteDashboard rejects when no pid is returned', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { ZELOOPath: '/x/Zeloo', profile: '', token: 't', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('scrapeReadyPort reads only the named spawn log', async () => {
  ...
  const ssh = fakeSsh([[/cat/, 'some noise\nZELOO_DASHBOARD_READY port=51234\n']])
...
test('connect() spawns fresh when there is no lockfile, adopts the served token', async () => {
  const ssh = fakeSsh([
    ...
    [/setsid/, '777\n'],
    [/kill -0 777/, 'ALIVE'],
    [/cat .*\.log/, 'ZELOO_DASHBOARD_READY port=51999\n']
  ])
...
test('managed SSH maps a local scope to a different non-default remote profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '778\n'],
    [/kill -0 778/, 'ALIVE'],
    [/cat .*\.log/, 'ZELOO_BACKEND_READY port=52000\n']
  ])
  ...
  assert.match(spawn, /\.Zeloo\/desktop-ssh\/[0-9a-f]{32}\/[0-9a-f]{16}\.token/)
...
test('connect() respawns when the requested remote profile differs from the lockfile profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/kill 333/, ''],
    [/--version/, 'Zeloo Agent v0.18.2\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/setsid/, '890\n'],
    [/kill -0 890/, 'ALIVE'],
    [/cat .*\.log/, 'ZELOO_DASHBOARD_READY port=52050\n']
  ])
...
test('connect() respawns when the lockfile ZELOOPath differs from the resolved path', async () => {
  const reuseToken = 'stored-token'
  const lock = ownedLock({ ZELOOPath: '/old/stale/Zeloo', tokenFingerprint: fingerprintToken(reuseToken) })
  ...
  const result = await connect(
    connectDeps(ssh, { reuseToken, remoteZELOOPath: '/new/Zeloo', adoptServedToken: async () => 'fresh' })
  )
...
test('connect() respawns when the lockfile protocolVersion is incompatible', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '901\n'],
    [/kill -0 901/, 'ALIVE'],
    [/cat .*\.log/, 'ZELOO_DASHBOARD_READY port=44100\n']
  ])
...
test('connect() fresh spawn writes ZELOOHome + protocolVersion into the lockfile', async () => {
  const writes: string[] = []
  ...
  const ssh = fakeSsh([
    ...
    [/\[ -x/, 'OK'],
    [/cat .*lock\.json/, ''], // no lockfile
    [/ZELOO_HOME/, '/home/alice/.Zeloo\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/printf '%s\\n'/, ''],
    [/setsid/, '700\n'],
    [/kill -0 700/, 'ALIVE'],
    [/cat .*\.log/, 'ZELOO_DASHBOARD_READY port=45500\n'],
    [
      /printf '%s' '/,
      c => {
        writes.push(c)
  ...
  assert.match(lockWrite, /"ZELOOHome":"\/home\/alice\/\.Zeloo"/)
})
...
test('connect() respawns when the lockfile pid is dead (killed dashboard)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '888\n'],
    [/kill -0 888/, 'ALIVE'],
    [/cat .*\.log/, 'ZELOO_DASHBOARD_READY port=42000\n']
  ])
...
test('connect() respawns when the dashboard is wedged (alive pid, probe fails)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '999\n'],
    [/kill -0 999/, 'ALIVE'],
    [/cat .*\.log/, 'ZELOO_DASHBOARD_READY port=43000\n']
  ])
...
test('validateRemotePath accepts absolute POSIX paths', () => {
  assert.doesNotThrow(() => validateRemotePath('/usr/bin/Zeloo'))
  assert.doesNotThrow(() => validateRemotePath('/home/user/.Zeloo/Zeloo-agent/venv/bin/Zeloo'))
})

test('validateRemotePath accepts ~/ prefix paths', () => {
  assert.doesNotThrow(() => validateRemotePath('~/bin/Zeloo'))
  assert.doesNotThrow(() => validateRemotePath('~/.Zeloo/logs/desktop-ssh.log'))
  assert.doesNotThrow(() => validateRemotePath('~'))
})

test('validateRemotePath accepts paths with spaces and quotes', () => {
  assert.doesNotThrow(() => validateRemotePath('/home/user/my project/Zeloo'))
...
test('validateRemotePath rejects relative paths', () => {
  assert.throws(() => validateRemotePath('Zeloo'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('./bin/Zeloo'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('../etc/passwd'), /absolute|relative/i)
})

test('validateRemotePath rejects NUL and newline', () => {
  assert.throws(() => validateRemotePath('/usr/bin/Zeloo\x00'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/Zeloo\n'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/Zeloo\r'), /unsafe/i)
})

test('validateRemotePath preserves shell metacharacters as path data', () => {
  for (const p of ['/usr/$(whoami)/Zeloo', '/usr/`id`/Zeloo', '/usr/a;b|c&d<e>f']) {
...
test('expandRemotePath expands ~/ to "$HOME"/', () => {
  const result = expandRemotePath('~/.Zeloo/logs/desktop-ssh.log')
...
test('expandRemotePath returns quoted absolute paths unchanged', () => {
  const result = expandRemotePath('/usr/local/bin/Zeloo')
  assert.ok(result.includes('/usr/local/bin/Zeloo'))
  assert.ok(!result.includes('eval'))
})

test('expandRemotePath preserves spaces as data', () => {
  const result = expandRemotePath('/home/user/my project/Zeloo')
...
test('buildSpawnCommand does not embed the token in the command string', () => {
  ...
  assert.ok(!cmd.includes('ZELOO_DASHBOARD_SESSION_TOKEN'), 'env var name must not appear')
})

test('buildSpawnCommand includes --ssh-session-token-file when tokenFilePath is provided', () => {
  const cmd = buildSpawnCommand('/x/Zeloo', 'work', {
    tokenFilePath: `~/.Zeloo/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.token`,
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    spawnNonce: SPAWN_NONCE
  })

  assert.match(cmd, /--ssh-session-token-file/)
  assert.match(cmd, /\.Zeloo\/desktop-ssh\//)
})

test('buildSpawnCommand always uses serve, never dashboard', () => {
  const cmd = buildSpawnCommand('/x/Zeloo', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /serve --isolated/)
  assert.doesNotMatch(cmd, /\bdashboard\b/)
  assert.doesNotMatch(cmd, /--skip-build/)
...
test('buildSpawnCommand raises the SSH child file limit before execing Zeloo', () => {
  const cmd = buildSpawnCommand('/x/Zeloo', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /ulimit -n 65536 2>\/dev\/null \|\| true; exec env ZELOO_DESKTOP=1/)
...
test('spawnRemoteDashboard removes a token file when upload reporting fails', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { ZELOOPath: '/x/Zeloo', profile: '', token: 'tok', ownershipId: OWNERSHIP_ID }),
    /channel closed/
  )
...
test('spawnRemoteDashboard fails with update-required when remote lacks --ssh-session-token-file', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { ZELOOPath: '/x/Zeloo', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('readLockfile treats a log path outside the exact ownership and spawn path as skew', async () => {
  const lock = ownedLock({ logPath: '~/.Zeloo/desktop-ssh/other.log' })
...
test('cleanupStale never deletes a lock-supplied unexpected log path', async () => {
  ...
  await cleanupStale(ssh, OWNERSHIP_ID, ownedLock({ logPath: '~/.Zeloo/unrelated.log' }))
...
test('pidIsOurDashboard requires an exact nonce option value', async () => {
  const prefix = `/x/Zeloo serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}ff`
  const suffix = `/x/Zeloo serve --isolated --ssh-owner-nonce xx${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/Zeloo'), false)
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/Zeloo'), false)
})
...
test('remote SSH ownership capability requires both secure bootstrap flags', async () => {
  ...
  assert.equal(await remoteSupportsSshOwnership(supported, '/x/Zeloo'), true)
  ...
  assert.equal(await remoteSupportsSshOwnership(unsupported, '/x/Zeloo'), false)
})
...
test.skipIf(process.platform === 'win32')(
  ...
  async () => {
    ...
    const root = await mkdtemp(path.join(os.tmpdir(), 'Zeloo-argv-shim-'))
    ...
    try {
      ...
      assert.equal(
        argv[2],
        `${fakeHome}/.Zeloo/.Zeloo-update-in-progress.mutex`,
      ...
      const base = `${fakeHome}/.Zeloo/desktop-ssh/${OWNERSHIP_ID}`
