...
test('terminalScriptEnv drops PATH in any casing and keeps the rest', () => {
  ...
  assert.deepEqual(env, { PYTHONPATH: '/repo', PYTHONUTF8: '1', ZELOO_HOME: '/home/b/.Zeloo' })
})
...
test('buildTerminalScript execs the resolved runtime with its env', () => {
  const script = buildTerminalScript({
    args: ['-m', 'zeloo_cli.main', '--tui', '--resume', 'sess'],
    command: '/home/b/.Zeloo/Zeloo-agent/venv/bin/python',
    cwd: "/home/b/o'brien",
    env: { PYTHONPATH: '/home/b/.Zeloo/Zeloo-agent' },
    platform: 'darwin'
  })
  ...
  assert.equal(
    ...
    [
      ...
      `export PYTHONPATH='/home/b/.Zeloo/Zeloo-agent'`,
      `exec '/home/b/.Zeloo/Zeloo-agent/venv/bin/python' '-m' 'zeloo_cli.main' '--tui' '--resume' 'sess'`,
      ''
    ].join('\n')
  )
})
...
test('buildTerminalScript emits a cmd script on Windows', () => {
  const script = buildTerminalScript({
    ...
    command: 'C:\\Zeloo\\venv\\Scripts\\Zeloo.exe',
    cwd: 'C:\\Users\\b',
    env: { PYTHONUTF8: '1' },
    platform: 'win32'
  })
  ...
  assert.deepEqual(script.split('\r\n'), [
    ...
    'cd /d "C:\\Users\\b"',
    'set "PYTHONUTF8=1"',
    '"C:\\Zeloo\\venv\\Scripts\\Zeloo.exe" "--tui" "--resume" "sess"',
    ''
  ])
})
