...
export function resolveDesktopRemoteRoute({
  ...
  if (envUrl) {
    ...
    if (!envToken) {
      throw new Error(
        'ZELOO_DESKTOP_REMOTE_URL is set but ZELOO_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Zeloo backend.'
      )
    }
  ...
  if (!modeIsRemoteLike(config.mode)) {
    ...
    // resolves local and spawns a loopback `Zeloo serve` the desktop never
