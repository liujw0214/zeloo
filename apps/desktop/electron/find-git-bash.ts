 ...
 *   1. ZELOO_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\Zeloo\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect ZELOO_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.ZELOO_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'Zeloo', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'Zeloo', 'git', 'usr', 'bin', 'bash.exe'))
  }
