...
export function registerNativeNotifications({ getMainWindow, focusWindow }: NotificationHost): void {
  ...
  ipcMain.handle('Zeloo:notify', (event, payload: ZELOONotification) => {
    ...
    notification.on('click', () => {
      ...
      if (focusSessionId) {
        window.webContents.send('Zeloo:focus-session', focusSessionId)
      }

      if (payload?.activate || payload?.notifyId) {
        window.webContents.send('Zeloo:notification-activate', {
    ...
    notification.on('action', (actionEvent, index) => {
      ...
      if (payload?.sessionId && !payload?.notifyId && !payload?.activate) {
        ...
        window.webContents.send('Zeloo:notification-action', { sessionId: payload.sessionId, actionId: action.id })
