...
export function registerHudIpc({
  ...
  ipcMain.on('Zeloo:hud:native-drag', event => {
    event.returnValue = hudWindowing().move === 'native-drag'
  })

  ipcMain.on('Zeloo:hud:windowing', event => {
  ...
  ipcMain.on('Zeloo:hud:workspace-transfer', (event, transferring) => {
  ...
  ipcMain.handle('Zeloo:hud:open', async (_event, request) => {
    openHudWindow(
  ...
  ipcMain.handle('Zeloo:hud:frost', (_event, showing) => {
  ...
  // reaches the bar.
  ipcMain.on('Zeloo:hud:ignore-mouse', (_event, ignore) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('Zeloo:hud:begin-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('Zeloo:hud:end-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('Zeloo:hud:move-by', (event, delta) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('Zeloo:hud:set-bounds', (event, bounds) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('Zeloo:hud:reset-layout', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('Zeloo:hud:session', (event, sessionId) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('Zeloo:hud:close', async () => {
    closeHudWindow()
