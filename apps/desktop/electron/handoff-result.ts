 ...
 * console. It writes ZELOO_HOME/.Zeloo-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen Zeloo within 30 minutes.
...
export function handoffResultPath(ZELOOHome: string): string {
  return path.join(ZELOOHome, '.Zeloo-update-result.json')
}
...
export function readAndConsumeHandoffResult(
  ...
  const file = handoffResultPath(ZELOOHome)
  let raw: string
