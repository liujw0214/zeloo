 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Zeloo Agent by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.Zeloo-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(ZELOOHome) {
  return path.join(ZELOOHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.Zeloo/Zeloo-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(ZELOOHome) {
  if (!ZELOOHome) {
    return null
  }

  const candidate = path.join(ZELOOHome, 'Zeloo-agent', 'scripts', installScriptName())
...
function cachedScriptPath(ZELOOHome, commit) {
  return path.join(bootstrapCacheDir(ZELOOHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/NousResearch/Zeloo-agent/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(ZELOOHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(ZELOOHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, ZELOOHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass ZELOO_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          ZELOO_HOME: ZELOOHome || process.env.ZELOO_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, ZELOOHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, ZELOOHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--Zeloo-home', ZELOOHome]
...
async function fetchManifest({
  ...
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, ZELOOHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, ZELOOHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(ZELOOHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, ZELOOHome, emit })
    abortSignal?.throwIfAborted()
