// Profile-delete routing logic for the `Zeloo:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_zeloo_home() and
...
/** Parse a `Zeloo:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its ZELOO_HOME.
 *
...
/**
 * Route the next `Zeloo:api` request away from the primary/window backend
 ...
 * ensure_zeloo_home() recreates the directory the delete just removed.
 */
