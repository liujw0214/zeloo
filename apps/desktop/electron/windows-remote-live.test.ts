...
//   ZELOO_WIN_SSH_HOST   ssh alias/host of the Windows box
//   ZELOO_WIN_SSH_USER   remote user
//   ZELOO_WIN_SSH_ZELOO absolute path to the remote Zeloo.exe under test
const liveHost = process.env.ZELOO_WIN_SSH_HOST || ''
const liveUser = process.env.ZELOO_WIN_SSH_USER || ''
const configuredZELOO = process.env.ZELOO_WIN_SSH_ZELOO || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Zeloo-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredZELOO)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredZELOO.replace('Zeloo.exe', 'python.exe')}' -m zeloo_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredZELOO.replace('Zeloo.exe', 'python.exe')
        const terminate = `& '${python}' -m zeloo_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.ZELOOPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m zeloo_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m zeloo_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
