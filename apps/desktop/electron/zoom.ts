...
export const ZOOM_STORAGE_KEY = 'Zeloo:desktop:zoomLevel'
 ...
 * but forgot to emit 'Zeloo:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('Zeloo:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
 ...
 * only learns of changes through `Zeloo:zoom:changed`, which never fired —
