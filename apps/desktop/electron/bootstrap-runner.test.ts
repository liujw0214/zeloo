...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'Zeloo-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/Zeloo-runner-test',
    sourceRepoRoot: null,
    ZELOOHome: '/tmp/Zeloo-runner-test',
    logRoot: '/tmp/Zeloo-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'Zeloo-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'Zeloo-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/Zeloo-agent',
      ZELOOHome: '/tmp/Zeloo'
    }),
    ['--dir', '/tmp/Zeloo-agent', '--Zeloo-home', '/tmp/Zeloo', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/Zeloo-agent', '--Zeloo-home', '/tmp/Zeloo', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/Zeloo',
      ZELOOHome: '/tmp/home'
    }),
    ['--dir', '/tmp/Zeloo', '--Zeloo-home', '/tmp/home', '--branch', 'main']
  )
})
