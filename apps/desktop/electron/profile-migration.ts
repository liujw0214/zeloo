...
export interface MigrationDeps {
  ...
  /** Default profile home (`~/.Zeloo`). Default's state.db and gateway.pid live here. */
  ZELOOHome: string
  /** Named-profile root (`~/.Zeloo/profiles`). Does not contain `default`. */
  profilesRoot: string
  ...
  isZELOOProcess: (pid: number) => boolean
  now: () => number
...
/**
 * Production layout: default IS `ZELOOHome`; named profiles are children of
 * `profilesRoot`. There is no `profiles/default` directory on a normal install.
 */
export function profileStateDbPath(name: string, ZELOOHome: string, profilesRoot: string): string {
  return name === 'default' ? `${ZELOOHome}/state.db` : `${profilesRoot}/${name}/state.db`
}

export function profileGatewayPidPath(name: string, ZELOOHome: string, profilesRoot: string): string {
  return name === 'default' ? `${ZELOOHome}/gateway.pid` : `${profilesRoot}/${name}/gateway.pid`
}

function resolveZELOOHome(profilesRoot: string, ZELOOHome?: string): string {
  if (ZELOOHome) {
    return ZELOOHome
  }

  // Tests that predate ZELOOHome pass only profilesRoot.
...
/**
 * Return the profile names whose gateway.pid file points to a live Zeloo process.
 ...
 *
 * `ZELOOHome` is optional so existing call sites that only pass `profilesRoot`
 * still work: it is derived as the parent of `…/profiles`.
 */
export function findRunningGatewayProfiles(
  profilesRoot: string,
  allProfiles: string[],
  deps: Pick<MigrationDeps, 'existsSync' | 'readFileSync' | 'isZELOOProcess'> & { ZELOOHome?: string }
): string[] {
  const ZELOOHome = resolveZELOOHome(profilesRoot, deps.ZELOOHome)
  const running: string[] = []

  for (const name of allProfiles) {
    const pidFile = profileGatewayPidPath(name, ZELOOHome, profilesRoot)
...
export function decideMigration(
  ...
  for (const name of candidates) {
    const s = score(profileStateDbPath(name, deps.ZELOOHome, deps.profilesRoot))
...
/** Default is always a candidate; it is `$ZELOO_HOME`, not `$ZELOO_HOME/profiles/default`. */
...
export function migrateActiveProfileIfMissing(desktopProfileConfigPath: string, deps: MigrationDeps): boolean {
  ...
  // launches `Zeloo --profile default` and is worse than writing nothing
