...
const READY_RE = /^ZELOO_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitZELOOPath = '') {
  const explicit = psLiteral(explicitZELOOPath)
  ...
  const script = [
    ...
    'if(-not $ZELOOHome){$ZELOOHome=Join-Path $env:LOCALAPPDATA "Zeloo"}',
    'Assert-NoReparse $ZELOOHome $true',
    '$candidate=[IO.Path]::Combine($ZELOOHome, "Zeloo-agent\\venv\\Scripts\\Zeloo.exe")',
    ...
    '$profileCandidate=[IO.Path]::Combine($HOME, "Zeloo-agent\\.venv\\Scripts\\Zeloo.exe")',
    ...
    '$fallbackHomeCandidate=Join-Path $ZELOOHome "Zeloo-agent\\venv\\Scripts\\Zeloo.exe"',
    '$fallbackProfileCandidate=Join-Path $HOME "Zeloo-agent\\.venv\\Scripts\\Zeloo.exe"',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command Zeloo.exe -ErrorAction SilentlyContinue',
    ...
    '$Zeloo=$null',
    'foreach($candidate in $candidates){Assert-NoReparse $candidate $true;$candidatePython=[IO.Path]::Combine([IO.Path]::GetDirectoryName($candidate), "python.exe");Assert-NoReparse $candidatePython $true;try{$item=Get-Item -LiteralPath $candidate -Force -ErrorAction Stop;if(($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0 -and -not $item.PSIsContainer){$Zeloo=$item.FullName;break}}catch [Management.Automation.ItemNotFoundException]{continue}}',
    'if(-not $Zeloo){throw "Zeloo is not installed on the remote Windows host."}',
    'Assert-NoReparse $Zeloo $false',
    'if($explicit -and $Zeloo -ne $explicit){throw "The configured Zeloo path is not an executable file."}',
    '$python=[IO.Path]::Combine([IO.Path]::GetDirectoryName($Zeloo), "python.exe")',
    'Assert-NoReparse $python $false',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;ZELOOHome=$ZELOOHome;ZELOOPath=$Zeloo;python=$python}|ConvertTo-Json -Compress'
  ].join(';')

  return JSON.parse((await ssh.exec(powerShellCommand(script))).trim())
}

function windowsUpdateMarkerProbeCommand(ZELOOHome) {
  const script = [
    '$ErrorActionPreference="Stop"',
...
public static class ZELOOMarkerNoFollow {
...
`,
    ...
    '}',
    '}',
    `$home=${psLiteral(ZELOOHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".Zeloo-update-in-progress"',
    '$result="UNCERTAIN"',
    '$stream=$null;$memory=$null',
    'try{',
    'Assert-NoReparse $marker $true',
    'if(-not (Test-Path -LiteralPath $marker -PathType Leaf)){$result="CLEAR"}else{$stream=[ZELOOMarkerNoFollow]::OpenRead($marker)',
 ...
 */
async function assertWindowsRemoteInstallUpdateClear(ssh, ZELOOHome) {
  let observation = ''

  try {
    observation =
      String(await ssh.exec(windowsUpdateMarkerProbeCommand(ZELOOHome)))
        .replace(/^\uFEFF/, '')
        .trim()
        .split(/\r?\n/)
        .pop() || ''
...
async function detectRemotePlatform(ssh, explicitZELOOPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitZELOOPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'zeloo_cli.windows_ssh_runtime', operation, ...args]
...
function atomicWindowsSpawnCommand(runtime, reservation: any = {}) {
  const argv = [runtime.python, '-m', 'zeloo_cli.windows_ssh_runtime', 'spawn']
  const helper = operation => [runtime.python, '-m', 'zeloo_cli.windows_ssh_runtime', operation]

  const script = [
    '$ErrorActionPreference="Stop"',
    `$home=${psLiteral(runtime.ZELOOHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".Zeloo-update-in-progress"',
    ...
    reservation.ownershipId
      ...
      : '  if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}',
    reservation.ownershipId
      ? `  $spawned=$spawnLines[-1]|ConvertFrom-Json; $lock=[ordered]@{schemaVersion=2;protocolVersion=1;ownershipId=${psLiteral(reservation.ownershipId)};spawnNonce=${psLiteral(reservation.spawnNonce)};pid=[int]$spawned.pid;creationTimeNs=[string]$spawned.creationTimeNs;port=0;profile=${psLiteral(reservation.profile)};ZELOOPath=${psLiteral(reservation.ZELOOPath)};ZELOOHome=${psLiteral(reservation.ZELOOHome)};tokenFingerprint=${psLiteral(reservation.tokenFingerprint)};startedAt=${psLiteral(reservation.startedAt)}}|ConvertTo-Json -Compress; ` +
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.ZELOOPath === 'string' &&
    typeof lock.ZELOOHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.ZELOOPath === runtime.ZELOOPath &&
    lock.ZELOOHome === runtime.ZELOOHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertBootstrapNotSuperseded(signal)
  const runtime = await probeWindowsRemote(ssh, remoteZELOOPath)
  await assertWindowsRemoteInstallUpdateClear(ssh, runtime.ZELOOHome)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.ZELOOPath])

  if (!inspection.supported) {
    const error: any = new Error('Update Zeloo on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.ZELOOPath = inspection.path
  const ZELOOVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located Zeloo at ${runtime.ZELOOPath}`)
  ...
  try {
    ...
    spawned = await atomicWindowsSpawn(
      ssh,
      runtime,
      JSON.stringify({ ownershipId, spawnNonce, profile, ZELOOPath: runtime.ZELOOPath }),
      {
        ownershipId,
        spawnNonce,
        profile,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForZELOO(baseUrl, token)
    assertBootstrapNotSuperseded(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Zeloo SSH"', 'powershell.exe -NoLogo')
