// Backend subcommand routing for the desktop-managed Zeloo process.
//
// The desktop app launches its own headless backend via `Zeloo serve` — it
...
// app hasn't updated yet, or an older `Zeloo` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Zeloo profile to pin via `--profile`.
 */
 ...
 * `-m zeloo_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `zeloo_cli/subcommands/dashboard.py` source registers
