...
export function markerPath(ZELOOHome) {
  return path.join(ZELOOHome, '.Zeloo-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(ZELOOHome)
  let raw
 ...
 *
 * The Tauri-based Zeloo-setup.exe takes several seconds to initialise its
...
export function writeUpdateMarker(
  ...
  const file = markerPath(ZELOOHome)
  const nowMs = now()
  const owner = readLiveUpdateMarker(ZELOOHome, { kill, maxAgeMs, now: () => nowMs })
 ...
 * Zeloo to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(ZELOOHome, opts)
