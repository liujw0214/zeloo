 ...
 * the three user-facing uninstall modes to the `Zeloo uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m zeloo_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `zeloo_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'zeloo_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Zeloo.app/Contents/MacOS/Zeloo  → …/Zeloo.app
 *   Windows: …\Zeloo\Zeloo.exe                 → …\Zeloo  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Zeloo.app/Contents/MacOS/Zeloo → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Zeloo.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Zeloo.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Zeloo$/i.test(dir) || /[\\/]Zeloo-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/Zeloo
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import zeloo_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, ZELOOHome }) {
 ...
 * modes + `pythonPath`=agentRoot so `import zeloo_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Zeloo install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "ZELOO_HOME=${String(ZELOOHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
