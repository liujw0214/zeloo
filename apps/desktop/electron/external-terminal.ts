// Launching the Zeloo TUI in the user's OWN terminal emulator.
//
...
// `Zeloo --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's Zeloo runtime is often a venv Python invoked
//     as `python -m zeloo_cli.main`, not a `Zeloo` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The Zeloo
