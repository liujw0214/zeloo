...
test('a live primary keeps answering for its booted profile after the preference moves', () => {
  ...
  // startZELOO() boots the primary as the preference at that moment.
  ...
  // Zeloo:profile:remember rewrites active-profile.json without re-homing.
  preference = 'claude'
  ...
  // spawned for the same ZELOO_HOME.
  assert.equal(
    pin.resolve(() => preference),
    'default'
  )
