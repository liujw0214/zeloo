 ...
 *
 * 1. TORN BUNDLE after an update (#95575): `Zeloo update` replaces the app
...
export interface RendererLoadErrorDetails {
  ...
  /** Repair command hint, e.g. `Zeloo desktop --force-build`. */
  repairHint?: string
  /**
...
export function buildRendererLoadErrorPage(details: RendererLoadErrorDetails = {}): string {
  ...
  const title = 'Zeloo couldn\u2019t start the desktop UI'
  ...
  const repair = details.repairHint ? `<p>Repair with: <code>Zeloo desktop --force-build</code></p>` : ''
...
<main>
  ...
  <code>Zeloo desktop --force-build</code>, then restart the app.</p>
