...
// registry, and the Zeloo:terminal:* IPC surface. Extracted from main.ts; the
...
export function registerTerminalIpc({
  ...
  function terminalShellCommand() {
    // ZELOO_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
    ...
    const override = (process.env.ZELOO_DESKTOP_SHELL || (isWindows ? '' : process.env.SHELL) || '').trim()
  ...
  function terminalShellEnv() {
    ...
    // /TERM=dumb; some terminals set COLORFGBG which would flip Zeloo' TUI into
    ...
    env.TERM_PROGRAM = 'Zeloo'
    env.TERM_PROGRAM_VERSION = app.getVersion()

    // Let a Zeloo/--tui launched in this pane know it's embedded in the desktop
    // GUI (build_environment_hints surfaces this). Distinct from ZELOO_DESKTOP,
    // which marks the agent *backend* and gates cron/gateway behavior.
    env.ZELOO_DESKTOP_TERMINAL = '1'

    return env
  }

  function terminalChannel(id, suffix) {
    return `Zeloo:terminal:${id}:${suffix}`
  }
  ...
  ipcMain.handle('Zeloo:terminal:start', async (event, payload = {}) => {
  ...
  ipcMain.handle('Zeloo:terminal:attach', (event, id) => {
  ...
  ipcMain.handle('Zeloo:terminal:write', (_event, id, data) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('Zeloo:terminal:resize', (_event, id, size = {}) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('Zeloo:terminal:cwd', async (_event, id) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('Zeloo:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))
