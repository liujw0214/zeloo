 ...
 * runtime, remote gateways, Zeloo Cloud instances, SSH hosts) that are all
 ...
 * apps/shared/src/backend-scope.ts (`@Zeloo/shared`) — tsconfig project
...
export async function reuseMatchingPrimarySshBackend({
  ...
  if (
    ...
    String(source.remoteZELOOPath || '').trim() !== String(activeSsh.remoteZELOOPath || '').trim() ||
...
/** Turn `ls ~/.Zeloo/profiles` output into roster names. Always includes
...
/**
 * Whether "Update all instances" may drive this connection. Zeloo Cloud
 * instances are platform-managed — we never run `Zeloo update` against them.
...
export function normalizeConnectionInput(input: ConnectionInput, registry: ConnectionRegistry): RegistryConnection {
  ...
  if (kind === 'ssh') {
    const ssh = normalizeSshConfig({
      ...
      remoteZELOOPath: input.remoteZELOOPath,
  ...
  if (kind === 'remote' || kind === 'cloud') {
    ...
    // transport-/Zeloo-managed names; an empty result stores nothing.
 ...
 * `remoteZELOOPath`/`remoteProfile`. Only fields the payload explicitly
...
export function migrateV1ToRegistry(v1: unknown): ConnectionRegistry {
  ...
  const addRemoteLike = (block: Record<string, any>, kind: 'cloud' | 'remote'): null | RegistryConnection => {
    ...
    const label = uniqueLabel(
      hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Zeloo Cloud' : 'Remote gateway'),
...
export function reconcileAppliedGlobalConnection(
  ...
  const hostLabel = hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Zeloo Cloud' : 'Remote gateway')
