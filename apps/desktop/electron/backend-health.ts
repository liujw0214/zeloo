...
export interface ZELOOReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForZELOOReady`).
   */
...
export const REMOTE_UNSIGNED_OAUTH_MESSAGE =
  'Remote Zeloo gateway uses OAuth, but you are not signed in. ' +
 ...
 *  - readiness-probe exhaustion in waitForZELOOReady().
 *
...
/**
 * True when the backend URL points at a Nous-managed Zeloo Cloud instance
 ...
 * `isReauthRequired` so startZELOO latches; a bare `needsOauthLogin` (the
...
export async function waitForZELOOReady(baseUrl: string, options: ZELOOReadyOptions): Promise<void> {
  ...
  throw new Error(`Zeloo backend did not become ready: ${detail}`)
}
