/**
 * windows-Zeloo-path.ts
 *
 * Pure, dependency-injected pieces of Windows `Zeloo` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvZELOOCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `Zeloo` shim shadowed the real
 *      Zeloo.cmd/Zeloo.exe; the shim then failed the --version probe and
 ...
 *      and Zeloo launcher. Marker-only or partial runtimes must use --repair;
 *      only a runnable pair can use --update.
 *   3. resolveVenvZELOOCommand() — unwrapWindowsVenvZELOOCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `Zeloo`) must
 * not shadow `Zeloo.cmd`/`Zeloo.exe`. The empty entry is kept LAST so
 ...
 * venv Python interpreter and the Zeloo launcher that drives `Zeloo update`.
...
export interface ResolveVenvZELOOCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportZELOOCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `Zeloo`/`Zeloo.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/Zeloo(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m zeloo_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportZELOOCli(). A
 * venv whose update died mid-`pip install` still has python.exe + Zeloo.exe
 ...
 *
 * Returns null when `command` is not a venv Zeloo shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvZELOOCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvZELOOCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^Zeloo(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Zeloo at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Zeloo Python at ${python}`,
    command: python,
