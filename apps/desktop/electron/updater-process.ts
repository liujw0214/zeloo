 ...
 *
 * The staged Tauri `Zeloo-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `Zeloo update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `Zeloo update`, swaps/relaunches
 * the app, and writes .Zeloo-update-result.json. With the app gone before
 * the update starts, the ZELOO_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `Zeloo update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into ZELOO_HOME on *every* platform
 * (`Zeloo-setup.exe` on Windows, `Zeloo-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_zeloo_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `Zeloo update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(ZELOOHome, 'Zeloo-setup.exe')
 ...
 *
 * `copy_self_to_zeloo_home` deliberately no-ops during `--update`
 ...
 * "Another Zeloo update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `Zeloo update`, which
 ...
 * the installer is written to ZELOO_HOME at install/repair time, making mtime
