// The standalone desktop-plugin root (`<ZELOO_HOME>/desktop-plugins`) and the
...
//      is COPIED out as `<root>/<name>/` with a `.Zeloo-package.json` marker
...
export const PACKAGE_MARKER = '.Zeloo-package.json'
...
async function packageOrigin(packageDir: string): Promise<Pick<DesktopHalfMarker, 'catalogName' | 'repo' | 'sha'>> {
  try {
    const sidecar = JSON.parse(await fs.promises.readFile(path.join(packageDir, '.Zeloo-catalog.json'), 'utf8')) as {
      catalog_name?: string
      repo?: string
      sha?: string
    }
...
/** Every Zeloo home the app knows about locally: the default plus each profile. */
export async function localHomes(ZELOOHome: string): Promise<string[]> {
  const profiles = await listDirs(path.join(ZELOOHome, 'profiles'))

  return [ZELOOHome, ...profiles.map(name => path.join(ZELOOHome, 'profiles', name))]
}
...
export async function migrateProfileScopedDesktopPlugins(ZELOOHome: string, appRoot: string): Promise<string[]> {
  const moved: string[] = []

  for (const profile of await listDirs(path.join(ZELOOHome, 'profiles'))) {
    const scopedRoot = path.join(ZELOOHome, 'profiles', profile, DESKTOP_PLUGINS_DIR)
...
export async function reconcileUnifiedDesktopHalves(ZELOOHome: string, appRoot: string): Promise<string[]> {
  const touched: string[] = []
  const seen = new Set<string>()

  for (const home of await localHomes(ZELOOHome)) {
