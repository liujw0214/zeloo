...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(ZELOO_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Zeloo-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.Zeloo/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'Zeloo-desktop-ssh')
  }

  return path.join(os.homedir(), '.Zeloo', 'desktop-ssh')
}
