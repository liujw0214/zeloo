 ...
 *
 * So we delegate to `Zeloo gateway stop --all`: the CLI discovers every
 ...
 * force-kills survivors — the same logic `Zeloo update`'s
...
export function stopGatewayBeforeUpdate(
  ...
  return runGatewayLifecycleCommand(ZELOOCliPath, ['gateway', 'stop', '--all'], deps)
}
 ...
 */
export function startGatewaysAfterUpdateAbort(ZELOOCliPath: string, deps: StopGatewayBeforeUpdateDeps = {}): boolean {
  return runGatewayLifecycleCommand(ZELOOCliPath, ['gateway', 'start', '--all'], deps)
}

function runGatewayLifecycleCommand(ZELOOCliPath: string, args: string[], deps: StopGatewayBeforeUpdateDeps): boolean {
  ...
  if (deps.spy) {
    deps.spy(ZELOOCliPath, args)
  }

  if (!existsSync(ZELOOCliPath)) {
    return false
  }

  try {
    exec(ZELOOCliPath, args, {
