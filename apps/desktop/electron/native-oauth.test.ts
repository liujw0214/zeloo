...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/Zeloo', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/Zeloo/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/Zeloo'), 'https://gw.example.com/Zeloo/auth/native/refresh')
})
