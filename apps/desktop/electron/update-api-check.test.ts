...
test('compare payload maps to the behind count and a newest-first commit list; malformed = null', () => {
  ...
  // canonicalGitHubRemote lowercases owner + repo, so the fixture value is
  // fully lowercase. (Historically the test asserted 'someone/Zeloo-agent'
  // with a capital Z, which is what GitHub's web UI shows, but the slug
  // returned by `githubRepoSlug` is the canonical key, not the display
  // name — so it has to match what the canonical function produces.)
