"""Lazy dependency installer for opt-in Zeloo backends.
...
(never system Python); durable-target mode (``ZELOO_LAZY_INSTALL_TARGET``, sealed images)
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # ─── Wake word ("Hey Zeloo") engines (sync with the `wake` extra) ──────
    ...
    # on mere presence, so `Zeloo update` re-asserts this pin everywhere hub exists. MUST stay
    # inside transformers' window and match uv.lock (tests/test_project_metadata.py enforces).
    # HF Agent Trace Viewer upload (Zeloo trace upload / /upload-trace). huggingface-hub is a SHARED
    ...
    # feature active from mere package presence, the `Zeloo update` lazy-refresh pass re-asserts THIS pin
...
_LAZY_TARGET_ENV = "ZELOO_LAZY_INSTALL_TARGET"
...
def _allow_lazy_installs() -> bool:
    ...
    modes; (2) the sealed venv (``ZELOO_DISABLE_LAZY_INSTALLS=1``) blocks only without a durable
    ...
    if cfg is not None and not bool((cfg.get("security") or {}).get("allow_lazy_installs", True)):
        return False
    if os.environ.get("ZELOO_DISABLE_LAZY_INSTALLS") == "1":
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    if sys.platform == "win32" and feature == "platform.matrix":
        return ("unsupported on Windows: Matrix E2EE depends on python-olm, which has no Windows wheel and "
                "requires make + libolm to build from sdist. Run Zeloo under WSL to use Matrix on Windows.")
    return None
...
def _is_satisfied(spec: str) -> bool:
    """Present AND inside the spec's version range, so ``Zeloo update`` propagates pin bumps to
...
def _core_constraints_file() -> Optional[Path]:
    ...
    try:
        ...
        if not pins:
            return None
        fd, path = tempfile.mkstemp(prefix="Zeloo-core-constraints-", suffix=".txt")
...
def _uv_binary() -> Optional[str]:
    """Managed uv first ($ZELOO_HOME/bin is never on PATH), then PATH. A lookup, not ensure_uv():
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    ``zeloo_cli.tools_config._pip_install`` (no CLI dependency)."""
    if not specs:
    ...
    try:
        ...
        uv_env = zeloo_subprocess_env(inherit_credentials=False)
...
def ensure(feature: str, *, prompt: bool = True) -> None:
    ...
    if _lazy_install_target() is None:
        ...
        with contextlib.suppress(Exception):
            from zeloo_cli.config import get_managed_system
            managed_by = get_managed_system()
        if managed_by:
            raise FeatureUnavailable(
                feature, missing,
                f"unsupported on {managed_by}-managed installs: this build's packages come from {managed_by}, "
                f"so Zeloo cannot install them at runtime. Add the dependencies for {feature!r} via "
                f"{managed_by} (or run a pip/uv install of Zeloo instead).")
...
def install_specs(specs: list[str] | tuple[str, ...], *, timeout: int = 300) -> InstallSpecsResult:
    ...
    target = _lazy_install_target()
    if not _allow_lazy_installs():
        sealed = os.environ.get("ZELOO_DISABLE_LAZY_INSTALLS") == "1" and target is None
        reason = ("runtime installs are disabled on this deployment: the agent environment is immutable "
                  "and no writable install target is configured (ZELOO_LAZY_INSTALL_TARGET)"
...
def active_features() -> list[str]:
    ...
    asyncpg are deliberately not proof a backend was enabled. Drives ``Zeloo update``."""
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    """Re-run ``ensure`` for every active feature (``Zeloo update``); returns
