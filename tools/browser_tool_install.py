...
from zeloo_constants import agent_browser_runnable, get_zeloo_home, is_termux as _is_termux_environment, node_tool_runnable
...
def _resolve_npx_bin() -> Optional[str]:
    """Resolve a runnable npx, extended (Zeloo-managed/Homebrew) PATH first.
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    local node_modules binary, which vanishes on the next ``Zeloo update`` and leaves a dangling link ``which``
...
def warm_agent_browser_npx_cache(timeout: float = 60.0) -> bool:
    """Best-effort pre-fetch of the agent-browser npm package via npx (``Zeloo update`` / ``doctor --fix``).
    ...
    registry-lookup/fetch cost. Calling this during ``Zeloo update`` (or ``Zeloo doctor --fix``) warms
