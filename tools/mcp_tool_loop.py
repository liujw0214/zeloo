...
def _try_acquire_mcp_discovery_lock() -> Any:
    ...
    try:
        ...
        if _origin._MCP_DISCOVERY_LOCK_PATH is None:
            _origin._MCP_DISCOVERY_LOCK_PATH = str(get_zeloo_home() / ".mcp-discovery.lock")
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local ZELOO_HOME override into ``coro`` (task-local on the MCP
    loop, so concurrent scopes don't interfere)."""
    try:
        from zeloo_constants import get_zeloo_home_override, reset_zeloo_home_override, set_zeloo_home_override
        home_override = get_zeloo_home_override()
    except Exception:
        home_override = None
    ...
    async def _scoped():
        token = set_zeloo_home_override(home_override)
        try:
            return await coro
        finally:
