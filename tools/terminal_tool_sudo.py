...
def _in_delegated_child_context() -> bool:
    ...
    threads and inherit process-wide ``ZELOO_INTERACTIVE=1``, which does NOT mean they can reach
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    is_gateway = env_var_enabled("ZELOO_GATEWAY_SESSION")
...
def _transform_sudo_command(
    ...
    # delegate_task children inherit ZELOO_INTERACTIVE=1 (and possibly a stale thread-local
    ...
    should_prompt_for_sudo = (
        env_var_enabled("ZELOO_INTERACTIVE") or _get_sudo_password_callback() is not None
