...
class MCPServerHealthMixin:
    ...
    def _make_logging_callback(self):
        """``logging_callback`` forwarding server ``notifications/message`` into Zeloo logging (SDK default drops them).

        Routes MCP ``notifications/message`` log notifications from the server into Zeloo' logging
        (agent.log via zeloo_logging), tagged with the server name. Without this, the SDK's default
