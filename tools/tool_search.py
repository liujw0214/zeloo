...
tool_search / tool_describe / tool_call. Invariants: core tools (``toolsets._ZELOO_CORE_TOOLS``)
...
def _config_from_loader(loader_name: str) -> ToolSearchConfig:
    """Tool-search config via ``zeloo_cli.config.<loader_name>`` (defaults on any failure)."""
    try:
...
def _core_tool_names() -> frozenset[str]:
    ...
    try:
        ...
        return frozenset(_ZELOO_CORE_TOOLS)
    except Exception:
        return frozenset()


# Session-gated GUI toolsets: off ``_ZELOO_CORE_TOOLS`` so non-GUI clients never pay
