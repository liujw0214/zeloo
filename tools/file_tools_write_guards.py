...
# resolve per call in production (below) because ``get_zeloo_home()`` /
...
_zeloo_config_resolved: str | None = None
_zeloo_config_resolved_loaded = False
_real_zeloo_home_cached: str | None = None
_real_zeloo_home_loaded = False
...
def _zeloo_home_real() -> str:
    from zeloo_constants import get_zeloo_home
    return os.path.realpath(str(get_zeloo_home()))


def _get_zeloo_config_resolved() -> str | None:
    """Resolved absolute path of the Zeloo config file for the ACTIVE profile.

    Resolved per call so it tracks the per-turn ``ZELOO_HOME`` scope (#107327);
    a test may pin it via ``_zeloo_config_resolved`` + ``_zeloo_config_resolved_loaded``."""
    if _zeloo_config_resolved_loaded:
        return _zeloo_config_resolved
    try:
        ...
        # home rather than the active multiplex ``ZELOO_HOME`` — comparing
        ...
        # ``get_zeloo_home()`` key the happy path uses, and substitute no
        # unrelated home if even that is gone (#107327 follow-up; PR #107335).
        try:
            from zeloo_constants import get_zeloo_home
            return str((Path(str(get_zeloo_home())) / "config.yaml").resolve())
        except Exception:
            return None


def _get_real_zeloo_home() -> str | None:
    """Realpath of the authoritative Zeloo home for the ACTIVE profile.

    Resolved per call so it tracks the per-turn ``ZELOO_HOME`` scope (#107327);
    a test may pin it via ``_real_zeloo_home_cached`` + ``_real_zeloo_home_loaded``."""
    if _real_zeloo_home_loaded:
        return _real_zeloo_home_cached
    try:
        return _zeloo_home_real()
    except Exception:
        # Same active-profile binding on the exception path (see
        # ``_get_zeloo_config_resolved``): re-derive from ``get_zeloo_home()``
        # rather than ``_expand_tilde("~/.Zeloo")`` so the protected-instruction
        ...
        # the consumer: the ``~/.Zeloo`` exemption is skipped, so the gate runs.
        try:
            from zeloo_constants import get_zeloo_home
            return os.path.realpath(str(get_zeloo_home()))
        except Exception:
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    zeloo_config = _get_zeloo_config_resolved()
    if zeloo_config and zeloo_config in candidates:
        return (
            f"Refusing to write to Zeloo config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.Zeloo/config.yaml directly or use 'Zeloo config' instead.")
    return None
...
# vector (AGENTS.md / CLAUDE.md / SOUL.md / .cursorrules / project .Zeloo tree).
...
def _protected_instruction_config() -> tuple[bool, list[str]]:
    ...
    try:
        from zeloo_cli.config import load_config, cfg_get
        cfg = load_config()
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # ~/.Zeloo itself is governed by its own guards (config.yaml hard-block,
    # mirror guard, write_approval); this gate targets PROJECT-LOCAL files only.
    # Must run before the ``.Zeloo`` component rule, which would match the home.
    real_home = _get_real_zeloo_home()
    ...
    for candidate in (normalized, resolved):
        ...
        if base_lower in _PROTECTED_INSTRUCTION_BASENAMES or any(
            ...
            return base
        # Project-local .Zeloo config dirs (<repo>/.Zeloo/config.yaml) steer
        # behavior too. Only the IMMEDIATE parent counts — matching any ancestor
        # would gate every write inside a checkout living under ~/.Zeloo.
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".Zeloo":
            return candidate
    return None
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Zeloo mirror prefix for persistent Docker file tools."""
    try:
        ...
        if env is not None:
            ...
            return "/root/.Zeloo" if persistent_docker else None
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    Zeloo state (a write the host never reads). Not profile isolation — that guard was
