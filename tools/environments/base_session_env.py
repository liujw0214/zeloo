...
# backend serves many sessions, so a snapshot carrying the FIRST session's ZELOO_SESSION_ID
# would make every LATER session source a foreign identity. Every bridged name starts with
# one of these prefixes (or is ZELOO_UI_SESSION_ID); unit tests use this regex as the
...
# "default" environment), so ``export -p`` dumping the FIRST session's ZELOO_SESSION_ID into the snapshot
# makes every LATER session ``source`` that stale value and see a FOREIGN session's identity — overriding
# the correct per-command Popen env (issue: cross-session ZELOO_SESSION_ID leak via the shared snapshot).
# Stripping them from the snapshot is safe because they are re-injected on every command; a snapshot should
# only carry the user's own shell state (PATH, functions, exports they set), not Zeloo' per-turn session
# identity. Used by unit tests as the Python-side contract for the exclusion set; the dump path unsets by
# name/prefix instead of grepping declare lines (see below / issue #71296).
_SNAPSHOT_EXCLUDED_ENV_REGEX = (
    "^declare -x (ZELOO_SESSION_|ZELOO_UI_SESSION_ID|ZELOO_CRON_AUTO_DELIVER_|"
    "ZELOO_CRON_SESSION|ZELOO_BROWSER_CONTROL_)")
_SHELL_ENV_NAME_RE = re.compile(r"^[A-Za-z_][A-Za-z0-9_]*$")

# mktemp template suffix + the shell variable holding the allocated temp path.
_SNAP_TMP_SUFFIX = ".tmp.XXXXXXXXXX"
_SNAP_TMP = '"$__zeloo_snap_tmp"'


def _cwd_marker(session_id: str) -> str:
    return f"__ZELOO_CWD_{session_id}__"
...
def _export_dump_excluding_session_vars(tmp_path: str, excluded_names: Iterable[str] = ()) -> str:
    ...
    ``curl … | bash #`` smuggled into a Matrix room/display name via ``ZELOO_SESSION_CHAT_NAME``) land in
    ...
    return (
        "{ ( unset ${!ZELOO_SESSION_*} ${!ZELOO_CRON_AUTO_DELIVER_*} "
        "${!ZELOO_BROWSER_CONTROL_*} "
        # AI_AGENT / ZELOO_AGENT are per-command attribution markers re-exported
        ...
        "AI_AGENT ZELOO_AGENT "
        f"ZELOO_UI_SESSION_ID{extra_unset} 2>/dev/null; "
        "export -p; ) || true; } "
        f"> {tmp_path}")
...
def _snapshot_bootstrap_script(
    *, quoted_cwd: str, quoted_snap: str, snap_tmp_template: str, excluded_names: Iterable[str], cwd_marker: str,
    ...
    return (
        "umask 077\n"
        f"__zeloo_snap_tmp=$(mktemp {snap_tmp_template}) || exit 1\n"
        f"{_export_dump_excluding_session_vars(_SNAP_TMP, excluded_names)}\n"
        "__zeloo_fns=$(declare -F | awk '{print $3}' | grep -vE '^_[^_]') || true\n"
        f"[ -n \"$__zeloo_fns\" ] && declare -f $__zeloo_fns >> {_SNAP_TMP} 2>/dev/null || true\n"
        f"alias -p >> {_SNAP_TMP}\n"
...
def _passthrough_save_restore(names: Iterable[str]) -> tuple[list[str], list[str]]:
    ...
    restore: list[str] = []
    for name in names:
        marker = f"_ZELOO_RUNTIME_PASSTHROUGH_{name}"
...
def _wrap_command_script(
    ...
    ``declare -x`` lines when sourcing. AI_AGENT/ZELOO_AGENT advertise the harness to remote
    ...
    parts += restore
    parts += [
        'export AI_AGENT="${AI_AGENT:-Zeloo-agent}" ZELOO_AGENT="${ZELOO_AGENT:-true}"',
