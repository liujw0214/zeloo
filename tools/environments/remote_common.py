...
from tools.environments.local_env_policy import _ZELOO_PROVIDER_ENV_BLOCKLIST, _is_zeloo_internal_secret


def load_zeloo_env_vars() -> dict[str, str]:
    """``~/.Zeloo/.env`` values, or ``{}`` — a broken .env must not fail command execution."""
    try:
...
def resolve_passthrough_env(explicit_forward: Iterable[str] = (),
                            zeloo_env_loader: Callable[[], dict[str, str]] = load_zeloo_env_vars,
    ...
    is filtered through the Zeloo provider-credential blocklist and the dynamic internal-secret
    ...
    try:
        ...
        pass
    implicit_forward = {k for k in passthrough_keys if not _is_zeloo_internal_secret(k)}
    forward_keys = set(explicit_forward) | (implicit_forward - _ZELOO_PROVIDER_ENV_BLOCKLIST)
    zeloo_env = zeloo_env_loader() if forward_keys else {}
    exec_env: dict[str, str] = {}
    unset_names: set[str] = set()
    for key in sorted(forward_keys):
        value = os.getenv(key) or zeloo_env.get(key)
