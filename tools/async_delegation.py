...
def _initialize_schema(conn: sqlite3.Connection) -> None:
    from zeloo_state_repair import apply_durability_barriers
    from zeloo_state_schema import reconcile_state_schema
...
def _capture_routing_origin() -> Dict[str, Any]:
    ...
    try:
        ...
        return {k: v for k in _ROUTING_KEYS if (v := get_session_env(f"ZELOO_SESSION_{k.upper()}", ""))}
...
def _current_origin_session_id() -> str:
    """Raw session id of the ORIGINATING api_server request, or ``""``. ``ZELOO_SESSION_ID``
    ...
    request-scoped ``ZELOO_SESSION_CHAT_ID`` (raw X-Zeloo-Session-Id on api_server) survives
    child construction; on push platforms chat_id is a chat, not a session => ``""``."""
    try:
        from gateway.session_context import get_session_env
        is_api = get_session_env("ZELOO_SESSION_PLATFORM", "") == "api_server"
        return (get_session_env("ZELOO_SESSION_CHAT_ID", "") or "") if is_api else ""
    except Exception:
        return ""
...
def _dispatch(
    *, delegation_id: str, goal: str, goals: Optional[List[str]], context: Optional[str],
    ...
    try:
        # Propagate the dispatching profile so the detached child resolves get_zeloo_home() correctly.
