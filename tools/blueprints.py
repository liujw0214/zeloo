...
``metadata.Zeloo.blueprint`` (``schedule`` required; optional ``deliver`` [default "origin"],
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.Zeloo.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.Zeloo.blueprint`` is a mapping with a
    ...
    Zeloo = meta.get("Zeloo") if isinstance(meta, dict) else None
    blueprint = Zeloo.get("blueprint") if isinstance(Zeloo, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.Zeloo.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    ``metadata.Zeloo.blueprint`` block) ready for ``Zeloo skills publish``.
    ...
    frontmatter = {
        ...
        "metadata": {"Zeloo": {"tags": ["blueprint", "automation"], "blueprint": block}},
    }
