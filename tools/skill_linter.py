...
def _check_frontmatter(frontmatter: Dict[str, Any], skill_dir: Optional[Path]) -> Iterator[LintFinding]:
    ...
    zeloo_meta = meta.get("Zeloo") if isinstance(meta, dict) else None
    if not isinstance(zeloo_meta, dict):
        yield _warn("missing-metadata", "frontmatter is missing metadata.Zeloo.{tags, related_skills}.")
    elif "tags" not in zeloo_meta:
        yield _warn("missing-metadata", "metadata.Zeloo.tags is missing.")
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("Zeloo", "agent", "Zeloo agent") and (
        author != "Zeloo Agent"):
        yield _warn("author-caps", f"author '{author}' should be 'Zeloo Agent' (proper caps) "
