"""Apply a layout preset in the Zeloo desktop GUI (``layout.apply`` via ``desktop_ui``).
...
def apply_layout_tool(preset: str) -> str:
    ...
    return desktop_ui.emit_or_error(
        ...
        "Layout apply is only available in the Zeloo desktop app.", {"success": True, "preset": name},
    )
...
APPLY_LAYOUT_SCHEMA = {
    "name": "apply_layout",
    "description": (
        "Apply a saved layout preset to the Zeloo desktop app when the user "
