...
ZELOO_HOME = get_zeloo_home()  # all skills live in ~/.Zeloo/skills/ (seeded from bundled)
...
def _skills_dir() -> Path:
    ...
    it, else live profile-scoped ZELOO_HOME (long-lived runtimes may import before profile set)."""
    configured = Path(SKILLS_DIR)
    return configured if configured != _SKILLS_DIR_AT_IMPORT else get_zeloo_home() / "skills"
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from ZELOO_HOME/.env."""
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    """``~/.Zeloo/skills/mlops/axolotl/SKILL.md`` -> ``"mlops"``; active profile dir first
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    """Disabled in config? Platform precedence: explicit arg, ``ZELOO_PLATFORM``, session
    ``ZELOO_SESSION_PLATFORM``. A globally-disabled skill stays disabled on every platform
    ...
    try:
        ...
        resolved_platform = platform or os.getenv("ZELOO_PLATFORM")
        if not resolved_platform:
            with suppress(Exception):
                from gateway.session_context import get_session_env
                resolved_platform = get_session_env("ZELOO_SESSION_PLATFORM") or ""
...
def _org_provenance_header(skill_dir: Path, active_skills_dir: Path):
    ...
    header = (
        ...
        "> `Zeloo sync propose` (or automatically, if your org enables it).\n\n")
    ...
    if skill_md is not None and project_dirs:
        ...
        if _under_any(skill_md, project_dirs) and is_quarantined_project_skill(skill_md):
            return _fail(
                ...
                "`Zeloo skills untrust`."), None, None
...
def _log_security_warnings(name: str, skill_md: Path, content: str, all_dirs, active_skills_dir):
    ...
    if not _under_any(skill_md, trusted_dirs):
        warnings.append(f"skill file is outside the trusted skills directory (~/.Zeloo/skills/): {skill_md}")
...
def skill_view(
    ...
    try:
        ...
        if _is_skill_disabled(resolved_name):
            return _fail(f"Skill '{resolved_name}' is disabled. Enable it with `Zeloo skills` or inspect the files directly on disk.")
        if file_path and skill_dir:
            return _serve_skill_file(
                skill_dir, file_path, name, list_available=True, mark_read=True,
                hint="Use a relative path within the skill directory")
        # tags/related_skills: metadata.Zeloo.* (agentskills.io) first, then top-level.
        metadata = frontmatter.get("metadata")
        zeloo_meta = (metadata.get("Zeloo", {}) or {}) if isinstance(metadata, dict) else {}
        tags, related_skills = (
            _parse_tags(zeloo_meta.get(k) or frontmatter.get(k, "")) for k in ("tags", "related_skills"))
