...
broad, declarative). New skills land in ~/.Zeloo/skills/ (or ``skills.create_dir``);
...
# All skills live in ~/.Zeloo/skills/ (single source of truth)
...
def _skills_dir() -> Path:
    ...
    An explicitly patched module-level ``SKILLS_DIR`` (tests) wins over the live ZELOO_HOME.

    Long-lived multi-profile runtimes (Dashboard/TUI/Desktop backend, cron, kanban workers) import this
    module once under the launch ZELOO_HOME and later bind a different profile per session (#40677).
    """
...
def _skill_not_found_error(name: str, suffix: str = "") -> str:
    ...
    if len(others) == 1:
        ...
        base += (
            ...
            f"it, switch profiles (`Zeloo -p {other_profile}`) or edit the file directly "
            f"(file tools / terminal).")
    elif others:
        names = ", ".join(f"'{p}'" for p, _ in others)
        base += (
            f" Skills by that name exist in other profiles: {names}. Switch profiles (`Zeloo -p "
...
def _attach_lint_findings(result: Dict[str, Any], skill_md: Path) -> None:
    ...
    result["lint_hint"] = (
        ...
        "— fix them with skill_manage(action='patch') to match Zeloo skill standards.")
...
def _delete_skill(name: str, absorbed_into: Optional[str] = None) -> Dict[str, Any]:
    ...
    if unsafe := _validate_delete_target(skill_dir):  # defense-in-depth before rmtree
        return _err(unsafe)
    # Curator consolidations must be RECOVERABLE (`Zeloo curator restore`): archive instead
...
def _record_success(action, name, result, *, file_path, absorbed_into, task_id,
    ...
    # record as STATE_ARCHIVED (`Zeloo curator status`/`restore`); only a hard delete forgets.
    with suppress(Exception):
        ...
        # ~/.Zeloo/skills/.archive/ is documented as the maximum destructive action the curator may take,
        # and `Zeloo curator restore` promises the skill can be brought back. Route through the recoverable
