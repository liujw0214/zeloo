...
~/.Zeloo/skills/, tracking each synced skill's origin hash in .bundled_manifest (v2 "name:hash"
...
from zeloo_constants import get_bundled_skills_dir, get_zeloo_home, get_optional_skills_dir
...
# retarget ZELOO_HOME after import, and frozen constants would resolve (and for
...
# once under the launch ZELOO_HOME and later scope requests to a different profile via
# set_zeloo_home_override(). See #65828.
_ZELOO_HOME_AT_IMPORT = ZELOO_HOME
...
def _zeloo_home() -> Path:
    return _live(ZELOO_HOME, _ZELOO_HOME_AT_IMPORT, get_zeloo_home)


def _skills_dir() -> Path:
    return _live(SKILLS_DIR, _SKILLS_DIR_AT_IMPORT, lambda: _zeloo_home() / "skills")
...
# Written by `Zeloo profile create --no-skills` / installer `--no-skills`: sync seeds only
# essential skills. Mirrors zeloo_cli.profiles.NO_BUNDLED_SKILLS_MARKER (no CLI import here).
NO_BUNDLED_SKILLS_MARKER = ".no-bundled-skills"


def _get_bundled_dir() -> Path:  # ZELOO_BUNDLED_SKILLS env first, then repo-relative
    ...
    for candidate in st.active_index.get(skill_name, []):
        ...
        if not _matches_origin_hash(candidate, origin_hash):  # moving a customized copy would edit user work
            st.say(
                ...
                f"Run `Zeloo skills reset {skill_name} --restore` to move to the new location.")
            continue
        try:
        ...
        if dest.exists():
            ...
            if _dir_hash(dest) == bundled_hash:
                ...
                st.say(
                    ...
                    f"by this name — yours was kept. Run `Zeloo skills reset {skill_name}` to "
...
def sync_skills(quiet: bool = False) -> dict:
    """Sync bundled skills into ~/.Zeloo/skills/ using the manifest; returns the per-category
    result dict. Opted-out profiles seed ONLY ESSENTIAL_SKILLS (the system prompt always
    points at ``Zeloo-agent``)."""
    essential_only = (_zeloo_home() / NO_BUNDLED_SKILLS_MARKER).exists()
...
def _rmtree_writable(path: Path) -> None:
    ...
    child of the active skills root (bad join / missing ZELOO_HOME / malicious manifest entry).
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.Zeloo/skills/ ...")
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (_zeloo_home() / NO_BUNDLED_SKILLS_MARKER).exists()
