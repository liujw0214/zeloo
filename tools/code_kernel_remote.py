...
"""Auto-generated Zeloo REMOTE session-kernel runner (file cell protocol)."""
import contextlib
import io
import json
import os
...
KDIR = os.environ["ZELOO_KERNEL_DIR"]
...
class RemoteKernel:
    ...
    # poll loop (same guard as tools.code_kernel, Zeloo-agent#101861).
    attached: int = 0
...
def _kernel_key(owner: str, env_type: str, task_env_id: str, sandbox_tools: frozenset) -> Tuple:
    """The zeloo_tools stub module is generated from ``sandbox_tools`` once, at spawn, so a kernel
...
def _spawn_remote_kernel(env, env_type: str, owner: str, task_env_id: str,
    ...
    from tools.code_execution_tool import (
        MAX_STDOUT_BYTES, _ship_file_to_remote, _env_temp_dir, generate_zeloo_tools_module,
    )
    kernel_dir = f"{_env_temp_dir(env)}/zeloo_rkernel_{uuid.uuid4().hex[:12]}"
    ...
    try:
        ...
        _ship_file_to_remote(env, f"{kernel_dir}/zeloo_tools.py",
                             generate_zeloo_tools_module(list(sandbox_tools), transport="file"))
        env_prefix = (f"ZELOO_KERNEL_DIR={q_dir} ZELOO_RPC_DIR={shlex.quote(kernel_dir + '/rpc')} "
                      f"ZELOO_RPC_TOKEN={shlex.quote(rpc_token)} PYTHONDONTWRITEBYTECODE=1 PYTHONPATH={q_dir}")
