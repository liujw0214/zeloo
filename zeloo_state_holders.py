...
an unlinked SQLite DB/WAL/SHM generation.  ``zeloo_state`` supplies only the
...
_ZELOO_EXECUTABLES = frozenset({"zeloo", "zeloo-agent", "zeloo-acp"})
_ZELOO_PYTHON_MODULES = frozenset({"acp_adapter", "zeloo_cli.main"})
_ZELOO_PYTHON_SCRIPTS = frozenset({"zeloo_cli/main.py", "run_agent.py"})
...
def _looks_like_ZELOO(argv: Sequence[str]) -> bool:
    """Return whether argv identifies a supported Zeloo execution target."""
    if not argv:
        return False
    program = os.path.basename(argv[0]).lower().removesuffix(".exe")
    if program in _ZELOO_EXECUTABLES:
        return True
    ...
    if kind == "module":
        return normalized in _ZELOO_PYTHON_MODULES
    return any(
        normalized == script or normalized.endswith(f"/{script}")
        for script in _ZELOO_PYTHON_SCRIPTS
    )
...
def _argv_scoped_to_other_home(argv: Sequence[str], db_path: Path) -> bool:
    ...
    ``state.db`` lives at the ZELOO_HOME root, so an absolute-path token
    containing a ``/.Zeloo`` segment (or naming a ``state.db``/WAL/SHM under
    some other parent) identifies that token's own Zeloo home.  When at least
    ...
    of ours (issue #92401: a second gateway under /home/demo/.Zeloo deferred
    ...
    for token in argv:
        ...
        if path_token is not None:
            ...
            if normalized in ours or normalized.startswith(this_home + os.sep):
                return False
            if "/.Zeloo" in normalized or normalized.endswith("/.Zeloo"):
