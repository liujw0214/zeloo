"""Shared utility functions for Zeloo-agent."""
...
def warn_if_credential_file_broadly_readable(path: Union[str, Path], *, label: str = "", log: logging.Logger | None = None) -> bool:
    ...
    Hand-made secret files (or ones older Zeloo wrote without an explicit mode) commonly end up
...
def atomic_roundtrip_yaml_update(path: Union[str, Path], key_path: str, value: Any) -> None:
    ...
    # blind splitting — same navigation as ``Zeloo config set``'s ``_set_nested``; otherwise
    # /model + TUI persistence wrote ``glm-5: {'3': ...}`` phantom siblings.
    # See #91607.
    from zeloo_cli.config import _greedy_literal_match, _split_key_path
...
def atomic_roundtrip_yaml_save(path: Union[str, Path], new_state: dict) -> None:
    ...
    from zeloo_cli.config import require_readable_config_before_write
