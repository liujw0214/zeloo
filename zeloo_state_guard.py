...
_STATE_DB_GUARD_BYPASS_ENV = "ZELOO_STATE_DB_GUARD_BYPASS"


def _real_platform_state_root() -> Optional[Path]:
    """The REAL platform-default Zeloo root. Avoids ``Path.home()`` /
    ``zeloo_constants`` (tests monkeypatch Path.home to a tempdir); ``expanduser``
    reads HOME/passwd, which the conftest never rewrites."""
    try:
        home = Path(os.path.expanduser("~"))
        if sys.platform == "win32":
            base = os.environ.get("LOCALAPPDATA", "").strip()
            root = Path(base) / "Zeloo" if base else home / "AppData" / "Local" / "Zeloo"
        else:
            root = home / ".Zeloo"
        return root.resolve()
    except Exception:
...
#: Exported by the hermetic conftest alongside the ZELOO_HOME redirect. Unlike
...
# : Env marker exported by the hermetic test conftest at the same moment it : redirects ``ZELOO_HOME`` to
...
# under Zeloo test : isolation", and it inherits into subprocess children by default — so a : child that
# received the patched ``ZELOO_HOME`` also received the marker, : and a child that resolves a production DB
# while carrying it is, by : definition, an isolation escape (#82770).
_TEST_ISOLATION_MARKER_ENV = "ZELOO_TEST_ISOLATION"
...
def _has_pytest_ancestor() -> bool:
    ...
    rebuilt env loses PYTEST_* and the ZELOO_HOME redirect together, ancestry
    ...
    loses at the same moment it loses the ``ZELOO_HOME`` redirect: that child aims at the production DB
