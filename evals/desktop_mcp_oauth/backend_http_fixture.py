...
def run_probe(repo, receipt):
    ...
    owner = Path(os.environ["ZELOO_HOME"])
    ...
    with serving(Provider()) as provider, serving(CallbackServer(
        ...
        reloaded = asyncio.run(ZELOOTokenStorage("positive", zeloo_home=owner).get_tokens())
        ...
        override = set_zeloo_home_override(other)
        try:
...
def main():
    ...
    if args.cold_probe:
        sys.path.insert(0, str(repo))
        logging.disable(logging.CRITICAL)
        from zeloo_cli.mcp_config import _get_mcp_servers, _probe_single_server
    ...
    if not args.isolated_worker:
        with tempfile.TemporaryDirectory(prefix="Zeloo-oauth-http-") as temp:
            home = Path(temp)
            (home / ".Zeloo").mkdir()
            env = {"PATH": os.environ.get("PATH", ""), "HOME": temp,
                   "ZELOO_HOME": str(home / ".Zeloo"), "LANG": "C.UTF-8", "TZ": "UTC",
                   "PYTHONNOUSERSITE": "1"}
