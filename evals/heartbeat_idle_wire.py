...
Run from the repo with a clean environment and a temporary ZELOO_HOME:
...
from zeloo_cli import heartbeat  # noqa: E402
...
async def main(base_poller):
    assert os.environ.get("ZELOO_HOME"), "Use a temporary ZELOO_HOME"
    ...
    with patch.object(heartbeat, "time", SimpleNamespace(time=lambda: clock.now)):
        ...
        if not base_poller:
            ...
            runner.session_store = SessionStore(
                sessions_dir=Path(os.environ["ZELOO_HOME"]) / "sessions", config=runner.config)
