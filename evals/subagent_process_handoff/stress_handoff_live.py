...
between-turn drain receives, and score it against the runtime contract. Never trusts model prose for the verdict.
  ...
  ZELOO_WORKTREE=<tree> ZELOO_HOME=<isolated home> LIVE_MODEL=<model> python evals/subagent_process_handoff/stress_handoff_live.py [names]
"""
import json
import os
import re
...
WORKTREE = os.environ["ZELOO_WORKTREE"]
...
SCENARIOS["sibling_theft_refused"] = dict(
    prompt=("Delegate TWO tasks in ONE delegate_task call with background=false. Goals: (A) "
            + child_goal("Run terminal command sleep 60; echo MINE background=true notify=true. Write its session_id to the file $ZELOO_HOME/a_sid.txt using terminal command echo SESSION_ID > $ZELOO_HOME/a_sid.txt. Then keep it alive by running terminal command sleep 25 in the foreground, then reply done.")
            + " (B) " + child_goal("Wait 8 seconds with terminal command sleep 8. Then read $ZELOO_HOME/a_sid.txt with terminal command cat. Call process_manage action=handoff on THAT session_id with data=stolen. Report exactly what the call returned.")
