...
Zeloo current          93.3%*  26.7%* 33.3%    30.0%  45.8%   ~162K
Zeloo lean closed-book 40.0%   60.0%  23.3%    36.7%  40.0%   ~49K
Zeloo lean+recovery    70.0%   80.0%  43.3%    80.0%  68.3%   ~49K
...
4. **What exact error message is returned when 'gh pr merge --auto' is attempted on the NousResearch/Zeloo-agent repository?**  
...
9. **Which two properties shape almost every design decision in Zeloo according to the Development Guide?**  
...
10. **What error message does the live-checkout git guard display when blocking a history-rewriting command?**  
   gold: `Blocked: `git <op>` would rewrite Zeloo's live source checkout (/home/teknium/.Zeloo/Zeloo-agent) and can mix module `
...
15. **What was the specific ModuleNotFoundError message caused by the wheel subpackage discovery trap in #34701?**  
   gold: `ModuleNotFoundError: No module named 'zeloo_cli.dashboard_auth'`
...
1. **What is the PR number for the authored fix addressing mid-turn message ordering bugs in Zeloo Desktop?**  
   gold: `#86617`
...
3. **What specific file and line number were identified as the cause of an AssertionError (assert 56 == 55) in the Python tests?**  
   gold: `tests/zeloo_cli/test_session_recovery_lost_and_found.py:327`
...
8. **What error message does the Zeloo terminal tool return when a git command is blocked to prevent rewriting the live source checkout?**  
   gold: `Blocked: `git <op>` would rewrite Zeloo's live source checkout`
9. **What is the core design principle regarding 'Narrow Waist' in Zeloo development?**  
...
6. **What exact error message did the agent receive when attempting to checkout a worktree while in the live source directory?**  
   gold: `Blocked: `git checkout` would rewrite Zeloo's live source checkout (/home/teknium/.Zeloo/Zeloo-agent) and can mix mod`
...
10. **What specific SQL update was added to the migration path in zeloo_cli/kanban_db.py to prevent losing active wake on upgrade?**  
