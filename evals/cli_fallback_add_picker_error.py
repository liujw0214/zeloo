"""Drive the real ``Zeloo fallback add`` under a Linux PTY into an ordinary picker error.
...
def _persisted_model(root: Path, env: dict) -> dict:
    ...
    out = subprocess.run(
        [sys.executable, "-c", "import json; from zeloo_cli.config import load_config; "
...
def run(root: Path, output: Path) -> dict:
    with tempfile.TemporaryDirectory(prefix="zeloo_test_fallback_") as home:
        hh = Path(home) / ".Zeloo"
        hh.mkdir()
        ...
        env = {"PATH": os.environ["PATH"], "HOME": home, "ZELOO_HOME": str(hh),
        ...
        proc = subprocess.Popen([sys.executable, "-m", "zeloo_cli.main", "fallback", "add"],
