...
Each probe runs in a FRESH subprocess with an isolated ZELOO_HOME so nothing is cached across reps.
...
ENV = {**os.environ, "ZELOO_HOME": HOME, "PYTHONPATH": TREE, "PYTHONDONTWRITEBYTECODE": "0", "OPENROUTER_API_KEY": "sk-bench-placeholder",
       "ZELOO_SKIP_UPDATE_CHECK": "1", "NO_COLOR": "1", "TERM": "dumb", "COLUMNS": "120"}
for k in list(ENV):
    if k.startswith(("ZELOO_SESSION", "ZELOO_PROFILE")): ENV.pop(k)
...
IMPORT_TARGETS = ["run_agent", "cli", "zeloo_cli.main", "gateway.run", "tools.registry", "zeloo_state", "tui_gateway.server", "zeloo_cli.web_server", "model_tools", "agent.prompt_builder"]
probe = r'''
...
tree=os.getcwd(); foreign=[m for m,mod in list(sys.modules.items()) if getattr(mod,"__file__",None) and "Zeloo-agent" in mod.__file__ and not mod.__file__.startswith(tree) and "site-packages" not in mod.__file__]
...
# 2. CLI end-to-end startup: `Zeloo --version`, `Zeloo --help`, `Zeloo doctor --help`, `Zeloo config get model` (no network)
CLI = {"version": ["zeloo_cli/main.py", "--version"], "help": ["zeloo_cli/main.py", "--help"], "config_get": ["zeloo_cli/main.py", "config", "get", "model"], "tools_list": ["zeloo_cli/main.py", "tools", "--help"], "skills_help": ["zeloo_cli/main.py", "skills", "--help"]}
cli = {}
...
try:
    ...
    T("resolve_toolset_zeloo_default_ms", lambda: resolve_toolset("Zeloo-default"), 200)
...
try:
    import zeloo_state, tempfile, uuid
    from pathlib import Path
    db=zeloo_state.SessionDB(Path(tempfile.mkdtemp())/"s.db") if hasattr(zeloo_state,"SessionDB") else None
    if db:
        sid=str(uuid.uuid4())
