...
- **`forensics/`** reads a *copy* of a Zeloo `state.db` (plus rotated `agent.log*` and git) and
...
- A Zeloo checkout with its venv (`.venv/bin/python`). NeMo Relay is not required for the forensics;
  it is what the run itself used for the wire captures in `live_ab/cache_prefix_wire.py`.
- For forensics: a **copy** of `~/.Zeloo/state.db` (never point at the live file; `sqlite3 state.db ".backup copy.db"`
  or `cp` while Zeloo is idle) and, optionally, the rotated `~/.Zeloo/logs/agent.log*`.
- For `--live` probes: real credentials in `ZELOO_HOME` (they spend cents per run).
...
$P -m evals.postmortem.forensics.logcalls   --db state_copy.db --logs "$HOME/.Zeloo/logs/agent.log*"
...
Zeloo recorded (an estimator, not an invoice).
...
```bash
# offline probes (fake providers, temp ZELOO_HOME), main vs a branch or integration checkout:
...
Forensic lanes: five parallel Zeloo subagents (2026-09-04), rewritten here to take `--db`/`--root`
