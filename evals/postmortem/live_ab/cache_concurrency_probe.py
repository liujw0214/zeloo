...
Runs N independent Zeloo ``AIAgent`` sessions concurrently, each the same ~8-step tool loop whose
...
Results that motivated Zeloo-agent #104284 / #104421 and NousResearch/api#227 (2026-09-05/06,
...
Usage:
  ...
  providers: nous (Portal creds from ZELOO_HOME), openrouter (OPENROUTER_API_KEY or --api-key),
             anthropic (ANTHROPIC_API_KEY or --api-key)
Cost: ~$50 per 20x6 arm on Fable 5.1 at the 5m tier. Every run starts from the real Zeloo request
...
def _parse():
    ...
    ap.add_argument("--repo", required=True, help="Zeloo-agent checkout to import from")
    ...
    ap.add_argument("--wire", choices=["chat", "native"], default=None,
                    help="force the wire (default: what Zeloo would pick for the provider/model)")
...
os.environ.setdefault("ZELOO_HOME", os.path.expanduser("~/.Zeloo"))
import anthropic
