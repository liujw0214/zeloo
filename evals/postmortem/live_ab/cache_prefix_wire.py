...
message prefix between call N and N+1. If Zeloo strips prior-turn thinking, call N+1's messages[:k]
...
#   python cache_prefix_wire.py <repo_root> <A|B> [--Zeloo-home DIR]   (default ZELOO_HOME: the real one, for credentials)
sys.path.insert(0, sys.argv[1])
os.environ.setdefault("ZELOO_HOME", os.path.expanduser("~/.Zeloo"))
if "--Zeloo-home" in sys.argv:
    os.environ["ZELOO_HOME"] = sys.argv[sys.argv.index("--Zeloo-home") + 1]
...
time.sleep(1)
log = subprocess.run(f"grep -h '\\[{sid}\\]' ~/.Zeloo/logs/agent.log | grep 'API call #'", shell=True, capture_output=True).stdout.decode("utf-8", "replace")
