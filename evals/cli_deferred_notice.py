...
def run_case(root, output, name, behind, early=False, cancel=False):
    with tempfile.TemporaryDirectory(prefix="zeloo_test_notice_") as home:
        hh = Path(home) / ".Zeloo"
        hh.mkdir()
        ...
        from zeloo_cli.banner import VERSION
        ...
        bootstrap = ("import zeloo_cli.main as m; import zeloo_cli.banner as b; "
        ...
        try:
            ...
            result = {"case": name, "ready": ready, "exited": exited,
                      ...
                      "loaded_worktree": str(root / "zeloo_cli/banner.py") in text,
