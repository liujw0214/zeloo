"""Run the read-tool eval through the REAL Zeloo AIAgent.

For each task: fresh temp ZELOO_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    zeloo_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".Zeloo"
    ...
    try:
        ...
        shutil.rmtree(zeloo_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.Zeloo/.env; set +a  — then relaunch."
        )
