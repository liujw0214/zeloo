...
def main():
    ...
    home = Path(tempfile.mkdtemp(prefix='Zeloo-acp-empty-'))
    Zeloo = home / '.Zeloo'
    Zeloo.mkdir()
    (Zeloo / 'config.yaml').write_text(
    ...
    env.update(HOME=str(home), ZELOO_HOME=str(Zeloo), PYTHONPATH=os.pathsep.join([str(repo), os.environ.get('PYTHONPATH', '')]),
               ZELOO_ACP_SKIP_CONFIGURED_MCP='1', OPENAI_API_KEY='local-fixture-key',
    ...
    def rows():
        with sqlite3.connect(Zeloo / 'state.db') as db:
