...
Run with the Zeloo venv Python and --repo CHECKOUT --output RECEIPT_DIR.
No user environment/config is inherited by Zeloo children. Unix PTY required.
"""
import argparse
import http.server
import json
...
def rows_worker():
    ...
    from zeloo_cli.model_switch_providers import (
...
def cli_pty(repo, env):
    ...
    proc = subprocess.Popen([sys.executable, "-m", "zeloo_cli.main", "model"],
    ...
    try:
        for schema in ("providers", "custom_providers"):
            for surface in ("cli", "rows"):
                ...
                with tempfile.TemporaryDirectory(prefix="Zeloo-keycmd-live-") as tmp:
                    home = Path(tmp)
                    state = home / ".Zeloo"
                    state.mkdir()
                    ...
                    env = {"HOME": str(home), "ZELOO_HOME": str(state),
