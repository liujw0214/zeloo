...
os.environ.clear()
os.environ.update(allowed, HOME=str(home), ZELOO_HOME=str(home / "Zeloo"))
...
def scoped_systemctl(argv, *, timeout):
    if "list-units" in argv:
        ...
        argv = [arg for arg in argv if arg not in ("Zeloo-gateway*", "Zeloo-serve*")] + [unit + ".service"]
...
unit = f"Zeloo-serve-audit-089c35aa-{tag}"
cmd = ["systemctl", "--user"]
def run(args):
