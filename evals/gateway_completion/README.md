...
Run with a fresh HOME and ZELOO_HOME using the repository virtualenv:

```sh
probe=$(mktemp -d)
HOME="$probe" ZELOO_HOME="$probe/state" .venv/bin/python evals/gateway_completion/delivery_lifecycle.py
```
...
and unavailable raw API-owner retention. `ZELOO_EVAL_REPO` selects another checkout for A/B.
