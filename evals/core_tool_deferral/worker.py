...
# --- hermetic env BEFORE any Zeloo import -------------------------------
for var in list(os.environ):
    if var.endswith(("_API_KEY", "_TOKEN")) and var != "OPENROUTER_API_KEY":
        os.environ.pop(var, None)
os.environ.pop("FAL_KEY", None)
os.environ.pop("ZELOO_PROFILE", None)

tmp_root = tempfile.mkdtemp(prefix=f"ab-{ARM}-{TASK_ID}-")
zeloo_home = os.path.join(tmp_root, ".Zeloo")
workspace = os.path.join(tmp_root, "ws")
os.makedirs(zeloo_home)
os.makedirs(workspace)
with open(os.path.join(zeloo_home, "config.yaml"), "w", encoding="utf-8") as f:
    f.write("model:\n  provider: openrouter\n  model: %s\n" % MODEL)
...
PREAMBLE = ("You are running inside the Zeloo desktop app on the user's machine. "
...
ctx = {
    "workspace": workspace, "zeloo_home": zeloo_home,
