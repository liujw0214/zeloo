...
def main():
    ...
    env = {"PATH": "/usr/bin:/bin", "HOME": str(root / "home"),
           "ZELOO_HOME": str(root / "home/.Zeloo"),
           "XDG_DATA_HOME": str(root / "xdg"), "LANG": "C.UTF-8",
           "ZELOO_NONINTERACTIVE": "1"}
    Path(env["ZELOO_HOME"]).mkdir(parents=True)
    venv_dir = root / "venv"
    ...
    install = (
        ...
        "from zeloo_cli.linux_desktop_entry import install_desktop_entry; "
        f"sys.argv[0]={str(repo / 'Zeloo')!r}; "
    ...
    rows["venv_import"] = run([str(python), "-I", "-c", "import yaml,zeloo_cli.main; print(yaml.__version__)"], env, "/")
    rows["base_import_negative"] = run([str(python.resolve()), "-I", "-c", "import yaml,zeloo_cli.main"], env, "/")
    ...
    custom = original.replace(b"Name=Zeloo\n", b"Name=Zeloo custom\n").replace(b"Terminal=false", b"Terminal=true")
    entry.write_bytes(custom)
    config = Path(env["ZELOO_HOME"]) / "config.yaml"
