...
env.update(HOME=str(home), ZELOO_HOME=str(home), ZELOO_DASHBOARD_SESSION_TOKEN='history-fixture-token', PYTHONPATH=repo, ZELOO_NONINTERACTIVE='1')
cmd=[sys.executable, '-m', 'zeloo_cli.main', 'serve', '--host', '127.0.0.1', '--port', str(args.port), '--isolated']
...
try:
    for _ in range(120):
        try:
            req=urllib.request.Request(f'http://127.0.0.1:{args.port}/api/status',headers={'X-Zeloo-Token':'history-fixture-token'})
