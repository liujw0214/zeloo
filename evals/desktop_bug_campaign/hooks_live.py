...
env = {k: v for k, v in os.environ.items() if not (k.startswith('zeloo_') or k.endswith(('_API_KEY', '_TOKEN')))}
env.update(HOME=str(out / 'os-home'), ZELOO_HOME=str(home), ZELOO_DASHBOARD_SESSION_TOKEN='hooks-fixture-token', ZELOO_IGNORE_RULES='1', PYTHONPATH=str(a.repo))
cmd = [sys.executable, '-m', 'zeloo_cli.main', 'serve', '--host', '127.0.0.1', '--port', str(a.port), '--skip-build']
