...
def backend(port):
    from zeloo_cli.web_server import app, start_server
...
''')
    ...
    (home / 'user').mkdir()
    env.update(ZELOO_HOME=str(home), ZELOO_DASHBOARD_SESSION_TOKEN='lease-probe',
    ...
    def control(port, action, sid):
        req = urllib.request.Request(f'http://127.0.0.1:{port}/lease-probe/{action}/{sid}', data=b'', method='POST', headers={'X-Zeloo-Session-Token': 'lease-probe'})
