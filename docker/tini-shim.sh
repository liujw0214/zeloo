...
# Zeloo WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: ZELOO_TINI_SHIM_TARGET overrides the `/init` path (and
# ZELOO_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${ZELOO_TINI_SHIM_TARGET:-/init}"
WRAPPER="${ZELOO_TINI_SHIM_WRAPPER:-/opt/Zeloo/docker/main-wrapper.sh}"
