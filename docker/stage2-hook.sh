...
#
# Wired into the image as /etc/cont-init.d/01-Zeloo-setup by the
...
ZELOO_HOME="${ZELOO_HOME:-/opt/data}"
INSTALL_DIR="/opt/Zeloo"

# Drop to Zeloo via s6-setuidgid, but skip it when already non-root.
as_ZELOO() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid Zeloo "$@"; }
...
# starts non-root. The baked install tree under /opt/Zeloo is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $ZELOO_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass ZELOO_UID/ZELOO_GID — or the PUID/PGID aliases — which the
...
# `id -u Zeloo` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# Zeloo UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u Zeloo)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-Zeloo UID).
...
and the baked /opt/Zeloo install tree is intentionally root-owned and
...
# --- Bootstrap ZELOO_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid Zeloo mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom ZELOO_HOME paths whose parents only
# root can create (e.g. `ZELOO_HOME=/home/Zeloo/.Zeloo` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for ZELOO_UID/ZELOO_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  ZELOO_UID/
# ZELOO_GID still win when both are set.  See #15290, salvages #25872.
ZELOO_UID="${ZELOO_UID:-${PUID:-}}"
ZELOO_GID="${ZELOO_GID:-${PGID:-}}"

if [ -n "${ZELOO_UID:-}" ] && validate_uid_gid "$ZELOO_UID" && [ "$ZELOO_UID" != "$(id -u Zeloo)" ]; then
    echo "[stage2] Changing Zeloo UID to $ZELOO_UID"
    usermod -u "$ZELOO_UID" Zeloo
fi
if [ -n "${ZELOO_GID:-}" ] && validate_uid_gid "$ZELOO_GID" && [ "$ZELOO_GID" != "$(id -g Zeloo)" ]; then
    echo "[stage2] Changing Zeloo GID to $ZELOO_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$ZELOO_GID" Zeloo 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised Zeloo user (UID 10000)
...
# Zeloo process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes Zeloo. Idempotent across container
...
#   - Zeloo is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G Zeloo 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] Zeloo already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" Zeloo 2>/dev/null; then
        echo "[stage2] Added Zeloo to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group Zeloo failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When ZELOO_UID is remapped or the top-level $ZELOO_HOME isn't owned by
# the runtime Zeloo UID, restore ownership to Zeloo — but ONLY for the
# directories Zeloo actually writes to. The full $ZELOO_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of Zeloo-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_zeloo_uid=$(id -u Zeloo)
...
chown_zeloo_tree() {
    ...
    chown -R Zeloo:Zeloo "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

tree_has_non_zeloo_owner() {
    target="$1"
    find "$target" \( ! -user Zeloo -o ! -group Zeloo \) -print -quit 2>/dev/null | grep -q .
}

needs_chown=false
if [ "$(stat -c %u "$ZELOO_HOME" 2>/dev/null)" != "$actual_zeloo_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $ZELOO_HOME (targeted) to Zeloo ($actual_zeloo_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $ZELOO_HOME: chown the directory itself (not its contents)
    # so Zeloo can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$ZELOO_HOME"; then
        :
    else
        chown Zeloo:Zeloo "$ZELOO_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $ZELOO_HOME failed (rootless container?) — continuing"
    fi
    # Zeloo-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by Zeloo (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$ZELOO_HOME/$sub" ] && tree_has_non_zeloo_owner "$ZELOO_HOME/$sub"; then
            chown_zeloo_tree "$ZELOO_HOME/$sub"
        fi
    done
fi
...
# Zeloo user. Hosted/container instances keep mutable state under
# $ZELOO_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# ZELOO_DISABLE_LAZY_INSTALLS=1. Keeping /opt/Zeloo root-owned and
...
# $ZELOO_HOME/lazy-packages dir on the data volume (Dockerfile sets
# ZELOO_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to Zeloo
...
# Always reset ownership of $ZELOO_HOME/profiles to Zeloo on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> Zeloo …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as Zeloo and walks
...
if [ -d "$ZELOO_HOME/profiles" ] && tree_has_non_zeloo_owner "$ZELOO_HOME/profiles"; then
    chown_zeloo_tree "$ZELOO_HOME/profiles"
fi

# Always reset ownership of $ZELOO_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged Zeloo runtime even
...
# gate as profiles/).
if [ -d "$ZELOO_HOME/cron" ] && tree_has_non_zeloo_owner "$ZELOO_HOME/cron"; then
    chown_zeloo_tree "$ZELOO_HOME/cron"
fi

# Always ensure logs/gateways is Zeloo-owned (#45258). Formerly healed by
...
# top-level $ZELOO_HOME is mis-owned, so a warm volume with Zeloo-owned
# ZELOO_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid Zeloo mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as Zeloo.
if [ -d "$ZELOO_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$ZELOO_HOME/logs/gateways"; then
        :
    else
        chown Zeloo:Zeloo "$ZELOO_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# Zeloo pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged Zeloo gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $ZELOO_HOME is
...
# and cron/.
if [ -d "$ZELOO_HOME/platforms/pairing" ] && tree_has_non_zeloo_owner "$ZELOO_HOME/platforms/pairing"; then
    chown_zeloo_tree "$ZELOO_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$ZELOO_HOME/pairing" ] && tree_has_non_zeloo_owner "$ZELOO_HOME/pairing"; then
    chown_zeloo_tree "$ZELOO_HOME/pairing"
fi

# Reset ownership of Zeloo-owned top-level state files on every boot.
# The targeted data-volume chown above only covers Zeloo-owned
# *subdirectories*; loose state files living directly under $ZELOO_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> Zeloo …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged Zeloo runtime then hits
...
# sweep so host-owned files in a bind-mounted $ZELOO_HOME are never
...
# entries of zeloo_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$ZELOO_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$ZELOO_HOME/$f"; then
            :
        else
            chown Zeloo:Zeloo "$ZELOO_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the Zeloo runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$ZELOO_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$ZELOO_HOME/config.yaml"; then
        :
    else
        chown Zeloo:Zeloo "$ZELOO_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$ZELOO_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as Zeloo user ---
# Run as Zeloo via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $ZELOO_HOME values
...
as_ZELOO mkdir -p \
    ...
    "$ZELOO_HOME/sessions" \
    "$ZELOO_HOME/logs" \
    "$ZELOO_HOME/logs/gateways" \
    "$ZELOO_HOME/hooks" \
    "$ZELOO_HOME/memories" \
    "$ZELOO_HOME/skills" \
    "$ZELOO_HOME/skins" \
    "$ZELOO_HOME/plans" \
    "$ZELOO_HOME/workspace" \
    "$ZELOO_HOME/home" \
    "$ZELOO_HOME/pairing" \
    "$ZELOO_HOME/platforms/pairing" \
    "$ZELOO_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/Zeloo/.install_method (see Dockerfile), NOT written here into
# $ZELOO_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $ZELOO_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.Zeloo:/opt/data) and sometimes shared with a
...
# 'Zeloo update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $ZELOO_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/Zeloo stamp, so deleting the data-dir copy is safe).
if [ -f "$ZELOO_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$ZELOO_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$ZELOO_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$ZELOO_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$ZELOO_HOME/$dest"; then
            :
        else
            as_ZELOO cp "$INSTALL_DIR/$src" "$ZELOO_HOME/$dest"
        fi
    fi
}
...
# the first-boot seed above depends on /opt/Zeloo/.env.example being present
...
# not generate one. Zeloo loads $ZELOO_HOME/.env with override=True, so
...
if [ -n "${API_SERVER_KEY:-}" ]; then
    if [ -f "$ZELOO_HOME/.env" ] && grep -q '^API_SERVER_KEY=..*' "$ZELOO_HOME/.env" 2>/dev/null; then
        echo "[stage2] Warning: API_SERVER_KEY is set in both the container environment and $ZELOO_HOME/.env — the .env value wins at runtime (loaded with override=True)"
    else
        ...
        if [ -f "$ZELOO_HOME/.env" ] && ! refuse_symlinked_path "clean" "$ZELOO_HOME/.env"; then
            sed -i '/^API_SERVER_KEY=$/d' "$ZELOO_HOME/.env" 2>/dev/null || true
        fi
        echo "[stage2] API_SERVER_KEY provided via container environment — skipping generation"
    fi
elif ! grep -q '^API_SERVER_KEY=..*' "$ZELOO_HOME/.env" 2>/dev/null; then
    if refuse_symlinked_path "append" "$ZELOO_HOME/.env"; then
        :
    else
        if [ ! -f "$ZELOO_HOME/.env" ]; then
            ...
            (umask 077 && as_ZELOO touch "$ZELOO_HOME/.env") 2>/dev/null || true
        fi
        if [ -f "$ZELOO_HOME/.env" ]; then
            ...
            if [ -n "$_gen_key" ]; then
                ...
                sed -i '/^API_SERVER_KEY=$/d' "$ZELOO_HOME/.env" 2>/dev/null || true
                if printf 'API_SERVER_KEY=%s\n' "$_gen_key" >> "$ZELOO_HOME/.env" 2>/dev/null; then
                    echo "[stage2] Generated API_SERVER_KEY for the loopback gateway api_server"
                else
                    echo "[stage2] Warning: could not write API_SERVER_KEY to $ZELOO_HOME/.env (read-only volume?) — gateway api_server (cron fires) will be unavailable"
                fi
            fi
            unset _gen_key
        else
            echo "[stage2] Warning: could not create $ZELOO_HOME/.env — gateway api_server (cron fires) will be unavailable"
        fi
    fi
fi
...
if [ -f "$ZELOO_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$ZELOO_HOME/.env"; then
        :
    else
        chown Zeloo:Zeloo "$ZELOO_HOME/.env" 2>/dev/null || true
        chmod 600 "$ZELOO_HOME/.env" 2>/dev/null || true
    fi
fi
...
# as the unprivileged `Zeloo` user — without this it gets EACCES on every
...
# Zeloo group (any group member could e.g. stop/suspend this machine), not
...
if [ -S /.fly/api ]; then
    if refuse_symlinked_path "chgrp/chmod" /.fly/api; then
        :
    elif chgrp Zeloo /.fly/api 2>/dev/null && chmod g+w /.fly/api 2>/dev/null; then
        echo "[stage2] Granted Zeloo group access to the Fly Machines API socket"
    else
...
# $ZELOO_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `Zeloo update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set ZELOO_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$ZELOO_HOME/config.yaml" ]; then
    s6-setuidgid Zeloo "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$ZELOO_HOME/auth.json" ] && [ -n "${ZELOO_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$ZELOO_HOME/auth.json"; then
        :
    else
        printf '%s' "$ZELOO_AUTH_JSON_BOOTSTRAP" > "$ZELOO_HOME/auth.json"
        chown Zeloo:Zeloo "$ZELOO_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$ZELOO_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via ZELOO_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$ZELOO_HOME/auth.json" ] && [ -n "${ZELOO_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$ZELOO_HOME/auth.json"; then
        :
    else
        s6-setuidgid Zeloo "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$ZELOO_HOME/auth.json" \
...
# can set ZELOO_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# ZELOO_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$ZELOO_HOME/gateway_state.json" ] && \
        [ "${ZELOO_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$ZELOO_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$ZELOO_HOME/gateway_state.json"
        chown Zeloo:Zeloo "$ZELOO_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$ZELOO_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_ZELOO "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/Zeloo/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Zeloo spawns for the
...
# main-wrapper.sh propagates it into the supervised ``Zeloo`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-Zeloo, dashboard, etc.).
