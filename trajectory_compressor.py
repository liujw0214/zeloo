...
from zeloo_constants import OPENROUTER_BASE_URL, get_zeloo_home
from agent.retry_utils import jittered_backoff
from zeloo_cli.env_loader import load_zeloo_dotenv

# Load .env from ZELOO_HOME first, then project root as a dev fallback.
load_zeloo_dotenv(zeloo_home=get_zeloo_home(), project_env=Path(__file__).parent / ".env")
...
class TrajectoryCompressor:
    ...
    def _init_summarizer(self):
        ...
        if provider:
            ...
            if client is None:
                raise RuntimeError(f"Provider '{provider}' is not configured. Check your API key or run: Zeloo setup")
