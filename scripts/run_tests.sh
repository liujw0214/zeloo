#!/usr/bin/env bash
# Canonical test runner for Zeloo-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.Zeloo/...)
#
# Usage:
...
# (ZELOO_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.Zeloo/Zeloo-agent/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.Zeloo/Zeloo-agent/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${ZELOO_PYTHON:-}" ] && [ -x "$ZELOO_PYTHON" ] \
    && "$ZELOO_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: ZELOO_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `Zeloo` binary rather
  # than the devShell hook.
  PYTHON="$ZELOO_PYTHON"
  echo "▶ no local venv — using Nix dev venv via ZELOO_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and ZELOO_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.Zeloo/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.Zeloo"
...
#
#   * ZELOO_TEST_WORKERS / PATHS / FILE_TIMEOUT / FILE_RETRIES / SLICE are
#     read by run_tests_parallel.py at argparse-default time — inside the
#     stripped environment.
#   * ZELOO_TEST_IMAGE is read by tests/docker/conftest.py to skip its
...
#     ZELOO_GIT_SHA build-arg the workflow bakes in).
#
# These are test-infrastructure knobs, not credentials — same class as the
# ZELOO_RUN_SLOW_PET_TESTS / ZELOO_E2E_BROWSER opt-ins already forwarded.
# Keep this an explicit allowlist (no ZELOO_TEST_* glob) so the "no
# credential can leak" property stays auditable at a glance.
TEST_ENV=()
for _test_var in ZELOO_TEST_IMAGE ZELOO_TEST_WORKERS ZELOO_TEST_PATHS \
  ZELOO_TEST_FILE_TIMEOUT ZELOO_TEST_FILE_RETRIES ZELOO_TEST_SLICE; do
  if [ -n "${!_test_var:-}" ]; then
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${ZELOO_RUN_SLOW_PET_TESTS:+ZELOO_RUN_SLOW_PET_TESTS="$ZELOO_RUN_SLOW_PET_TESTS"} \
  ${ZELOO_E2E_BROWSER:+ZELOO_E2E_BROWSER="$ZELOO_E2E_BROWSER"} \
