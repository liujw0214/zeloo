#!/usr/bin/env python3
"""Live test harness for Zeloo Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any Zeloo imports.
ORIGINAL_HOME = os.environ.get("ZELOO_HOME")
ORIGINAL_AUTH = Path.home() / ".Zeloo" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.Zeloo/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.Zeloo/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="zeloo_ts_live_"))
    zeloo_home = home_dir / ".Zeloo"
    zeloo_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, zeloo_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".Zeloo" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, zeloo_home / ".env")
        ...
        load_zeloo_dotenv(zeloo_home=str(Path.home() / ".Zeloo"))
    ...
    (zeloo_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return zeloo_home
...
def reset_module_state():
    """Drop cached modules so the new ZELOO_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "zeloo_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["ZELOO_HOME"] = str(home)
...
def main():
    ...
    # Restore original ZELOO_HOME
    if ORIGINAL_HOME is not None:
        os.environ["ZELOO_HOME"] = ORIGINAL_HOME
    else:
