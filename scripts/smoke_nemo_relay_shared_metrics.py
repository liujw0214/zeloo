"""Run a real Zeloo CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_zeloo_executable(zeloo_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "Zeloo",
        Path(".venv") / "Scripts" / "Zeloo.exe",
    ):
        candidate = zeloo_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("Zeloo")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Zeloo executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Zeloo source checkout containing .venv/bin/Zeloo",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated ZELOO_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["Zeloo.client.active"] != [
        {
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['Zeloo.client.active']}"
        )
    [model] = by_name["Zeloo.model_route.count"]
    expected_model = {
        "name": "Zeloo.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['Zeloo.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["Zeloo.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['Zeloo.task_run.started']}"
        )
    [terminal] = by_name["Zeloo.task_run.finished"]
    ...
    [tool] = by_name["Zeloo.tool_call.count"]
    ...
    lifecycle = by_name["Zeloo.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["Zeloo.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Zeloo development environment requires jsonschema"
        ) from exc
    ...
    if metrics["Zeloo.client.active"] != [
        {
            "name": "Zeloo.client.active",
            "type": "counter",
            "dimensions": {},
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['Zeloo.client.active']}"
        )
    [model] = metrics["Zeloo.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['Zeloo.model_route.count']}"
        )
    [terminal] = metrics["Zeloo.task_run.finished"]
    ...
    [tool] = metrics["Zeloo.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["Zeloo.skill.lifecycle.count"]
    ...
    loads = metrics["Zeloo.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    zeloo_repo = args.zeloo_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    Zeloo = _resolve_zeloo_executable(zeloo_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="Zeloo-relay-shared-metrics-"))
    home = root / "Zeloo-home"
    ...
    (root / "Zeloo.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "Zeloo.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Zeloo exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Zeloo model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Zeloo did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Zeloo did not print the mock model response")
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "Zeloo.shared_metrics.v2.schema.json",
    )

    print("Zeloo -> NeMo Relay shared-metrics smoke test passed")
