...
# tree generates llms.txt — the index every LLM (Zeloo included, via the
# Zeloo-agent skill) reads to learn what Zeloo can do — and
...
_MCP_CATALOG_FILES = {"zeloo_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (Zeloo-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``Zeloo serve`` backend, the Docker image)
