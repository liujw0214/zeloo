# Behavioral test for install.ps1's Zeloo launcher staging (PR #92092,
...
#
# The test lifts the real Install-ZELOOCommandLaunchers function from the
...
# the caller (Set-PathVariable passes $ZELOOHome\bin -- the managed binary
...
$fn = $ast.Find({
    ...
    $n.Name -eq 'Install-ZELOOCommandLaunchers'
}, $true)

if (-not $fn) {
    throw "Install-ZELOOCommandLaunchers not found in $installPs1"
}
...
$caseRoot = [System.IO.Path]::GetFullPath((Join-Path $tempBase (
    'Zeloo-cli-launcher-test-' + [guid]::NewGuid().ToString('N')
)))
...
try {
    $installRoot = Join-Path $caseRoot 'Zeloo-agent'
    ...
    $missingThrew = $false
    try {
        Install-ZELOOCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    } catch {
        $missingThrew = $_.Exception.Message -like '*required launcher not found*'
    }
    Assert-True $missingThrew 'missing Zeloo.exe fails the launcher stage'
    ...
    $ZELOOV1 = [byte[]](77, 90, 1)
    $ZELOOV2 = [byte[]](77, 90, 2)
    $acp = [byte[]](77, 90, 3)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'Zeloo.exe'), $ZELOOV1)
    Set-Content -Path (Join-Path $installRoot 'venv\pyvenv.cfg') `
        -Value "home = X" -Encoding Ascii

    $staged = Install-ZELOOCommandLaunchers -Root $installRoot -Destination $binDir
    Assert-True ($staged -eq $binDir) 'returns the destination it staged into'
    Assert-BytesEqual $ZELOOV1 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'Zeloo.exe'))) `
        'normal venv: exe copy lands in the destination'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'Zeloo-acp.exe'))) `
        'optional ACP launcher may be absent'

    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'Zeloo.exe'), $ZELOOV2)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'Zeloo-acp.exe'), $acp)
    Install-ZELOOCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-BytesEqual $ZELOOV2 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'Zeloo.exe'))) `
        'installer refreshes an existing Zeloo launcher'
    Assert-BytesEqual $acp `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'Zeloo-acp.exe'))) `
    ...
    Install-ZELOOCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-True (Test-Path -LiteralPath (Join-Path $binDir 'Zeloo.cmd')) `
        'relocatable venv: .cmd delegator staged'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'Zeloo.exe'))) `
        'relocatable venv: stale exe copy removed'
    $cmdBody = [System.IO.File]::ReadAllText((Join-Path $binDir 'Zeloo.cmd'))
    Assert-True ($cmdBody.Contains((Join-Path $scriptsDir 'Zeloo.exe')) -and $cmdBody.Contains('%*')) `
