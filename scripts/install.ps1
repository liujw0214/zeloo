# ============================================================================
# Zeloo Agent Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   iex (irm https://Zeloo-agent.nousresearch.com/install.ps1)
#
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$ZELOOHome = $(if ($env:ZELOO_HOME) { $env:ZELOO_HOME } else { "$env:LOCALAPPDATA\Zeloo" }),
    [string]$InstallDir = $(if ($env:ZELOO_HOME) { "$env:ZELOO_HOME\Zeloo-agent" } else { "$env:LOCALAPPDATA\Zeloo\Zeloo-agent" }),
    ...
    # builds apps/desktop into a launchable Zeloo.exe.
    #
    # Why opt-in:
    #   * Zeloo-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Zeloo.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Zeloo.exe
    ...
    #     `Zeloo desktop` already builds on demand.
...
# ZELOO_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[Zeloo] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'ZELOOInstall.LongPath').Type) {
            Add-Type -Namespace 'ZELOOInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [ZELOOInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -ZELOOHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('ZELOOHome')) {
    $ZELOOHome = ConvertTo-LongPath $ZELOOHome
} else {
    $ZELOOHome = ConvertTo-LongPath $(
        if ($env:ZELOO_HOME) { $env:ZELOO_HOME } else { "$env:LOCALAPPDATA\Zeloo" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:ZELOO_HOME) { "$env:ZELOO_HOME\Zeloo-agent" } else { "$env:LOCALAPPDATA\Zeloo\Zeloo-agent" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: ZELOOHome=$ZELOOHome InstallDir=$InstallDir"
}
...
$RepoUrlSsh = "git@github.com:NousResearch/Zeloo-agent.git"
$RepoUrlHttps = "https://github.com/NousResearch/Zeloo-agent.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * Zeloo Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $ZELOOHome)) {
        New-Item -ItemType Directory -Force -Path $ZELOOHome | Out-Null
    }
    $envFile = Join-Path $ZELOOHome ".env"
...
function Install-AgentBrowser {
    ...
    # which every consumer (tools/browser_tool.py, `Zeloo update`'s npx
    ...
    $prefixDir = Join-Path $ZELOOHome "node"
...
function Install-Uv {
    # Zeloo owns its own uv at $ZELOOHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (zeloo_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `Zeloo update` stay in sync.
    $managedUv = Join-Path $ZELOOHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $ZELOOHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $ZELOOHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $ZELOOHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $ZELOOHome "bin"
        ...
        # (zeloo_cli/managed_uv.py looks only at $ZELOOHome\bin\uv.exe).
...
# Put the Zeloo-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone Zeloo-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Zeloo-managed Node tree's bundled npm into $NpmRange when
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "Zeloo-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("Zeloo-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and ZELOO_HOME wasn't propagated).
...
function Initialize-ManagedPythonEnvironment {
    # Python used by Zeloo belongs to the checkout, never to another
    # application or a user-level uv configuration. Keep this aligned with
    # zeloo_cli.managed_uv.managed_python_env(), which owns the update path.
    foreach ($name in @(
    ...
    $managedRoot = Join-Path $InstallDir ".Zeloo-runtime\python"
...
function Resolve-AvailablePythonVersion {
    # Return the path and minor version of the first Zeloo-managed interpreter
    ...
    #
    # Under Zeloo-Setup.exe each stage runs in a fresh powershell.exe. The
    ...
    foreach ($ver in $candidates) {
        ...
        try {
            ...
            throw "Failed to resolve Zeloo-managed Python $ver`: $_"
        } finally {
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Zeloo setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Zeloo uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\Zeloo\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Zeloo needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Zeloo copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\Zeloo\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``ZELOO_GIT_BASH_PATH`` (User scope) so Zeloo can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Zeloo-managed PortableGit install instead..."
    }

    # Download PortableGit into $ZELOOHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $ZELOOHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Zeloo features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Zeloo needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``ZELOO_GIT_BASH_PATH`` (User env scope) so Zeloo can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $ZELOOHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $ZELOOHome\git\usr\bin\bash.exe
    $candidates += "$ZELOOHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$ZELOOHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("ZELOO_GIT_BASH_PATH", $candidate, "User")
            $env:ZELOO_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set ZELOO_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Zeloo may not find Git Bash."
    Write-Info "If needed, set ZELOO_GIT_BASH_PATH manually to your bash.exe path."
}
...
# range used to provision the Zeloo-managed tree. Keeping this probe separate
...
function Test-SystemNodeReady {
    ...
    if (Test-NodeVersionOk $version) {
        ...
        Write-Warn "Node.js $version is unsupported (Zeloo requires Node 22.22+, 24.11+, or 26+)"
        return $false
    }
    ...
    if ($npmVersion) {
        Write-Warn "Node.js $version uses npm $npmVersion, which does not satisfy Zeloo requirement $npmRange"
    } else {
...
function Test-Node {
    ...
    Write-Info "Using a Zeloo-managed Node.js installation instead..."

    # Prefer a Zeloo-managed Node from a previous run over a too-old system one.
    $managedNode = "$ZELOOHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$ZELOOHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$ZELOOHome\node"
        Write-Success "Node.js $version found (Zeloo-managed)"
        ...
        Update-ManagedNpm "$ZELOOHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Zeloo-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $ZELOOHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $ZELOOHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\Zeloo-node-extract"
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$ZELOOHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$ZELOOHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$ZELOOHome\node") {
                    try {
                        Rename-Item "$ZELOOHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "Zeloo-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$ZELOOHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $ZELOOHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\Zeloo-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `Zeloo
                    # update` path (#4735).
                    ...
                    $stashName = "Zeloo-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. Zeloo-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``Zeloo update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Zeloo behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running Zeloo processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/NousResearch/Zeloo-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/NousResearch/Zeloo-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/NousResearch/Zeloo-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\Zeloo-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\Zeloo-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `Zeloo update` (see the notes at the shared clone-path
    ...
    # next `Zeloo update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Zeloo-Setup.exe
    ...
    if (-not $resolvedPython) {
        throw "Zeloo-managed Python is unavailable. Run install.ps1 -Stage python first."
    }
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running Zeloo process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running Zeloo processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*ZELOO_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (Zeloo.exe) plus its child tree.
            & taskkill /F /T /IM Zeloo.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM Zeloo.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m zeloo_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not Zeloo.exe.
            ...
            # Zeloo children re-exec through .Zeloo-runtime, so killing only
            ...
            # user (see zeloo_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-Zeloo " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `Zeloo update` / the
...
function Restore-VenvBackup {
    ...
    # venv so Zeloo (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `Zeloo.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install Zeloo-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and Zeloo.exe broken with
    ...
    # here, not 30 seconds later when the user runs `Zeloo`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close Zeloo processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close Zeloo processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves Zeloo and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register Zeloo.exe in dist-info/RECORD but fail to
        ...
        # `Zeloo` command while Zeloo-agent.exe / Zeloo-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m zeloo_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `Zeloo dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\zeloo_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `Zeloo dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `Zeloo dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: zeloo_cli/web_server.py"
        }
    }
...
function Install-ZELOOCommandLaunchers {
    param(
        [Parameter(Mandatory=$true)] [string]$Root,
        [Parameter(Mandatory=$true)] [string]$Destination
    )

    # Expose ONLY the Zeloo launchers on PATH -- never the whole
    # venv\Scripts directory, which contains python.exe / pip.exe and
    # silently hijacks the `python` command in every terminal (#83797).
    # Requiring Zeloo.exe before creating the destination keeps the PATH
    # stage from reporting success with an unusable command (PR #92092).
    $scriptsDir = Join-Path $Root "venv\Scripts"
    $requiredSource = Join-Path $scriptsDir "Zeloo.exe"
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the Zeloo command: required launcher not found: $requiredSource"
    }

    New-Item -ItemType Directory -Force -Path $Destination | Out-Null

    # Launcher form depends on the venv (keep in lockstep with
    # zeloo_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    foreach ($launcher in @("Zeloo", "Zeloo-acp")) {
    ...
    $requiredExe = Join-Path $Destination "Zeloo.exe"
    $requiredCmd = Join-Path $Destination "Zeloo.cmd"
    if (-not ((Test-Path -LiteralPath $requiredExe -PathType Leaf) -or
              (Test-Path -LiteralPath $requiredCmd -PathType Leaf))) {
        throw "Cannot set up the Zeloo command: launcher was not installed: $requiredExe"
    }
    return $Destination
}

function Set-PathVariable {
    Write-Info "Setting up Zeloo command..."
    
    if ($NoVenv) {
        $ZELOOBin = "$InstallDir"
    } else {
        # $ZELOOHome\bin is the managed binary dir (shared with the managed
        # uv), OUTSIDE the git checkout: `Zeloo update`'s autostash
        ...
        # installer staged under Zeloo-agent\bin. No git operation can ever
        # touch this dir. Staging and verification live in
        # Install-ZELOOCommandLaunchers, which throws BEFORE any PATH
        # mutation when the launchers cannot be staged.
        $ZELOOBin = "$ZELOOHome\bin"
        Install-ZELOOCommandLaunchers -Root $InstallDir -Destination $ZELOOBin | Out-Null
    }
    ...
    #   Zeloo-agent\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The Zeloo-agent\bin FILES are left in place on purpose: editor/ACP
    ...
    if (-not $NoVenv) {
        ...
        if ($cleaned.Count -ne $items.Count) {
            ...
            Write-Info "Removed legacy launcher entries from user PATH (kept Zeloo via $ZELOOBin)"
        }
    }
    
    if ($currentPath -notlike "*$ZELOOBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$ZELOOBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $ZELOOBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set ZELOO_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\Zeloo instead
    # of the Unix default ~/.Zeloo
    $currentZELOOHome = [Environment]::GetEnvironmentVariable("ZELOO_HOME", "User")
    if (-not $currentZELOOHome -or $currentZELOOHome -ne $ZELOOHome) {
        [Environment]::SetEnvironmentVariable("ZELOO_HOME", $ZELOOHome, "User")
        Write-Success "Set ZELOO_HOME=$ZELOOHome"
    }
    $env:ZELOO_HOME = $ZELOOHome
    
    # Update current session
    $env:Path = "$ZELOOBin;$env:Path"
    
    Write-Success "Zeloo command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.Zeloo-bootstrap-complete which tells the Zeloo
    ...
    $markerPath = Join-Path $InstallDir ".Zeloo-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the ZELOO_HOME directory structure ($ZELOOHome, default %LOCALAPPDATA%\Zeloo)
    New-Item -ItemType Directory -Force -Path "$ZELOOHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZELOOHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZELOOHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZELOOHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZELOOHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZELOOHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZELOOHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZELOOHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZELOOHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$ZELOOHome\.env"
    ...
    # Create config.yaml
    $configPath = "$ZELOOHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Zeloo's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$ZELOOHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in zeloo_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $ZELOOHome"
    
    # Seed bundled skills into $ZELOOHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $ZELOOHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $ZELOOHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$ZELOOHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $ZELOOHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Zeloo-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'Zeloo setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\Zeloo-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\Zeloo-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\Zeloo-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `Zeloo tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $ZELOOHome "bin"
    $managedBu = Join-Path $managedBin "browser-use.exe"
    # MANAGED-FIRST: only Zeloo' managed copy short-circuits. A browser-use
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside Zeloo' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'Zeloo tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `Zeloo
...
# paths still lazy-install via install_cua_driver() (zeloo_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `Zeloo computer-use install` runs. Bounded
        ...
        # Zeloo' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'Zeloo tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: Zeloo computer-use install"
            }
        } else {
...
# the final `electron` -> `Zeloo` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Zeloo.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_zeloo_desktop command
    # resolves apps/desktop/release/win-unpacked/Zeloo.exe directly,
    ...
    # enforces the supported Node lines and prepends the Zeloo-managed Node to
    ...
    # The --dir mode produces an unpacked Zeloo.exe in
    ...
    # are NOT trying to work around). The Zeloo icon + product name are
    # stamped onto Zeloo.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\Zeloo-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Zeloo.exe",
        "$desktopDir\release\win-arm64-unpacked\Zeloo.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Zeloo.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Zeloo icon + identity are stamped onto Zeloo.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, Zeloo-agent#38216).
    ...
    #    Zeloo.exe. We deliberately do NOT point them at `Zeloo desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Zeloo.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Zeloo.lnk')
        )
        ...
        # with the Zeloo icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.Zeloo/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'Zeloo setup'."
        return
    }
    ...
    # Run Zeloo setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m zeloo_cli.main setup
    } else {
        python -m zeloo_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $ZELOOCmd = "$InstallDir\venv\Scripts\Zeloo.exe"
    if (-not (Test-Path $ZELOOCmd)) {
        $ZELOOCmd = "Zeloo"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$ZELOOHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'Zeloo whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: Zeloo gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$ZELOOHome\logs\gateway.log"
            Start-Process -FilePath $ZELOOCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$ZELOOHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: Zeloo gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: Zeloo gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$ZELOOHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$ZELOOHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ZELOOHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ZELOOHome\Zeloo-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   Zeloo              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   Zeloo setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   Zeloo config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   Zeloo config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   Zeloo gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   Zeloo update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Zeloo repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    # Managed Python lives under $InstallDir\.Zeloo-runtime, so the checkout
...
if ($IncludeDesktop) {
    ...
    # (Zeloo-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Zeloo to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://Zeloo-agent.nousresearch.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
