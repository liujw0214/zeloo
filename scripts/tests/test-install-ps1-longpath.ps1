...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[Zeloo] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[Zeloo`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[Zeloo`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        ZELOOHome = $(if ($paths) { $paths.zeloo_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\Zeloo\Zeloo-agent'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-Zeloo'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-Zeloo') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
