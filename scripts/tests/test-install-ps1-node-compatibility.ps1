...
# changing PATH, installing software, or touching the user's Zeloo home.

$repoRoot = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path))
$installScript = Join-Path $repoRoot 'scripts\install.ps1'
$testRoot = Join-Path $env:TEMP ("Zeloo-node-compatibility-test-" + [Guid]::NewGuid().ToString('N'))
$ZELOOHome = Join-Path $testRoot 'home'
$InstallDir = Join-Path $testRoot 'missing-checkout'
. $installScript -ZELOOHome $ZELOOHome -InstallDir $InstallDir
