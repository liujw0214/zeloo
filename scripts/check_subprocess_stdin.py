...
When Zeloo runs in TUI mode, the gateway child process communicates with
...
# ``get_zeloo_home() / "plugins"`` (user) and ``./.Zeloo/plugins/`` (project,
# gated behind ``ZELOO_ENABLE_PROJECT_PLUGINS``) — see
# ``zeloo_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``zeloo_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import zeloo_constants (this script
    ...
    # plugins in get_zeloo_home()/plugins/ and project plugins in
    # ./.Zeloo/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_zeloo_home() / "plugins"]
    if os.environ.get("ZELOO_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".Zeloo" / "plugins")
