 ...
 *   The artifact published on the website right now -- Zeloo-Setup.exe has
 ...
 * @typedef {InstallMethod | 'Zeloo-update' | 'open-app-update' | 'Zeloo-desktop-app-update'} UpdateMethod
 ...
 *   desktop-installer install; Zeloo-desktop-app-update starts the app
 *   via `Zeloo desktop`, which every install method provides on every
...
export const SPEC = {
  windows: {
    install: [
      // irm https://Zeloo.nousresearch.com/install.ps1 | iex
      { method: 'installer-script' },
      // The same one-liner with -IncludeDesktop: builds Zeloo.exe AND
      ...
      // Website Zeloo-Setup.exe, clicked through the GUI.
    ...
    update: [
      ...
      // Same button, app launched via `Zeloo desktop`.
      { method: 'Zeloo-desktop-app-update' },
    ],
  },
  macos: {
    install: [
      { method: 'installer-script' },
      { method: 'installer-script+desktop' },
      // The published Zeloo-Setup.dmg from the website, mounted and run.
      { method: 'desktop-installer', versions: ['latest'] },
    ],
    update: [
      { method: 'installer-script' },
  ...
  linux: {
    ...
    update: [
      ...
      // linux, so there is no open-app-update; `Zeloo desktop` is always
...
export function methodNeedsDesktop(m) {
  return m.startsWith('desktop-installer') || m === 'installer-script+desktop' ||
    m === 'open-app-update' || m === 'Zeloo-desktop-app-update';
}
