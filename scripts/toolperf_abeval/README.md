...
batch (tracker: [#77056](https://github.com/NousResearch/Zeloo-agent/issues/77056)).
...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  Zeloo home, same model, same tasks, same reps.
...
1. Create a dedicated Zeloo home with credentials for the models under test:
   ...
   integration at it; no Zeloo observability plugin needs to be enabled.
