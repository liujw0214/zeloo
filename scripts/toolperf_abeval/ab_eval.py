...
measured in the production session DB) through `Zeloo chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    ZELOO_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured Zeloo home with credentials for the
...
(tracker: NousResearch/Zeloo-agent#77056).
"""
import json
import os
import re
...
""".strip(),
            ...
            env.update({
                ...
                "ZELOO_HOME": str(HOME),
                "ZELOO_NEMO_RELAY_PLUGINS_TOML": str(relay_config),
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "zeloo_cli.main", "chat", "--query", q,
