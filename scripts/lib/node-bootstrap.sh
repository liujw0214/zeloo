...
#   2. ~/.Zeloo/node/ from a prior Zeloo-managed install
#   3. fnm, proto, nvm (in that order) if the user already uses a version manager
#   4. Termux `pkg`, macOS Homebrew
#   5. pinned nodejs.org tarball into ~/.Zeloo/node/ (always works, zero shell rc edits)
#
# Usage:
#   source scripts/lib/node-bootstrap.sh
#   ensure_node   # returns 0 on success, non-zero on failure
#   if [ "$ZELOO_NODE_AVAILABLE" = true ]; then ...; fi
#
# Env inputs (set before sourcing to override defaults):
#   ZELOO_NODE_MIN_VERSION   (default: 20)   — accepted on PATH
#   ZELOO_NODE_TARGET_MAJOR  (default: 22)   — installed when we install
#   ZELOO_HOME               (default: $HOME/.Zeloo)
# ============================================================================

ZELOO_NODE_MIN_VERSION="${ZELOO_NODE_MIN_VERSION:-20}"
ZELOO_NODE_TARGET_MAJOR="${ZELOO_NODE_TARGET_MAJOR:-22}"
ZELOO_HOME="${ZELOO_HOME:-$HOME/.Zeloo}"
ZELOO_NODE_AVAILABLE=false
...
# Redirect a Zeloo-managed Node's `npm install -g` to the command link dir
# (already on PATH) instead of the default $ZELOO_HOME/node/bin, which is off
...
_nb_configure_npm_prefix() {
    ...
    printf 'prefix=%s\n' "$(dirname "$_link_dir")" > "$ZELOO_HOME/node/etc/npmrc"
}
...
_nb_npm_range() {
    if [ -n "${ZELOO_NPM_TARGET_RANGE:-}" ]; then
        printf '%s\n' "$ZELOO_NPM_TARGET_RANGE"
        return 0
    fi
...
# The Python side recovers through zeloo_cli/npm_engine.py; the installer path
...
_nb_ensure_bundled_npm_range() {
    local npm_bin="$ZELOO_HOME/node/bin/npm"
    [ -x "$npm_bin" ] || return 0
    ...
    _nb_warn "Fix manually: npm install -g --prefix \"$ZELOO_HOME/node\" npm@\"$range\""
    return 1
}
...
_nb_have_modern_node() {
    ...
    [ "$(_nb_node_major)" -ge "$ZELOO_NODE_MIN_VERSION" ]
}
...
_nb_try_fnm() {
    ...
    _nb_log "fnm detected — installing Node $ZELOO_NODE_TARGET_MAJOR..."
    eval "$(fnm env 2>/dev/null)" || true
    fnm install "$ZELOO_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    fnm use     "$ZELOO_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
...
_nb_try_proto() {
    ...
    _nb_log "proto detected — installing Node $ZELOO_NODE_TARGET_MAJOR..."
    proto install node "$ZELOO_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_nvm() {
    ...
    _nb_log "nvm detected — installing Node $ZELOO_NODE_TARGET_MAJOR..."
    nvm install "$ZELOO_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    nvm use     "$ZELOO_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_brew() {
    ...
    brew install "node@${ZELOO_NODE_TARGET_MAJOR}" >/dev/null 2>&1 \
        || brew install node >/dev/null 2>&1 \
        || return 1
    brew link --overwrite --force "node@${ZELOO_NODE_TARGET_MAJOR}" >/dev/null 2>&1 || true
    _nb_have_modern_node || return 1
...
_nb_install_bundled_node() {
    ...
    local index_url="https://nodejs.org/dist/latest-v${ZELOO_NODE_TARGET_MAJOR}.x/"
    local tarball
    tarball=$(curl -fsSL "$index_url" \
        | grep -oE "node-v${ZELOO_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.xz" \
        | head -1)
    if [ -z "$tarball" ]; then
        tarball=$(curl -fsSL "$index_url" \
            | grep -oE "node-v${ZELOO_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.gz" \
            | head -1)
    fi
    if [ -z "$tarball" ]; then
        _nb_warn "Could not resolve Node $ZELOO_NODE_TARGET_MAJOR binary for $node_os-$node_arch"
        return 1
    fi
    ...
    _nb_log "Extracting to $ZELOO_HOME/node/..."
    ...
    mv "$extracted" "$ZELOO_HOME/node"
    rm -rf "$tmp"

    local _link_dir
    _link_dir="$(_nb_get_link_dir)"
    # ZELOO_NODE_SKIP_LINKS=1: the caller only wants the private managed tree
    ...
    if [ "${ZELOO_NODE_SKIP_LINKS:-0}" != "1" ]; then
        mkdir -p "$_link_dir"
        ln -sf "$ZELOO_HOME/node/bin/node" "$_link_dir/node"
        ln -sf "$ZELOO_HOME/node/bin/npm"  "$_link_dir/npm"
        ln -sf "$ZELOO_HOME/node/bin/npx"  "$_link_dir/npx"
    fi
    ...
    _nb_have_modern_node || return 1
    _nb_ok "Node $(node --version) installed to $ZELOO_HOME/node/"
...
# ---------------------------------------------------------------------------
# Heal a broken Zeloo-managed Node tree (partial upgrade / missing lib/)
# ---------------------------------------------------------------------------
...
_nb_managed_tool_broken() {
    ...
    local probe
    for probe in \
        "$ZELOO_HOME/node/bin/$tool" \
        "$ZELOO_HOME/node/${tool}.exe" \
        "$ZELOO_HOME/node/$tool"; do
...
# The managed node runs but is below ZELOO_NODE_TARGET_MAJOR — an old tree
...
_nb_managed_node_outdated() {
    ...
    for probe in "$ZELOO_HOME/node/bin/node" "$ZELOO_HOME/node/node"; do
        [ -x "$probe" ] || continue
        ver="$("$probe" --version 2>/dev/null)" || return 1
        major="${ver#v}"; major="${major%%.*}"
        case "$major" in ''|*[!0-9]*) return 1 ;; esac
        [ "$major" -lt "$ZELOO_NODE_TARGET_MAJOR" ] && return 0
        return 1
    done
    return 1
}
...
# absent. Used by zeloo_constants.find_zeloo_node_executable() and safe
# to call from install reruns.
heal_managed_node() {
    [ -d "$ZELOO_HOME/node" ] || return 1
    if ! _nb_managed_node_needs_heal; then
        return 0
    fi
    _nb_log "Zeloo-managed Node is broken — redownloading to $ZELOO_HOME/node/..."
    _nb_install_bundled_node
}
...
ensure_node() {
    ...
    if _nb_have_modern_node; then
        ...
        ZELOO_NODE_AVAILABLE=true
        return 0
    fi

    if [ -x "$ZELOO_HOME/node/bin/node" ]; then
        export PATH="$ZELOO_HOME/node/bin:$PATH"
        if _nb_have_modern_node; then
            _nb_ok "Node $(node --version) found (Zeloo-managed)"
            ZELOO_NODE_AVAILABLE=true
    ...
    _nb_try_fnm   && { ZELOO_NODE_AVAILABLE=true; return 0; }
    _nb_try_proto && { ZELOO_NODE_AVAILABLE=true; return 0; }
    _nb_try_nvm   && { ZELOO_NODE_AVAILABLE=true; return 0; }

    # Platform package managers.
    _nb_try_termux_pkg && { ZELOO_NODE_AVAILABLE=true; return 0; }
    _nb_try_brew       && { ZELOO_NODE_AVAILABLE=true; return 0; }

    # Last resort: pinned nodejs.org tarball.
    _nb_install_bundled_node && { ZELOO_NODE_AVAILABLE=true; return 0; }

    _nb_warn "Node.js install failed — TUI and browser tools will be unavailable."
    _nb_warn "Install manually: https://nodejs.org/en/download/  (or: \`brew install node\`, \`fnm install $ZELOO_NODE_TARGET_MAJOR\`, etc.)"
    return 1
}
