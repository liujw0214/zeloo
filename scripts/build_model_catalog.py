#!/usr/bin/env python3
"""Build the Zeloo Model Catalog — a centralized JSON manifest of curated models.
...
Zeloo CLI fetches at runtime. Publishing the catalog through the docs site
lets maintainers update model lists without shipping a Zeloo release.
...
# Ensure ZELOO_HOME is set for imports that touch it at module level.
os.environ.setdefault("ZELOO_HOME", os.path.join(os.path.expanduser("~"), ".zeloo"))

from zeloo_cli.models import (  # noqa: E402
    OPENROUTER_MODELS,
...
def build_catalog() -> dict:
    return {
        ...
        "metadata": {
            "source": "zeloo-agent repo",
            "docs": "https://zeloo-agent.nousresearch.com/docs/reference/model-catalog",
        },
        "providers": {
            "openrouter": {
                "metadata": {
                    ...
                    "note": (
                        ...
                        'The entry labeled "default": true is the model Zeloo '
