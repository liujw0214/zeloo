#!/usr/bin/env python3
"""Canvas LMS API CLI for Zeloo Agent.
...
def _check_config():
    ...
    if missing:
        zeloo_env = os.path.join(
            os.environ.get("ZELOO_HOME", os.path.expanduser("~/.Zeloo")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {zeloo_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Zeloo Agent"
    )
