...
### 1. Start order: editor first, Zeloo second

Zeloo probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Zeloo session.
...
restart the editor, restart the Zeloo session.
...
server fails to bind (Output Log) or Zeloo' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.Zeloo/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Zeloo

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Zeloo' connection
lives in `~/.Zeloo/config.yaml` via `Zeloo mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Zeloo — just don't
mistake it for the Zeloo setup step.
...
explicitly warns against overlapping calls. Zeloo executes same-turn tool
...
### 9. Timeouts: Zeloo gives up before Unreal does

Zeloo' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Zeloo runs elsewhere (SSH backend,
