...
## 5. Zeloo Configuration

### Config location

`$ZELOO_HOME/config.yaml` (defaults to `~/.Zeloo/config.yaml` when `ZELOO_HOME` is unset)
...
Restart the Zeloo session for changes to take effect. The MCP connection is
