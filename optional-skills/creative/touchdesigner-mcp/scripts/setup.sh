...
ZELOO_HOME_DIR="${ZELOO_HOME:-$HOME/.Zeloo}"
ZELOO_CFG="${ZELOO_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Zeloo config has twozero_td MCP entry ──
if [[ ! -f "$ZELOO_CFG" ]]; then
    echo -e " ${FAIL} Zeloo config not found at ${ZELOO_CFG}"
    manual_steps+=("Create ${ZELOO_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$ZELOO_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Zeloo config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Zeloo config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${ZELOO_CFG} manually"); }
    manual_steps+=("Restart Zeloo session to pick up config change")
fi
