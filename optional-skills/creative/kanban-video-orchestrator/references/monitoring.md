...
Zeloo kanban watch --tenant <project-slug>

# Snapshot of the board
Zeloo kanban list --tenant <project-slug>
Zeloo kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
Zeloo kanban stats --tenant <project-slug>

# Visual dashboard (browser)
Zeloo dashboard

# Inspect a specific task (includes comments + events)
Zeloo kanban show <task-id>

# Follow a single task's event stream
Zeloo kanban tail <task-id>
```

Verify available subcommands with `Zeloo kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `Zeloo kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `Zeloo kanban block <id>` then `Zeloo kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `Zeloo kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `Zeloo kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `Zeloo profile list` (profile exists?), `Zeloo status` (gateway/dispatcher up?), and verify tenant. |
...
Zeloo kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
Zeloo kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(Zeloo kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
Zeloo kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
Zeloo kanban block <task-id>
Zeloo kanban archive <task-id>

# Diagnose what happened
Zeloo kanban show <task-id>      # task body, comments, recent events
Zeloo kanban tail <task-id>      # follow the live event stream
Zeloo kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `Zeloo kanban create` for the director
...
while true; do
    clear
    Zeloo kanban list --tenant <slug>
    echo "---"
    Zeloo kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `Zeloo kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `Zeloo profile list` and start
