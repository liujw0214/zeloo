...
the actual `setup.sh` that creates the project workspace, configures Zeloo
...
2. **Create profiles** — `Zeloo profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.Zeloo/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `Zeloo kanban create` assigned to the director
...
```bash
Zeloo profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.Zeloo/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.Zeloo/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Zeloo
...
Each profile gets a `SOUL.md` at `~/.Zeloo/profiles/<name>/SOUL.md` that
...
```bash
Zeloo kanban create "Direct production of <video title>" \
    --assignee director \
...
the Zeloo `.env` (`${ZELOO_HOME:-$HOME/.Zeloo}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _zeloo_env="${ZELOO_HOME:-$HOME/.Zeloo}/.env"
    if grep -q "^${var}=" "$_zeloo_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_zeloo_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_zeloo_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY Zeloo ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY Zeloo OPENROUTER_API_KEY || exit 1
# ...
```
