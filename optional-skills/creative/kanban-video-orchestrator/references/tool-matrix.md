...
Maps each role archetype to the Zeloo skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public Zeloo-agent
...
## Zeloo skills relevant to video production

### Visual / rendering skills (`Zeloo-agent/skills/creative/`)
...
### Design / pre-production skills (`Zeloo-agent/skills/creative/`)
...
### Audio / media skills (`Zeloo-agent/skills/creative/` + `skills/media/`)
...
These are **not** Zeloo skills but external CLIs / APIs that profiles invoke.
...
## Built-in Zeloo tools for media review

These are native Zeloo tools — not invoked via terminal but through their own
...
key is present in `${ZELOO_HOME:-~/.Zeloo}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Zeloo profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${ZELOO_HOME:-~/.Zeloo}/.env` → environment variable.
...
When a new Zeloo-public video skill ships:
