...
Usage (via cron with --no-agent):
    ...
    Zeloo cron create Zeloo-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$ZELOO_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name Zeloo-issues --repo NousResearch/Zeloo-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Zeloo .env file
(``${ZELOO_HOME:-~/.Zeloo}/.env``) to avoid the 60 req/hr
