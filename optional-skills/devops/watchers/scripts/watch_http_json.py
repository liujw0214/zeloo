...
Usage (via cron with --no-agent):
    ...
    Zeloo cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$ZELOO_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Zeloo-Watcher/1.0"})
