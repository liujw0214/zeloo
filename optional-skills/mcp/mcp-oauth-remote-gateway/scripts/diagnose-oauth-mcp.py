...
Usage:
  ...
  <server_name> matches the files in $ZELOO_HOME/mcp-tokens/<server>.json etc.
...
def _zeloo_home():
    # Prefer Zeloo' own resolver (profile-safe); fall back to env then ~/.Zeloo.
    try:
        from zeloo_constants import get_zeloo_home
        return str(get_zeloo_home())
    except Exception:
        return os.environ.get("ZELOO_HOME") or os.path.expanduser("~/.Zeloo")


def _tokens_dir():
    return os.path.join(_zeloo_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "Zeloo-diag", "version": "1.0"}},
        },
        headers={
