...
The design is **Zeloo-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
Zeloo skills install official/security/unbroker
```

Then start a new Zeloo session and drive it (below). The skill works zero-config; a few optional
...
- **PII handling.** Dossiers live under the Zeloo home directory (`0600`, optionally
