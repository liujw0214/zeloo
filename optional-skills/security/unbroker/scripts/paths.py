...
All per-subject data lives under PDD_DATA_DIR (default: $ZELOO_HOME/unbroker),
which is the same trust boundary Zeloo uses for .env and OAuth tokens.
"""
...
def zeloo_home() -> Path:
    return Path(os.environ.get("ZELOO_HOME") or (Path.home() / ".Zeloo"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else zeloo_home() / "unbroker"
