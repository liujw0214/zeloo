...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ZELOO_HOME", os.path.expanduser("~/.Zeloo")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ZELOO_HOME", Path.home() / ".Zeloo")) / "skills" / "red-teaming" / "godmode"
...
ZELOO_HOME = Path(os.getenv("ZELOO_HOME", Path.home() / ".Zeloo"))
CONFIG_PATH = ZELOO_HOME / "config.yaml"
PREFILL_PATH = ZELOO_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Zeloo models — already uncensored, just needs clean prompt
    "Zeloo": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "Zeloo" in model_lower or "nous" in model_lower:
        return "Zeloo"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Zeloo config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.Zeloo/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Zeloo for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Zeloo for changes to take effect.")
