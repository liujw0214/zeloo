...
## 5. GODMODE FAST — Zeloo 4 405B (Zero Refusal)

**Model:** `nousresearch/Zeloo-4-405b`
**Strategy:** Zeloo models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Zeloo Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.Zeloo/config.yaml`:
