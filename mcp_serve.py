"""
Zeloo MCP Server — expose messaging conversations as MCP tools (`Zeloo mcp serve`).
...
Matches OpenClaw's 9-tool channel bridge surface plus the Zeloo-specific
channels_list. Client config: {"mcpServers": {"Zeloo": {"command": "Zeloo", "args": ["mcp", "serve"]}}}
"""
...
logger = logging.getLogger("Zeloo.mcp_serve")
...
def _zeloo_home() -> Path:
    try:
        ...
        return Path(os.environ.get("ZELOO_HOME", Path.home() / ".Zeloo"))


def _get_sessions_dir() -> Path:
    return _zeloo_home() / "sessions"


def _read_state_db_mtime() -> float:
    try:
        return (_zeloo_home() / "state.db").stat().st_mtime
...
def _load_channel_directory() -> dict:
    ...
    return _read_json(_zeloo_home() / "channel_directory.json")
...
class EventBridge:
    ...
    event queue with waiter support (the Zeloo analogue of OpenClaw's WebSocket
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the Zeloo MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(f"MCP server requires the 'mcp' package. Install with: {sys.executable} -m pip install 'mcp'")
    mcp = MCPServer("Zeloo", instructions=(
        "Zeloo Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Zeloo MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
