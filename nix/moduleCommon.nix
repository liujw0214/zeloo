...
#
# `services.Zeloo-agent` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  ZELOOHome, programs.Zeloo-agent (the CLI and
...
let
  ...
  deepConfigType = types.mkOptionType {
    name = "Zeloo-config-attrs";
    description = "Zeloo YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    options = {
      ...
      auth = mkOption {
        ...
        description = ''
          ...
          (remote MCP servers). Tokens are stored in $ZELOO_HOME/mcp-tokens/.
        '';
      };
  ...
  sharedOptions =
    ...
    {
      enable = lib.mkEnableOption "Zeloo Agent";
      ...
      package = mkOption {
        ...
        description = "The Zeloo-agent package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The Zeloo configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `Zeloo config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $ZELOO_HOME/.env. Zeloo reads that file at each start, with
          load_zeloo_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."Zeloo/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that Zeloo refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          Zeloo reads SOUL.md and memories/ from ZELOO_HOME, so put those
          files in `ZELOOHomeFiles`.
      ...
      ZELOOHomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into ZELOO_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          Zeloo reads SOUL.md and the memory files from ZELOO_HOME and not
          from the working directory. Declare those files here, or Zeloo
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the Zeloo plugins
          directory. Each package must contain a plugin.yaml and __init__.py
          at its root. Zeloo discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          zeloo_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as Zeloo (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in Zeloo-agent's
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `Zeloo gateway`.";
      };
      ...
      # ── The backend: `Zeloo serve` or `Zeloo dashboard` ──────────────
      # `Zeloo serve` and `Zeloo dashboard` are the same entry point,
      # zeloo_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that Zeloo Desktop
      ...
      # controls an external gateway, with `Zeloo gateway restart`. It does
      ...
      backend = {
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that Zeloo
                            Desktop connects to.
        ...
        sessionTokenFile = mkOption {
          ...
          description = ''
            ...
            ZELOO_DASHBOARD_SESSION_TOKEN. That token authorizes the /api
            routes and the /api/ws socket. Zeloo Desktop presents the same
          ...
          '';
          example = literalExpression ''config.sops.secrets."Zeloo/desktop-token".path'';
        };
      };
    };
  ...
  # removal leaves them with no `Zeloo` on the PATH and no message. The
  ...
  installPackageRemovedMessage =
    value:
    ''
      services.Zeloo-agent.installPackage was removed. Zeloo now
      separates the installation from the services, which is the
      Home Manager convention:

        programs.Zeloo-agent.enable = ${lib.boolToString (value != false)};  # the Zeloo CLI, and ZELOO_HOME for your shells
        programs.Zeloo-agent.desktop.enable = true;  # the desktop application

      `services.Zeloo-agent` keeps the state, the configuration and
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "Zeloo-config.yaml" (
  ...
  mkDocumentTree =
    { pkgs, documents }:
    pkgs.runCommand "Zeloo-documents" { } (
      ''
        mkdir -p $out
      ''
  ...
  mkEnvScript =
    { pkgs, environment }:
    let
      base = pkgs.writeText "Zeloo-env-base" (
        lib.concatStringsSep "\n" (lib.mapAttrsToList (k: v: "${k}=${v}") environment)
        + lib.optionalString (environment != { }) "\n"
      );
    in
    pkgs.writeShellScript "Zeloo-env-merge" ''
      set -eu
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "Zeloo-agent: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # ZELOO_MANAGED variable of the service. The value tells the shell
    ...
    in
    ''
      # Directories. The service units and Zeloo make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${ZELOOHome}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. Zeloo
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${ZELOOHome}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${ZELOOHome}/config.yaml
            ${run}chmod ${modes.config} ${ZELOOHome}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "Zeloo-managed" managedSystem} ${ZELOOHome}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${ZELOOHome}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${ZELOOHome}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${ZELOOHome}/auth.json"
        else
          ''
            if [ ! -e ${ZELOOHome}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${ZELOOHome}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree ZELOOHome cfg.ZELOOHomeFiles}
      ...
      ${run}find ${ZELOOHome}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "Zeloo-agent: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${ZELOOHome}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  gatewayArgv =
    cfg:
    [
      "${effectivePackage cfg}/bin/Zeloo"
      "gateway"
    ]
    ++ cfg.extraArgs;
  ...
  #
  # `exec` on the last line keeps Zeloo as the MainPID of the unit. No shell
  ...
  backendLauncher =
    ...
    pkgs.writeShellScript "Zeloo-backend-launch" (
      builtins.replaceStrings [ "@HOST@" ] [ ''"$_target"'' ] ''
        ...
        ${lib.optionalString (cfg.backend.sessionTokenFile != null) ''
          ...
          if [ ! -r "$_token_file" ]; then
            echo "Zeloo-backend: cannot read the session token file '$_token_file'. The unit stops." >&2
            echo "Zeloo-backend: backend.sessionTokenFile must name a runtime path that this user can read." >&2
            exit 1
          fi

          ZELOO_DASHBOARD_SESSION_TOKEN="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$_token_file")"
          export ZELOO_DASHBOARD_SESSION_TOKEN

          if [ -z "$ZELOO_DASHBOARD_SESSION_TOKEN" ]; then
            echo "Zeloo-backend: the session token file '$_token_file' is empty. The unit stops." >&2
            exit 1
          fi
        ''}
        ${
          if cfg.backend.waitFor == null then
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "Zeloo-backend: '$_target' did not resolve after ''${_timeout}s. The unit stops." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "Zeloo-backend: waits for '$_target' to resolve..." >&2
                fi
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "Zeloo-backend: interface '$_iface' had no IPv4 address after ''${_timeout}s. The unit stops." >&2
                  echo "Zeloo-backend: a fallback address can expose the backend more widely than you intend." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "Zeloo-backend: waits for an IPv4 address on '$_iface'..." >&2
                fi
                ${pkgs.coreutils}/bin/sleep 2
                _waited=$(( _waited + 2 ))
              done
        ...
        echo "Zeloo-backend: binds to $_target:${toString cfg.backend.port} (from $_how)" >&2
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "Zeloo Agent web dashboard and desktop backend"
    else
      "Zeloo Agent backend for Zeloo Desktop";

  # The environment that each Zeloo process needs, from either module.
  #
  # managedSystem gives the value of ZELOO_MANAGED. The CLI reads that
    ...
    [
      {
        ...
        message = ''
          ...
          To give Zeloo an identity and a memory, use
          ${optionPath}.ZELOOHomeFiles instead. Those files go to
          ZELOO_HOME. Zeloo reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of ZELOO_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"
