...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Zeloo derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            Zeloo-agent = prev.Zeloo-agent.overrideAttrs (_old: {
              ZELOO_NIX_BUILD = "1";
            });
          })
        ]
  ...
  # $ZELOO_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$ZELOO_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        Zeloo-agent = prev.Zeloo-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "Zeloo-agent-env" {
    Zeloo-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "Zeloo-agent-editable-env" {
    Zeloo-agent = dependency-groups;
  };
}
