# nix/overlays.nix — Expose pkgs.Zeloo-agent for external NixOS configs
#
...
# `pkgs.Zeloo-agent`, `nix build .#default`, and the NixOS module's
...
{ inputs, ... }:
{
  flake.overlays.default = final: _: {
    Zeloo-agent = inputs.self.packages.${final.stdenv.hostPlatform.system}.default;
  };
}
