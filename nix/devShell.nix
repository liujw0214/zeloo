...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      ZELOONpmLib = self'.packages.default.passthru.ZELOONpmLib;
      ...
      ZELOOAgentDevShellHook = self'.packages.default.passthru.devShellHook;
    in
    {
      devShells.default = pkgs.mkShell {
        packages = with pkgs; [
          (pkgs.runCommand "Zeloo" { } ''
            mkdir -p $out/bin
            install -Dm755 ${../Zeloo} $out/bin/Zeloo
          '')
        ...
        shellHook = ''
          ${ZELOOAgentDevShellHook}
          ${ZELOONpmLib.mkNpmDevShellHook npmPackageJsonPaths}
          ...
          export ZELOO_PYTHON_SRC_ROOT=$(git rev-parse --show-toplevel)
          ...
          echo "Zeloo Agent dev shell in $ZELOO_PYTHON_SRC_ROOT"
          echo "Ready. Run 'Zeloo' or 'sandbox Zeloo' to start."
        '';
      };
    };
}
