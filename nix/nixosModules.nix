# nix/nixosModules.nix — the NixOS module for Zeloo-agent
#
...
#
# Container mode: Zeloo runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $ZELOO_HOME/.env
# and read by Zeloo at startup — no container recreation needed for env changes.
#
# Tool resolution: the Zeloo wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }:
{
  flake.nixosModules.default =
    ...
    let
      ...
      # ZELOO_HOME via addToSystemPackages, so they can save settings through the
      ...
      # Zeloo user + sudo on first boot (writable layer persists), then
      # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
      containerEntrypoint = pkgs.writeShellScript "Zeloo-container-entrypoint" ''
        set -eu

        ZELOO_UID="''${ZELOO_UID:?ZELOO_UID must be set}"
        ZELOO_GID="''${ZELOO_GID:?ZELOO_GID must be set}"

        # ── Group: ensure a group with GID=$ZELOO_GID exists ──
        # Check by GID (not name) to avoid collisions with pre-existing groups
        # (e.g. GID 100 = "users" on Ubuntu)
        EXISTING_GROUP=$(getent group "$ZELOO_GID" 2>/dev/null | cut -d: -f1 || true)
        if [ -n "$EXISTING_GROUP" ]; then
          GROUP_NAME="$EXISTING_GROUP"
        else
          GROUP_NAME="Zeloo"
          if command -v groupadd >/dev/null 2>&1; then
            groupadd -g "$ZELOO_GID" "$GROUP_NAME"
          elif command -v addgroup >/dev/null 2>&1; then
            addgroup -g "$ZELOO_GID" "$GROUP_NAME" 2>/dev/null || true
          fi
        fi

        # ── User: ensure a user with UID=$ZELOO_UID exists ──
        PASSWD_ENTRY=$(getent passwd "$ZELOO_UID" 2>/dev/null || true)
        if [ -n "$PASSWD_ENTRY" ]; then
          TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
          TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
        else
          TARGET_USER="Zeloo"
          TARGET_HOME="/home/Zeloo"
          if command -v useradd >/dev/null 2>&1; then
            useradd -u "$ZELOO_UID" -g "$ZELOO_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
          elif command -v adduser >/dev/null 2>&1; then
            adduser -u "$ZELOO_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
          fi
        fi
        mkdir -p "$TARGET_HOME"
        chown "$ZELOO_UID:$ZELOO_GID" "$TARGET_HOME"
        chmod 0750 "$TARGET_HOME"

        # Ensure ZELOO_HOME is owned by the target user.
        ...
        if [ -n "''${ZELOO_HOME:-}" ] && [ -d "$ZELOO_HOME" ]; then
          find "$ZELOO_HOME" \! -user "$ZELOO_UID" -exec chown "$ZELOO_UID:$ZELOO_GID" {} +
        fi
        ...
        if [ ! -f /var/lib/Zeloo-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
          ...
          touch /var/lib/Zeloo-tools-provisioned
        fi

        if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/Zeloo ]; then
          mkdir -p /etc/sudoers.d
          echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/Zeloo
          chmod 0440 /etc/sudoers.d/Zeloo
        fi
        ...
        if command -v setpriv >/dev/null 2>&1; then
          exec setpriv --reuid="$ZELOO_UID" --regid="$ZELOO_GID" --init-groups "$@"
      ...
      # so they can update without recreation. Env vars go through $ZELOO_HOME/.env.
      ...
      containerModeFile = pkgs.writeText "Zeloo-container-mode" ''
        # Written by the NixOS activation script. Do not edit manually.
        backend=${cfg.container.backend}
        container_name=${containerName}
        exec_user=${cfg.user}
        zeloo_bin=${containerDataDir}/current-package/bin/Zeloo
      '';

      # Default: /var/lib/Zeloo/workspace → /data/workspace.
      ...
      commonServiceConfig = {
        ...
        # so interactive users in the Zeloo group can read/write them.
        UMask = "0007";
      ...
      // common.processEnvironment { inherit ZELOOHome; };
    ...
    in
    {
        ...
        // (
          with lib;
          {
            ...
            stateDir = mkOption {
              ...
              description = "State directory. Contains .Zeloo/ subdir (ZELOO_HOME).";
            };
            ...
            addToSystemPackages = mkOption {
              ...
              default = false;
              description = ''
                Add the Zeloo CLI to environment.systemPackages and export
                ZELOO_HOME system-wide (via environment.variables) so interactive
            ...
            container = {
              ...
              hostUsers = mkOption {
                ...
                default = [ ];
                description = ''
                  Interactive users who get a ~/.Zeloo symlink to the service
                  stateDir. These users are automatically added to the Zeloo group.
                '';
                example = [ "sidbin" ];
              };
            };
      ...
      config = lib.mkIf cfg.enable (
        lib.mkMerge [
          ...
          # ── Host CLI ──────────────────────────────────────────────────────
          # Add the Zeloo CLI to system PATH and export ZELOO_HOME system-wide
          # so interactive shells share state (sessions, skills, cron) with the
          # gateway service instead of creating a separate ~/.Zeloo/.
          (lib.mkIf cfg.addToSystemPackages {
            environment.systemPackages = [ effectivePackage ];
            environment.variables.ZELOO_HOME = ZELOOHome;
          })
              ...
              ++ [
                {
                  ...
                  message = "services.Zeloo-agent: backend.mode is not supported together with container.enable — the container runs the gateway only.";
                }
              ];
          }

          # ── Per-user profile for extraPackages ───────────────────────────
          # Wire extraPackages into the Zeloo user's per-user profile so the
          ...
          (lib.mkIf
            (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != [ ])
            {
              warnings = [
                ''
                  services.Zeloo-agent: container.enable is true and container.hostUsers
                  is set, but addToSystemPackages is false. Without a host-installed Zeloo
                  binary, container routing will not work for interactive users.
                  Set addToSystemPackages = true or ensure Zeloo is on PATH.
                ''
              ];
            }
          )
          ...
          # ── Directories ───────────────────────────────────────────────────
          {
            systemd.tmpfiles.rules = [
              ...
              "d ${ZELOOHome}                  2770 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.stateDir}/home           0750 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.workingDirectory}        2770 ${cfg.user} ${cfg.group} - -"
            ]
            ++ map (d: "d ${ZELOOHome}/${d} 2770 ${cfg.user} ${cfg.group} - -") common.stateSubdirs;
          }

          # ── Activation: link config + auth + documents ────────────────────
          {
            system.activationScripts."Zeloo-agent-setup" =
              lib.stringAfter
                (
                ...
                ''
                  ...
                  chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${ZELOOHome} ${cfg.stateDir}/home ${cfg.workingDirectory}
                  chmod 2770 ${cfg.stateDir} ${ZELOOHome} ${cfg.workingDirectory}
                  ...
                  find ${ZELOOHome} -maxdepth 1 \
                    \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
                    -exec chmod g+rw {} + 2>/dev/null || true
                  for _subdir in ${lib.concatStringsSep " " common.stateSubdirs}; do
                    mkdir -p "${ZELOOHome}/$_subdir"
                    chown ${cfg.user}:${cfg.group} "${ZELOOHome}/$_subdir"
                    chmod 2770 "${ZELOOHome}/$_subdir"
                    find "${ZELOOHome}/$_subdir" -type f \
                      -exec chmod g+rw {} + 2>/dev/null || true
                  done

                  ${common.mkStateScript {
                    inherit pkgs cfg ZELOOHome;
                    workingDirectory = cfg.workingDirectory;
                  ...
                  chown -h ${cfg.user}:${cfg.group} ${ZELOOHome}/plugins/nix-managed-* 2>/dev/null || true
                  ...
                  ${
                    if cfg.container.enable then
                      ''
                        install -o ${cfg.user} -g ${cfg.group} -m 0644 ${containerModeFile} ${ZELOOHome}/.container-mode
                      ''
                    else
                      ''
                        rm -f ${ZELOOHome}/.container-mode
                        ...
                        ${lib.concatStringsSep "\n" (
                          map (
                            ...
                            let
                              ...
                              symlinkPath = "${userHome}/.Zeloo";
                            in
                            ''
                              if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${ZELOOHome}" ]; then
                                rm -f "${symlinkPath}"
                                echo "Zeloo-agent: removed symlink ${symlinkPath}"
                              fi
                            ''
                  ...
                  # Create ~/.Zeloo -> stateDir/.Zeloo for each hostUser so the
          ...
          (lib.mkIf (!cfg.container.enable) {
            systemd.services.Zeloo-agent = {
              ...
              # $ZELOO_HOME/.env by the activation script. load_zeloo_dotenv()
          ...
          (lib.mkIf (!cfg.container.enable && cfg.backend.mode != "none") {
            systemd.services.Zeloo-backend = {
          ...
          (lib.mkIf cfg.container.enable {
            ...
            systemd.services.Zeloo-agent = {
              description = "Zeloo Agent Gateway (container)";
              wantedBy = [ "multi-user.target" ];
              after = [
              ...
              preStart = ''
                ...
                if [ "$NEED_CREATE" = "true" ]; then
                  ...
                  ZELOO_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
                  ZELOO_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
                  ...
                  ${containerBin} create \
                    ...
                    --env ZELOO_UID="$ZELOO_UID" \
                    --env ZELOO_GID="$ZELOO_GID" \
                    --env ZELOO_HOME=${containerDataDir}/.Zeloo \
                    --env ZELOO_MANAGED=true \
                    --env HOME=${containerHomeDir} \
                    ${lib.concatStringsSep " " cfg.container.extraOptions} \
                    ${cfg.container.image} \
                    ${containerDataDir}/current-package/bin/Zeloo gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
