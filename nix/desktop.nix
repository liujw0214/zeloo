# nix/desktop.nix — Zeloo Desktop (Electron) app build + wrapper
#
# `ZELOOAgent` is the fully-built `.#default` package — it ships the
# `Zeloo` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `ZELOO_DESKTOP_ZELOO` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Zeloo CLI").
# No reimplementation of the agent resolution in this wrapper.
{
  ...
  # ZELOO_HOME and ZELOO_MANAGED here, which gives the app the same state
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "Zeloo-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "Zeloo-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = ZELOONpmLib.buildNpmPackage {
    dirs = [
      "apps/desktop"
      "apps/shared"
    ];
    pname = "Zeloo-desktop-renderer";
    ...
    buildPhase = ''
      ...
      pushd apps/desktop
        ...
        ${lib.getExe ZELOONpmLib.node-gyp} rebuild \
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/Zeloo-desktop $out/bin
    cp -r ${renderer}/* $out/share/Zeloo-desktop/
    ...
    substituteInPlace $out/share/Zeloo-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/Zeloo-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # ZELOO_DESKTOP_ZELOO to the absolute path of the nix-built `Zeloo`
    # binary so the desktop's resolver step 4 ("existing Zeloo CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/Zeloo-desktop \
      --add-flags "$out/share/Zeloo-desktop" \
      --set ZELOO_DESKTOP_ZELOO "${lib.getExe ZELOOAgent}" \
    ...
    install -m 0644 ${../apps/desktop/assets/icon.png} \
      $out/share/icons/hicolor/1024x1024/apps/Zeloo.png
    export PYTHONPATH=$(mktemp -d)
    cp ${../zeloo_cli/linux_desktop_entry.py} "$PYTHONPATH/linux_desktop_entry.py"
    export DESKTOP_EXEC="$out/bin/Zeloo-desktop"
    export DESKTOP_ICON="$out/share/icons/hicolor/1024x1024/apps/Zeloo.png"
    python3 -c 'import os; from linux_desktop_entry import render_desktop_entry; print(render_desktop_entry(os.environ["DESKTOP_EXEC"], os.environ["DESKTOP_ICON"]))' > $out/share/applications/Zeloo.desktop
    runHook postInstall
  '';
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Zeloo Agent";
    homepage = "https://github.com/NousResearch/Zeloo-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "Zeloo-desktop";
  };
}
