# nix/packages.nix — Zeloo Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      ...
      minimal = pkgs.callPackage ./Zeloo-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.ZELOOTui;
        web = full.ZELOOWeb;
        desktop = full.ZELOODesktop;

        update-npm-lockfile = full.ZELOONpmLib.updateNpmLockfile;
      };
    };
}
