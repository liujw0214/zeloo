...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      Zeloo-agent = self'.packages.default;
      ZELOOVenv = Zeloo-agent.ZELOOVenv;
      ...
      evalHomeModule =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              home = {
                username = "Zeloo-check";
                homeDirectory = "/home/Zeloo-check";
                stateVersion = "24.11";
              };
            }
            { services.Zeloo-agent = settings; }
      ...
      evalHomeSplit =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              programs.Zeloo-agent = programs;
              services.Zeloo-agent = services;
            }
          ];
        };

      # The option names that each module defines under
      # services.Zeloo-agent. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.Zeloo-agent);
      ...
      configKeys = pkgs.runCommand "Zeloo-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${ZELOOVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "Zeloo-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "Zeloo-build-package" { } ''
          echo "PASS: package built at ${Zeloo-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "Zeloo-build-devshell" { } ''
          ...
          let
            enabled = evalHomeSplit {
              ...
              services = {
                ...
                environment.ZELOO_TEST = "1";
                environmentFiles = [ "/run/secrets/Zeloo-env" ];
                ZELOOHomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "Zeloo" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "Zeloo" n) cfg.systemd.user.services);
            ...
            activation = cfg.home.activation.ZELOOAgentSetup.data;

            failures =
              lib.optional (names != [
                "Zeloo-agent"
                "Zeloo-backend"
              ]) "expected Zeloo-agent + Zeloo-backend processes, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/Zeloo gateway" (argvOf "Zeloo-agent")
              ) "gateway process does not run `Zeloo gateway`: ${argvOf "Zeloo-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/Zeloo serve" (argvOf "Zeloo-backend")
              ) "backend process does not run `Zeloo serve`: ${argvOf "Zeloo-backend"}"
              ++ lib.optional (
                !lib.hasInfix "--no-open" (argvOf "Zeloo-backend")
              ) "backend must pass --no-open so a service never opens a browser"
              ++ lib.optional (
                lib.any (n: !lib.hasInfix "/home/Zeloo-check/.Zeloo" (envOf n)) names
              ) "gateway and backend must share one ZELOO_HOME"
              ++ lib.optional (
                cfg.home.sessionVariables.ZELOO_HOME or null != "/home/Zeloo-check/.Zeloo"
              ) "programs.Zeloo-agent.enable must export ZELOO_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "Zeloo-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/Zeloo-check/.Zeloo/SOUL.md" activation
              ) "ZELOOHomeFiles must install into ZELOO_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads ZELOO_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "ZELOO_MANAGED=home-manager" (envOf n)) names
              ) "processes must report ZELOO_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "Zeloo-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "Zeloo-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            # the priority.
            sameAsDefault = "/home/Zeloo-check";
            ...
            cases = [
              ...
              {
                name = "ZELOOHomeFiles needs no directory";
                ok = accepts { ZELOOHomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "Zeloo-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        # ── The desktop application shares one ZELOO_HOME ───────────────
        # `programs.enable` exports ZELOO_HOME with home.sessionVariables,
        ...
        # no shell profile. Thus the desktop application would open ~/.Zeloo
        # while the services use the ZELOO_HOME of the module, and the user
        ...
        home-manager-desktop =
          let
            tokenFile = "/run/secrets/Zeloo-desktop-token";
            ...
            enabled = evalHomeSplit {
              ...
              services = {
                enable = true;
                ZELOOHome = "/home/Zeloo-check/.Zeloo-work";
            ...
            desktopPackages = builtins.filter (p: (p.pname or "") == "Zeloo-desktop") cfg.home.packages;
            ...
            # second Zeloo runtime beside the services is the fault that
            # `programs.enable` plus a plain desktop package would give.
            agentPackages = builtins.filter (p: (p.pname or "") == "Zeloo-agent") cfg.home.packages;
            ...
            backendScript =
              let
                argv =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    cfg.launchd.agents.Zeloo-backend.config.ProgramArguments
                  else
                    [ cfg.systemd.user.services.Zeloo-backend.Service.ExecStart ];
            ...
            failures =
              ...
              ) "programs.desktop.enable must install exactly one Zeloo-desktop package, got ${toString (lib.length desktopPackages)}"
              ++ lib.optional (
                setValue "ZELOO_HOME" != "/home/Zeloo-check/.Zeloo-work"
              ) "the launcher must carry ZELOO_HOME: a GUI launcher reads no shell profile, so home.sessionVariables never reaches it (got: ${toString (setValue "ZELOO_HOME")})"
              ++ lib.optional (
                setValue "ZELOO_MANAGED" != "home-manager"
              ) "the launcher must report ZELOO_MANAGED=home-manager while the services own the configuration (got: ${toString (setValue "ZELOO_MANAGED")})"
              ++ lib.optional (
                lib.length agentPackages == 1
                && setValue "ZELOO_DESKTOP_ZELOO" != "${lib.head agentPackages}/bin/Zeloo"
              ) "the launcher must pin the agent package that programs.enable installs, and not a second runtime: ${toString (setValue "ZELOO_DESKTOP_ZELOO")}"

              # ── The application reaches the backend of the service ──────
              ++ lib.optional (
                setValue "ZELOO_DESKTOP_REMOTE_URL" != "http://127.0.0.1:9231"
              ) "the launcher must name the backend of the service, or the application starts a second one (got: ${toString (setValue "ZELOO_DESKTOP_REMOTE_URL")})"
              ++ lib.optional (
                !lib.hasInfix "ZELOO_DESKTOP_REMOTE_TOKEN" wrapper
              ) "the launcher must give a token with the URL: the desktop resolver throws when the URL is set alone"
              ++ lib.optional (
                !lib.hasInfix "ZELOO_DASHBOARD_SESSION_TOKEN" backendScript
              ...
              ++ lib.optional (
                setValue "ZELOO_DESKTOP_REMOTE_TOKEN" != null
              ) "the token must never be a --set value: makeWrapper writes it into the world-readable Nix store";
          in
          pkgs.runCommand "Zeloo-home-manager-desktop" { } (
            if failures != [ ] then
              throw "Home Manager desktop check failed:\n${lib.concatMapStringsSep "\n" (f: "  - ${f}") failures}"
            else
              ''
                echo "PASS: the desktop launcher shares ZELOO_HOME, the runtime and the backend of the service"
                mkdir -p $out
                echo "ok" > $out/result
              ''
          );
        ...
        home-manager-desktop-standalone =
          let
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "--set ZELOO_HOME" wrapper
              ) "the launcher must carry ZELOO_HOME even with no services"
              ++ lib.optional (
                lib.hasInfix "ZELOO_MANAGED" wrapper
              ) "the launcher must not claim a managed install when no activation writes one"
              ++ lib.optional (
                lib.hasInfix "ZELOO_DESKTOP_REMOTE_URL" wrapper
              ) "the launcher must not name a backend when the services run none"
              ++ lib.optional (
                cfg.systemd.user.services ? Zeloo-backend || cfg.launchd.agents ? Zeloo-backend
              ) "programs.enable alone must start no service";
          in
          pkgs.runCommand "Zeloo-home-manager-desktop-standalone" { } (
            if failures != [ ] then
        ...
        # command line. A silent removal thus leaves them with no `Zeloo`
        ...
        home-manager-install-package-removed =
          let
            ...
            cases = [
              {
                value = true;
                expect = "programs.Zeloo-agent.enable = true;";
              }
              {
                value = false;
                expect = "programs.Zeloo-agent.enable = false;";
              }
            ];
          ...
          in
          pkgs.runCommand "Zeloo-home-manager-install-package-removed" { } (
            if failures != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "Zeloo-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "Zeloo" n) cfg.systemd.services;
            names = lib.attrNames units;
            execOf = name: units.${name}.serviceConfig.ExecStart;
            activation = cfg.system.activationScripts."Zeloo-agent-setup".text;

            failures =
              lib.optional (names != [
                "Zeloo-agent"
                "Zeloo-backend"
              ]) "expected Zeloo-agent + Zeloo-backend units, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/Zeloo gateway" (execOf "Zeloo-agent")
              ) "gateway unit does not run `Zeloo gateway`: ${execOf "Zeloo-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/Zeloo dashboard" (execOf "Zeloo-backend")
              ) "backend unit does not run `Zeloo dashboard`: ${execOf "Zeloo-backend"}"
              ++ lib.optional (
                units.Zeloo-agent.environment.ZELOO_HOME != units.Zeloo-backend.environment.ZELOO_HOME
              ) "gateway and backend must share one ZELOO_HOME"
              ++ lib.optional (
                !lib.hasInfix "/var/lib/Zeloo/.Zeloo/SOUL.md" activation
              ) "ZELOOHomeFiles must install into ZELOO_HOME";
          ...
          in
          pkgs.runCommand "Zeloo-nixos-module" { } (
            if failures != [ ] then
        ...
        # polls and then execs Zeloo, and the assertions reject a
        # configuration that cannot work.
        backend-bind-wait =
          let
            execOf =
              settings:
              (evalNixosModule ({ enable = true; } // settings)).config.systemd.services.Zeloo-backend.serviceConfig.ExecStart;
            ...
            failures =
              ...
              lib.optional (!lib.hasInfix "bin/Zeloo serve --host 127.0.0.1" direct)
                "without waitFor the backend must exec Zeloo directly, got: ${direct}"
              ++ lib.optional (lib.hasInfix "Zeloo-backend-launch" direct)
                "without waitFor the backend must not use the launcher"

              # The hostname mode polls the resolver, then binds the name.
              ++ lib.optional (!lib.hasInfix "Zeloo-backend-launch" hostnameWait)
              ...
              ++ lib.optional (!lib.hasInfix "exec " hostnameScript)
                "the launcher must exec Zeloo, so that it keeps the MainPID"
              ...
              ++ lib.optional (!lib.hasInfix "bin/Zeloo dashboard" interfaceScript)
          ...
          in
          pkgs.runCommand "Zeloo-backend-bind-wait" { } (
            if failures != [ ] then
        ...
        # $ZELOO_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          let
            envScript = (import ./moduleCommon.nix { inherit lib; }).mkEnvScript {
              inherit pkgs;
              environment = {
                ZELOO_PUBLIC = "visible";
              };
            };
          in
          pkgs.runCommand "Zeloo-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'ZELOO_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--Zeloo-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "Zeloo-service-argv" { } ''
            set -e
        ...
        package-contents = pkgs.runCommand "Zeloo-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${Zeloo-agent}/bin/Zeloo || (echo "FAIL: Zeloo binary missing"; exit 1)
          test -x ${Zeloo-agent}/bin/Zeloo-agent || (echo "FAIL: Zeloo-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${Zeloo-agent}/bin/Zeloo --version 2>&1 | grep -qi "Zeloo" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "Zeloo-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in Zeloo Zeloo-agent Zeloo-acp; do
            test -x ${Zeloo-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "Zeloo-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking Zeloo --help ==="
          ${Zeloo-agent}/bin/Zeloo --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${Zeloo-agent}/bin/Zeloo --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "Zeloo-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${Zeloo-agent}/share/Zeloo-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${Zeloo-agent}/share/Zeloo-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "ZELOO_BUNDLED_SKILLS" ${Zeloo-agent}/bin/Zeloo || \
            (echo "FAIL: ZELOO_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: ZELOO_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${Zeloo-agent}/share/Zeloo-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${Zeloo-agent}/share/Zeloo-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "ZELOO_OPTIONAL_SKILLS" ${Zeloo-agent}/bin/Zeloo || \
            (echo "FAIL: ZELOO_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, ZELOO_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "Zeloo-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${Zeloo-agent}/share/Zeloo-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${Zeloo-agent}/share/Zeloo-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "ZELOO_BUNDLED_PLUGINS" ${Zeloo-agent}/bin/Zeloo || \
            (echo "FAIL: ZELOO_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: ZELOO_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "Zeloo-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${Zeloo-agent}/share/Zeloo-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${Zeloo-agent}/share/Zeloo-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${Zeloo-agent}/share/Zeloo-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "ZELOO_BUNDLED_LOCALES" ${Zeloo-agent}/bin/Zeloo || \
            (echo "FAIL: ZELOO_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: ZELOO_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + ZELOO_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && ZELOO_BUNDLED_LOCALES=${Zeloo-agent}/share/Zeloo-agent/locales \
            ${ZELOOVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with ZELOO_BUNDLED_LOCALES set"; exit 1)
        ...
        # ZELOO_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "Zeloo-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${Zeloo-agent}/share/Zeloo-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${Zeloo-agent}/share/Zeloo-agent/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "ZELOO_OPTIONAL_MCPS" ${Zeloo-agent}/bin/Zeloo || \
            (echo "FAIL: ZELOO_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: ZELOO_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${Zeloo-agent}/bin/Zeloo mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: Zeloo mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "Zeloo-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${Zeloo-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${Zeloo-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "ZELOO_TUI_DIR" ${Zeloo-agent}/bin/Zeloo || \
            (echo "FAIL: ZELOO_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: ZELOO_TUI_DIR set in wrapper"
        ...
        # Verify ZELOO_NODE is set in wrapper and points to Node 26+
        # (Zeloo pins its toolchain to Node 26 everywhere)
        Zeloo-node = pkgs.runCommand "Zeloo-node-version" { } ''
          set -e
          echo "=== Checking ZELOO_NODE in wrapper ==="
          grep -q "ZELOO_NODE" ${Zeloo-agent}/bin/Zeloo || \
            (echo "FAIL: ZELOO_NODE not set in wrapper"; exit 1)
          echo "PASS: ZELOO_NODE present in wrapper"

          ZELOO_NODE=$(sed -n "s/^export ZELOO_NODE='\(.*\)'/\1/p" ${Zeloo-agent}/bin/Zeloo)
          test -x "$ZELOO_NODE" || (echo "FAIL: ZELOO_NODE=$ZELOO_NODE not executable"; exit 1)
          echo "PASS: ZELOO_NODE executable at $ZELOO_NODE"

          NODE_MAJOR=$("$ZELOO_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, Zeloo requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All ZELOO_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify ZELOO_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "Zeloo-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(ZELOO_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking ZELOO_MANAGED guards ==="
          check_blocked "config set" ${Zeloo-agent}/bin/Zeloo config set model foo
          check_blocked "config edit" ${Zeloo-agent}/bin/Zeloo config edit
        ...
        extra-python-packages = let
          ...
          ZELOOWithExtra = Zeloo-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "Zeloo-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${ZELOOWithExtra}/bin/Zeloo || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${ZELOOWithExtra}/bin/Zeloo || \
          ...
          if grep -q "PYTHONPATH" ${Zeloo-agent}/bin/Zeloo; then
        ...
        extra-dependency-groups = let
          ZELOOWithGroups = Zeloo-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "Zeloo-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${ZELOOWithGroups}"
          echo "venv: ${ZELOOWithGroups.ZELOOVenv}"
        ...
        messaging-variant = pkgs.runCommand "Zeloo-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.ZELOOVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "Zeloo-config-roundtrip" {
          ...
          merge_and_load() {
            local zeloo_home="$1"
            export ZELOO_HOME="$zeloo_home"
            ${configMergeScript} ${nixSettings} "$zeloo_home/config.yaml"
            ${ZELOOVenv}/bin/python3 -c '
import json, sys
