 ...
 * dev:mock dev flow.
 *
 * Implements just enough of the /v1/* surface for `Zeloo serve` to resolve a
...
/**
 * Drives a real code edit followed by two finish attempts. Zeloo should add
...
export function createBackgroundReleaseHandle(): BackgroundReleaseHandle {
  const path = nodePath.join(
    os.tmpdir(),
    `Zeloo-e2e-bg-release-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
  )
...
/** Create an isolated ZELOO_HOME + Electron user-data dir in the OS temp dir. */
function createDevSandbox(): DevSandbox {
  const root = fs.mkdtempSync(nodePath.join(os.tmpdir(), `Zeloo-dev-mock-${Date.now()}`))
  const ZELOOHome = nodePath.join(root, 'Zeloo-home')
...
function writeMockConfig(ZELOOHome: string, mockUrl: string): void {
  fs.writeFileSync(
    nodePath.join(ZELOOHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(nodePath.join(ZELOOHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function runDevLaunch(): Promise<void> {
  ...
  writeMockConfig(sandbox.ZELOOHome, mock.url)
  console.log(`  ZELOO_HOME: ${sandbox.ZELOOHome}`)
  ...
  const env: Record<string, string> = {
    ...
    ZELOO_DESKTOP_ZELOO_ROOT: repoRoot,
    ZELOO_DESKTOP_APP_NAME: `ZELOODevMock-${Date.now()}`,
  }
