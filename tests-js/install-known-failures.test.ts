...
const lockedLog = [
  'error: failed to remove file `C:/install/venv/Lib/site-packages/../../Scripts/Zeloo.exe`: Access is denied. (os error 5)',
  'File "C:/install/venv/Scripts/Zeloo.exe/__main__.py", line 10, in <module>',
...
const base = {
  ...
  installMethod: 'installer-script', updateMethod: 'Zeloo-update',
  error: 'E2E ASSERTION FAILED: Zeloo update exited 1 (expected 0)', logs: { update: lockedLog },
}
...
describe('known install failures', () => {
  it('recognizes the released launcher self-lock, not generic access denied', () => {
    ...
    expect(matchKnownFailure({ ...base, logs: { update: lockedLog.replaceAll('Zeloo.exe', 'other.exe') } })).toBeNull()
  ...
  it('matches manual-only app updates only for the three proven July script cases', () => {
    const sample = {
      ...
      updateMethod: 'Zeloo-desktop-app-update',
      error: 'E2E ASSERTION FAILED: app driven via captured Zeloo desktop spec; update completed',
      logs: { desktop: '[Zeloo] [updates] no staged updater; surfacing manual `Zeloo update` for CLI install at C:/install\n[Zeloo] [updates] manual: Zeloo update\n' },
    }
    ...
    expect(matchKnownFailure({ ...sample, logs: { desktop: '[updates] manual: Zeloo update' } })).toBeNull()
  })
...
it('renders known receipts as footnotes, without suppressing a red job', async () => {
  ...
  const name = 'windows: installer-script -> Zeloo-update (v2026.3.12 -> HEAD)'
