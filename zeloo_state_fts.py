...
from zeloo_state_common import FTS_CJK_STALE_KEY, FTS_STALE_KEY, _FTS_CJK_TRIGGERS, _FTS_TRIGGERS
from zeloo_state_errors import is_fts_scoped_corruption_error

# caplog tests pin the "zeloo_state" logger name.
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("ZELOO_FTS5_CJK_SO")
    return Path(env).expanduser() if env else get_zeloo_home() / "lib" / "libfts5_cjk.so"


def _cjk_fts_config_enabled() -> bool:
    """config.yaml ``sessions.cjk_fts`` (default on), via its env bridge."""
    return os.getenv("ZELOO_CJK_FTS", "1").strip().lower() not in ("0", "false", "off", "no")
...
class SessionFtsSetupMixin:
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `Zeloo update` to rebuild the venv with a "
                ...
                if cjk_present:
                    ...
                    if live:
                        ...
                        logger.warning(
                            ...
                            "run `Zeloo sessions optimize-storage` on a host "
    ...
    def _is_fts_write_corruption_error(exc: sqlite3.DatabaseError) -> bool:
        ...
        gateway transcript retry: see :func:`zeloo_state_errors.is_fts_scoped_corruption_error`."""
