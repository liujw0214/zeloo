/**
 * Zeloo Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.zeloo/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__ZELOO_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const COLUMN_DOT = {
    triage: "zeloo-kanban-dot-triage",
    todo: "zeloo-kanban-dot-todo",
    ready: "zeloo-kanban-dot-ready",
    running: "zeloo-kanban-dot-running",
    blocked: "zeloo-kanban-dot-blocked",
    review: "zeloo-kanban-dot-review",
    done: "zeloo-kanban-dot-done",
    archived: "zeloo-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-zeloo-task";
  ...
  const DOCS_URL = "https://zeloo-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://zeloo-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "zeloo.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="zeloo-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])zeloo-kanban-md-code\1/i.test(attrs)) {
      return ' class="zeloo-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "zeloo-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "zeloo-kanban-md",
  ...
  // standard `drop` event and our `zeloo-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("zeloo-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("zeloo-kanban-column--drop");
          if (target) target.classList.add("zeloo-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("zeloo-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("zeloo-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("zeloo-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__ZELOO_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "zeloo-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "zeloo-kanban-attention",
        "zeloo-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "zeloo-kanban-attention-bar" },
        h("span", { className: "zeloo-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "zeloo-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "zeloo-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "zeloo-kanban-attention-row",
                  "zeloo-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "zeloo-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "zeloo-kanban-attention-row-id" }, task.id),
                h("span", { className: "zeloo-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "zeloo-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "zeloo-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "zeloo-kanban-diag-action-btn",
      action.suggested ? "zeloo-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " zeloo-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".zeloo-kanban-drawer-comment-row input, .zeloo-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "zeloo-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("zeloo-kanban-diag", sevClass) },
      h("div", { className: "zeloo-kanban-diag-header" },
        h("span", { className: "zeloo-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "zeloo-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "zeloo-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "zeloo-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "zeloo-kanban-diag-data-row" },
                  h("span", { className: "zeloo-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "zeloo-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "zeloo-kanban-diag-data-row" },
                h("span", { className: "zeloo-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "zeloo-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "zeloo-kanban-diag-reassign-row" },
            h("span", { className: "zeloo-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "zeloo-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "zeloo-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "zeloo-kanban-diag-msg",
              msg.ok ? "zeloo-kanban-diag-msg--ok" : "zeloo-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "zeloo-kanban-section" },
      h("div", { className: "zeloo-kanban-section-head-row" },
        h("span", { className: "zeloo-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "zeloo-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "zeloo-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "zeloo-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "zeloo-kanban-docs-link",
      title: "Open Zeloo Kanban docs in a new tab",
      "aria-label": "Zeloo Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `zeloo kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "zeloo-kanban-msg-ok" : "zeloo-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "zeloo-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "zeloo-kanban-boardswitcher" },
      h("div", { className: "zeloo-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "zeloo-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "zeloo-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "zeloo-kanban-dialog-title" },
        ...
        h("div", { className: "zeloo-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Zeloo profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "zeloo-kanban-bulk" },
      h("span", { className: "zeloo-kanban-bulk-count" },
      ...
      h("div", { className: "zeloo-kanban-bulk-priority",
      ...
      h("div", { className: "zeloo-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Zeloo profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "zeloo-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("zeloo-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("zeloo-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "zeloo-kanban-trash",
        dragOver ? "zeloo-kanban-trash--drop" : "",
        props.draggingTaskId ? "zeloo-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "zeloo-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "zeloo-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".zeloo-kanban-card")) return true;
      if (target.closest && target.closest(".zeloo-kanban-column-add")) return true;
      if (target.closest && target.closest(".zeloo-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("zeloo-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("zeloo-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".zeloo-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "zeloo-kanban-columns",
        isScrollable ? "zeloo-kanban-columns--scrollable" : "",
        isPanning ? "zeloo-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("zeloo-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("zeloo-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "zeloo-kanban-column",
        dragOver ? "zeloo-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "zeloo-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "zeloo-kanban-col-check",
        ...
        h("span", { className: cn("zeloo-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "zeloo-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "zeloo-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "zeloo-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "zeloo-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "zeloo-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "zeloo-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "zeloo-kanban-lane" },
                  h("div", { className: "zeloo-kanban-lane-head" },
                    h("span", { className: "zeloo-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "zeloo-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "zeloo-kanban-card--stale-red";
    if (age >= tier.amber) return "zeloo-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".zeloo-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "zeloo-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "zeloo-kanban-card",
        props.selected ? "zeloo-kanban-card--selected" : "",
        props.failed ? "zeloo-kanban-card--failed" : "",
        props.draggingSource ? "zeloo-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "zeloo-kanban-card-content" },
          h("div", { className: "zeloo-kanban-card-row" },
            h("label", {
              className: "zeloo-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "zeloo-kanban-card-check",
            ...
            h("span", { className: "zeloo-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or zeloo kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "zeloo-kanban-warning-badge",
                    "zeloo-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "zeloo-kanban-needs-assignee",
          ...
          h("div", { className: "zeloo-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "zeloo-kanban-card-row zeloo-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "zeloo-kanban-assignee",
                            title: `Assigned to Zeloo profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "zeloo-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "zeloo-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "zeloo-kanban-ago",
  ...
  // centered modal reusing the zeloo-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "zeloo-kanban-dialog zeloo-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "Zeloo profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "Zeloo profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "zeloo-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "zeloo-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "zeloo-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "zeloo-kanban-drawer-close",
        ...
        data ? h("div", { className: "zeloo-kanban-drawer-comment-foot" },
          h("div", {
            className: "zeloo-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "zeloo-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "zeloo-kanban-section" },
      h("div", { className: "zeloo-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "zeloo-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "zeloo-kanban-drawer-body" },
      h("div", { className: "zeloo-kanban-drawer-title" },
        h("span", { className: cn("zeloo-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "zeloo-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "zeloo-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "zeloo-kanban-section" },
            h("div", { className: "zeloo-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "zeloo-kanban-section" },
            h("div", { className: "zeloo-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "zeloo-kanban-done-no-result zeloo-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "zeloo-kanban-section" },
            h("div", { className: "zeloo-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "zeloo-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "zeloo-kanban-section" },
        h("div", { className: "zeloo-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "zeloo-kanban-comment" },
            h("div", { className: "zeloo-kanban-comment-head" },
              h("span", { className: "zeloo-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "zeloo-kanban-diag-action-btn",
      ...
      h("div", { className: "zeloo-kanban-section" },
        h("div", { className: "zeloo-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "zeloo-kanban-comment" },
                h("div", { className: "zeloo-kanban-comment-head" },
                  h("span", { className: "zeloo-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "zeloo-kanban-comment-ago" },
      ...
      h("div", { className: "zeloo-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "zeloo-kanban-event",
              isDiag ? "zeloo-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "zeloo-kanban-event-header" },
                  h("span", { className: "zeloo-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "zeloo-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "zeloo-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "zeloo-kanban-event-header-plain" },
                  h("span", { className: "zeloo-kanban-event-kind" }, e.kind),
                  h("span", { className: "zeloo-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "zeloo-kanban-event-phantom-row" },
                  h("span", { className: "zeloo-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "zeloo-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "zeloo-kanban-event-payload" },
    ...
    return h("div", { className: "zeloo-kanban-section" },
      h("div", { className: "zeloo-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "zeloo-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `zeloo-kanban-run--${r.outcome || r.status || "ended"}`
          : "zeloo-kanban-run--active";
        return h("div", { key: r.id, className: cn("zeloo-kanban-run", outcomeClass) },
          h("div", { className: "zeloo-kanban-run-head" },
            h("span", { className: "zeloo-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "zeloo-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "zeloo-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "zeloo-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "zeloo-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "zeloo-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "zeloo-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "zeloo-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "zeloo-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "zeloo-kanban-pre zeloo-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "zeloo-kanban-meta-row" },
      h("span", { className: "zeloo-kanban-meta-label" }, props.label),
      h("span", { className: "zeloo-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "zeloo-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "zeloo-kanban-meta-row" },
        h("span", { className: "zeloo-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "zeloo-kanban-meta-value zeloo-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "zeloo-kanban-meta-row" },
        h("span", { className: "zeloo-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "zeloo-kanban-meta-value zeloo-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "zeloo-kanban-meta-row" },
        h("span", { className: "zeloo-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "zeloo-kanban-meta-value zeloo-kanban-editable",
    ...
    return h("div", { className: "zeloo-kanban-meta-row" },
      h("span", { className: "zeloo-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "zeloo-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "zeloo-kanban-section" },
      h("div", { className: "zeloo-kanban-section-head-row" },
        h("span", { className: "zeloo-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "zeloo-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "zeloo-kanban-section" },
      h("div", { className: "zeloo-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "zeloo-kanban-deps-row" },
        h("span", { className: "zeloo-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "zeloo-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "zeloo-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "zeloo-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "zeloo-kanban-dep-chip-x",
      ...
      h("div", { className: "zeloo-kanban-deps-row" },
        h("span", { className: "zeloo-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "zeloo-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "zeloo-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "zeloo-kanban-section" },
      ...
      h("div", { className: "zeloo-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "zeloo-kanban-home-sub zeloo-kanban-home-sub--on"
              : "zeloo-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__ZELOO_PLUGINS__ && typeof window.__ZELOO_PLUGINS__.register === "function") {
    window.__ZELOO_PLUGINS__.register("kanban", KanbanPage);
  }
})();
