...
Auto-tracks and cleans up ephemeral files created during Zeloo Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$ZELOO_HOME` and `/tmp/Zeloo-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ZELOO_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `ZELOO_HOME` or `/tmp/Zeloo-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ZELOO_HOME/disk-cleanup/` is itself excluded
- `$ZELOO_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
