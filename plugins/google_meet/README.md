...
Let the Zeloo agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where Zeloo runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `Zeloo meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `Zeloo meet` CLI |
...
| `cli.py` | `Zeloo meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$ZELOO_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `Zeloo meet node {run,list,approve,remove,status,ping}` |
...
Zeloo meet install                                      # pip + Chromium
Zeloo meet setup                                        # preflight
Zeloo meet auth                                         # optional
Zeloo meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
Zeloo meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.Zeloo/.env
Zeloo meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
Zeloo meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
Zeloo meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.Zeloo/.env
Zeloo meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, Zeloo will **not** switch your system audio input automatically — the
...
Zeloo plugins enable google_meet
Zeloo meet node run --display-name my-mac --host 0.0.0.0 --port 18789
