# A2A — Agent-to-Agent protocol for Zeloo
...
peer (another Zeloo, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
Zeloo gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Zeloo serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.Zeloo/a2a_audit.jsonl`.
- Conversations persist to `~/.Zeloo/a2a_conversations/` — they survive context
