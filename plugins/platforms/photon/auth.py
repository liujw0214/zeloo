...
``(id, projectSecret)``. Storage: runtime SDK creds -> ``~/.Zeloo/.env``; management
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a Zeloo dependency
...
# published CLI client until Zeloo gets its own client_id.
...
DEFAULT_PROJECT_NAME = "Zeloo Agent"
DEFAULT_POLL_INTERVAL = 5  # RFC 8628 polling defaults; Photon's `interval` / `expires_in` win
DEFAULT_POLL_TIMEOUT = 1800
E164_RE = re.compile(r"^\+[1-9]\d{6,14}$")


# -- auth.json helpers (shares the file with the rest of Zeloo-agent) ------------

def _auth_json_path() -> Path:
    """``~/.Zeloo/auth.json`` honouring the active Zeloo profile."""
    try:
        from zeloo_constants import get_zeloo_home
        return Path(get_zeloo_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.Zeloo")) / "auth.json"
...
def _store_pool_record(key: str, record: Dict[str, Any]) -> None:
    ...
    from zeloo_cli.auth import _auth_store_lock
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.Zeloo/.env`` (secret never bound to a printable local
    in a caller — CodeQL clean flow)."""
    try:
        from zeloo_cli.config import save_env_value
    except ImportError:
        logger.warning("photon: zeloo_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    rows = [
        ...
        "──────────────────────",
        "  device token        : " + (
            "✓ stored" if load_photon_token() else "✗ missing (run `Zeloo photon setup`)"),
        "  project id          : " + (sid if sid else "✗ missing"),
        "  project secret      : " + ("✓ stored" if sec else "✗ missing"),
        "  my number           : " + (phone if phone else "✗ missing (run `Zeloo photon setup --phone ...`)"),
        "  assigned number     : " + (assigned if assigned else "✗ missing (run `Zeloo photon setup`)")]
    emit("\n".join(rows))
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_token() -> str:
        return (
            ...
            else "✗ missing (run `Zeloo photon setup`)"
        )
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `Zeloo photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `Zeloo photon setup`)"
