...
Small Node helper that bridges Zeloo Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Zeloo is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Zeloo-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Zeloo can receive inbound messages
...
The Zeloo plugin's `Zeloo photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Zeloo is Python, so replies go through this sidecar
