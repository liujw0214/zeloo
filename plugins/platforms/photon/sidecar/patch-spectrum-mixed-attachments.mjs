...
// `message.content.text` before Zeloo can see it. We rewrite the two inbound
...
const MARKER = "Zeloo patch: Preserve mixed text + attachment iMessage payloads";
...
export function patchSpectrumTs(root = scriptDir()) {
  ...
  for (const file of files) {
    ...
    // Zeloo to patch; keep the legacy v8 path below for older pinned installs.
    if (
