// Zeloo Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Zeloo): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Zeloo -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Zeloo-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `Zeloo photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Zeloo can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "Zeloo-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Zeloo'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Zeloo allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Zeloo, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-Zeloo-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Zeloo supplied them so a known-good
