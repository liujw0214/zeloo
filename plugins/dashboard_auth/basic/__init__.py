...
``ZELOO_DASHBOARD_BASIC_AUTH_*`` env vars (env wins when non-empty; see ``_settings``).
"""
...
from zeloo_cli.dashboard_auth import DashboardAuthProvider, InvalidCredentialsError, RefreshExpiredError, Session
...
def _resolve_secret(cfg_section: dict) -> bytes:
    ...
    raw = resolve_env_or_cfg("ZELOO_DASHBOARD_BASIC_AUTH_SECRET", cfg_section.get("secret"))
    if not raw:
        logger.info(
            ...
            "ZELOO_DASHBOARD_BASIC_AUTH_SECRET) for stable sessions.")
...
def _settings() -> dict:
    ...
    username = setting("ZELOO_DASHBOARD_BASIC_AUTH_USERNAME", "username")
    ...
    plaintext_from_env = os.environ.get("ZELOO_DASHBOARD_BASIC_AUTH_PASSWORD", "").strip()
