...
secret (``ZELOO_DASHBOARD_DRAIN_SECRET``, env-only — it is a credential); an inbound bearer
...
from zeloo_cli.dashboard_auth import DashboardAuthProvider, Session, TokenPrincipal
...
def _settings() -> dict:
    ...
    secret = os.environ.get("ZELOO_DASHBOARD_DRAIN_SECRET", "").strip()
    if not secret:
        raise SkipRegistration(
            "ZELOO_DASHBOARD_DRAIN_SECRET is not set. Set a per-agent >=256-bit secret "
    ...
    if reason is not None:
        raise SkipRegistration(
            f"ZELOO_DASHBOARD_DRAIN_SECRET rejected — {reason}. The drain endpoint stays disabled (fail-closed).",
            level="warning")
...
def register(ctx) -> None:
    ...
    reason) when ``ZELOO_DASHBOARD_DRAIN_SECRET`` is unset or fails the entropy gate. On
    ...
    try:
        from zeloo_cli.dashboard_auth.token_auth import register_token_route
...
_PLUGIN_COMPAT_LAZY = {
    'LoginStart': ('zeloo_cli.dashboard_auth', 'LoginStart'),
}
