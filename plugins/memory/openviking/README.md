...
- OpenViking server running and reachable from Zeloo

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
Zeloo can identify older servers that expose the legacy status-only health
...
connection values into Zeloo, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.Zeloo/.env`; for a named profile use
...
OpenViking's server config is separate from Zeloo:
...
Zeloo-side provider config is read from environment variables in the active
profile's `.env`:
...
When `OPENVIKING_API_KEY` is set, Zeloo lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Zeloo sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
Zeloo also sends `User-Agent: openviking-memory-Zeloo/<version>` on
OpenViking requests. This standard harness identifier contains the Zeloo
...
does not ask for a peer ID. Without a configured peer, Zeloo sends neither
...
order remains environment, linked OpenViking config, then Zeloo YAML. To use
...
on the old implicit `Zeloo` peer now use user memory for new writes. Without
...
Set `agent: Zeloo` to restore peer-scoped writes. Memories written at user
...
`viking_remember` creates a one-shot `Zeloo-remember-<random>` OpenViking
...
not advertised or used. The one-shot session is separate from the live Zeloo
...
Use the same OpenViking profile and credentials as Zeloo for manual recovery.
...
Zeloo built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Zeloo action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under user memory, or the configured peer memory directory |

Built-in `replace` and `remove` operations are not mirrored because Zeloo
...
file URIs, such as
`viking://user/default/peers/Zeloo/memories/preferences/mem_abc123.md` (any
