...
- Hosted: API key from [app.supermemory.ai/integrations?connect=Zeloo](http://app.supermemory.ai/integrations?connect=Zeloo)
...
Before running `Zeloo memory setup`, add the local endpoint to
...
Then run `Zeloo memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$ZELOO_HOME/supermemory.json`
...
| `container_tag` | `Zeloo` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `Zeloo-{identity}` → `Zeloo-coder`). |
...
`https://api.supermemory.ai`. Zeloo resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: Zeloo`, and document writes stamp
`metadata.sm_source: Zeloo`. This is a **functional routing key, not telemetry**:
it groups Zeloo-written memories into a dedicated "Zeloo" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Zeloo profile:

```json
{
  "container_tag": "Zeloo-{identity}"
}
```

For a profile named `coder`, this resolves to `Zeloo-coder`. The default profile resolves to `Zeloo-default`. Without `{identity}`, all profiles share the same container.
