...
```bash
Zeloo memory setup    # select "mem0"
```
...
Behavioral settings live in `$ZELOO_HOME/mem0.json` (set them via `Zeloo memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.Zeloo/.env`.
...
| `user_id` | `Zeloo-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Zeloo session and call `mem0_search` — it connects to your server.
...
```bash
Zeloo memory setup mem0 --mode oss \
...
```bash
Zeloo memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$ZELOO_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.Zeloo/mem0_qdrant"}}
  }
}
```
...
```bash
Zeloo memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$ZELOO_HOME/mem0.json`).
