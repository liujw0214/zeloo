...
catalog, filter to the ``Zeloo`` integration, apply a manifest via
...
def fetch_zeloo_templates(url: str | None = None) -> list[dict]:
    """Return catalog entries tagged for the ``Zeloo`` integration."""
    catalog = _get_json(url or catalog_url())
    entries = catalog.get("templates", []) if isinstance(catalog, dict) else []
    return [e for e in entries if "Zeloo" in (e.get("integrations") or [])]
