"""Per-plugin persistent storage: ``<Zeloo home>/plugin-data/<name>/``.

Plugins must NOT park state in ``<Zeloo home>/plugins/<name>/`` (the install dir, deleted by
...
# Mirrors the plugin-name shape `Zeloo plugins install` accepts (no separators/traversal).
...
def plugin_data_dir(name: str) -> Path:
    """Return (and create) ``<Zeloo home>/plugin-data/<name>/``; resolves ``get_zeloo_home()`` on
