...
_ZELOO_WEBHOOK_SAFE_TOOLS = ["web_search", "web_extract", "vision_analyze", "clarify"]
...
def _bundle(description, extras=()):
    """A `Zeloo-*` platform bundle: the shared core tools plus optional platform extras."""
    return _ts(description, _ZELOO_CORE_TOOLS + list(extras))


def _core_without(*excluded, kanban=True):
    """_ZELOO_CORE_TOOLS minus *excluded* (and, unless kanban=True, every kanban_* tool); order preserved."""
    return [t for t in _ZELOO_CORE_TOOLS if t not in excluded and (kanban or not t.startswith("kanban_"))]
...
TOOLSETS = {
    ...
    "x_search": _ts(
        ...
        "(SuperGrok OAuth or XAI_API_KEY). Off by default; enable in `Zeloo tools` → "
        "X (Twitter) Search.",
        ["x_search"],
    ),
    ...
    "video_gen": _ts(
        ...
        "separate tools. Configure via ``Zeloo tools`` → Video Generation.",
    ...
    "browser": _ts(
        ...
        "iframes, hold-click)",
        [t for t in _ZELOO_CORE_TOOLS if t.startswith("browser_")],
    ),
    "cronjob": _ts(
    ...
    "kanban": _ts(
        ...
        "the kanban dispatcher (ZELOO_KANBAN_TASK env set). The dispatcher runs "
        ...
        [t for t in _ZELOO_CORE_TOOLS if t.startswith("kanban_")],
    ),
    ...
    # config (see the non-configurable-toolset recovery loop in zeloo_cli/tools_config.py).
    "coding": _ts(
    ...
    # Full Zeloo toolsets (CLI + messaging platforms). All share the core tools;
    # there is deliberately no agent-callable send_message tool. Zeloo-acp is the
    ...
    "Zeloo-cli": _bundle("Full interactive CLI toolset - all default tools plus cronjob management"),

    # Mirrors Zeloo-cli; `Zeloo tools` platform config filters it down and
    # _get_platform_tools() drops _DEFAULT_OFF_TOOLSETS unless user-enabled.
    "Zeloo-cron": _bundle("Default cron toolset - same core tools as Zeloo-cli; gated by `Zeloo tools`"),
    "Zeloo-telegram": _bundle("Telegram bot toolset - full access for personal use (terminal has safety checks)"),
    "Zeloo-discord": _bundle(
        "Discord bot toolset - full access (terminal has safety checks via dangerous "
        "command approval)",
        ["discord", "discord_admin"],
    ),
    "Zeloo-whatsapp": _bundle("WhatsApp bot toolset - similar to Telegram (personal messaging, more trusted)"),
    "Zeloo-slack": _bundle("Slack bot toolset - full access for workspace use (terminal has safety checks)"),
    "Zeloo-signal": _bundle("Signal bot toolset - encrypted messaging platform (full access)"),
    "Zeloo-bluebubbles": _bundle("BlueBubbles iMessage bot toolset - Apple iMessage via local BlueBubbles server"),
    "Zeloo-homeassistant": _bundle("Home Assistant bot toolset - smart home event monitoring and control"),
    "Zeloo-email": _bundle("Email bot toolset - interact with Zeloo via email (IMAP/SMTP)"),
    "Zeloo-mattermost": _bundle("Mattermost bot toolset - self-hosted team messaging (full access)"),
    "Zeloo-matrix": _bundle("Matrix bot toolset - decentralized encrypted messaging (full access)"),
    "Zeloo-dingtalk": _bundle("DingTalk bot toolset - enterprise messaging platform (full access)"),
    "Zeloo-feishu": _bundle("Feishu/Lark bot toolset - enterprise messaging via Feishu/Lark (full access)", _FEISHU_TOOLS),
    "Zeloo-weixin": _bundle("Weixin bot toolset - personal WeChat messaging via iLink (full access)"),
    "Zeloo-qqbot": _bundle("QQBot toolset - QQ messaging via Official Bot API v2 (full access)"),
    "Zeloo-wecom": _bundle("WeCom bot toolset - enterprise WeChat messaging (full access)"),
    "Zeloo-wecom-callback": _bundle("WeCom callback toolset - enterprise self-built app messaging (full access)"),
    "Zeloo-yuanbao": {
        "description": "Yuanbao Bot 元宝消息平台工具集 - 群信息、成员查询、私聊、贴纸表情",
        "tools": _ZELOO_CORE_TOOLS + _YUANBAO_TOOLS,
        "module": "tools.yuanbao_tools",
        "includes": [],
    },
    "Zeloo-sms": _bundle("SMS bot toolset - interact with Zeloo via SMS (Twilio)"),
    "Zeloo-webhook": _ts("Webhook toolset - receive and process external webhook events", _ZELOO_WEBHOOK_SAFE_TOOLS),
    "Zeloo-gateway": _ts(
        "Gateway toolset - union of all messaging platform tools",
        [],
        includes=[
            "Zeloo-telegram", "Zeloo-discord", "Zeloo-whatsapp", "Zeloo-slack",
            "Zeloo-signal", "Zeloo-bluebubbles", "Zeloo-homeassistant", "Zeloo-email",
            "Zeloo-sms", "Zeloo-mattermost", "Zeloo-matrix", "Zeloo-dingtalk",
            "Zeloo-feishu", "Zeloo-wecom", "Zeloo-wecom-callback", "Zeloo-weixin",
            "Zeloo-qqbot", "Zeloo-webhook", "Zeloo-yuanbao",
        ],
    ),
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """A bundle's tools minus _ZELOO_CORE_TOOLS (one level of includes).

    Disabling a `core + extras` bundle must not strip the core tools every other
    toolset shares. One `includes` pass suffices (only Zeloo-gateway nests
    bundles). Unknown names: full resolution minus core.
    """
    core = set(_ZELOO_CORE_TOOLS)
...
def _plugin_platform_bundle(name: str) -> List[str]:
    """Implicit `Zeloo-<platform>` bundle for a registered plugin platform: core
    tools plus whatever the plugin registered under the platform name. [] otherwise."""
    if not name.startswith("Zeloo-"):
        return []
    platform_name = name[len("Zeloo-"):]
    try:
