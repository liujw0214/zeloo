import type { InputEvent, Key } from '@Zeloo/ink'
import * as Ink from '@Zeloo/ink'
 ...
 * but that accepts `key.meta` on macOS — and Zeloo-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.ZELOO_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which Zeloo-ink decodes as meta+'d'; without this branch it
