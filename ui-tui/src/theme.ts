import type { SkinBranding, SkinColors } from '@Zeloo/shared/skin'
...
// Mirrors @Zeloo/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @Zeloo/ink is bundled separately from packages/.
...
export const DARK_SEEDS: ThemeSeeds = {
  accent: '#FFBF00',
  // The classic Zeloo navy surfaces are IDENTITY, not derivation drift —
...
// The classic light-mode Zeloo look was never hand-authored: for years the
 ...
 *  answer when known (cached in ZELOO_TUI_BACKGROUND), else the mode's
 *  assumed pole. */
function referenceBackground(isLight: boolean, env: NodeJS.ProcessEnv = process.env): string {
  const cached = (env.ZELOO_TUI_BACKGROUND ?? '').trim()
...
// Terminal. Explicit ZELOO_TUI_THEME / COLORFGBG signals above still win,
...
// `ZELOO_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `ZELOO_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `ZELOO_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `ZELOO_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default Zeloo palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.ZELOO_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.ZELOO_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.ZELOO_TUI_BACKGROUND ?? '')
 ...
 * the OSC-11 background answer cached into ZELOO_TUI_BACKGROUND after
...
export function fromSkin(
  ...
  // itself into ZELOO_TUI_BACKGROUND. See #applySkin / syncThemeToTerminalBackground).
  ...
  //    base seeds for this polarity. The base's IDENTITY FILLS (Zeloo navy
  ...
  // the derived value already carries the identity seeds (e.g. Zeloo navy).
  const activeRow =
