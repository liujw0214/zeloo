 ...
 * `Zeloo-boot-background` / `Zeloo-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports ZELOO_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.Zeloo matches get_zeloo_home()'s default.
const bootFilePath = () => join(process.env.ZELOO_HOME ?? join(homedir(), '.Zeloo'), 'tui-theme-boot.json')

// Never touch the user's real ~/.Zeloo from test runs (the TS suite has no
// ZELOO_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into ZELOO_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.ZELOO_TUI_THEME || env.ZELOO_TUI_LIGHT) {
    return result
  }
  ...
  if (
    ...
    !env.ZELOO_TUI_BACKGROUND
  ) {
    env.ZELOO_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.ZELOO_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.ZELOO_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into ZELOO_TUI_THEME.
