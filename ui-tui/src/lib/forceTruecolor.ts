...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.ZELOO_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Zeloo on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via ZELOO_TUI_TRUECOLOR=1.
