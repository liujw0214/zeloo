...
const logDir = join(process.env.ZELOO_HOME?.trim() || join(homedir(), '.Zeloo'), 'logs')
...
// a real ~/.Zeloo (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('Zeloo-tui: parent lifecycle log unavailable\n')
    }
  }
}
