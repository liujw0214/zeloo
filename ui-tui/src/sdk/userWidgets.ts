...
import { Box, Text } from '@Zeloo/ink'
...
/**
 * User widget apps — Zeloo authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$ZELOO_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.Zeloo/plugins/`: files under ZELOO_HOME execute
...
const widgetsDir = () => join(process.env.ZELOO_HOME?.trim() || join(homedir(), '.Zeloo'), 'tui-widgets')
 ...
 *  every change, so a widget Zeloo writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Zeloo) ever writes must hot-load too — a 10s poll here read
