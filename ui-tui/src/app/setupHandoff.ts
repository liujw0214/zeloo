import type { RunExternalProcess } from '@Zeloo/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`Zeloo ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching Zeloo: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`Zeloo ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
