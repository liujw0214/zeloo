...
import { terminalBackgroundHex } from '@Zeloo/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $ZELOO_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see ZELOO_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['ZELOO_TUI_BACKGROUND', process.env.ZELOO_TUI_BACKGROUND ?? '(unset)'],
            ['ZELOO_TUI_THEME', process.env.ZELOO_TUI_THEME ?? '(unset)'],
