...
React + Ink terminal UI for Zeloo. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `ZELOO_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `Zeloo setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.Zeloo/.zeloo_history` or under `ZELOO_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `Zeloo setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/Zeloo-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external Zeloo setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.Zeloo/tui-parent.log
    ...
    types/
      Zeloo-ink.d.ts            type declarations for @Zeloo/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent ZELOOCLI subprocess for slash commands
```
