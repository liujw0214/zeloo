...
# 5. Symlinks the 'Zeloo' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Zeloo Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/Zeloo-uv-install.$$.log")"
...
if is_termux; then
    # Zeloo currently declares requires-python >=3.11,<3.14. Termux can expose
    ...
    if [ -z "${PYTHON_PATH:-}" ]; then
        if command -v python >/dev/null 2>&1; then
            ...
            echo -e "${RED}✗${NC} Termux Python $PYTHON_FOUND_VERSION is not supported; Zeloo requires Python >=3.11,<3.14"
...
# ============================================================================
# PATH setup — symlink Zeloo into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up Zeloo command..."

ZELOO_BIN="$SCRIPT_DIR/venv/bin/Zeloo"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$ZELOO_BIN" "$COMMAND_LINK_DIR/Zeloo"
echo -e "${GREEN}✓${NC} Symlinked Zeloo → $COMMAND_LINK_DISPLAY_DIR/Zeloo"
...
# ============================================================================
# Seed bundled skills into ~/.Zeloo/skills/
# ============================================================================

ZELOO_SKILLS_DIR="${ZELOO_HOME:-$HOME/.Zeloo}/skills"
mkdir -p "$ZELOO_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.Zeloo/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$ZELOO_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  Zeloo status        # Check configuration"
if is_termux; then
    echo "  Zeloo gateway       # Run gateway in foreground"
else
    echo "  Zeloo gateway install # Install gateway service (messaging + cron)"
fi
echo "  Zeloo cron list     # View scheduled jobs"
echo "  Zeloo doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m zeloo_cli.main setup
fi
