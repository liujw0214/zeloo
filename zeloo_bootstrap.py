"""Windows UTF-8 bootstrap for Zeloo entry points (no-op on POSIX).
...
point (``Zeloo``, ``Zeloo-agent``, ``Zeloo-acp``, ``gateway.run``, ``batch_runner``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``zeloo_cli._subprocess_compat.suppress_platform_ver_console`` for callers
    that never import ``zeloo_cli.main``; double application is harmless.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    ...
    Zeloo ships top-level modules with common names (``utils``, ``proxy``, ``ui``); a
    ...
    dropped and the Zeloo root is *relocated* to the front, not merely inserted when
    ...
    """
    root = src_root or os.environ.get("ZELOO_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    """Put the durable lazy-install dir (``ZELOO_LAZY_INSTALL_TARGET``) on ``sys.path``.
...
# Apply on import — entry points only need ``import zeloo_bootstrap`` first.
