...
``ANTHROPIC_API_KEY``, Zeloo-owned OAuth grants in the ``auth.json`` credential
pool, then ``~/.claude/.credentials.json`` / macOS Keychain as a borrowed fallback.
``~/.Zeloo/.anthropic_oauth.json`` (Zeloo PKCE) and
...
_SPENT_ROTATION_SIDECAR_COMMENT = (
    ...
    "consumed server-side but never durably committed. Written by Zeloo so sibling "
...
def _spent_rotation_sidecar_path(source_path: Path) -> Path:
    return source_path.with_name(source_path.name + ".Zeloo-spent-rotations.json")
...
_SINGLETON_SOURCE_PATHS = {
    "claude_code": lambda: claude_code_credentials_path(), "zeloo_pkce": lambda: _get_zeloo_oauth_file()
}
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    try:
        from zeloo_cli.auth import AUTH_LOCK_TIMEOUT_SECONDS, _auth_store_lock, env_float
        refresh_timeout_seconds = env_float("ZELOO_ANTHROPIC_REFRESH_TIMEOUT_SECONDS", 20)
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    """Prefer refreshable Claude Code creds over a static env OAuth token: Zeloo historically persisted setup tokens
...
def _resolve_anthropic_pool_token(*, skip_borrowed: bool = False) -> Optional[str]:
    ...
    refresh=False`` (never ``select()``) so diagnostic call sites (account_usage, ``Zeloo models``) never mutate
...
# ── Zeloo-native PKCE OAuth flow (~/.Zeloo/.anthropic_oauth.json); mirrors Claude Code / pi-ai / OpenCode ──


def _get_zeloo_oauth_file() -> Path:
    return get_zeloo_home() / ".anthropic_oauth.json"


def _root_zeloo_oauth_file() -> Optional[Path]:
    ...
    try:
        ...
        root = get_default_zeloo_root()
        return None if root.resolve(strict=False) == get_zeloo_home().resolve(strict=False) else root / ".anthropic_oauth.json"
    except Exception:
        return None
...
def run_zeloo_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Zeloo-native OAuth PKCE flow and return credential state."""
    import webbrowser
    ...
    print("\n".join([
        "", "Authorize Zeloo with your Claude Pro/Max subscription.", "",
    ...
    try:
        from zeloo_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_zeloo_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Zeloo-managed OAuth credentials from ~/.Zeloo/.anthropic_oauth.json."""
    data = _load_json_if_exists(_get_zeloo_oauth_file(), "Zeloo OAuth credentials")
    return data if data is not None and data.get("accessToken") else None


def _write_zeloo_oauth_credentials(
    access_token: str, refresh_token: Optional[str], expires_at_ms: Optional[int], *, target: Optional[Path] = None
) -> None:
    """Commit refreshed zeloo_pkce tokens to ~/.Zeloo/.anthropic_oauth.json (``CredentialPersistError`` on failure).
    ``target`` lets a named profile commit a grant it BORROWED from the global root back to the ROOT singleton
    instead of forking a copy under its own ZELOO_HOME; without this write-through the next ``load_pool()``
    re-seeds the stale (consumed) pair from the file over the rotated pool entry."""
    _commit_private_json(
        target if target is not None else _get_zeloo_oauth_file(),
        {"accessToken": access_token, "refreshToken": refresh_token, "expiresAt": expires_at_ms},
        "Zeloo OAuth credentials",
    )
