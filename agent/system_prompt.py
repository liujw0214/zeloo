...
from agent.prompt_builder import (
    ...
    ZELOO_AGENT_HELP_GUIDANCE, ZELOO_AGENT_HELP_GUIDANCE_NO_SKILLS, KANBAN_GUIDANCE,
...
from zeloo_constants import get_default_zeloo_root, get_zeloo_home
from utils import is_truthy_value

logger = logging.getLogger(__name__)
_PLUGIN_SECTION_FRAME_RE = re.compile(
    r"^## Plugin Context: (?P<id>[a-z0-9][a-z0-9._-]{0,127})\n<!-- Zeloo-plugin-section-chars:(?P<chars>[0-9]{1,4}) -->\n\n",
    re.MULTILINE,
)
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    """Append the desktop embedded-terminal clarifier when ``ZELOO_DESKTOP_TERMINAL``
    is set (only the desktop's TUI PTY, never the chat backend). Idempotent."""
    if not hint or _TUI_EMBEDDED_PANE_CLARIFIER in hint or not is_truthy_value(os.getenv("ZELOO_DESKTOP_TERMINAL")):
        return hint
...
def _active_profile_name(agent: Any, ambient) -> str:
    ...
    failure. Ambient resolution misreports on threads that lost the ZELOO_HOME
...
def _frozen_plugin_prompt_sections(agent: Any) -> tuple:
    ...
    if isinstance(stored_prompt, str) and stored_prompt:
        ...
        try:
            from zeloo_cli.plugins import render_system_prompt_sections
...
def _plugin_section_blocks(sections: tuple, position: str) -> List[str]:
    from zeloo_cli.plugins import format_system_prompt_sections
...
def _agent_home(agent: Any) -> Optional[Path]:
    ...
    A bound ZELOO_HOME ContextVar override wins (the gateway multiplexes
...
def _profile_name_for_home(home: Path) -> str:
    ...
    Uses ``get_default_zeloo_root()`` (NOT ``get_zeloo_home()``): on a bound
    ...
    try:
        ...
        rel = home.resolve().relative_to((get_default_zeloo_root() / "profiles").resolve())
...
def _active_profile_line(agent: Any) -> str:
    """Name the running profile so the agent doesn't conflate ``~/.Zeloo/skills``
    (default) with ``~/.Zeloo/profiles/<active>/skills``.  Resolved from the
    ...
    if active_profile == "default":
        ...
        # ROOT (get_zeloo_home() on a bound profile session is the PROFILE dir).
        # Without one, keep the ambient (patchable) resolution byte-identical.
        _root_str = str(get_default_zeloo_root() if _agent_home_path is not None else get_zeloo_home())
        return (
            "Active Zeloo profile: default. Other profiles (if any) live "
    ...
    profile_home = str(_agent_home_path) if _agent_home_path is not None else str(get_zeloo_home())
    ...
    # default profile's data sits at the ROOT (get_default_zeloo_root()), which in ambient profile mode is
    # NOT get_zeloo_home().
    default_root = get_default_zeloo_root()
    return (
        f"Active Zeloo profile: {active_profile}. This session reads "
...
def _timestamp_line(agent: Any) -> str:
    ...
    from zeloo_time import get_timezone as _zeloo_tz, now as _zeloo_now
    now = _zeloo_now()
    _bits = _zone_bits(now, _zeloo_tz())
...
def _context_files_part(agent: Any, ctx_len: Optional[int], soul_loaded: bool) -> List[str]:
    ...
    fallback they really are so the guard can reject Zeloo's bundled AGENTS.md."""
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    # Zeloo-agent skill installed, so the variant is chosen after the skills
    # index is built; this slot holds its position.
    _help_guidance_slot = len(stable_parts)
    stable_parts.append(ZELOO_AGENT_HELP_GUIDANCE_NO_SKILLS)
    stable_parts.extend(_guidance_parts(agent))
    skills_prompt = _skills_prompt(agent)
    # Skill-pointer variant requires BOTH skill_view AND the Zeloo-agent skill
    # in the rendered index (pure string check — inherits the index's stability).
    if "skill_view" in (agent.valid_tool_names or set()) and "- Zeloo-agent:" in skills_prompt:
        stable_parts[_help_guidance_slot] = ZELOO_AGENT_HELP_GUIDANCE
