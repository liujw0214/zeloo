...
YAML files in ``<ZELOO_HOME>/skill-bundles/`` (``name``, ``description``,
...
def _bundles_dir() -> Path:
    """Bundles directory: ``ZELOO_BUNDLES_DIR`` override (tests) or ``<ZELOO_HOME>/skill-bundles``."""
    override = os.environ.get("ZELOO_BUNDLES_DIR")
    return Path(override).expanduser() if override else get_zeloo_home() / "skill-bundles"
...
# File-level CRUD — used by `Zeloo bundles`.
