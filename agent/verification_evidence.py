...
_AD_HOC_SCRIPT_NAME_PREFIXES = ("Zeloo-verify-", "Zeloo-ad-hoc-")
...
def _db_path() -> Path:
    return get_zeloo_home() / "verification_evidence.db"
...
def record_verify_run(
    *, root: str | Path, session_id: str | None = None, ok: bool, command: str = "Zeloo verify",
    scope: str = "full", output: str = "",
) -> Optional[dict[str, Any]]:
    """Record a completed ``Zeloo verify`` run as verification evidence.
    ...
    return _insert_evidence(VerificationEvidence(
        command=command, canonical_command="Zeloo verify", kind="verify",
