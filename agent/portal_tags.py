...
Every Zeloo request to the Nous Portal (main loop, auxiliary client, fallback
paths) must carry the same product-attribution tags, sent in OpenAI-compatible
``extra_body['tags']``: ``["product=Zeloo-agent", "client=Zeloo-client-v<__version__>"]``.
The version is read live from ``zeloo_cli.__version__`` — do NOT pre-compute it
...
_affinity_scope: ContextVar[Optional[str]] = ContextVar("zeloo_affinity_scope", default=None)
...
def zeloo_client_tag() -> str:
    """``client=Zeloo-client-v<MAJOR>.<MINOR>.<PATCH>`` ("unknown" if zeloo_cli is unimportable)."""
    try:
        from zeloo_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"client=Zeloo-client-v{__version__}"
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    """
    tags = ["product=Zeloo-agent", zeloo_client_tag()]
