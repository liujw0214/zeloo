"""Core NeMo Relay adapters for physical Zeloo provider attempts."""
...
def _relay_operation_name(provider_name: str, metadata: dict[str, Any] | None) -> str:
    """Return Relay's canonical operation name when Zeloo knows the API mode."""
...
def _relay_metadata(provider_name: str, metadata: dict[str, Any] | None) -> dict[str, Any]:
    ...
    relay_metadata.setdefault("Zeloo.provider", provider_name)
...
class _ManagedAttempt:
    ...
    def run_callback(self, callback: Callable[..., Any], *args: Any) -> Any:
        """Run a Zeloo callback in a fresh copy of the captured context.
        ...
        def guarded() -> Any:
            # See #77244.
            # See #77244.
            # Zeloo-side callbacks run while the native pipeline drives this stream; nested relay calls
...
def _current_session_id() -> str | None:
    """Return the inherited Zeloo turn's session id, or None outside a live turn."""
...
def execute(
    ...
    ``session_id`` defaults to the inherited Zeloo turn's session (unmanaged when there is none)."""
...
# Run under the inherited Zeloo turn when present (callers that do not know a session id).
...
def stream_current(
    ...
    """Run a provider stream under the inherited Zeloo turn when present.
...
class AnthropicStreamAccumulator:
    ...
    def response(self, base: Any = None) -> Any:
        """Return the attribute-shaped response consumed by Zeloo."""
        ...
        with turn.logical_llm_lock:
            ...
            if handle is None:
                ...
                handle = turn.logical_llm_calls[request_id] = runtime.run_in_session(
                    ...
                    metadata=relay_runtime.runtime_metadata(runtime.runtime_id, **{"Zeloo.call_role": call_role}),
                )
...
def _complete_logical(
    ...
    with turn.finalize_lock:
        ...
        try:
            ...
            logger.warning("Zeloo Relay logical LLM finalization failed", exc_info=True)
            return
        with turn.logical_llm_lock:
