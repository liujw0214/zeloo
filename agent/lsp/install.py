...
Installs go to a Zeloo-owned staging dir, ``<ZELOO_HOME>/lsp/bin/``, so the
...
missing binary skips the server and ``Zeloo lsp status`` reports it).
...
# ``bin`` in ``<ZELOO_HOME>/lsp/bin/`` first, then on PATH.  ``extra_pkgs``
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # the host so `Zeloo lsp status` reports its presence.
...
def zeloo_lsp_bin_dir() -> Path:
    """Return the Zeloo-owned bin staging dir for LSP servers."""
    from zeloo_constants import get_zeloo_home

    p = get_zeloo_home() / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(zeloo_lsp_bin_dir() / name):
...
def _link_into_bin(target: Path) -> str:
    ...
    link = zeloo_lsp_bin_dir() / target.name
    if not link.exists():
        try:
...
def _install_npm(pkg: str, bin_name: str, extra_pkgs: Optional[list] = None) -> Optional[str]:
    ...
    # Managed npm first: $ZELOO_HOME/node isn't on an arbitrary process's
    # PATH, so a bare which() would miss the Node that Zeloo installed.
    npm = find_node_executable("npm")
    if npm is None:
        logger.info("[install] cannot install %s: no usable npm found", pkg)
        return None
    staging = zeloo_lsp_bin_dir().parent  # <ZELOO_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    ...
    pip_target = zeloo_lsp_bin_dir().parent / "python-packages"
...
def detect_status(pkg: str) -> str:
    """Return ``installed``, ``missing``, or ``manual-only`` (for ``Zeloo lsp status``; spawns nothing)."""
    recipe = INSTALL_RECIPES.get(pkg)
...
__all__ = ["INSTALL_RECIPES", "try_install", "detect_status", "zeloo_lsp_bin_dir"]
