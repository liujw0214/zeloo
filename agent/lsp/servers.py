...
def _find_pses_bundle(ctx: ServerContext) -> Optional[str]:
    ...
    ``PSES_BUNDLE_PATH`` env, then ``<ZELOO_HOME>/lsp/PowerShellEditorServices``."""
    ...
    candidates = [
        ...
        os.path.join(str(get_zeloo_home()), "lsp", "PowerShellEditorServices"),
    ]
...
_PSES_MISSING_MSG = (
    ...
    "lsp.servers.powershell.command to the bundle path or unzip it to <ZELOO_HOME>/lsp/PowerShellEditorServices."
)
...
def _spawn_powershell_es(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    session_dir = zeloo_lsp_session_dir()
    inner = (
        ...
        f"-HostName Zeloo -HostProfileId Zeloo -HostVersion 1.0.0 -Stdio -LogLevel Normal"
    )
    return SpawnSpec(
...
def zeloo_lsp_session_dir() -> str:
    """Return (and create) the dir for PSES session/log scratch files."""
    from zeloo_constants import get_zeloo_home
    d = os.path.join(str(get_zeloo_home()), "lsp", "pses")
