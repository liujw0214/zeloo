"""OpenAI-shape bridge shared by Zeloo' ACP clients.

ACP has no OpenAI-style ``tools``/``tool_calls`` channel, so Zeloo' tool schemas travel INTO the
...
read/edit/execute tools forwards only Zeloo' agent-level tools, since re-offering overlapping ones
makes Zeloo redo finished work.
"""
...
class StreamChunks(list):
    """Chunk list that also carries response-level attributes (e.g. ``zeloo_projected_messages``)
    Zeloo reads off the ``create`` result; a plain list would drop them on the stream path."""
