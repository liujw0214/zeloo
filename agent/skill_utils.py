...
from zeloo_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_kanban() -> bool:
    ...
    if os.getenv("ZELOO_KANBAN_TASK") or os.getenv("ZELOO_KANBAN_BOARD"):
        try:
...
def _load_raw_config() -> Dict[str, Any]:
    """Read config.yaml with an mtime+size keyed cache (no zeloo_cli.config import)."""
...
def _home_relative(p: Path) -> Path:
    """Anchor a relative config path at ZELOO_HOME; absolute paths pass through."""
    from zeloo_constants import get_zeloo_home
    return p if p.is_absolute() else get_zeloo_home() / p


# Never disableable: `Zeloo-agent` is the agent's own operating manual and the
# system prompt points at it unconditionally.
ESSENTIAL_SKILLS: frozenset = frozenset({"Zeloo-agent"})


def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    """Disabled skill names from config.yaml: global list ∪ platform list
    (*platform* defaults to ``ZELOO_PLATFORM`` / ``ZELOO_SESSION_PLATFORM``)."""
    skills_cfg = _skills_cfg()
    if skills_cfg is None:
        return set()
    from gateway.session_context import get_session_env
    resolved_platform = platform or os.getenv("ZELOO_PLATFORM") or get_session_env("ZELOO_SESSION_PLATFORM")
...
def parse_config_string_list(value) -> List[str]:
    ...
    ``Zeloo config set`` stores lists as quoted JSON/Python-literal strings;
...
def get_external_skills_dirs() -> List[Path]:
    ...
    are ``~``/``${VAR}`` expanded, relative to ZELOO_HOME; the local skills dir is skipped."""
    config_path = get_config_path()
    if not config_path.exists():
        return []
...
def get_skill_create_dir() -> Optional[Path]:
    ...
    relative to ZELOO_HOME; a value equal to the local skills dir counts as unset."""
...
def display_skill_create_dir() -> str:
    ...
    if create_dir is None:
        return f"{display_zeloo_home()}/skills/"
...
def get_all_skills_dirs() -> List[Path]:
    """Skill dirs: local ``~/.Zeloo/skills/`` first, then create_dir, then external.
...
# Project-local skills (<root>/.Zeloo/skills, <root>/.agents/skills; root = nearest
...
PROJECT_SKILLS_SUBDIRS = (os.path.join(".Zeloo", "skills"), os.path.join(".agents", "skills"))
...
def _candidate_project_skills_dirs(root: Path) -> List[Path]:
    ...
    (ZELOO_HOME itself may live inside a git checkout)."""
    local_skills = get_skills_dir().resolve()
    dirs: List[Path] = []
...
# cached under ZELOO_HOME, never inside the repo); "dangerous" excludes the
# skill from index, list, view and slash commands ("caution" loads, as on the hub).

# ── Project skill quarantine (scan-time injection defense) ──────────────── Trust (`Zeloo skills trust`)
...
# high-confidence findings only. The scan cache lives under ZELOO_HOME, never inside the repo (we don't
...
def is_quarantined_project_skill(skill_md) -> bool:
    ...
    try:
        ...
        from zeloo_constants import get_zeloo_home
        cache_dir = get_zeloo_home() / "cache" / "project_skill_scans"
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    # (which follows the live profile-scoped ZELOO_HOME), so normalization
    ...
    # ~/.Zeloo/skills/<name> may be a symlink to a checkout elsewhere, and
...
def _zeloo_metadata(frontmatter: Dict[str, Any]) -> Dict[str, Any]:
    """``metadata.Zeloo`` mapping from frontmatter, or ``{}`` when malformed."""
    metadata = frontmatter.get("metadata")
    Zeloo = metadata.get("Zeloo") if isinstance(metadata, dict) else None
    return Zeloo if isinstance(Zeloo, dict) else {}
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    Zeloo = _zeloo_metadata(frontmatter)
    return {key: Zeloo.get(key, []) for key in _CONDITION_KEYS}


def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Extract ``metadata.Zeloo.config`` declarations (key/description/default/prompt).
    Entries missing ``key`` or ``description`` are skipped; ``prompt`` defaults to the description."""
    raw = _zeloo_metadata(frontmatter).get("config")
