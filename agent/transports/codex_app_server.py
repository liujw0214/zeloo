...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # codex needs LLM provider creds but must not receive Tier-1 Zeloo secrets (gateway/GitHub/infra tokens).
        ...
        # `os.environ.copy()` also handed it every Tier-1 Zeloo secret — gateway bot tokens, GitHub auth,
        ...
        spawn_env = zeloo_subprocess_env(inherit_credentials=True)
        if env:
        ...
        # Native shell children remain unowned. Only Zeloo' managed MCP tool
        ...
        owned_task = os.environ.get("ZELOO_KANBAN_TASK") and is_dispatcher_owned_worker_context()
        if owned_task:
            for key in (*KANBAN_ENV_KEYS, "ZELOO_KANBAN_DB", "ZELOO_KANBAN_BOARD"):
                if key in os.environ:
                    cmd += ["-c", f"mcp_servers.Zeloo-mcp.env.{key}={json.dumps(os.environ[key])}"]
            cmd += ["-c", f'mcp_servers.Zeloo-mcp.env.{DELEGATED_CHILD_ENV_MARKER}=""']
        ...
        if owned_task:
            kanban_db = spawn_env.get("ZELOO_KANBAN_DB")
            default_root = os.path.join(spawn_env.get("ZELOO_HOME", os.path.expanduser("~/.Zeloo")), "kanban")
            kanban_root = os.path.dirname(kanban_db) if kanban_db else spawn_env.get("ZELOO_KANBAN_ROOT", default_root)
            cmd += [
    ...
    def initialize(
        self, client_name: str = "Zeloo", client_title: str = "Zeloo Agent",
