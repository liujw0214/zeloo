...
# long"). Codex message ids can run 400+ chars; Zeloo ``msg_...`` ids stay under the cap.
...
def _chat_messages_to_responses_input(
    ...
    decision was wrong — xAI explicitly relies on Zeloo threading encrypted reasoning back across turns for
    ...
    Zeloo' local history is never truncated by native compaction, so the full conversation is still on the
    wire.
    """
