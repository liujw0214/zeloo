...
``ZELOO_SECRET_KEY`` (never interpolated, so a hostile key name is inert); hard
...
def _run_helper(command: str, secret_key: str, timeout_seconds: float, max_output_bytes: int) -> Optional[str]:
    ...
    The key travels as DATA in ``ZELOO_SECRET_KEY``. stdout/stderr are piped
    ...
    env["ZELOO_SECRET_KEY"] = secret_key
...
class CommandSource(SecretSource):
    ...
    remediation_hints = {
        ...
        ErrorKind.INTERNAL: "Run the helper manually in a shell to see its real error — "
                            "Zeloo discards helper stderr so diagnostics can't leak "
                            "secret material.",
    }
...
def apply_command_secrets(
    *,
    ...
    # ZELOO_SECRET_KEY and parse its stdout as a dotenv blob.
...
def get_command_secret(
    *,
    ...
    ``ZELOO_SECRET_KEY``.  Returns None on any failure — never raises."""
    command = (command or "").strip()
    if not command:
        return None
