...
env-var-shaped values at process startup, AFTER ``~/.Zeloo/.env`` has loaded
and BEFORE the rest of Zeloo reads ``os.environ``. The contract is deliberately
narrow: read-only; startup-time and synchronous (one ``fetch()`` per process per
ZELOO_HOME, under a registry-enforced wall-clock timeout, no background
...
_SOURCE_ENVIRONMENT = ContextVar("zeloo_secret_source_environment", default=None)
...
_GENERIC_REMEDIATION = {
    ErrorKind.NOT_CONFIGURED: "Run `Zeloo secrets {name} setup` to finish configuration.",
    ErrorKind.BINARY_MISSING: "Run `Zeloo secrets {name} setup` to install the helper CLI.",
    ErrorKind.AUTH_FAILED: "Credentials rejected — run `Zeloo secrets {name} setup` to re-authenticate.",
    ErrorKind.AUTH_EXPIRED: "Credentials expired — run `Zeloo secrets {name} setup` to re-authenticate.",
