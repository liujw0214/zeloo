...
Pulls API keys from BSM at startup so they need not live in ``~/.Zeloo/.env``.
``bws`` is auto-installed into ``<zeloo_home>/bin/bws`` (one pinned version,
...
# <zeloo_home>/cache/bws_cache.json holds only secret VALUES (never the access
...
_ENCRYPTED_CACHE_INFO = b"Zeloo-bws-encrypted-cache-v1"
...
def _zeloo_bin_dir() -> Path:
    """Where Zeloo stores its managed binaries. Profile-aware."""
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    """Managed ``<zeloo_home>/bin/bws`` first, then PATH, then optional auto-install."""
...
def install_bws(*, force: bool = False) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="Zeloo-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
class BitwardenSource(SecretSource):
    ...
    _AUTH_HINT = (
        "Run `Zeloo secrets bitwarden token` to paste a fresh access "
        "token (create one in the Bitwarden web app: Secrets Manager → "
        "Machine accounts → Access tokens).  Wrong region?  Re-run "
        "`Zeloo secrets bitwarden setup` and pick EU/self-hosted."
    )
    ...
    def fetch(self, cfg: dict, home_path: Path) -> FetchResult:
        ...
        if not access_token:
            return result.fail(f"secrets.bitwarden.enabled is true but {access_token_env} is "
                               "not set.  Run `Zeloo secrets bitwarden setup`.", ErrorKind.NOT_CONFIGURED)
        project_id = str(cfg.get("project_id") or "")
        if not project_id:
            return result.fail("secrets.bitwarden.project_id is empty.  Run `Zeloo secrets bitwarden setup`.",
        ...
        if binary is None:
            return result.fail("bws binary not available and auto-install is disabled.  "
                               "Run `Zeloo secrets bitwarden setup` to install.", ErrorKind.BINARY_MISSING)
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_zeloo_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `Zeloo secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `Zeloo secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `Zeloo secrets bitwarden setup` to install."
        )
        return result
