...
INSPECTION_STAMP_ATTR = "data-Zeloo-vault-slot"
...
_LOGIN_CONTROL_INSPECTION_JS_TEMPLATE = """(() => {
  ...
  elements.forEach((element, index) => element.setAttribute("data-Zeloo-vault-slot", nonce + ":" + index));
...
_FILL_JS_TEMPLATE = """(() => {
  ...
  for (const f of fills) {
    const el = document.querySelector('[data-Zeloo-vault-slot="' + nonce + ':' + f.index + '"]');
  ...
  document.querySelectorAll("[data-Zeloo-vault-slot]").forEach((n) => n.removeAttribute("data-Zeloo-vault-slot"));
