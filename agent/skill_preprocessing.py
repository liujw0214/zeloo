"""Shared SKILL.md preprocessing helpers: ``${ZELOO_*}`` template tokens and
...
from zeloo_cli._subprocess_compat import IS_WINDOWS, windows_hide_flags

logger = logging.getLogger(__name__)

# ${ZELOO_SKILL_DIR} / ${ZELOO_SESSION_ID} tokens. Unresolvable ones (e.g. no
# session) are left as-is so the author can spot them.
_SKILL_TEMPLATE_RE = re.compile(r"\$\{(ZELOO_SKILL_DIR|ZELOO_SESSION_ID)\}")
...
def substitute_template_vars(content: str, skill_dir: Path | None, session_id: str | None) -> str:
    """Replace ${ZELOO_SKILL_DIR} / ${ZELOO_SESSION_ID}; tokens without a value stay in place."""
    if not content:
        return content
    values = {
        "ZELOO_SKILL_DIR": str(skill_dir) if skill_dir else None,
        "ZELOO_SESSION_ID": str(session_id) if session_id else None,
    }
