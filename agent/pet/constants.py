...
inferred from the atlas shape so Zeloo renders both legacy 8-row sheets and
...
class PetState(str, Enum):
    """Animation state a pet can be shown in (Zeloo names; Codex rows say ``jumping``/``running`` for ``jump``/``run``)."""
...
# Legacy Zeloo/petdex row order (top -> bottom) for the older 8-row, 9-column atlas.
...
# Canonical Zeloo names -> accepted row-name aliases in descending preference.
