...
try:
    import zeloo_cli as _zeloo_cli

    _ZELOO_VERSION = str(_zeloo_cli.__version__)
except Exception:
    _ZELOO_VERSION = "0.0.0"
_API_CLIENT = f"Zeloo-agent/{_ZELOO_VERSION}"  # client context per Gemini's partner-integration guidance
...
_FREE_TIER_GUIDANCE = (
    ...
    "Zeloo typically makes 3-10 API calls per user turn, so the free tier is exhausted in a handful of "
...
_STANDARD_KEY_GUIDANCE = (
    ...
    "generativelanguage.googleapis.com). Then update GEMINI_API_KEY / GOOGLE_API_KEY in ~/.Zeloo/.env and "
...
_MISSING_KEY_ERROR = (
    ...
    "environment / ~/.Zeloo/.env (get one at https://aistudio.google.com/app/apikey), or run `Zeloo setup` to "
...
class GeminiAPIError(Exception):
    """Error shape compatible with Zeloo retry/error classification."""
...
class GeminiNativeClient:
    ...
    # (No ZELOO_SKIP_ASYNC_WRAP — the async path has a real conversion, AsyncGeminiNativeClient.)
