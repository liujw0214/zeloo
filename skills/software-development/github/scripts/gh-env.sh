#!/usr/bin/env bash
# GitHub environment detection helper for Zeloo Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _zeloo_env="${ZELOO_HOME:-$HOME/.Zeloo}/.env"; [ -f "$_zeloo_env" ] && grep -q "^GITHUB_TOKEN=" "$_zeloo_env" 2>/dev/null; then
