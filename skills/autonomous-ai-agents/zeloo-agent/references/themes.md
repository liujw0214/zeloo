# Themes / Skins — Author a Zeloo Color Theme

Author a Zeloo **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`zeloo_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.Zeloo/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Zeloo itself.
...
- Write access to the Zeloo home dir — `~/.Zeloo` by default, or `$ZELOO_HOME`
  / the active profile's dir. Skins live in `<Zeloo-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `Zeloo config set`).
...
3. `write_file` it to `<Zeloo-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `Zeloo config set display.skin default` (or they can `/skin default`).
...
```
Zeloo skin set <key> <hex>      # e.g. Zeloo skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `Zeloo skin set background "#08201f"` changes
only the background; `Zeloo skin use <name>` / `Zeloo skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.Zeloo`** when a profile is active — resolve the real home
  from `$ZELOO_HOME` first, falling back to `~/.Zeloo`.
...
- **Never hand-edit `config.yaml` to activate.** Use `Zeloo config set
...
- **You apply it, not the user.** `Zeloo config set display.skin <name>` is
...
- `read_file` the written `<Zeloo-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `Zeloo config get display.skin` and confirm it reports `<name>`.
