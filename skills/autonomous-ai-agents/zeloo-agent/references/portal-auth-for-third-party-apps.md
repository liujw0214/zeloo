...
## Layer 1 — Is this thing a Zeloo plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Zeloo process**. Its LLM calls go through Zeloo's already-configured provider. | Already uses Portal if user's Zeloo is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Zeloo. That LLM call is already authenticated
via Zeloo's provider config. The plugin's own server auth (e.g.
...
What Zeloo Agent has that *feels* like OAuth — `Zeloo login --provider nous`
opening a browser, user signs in, token lands in `~/.Zeloo/auth.json` — is a
**Zeloo-specific browser flow**. Under the hood it produces a credential
Zeloo uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Zeloo's existing Portal credential out of `~/.Zeloo/auth.json`.
...
rides on the credential Zeloo already holds.

Where this could live in Zeloo:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Zeloo itself.
...
1. First decide: Zeloo plugin (runs inside Zeloo) or separate app? If plugin,
   it already uses Portal via Zeloo's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Zeloo agent itself") is fully supported. Zeloo treats agent
...
pitch in the Tool Gateway announcement is specifically about Zeloo Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Zeloo is configured to use Portal as its provider. It is
