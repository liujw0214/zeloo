...
For occasional contributors and PR authors. Full developer docs: https://Zeloo-agent.nousresearch.com/docs/developer-guide/
...
Config: `~/.Zeloo/config.yaml` (settings), `~/.Zeloo/.env` (API keys) — both under `$ZELOO_HOME` when it is set.
...
`_ZELOO_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_zeloo_home()` for paths,
never hardcode `~/.Zeloo`. For custom/local-only tools, write a plugin in
`~/.Zeloo/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `zeloo_cli/commands.py`
...
- Tests auto-redirect `ZELOO_HOME` to temp dirs — never touch real `~/.Zeloo/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/zeloo_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_zeloo_home()` from `zeloo_constants` for all paths (profile-safe)
