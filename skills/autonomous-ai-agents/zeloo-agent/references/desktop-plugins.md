...
Write plugins for the Zeloo desktop app: statusbar items, layout panes,
...
system (`~/.Zeloo/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$ZELOO_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$ZELOO_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
  ...
  `.Zeloo-package.json` marker) — the ONLY root the renderer loads from — so
...
- The Zeloo desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$ZELOO_HOME/desktop-plugins/` (usually
  `~/.Zeloo/desktop-plugins/`).

## How to Run

1. Create `$ZELOO_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.Zeloo/...` by default, or `~/.Zeloo/profiles/<profile>/...` under a
...
The ONLY import surface is `@Zeloo/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Zeloo
  ...
  (`Zeloo://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.Zeloo/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@Zeloo/plugin-sdk`, `react`, and
