...
Set via `Zeloo model` (picker) or `Zeloo setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://Zeloo-agent.nousresearch.com/docs/integrations/providers
...
| nous | OAuth device code | `Zeloo auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `Zeloo auth add openai-codex` |
| qwen-oauth | OAuth | `Zeloo auth add qwen-oauth` |
| minimax-oauth | OAuth | `Zeloo auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`Zeloo auth`).
Fallback chain when the primary fails: `Zeloo fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`zeloo_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   Zeloo config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
