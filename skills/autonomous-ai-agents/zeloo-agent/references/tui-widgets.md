...
Author widget apps for the Zeloo TUI (`Zeloo --tui`): glanceable ambient
...
- The TUI must be in use (`Zeloo --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.Zeloo/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.Zeloo/logs/tui_gateway_crash.log` if a widget never appears.
