...
Edit with `Zeloo config edit` or `Zeloo config set section.key value`.
Full reference: https://Zeloo-agent.nousresearch.com/docs/user-guide/configuration
...
`Zeloo config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `Zeloo tools` (interactive) or `Zeloo tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_ZELOO_CORE_TOOLS` is the default bundle most platforms inherit).
