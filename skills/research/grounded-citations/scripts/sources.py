...
Ledger path resolution (first wins):
  --ledger PATH
  $ZELOO_CITATION_LEDGER
  $ZELOO_HOME/cache/citations/ledger.json
"""
...
from _zeloo_home import get_zeloo_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("ZELOO_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_zeloo_home() / "cache" / "citations" / "ledger.json"
