#!/usr/bin/env python3
"""Google Workspace API CLI for Zeloo Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Zeloo-facing JSON contract and falls back to the Python client
...
from _zeloo_home import get_zeloo_home
...
def _gws_binary() -> str | None:
    override = os.getenv("ZELOO_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Zeloo Agent")
