...
@pytest.fixture
def cli():
    obj = ZELOOCLI.__new__(ZELOOCLI)  # bypass __init__ (no full app needed)
...
def _capture_opener(monkeypatch, *, opened_ok=False):
    ...
    monkeypatch.setattr(ZELOOCLI, "_open_url_in_browser", _open, raising=False)
    return seen
...
def test_free_prints_catalog_and_deep_links_with_plan(cli, monkeypatch, capsys):
    ...
    monkeypatch.setattr(ZELOOCLI, "_prompt_text_input_modal", _scripted_modal("plus"), raising=False)
...
def test_open_url_in_browser_refuses_remote_session(cli, monkeypatch):
    ...
    import zeloo_cli.auth_device_flow as auth_device_flow
...
def test_upgrade_confirm_names_the_subscription_card(cli, monkeypatch, capsys):
    ...
    monkeypatch.setattr(ZELOOCLI, "_prompt_text_input_modal", _scripted_modal("change", "ultra", "cancel"), raising=False)
