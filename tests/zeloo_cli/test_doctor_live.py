"""Tests for ``Zeloo doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from zeloo_cli import doctor_live
from zeloo_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `Zeloo doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("zeloo_cli.doctor.ZELOO_HOME", tmp_path / "home")
        monkeypatch.setattr("zeloo_cli.doctor.PROJECT_ROOT", tmp_path / "root")
