...
pointer — ``model.key_env`` in config.yaml → the env var in ``$ZELOO_HOME/.env``
(e.g. ``ZELOO_CUSTOM_LMSTUDIO_API_KEY``) — while keeping ``model.provider`` on
...
def zeloo_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("ZELOO_CUSTOM_LMSTUDIO_API_KEY", raising=False)
    return home
...
def test_model_key_env_pointer_is_honored_for_registry_provider(zeloo_home):
    _write(
        zeloo_home,
        "model:\n  default: some-model\n  provider: lmstudio\n"
        "  base_url: http://127.0.0.1:1234/v1\n"
        "  key_env: ZELOO_CUSTOM_LMSTUDIO_API_KEY\n",
        "ZELOO_CUSTOM_LMSTUDIO_API_KEY=sk-lm-desktop-saved\n",
    )
    ...
    assert creds["source"] == "ZELOO_CUSTOM_LMSTUDIO_API_KEY"


def test_model_key_env_does_not_leak_across_providers(zeloo_home):
    ...
    _write(
        ...
        "model:\n  default: gpt-x\n  provider: openai\n"
        "  key_env: ZELOO_CUSTOM_LMSTUDIO_API_KEY\n",
        "ZELOO_CUSTOM_LMSTUDIO_API_KEY=sk-lm-should-not-leak\n",
    )
    from zeloo_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER, resolve_api_key_provider_credentials
