"""Tests for zeloo_cli.update_inventory — the plan phase (#91277 Phase 2)."""
...
def fleet(monkeypatch, tmp_path):
    ...
    import re
    monkeypatch.setattr("zeloo_cli.profiles._get_default_zeloo_home", lambda: default_home)
    monkeypatch.setattr("zeloo_cli.profiles._get_profiles_root", lambda: default_home / "profiles")
    monkeypatch.setattr("zeloo_cli.profiles._PROFILE_ID_RE", re.compile(r"^[a-z0-9][a-z0-9_-]*$"), raising=False)
    monkeypatch.setattr("gateway.status._pid_exists", lambda pid: pid in (100, 200))
    monkeypatch.setattr("zeloo_cli.gateway._get_service_pids", lambda all_profiles=False: {100})
    monkeypatch.setattr("zeloo_cli.gateway.supports_systemd_services", lambda: True)
    monkeypatch.setattr("zeloo_cli.gateway.find_profile_gateway_processes", lambda exclude_pids=None: [])
    monkeypatch.setattr(
        "zeloo_cli.build_info.get_code_identity",
        lambda refresh=False: {"sha": "a" * 40, "short_sha": "a" * 8, "version": "1.0", "source": "git"},
    )
    monkeypatch.setattr("zeloo_cli.config.detect_install_method", lambda *a, **k: "git")
    monkeypatch.setattr("zeloo_cli.config.get_managed_system", lambda: None)
    return tmp_path
...
class TestCollectInventory:
    def test_two_profile_fleet(self, fleet):
        ...
        from zeloo_cli.update_inventory import describe_restart_mechanism

        assert "Zeloo -p work gateway restart" in describe_restart_mechanism(
            by_profile["work"].restart_via, "work"
        )

    def test_docker_install_not_updatable_in_place(self, fleet, monkeypatch):
        monkeypatch.setattr("zeloo_cli.config.detect_install_method", lambda *a, **k: "docker")
        monkeypatch.setattr(
            "zeloo_cli.config.recommended_update_command_for_method",
            lambda m: "docker pull nousresearch/Zeloo-agent:latest",
        )
        plan = ui.collect_runtime_inventory()
    ...
    def test_pid_file_fallback_covers_unstamped_profiles(self, fleet, monkeypatch):
        ...
        from zeloo_cli.gateway import ProfileGatewayProcess
    ...
    def test_never_raises_when_everything_fails(self, monkeypatch):
        ...
        for target in (
            "zeloo_cli.config.detect_install_method",
            "zeloo_cli.build_info.get_code_identity",
            "zeloo_cli.profiles._get_default_zeloo_home",
...
class TestReceiptIntegration:
    def test_plan_recorded_into_active_receipt(self, fleet, monkeypatch, tmp_path):
        ...
        home.mkdir()
        monkeypatch.setattr("zeloo_cli.config.get_zeloo_home", lambda: home, raising=False)
        ur._current = None
