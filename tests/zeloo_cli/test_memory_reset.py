"""Tests for the `Zeloo memory reset` CLI command.
...
- Profile-scoped reset (uses ZELOO_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake ZELOO_HOME with memory files."""
    zeloo_home = tmp_path / ".Zeloo"
    memories = zeloo_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("ZELOO_HOME", str(zeloo_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nZELOO repo is at ~/.Zeloo/Zeloo-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `Zeloo memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        zeloo_home = tmp_path / ".Zeloo"
        (zeloo_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("ZELOO_HOME", str(zeloo_home))
