...
def _uncached_sidebar_endpoints(monkeypatch):
    ...
    from zeloo_cli.web_routers import profiles as profiles_routes

    monkeypatch.setattr(profiles_routes, "_SIDEBAR_CACHE_TTL_SECONDS", 0.0)


@pytest.fixture
def profiles_on_disk(tmp_path, monkeypatch, _isolate_zeloo_home):
...
def client(monkeypatch, profiles_on_disk):
    ...
    import zeloo_state
    from zeloo_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN, app
    from zeloo_constants import get_zeloo_home
...
def _seed_project(home, name, folder):
    from zeloo_cli import projects_db
...
class TestCrossProfileProjectTree:
    ...
    def test_each_profile_contributes_its_own_projects_db(self, client, profiles_on_disk, tmp_path):
        ...
        filters through ``get_zeloo_home()``. If the fan-out failed to rebind
