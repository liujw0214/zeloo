...
When ``ZELOO_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.Zeloo/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.Zeloo            (has its own auth.json fixture)
    * Profile     -> tmp_path/.Zeloo/profiles/coder   (active, ZELOO_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Zeloo is not
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (ZELOO_HOME != ~/.Zeloo, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    zeloo_home = tmp_path / "classic"
    zeloo_home.mkdir()
