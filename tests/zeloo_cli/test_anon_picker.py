...
against a temp ``ZELOO_HOME`` with the network catalog fetch stubbed.
"""
...
from zeloo_cli.auth import _save_auth_store
...
def guest_home(monkeypatch, tmp_path):
    ...
    from zeloo_cli import model_switch_providers as msp
    ...
    return Path(os.environ["ZELOO_HOME"])
...
def _cli_nous_rows(config):
    from zeloo_cli.main_provider_setup import _build_provider_picker_rows
...
def test_guest_identity_shows_free_tier_row_with_only_welcome_model(guest_home, monkeypatch):
    ...
    # The `Zeloo model` provider picker applies the same rule from the same helper.
