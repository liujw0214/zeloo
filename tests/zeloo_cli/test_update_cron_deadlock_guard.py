...
When `Zeloo update` runs INSIDE the gateway's own process tree (the
Zeloo-auto-update cron job), waiting for that gateway to exit is a
circular wait:

  gateway waits on all in-flight work units (#77184)
    -> cron agent session waits on the `Zeloo update` process
      -> `Zeloo update` waits on the gateway to exit  [back to A]
...
class TestSelfRestartFireAndForget:
    ...
    def test_refuses_non_ancestor_pid(self):
        from zeloo_cli.gateway import _request_gateway_self_restart
...
class TestDrainOrSignalTriage:
    ...
    def test_ancestor_gateway_is_signalled_fire_and_forget(self, monkeypatch):
        ...
        from zeloo_cli.update_cmd import _drain_or_signal_gateway_for_update
