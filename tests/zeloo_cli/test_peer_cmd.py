"""Tests for ``Zeloo peer`` — cross-machine bot-to-bot DMs."""
...
from zeloo_cli.subcommands import peer as peer_cmd
...
class _FakePeer(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == "/api/sessions":
            ...
            return self._json({"object": "Zeloo.session", "session": {"id": "bc_1", "title": body.get("title")}}, 201)
...
# ── per-turn author (ZELOO_TURN_AUTHOR set by the message_agent runner) ─────
...
class _AttackerOrigin(BaseHTTPRequestHandler):
    ...
    a compromised/MITM'd peer could redirect a ``Zeloo peer dm`` request to."""
...
class _RedirectingPeer(BaseHTTPRequestHandler):
    ...
    the shape of a compromised peer or a LAN MITM answering ``Zeloo peer
