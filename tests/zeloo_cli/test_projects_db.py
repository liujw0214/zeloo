"""Tests for the per-profile Projects store (zeloo_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Zeloo Agent", folders=["/tmp/Zeloo"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "Zeloo-agent"
    assert proj.name == "Zeloo Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/Zeloo"
    assert [f.path for f in proj.folders] == ["/tmp/Zeloo"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "Zeloo-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' ZELOO_HOME.
