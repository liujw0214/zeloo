"""Tests for zeloo_cli.mcp_catalog and zeloo_cli.mcp_picker.
...
def catalog_dir(tmp_path, monkeypatch):
    ...
    cat.mkdir()
    monkeypatch.setenv("ZELOO_OPTIONAL_MCPS", str(cat))
    return cat


@pytest.fixture(autouse=True)
def _isolate_zeloo_home(tmp_path, monkeypatch):
    """Redirect all config I/O to a temp ZELOO_HOME."""
    hh = tmp_path / "Zeloo-home"
    hh.mkdir()
    monkeypatch.setenv("ZELOO_HOME", str(hh))
    monkeypatch.setattr(
        "zeloo_cli.config.get_zeloo_home", lambda: hh
    )
    monkeypatch.setattr(
        "zeloo_cli.config.get_config_path", lambda: hh / "config.yaml"
    )
    monkeypatch.setattr(
        "zeloo_cli.config.get_env_path", lambda: hh / ".env"
    )
    # mcp_catalog grabs get_zeloo_home() lazily through zeloo_constants
    monkeypatch.setattr(
        "zeloo_constants.get_zeloo_home", lambda: hh
    )
    return hh
...
def _entry(name: str):
    ...
    from zeloo_cli.mcp_catalog import get_entry
...
class TestManifestParsing:
    def test_minimal_valid(self, catalog_dir):
        ...
        from zeloo_cli.mcp_catalog import list_catalog
    ...
    def test_suggest_empty_block_rejected(self, catalog_dir):
        ...
        from zeloo_cli.mcp_catalog import list_catalog, catalog_diagnostics
    ...
    def test_http_api_key_builds_bearer_headers_template(self, catalog_dir):
        ...
        _write_manifest(catalog_dir, "demo", body)
        from zeloo_cli.mcp_catalog import _build_server_config
    ...
    def test_http_api_key_requires_matching_env_declaration(self, catalog_dir):
        ...
        from zeloo_cli.mcp_catalog import CatalogError, _parse_manifest
...
class TestInstall:
    def test_install_simple_stdio_writes_config(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from zeloo_cli.mcp_catalog import install_entry
    ...
    def test_install_rejects_exfil_shaped_stdio_manifest(self, catalog_dir):
        ...
        from zeloo_cli.config import load_config
        from zeloo_cli.mcp_catalog import CatalogError, install_entry
    ...
    def test_install_with_api_key_prompts_and_saves(self, catalog_dir, monkeypatch):
        ...
        from zeloo_cli.mcp_catalog import install_entry
        from zeloo_cli.config import get_env_value, load_config
...
class TestUninstall:
    def test_uninstall_removes_server_block(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from zeloo_cli.mcp_catalog import install_entry, uninstall_entry
        from zeloo_cli.config import load_config
    ...
    def test_uninstall_missing_returns_false(self):
        from zeloo_cli.mcp_catalog import uninstall_entry
...
class TestPicker:
    def test_show_catalog_empty(self, catalog_dir, capsys):
        from zeloo_cli.mcp_picker import show_catalog
    ...
    def test_install_by_name_success(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from zeloo_cli.mcp_picker import install_by_name
        from zeloo_cli.config import load_config
...
class TestGitInstallShaRef:
    def test_sha_ref_skips_branch_attempt(self, catalog_dir, monkeypatch, tmp_path):
        ...
        from zeloo_cli import mcp_catalog
        from zeloo_cli.mcp_catalog import _do_git_install
        ...
        from zeloo_cli import git_credentials
...
class TestToolsConfigIncludeMode:
    def test_configure_mcp_writes_include_not_exclude(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "zeloo_cli.curses_ui.curses_checklist",
...
class TestShippedCatalog:
    def test_all_shipped_manifests_parse(self, monkeypatch):
        ...
        """
        # Use the actual repo's optional-mcps directory (no ZELOO_OPTIONAL_MCPS
        # override) so this test catches real manifests.
        monkeypatch.delenv("ZELOO_OPTIONAL_MCPS", raising=False)
        from zeloo_cli.mcp_catalog import _catalog_root, _parse_manifest
    ...
    def test_all_shipped_manifests_are_version_locked(self, monkeypatch):
        ...
        pyproject dependencies — everything Zeloo fetches/launches is pinned
        to an exact version.
