...
from zeloo_cli.web_server_gateway import _collect_profile_gateway_topology, _profile_platform_ports
...
class TestStatusEndpointTopology:
    ...
    def _setup_client(self, monkeypatch, _isolate_zeloo_home):
        try:
    ...
    def test_profile_names_and_mode_public_when_auth_gated(self, monkeypatch):
        ...
        # Zeloo Cloud Portal reads /api/status over the network (a gated bind)
        ...
        try:
            ...
            # alongside zeloo_home / gateway_pid.
            assert "gateways" not in data
            assert "zeloo_home" not in data
