"""Tests for zeloo_cli.profile_distribution — git-based profile installs.
...
from zeloo_cli.profile_distribution import (
    DEFAULT_DIST_OWNED,
    DistributionError,
...
# ---------------------------------------------------------------------------
# Isolated profile env (matches tests/zeloo_cli/test_profiles.py)
# ---------------------------------------------------------------------------
...
class TestVersionRequires:
    ...
    def test_check_matrix(self, spec, cur, ok):
        if ok:
            check_zeloo_requires(spec, cur)
        else:
            with pytest.raises(DistributionError, match="requires Zeloo"):
                check_zeloo_requires(spec, cur)
...
class TestInstall:
    ...
    def test_update_respects_distribution_owned_allowlist(self, profile_env):
        ...
        from zeloo_cli.profile_distribution import read_manifest as _read
    ...
    def test_install_enforces_zeloo_requires(self, profile_env, monkeypatch):
        # Pin current Zeloo version to something well below the requirement
        import zeloo_cli
        monkeypatch.setattr(zeloo_cli, "__version__", "0.1.0", raising=False)

        mf = DistributionManifest(
            name="future",
            version="1.0.0",
            zeloo_requires=">=99.0.0",
        )
...
class TestInstalledAtStamp:
    ...
    def test_update_refreshes_installed_at(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr(
            "zeloo_cli.profile_distribution.datetime", _FakeDT, raising=True
        )

        from zeloo_cli.profile_distribution import update_distribution
...
class TestProfileInfoDistribution:
    ...
    def test_malformed_manifest_does_not_break_list(self, profile_env):
        from zeloo_cli.profiles import create_profile, list_profiles, get_profile_dir
