...
from zeloo_cli import kanban_ops
from zeloo_cli.kanban import kanban_command
from zeloo_cli.kanban_parser import build_parser
...
def test_watch_names_resolved_board(tmp_path, monkeypatch, capsys, environment, explicit, expected):
    ...
    monkeypatch.setenv("ZELOO_KANBAN_HOME", str(tmp_path))
    ...
    if environment:
        monkeypatch.setenv("ZELOO_KANBAN_BOARD", environment)
