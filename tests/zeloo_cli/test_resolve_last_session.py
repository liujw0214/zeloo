"""Verify `Zeloo -c` picks the session the user most recently used."""
...
def test_search_sessions_exposes_last_active_column(tmp_path, monkeypatch):
    ...
    db = zeloo_state.SessionDB(db_path=Path(tmp_path / "state.db"))
    try:
...
def test_resolve_last_session_real_db_prefers_workspace(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "zeloo_cli.main.subprocess.run",
        lambda cmd, **kw: __import__("subprocess").CompletedProcess(
            cmd, 0, stdout=str(repo_a), stderr=""
        ),
    )
    monkeypatch.setattr("zeloo_state.SessionDB", lambda: real_db(db_path=state_db))
