...
`ZELOO_REDACT_SECRETS` at module-import time. `zeloo_cli/main.py` at
...
in .env), the toggle was silently ignored in both `Zeloo chat` and
`Zeloo gateway run`.

Fix: bridge `security.redact_secrets` from config.yaml → `ZELOO_REDACT_SECRETS`
env var in `zeloo_cli/main.py` BEFORE the `setup_logging()` call.
"""
import os
import subprocess
import sys
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports zeloo_cli.main and checks
    # _REDACT_ENABLED. Must be a subprocess — we need a clean module state.
    probe = textwrap.dedent(
        """\
        import sys, os
        # Make absolutely sure the env var is not pre-set
        os.environ.pop("ZELOO_REDACT_SECRETS", None)
        sys.path.insert(0, %r)
        import zeloo_cli.main  # triggers the bridge + setup_logging
        import agent.redact
        print(f"REDACT_ENABLED={agent.redact._REDACT_ENABLED}")
        print(f"ENV_VAR={os.environ.get('ZELOO_REDACT_SECRETS', '<unset>')}")
        """
    ) % str(REPO_ROOT)

    env = dict(os.environ)
    env["ZELOO_HOME"] = str(zeloo_home)
    env.pop("ZELOO_REDACT_SECRETS", None)
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    """.env ZELOO_REDACT_SECRETS takes precedence over config.yaml."""
    zeloo_home = tmp_path / ".Zeloo"
    zeloo_home.mkdir()
    (zeloo_home / "config.yaml").write_text(
        textwrap.dedent(
    ...
    (zeloo_home / ".env").write_text("ZELOO_REDACT_SECRETS=true\n")
