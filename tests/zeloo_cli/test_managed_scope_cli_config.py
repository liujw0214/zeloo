...
zeloo_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `Zeloo config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_zeloo_home = get_zeloo_home()`` at import time (module
    singleton), so monkeypatching ZELOO_HOME after import doesn't move it.
    ...
    this only matters for tests that swap ZELOO_HOME.)
    """
    import cli

    cli._zeloo_home = home
