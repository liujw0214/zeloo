"""Tests for zeloo_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $ZELOO_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Zeloo-managed node dir.
"""
...
def _make_zeloo_node(zeloo_home: Path) -> Path:
    """Create a fake $ZELOO_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = zeloo_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    zeloo_home = fake_home / ".Zeloo"
    _make_zeloo_node(zeloo_home)
    ...
    removed = uninstall.remove_node_symlinks(zeloo_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    zeloo_home = fake_home / ".Zeloo"
    node_bin = _make_zeloo_node(zeloo_home)
