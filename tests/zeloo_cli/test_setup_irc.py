"""Tests for IRC gateway configuration via `Zeloo setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Zeloo install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Zeloo install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#Zeloo")
            monkeypatch.setenv("IRC_NICKNAME", "Zeloo-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `Zeloo setup gateway` experience with IRC present."""
