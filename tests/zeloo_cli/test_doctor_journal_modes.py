...
`Zeloo doctor` lists each Zeloo-managed database with its journal mode and
...
class TestUnreadableReason:
    def test_missing_file_keeps_the_os_error_text(self, tmp_path):
        ...
        # The OS error text is preserved verbatim.  On Linux this is
        # "No such file or directory"; on Windows it's a localized
        # WinError string.  The file path appears in both.
