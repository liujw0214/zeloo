...
The notice prints the most recent curator run summary on `Zeloo update`,
...
session start) so users normally never see the rename map. `Zeloo update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    from zeloo_cli import update_cmd_maint
