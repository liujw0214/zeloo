"""``Zeloo update`` skips the editable reinstall when the pull can't affect it.
...
that rewrite is the only reason the running ``Zeloo.exe`` gets quarantined,
...
from zeloo_cli.update_cmd import _editable_install_is_current
