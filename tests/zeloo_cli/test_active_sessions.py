...
from zeloo_cli import active_sessions
...
def test_release_orphaned_leases_reclaims_only_unowned_own_pid_entries(tmp_path, monkeypatch):
    ...
    ``_prune_dead`` only fires when the owning pid dies, so a ``Zeloo
    ...
    _backdate_leases(tmp_path / ".Zeloo")
...
def test_release_under_profile_home_override_targets_acquisition_registry(
    ...
    """Regression for #85431: a lease acquired against the root ZELOO_HOME
