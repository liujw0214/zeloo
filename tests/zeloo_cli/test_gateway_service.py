...
import zeloo_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    ...
    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`Zeloo gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `Zeloo gateway restart` in the background; when a
        ...
        assert "Zeloo gateway status" in output
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``ZELOO_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../zeloo_test`` ZELOO_HOME — silently breaking
        ...
        carry those markers.
        """
        unit_path = tmp_path / "Zeloo-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")

        monkeypatch.setattr(
            gateway_cli, "get_systemd_unit_path", lambda system=False: unit_path
        )
        # Realistic generated unit referencing a pytest tmpdir ZELOO_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="ZELOO_HOME=/tmp/pytest-of-alice/pytest-42/'
            'popen-gw0/test_x/zeloo_test"\n'
        )
        monkeypatch.setattr(
            gateway_cli,
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_tmp_home_in_systemd_unit(self):
        unit = '[Service]\nEnvironment="ZELOO_HOME=/tmp/Zeloo-e2e-41264"\n'
        assert (
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="ZELOO_HOME={tmp_path}/Zeloo-home"\n'
...
class TestServiceIdentityForForeignHome:
    """A ZELOO_HOME that is neither ``~/.Zeloo`` nor ``~/.Zeloo/profiles/<name>`` must never resolve to
    the default profile's ``Zeloo-gateway`` unit (a temp-home harness uninstalled the production gateway)."""
    ...
    def test_foreign_home_gets_its_own_unit(self, machine_home, tmp_path, monkeypatch):
        ...
        foreign.mkdir()
        monkeypatch.setenv("ZELOO_HOME", str(foreign))

        default_unit = machine_home / ".config" / "systemd" / "user" / "Zeloo-gateway.service"
        assert gateway_cli.get_service_name() != "Zeloo-gateway"
    ...
    def test_default_and_named_profile_homes_keep_their_names(self, machine_home, monkeypatch):
        default_home = machine_home / ".Zeloo"
        (default_home / "profiles" / "alpha").mkdir(parents=True)

        monkeypatch.setenv("ZELOO_HOME", str(default_home))
        assert gateway_cli.get_service_name() == "Zeloo-gateway"

        monkeypatch.setenv("ZELOO_HOME", str(default_home / "profiles" / "alpha"))
        assert gateway_cli.get_service_name() == "Zeloo-gateway-alpha"


class TestUninstallRefusesForeignUnit:
    """systemd_uninstall must not stop/disable/unlink a unit pinned to another ZELOO_HOME."""

    def test_unit_for_other_home_is_left_alone(self, tmp_path, monkeypatch, capsys):
        unit_path = tmp_path / "Zeloo-gateway.service"
        unit_path.write_text('[Service]\nEnvironment="ZELOO_HOME=/somewhere/else"\n', encoding="utf-8")
        monkeypatch.setenv("ZELOO_HOME", str(tmp_path / "mine"))
        monkeypatch.setattr(gateway_cli, "get_systemd_unit_path", lambda system=False: unit_path)
...
class TestGetCronDrainTimeout:
    def test_missing_config_falls_back_to_default(self, monkeypatch):
        monkeypatch.delenv("ZELOO_CRON_DRAIN_TIMEOUT", raising=False)
    ...
    def test_env_overrides_config(self, monkeypatch):
        monkeypatch.setenv("ZELOO_CRON_DRAIN_TIMEOUT", "45")
        monkeypatch.setattr(
            gateway_cli,
            "read_raw_config",
...
class TestGeneratedSystemdUnits:
    ...
    def test_timeout_stop_sec_covers_default_cron_drain_floor(self, monkeypatch):
        ...
        monkeypatch.delenv("ZELOO_RESTART_DRAIN_TIMEOUT", raising=False)
        monkeypatch.delenv("ZELOO_CRON_DRAIN_TIMEOUT", raising=False)
    ...
    def test_restart_exit_code_is_also_declared_a_success_status(self):
        ...
        and any OnFailure= alert unit fires on every routine restart (Zeloo update,
        Zeloo gateway restart, the in-app restart). SuccessExitStatus=75 keeps the same
    ...
    def test_user_unit_does_not_leak_profile_node_symlink_target(self, tmp_path, monkeypatch):
        ...
        profile_node_bin = tmp_path / ".Zeloo" / "profiles" / "jarvis" / "node" / "bin"
    ...
    def test_launchd_plist_persists_configured_nofile_soft_limit(self, monkeypatch):
        ...
        plist rewrite by `Zeloo gateway start` cannot strip the FD floor and
        reintroduce EMFILE crashes (launchd default soft limit is 256)."""
        import zeloo_cli.resource_limits as resource_limits
...
class TestLaunchdServiceRecovery:
    ...
    def test_refresh_defers_reload_when_running_inside_gateway_tree(self, tmp_path, monkeypatch):
        ...
        plist_path = tmp_path / "ai.Zeloo.gateway.plist"
        ...
        monkeypatch.setattr(
            ...
            lambda: (
                "<plist>--replace\n<key>ZELOO_HOME</key>"
                "<string>/Users/alice/.Zeloo</string></plist>"
            ),
        )
    ...
    def test_launchd_status_reports_fallback_when_unsupported_and_pid_running(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "get_zeloo_home", lambda: tmp_path)
...
class TestGatewaySystemServiceRouting:
    ...
    def test_launchd_restart_uses_sigusr1_and_exit_wait_budget(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "get_launchd_label", lambda: "ai.Zeloo.gateway")
        ...
        assert ("observe", "ai.Zeloo.gateway", 654, "gui/501") in calls
        out = capsys.readouterr().out
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.Zeloo.gateway"])
            ),
        )
...
class TestSystemUnitZELOOHome:
    """ZELOO_HOME in system units must reference the target user, not root."""

    def test_empty_managed_node_dir_uses_only_ambient_fallback(
        self, monkeypatch, tmp_path
    ):
        managed_bin = tmp_path / ".Zeloo" / "node" / "bin"
        ...
        gateway_cli._append_node_dir_for_service(entries, tmp_path / ".Zeloo")
    ...
    def test_managed_node_makes_system_unit_independent_of_callers_path(
        ...
        target_ZELOO = target_home / ".Zeloo"
        root_home = tmp_path / "root"
        root_ZELOO = root_home / ".Zeloo"
        managed_bin = target_ZELOO / "node" / "bin"
        managed_bin.mkdir(parents=True)
        node = managed_bin / "node"
        node.write_text("#!/bin/sh\n")
        node.chmod(0o755)
        root_ZELOO.mkdir(parents=True)

        monkeypatch.setattr(Path, "home", staticmethod(lambda: root_home))
        monkeypatch.setenv("ZELOO_HOME", str(root_ZELOO))
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", str(target_home), 1001),
        )
        monkeypatch.setattr(gateway_cli, "get_zeloo_home", lambda: root_ZELOO)
    ...
    def test_node_path_lookup_remains_fallback_without_managed_node(
        ...
        monkeypatch.setattr(
            "zeloo_constants.iter_zeloo_node_dirs", lambda root=None: []
        )
        monkeypatch.setattr(
            "zeloo_constants.zeloo_managed_node_tree_present",
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        monkeypatch.delenv("ZELOO_HOME", raising=False)
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
        ...
        assert 'ZELOO_HOME=/home/alice/.Zeloo' in unit
        assert '/root/.Zeloo' not in unit


    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's ZELOO_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        zeloo_home = str(gateway_cli.get_zeloo_home().resolve())
        assert f'ZELOO_HOME={zeloo_home}' in unit


class TestSystemUnitRefreshSyncsZELOOHome:
    """sudo system refresh must not flip TimeoutStopSec via /root/.Zeloo."""

    def test_refresh_adopts_unit_zeloo_home_before_rewriting(self, tmp_path, monkeypatch):
        root_home = tmp_path / "root"
        alice_home = tmp_path / "alice"
        root_ZELOO = root_home / ".Zeloo"
        alice_ZELOO = alice_home / ".Zeloo"
        root_ZELOO.mkdir(parents=True)
        alice_ZELOO.mkdir(parents=True)
        (root_ZELOO / "config.yaml").write_text(
            "agent:\n  restart_drain_timeout: 60\n", encoding="utf-8"
        )
        (alice_ZELOO / "config.yaml").write_text(
        ...
        # Correct installed unit (operator's ZELOO_HOME + drain timeout).
        monkeypatch.setenv("ZELOO_HOME", str(alice_ZELOO))
        ...
        # Simulate sudo without inherited ZELOO_HOME (falls back to root).
        monkeypatch.setenv("ZELOO_HOME", str(root_ZELOO))
        ...
        assert os.environ["ZELOO_HOME"] == str(alice_ZELOO)
    ...
    def test_is_current_syncs_before_reading_unit(self, tmp_path, monkeypatch):
        ...
        unit's pinned ZELOO_HOME *before* it reads/compares the unit.
        ...
        monkeypatch.setattr(gateway_cli, "_sync_zeloo_home_from_systemd_unit", tracking_sync)
    ...
    def test_start_and_restart_delegate_sync_to_chokepoint(self, monkeypatch):
        ...
        for entry in ("systemd_start", "systemd_restart"):
            ...
            monkeypatch.setattr(
                gateway_cli,
                "_sync_zeloo_home_from_systemd_unit",
...
class TestZELOOHomeForTargetUser:
    """Unit tests for _zeloo_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("ZELOO_HOME", raising=False)

        result = gateway_cli._zeloo_home_for_target_user("/home/alice")
        assert result == "/home/alice/.Zeloo"
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "Zeloo gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        root_home = tmp_path / "root"
        target_home = tmp_path / "home" / "alice"
        root_profile = root_home / ".Zeloo" / "profiles" / "mybot"
        root_profile.mkdir(parents=True)

        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("ZELOO_HOME", str(root_profile))
        monkeypatch.setattr(gateway_cli, "get_zeloo_home", lambda: root_profile)
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", str(target_home), 1001),
        ...
        assert f'ZELOO_HOME={target_home / ".Zeloo" / "profiles" / "mybot"}' in unit

    def test_launchd_plist_wraps_gateway_stderr_with_timestamps(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".Zeloo" / "profiles" / "mybot"
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZELOO_HOME", str(profile_dir))
        monkeypatch.setattr(gateway_cli, "get_zeloo_home", lambda: profile_dir)
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".Zeloo" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.Zeloo.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".Zeloo" / "Zeloo-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".Zeloo" / "Zeloo-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        root_home = tmp_path / "root"
        root_home.mkdir()
        project = root_home / ".Zeloo" / "Zeloo-agent"
        ...
        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("ZELOO_HOME", str(root_home / ".Zeloo"))
        monkeypatch.setattr(gateway_cli, "get_zeloo_home", lambda: root_home / ".Zeloo")
        ...
        # WorkingDirectory is anchored at the target user's ZELOO_HOME (stable,
        ...
        assert "WorkingDirectory=/home/alice/.Zeloo" in unit
        assert "WorkingDirectory=/home/alice/.Zeloo/Zeloo-agent" not in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "Zeloo-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "Zeloo-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'Zeloo gateway install' inside Docker exits 0 with container guidance."""
        import pytest
...
class TestLegacyZELOOUnitDetection:
    """Tests for _find_legacy_zeloo_units / has_legacy_zeloo_units.

    These guard against the scenario that tripped Luis in April 2026: an
    older install left a ``Zeloo.service`` unit behind when the service was
    renamed to ``Zeloo-gateway.service``. After PR #5646 (signal recovery
    via systemd), the two services began SIGTERM-flapping over the same
    Telegram bot token in a 30-second cycle.

    The detector must flag ``Zeloo.service`` ONLY when it actually runs our
    gateway, and must NEVER flag profile units
    (``Zeloo-gateway-<profile>.service``) or unrelated third-party services.
    """

    # Minimal ExecStart that looks like our gateway
    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=Zeloo Gateway\n[Service]\n"
        "ExecStart=/usr/bin/python -m zeloo_cli.main gateway run --replace\n"
    )
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        (user_dir / "Zeloo.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        (system_dir / "Zeloo.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        results = gateway_cli._find_legacy_zeloo_units()
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m zeloo_cli.main gateway run
          - python path/to/zeloo_cli/main.py gateway run
          - Zeloo gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m zeloo_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/Zeloo/zeloo_cli/main.py gateway run",
            "ExecStart=/usr/local/bin/Zeloo gateway run --replace",
            "ExecStart=/venv/bin/python /opt/Zeloo/gateway/run.py",
        ]
        for i, execstart in enumerate(variants):
            name = "Zeloo.service" if i == 0 else "Zeloo.service"  # same name
            # Test each variant fresh
            (user_dir / "Zeloo.service").write_text(
                f"[Unit]\nDescription=Old Zeloo\n[Service]\n{execstart}\n",
                encoding="utf-8",
            )
            results = gateway_cli._find_legacy_zeloo_units()
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "Zeloo.service" in out
        assert "Zeloo gateway migrate-legacy" in out



class TestRemoveLegacyZELOOUnits:
    """Tests for remove_legacy_zeloo_units (the migration action)."""
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        legacy = user_dir / "Zeloo.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        removed, remaining = gateway_cli.remove_legacy_zeloo_units(interactive=False)
        ...
        assert any("--user stop Zeloo.service" in c for c in cmds_joined)
        assert any("--user disable Zeloo.service" in c for c in cmds_joined)
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (Zeloo-gateway-coder.service)
        ...
        profile_unit = user_dir / "Zeloo-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "Zeloo-gateway.service"
...
class TestMigrateLegacyCommand:
    """Tests for the `Zeloo gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import zeloo_cli.main as cli_main
        ...
        # public, just confirm that `Zeloo gateway --help` shows it.
        import subprocess
        import sys

        project_root = cli_main.PROJECT_ROOT if hasattr(cli_main, "PROJECT_ROOT") else None
        if project_root is None:
            import zeloo_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "zeloo_cli.main", "gateway", "--help"],
    ...
    def test_gateway_command_migrate_legacy_dispatches(
        ...
        monkeypatch.setattr(gateway_cli, "remove_legacy_zeloo_units", fake_remove)
        monkeypatch.setattr(gateway_cli, "supports_systemd_services", lambda: True)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: False)
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "zeloo_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemdInstallOffersLegacyRemoval:
    ...
    def test_install_offers_removal_when_legacy_detected(
        ...
        # has_legacy_zeloo_units must return True
        monkeypatch.setattr(gateway_cli, "has_legacy_zeloo_units", lambda: True)
        monkeypatch.setattr(gateway_cli, "remove_legacy_zeloo_units", fake_remove)
    ...
    def test_install_skips_legacy_check_when_none_present(
        ...
        monkeypatch.setattr(gateway_cli, "has_legacy_zeloo_units", lambda: False)
        monkeypatch.setattr(gateway_cli, "remove_legacy_zeloo_units", fake_remove)
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "Zeloo-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "Zeloo-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "Zeloo-gateway.service",
        )
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``Zeloo gateway start --system``).
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "Zeloo-gateway")
        ...
        assert "sudo systemctl start Zeloo-gateway" in out
        assert "sudo Zeloo gateway uninstall --system" in out
        assert "Zeloo gateway install" in out


class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``Zeloo gateway start --system`` etc.) must
...
class TestServiceWorkingDirIsStable:
    ...
    (ZELOO_HOME), never the source checkout / worktree, so a relocated or
    deleted checkout can't crash-loop the unit on CHDIR (status=200).
    """



    def test_user_unit_workingdirectory_is_zeloo_home_not_checkout(self, tmp_path, monkeypatch):
        home = tmp_path / ".Zeloo"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_zeloo_home", lambda: home)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_workingdirectory_is_zeloo_home(self, tmp_path, monkeypatch):
        import re
...
class TestLaunchctlBootstrapEioRetry:
    ...
    PLIST = "/tmp/ai.Zeloo.gateway.plist"
    DOMAIN = "gui/501"
    LABEL = "ai.Zeloo.gateway"
...
class TestLaunchdUnloadedJobStderrStaysOffTerminal:
    ...
    def fake_launchctl(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_launchd_label", lambda: "ai.Zeloo.gateway")
        monkeypatch.setattr(gateway_cli, "_launchd_domain", lambda: "gui/501")
        monkeypatch.setattr(gateway_cli, "get_launchd_plist_path", lambda: tmp_path / "ai.Zeloo.gateway.plist")
        monkeypatch.setattr(gateway_cli, "_clear_launchd_unsupported_marker", lambda: None)
...
class TestTimeoutStopSecCoversCronFloor:
    ...
    def _unit_with_config(self, tmp_path, monkeypatch, config_yaml, env=None):
        Zeloo = tmp_path / "home" / ".Zeloo"
        Zeloo.mkdir(parents=True)
        (Zeloo / "config.yaml").write_text(config_yaml, encoding="utf-8")
        monkeypatch.setenv("ZELOO_HOME", str(Zeloo))
        monkeypatch.delenv("ZELOO_RESTART_DRAIN_TIMEOUT", raising=False)
        monkeypatch.delenv("ZELOO_CRON_DRAIN_TIMEOUT", raising=False)
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
        monkeypatch.setattr(
    ...
    def test_env_override_extends_the_leash(self, tmp_path, monkeypatch):
        unit = self._unit_with_config(
            ...
            monkeypatch,
            "agent:\n  restart_drain_timeout: 0\n",
            env={"ZELOO_CRON_DRAIN_TIMEOUT": "200"},
        )
