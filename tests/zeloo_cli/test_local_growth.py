...
def test_overrides_roundtrip_and_clear(zeloo_home):
    from zeloo_cli.local_runtime.growth import (
...
def test_corrupt_overrides_read_as_empty(zeloo_home):
    from zeloo_cli.local_runtime.growth import (
        load_window_overrides,
...
def test_growth_declines_foreign_endpoints(zeloo_home):
    ...
    from zeloo_cli.local_runtime.growth import maybe_grow_window
...
def test_occupancy_confirmed_skips_gate_one():
    ...
    from zeloo_cli.local_runtime.context_policy import growth_decision
    from zeloo_cli.local_runtime.estimator import (
        HardwareBudget,
        LayerKind,
        ModelProfile,
    )
    ...
    # Zeloo' threshold (e.g. 80% of window) can sit BELOW the ladder's 85%
...
def _tiny_profile(model_id: str):
    from zeloo_cli.local_runtime.estimator import LayerKind, ModelProfile
...
def test_preset_generation_for_catalog_model_with_mmproj(zeloo_home, tmp_path, monkeypatch):
    ...
    import zeloo_cli.local_runtime.presets as presets_mod

    from zeloo_cli.local_runtime.catalog import CATALOG
    from zeloo_cli.local_runtime.estimator import HardwareBudget
...
def test_preset_restores_grown_window_capped_at_native(zeloo_home, tmp_path, monkeypatch):
    ...
    from zeloo_cli.local_runtime.estimator import HardwareBudget
    from zeloo_cli.local_runtime.growth import save_window_override
...
def test_preset_ignores_override_below_launch_window(zeloo_home, tmp_path, monkeypatch):
...
def test_preset_restores_grown_window_midladder(zeloo_home, tmp_path, monkeypatch):
    """The real growth shape: launch at a lower rung, override to a middle
    rung -> the preset window follows the override."""
    import zeloo_cli.local_runtime.presets as presets_mod

    from zeloo_cli.local_runtime.estimator import HardwareBudget, LayerKind, ModelProfile
    from zeloo_cli.local_runtime.growth import save_window_override
...
def test_mtp_plan_matches_cost_at_initial_and_restored_windows(zeloo_home, tmp_path, monkeypatch):
    from dataclasses import replace
    from types import SimpleNamespace

    from zeloo_cli.local_runtime import presets
    from zeloo_cli.local_runtime.context_policy import FLOOR, RUNTIME_OVERHEAD_BYTES, ub_logits_bytes
    from zeloo_cli.local_runtime.estimator import HardwareBudget, LayerKind, ModelProfile, ctx_bytes
    from zeloo_cli.local_runtime.growth import save_window_override
...
def test_growth_requires_an_admissible_materialized_preset(zeloo_home, tmp_path, monkeypatch):
    from dataclasses import replace
    from types import SimpleNamespace

    from zeloo_cli.local_runtime import bootstrap, catalog, growth, hardware, presets
    from zeloo_cli.local_runtime.context_policy import FLOOR, RUNTIME_OVERHEAD_BYTES, ub_logits_bytes
    from zeloo_cli.local_runtime.estimator import HardwareBudget, ctx_bytes
    ...
    from zeloo_cli.local_runtime import gguf, estimator
    ...
    from zeloo_cli.local_runtime.binaries import runtimes_root
...
def test_sampling_ladder_file_beats_catalog_beats_nothing(zeloo_home, tmp_path, monkeypatch):
