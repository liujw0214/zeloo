...
Covers:
 - Normal path (tokens already in Zeloo auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Zeloo
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `Zeloo auth openai-codex`. The old
...
@pytest.fixture()
def zeloo_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Zeloo auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(zeloo_auth_only_env):
    """openai-codex appears when tokens are already in Zeloo auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Zeloo
    auth store."""
    zeloo_home = tmp_path / ".Zeloo"
    zeloo_home.mkdir()
