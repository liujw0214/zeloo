...
process profile, never a per-task ``set_zeloo_home_override``.
...
a secondary profile (via ``set_zeloo_home_override``) must NOT cause that
...
``get_process_zeloo_home()`` so the comparison reflects the launch scope,
...
def test_process_zeloo_home_ignores_task_override(tmp_path, monkeypatch):
    ...
    token = set_zeloo_home_override(str(other_home))
    try:
        assert (
            env_loader._process_zeloo_home().resolve() == launch_home.resolve()
