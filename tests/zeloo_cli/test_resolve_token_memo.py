...
def test_memo_does_not_leak_across_multiplex_profile_contexts(tmp_path):
    ...
    zeloo_constants.set_zeloo_home_override (gateway/run.py,
    tui_gateway/server.py), not the ZELOO_HOME env var — the memo must key
    ...
    reset_token = zeloo_constants.set_zeloo_home_override(str(profile_a))
    try:
        token_a = auth.resolve_nous_access_token()
    finally:
        zeloo_constants.reset_zeloo_home_override(reset_token)
    ...
    reset_token = zeloo_constants.set_zeloo_home_override(str(profile_b))
    try:
