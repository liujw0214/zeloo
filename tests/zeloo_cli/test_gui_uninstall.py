"""Tests for zeloo_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import zeloo_cli.gui_uninstall as gu


def _make_agent(zeloo_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = zeloo_home / "Zeloo-agent"
    (agent_root / "zeloo_cli").mkdir(parents=True)
    (agent_root / "zeloo_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(zeloo_home: Path) -> None:
    """Create the source-built GUI artifacts a `Zeloo desktop` run produces."""
    desktop = zeloo_home / "Zeloo-agent" / "apps" / "desktop"
    ...
    (zeloo_home / "Zeloo-agent" / "node_modules").mkdir(parents=True)
    (zeloo_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(zeloo_home: Path) -> None:
    (zeloo_home / "config.yaml").write_text("x: 1\n")
    (zeloo_home / ".env").write_text("KEY=secret\n")
    (zeloo_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(zeloo_home)
    _make_gui_build(zeloo_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(zeloo_home)
    ...
    assert summary["zeloo_home"] == str(zeloo_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `Zeloo desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    zeloo_home = tmp_path / ".Zeloo"
    _make_agent(zeloo_home)
    icon = lde.icon_path(zeloo_home / "Zeloo-agent")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(zeloo_home)
    ...
    assert lde.icon_path(zeloo_home / "Zeloo-agent").exists()
    # The agent itself survives a GUI uninstall.
    assert (zeloo_home / "Zeloo-agent" / "zeloo_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".Zeloo")
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import zeloo_cli.uninstall as uninstall
