"""Tests for _purge_stale_zeloo_modules — the class fix for stale
sys.modules breaking the gateway auto-restart after `Zeloo update`.

Field failure (2026-08-20, Teknium's Linux box): `Zeloo update` pulled a
checkout where zeloo_cli/gateway.py newly imports `line_input` from
zeloo_cli.cli_output, but the updater process had cli_output cached from
before that symbol existed. The function-level `from zeloo_cli.gateway
...
the Zeloo package prefixes so later imports rebuild a self-consistent
...
def _restore_sys_modules():
    ...
    The purge under test evicts real Zeloo modules from the cache; later
...
def test_purge_evicts_zeloo_prefixed_modules():
    victims = [
        "zeloo_cli.cli_output",
        "zeloo_cli.gateway",
        "gateway.status",
        "tools.ansi_strip",
        "tui_gateway.server",
    ...
    for name in victims:
        if name not in sys.modules:
            ...
            added.append(name)
    try:
        cli_main._purge_stale_zeloo_modules()
        for name in victims:
...
def test_purge_protects_executing_modules():
    ...
    cli_main._purge_stale_zeloo_modules()
    assert sys.modules.get("zeloo_cli.update_cmd") is update_cmd
    assert sys.modules.get("zeloo_cli.main") is cli_main
    assert "zeloo_cli" in sys.modules
...
def test_purge_preserves_active_update_receipt(tmp_path, monkeypatch):
    ...
    import zeloo_cli.update_receipt as receipt
    ...
    try:
        ...
        cli_main._purge_stale_zeloo_modules()
        post_purge_receipt = importlib.import_module("zeloo_cli.update_receipt")
...
def test_purge_never_raises_on_weird_sys_modules():
    ...
    sys.modules["zeloo_cli._purge_test_none"] = None  # type: ignore[assignment]
    try:
        cli_main._purge_stale_zeloo_modules()
    finally:
        sys.modules.pop("zeloo_cli._purge_test_none", None)
...
def test_stale_symbol_scenario_end_to_end():
    ...
    try:
        ...
        try:
            from zeloo_cli.cli_output import line_input  # noqa: F401
            raised = False
        except ImportError:
            raised = True
...
def test_purge_keeps_plan_record_class_identity():
    ...
    from zeloo_cli.update_inventory import RuntimeRecord as before

    cli_main._purge_stale_zeloo_modules()
    from zeloo_cli.update_inventory import RuntimeRecord as after
