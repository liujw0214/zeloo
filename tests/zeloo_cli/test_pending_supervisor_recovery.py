...
from zeloo_cli import gateway, main, update_cmd_fleet as fleet
...
def test_pending_marker_requires_complete_systemd_recovery(monkeypatch, tmp_path, failure):
    ...
    def systemctl(cmd, **kw):
        if "list-units" in cmd:
            ...
            return SimpleNamespace(returncode=int(failure == "listing"), stdout=(
                "Zeloo-gateway-one.service loaded active running\n"
                "Zeloo-gateway-two.service loaded failed failed\n"), stderr="")
        bad = cmd[-1] == "Zeloo-gateway-two"
        if "restart" in cmd:
    ...
    if failure not in (None, "running"):
        ...
        assert set(recovered) == {"Zeloo-gateway-one", "Zeloo-gateway-two"}
...
def test_pending_launchd_requires_complete_supervision(monkeypatch, tmp_path, failure):
    ...
    current, sibling = "ai.Zeloo.gateway", "ai.Zeloo.gateway-two"
