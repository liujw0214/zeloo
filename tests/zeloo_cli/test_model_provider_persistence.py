"""Tests that provider selection via `Zeloo model` always persists correctly.
...
def config_home(tmp_path, monkeypatch):
    """Isolated ZELOO_HOME with a minimal string-format config."""
    home = tmp_path / "Zeloo"
    home.mkdir()
...
class TestProviderPersistsAfterModelSave:
    def test_update_config_for_provider_uses_atomic_yaml_write(self, config_home):
        ...
        with patch("zeloo_cli.auth.atomic_yaml_write", side_effect=_boom) as mock_write:
    ...
    def test_api_key_provider_saved_when_model_was_string(self, config_home, monkeypatch):
        ...
        with patch("zeloo_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("zeloo_cli.auth.deactivate_provider"), \
...
class TestBaseUrlValidation:
    ...
    def test_empty_base_url_keeps_default(self, config_home, monkeypatch):
        ...
        with patch("zeloo_cli.auth._prompt_model_selection", return_value="MiniMax-M2"), \
             patch("zeloo_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
...
class TestZaiEndpointPicker:
    ...
    def test_custom_proxy_rejects_invalid_url(self, config_home, monkeypatch, capsys):
        ...
        with patch("zeloo_cli.main_provider_setup._prompt_provider_choice", return_value=custom_idx), \
             patch("zeloo_cli.auth._prompt_model_selection", return_value="glm-5"), \
             patch("zeloo_cli.auth.deactivate_provider"), \
    ...
    def test_current_endpoint_is_default_choice(self, config_home, monkeypatch):
        ...
        from zeloo_cli.auth import ZAI_ENDPOINTS
        from zeloo_cli.model_setup_flows import _select_zai_endpoint
        ...
        with patch("zeloo_cli.main_provider_setup._prompt_provider_choice", side_effect=fake_choice):
