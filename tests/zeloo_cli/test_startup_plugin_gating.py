...
``zeloo_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``Zeloo --help``, ``Zeloo --version``,
``Zeloo logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
from zeloo_cli._parser import build_top_level_parser, top_level_value_flag_sets
from zeloo_cli.main import (
...
def _live_subcommand_names() -> set[str]:
    """Run ``Zeloo --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from zeloo_cli import main as _main
...
def test_reasoning_value_is_not_misclassified_as_subcommand(monkeypatch):
    monkeypatch.setattr(
        sys,
        "argv",
        ["Zeloo", "--reasoning", "high", "chat", "hello"],
    )
...
def test_deferred_platform_cli_resolution_targets_matching_platform():
    ...
    Photon registers ``Zeloo photon`` only when its adapter module is
...
def test_deferred_platform_loader_registers_cli_command_before_parser_table():
    ...
    helper calls it. This regression exercises the full chain Zeloo-sweeper
    asked for:
    ...
    def _fake_loader():
        ...
        # top-level Zeloo <name> CLI command via PluginContext.
