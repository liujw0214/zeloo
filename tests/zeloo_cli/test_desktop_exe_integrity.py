...
The desktop self-update chain (Desktop → Zeloo-setup --update →
``Zeloo update`` → ``Zeloo desktop --build-only`` → relaunch) rebuilds
Zeloo.exe on the end user's machine. Before this gate, "build succeeded" was
...
# MACHINE_ATTRIBUTES.UserEnabled — mirrors zeloo_cli.main.
...
def test_rollback_restores_backup_and_keeps_corrupt_copy(tmp_path):
    ...
    backup_exe = desktop_dir / "release" / "win-unpacked.bak" / "Zeloo.exe"
    make_pe(backup_exe, PE_AMD64)  # valid old build

    with patch("zeloo_cli.main_desktop._windows_native_machine", return_value="AMD64"):
    ...
    assert (desktop_dir / "release" / "win-unpacked.corrupt" / "Zeloo.exe").exists()
...
def test_gate_fails_clearly_without_backup(tmp_path, capsys):
    ...
    with patch("zeloo_cli.main_desktop._purge_electron_build_cache", return_value=[]), \
         patch("zeloo_cli.main_desktop._desktop_stamp_path", return_value=tmp_path / "stamp.json"):
...
# ─── end-to-end: `Zeloo desktop --build-only` exits nonzero on corrupt exe ─
...
def test_build_only_fails_when_pack_produces_corrupt_exe(tmp_path, monkeypatch, capsys):
    """The updater chain's contract: a rebuild whose Zeloo.exe cannot launch
    must exit nonzero (so Zeloo-setup's retry-once kicks in) and must leave
    ...
    live_exe = desktop_dir / "release" / "win-unpacked" / "Zeloo.exe"
    ...
    def pack_into_staging(cmd, *args, **kwargs):
        ...
        make_pe(staging / "win-unpacked" / "Zeloo.exe", PE_AMD64, truncate_to=0x300)
        return subprocess.CompletedProcess(list(cmd), 0)

    with patch("zeloo_cli.main_desktop.shutil.which", return_value="/usr/bin/npm"), \
         patch("zeloo_cli.main_install_repair._resolve_node_runtime_npm", return_value="npm.cmd"), \
         ...
         patch("zeloo_cli.main_desktop._desktop_stamp_path", return_value=tmp_path / "stamp.json"), \
         patch("zeloo_cli.main_desktop._write_desktop_build_stamp") as mock_stamp, \
         patch("zeloo_cli.main_desktop._windows_native_machine", return_value="AMD64"), \
         patch("zeloo_cli.main_desktop.subprocess.run", side_effect=pack_into_staging), \
