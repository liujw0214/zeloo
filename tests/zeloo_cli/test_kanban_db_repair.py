...
and the ``Zeloo kanban repair`` CLI verb."""
...
def test_dispatch_tick_runs_wal_checkpoint_at_interval(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ZELOO_KANBAN_DB", str(db_path))
...
# ---------------------------------------------------------------------------
# repair_db() API + `Zeloo kanban repair` CLI verb
# ---------------------------------------------------------------------------

def _run_kanban_cli(argv: list[str]) -> int:
    """Drive the real argparse surface exactly like `Zeloo kanban …`."""
    import argparse
...
def cli_home(tmp_path, monkeypatch):
    """Isolated ZELOO_HOME so kanban_db_path() resolves inside tmp_path."""
    home = tmp_path / ".Zeloo"
    home.mkdir()
