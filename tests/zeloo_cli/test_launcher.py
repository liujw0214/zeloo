"""Tests for the top-level `./Zeloo` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./Zeloo` should use `zeloo_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "Zeloo"
    called = []

    fake_main_module = types.ModuleType("zeloo_cli.main")

    def fake_main():
        called.append("zeloo_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "zeloo_cli.main", fake_main_module)
