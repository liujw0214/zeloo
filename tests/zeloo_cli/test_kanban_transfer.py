"""Tests for kanban board export / import (``zeloo_cli.kanban_transfer``).
...
from zeloo_cli import kanban_transfer as kt
from zeloo_cli.archive_safe import normalize_archive_parts, safe_extract_targz
...
def kanban_root(tmp_path, monkeypatch):
    ...
    def _use(name: str) -> Path:
        ...
        monkeypatch.setenv("ZELOO_KANBAN_HOME", str(root))
        for var in ("ZELOO_KANBAN_DB", "ZELOO_KANBAN_WORKSPACES_ROOT",
                    "ZELOO_KANBAN_ATTACHMENTS_ROOT", "ZELOO_KANBAN_BOARD"):
...
def test_import_rejects_a_future_format_version(kanban_root, tmp_path):
    ...
    kanban_root("target")
    with pytest.raises(ValueError, match="newer than this Zeloo"):
