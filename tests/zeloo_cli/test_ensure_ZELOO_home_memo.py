"""ensure_zeloo_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".Zeloo"
    second = tmp_path / "b" / ".Zeloo"

    monkeypatch.setenv("ZELOO_HOME", str(first))
    cfg.ensure_zeloo_home()

    # Profile switch: ZELOO_HOME moves → the new path is ensured too.
