...
zeloo_cli.config.load_config, so the managed overlay has to be wired into each.
...
def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # zeloo_time checks an env override first; ensure it's unset so config wins.
    ...
    assert zeloo_time._resolve_timezone_name() == "Asia/Tokyo"
...
def test_gateway_env_bridge_honors_managed(homes, monkeypatch):
    ...
    every turn (ZELOO_TIMEZONE, ZELOO_REDACT_SECRETS, ZELOO_MAX_ITERATIONS,
    ...
    # writing ZELOO_TIMEZONE = cfg["timezone"]. Prove the overlay flips the value.
    import yaml
