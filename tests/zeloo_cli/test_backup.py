"""Tests for Zeloo backup and import commands."""
...
def _no_real_gateway_service(monkeypatch):
    ...
    assert the wiring."""
    import zeloo_cli.gateway as gateway_mod
...
def _advance_backup_clock(seconds: float = 1.1) -> None:
    """Skew zeloo_cli.backup's datetime forward instead of sleeping.
    ...
    import zeloo_cli.backup as _backup

    shim = getattr(_backup.datetime, "_zeloo_test_shim", None)
    if shim is None:
        class _ShimDatetime(_dt.datetime):
            _zeloo_test_shim = True
...
def _make_zeloo_tree(root: Path) -> None:
    """Create a realistic ~/.Zeloo directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    for db_name in ("memory_store.db", "zeloo_state.db"):
    ...
    # Zeloo-agent repo (should be EXCLUDED)
    (root / "Zeloo-agent").mkdir(exist_ok=True)
    (root / "Zeloo-agent" / "run_agent.py").write_text("# big file\n")
    (root / "Zeloo-agent" / ".git").mkdir()
    (root / "Zeloo-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_zeloo_agent(self):
        from zeloo_cli.backup import _should_exclude
        assert _should_exclude(Path("Zeloo-agent/run_agent.py"))
        assert _should_exclude(Path("Zeloo-agent/.git/HEAD"))
    ...
    def test_excludes_state_snapshots_dir(self):
        ...
        from zeloo_cli.backup import _EXCLUDED_DIRS, _QUICK_SNAPSHOTS_DIR, _should_exclude
    ...
    def test_excludes_desktop_emergency_state_db_baks(self):
        ...
        state.db.pre-update-emergency-*.bak files at the ZELOO_HOME root —
...
class TestIterBackupFiles:
    def test_manual_and_automatic_paths_share_one_walk(self, tmp_path):
        ...
        ``Zeloo-agent`` at ANY depth, silently dropping nested skill dirs
        like ``skills/autonomous-ai-agents/Zeloo-agent/`` that the manual
        path preserved. One shared iterator makes that drift impossible;
        this test pins the contract."""
        from zeloo_cli.backup import _iter_backup_files

        root = tmp_path / ".Zeloo"
        root.mkdir()
        _make_zeloo_tree(root)

        # The case the old automatic walk got wrong: a nested dir named
        # Zeloo-agent holding real skill content.
        nested = root / "skills" / "autonomous-ai-agents" / "Zeloo-agent"
        ...
        rel_nested = str(Path("skills/autonomous-ai-agents/Zeloo-agent/SKILL.md"))
        assert rel_nested in selected
        assert str(Path("models/big.gguf")) not in selected
        assert not any(s.startswith("Zeloo-agent") for s in selected)
...
class TestBackup:
    ...
    def test_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        import zeloo_cli.backup as backup_mod
        staged_dirs = []
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        out_zip = zeloo_home / "backups" / "pre-update-test.zip"
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, zeloo_home)
    ...
    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside ZELOO_HOME."""
        zeloo_home = tmp_path / ".Zeloo"
        zeloo_home.mkdir()
        _make_zeloo_tree(zeloo_home)
        outside = tmp_path / "outside-secret.txt"
        outside.write_text("outside secret\n")
        _symlink_file_or_skip(zeloo_home / "skills" / "outside-link.txt", outside)
        ...
        from zeloo_cli.backup import run_backup
        run_backup(args)
    ...
    def test_state_snapshots_not_nested_into_backup(self, tmp_path, monkeypatch):
        ...
        zeloo_home.mkdir()
        _make_zeloo_tree(zeloo_home)
        with sqlite3.connect(zeloo_home / "state.db") as conn:
            conn.execute("CREATE TABLE sessions (id TEXT PRIMARY KEY)")
            conn.execute("INSERT INTO sessions VALUES ('s1')")

        from zeloo_cli.backup import _QUICK_SNAPSHOTS_DIR, create_quick_snapshot, run_backup

        # Real producer, so the layout under state-snapshots/ is whatever the
        # code actually writes (manifest.json + state.db copy + ...).
        snap_id = create_quick_snapshot(zeloo_home=zeloo_home)
        assert snap_id and (zeloo_home / _QUICK_SNAPSHOTS_DIR / snap_id / "state.db").exists()
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Zeloo backup."""
        from zeloo_cli.backup import _validate_backup_zip
...
class TestImport:
    ...
    def test_import_auto_installs_gateway_service(self, tmp_path, monkeypatch):
        ...
        from zeloo_cli.backup import run_import
    ...
    def test_import_survives_service_layer_import_failure(self, tmp_path, monkeypatch, capsys):
        ...
        assert "Done. Your Zeloo configuration has been restored." in out
        assert "Zeloo gateway install" in out
    ...
    def test_preserves_per_profile_gateway_state(self, tmp_path, monkeypatch):
        ...
        zeloo_home = tmp_path / ".Zeloo"
        (zeloo_home / "profiles" / "coder").mkdir(parents=True)
        monkeypatch.setenv("ZELOO_HOME", str(zeloo_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        live_state = '{"gateway_state": "running"}'
        (zeloo_home / "profiles" / "coder" / "gateway_state.json").write_text(live_state)
        ...
        assert (zeloo_home / "profiles" / "coder" / "config.yaml").read_text() == "model: anthropic\n"
        assert (
            zeloo_home / "profiles" / "coder" / "gateway_state.json"
    ...
    def test_preserves_runtime_pid_and_process_files(self, tmp_path, monkeypatch):
        ...
        (zeloo_home / "gateway.pid").write_text("4242")
        (zeloo_home / "processes.json").write_text('{"live": true}')
        ...
        assert (zeloo_home / "gateway.pid").read_text() == "4242"
        assert (zeloo_home / "processes.json").read_text() == '{"live": true}'
        # cron.pid / gateway.lock had no live copy and were not seeded.
        assert not (zeloo_home / "cron.pid").exists()
        assert not (zeloo_home / "gateway.lock").exists()
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (zeloo_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".Zeloo"
        src_home.mkdir(parents=True)
        _make_zeloo_tree(src_home)

        monkeypatch.setenv("ZELOO_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from zeloo_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".Zeloo"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("ZELOO_HOME", str(dst_home))
        ...
        # Zeloo-agent should NOT be present
        assert not (dst_home / "Zeloo-agent").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from zeloo_cli.sizefmt import format_bytes as _format_size
...
class TestValidation:
    ...
    def test_detect_prefix_only_dirs(self):
        ...
        import io
        from zeloo_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            # Only directory entries (trailing slash)
            zf.writestr(".Zeloo/", "")
            zf.writestr(".Zeloo/skills/", "")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
...
class TestBackupEdgeCases:
    ...
    def test_empty_zeloo_home(self, tmp_path, monkeypatch):
        """Backup handles empty Zeloo home (no files to back up)."""
        zeloo_home = tmp_path / ".Zeloo"
        zeloo_home.mkdir()
        # Only excluded dirs, no actual files
        (zeloo_home / "__pycache__").mkdir()
        (zeloo_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        zeloo_home = tmp_path / ".Zeloo"
        zeloo_home.mkdir()
        (zeloo_home / "config.yaml").write_text("model: test\n")

        # Create a file with epoch timestamp (1970-01-01)
        old_file = zeloo_home / "ancient.txt"
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        zeloo_home = tmp_path / ".Zeloo"
        zeloo_home.mkdir()
        (zeloo_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("ZELOO_HOME", str(zeloo_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (zeloo_home / "config.yaml").exists()
        assert (zeloo_home / "sessions" / "s0599.json").exists()
...
class TestImportAtomicWrites:
    """`Zeloo import` must never leave a user's file truncated.
    ...
    tests pin the invariant for both restore branches: the ZELOO_HOME branch
    ...
    def test_failed_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        ...
        (zeloo_home / "config.yaml").write_text(original)
        monkeypatch.setenv("ZELOO_HOME", str(zeloo_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (zeloo_home / "config.yaml").read_text() == original
        # And the aborted write must not litter the directory it staged in.
        assert list(zeloo_home.glob(".config.yaml.*")) == []

    def test_failed_external_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        """Same invariant on the `_external/` branch, which writes outside ZELOO_HOME."""
        dst_home = tmp_path / "dst"
        dst_home.mkdir()
        zeloo_home = dst_home / ".Zeloo"
        zeloo_home.mkdir()
    ...
    def test_restore_preserves_existing_file_owner(self, tmp_path, monkeypatch):
        ...
        ``sudo Zeloo import`` onto a user-owned (or Docker/NAS volume-owned)
        ZELOO_HOME would hand every restored file to root. The uid/gid is
        ...
        monkeypatch.setattr(
            "zeloo_cli.backup._preserve_file_owner",
    ...
    def test_restore_does_not_carry_setuid_onto_archive_content(
        ...
        re-serialize content this process produced; ``Zeloo import`` is the
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from zeloo_cli.backup import _safe_copy_db
    ...
    def test_aborts_when_source_remains_busy_past_deadline(
        ...
        from zeloo_cli import backup as backup_mod
    ...
    def test_is_zeroed_sqlite_file_detects_nul_header(self, tmp_path):
        from zeloo_cli.backup import is_zeroed_sqlite_file
        p = tmp_path / "state.db"
...
class TestQuickSnapshot:
    @pytest.fixture
    def zeloo_home(self, tmp_path):
        """Create a fake ZELOO_HOME with critical state files."""
        home = tmp_path / ".Zeloo"
        home.mkdir()
    ...
    def test_state_db_safely_copied(self, zeloo_home):
        from zeloo_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(zeloo_home=zeloo_home)
        db_copy = zeloo_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_failed_state_db_copy_is_loud(self, zeloo_home, monkeypatch, capsys):
    ...
    def test_restore_refused_db_is_not_counted(self, zeloo_home, monkeypatch):
        ...
        restore_quick_snapshot(snap_id, zeloo_home=zeloo_home)

        manifest = json.loads(
            (backup_mod._quick_snapshot_root(zeloo_home) / snap_id / "manifest.json").read_text()
        )
    ...
    def test_restore_state_db_live_connection(self, zeloo_home):
        ...
        live_conn = sqlite3.connect(str(zeloo_home / "state.db"))
        ...
        result = restore_quick_snapshot(snap_id, zeloo_home=zeloo_home)
        assert result is True
    ...
    def test_snapshot_includes_pairing_directories(self, zeloo_home):
        ...
        (zeloo_home / "platforms" / "pairing").mkdir(parents=True)
        (zeloo_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (zeloo_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (zeloo_home / "pairing").mkdir()
        (zeloo_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (zeloo_home / "feishu_comment_pairing.json").write_text(
        ...
        snap_dir = zeloo_home / "state-snapshots" / snap_id
...
# ---------------------------------------------------------------------------
# Pre-update backup (Zeloo update safety net)
# ---------------------------------------------------------------------------
    ...
    def test_oversized_db_suppresses_pruning(self, zeloo_home, capsys):
        ...
        """
        import json
        from zeloo_cli.backup import create_quick_snapshot, list_quick_snapshots

        # First snapshot: complete (state.db is small, under any cap)
        first_id = create_quick_snapshot(label="complete", zeloo_home=zeloo_home)
        assert first_id is not None
        first_dir = zeloo_home / "state-snapshots" / first_id
        ...
        second_id = create_quick_snapshot(
            label="oversized", zeloo_home=zeloo_home, max_file_size=1024, keep=1
        )
        assert second_id is not None
        second_dir = zeloo_home / "state-snapshots" / second_id
        ...
        all_snaps = list_quick_snapshots(limit=100, zeloo_home=zeloo_home)
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_non_default_kanban_board_snapshotted(self, zeloo_home):
        ...
        board_dir = zeloo_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(zeloo_home=zeloo_home)
        copy = (
            zeloo_home / "state-snapshots" / snap_id
        ...
        assert restore_quick_snapshot(snap_id, zeloo_home=zeloo_home) is True
        conn = sqlite3.connect(str(board_dir / "kanban.db"))
    ...
    def test_board_db_copied_wal_safely(self, zeloo_home, monkeypatch):
        ...
        import zeloo_cli.backup as bk
        from zeloo_cli.backup import create_quick_snapshot

        board = zeloo_home / "kanban" / "boards" / "work"
        ...
        copy = zeloo_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work" / "kanban.db"
        rows = sqlite3.connect(str(copy)).execute("SELECT * FROM tasks").fetchall()
        assert rows == [("w1", "ship")]


class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``Zeloo update``
    ...
    def test_backup_contents_match_full_backup(self, zeloo_home):
        """Pre-update backup should include the same user data that
        ``Zeloo backup`` would, and should exclude the same directories."""
        from zeloo_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(zeloo_home=zeloo_home)
        ...
        # Zeloo-agent repo excluded
        assert not any(n.startswith("Zeloo-agent/") for n in names)
        # __pycache__ excluded
        assert not any("__pycache__" in n for n in names)
        # pid files excluded
        assert "gateway.pid" not in names

    def test_pre_update_zip_does_not_nest_the_pre_update_snapshot(self, zeloo_home):
        """``Zeloo update`` in ``full`` mode takes the quick snapshot *before*
        ...
        snap_id = create_quick_snapshot(label="pre-update", zeloo_home=zeloo_home)
        assert snap_id and (zeloo_home / _QUICK_SNAPSHOTS_DIR / snap_id / "state.db").exists()
    ...
    def test_rotation_keeps_only_n(self, zeloo_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(zeloo_home=zeloo_home, keep=3)
            created.append(out)
            _advance_backup_clock()

        remaining = sorted(
            p.name for p in (zeloo_home / "backups").iterdir()
    ...
    def test_skips_symlinked_files(self, zeloo_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside ZELOO_HOME."""
        from zeloo_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def zeloo_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".Zeloo"
        root.mkdir()
        _make_zeloo_tree(root)
        # Point ZELOO_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("ZELOO_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Config reads resolve ZELOO_HOME dynamically and their caches are
    ...
    @staticmethod
    def _set_mode(zeloo_home, value):
        import yaml
        (zeloo_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": value},
        }))

    @staticmethod
    def _zips(zeloo_home):
        d = zeloo_home / "backups"
        return list(d.glob("pre-update-*.zip")) if d.exists() else []

    @staticmethod
    def _snaps(zeloo_home):
        d = zeloo_home / "state-snapshots"
        return [p for p in d.iterdir() if p.is_dir()] if d.exists() else []




    def test_config_off_disables_everything_silently(self, zeloo_home, capsys):
        ...
        self._set_mode(zeloo_home, "off")
        from zeloo_cli.update_cmd import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is None
        assert out == ""
        assert not self._snaps(zeloo_home)
        assert not self._zips(zeloo_home)



    def test_config_full_mode(self, zeloo_home, capsys):
        self._set_mode(zeloo_home, "full")
        from zeloo_cli.update_cmd import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
        ...
        assert len(self._zips(zeloo_home)) == 1




# ---------------------------------------------------------------------------
# Pre-migration backup (Zeloo claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``Zeloo claw migrate`` runs before mutating ~/.Zeloo/."""
    ...
    def test_restorable_with_zeloo_import(self, zeloo_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Zeloo
        backup — `Zeloo import` should accept it."""
        from zeloo_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(zeloo_home=zeloo_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_touch_pre_update_backups(self, zeloo_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from zeloo_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(zeloo_home=zeloo_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        for _ in range(3):
            out = create_pre_migration_backup(zeloo_home=zeloo_home, keep=1)
            assert out is not None
            _advance_backup_clock()
...
class TestRestoreCronJobsIfEmptied:
    """`Zeloo update` config migration can leave cron/jobs.json valid-but-empty,
    ...
    def _make_snapshot(self, zeloo_home: Path, label="pre-update"):
        from zeloo_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, zeloo_home=zeloo_home, keep=5)

    def test_restores_when_emptied_after_migration(self, tmp_path):
        from zeloo_cli.backup import restore_cron_jobs_if_emptied
        zeloo_home = tmp_path / ".Zeloo"
        jobs_path = zeloo_home / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(zeloo_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, zeloo_home=zeloo_home)
        assert result is not None
...
class TestRestoreConfigModelSettingsIfRewritten:
    ...
    def _seed(self, zeloo_home: Path) -> Path:
        zeloo_home.mkdir(parents=True, exist_ok=True)
        cfg = zeloo_home / "config.yaml"
        cfg.write_text(self.USER_CONFIG, encoding="utf-8")
        return cfg

    def test_restores_rewritten_provider_and_dropped_moa(self, tmp_path):
        import yaml
        from zeloo_cli.backup import restore_config_model_settings_if_rewritten

        zeloo_home = tmp_path / ".Zeloo"
        cfg = self._seed(zeloo_home)
        snap_id = self._make_snapshot(zeloo_home)
        assert snap_id
...
# location, NOT under ZELOO_HOME. (backup/import cycle data-loss fix)
# ---------------------------------------------------------------------------

class TestMemoryProviderExternalPaths:
    def _make_min_tree(self, zeloo_home: Path) -> None:
        zeloo_home.mkdir(parents=True, exist_ok=True)
        (zeloo_home / "config.yaml").write_text("model:\n  provider: openrouter\n")
        (zeloo_home / ".env").write_text("OPENROUTER_API_KEY=sk-test\n")
        (zeloo_home / "state.db").write_bytes(b"x")
    ...
    def test_backup_skips_external_paths_outside_home(self, tmp_path, monkeypatch):
        ...
        zeloo_home = tmp_path / ".Zeloo"
        self._make_min_tree(zeloo_home)
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        """_external/ members restore to ~/<relpath>, not under ZELOO_HOME,
        ...
        # External state did NOT leak into ZELOO_HOME.
        assert not (zeloo_home / "_external").exists()


# ---------------------------------------------------------------------------
# run_import: ZELOO_HOME override handling (issue #99839)
# ---------------------------------------------------------------------------


class TestImportHonorsZELOOHomeOverride:
    """`Zeloo import` must restore into the home the command runs under.

    Resolving the target through get_default_zeloo_root() maps a profile
    ...
    def test_import_targets_named_profile_home(self, tmp_path, monkeypatch):
        """ZELOO_HOME=<root>/profiles/<name> must restore INTO the profile,
        not into <root> (which would clobber the live root config)."""
        root = tmp_path / "Zeloo-root"
        profile = root / "profiles" / "coder"
        profile.mkdir(parents=True)
        # Live root config that must survive untouched.
        (root / "config.yaml").write_text("model:\n  provider: openai\n")

        monkeypatch.setenv("ZELOO_HOME", str(profile))
        from zeloo_constants import get_zeloo_home

        assert get_zeloo_home() == profile
    ...
    def test_import_skips_gateway_install_for_non_default_home(
        ...
        monkeypatch.setenv("ZELOO_HOME", str(sandbox))
        ...
        monkeypatch.setattr(
            backup_mod,
            "_get_platform_default_zeloo_home",
            lambda: native_default,
        )

        calls = []
        monkeypatch.setattr(
            "zeloo_cli.gateway.ensure_gateway_service",
            lambda *a, **kw: calls.append(kw),
        )
        monkeypatch.setattr(
            "zeloo_cli.gateway._is_service_running",
            lambda: False,
        )
    ...
    def test_import_installs_gateway_when_default_home_is_target(
        ...
        native_default = tmp_path / "native-default"
        native_default.mkdir()
        monkeypatch.setenv("ZELOO_HOME", str(native_default))
...
def _write_session_db(path: Path, sessions: int, messages_per_session: int) -> None:
    """Create a minimal Zeloo-shaped session database at *path*."""
...
class TestImportLiveSessionDatabase:
    """`Zeloo import` must not swap the inode of a database Zeloo holds open.
...
def test_run_backup_prunes_older_default_named_zips_but_not_others(tmp_path, monkeypatch):
    """Hourly `Zeloo backup` callers accumulated 150+ zips; --keep bounds the default-named
    ...
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    for i in range(4):
        (tmp_path / f"Zeloo-backup-2026-01-0{i + 1}-000000.zip").write_bytes(b"old")
    (tmp_path / "my-archive.zip").write_bytes(b"mine")

    backup_mod.run_backup(Namespace(output=None, keep=2))

    kept = sorted(p.name for p in tmp_path.glob("Zeloo-backup-*.zip"))
    assert len(kept) == 2 and kept[0] == "Zeloo-backup-2026-01-04-000000.zip"
