"""Tests for zeloo_cli.managed_uv — one path, no guessing."""
...
def _runtime_info(
    ...
    from zeloo_cli.sqlite_runtime import SQLiteRuntimeInfo
...
def _RRR(status):
    ...
    """
    from zeloo_cli.managed_uv import RuntimeRepairResult
...
class TestManagedUvPath:
    ...
    def test_posix(self, tmp_path):
        with patch("zeloo_cli.managed_uv.get_zeloo_home", return_value=tmp_path):
            from zeloo_cli.managed_uv import managed_uv_path
...
class TestMacOSManagedPythonSigning:
    def test_signs_with_stable_identifier_and_verifies(self, tmp_path, monkeypatch):
        import zeloo_cli.managed_uv as managed_uv
        ...
        assert calls[0][0] == [
            ...
            "--timestamp=none",
            "--identifier",
            "com.nousresearch.Zeloo.managed-python",
            "--requirements",
            '=designated => identifier "com.nousresearch.Zeloo.managed-python"',
            str(python),
        ]
        assert calls[1][0] == [
            "/usr/bin/codesign",
...
class TestResolveUv:
    ...
    def test_existing_executable(self, tmp_path):
        ...
        _make_executable(uv)
        with patch("zeloo_cli.managed_uv.get_zeloo_home", return_value=tmp_path):
            from zeloo_cli.managed_uv import resolve_uv
            result = resolve_uv()
...
class TestEnsureUv:
    ...
    def test_installs_if_missing(self, tmp_path):
        uv = tmp_path / "bin" / _UV_BINARY_NAME
        with patch("zeloo_cli.managed_uv.get_zeloo_home", return_value=tmp_path), \
             patch("zeloo_cli.managed_uv.repair_vulnerable_runtime", return_value=_RRR("not-applicable")), \
             patch("zeloo_cli.managed_uv._uv_version", return_value="uv 0.1.2"), \
             patch("zeloo_cli.managed_uv._install_uv") as mock_install:
    ...
    def test_install_reports_runtime_repair_to_observer(self, tmp_path):
        from zeloo_cli.managed_uv import (
            RuntimeRepairResult,
            ensure_uv,
        )
        ...
        observed = []
        with patch(
            "zeloo_cli.managed_uv.get_zeloo_home",
            return_value=tmp_path,
        ), patch(
            "zeloo_cli.managed_uv._install_uv",
            side_effect=fake_install,
        ), patch(
            "zeloo_cli.managed_uv._uv_version",
            return_value="uv 0.1.2",
        ), patch(
            "zeloo_cli.managed_uv.repair_vulnerable_runtime",
            return_value=repair,
        ):
...
class TestEnsureUvUpdateBoundary:
    ...
    ``Zeloo update`` runs the call site from the old, already-imported
    ``zeloo_cli.main`` against the freshly pulled ``managed_uv``. A release
    ...
    def test_failure_unpacks_without_raising(self, tmp_path):
        with patch("zeloo_cli.managed_uv.get_zeloo_home", return_value=tmp_path), \
             patch("zeloo_cli.managed_uv.repair_vulnerable_runtime", return_value=_RRR("not-applicable")), \
             patch("zeloo_cli.managed_uv._install_uv", side_effect=RuntimeError("network down")):
            from zeloo_cli.managed_uv import ensure_uv
...
class TestEnsureUvWindowsSafe:
    ...
    def test_uvresult_would_break_windows_list2cmdline(self):
        ...
        import subprocess
        from zeloo_cli.managed_uv import _UvResult
        with pytest.raises(TypeError):
            subprocess.list2cmdline([_UvResult("C:\\Zeloo\\uv.exe"), "pip"])
    ...
    def test_windows_returns_plain_str_safe_for_subprocess(self, tmp_path):
        ...
        with patch("zeloo_cli.managed_uv.get_zeloo_home", return_value=tmp_path), \
             patch("zeloo_cli.managed_uv.repair_vulnerable_runtime", return_value=_RRR("not-applicable")):
            from zeloo_cli.managed_uv import _UvResult, ensure_uv
            uv_bin = ensure_uv()
...
class TestUpdateManagedUv:
    ...
    def test_fresh_stamp_skips_network_self_update_but_not_repair(self, tmp_path):
        ...
        from zeloo_cli.managed_uv import RuntimeRepairResult, update_managed_uv

        uv = tmp_path / "bin" / _UV_BINARY_NAME
        _make_executable(uv)
        # The stamp reader imports get_zeloo_home separately from the binary
        ...
        with patch("zeloo_cli.managed_uv.get_zeloo_home", return_value=tmp_path), \
             patch("zeloo_cli.managed_uv._uv_self_update_stamp", return_value=stamp), \
             patch("zeloo_cli.managed_uv.repair_vulnerable_runtime",
                   return_value=RuntimeRepairResult("skipped")) as mock_repair, \
             patch("zeloo_cli.managed_uv.subprocess.run") as mock_run:
    ...
    def test_stale_stamp_runs_self_update_and_refreshes_stamp(self, tmp_path):
        ...
        from zeloo_cli.managed_uv import UV_SELF_UPDATE_INTERVAL_SECONDS, update_managed_uv
        ...
        with patch("zeloo_cli.managed_uv.get_zeloo_home", return_value=tmp_path), \
             patch("zeloo_cli.managed_uv._uv_self_update_stamp", return_value=stamp), \
             patch("zeloo_cli.managed_uv.repair_vulnerable_runtime", return_value=_RRR("not-applicable")), \
             patch("zeloo_cli.managed_uv._uv_version", return_value="uv 0.2.0"), \
             patch("zeloo_cli.managed_uv.subprocess.run") as mock_run:
...
class TestManagedPythonStore:
    def test_store_is_checkout_scoped_across_profiles(self, tmp_path, monkeypatch):
        from zeloo_cli.managed_uv import managed_python_install_dir

        checkout = tmp_path / "checkout"
        monkeypatch.setenv("ZELOO_HOME", str(tmp_path / "profiles" / "alpha"))
        alpha = managed_python_install_dir(checkout)
        monkeypatch.setenv("ZELOO_HOME", str(tmp_path / "profiles" / "beta"))
        beta = managed_python_install_dir(checkout)

        expected = checkout / ".Zeloo-runtime" / "python"
...
class TestRuntimeRepair:
    def test_safe_runtime_is_a_noop(self, tmp_path):
        from zeloo_cli.managed_uv import repair_vulnerable_runtime

        root, live, sentinel = _make_runtime_install(tmp_path)
        current = _runtime_info(live / "bin" / "python", (3, 53, 1))
        with patch(
                 "zeloo_cli.managed_uv.probe_sqlite_runtime",
                 return_value=current,
             ), \
             patch(
                 "zeloo_cli.managed_uv._install_safe_python_generation"
             ) as mock_install:
        ...
        assert not (root / ".Zeloo-runtime").exists()
    ...
    def test_failed_candidate_preserves_live_venv(self, tmp_path):
        ...
        generation = root / ".Zeloo-runtime" / "python" / "generation-test"
        ...
        with patch(
             ...
             patch(
                 "zeloo_cli.managed_uv._stage_candidate_venv",
                 return_value=None,
             ):
            result = repair_vulnerable_runtime("uv", project_root=root)
        ...
        reacquired = _acquire_repair_lock(root / ".Zeloo-runtime")
    ...
    def test_successful_repair_removes_parked_backup(self, tmp_path):
        ...
        candidate_python.write_text("candidate interpreter", encoding="utf-8")
        fixed = _runtime_info(candidate_python, (3, 53, 1))
        candidate_venv = root / ".Zeloo-runtime" / "venv-candidate"
        ...
        with patch(
             ...
             patch(
                 "zeloo_cli.managed_uv._smoke_candidate_venv",
...
class TestStageCandidateVenvCrossPlatform:
    ...
    def test_sync_keeps_uv_project_config_and_merges_stderr(self, tmp_path):
        ...
        from zeloo_cli.managed_uv import _stage_candidate_venv

        root = tmp_path / "checkout"
        root.mkdir()
        (root / "uv.lock").write_text("# lock\n", encoding="utf-8")
        generation = root / ".Zeloo-runtime" / "python" / "gen"
        ...
        with patch("zeloo_cli.managed_uv.subprocess.run", side_effect=fake_run), \
             patch(
                 "zeloo_cli.managed_uv._smoke_candidate_venv",
...
class TestRuntimeCutover:
    def test_os_lock_blocks_concurrent_repair_and_releases(self, tmp_path):
        from zeloo_cli.managed_uv import _acquire_repair_lock, _release_repair_lock

        runtime_root = tmp_path / ".Zeloo-runtime"
    ...
    def test_post_swap_smoke_failure_rolls_back_live_venv(self, tmp_path):
        from zeloo_cli.managed_uv import _cut_over_candidate

        root, live, sentinel = _make_runtime_install(tmp_path)
        runtime_root = root / ".Zeloo-runtime"
...
class TestInstallUvInternals:
    def test_installer_uses_host_branch_and_managed_directory(self, tmp_path):
        ...
        with patch("zeloo_cli.managed_uv._install_uv_posix") as mock_posix, \
             patch("zeloo_cli.managed_uv._install_uv_windows") as mock_windows:
...
class TestRuntimeRequestMinorLine:
    ...
    def test_requests_minor_line(self):
        from zeloo_cli.managed_uv import _runtime_request
...
class TestMinorLineFallForward:
    ...
    `Zeloo update` with no path to a fixed runtime.
    """
...
class TestRefreshManagedUvCatalog:
    ...
    def test_version_change_reports_true(self, tmp_path):
        ...
        with patch("zeloo_cli.managed_uv.get_zeloo_home", return_value=tmp_path), \
             patch("zeloo_cli.managed_uv._install_uv"), \
             patch(
                 "zeloo_cli.managed_uv._uv_version_string",
...
class TestRepairRetriesAfterUvRefresh:
    def _run_repair(self, tmp_path, *, refresh_result, second_attempt):
        ...
        with patch(
             ...
             patch(
                 "zeloo_cli.managed_uv._refresh_managed_uv_catalog",
    ...
    def test_retry_success_proceeds_to_staging(self, tmp_path):
        def second_attempt(root):
            generation = root / ".Zeloo-runtime" / "python" / "generation-retry"
            candidate_python = generation / "bin" / "python"
            candidate_python.parent.mkdir(parents=True)
...
class TestDefaultLiveVenv:
    ...
    'not-applicable' on every Zeloo update and stayed on journal_mode=DELETE
    ...
    def test_dot_venv_only_is_targeted(self, tmp_path):
        from zeloo_cli.managed_uv import _default_live_venv
...
class TestVenvPythonUpdateBoundary:
    """``_venv_python`` must survive a zeloo_constants predating its symbol.

    ``Zeloo update`` imports zeloo_constants from the OLD checkout, ``git
    pull`` replaces that file, and the freshly-pulled managed_uv then runs its
    lazy ``from zeloo_constants import venv_python_path`` against the module
    ...
    plainly contains it, which is what made the error so confusing:
        ...
        cannot import name 'venv_python_path' from 'zeloo_constants'
        (~/.Zeloo/Zeloo-agent/zeloo_constants.py)
    ...
    def test_recovers_when_the_cached_module_predates_the_symbol(self, monkeypatch):
        ...
        from zeloo_cli.managed_uv import _venv_python
        ...
        monkeypatch.delattr(zeloo_constants, "venv_python_path", raising=False)
        ...
        expected = Path("/opt/Zeloo/venv/Scripts/python.exe") \
            if sys.platform == "win32" else Path("/opt/Zeloo/venv/bin/python")
        assert _venv_python(Path("/opt/Zeloo/venv")) == expected

    def test_recovery_uses_the_shared_helper_not_a_second_copy(self, monkeypatch):
        """The reload must resolve through zeloo_constants, not open-code it.

        Hand-rolling `Scripts`/`bin` here is what #76105 deduped away and what
        `test_no_open_coded_venv_layout_remains_in_zeloo_cli` bans.
        """
        import zeloo_constants
        ...
        assert _venv_python(Path("/opt/Zeloo/venv")) == sentinel
...
class TestWindowsRuntimeSelfLock:
    ...
    A CLI ``Zeloo update`` runs from the venv's own python, and
    ...
    def test_self_lock_defers_repair_before_provisioning(
        ...
        from zeloo_cli import managed_uv
        from zeloo_cli.managed_uv import repair_vulnerable_runtime
        ...
        with patch(
                 "zeloo_cli.managed_uv._windows_runtime_holders",
                 return_value=(False, ""),
             ), \
             patch(
                 "zeloo_cli.managed_uv.probe_sqlite_runtime",
    ...
    def test_venv_launcher_ancestor_is_a_self_lock(self, tmp_path, monkeypatch):
        r"""The venv\Scripts\Zeloo.exe shim stays mapped while it waits for
