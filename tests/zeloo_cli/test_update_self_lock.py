...
ancestors by design — a CLI ``Zeloo update`` IS the venv python.  Before
...
Windows ``Zeloo update`` exited 2 before even fetching and the update
...
def test_recovered_update_retry_builds_parser_without_native_secret_modules(
    ...
    monkeypatch.setattr(sys, "argv", ["Zeloo", "update", "--yes"])
    sys.modules.pop("cryptography.hazmat.bindings._rust", None)
    ...
    def test_import_main_does_not_load_self_locking_modules(self):
        result = self._run(
            textwrap.dedent(
                ...
                from zeloo_cli.update_cmd import _SELF_LOCKING_NATIVE_MODULES
                loaded = [
    ...
    def test_update_dispatch_does_not_load_cryptography(self):
        result = self._run(
            textwrap.dedent(
                ...
                with patch("zeloo_cli.update_cmd._cmd_update_check", lambda *a, **k: 0):
                    try:
                        m.main()
                    except SystemExit:
                        pass
