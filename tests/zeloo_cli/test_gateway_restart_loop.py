...
Covers:
- Defense 1: gateway stop/restart refuse when _ZELOO_GATEWAY=1
...
from zeloo_cli.cron import cron_command
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "Zeloo gateway stop",
        "Zeloo gateway uninstall",
        "Zeloo  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "Zeloo GATEWAY RESTART",           # uppercase
    ])
    def test_zeloo_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.Zeloo.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.Zeloo/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l Zeloo-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.Zeloo.gateway.restart-once.plist",
        "launchctl bootout gui/501/ai.Zeloo.gateway",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.Zeloo.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.Zeloo/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.Zeloo.gateway-apollo:/a.plist' "
            "'ai.Zeloo.gateway-cronus:/c.plist' 'ai.Zeloo.gateway-plutus:/p.plist' "
            "'ai.Zeloo.gateway:/Users/botuser/Library/LaunchAgents/ai.Zeloo.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated Zeloo note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.Zeloo.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.Zeloo.gateway',
        "launchctl kick'start' -k gui/501/ai.Zeloo.gateway",
        "launchctl kick\\start -k gui/501/ai.Zeloo.gateway",
        'launchctl "kickstart" -k gui/501/ai.Zeloo.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.Zeloo.gateway',
        "launchctl dis\\able gui/501/ai.Zeloo.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.Zeloo."gateway"',
        # Same class on the systemctl and Zeloo-CLI branches.
        'systemctl re"start" Zeloo-gateway',
        'Zeloo gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.Zeloo.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.Zeloo.update-checker',
        'systemctl re"start" Zeloo-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "Zeloo cron list",
        "Zeloo update",
        "Zeloo config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `Zeloo gateway start` is benign — starting a gateway from inside a
        ...
        "Zeloo gateway start",
        "Zeloo gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway Zeloo
        # services must not be falsely blocked (the old `.*Zeloo` matched any
        # Zeloo token).
        "launchctl unload ai.Zeloo.update-checker.plist",
        "launchctl restart ai.Zeloo.daemon",
        ...
        "systemctl restart Zeloo-meta.service",
        "systemctl restart Zeloo-cron-helper",
        ...
        "echo after the Zeloo gateway restarted cleanly",
        "the Zeloo gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "Zeloo skill view gateway-notes && echo Zeloo gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `Zeloo` is a path component
        # there, not a command.
        "cat '/docs/Zeloo gateway restart-notes.md'",
        "less /home/user/notes/Zeloo gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "Zeloo gateway restart",
        "Zeloo gateway restart; echo done",
        "Zeloo gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;Zeloo gateway restart",
        "true && Zeloo gateway stop",
        "echo $(Zeloo gateway restart)",
        "echo `Zeloo gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.Zeloo.gateway"])',
        'subprocess.run(["Zeloo", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.Zeloo.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["Zeloo", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: Zeloo gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `Zeloo -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry ZELOO_HOME/ZELOO_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("ZELOO_PROFILE", "zeus")
        monkeypatch.delenv("ZELOO_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "Zeloo -p zeus gateway stop",
        "Zeloo -p zeus gateway restart",
        "Zeloo --profile zeus gateway restart",
        "Zeloo --profile zeus gateway stop",
        "Zeloo --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "Zeloo -v -p zeus gateway restart",
        "Zeloo -p zeus -v gateway restart",
        "Zeloo --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "Zeloo -p venus gateway stop",
        "Zeloo -p venus gateway restart",
        "Zeloo --profile venus gateway restart",
        "Zeloo --profile=venus gateway stop",
        "Zeloo -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "Zeloo -p zeus gateway start",
        "Zeloo -p zeus gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("Zeloo gateway restart")
        assert _contains_gateway_lifecycle_command("Zeloo gateway stop")

    def test_zeloo_home_derived_profile(self, monkeypatch):
        # Without ZELOO_PROFILE the guard falls back to the ZELOO_HOME-
        # derived profile identity (get_active_profile_name) — the signal the
        # gateway process itself carries.
        monkeypatch.delenv("ZELOO_PROFILE", raising=False)
        monkeypatch.delenv("ZELOO_PROFILE_NAME", raising=False)
        import zeloo_cli.profiles as profiles_mod

        monkeypatch.setattr(profiles_mod, "get_active_profile_name", lambda: "zeus")
        assert _contains_gateway_lifecycle_command("Zeloo -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("Zeloo -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("Zeloo -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("Zeloo gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_zeloo_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade Zeloo then run Zeloo gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_zeloo_gateway_once.sh). The script must live under
        # ZELOO_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("ZELOO_HOME", str(tmp_path / ".Zeloo"))
        scripts_dir = tmp_path / ".Zeloo" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_ZELOO_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _ZELOO_GATEWAY=1.

    Issue #37453: systemctl --user restart Zeloo-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart Zeloo-gateway",
        "systemctl --user restart Zeloo-gateway",
        "systemctl stop Zeloo-gateway.service",
        "Zeloo gateway restart",
        "Zeloo gateway uninstall",
        "launchctl kickstart gui/501/ai.Zeloo.gateway",
        "launchctl bootout gui/501/ai.Zeloo.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.Zeloo.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.Zeloo/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.Zeloo.gateway.restart-once.plist",
        "pkill -f Zeloo.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart Zeloo-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.Zeloo.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-Zeloo label: label-independent detection is the point
        # (#62891 second reproduction used `ai.Zeloo.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.Zeloo.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _ZELOO_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _ZELOO_GATEWAY=1 is in the
        ...
        self._patch_env(monkeypatch, _FakeEnv(), inside_gateway=False)
        monkeypatch.setenv("_ZELOO_GATEWAY", "1")
        monkeypatch.setattr(
            tt, "_check_all_guards", lambda cmd, env, **kwargs: {"approved": True}
        )

        result = json.loads(tt.terminal_tool(command="Zeloo gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["Zeloo gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.Zeloo.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.Zeloo.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-Zeloo systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run Zeloo gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("Zeloo gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfeZELOO gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under ZELOO_HOME/scripts (the
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("Zeloo gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/Zeloo\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 Zeloo gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_zeloo_home()` falls back to Path.home(), which raises
        RuntimeError when neither ZELOO_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart Zeloo-gateway' .",
        "grep -rn 'Zeloo gateway restart' ./logs",
        "rg 'Zeloo gateway restart' ../archive",
        "grep -c 'systemctl stop Zeloo-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM zeloo_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell Zeloo gateway restart"',
        'sqlite3 ./db ".system systemctl restart Zeloo-gateway"',
        'psql ./x -c "\\! systemctl restart Zeloo-gateway"',
        "grep -r 'Zeloo gateway restart' . | sh",
        "grep -r 'Zeloo gateway restart' ./logs | bash",
        "grep -r 'Zeloo gateway restart' . | sudo sh",
        "grep -r 'x' . ; Zeloo gateway restart",
        "grep -r 'x' . && systemctl restart Zeloo-gateway",
        'grep -r "$(Zeloo gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart Zeloo-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell Zeloo gateway restart"',
        'sqlite3 db ".system systemctl restart Zeloo-gateway"',
        'psql -c "\\! systemctl restart Zeloo-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    zeloo_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run Zeloo gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ZELOO_HOME", str(tmp_path / ".Zeloo"))
        (tmp_path / ".Zeloo").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run Zeloo gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop Zeloo-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart Zeloo-gateway' /var/log/syslog",
        "rg 'Zeloo gateway restart' /home/user/.Zeloo/logs/",
        "journalctl -u Zeloo-gateway --grep 'systemctl restart Zeloo-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'zeloo_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell Zeloo gateway restart"',
        'psql -c "\\! systemctl restart Zeloo-gateway"',
        "grep 'systemctl restart Zeloo-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart Zeloo-gateway",
        'grep "$(systemctl restart Zeloo-gateway)" f',
        "grep 'restart' log; systemctl restart Zeloo-gateway",
        'sqlite3 db "SELECT 1"; Zeloo gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "Zeloo gateway restart",
        "sudo systemctl stop Zeloo-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart Zeloo-gateway%'\""
        )
