"""Test the platform-branched PTY bridge import in zeloo_cli.web_server_chat.
...
def test_web_server_uses_posix_pty_bridge_on_posix():
    ...
    from zeloo_cli.pty_bridge import PtyBridge as PosixBridge
    from zeloo_cli.pty_bridge import PtyUnavailableError as PosixErr
...
def test_pty_bridge_import_block_is_platform_branched():
    ...
    from zeloo_cli import web_server_chat
    ...
    #       try:
    #           from zeloo_cli.win_pty_bridge import WinPtyBridge as PtyBridge, ...
    #       except ImportError:
    #           PtyBridge = None
    #           ...
    #   else:
    ...
    assert "from zeloo_cli.win_pty_bridge import" in src
    assert "from zeloo_cli.pty_bridge import" in src
