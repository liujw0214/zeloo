...
def zeloo_home(tmp_path, monkeypatch):
    ...
    # /etc/Zeloo on the dev/CI box can't influence the test.
    monkeypatch.setenv("ZELOO_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(zeloo_home, monkeypatch):
...
def test_env_expansion_in_user_config(zeloo_home, monkeypatch):
    from zeloo_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_zeloo_dotenv(zeloo_home=str(home))
