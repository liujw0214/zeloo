...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "zeloo_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # ZELOO_HOME.  For a standard install (ZELOO_HOME unset) that root is
        # the platform-native default (~/.Zeloo), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from zeloo_constants import get_default_zeloo_root
        assert env.get("ZELOO_HOME") == str(get_default_zeloo_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (ZELOO_DESKTOP=1) must NOT
