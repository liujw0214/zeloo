"""Tests for the zeloo_cli models module."""
...
import zeloo_cli.models as _models_mod
from zeloo_cli import models_local
from zeloo_cli import models_validate
...
class TestModelIds:
    def test_returns_non_empty_list(self):
        with patch("zeloo_cli.models.fetch_openrouter_models", return_value=LIVE_OPENROUTER_MODELS):
            ids = model_ids()
...
class TestFetchOpenRouterModels:
    ...
    def test_falls_back_to_static_snapshot_on_fetch_failure(self, monkeypatch):
        ...
        with patch("zeloo_cli.model_catalog.get_curated_openrouter_models", return_value=None), \
             patch("zeloo_cli.models._urlopen_model_catalog_request", side_effect=OSError("boom")):
    ...
    def test_filters_out_models_without_tool_support(self, monkeypatch):
        ...
        Zeloo-agent is tool-calling-first — surfacing a non-tool model leads to
        ...
        monkeypatch.setattr(_models_mod, "_openrouter_catalog_cache", None)
        with (
            patch("zeloo_cli.model_catalog.get_curated_openrouter_models", return_value=[]),
            patch("zeloo_cli.models._urlopen_model_catalog_request", return_value=_Resp()),
        ):
            models = fetch_openrouter_models(force_refresh=True)
...
class TestOpenRouterToolSupportHelper:
    ...
    def test_tools_in_supported_parameters(self):
        from zeloo_cli.models import _openrouter_model_supports_tools
...
class TestFindOpenrouterSlug:
    def test_exact_match(self):
        from zeloo_cli.models import _find_openrouter_slug
        with patch("zeloo_cli.models.fetch_openrouter_models", return_value=LIVE_OPENROUTER_MODELS):
...
class TestDetectProviderForModel:
    ...
    def test_short_alias_resolves_to_static_model(self):
        ...
        ), patch("zeloo_cli.models_detect.provider_has_credentials", return_value=True):
...
class TestUnionWithPortalFreeRecommendations:
    ...
    def test_adds_portal_free_model_missing_from_curated(self):
        ...
        with patch(
            "zeloo_cli.models.fetch_nous_recommended_models",
...
class TestCheckNousFreeTierCache:
    ...
    @patch("zeloo_cli.nous_account.get_nous_portal_account_info")
    ...
    def test_entitlement_cache_is_profile_scoped(self, mock_account, tmp_path):
        ...
        def account_for_active_profile(*, force_fresh=False):
            is_free = zeloo_home_key() == zeloo_home_key(tmp_path / "free")
...
class TestNousRecommendedModels:
    ...
    def test_fetch_caches_per_portal_url(self):
        from zeloo_cli.models import fetch_nous_recommended_models
        mock_cm = self._mock_urlopen(self._SAMPLE_PAYLOAD)
        with patch("zeloo_cli.models._urlopen_model_catalog_request", return_value=mock_cm) as mock_urlopen:
    ...
    def test_paid_tier_prefers_paid_recommendation(self):
        ...
        with patch("zeloo_cli.models.fetch_nous_recommended_models", return_value=payload):
    ...
    def test_tier_detection_error_defaults_to_paid(self):
        ...
        with (
            patch("zeloo_cli.models.fetch_nous_recommended_models", return_value=payload),
            patch("zeloo_cli.models.check_nous_free_tier", side_effect=RuntimeError("boom")),
        ):
...
class TestCodexSoftAcceptPlausibilityGate:
    ...
    def test_unrelated_name_rejected_on_openai_codex(self):
        from zeloo_cli.models_validate import validate_requested_model
...
class TestFormatPricePerMtok:
    ...
    def test_standard_prices_keep_two_decimals(self):
        from zeloo_cli.models_pricing import _format_price_per_mtok
...
class TestLocalOllamaModelDiscovery:
    ...
    def test_native_tags_cache_expires(self, monkeypatch):
        from zeloo_cli.models_local import fetch_ollama_local_models
        ...
        try:
            ...
            monkeypatch.setattr("zeloo_cli.models.time.monotonic", lambda: 301.0)
    ...
    def test_runtime_error_from_config_load_does_not_escape_ollama_helpers(self):
        ...
        from zeloo_cli.models import _get_ollama_base_url
        from zeloo_cli.models_local import should_use_ollama_native_catalog

        with patch("zeloo_cli.config.load_config", side_effect=RuntimeError("bad home")), patch(
            "zeloo_cli.models_local.probe_ollama_local_models",
            return_value=None,
        ):
    ...
    def test_probe_ollama_models_malformed_base_url_returns_none(self):
        ...
        from zeloo_cli.models_local import probe_ollama_local_models
    ...
    def test_fetch_ollama_models_preserves_probe_failure(self):
        ...
        with patch("zeloo_cli.models_local.probe_ollama_local_models", return_value=None):
    ...
    def test_ollama_port_detection_requires_working_api_tags(self):
        ...
        with patch("zeloo_cli.models_local.probe_ollama_local_models", return_value=["qwen3:1.7b"]):
    ...
    def test_provider_model_ids_ollama_cloud_config_uses_generic_catalog(self):
        ...
        ), patch("zeloo_cli.models_local.fetch_ollama_local_models") as fetch_local, patch(
            "zeloo_cli.models.fetch_api_models",
    ...
    def test_native_ollama_catalog_uses_configured_key_env(self, monkeypatch):
        from zeloo_cli.models_local import _get_ollama_request_headers
    ...
    def test_provider_model_ids_ignores_active_non_ollama_custom_endpoint(self):
        ...
        with patch(
            ...
            "zeloo_cli.models_local.fetch_ollama_local_models",
            return_value=["qwen3:1.7b"],
        ) as fetch_local:
            assert provider_model_ids("ollama", force_refresh=True) == ["qwen3:1.7b"]
    ...
    def test_ollama_cache_fingerprint_does_not_probe_custom_endpoint(self):
        from zeloo_cli.models import _credential_fingerprint
        ...
        ), patch("zeloo_cli.models_local.probe_ollama_local_models") as probe_ollama:
    ...
    def test_clear_provider_models_cache_clears_ollama_native_tags_cache(self):
        import zeloo_cli.models as models
        from zeloo_cli import models_local
        from zeloo_cli import models_validate
    ...
    def test_picker_user_provider_verifies_ambiguous_ollama_port(self):
        ...
        with patch("zeloo_cli.config.load_config", return_value={"providers": {}}), patch(
            "zeloo_cli.models_local.should_use_ollama_native_catalog",
            return_value=True,
        ), patch(
            "zeloo_cli.models_local.fetch_ollama_local_models",
            return_value=["qwen3:1.7b"],
        ), patch("zeloo_cli.models.fetch_api_models", return_value=[]) as fetch_api:
            rows = list_authenticated_providers(
    ...
    def test_named_custom_model_flow_discovers_ollama_api_tags(self):
        ...
        from zeloo_cli.model_setup_flows import _model_flow_named_custom
        ...
        try:
            with patch("zeloo_cli.config.load_config", return_value=config), patch(
                "zeloo_cli.config.save_config"
            ), patch("zeloo_cli.auth._save_model_choice"), patch(
                "zeloo_cli.auth.deactivate_provider"
            ), patch("zeloo_cli.main_provider_setup._save_custom_provider"), patch(
                "zeloo_cli.curses_ui.curses_radiolist",
    ...
    def test_model_validation_ollama_cloud_config_does_not_use_local_tags(self):
        ...
        ), patch("zeloo_cli.models_local.probe_ollama_local_models") as probe_ollama, patch(
            "zeloo_cli.models.probe_api_models",
            return_value={
                "models": ["qwen3:1.7b"],
    ...
    def test_switch_model_on_current_ollama_custom_endpoint_keeps_base_url(self):
        ...
        from zeloo_cli.model_switch import switch_model
        ...
        try:
            with patch(
                ...
                "zeloo_cli.model_switch.get_model_info",
                return_value=None,
            ):
                result = switch_model(
    ...
    def test_switch_model_on_non_ollama_custom_endpoint_still_resolves_runtime(self):
        ...
        with patch(
            ...
            "zeloo_cli.models_validate.validate_requested_model",
            return_value={
                "accepted": True,
                "persist": True,
                "recognized": True,
    ...
    def test_ollama_root_matching_is_case_insensitive_for_hostnames(self):
        from zeloo_cli.models_local import _same_ollama_native_root
    ...
    def test_ollama_host_environment_forms_are_normalized(self, monkeypatch):
        from zeloo_cli.models import _get_ollama_base_url, _root_for_ollama_native_api
    ...
    def test_ollama_failed_probe_is_cached_briefly(self):
        ...
        with patch(
            "zeloo_cli.models._urlopen_model_catalog_request",
    ...
    def test_switch_model_direct_ollama_alias_preserves_matching_origin_api_key(self):
        import zeloo_cli.model_switch as model_switch
        ...
        try:
            with patch.object(model_switch, "get_model_info", return_value=None), patch(
                "zeloo_cli.models._get_provider_config_dict",
...
class TestOpenRouterCatalogDiskCache:
    ...
    def _install_network(monkeypatch, calls):
        ...
        monkeypatch.setattr("zeloo_cli.model_catalog.get_curated_openrouter_models",
                            lambda: [("a/one", "")])
