...
from zeloo_cli import update_cmd_fleet as fleet
...
def test_unit_transaction_budget_preserves_scope_and_health(monkeypatch, graceful, retry):
    ...
    def systemctl(cmd, *, timeout):
        ...
        if "list-units" in cmd:
            return subprocess.CompletedProcess(cmd, 0, "Zeloo-serve-test.service loaded active running", "")
        if "show" in cmd:
    ...
    name = "Zeloo-gateway-test" if graceful else "Zeloo-serve-test"
    restarted, failed = [], []
...
def test_budget_fallback_keeps_real_errors(monkeypatch, limits, outcome):
    ...
    if outcome == "timeout":
        with pytest.raises(subprocess.TimeoutExpired):
            fleet._systemctl_reset_and_restart(["systemctl"], "Zeloo-serve-test")
    else:
        result = fleet._systemctl_reset_and_restart(["systemctl"], "Zeloo-serve-test")
