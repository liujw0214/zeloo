...
from zeloo_cli.subcommands.dashboard import build_dashboard_parser
...
from zeloo_cli.subcommands.whatsapp import build_whatsapp_parser
...
def test_config_get_unset_subcommands_parse():
    """`Zeloo config get/unset` parse key args (and --json for get)."""
...
# ── deprecated `Zeloo login` fails gracefully, not with argparse error ────
#
# `Zeloo login` is a removed command; its handler (`login_command` in
# `zeloo_cli/auth.py`) prints a deprecation notice pointing at `Zeloo auth` /
# `Zeloo model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `Zeloo --help`.
