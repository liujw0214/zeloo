...
def test_termux_skips_bundled_skill_sync_when_stamp_fresh(monkeypatch, tmp_path, main_mod):
    ...
    monkeypatch.setattr(main_mod, "get_zeloo_home", lambda: tmp_path)
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        ...
        from zeloo_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """Zeloo -z bypasses ZELOOCLI, but recall still needs SessionDB."""
    from zeloo_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "zeloo_state", mod("zeloo_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "zeloo_cli.config",
        mod("zeloo_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "zeloo_cli.models",
        mod("zeloo_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    monkeypatch.setitem(
        ...
        mod("zeloo_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["ZELOO_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nous/Zeloo-test", provider="nous", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["ZELOO_MODEL"] == "nous/Zeloo-test"
    assert env["ZELOO_INFERENCE_MODEL"] == "nous/Zeloo-test"
    assert env["ZELOO_TUI_PROVIDER"] == "nous"
    assert env["ZELOO_INFERENCE_PROVIDER"] == "nous"
    assert env["ZELOO_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["ZELOO_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("Zeloo-tui-active-session-")
...
def test_make_tui_argv_dev_prebuilds_zeloo_ink(monkeypatch, main_mod, tmp_path):
    tui_dir = tmp_path / "ui-tui"
    tsx = tui_dir / "node_modules" / ".bin" / "tsx"
    ink_dir = tui_dir / "packages" / "Zeloo-ink"
    ...
    monkeypatch.delenv("ZELOO_TUI_DIR", raising=False)
