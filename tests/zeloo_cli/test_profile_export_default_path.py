...
import pytest
from zeloo_cli import profile_cmd
...
def profiles():
    ...
    Sibling test files reload ``zeloo_cli.profiles`` / ``zeloo_cli.main``;
    a top-level ``from zeloo_cli.profiles import ...`` here would bind the
    ...
    """
    return importlib.import_module("zeloo_cli.profiles")
...
def test_custom_zeloo_home_inside_a_checkout_uses_a_sibling_store(
    tmp_path, monkeypatch, profiles
):
    ...
    monkeypatch.setattr(profiles, "_get_default_zeloo_home", lambda: checkout)
...
def test_checkout_detection_does_not_depend_on_cwd(tmp_path, monkeypatch, profiles):
    """ZELOO_HOME inside a checkout must be detected even when cwd is elsewhere.
    ...
    checkout.mkdir()
    (checkout / ".git").write_text("gitdir: ../git\n", encoding="utf-8")
    zeloo_home = checkout / "data_home"
    zeloo_home.mkdir()
    elsewhere = tmp_path / "elsewhere"
    elsewhere.mkdir()
    monkeypatch.chdir(elsewhere)  # cwd has no .git ancestor inside tmp_path
    monkeypatch.setattr(Path, "home", lambda: tmp_path / "home")
    monkeypatch.setattr(profiles, "_get_default_zeloo_home", lambda: zeloo_home)
...
def test_cli_export_default_does_not_write_into_the_current_checkout(
    ...
    monkeypatch.setattr(profiles, "_get_default_zeloo_home", lambda: default_home)
    monkeypatch.setattr(
        "zeloo_constants.get_default_zeloo_root", lambda: default_home
    )
...
def test_slash_export_uses_the_same_managed_destination(
    ...
    mixin_mod = importlib.import_module("zeloo_cli.cli_commands_mixin")
    default_home = tmp_path / ".Zeloo"
    default_home.mkdir()
    monkeypatch.setattr(profiles, "_get_default_zeloo_home", lambda: default_home)
...
async def test_profile_export_api_uses_the_shared_managed_destination(
    ...
    from zeloo_cli.web_models import ProfileExport

    router_mod = importlib.import_module("zeloo_cli.web_routers.profiles")
...
def test_cwd_in_unrelated_checkout_does_not_prove_safety(
    ...
    of a ZELOO_HOME inside checkout B."""
    ...
    monkeypatch.setattr(Path, "home", lambda: tmp_path / "home")
    monkeypatch.setattr(profiles, "_get_default_zeloo_home", lambda: checkout_b)
