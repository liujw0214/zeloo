...
#88251: ``Zeloo update`` treated a failed desktop pack as non-fatal, printed
...
import zeloo_cli.update_cmd_maint as update_cmd_maint
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "zeloo_constants.with_zeloo_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "zeloo_constants.display_zeloo_home", lambda: str(tmp_path), raising=False
    )
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_zeloo_home", lambda: tmp_path)
