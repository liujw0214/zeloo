...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_zeloo_home", lambda: zeloo_home)
    monkeypatch.setattr(uninstall, "_is_default_zeloo_home", lambda home: False)
    ...
    assert str(zeloo_home) in output
    assert project_root.exists()
    assert zeloo_home.exists()
