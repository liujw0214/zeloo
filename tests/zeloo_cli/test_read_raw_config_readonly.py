...
@pytest.fixture()
def isolated_zeloo_home():
    """Per-test ZELOO_HOME dir (already redirected by the autouse conftest
...
def test_freshness_after_config_edit(isolated_zeloo_home):
    from zeloo_cli.config import read_raw_config_readonly

    cfg = _write_config(isolated_zeloo_home, {"display": {"ephemeral_system_ttl": 1}})
    first = read_raw_config_readonly()
    assert first["display"]["ephemeral_system_ttl"] == 1

    _write_config(isolated_zeloo_home, {"display": {"ephemeral_system_ttl": 7}})
...
def test_missing_config_returns_empty(isolated_zeloo_home):
    from zeloo_cli.config import read_raw_config_readonly

    cfg = isolated_zeloo_home / "config.yaml"
    if cfg.exists():
        cfg.unlink()
