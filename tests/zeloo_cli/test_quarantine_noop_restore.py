...
``Zeloo*.exe`` shims aside (``Zeloo.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``Zeloo`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("Zeloo", "Zeloo-agent", "Zeloo-acp", "Zeloo-gateway"):
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "Zeloo.exe" in names, "Zeloo.exe must be restored after a no-op install"
    assert "Zeloo-acp.exe" in names
    assert "Zeloo-gateway.exe" in names
    assert (scripts / "Zeloo.exe").read_bytes() == b"MZ-old-Zeloo"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "Zeloo.exe").read_bytes() == b"MZ-new-Zeloo"
...
# ---------------------------------------------------------------------------
# zeloo_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "Zeloo.exe" in names, "Zeloo.exe must be restored after a no-op recovery install"
    assert (scripts / "Zeloo.exe").read_bytes() == b"MZ-old-Zeloo"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "Zeloo.exe").write_bytes(b"MZ-new-Zeloo")
