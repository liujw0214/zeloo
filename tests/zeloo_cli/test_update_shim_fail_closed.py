...
The bug class: on Windows, when `Zeloo.exe`/sibling shims could not be
...
def _make_shims(scripts_dir: Path, names=("Zeloo", "Zeloo-gateway")) -> list[Path]:
...
def test_strict_quarantine_refuses_before_install(windows, tmp_path, monkeypatch):
    ...
    # Zeloo.exe cannot be renamed; Zeloo-gateway.exe can
    real_rename = Path.rename

    def deny_ZELOO(self, target):
        if self.name == "Zeloo.exe":
            raise PermissionError(13, "held open")
        return real_rename(self, target)

    monkeypatch.setattr(Path, "rename", deny_ZELOO)
    monkeypatch.setattr(main_install_repair, "_zeloo_exe_shims", lambda d: shims)
    ...
    assert install_ran == []
    assert "Zeloo.exe" in exc_info.value.failed_shims
    # The successful rename (Zeloo-gateway.exe) was rolled back.
    assert (scripts / "Zeloo-gateway.exe").exists()
    assert not list(scripts.glob("Zeloo-gateway.exe.old.*"))


def test_non_strict_keeps_warn_and_try(windows, tmp_path, monkeypatch):
    scripts = tmp_path / "venv" / "Scripts"
    shims = _make_shims(scripts, names=("Zeloo",))
    monkeypatch.setattr(
        Path, "rename",
...
def test_recovery_install_cmd_fail_closed(windows, tmp_path, monkeypatch):
    ...
    _make_shims(scripts, names=("Zeloo",))
...
def test_refusal_writes_marker_and_exits_2(monkeypatch, capsys):
    ...
    exc = main_install_repair.ShimQuarantineError(["Zeloo.exe"])
    with pytest.raises(SystemExit) as exit_info:
        update_cmd._refuse_update_for_contended_shims(exc)
    assert exit_info.value.code == 2
    assert wrote == [1]
    out = capsys.readouterr().out
    assert "Zeloo.exe" in out
