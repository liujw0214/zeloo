...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (ZELOO_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via ZELOO_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in ZELOO_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from zeloo_cli.providers import ZELOO_OVERLAYS
        assert "bedrock" in ZELOO_OVERLAYS
