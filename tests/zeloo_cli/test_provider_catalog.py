"""Tests for the unified provider catalog (zeloo_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``Zeloo model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from zeloo_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
