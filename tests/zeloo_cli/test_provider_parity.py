...
the SAME provider universe as ``Zeloo model`` (the CLI/TUI picker).
...
def test_every_zeloo_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `Zeloo model` universe."""
    ...
    assert not missing, (
        "providers shown in `Zeloo model` but not configurable in the desktop "
