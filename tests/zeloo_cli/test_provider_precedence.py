...
from zeloo_cli.auth import resolve_provider, AuthError


def _login(monkeypatch, provider_id):
    """Simulate a logged-in OAuth active_provider in auth.json."""
    monkeypatch.setattr("zeloo_cli.auth._load_auth_store",
                        lambda: {"active_provider": provider_id})
    monkeypatch.setattr("zeloo_cli.auth.get_auth_status",
                        lambda p: {"logged_in": p == provider_id})


def _config(monkeypatch, model_cfg):
    monkeypatch.setattr("zeloo_cli.config.load_config", lambda: {"model": model_cfg})
...
def _clear_provider_env(monkeypatch):
    for var in ("OPENAI_API_KEY", "OPENROUTER_API_KEY", "GLM_API_KEY", "ZAI_API_KEY",
                "KIMI_API_KEY", "MINIMAX_API_KEY", "ZELOO_INFERENCE_PROVIDER"):
...
def _logged_out(monkeypatch):
    ...
    monkeypatch.setattr("zeloo_cli.auth.get_auth_status", lambda p: {"logged_in": False})
...
def _free_tier(monkeypatch, *, on=True, identity=False):
    ...
    monkeypatch.setattr("zeloo_cli.anon_auth.guest_enabled", lambda: on)
    monkeypatch.setattr("zeloo_cli.anon_auth.has_guest", lambda: identity)
    monkeypatch.setattr("zeloo_cli.anon_auth.ensure_portal_identity",
