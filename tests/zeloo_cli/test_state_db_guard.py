...
from zeloo_cli.backup import verify_sqlite_integrity
...
class TestPreUpdateBackupIntegrityGuard:
    ...
    ZELOO_HOME whose state.db is corrupted mid-flight (#68474)."""
    ...
    def zeloo_home(self, tmp_path, monkeypatch):
        ...
        for mod in list(sys.modules.keys()):
            if mod.startswith("zeloo_cli.config") or mod == "zeloo_constants":
                del sys.modules[mod]
        return root

    def test_healthy_db_stays_quiet(self, zeloo_home, capsys):
    ...
    def test_zeroed_db_after_snapshot_is_loud(self, zeloo_home, capsys, monkeypatch):
