"""Tests for the zeloo_cli.nous_billing HTTP client's response handling.
...
from zeloo_cli import nous_billing as nb
...
def test_billing_token_cache_is_scoped_per_profile_home(monkeypatch, tmp_path):
    ...
    multiplex gateway, where the per-turn ZELOO_HOME override selects the auth.json."""
    ...
    monkeypatch.setattr(
        ...
        lambda **kw: f"tok-{zeloo_constants.get_zeloo_home().name}")
    a, b = tmp_path / "a", tmp_path / "b"
    a.mkdir(); b.mkdir()
    seen = []
    for home in (a, b):
        tok = zeloo_constants.set_zeloo_home_override(str(home))
        try:
