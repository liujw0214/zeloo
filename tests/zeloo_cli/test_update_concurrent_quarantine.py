"""Tests for issue #26670 — concurrent Zeloo.exe detection and improved
quarantine retry / reboot-deferred fallback during `Zeloo update` on Windows.
...
# Tests in this module either exercise the REAL _detect_concurrent_zeloo_instances
# helper (and need the autouse stub in tests/zeloo_cli/conftest.py disabled),
...
# ---------------------------------------------------------------------------
# _detect_concurrent_zeloo_instances
# ---------------------------------------------------------------------------


def _make_proc(pid: int, exe: str, name: str = "Zeloo.exe"):
...
def test_detect_concurrent_parents_call_robust_to_one_bad_hop(_winp, tmp_path):
    ...
    rows = [
        ...
        _make_proc(launcher_pid, str(shim), "Zeloo.exe"),
    ]
    ...
    with patch.dict(sys.modules, {"psutil": fake_psutil}):
        result = cli_main._detect_concurrent_zeloo_instances(scripts_dir)

    # No crash; helper completes. (Degenerate stub: launcher exe unreadable.)
    assert result == [(launcher_pid, "Zeloo.exe")]
...
# ---------------------------------------------------------------------------
# _quarantine_running_zeloo_exe — retry, then report
# ---------------------------------------------------------------------------
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    shim = tmp_path / "Zeloo.exe"
    shim.write_bytes(b"old")

    pairs = main_install_repair._quarantine_running_zeloo_exe(tmp_path)

    assert len(pairs) == 1
    orig, quarantine = pairs[0]
    assert orig == shim
    assert quarantine.name.startswith("Zeloo.exe.old.")
...
def test_quarantine_reports_a_lock_it_cannot_break(_winp, tmp_path, capsys, monkeypatch):
    ...
    monkeypatch.setattr(main_install_repair, "_zeloo_exe_shims", lambda d: [shim])
...
def test_pause_windows_gateways_for_update_stops_profile_and_unmapped_pids(
    ...
    monkeypatch.setattr(
        ...
        lambda pid: ["pythonw.exe", "-m", "zeloo_cli.main", "gateway", "run"]
        if pid == 202
        else None,
    )
    ...
    assert token == {
        ...
        "unmapped": [
            {
                "pid": 202,
                "argv": ["pythonw.exe", "-m", "zeloo_cli.main", "gateway", "run"],
            }
        ],
    }
...
def test_pause_and_resume_windows_gateway_service(
    ...
    import zeloo_cli.update_cmd_windows as update_cmd_windows
    ...
    assert token == {
        ...
        "unmapped_pids": [],
        "unmapped": [],
        "services": ["ZELOOGateway"],
        "expected_services": ["ZELOOGateway"],
        "restarted_services": [],
        "service_profiles": {"ZELOOGateway": "default"},
    }
    assert stopped == ["ZELOOGateway"]

    cli_main._resume_windows_gateways_after_update(token)
    assert started == ["ZELOOGateway"]
...
def test_pause_windows_gateway_service_failure_restores_every_attempted_service(
    ...
    services = [
        SimpleNamespace(name="ZELOOGateway", service_pid=11, service_create_time=11.0, gateway_pid=101, gateway_create_time=101.0, descendant_identities=()),
        SimpleNamespace(name="ZELOOGatewayPicasso", service_pid=22, service_create_time=22.0, gateway_pid=202, gateway_create_time=202.0, descendant_identities=()),
    ]
    monkeypatch.setattr(gateway_mod, "find_gateway_pids", lambda **_k: [])
    monkeypatch.setattr(
    ...
    with pytest.raises(RuntimeError, match="ZELOOGatewayPicasso"):
        cli_main._pause_windows_gateways_for_update()

    assert restarted == ["ZELOOGatewayPicasso", "ZELOOGateway"]
...
def test_pause_windows_gateway_service_surfaces_rollback_start_failure(
    ...
    with pytest.raises(RuntimeError, match="rollback failures: ZELOOGateway"):
        cli_main._pause_windows_gateways_for_update()
...
def test_restore_windows_gateway_service_waits_out_stop_pending(monkeypatch):
    ...
    update_cmd._restore_windows_gateway_service("ZELOOGateway")

    assert restarted == ["ZELOOGateway"]
...
def test_resume_windows_gateway_service_failure_stays_retryable(
    ...
    with pytest.raises(RuntimeError, match="ZELOOGateway"):
        cli_main._resume_windows_gateways_after_update(token)

    assert token["resume_needed"] is True
    assert token["services"] == ["ZELOOGateway"]
...
def test_unreadable_argv_falls_back_to_the_captured_prefix(monkeypatch):
    ...
    gateway_prefix = r"venv\Scripts\python.exe -m zeloo_cli.main gateway run"
...
# when every concurrent Zeloo.exe is a gateway runtime — the pause
...
def test_classify_concurrent_instance_recognises_gateway_runtimes(monkeypatch):
    ...
    launcher shape (python -m, Zeloo.exe shim, Zeloo-gateway.exe,
    gateway/run.py, bare `Zeloo gateway` which defaults to run)."""
    cases = [
        [r"C:\venv\Scripts\python.exe", "-m", "zeloo_cli.main", "gateway", "run"],
        [r"C:\venv\Scripts\Zeloo.exe", "gateway", "run"],
        [r"C:\venv\Scripts\Zeloo-gateway.exe"],
        [r"C:\venv\Scripts\python.exe", "gateway/run.py"],
        ["Zeloo.exe", "GATEWAY", "RUN"],  # matcher is case-insensitive
        ["Zeloo.exe", "gateway"],  # bare `Zeloo gateway` defaults to run
        # profile selector before the subcommand — canonical matcher strips it
        ["Zeloo.exe", "--profile", "work", "gateway", "run"],
    ]
    for argv in cases:
...
def test_classify_concurrent_instance_recognises_non_gateways(monkeypatch):
    ...
    cases = [
        [r"C:\venv\Scripts\Zeloo.exe"],  # interactive REPL
        [r"C:\venv\Scripts\Zeloo.exe", "dashboard"],
        ["Zeloo.exe", "gateway", "status"],  # management, not runtime
        ["Zeloo.exe", "gateway", "stop"],
...
def test_filter_non_gateway_concurrent_instances_splits(monkeypatch):
    ...
    monkeypatch.setitem(
        ...
        _fake_psutil_classify(
            {
                100: ["Zeloo.exe", "gateway", "run"],
                200: ["Zeloo.exe"],  # REPL — keep
                300: ["Zeloo.exe", "dashboard"],  # keep
    ...
    assert kept == [(200, "Zeloo.exe"), (300, "Zeloo.exe"), (400, "Zeloo.exe")]
...
def test_filter_non_gateway_concurrent_instances_gateway_only(monkeypatch):
    ...
    monkeypatch.setitem(
        ...
        _fake_psutil_classify(
            {
                111: ["Zeloo.exe", "gateway", "run"],
                222: [r"C:\venv\Scripts\Zeloo-gateway.exe"],
            }
        ),
    )
    matches = [(111, "Zeloo.exe"), (222, "Zeloo-gateway.exe")]
...
def test_update_gate_skips_abort_when_only_concurrent_is_gateway(
    ...
    with patch.object(
        ...
        cli_main,
        "_detect_concurrent_zeloo_instances",
        return_value=[(1000, "Zeloo.exe"), (2000, "Zeloo-gateway.exe")],
    ), patch.object(
    ...
    captured = capsys.readouterr().out
    assert "Another Zeloo.exe is running" not in captured
...
def test_update_gate_still_aborts_on_non_gateway_concurrent(
    ...
    with patch.object(
        ...
        cli_main,
        "_detect_concurrent_zeloo_instances",
        return_value=[(1000, "Zeloo.exe"), (3000, "Zeloo.exe")],
    ), patch.object(
        cli_main,
        "_filter_non_gateway_concurrent_instances",
        return_value=[(3000, "Zeloo.exe")],
    ), patch.object(
        cli_main, "_run_pre_update_backup"
    ) as mock_backup:
...
def test_update_impl_refuses_before_terminating_gateway_ancestor(
    ...
    holder = (
        300,
        "python.exe",
        r"C:\x\venv\Scripts\python.exe -m zeloo_cli.main gateway run",
    )
    monkeypatch.setattr(
        gateway_cli,
        "_is_pid_ancestor_of_current_process",
...
def test_stop_service_refuses_pid_reuse_before_sc_stop(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="identity changed"):
        update_cmd._stop_windows_gateway_service(
            "ZELOOGateway", expected_service_identity=(11, 11.0)
        )
