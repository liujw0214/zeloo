...
``Zeloo update`` can legitimately leave an autostash behind (--keep-stash
parks it; a conflicted restore preserves it), but nothing ever mentioned those
entries again — they persisted invisibly for weeks. ``Zeloo update`` now
warns about ``Zeloo-update-autostash-*`` entries older than the threshold.
...
def _make_repo_with_autostash(tmp_path, age_days: float):
    """Real repo with one Zeloo-update-autostash entry aged ``age_days``."""
    import shutil
    ...
    name = f"Zeloo-update-autostash-{stamp}"
...
def test_non_zeloo_stash_is_ignored(tmp_path, capsys):
    import shutil
...
def test_unparseable_autostash_timestamp_left_alone(tmp_path, capsys):
    ...
    _git(tmp_path, "stash", "push", "-m", "Zeloo-update-autostash-notadate")
    count = update_cmd._warn_orphaned_update_autostashes(["git"], tmp_path)
    assert count == 0
