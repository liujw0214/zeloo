...
``Zeloo profile delete`` removes the tree. That empty shell then
reappears in ``Zeloo profile list`` and Desktop Bot Mode. These tests
...
from zeloo_constants import named_profile_home
...
def _delete(name: str) -> None:
    with patch("zeloo_cli.profiles._cleanup_gateway_service"), patch(
        "zeloo_cli.profiles._stop_profile_backends"
    ):
...
class TestDeletedProfileTombstone:
    def test_delete_then_logging_setup_does_not_recreate_home(self, profile_env, monkeypatch):
        ...
        with pytest.raises(FileNotFoundError, match="Named profile home does not exist"):
            setup_logging(zeloo_home=profile_dir, force=True)

        assert not profile_dir.exists()
        monkeypatch.setenv("ZELOO_HOME", str(profile_env / ".Zeloo"))
        assert "worker" not in _named_homes(profile_env)
    ...
    def test_backfill_skips_tombstoned_directory(self, profile_env):
        ...
        _delete("worker")
        profile_dir.mkdir(parents=True)
        (profile_env / ".Zeloo" / ".env").write_text(
...
class TestNamedProfileHome:
    def test_logs_under_named_profile_resolve_to_profile_home(self, tmp_path):
        # tmp_path acts as a real Zeloo home (Docker/custom layout): it
    ...
    def test_dot_zeloo_layout_resolves_without_markers(self, tmp_path):
        worker = tmp_path / ".Zeloo" / "profiles" / "worker"
        assert named_profile_home(worker / "logs") == worker
        assert named_profile_home(worker) == worker

    def test_default_home_with_profiles_in_path_is_not_named(self, tmp_path):
        default_home = tmp_path / "foo" / "profiles" / "notahome" / ".Zeloo"
    ...
    def test_unrelated_profiles_dir_is_not_named(self, tmp_path):
        ...
        # its parent is not a Zeloo home, so logging must keep mkdir-ing.
    ...
    def test_unrelated_profiles_dir_still_mkdirs(self, tmp_path):
        from zeloo_constants import mkdir_under_zeloo_home

        custom_home = tmp_path / "srv" / "profiles" / "buildcache"
        log_dir = mkdir_under_zeloo_home(custom_home / "logs")
    ...
    def test_setup_logging_under_unrelated_profiles_dir_succeeds(self, tmp_path):
        ...
        log_dir = setup_logging(zeloo_home=custom_home, force=True)
    ...
    def test_tombstone_dir_marks_profiles_root(self, tmp_path):
        ...
        # `Zeloo profile delete` — its presence alone anchors recognition,
