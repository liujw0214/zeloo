...
from zeloo_cli import gateway as zeloo_gateway
...
from zeloo_cli import process_identity
...
def test_control_plane_argv_is_not_a_gateway():
    ...
    serve = "C:\\Zeloo\\.venv\\Scripts\\python.exe -m zeloo_cli.main serve --host 127.0.0.1"
    run = "C:\\Zeloo\\.venv\\Scripts\\python.exe -m zeloo_cli.main gateway run"
...
def test_control_plane_classifier_is_token_based_not_substring():
    ...
    py = "C:\\Zeloo\\.venv\\Scripts\\python.exe -m zeloo_cli.main"
...
def test_pause_skips_cold_start_plan_when_desktop_owns_lifecycle(monkeypatch):
    ...
    monkeypatch.setattr(zeloo_gateway, "find_gateway_pids", lambda **_k: [])
    monkeypatch.setattr(
        zeloo_gateway, "find_windows_gateway_services", lambda **_k: []
    )
