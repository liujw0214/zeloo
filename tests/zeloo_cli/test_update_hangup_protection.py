"""Tests for SIGHUP protection and stdout mirroring in ``Zeloo update``.

Covers ``_UpdateOutputStream``, ``_install_hangup_protection``, and
``_finalize_update_output`` in ``zeloo_cli/main.py``.  These exist so
that ``Zeloo update`` survives a terminal disconnect mid-install
...
from zeloo_cli.main_dashboard import _UpdateOutputStream, _finalize_update_output, _install_hangup_protection
from zeloo_cli.update_cmd import _log_only_write, _print_update_completion, _run_logged_subprocess


def test_update_completion_includes_bounded_action_identity(monkeypatch, capsys):
    monkeypatch.setenv("ZELOO_ACTION_ID", "a" * 32)
    ...
    monkeypatch.setattr("zeloo_cli.update_cmd._branch_head_suffix", lambda: "")

    _print_update_completion("✓ Update complete!")

    assert capsys.readouterr().out.splitlines() == [
        "✓ Update complete!",
        f"=== Zeloo-update completed {'a' * 32} ===",
    ]


def test_update_completion_rejects_untrusted_action_identity(monkeypatch, capsys):
    monkeypatch.setenv("ZELOO_ACTION_ID", "not-safe\nforged")
    monkeypatch.setattr("zeloo_cli.update_cmd._branch_head_suffix", lambda: "")
...
class TestInstallHangupProtection:
    ...
    def test_wraps_stdout_and_stderr_with_mirror(self, tmp_path, monkeypatch):
        ...
        import zeloo_cli.config as _cfg
        if hasattr(_cfg, "_ZELOO_HOME_CACHE"):
            _cfg._ZELOO_HOME_CACHE = None  # type: ignore[attr-defined]
        ...
        try:
            ...
            assert "Zeloo update started" in contents
        finally:
    ...
    def test_non_fatal_if_log_setup_fails(self, monkeypatch):
        """If get_zeloo_home() raises, stdio must be left untouched but SIGHUP still handled."""
        prev_out, prev_err = sys.stdout, sys.stderr
        ...
        monkeypatch.setattr(
            "zeloo_cli.config.get_zeloo_home", _boom, raising=True
        )
