...
- Probe: ``named_profile_served_by_running_multiplexer`` reads the pid-verified default
  ``gateway_state.json`` first (env-only opt-in on the default profile is invisible to ``Zeloo -p X``);
...
- ``Zeloo status`` / ``cron status`` on a satellite say running-via-multiplexer; the default lists served.
"""
...
def served_root(tmp_path, monkeypatch):
    ...
    (root / "gateway.pid").write_text(json.dumps({"pid": os.getpid(), "zeloo_home": str(root)}))
    (root / "gateway_state.json").write_text(json.dumps(
        {"pid": os.getpid(), "zeloo_home": str(root), "served_profiles": ["default", "coder"]}))
    monkeypatch.setenv("ZELOO_HOME", str(root / "profiles" / "coder"))
...
def test_status_surfaces_agree_for_a_satellite_profile(served_root, monkeypatch):
    import zeloo_cli.gateway as gw
    import zeloo_cli.status as st
    import zeloo_cli.cron as cr
