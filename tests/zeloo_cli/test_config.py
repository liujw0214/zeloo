"""Tests for zeloo_cli configuration management."""
    ...
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            if sys.platform == "win32":
                # Windows default is %LOCALAPPDATA%\Zeloo — see
                # zeloo_constants._get_platform_default_zeloo_home.
                ...
                assert home == base / "Zeloo"
            else:
                assert home == Path.home() / ".Zeloo"


class TestEnsureZELOOHome:
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from zeloo_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
    ...
    _PRE_REWRITE_DEFAULT_SOUL = (
        "You are Zeloo Agent, an intelligent AI assistant created by Nous "
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``Zeloo logs`` surfaces it)
    ...
    def test_corrupt_config_is_backed_up(self, tmp_path, capsys):
        ...
        """
        from zeloo_cli import config as cfg_mod
...
class TestSaveAndLoadRoundtrip:
    ...
    def test_atomic_config_write_refuses_unparseable_existing_config(self, tmp_path):
        ...
        from zeloo_cli.config import atomic_config_write
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_quotes_values_containing_hash(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ZELOO_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ZELOO_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ZELOO_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ZELOO_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestSanitizeEnvLines:
    ...
    def test_migrate_reports_normalized_line_formatting(self, capsys):
        ...
        with (
            patch("zeloo_cli.config.sanitize_env_file", return_value=2),
            patch(
                "zeloo_cli.config.check_config_version",
                return_value=(latest_version, latest_version),
            ),
            patch("zeloo_cli.config.read_raw_config", return_value={}),
            patch("zeloo_cli.config.get_missing_env_vars", return_value=[]),
            patch("zeloo_cli.config.get_missing_config_fields", return_value=[]),
            patch("zeloo_cli.config.get_missing_skill_config_vars", return_value=[]),
        ):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from zeloo_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """ZELOO_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `Zeloo setup`) lets a
        ...
        via config.yaml; ZELOO_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from zeloo_cli.config import OPTIONAL_ENV_VARS
        assert "ZELOO_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestMemoryProviderEnvVarsRegistry:
    ...
    and `Zeloo setup` surface it (previously only Honcho was listed, leaving
...
class TestConfigSupportFloor:
    ...
    def test_v11_config_is_refused_and_untouched(self, tmp_path, capsys):
        ...
        assert "run `Zeloo setup` to regenerate" in captured.out
    ...
    def test_floor_message_uses_display_zeloo_home(self):
        from zeloo_cli.config_migrations import support_floor_message
        from zeloo_constants import display_zeloo_home

        msg = support_floor_message()
        assert f"{display_zeloo_home()}/config.yaml" in msg
...
class TestCustomProviderCompatibility:
    ...
    def _run_ladder(current_ver: int):
        from zeloo_cli.config_migrations import run_migrations
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or selected Zeloo runtime/security controls.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Zeloo
    process on next startup via the dotenv → ``os.environ`` chain in
    ``zeloo_cli/env_loader.py``.
    ...
    def _zeloo_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        "allowed_key",
        [
            "ZELOO_LANGFUSE_PUBLIC_KEY",
            "ZELOO_SPOTIFY_CLIENT_ID",
            "ZELOO_QWEN_BASE_URL",
            "ZELOO_MAX_ITERATIONS",
        ],
    )
    def test_zeloo_integration_keys_still_writable(self, allowed_key):
        """``ZELOO_*`` overall is NOT blocked.
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "ZELOO_ENV_PATH",
            "ZELOO_OPTIONAL_MCPS",
            "ZELOO_COPILOT_ACP_COMMAND",
            "ZELOO_COPILOT_ACP_ARGS",
            "ZELOO_YOLO_MODE",
            "ZELOO_ACCEPT_HOOKS",
            ...
            "ZELOO_CRON_SESSION",
            "ZELOO_SINGLE_QUERY_SESSION",
            "ZELOO_SESSION_KEY",
            "ZELOO_SESSION_PLATFORM",
        ],
    )
    def test_zeloo_security_control_keys_are_not_writable(self, protected_key):
    ...
    def test_preexisting_optional_mcps_override_still_loads(self, tmp_path):
        ...
        from zeloo_cli.config import invalidate_env_cache

        catalog = tmp_path / "custom-mcp-catalog"
        (tmp_path / ".env").write_text(
            f"ZELOO_OPTIONAL_MCPS={catalog}\n",
            encoding="utf-8",
        )
        invalidate_env_cache()

        assert load_env()["ZELOO_OPTIONAL_MCPS"] == str(catalog)

    @pytest.mark.parametrize(
        ("key", "expected"),
        [
            ("Path", "PATH"),
            ("ZELOO_Yolo_Mode", "ZELOO_YOLO_MODE"),
            ("ZELOO_Optional_Mcps", "ZELOO_OPTIONAL_MCPS"),
            ("ZELOO_Copilot_Acp_Command", "ZELOO_COPILOT_ACP_COMMAND"),
            ("ZELOO_Copilot_Acp_Args", "ZELOO_COPILOT_ACP_ARGS"),
        ],
    )
    def test_windows_policy_names_are_case_insensitive(self, key, expected):
        from zeloo_cli.config import _env_var_policy_name
    ...
    def test_windows_env_assignment_matching_is_case_insensitive(self, prefix):
        from zeloo_cli.config import _env_line_defines_key
...
"""
    ...
    def test_persist_migration_writes_full_read_raw_config(self, tmp_path):
        from zeloo_cli.config import _persist_migration, read_raw_config
    ...
    def test_preserves_existing_codex_app_server_auto_value(self, tmp_path):
        with patch.dict(os.environ, {"ZELOO_HOME": str(tmp_path)}):
            self._write(
                ...
                "_config_version: 31\n"
                "compression:\n"
                "  codex_app_server_auto: Zeloo\n",
            )

            migrate_config(interactive=False, quiet=True)

            raw = yaml.safe_load((tmp_path / "config.yaml").read_text())
            assert raw["compression"]["codex_app_server_auto"] == "Zeloo"
...
def test_default_config_has_no_duplicate_top_level_keys():
    ...
    import ast
    import zeloo_cli.config as cfg_mod
...
class TestConfigCommandFailClosedSurface:
    """`Zeloo config set/unset` must exit cleanly (no traceback) when the
