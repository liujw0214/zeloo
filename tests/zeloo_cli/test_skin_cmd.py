"""`Zeloo skin set` — deterministic single-color tweak of the active skin.
...
from zeloo_cli import skin_cmd
from zeloo_constants import get_zeloo_home


def _skins():
    d = get_zeloo_home() / "skins"
    d.mkdir(parents=True, exist_ok=True)
    return d


def _activate(name: str) -> None:
    (get_zeloo_home() / "config.yaml").write_text(f"display:\n  skin: {name}\n", encoding="utf-8")
...
def test_set_forks_a_builtin_without_inventing_a_background():
    ...
    assert (get_zeloo_home() / "config.yaml").read_text().find("default-custom") != -1
