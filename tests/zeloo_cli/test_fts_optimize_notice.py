...
def test_update_notice_offers_v1_trigram_tool_calls_rebuild(tmp_path, monkeypatch, capsys):
    ...
    class FakeSessionDB:
        ...
        _db_needs_fts_storage_upgrade = staticmethod(
            zeloo_state.SessionDB._db_needs_fts_storage_upgrade
        )

    monkeypatch.setattr(zeloo_constants, "get_zeloo_home", lambda: tmp_path)
    monkeypatch.setattr(zeloo_state, "SessionDB", FakeSessionDB)
    ...
    assert "Zeloo sessions optimize-storage" in capsys.readouterr().out
    conn.close()
