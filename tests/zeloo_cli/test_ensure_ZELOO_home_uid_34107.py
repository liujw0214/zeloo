"""Regression tests for #34107 — Docker UID/GID handling in ensure_zeloo_home.

When Zeloo runs in Docker with ``ZELOO_UID=1000`` / ``ZELOO_GID=911``,
the entrypoint chowns the top-level ``ZELOO_HOME`` once at startup. But
subdirectories created at runtime by ``ensure_zeloo_home()`` — especially
...
The fix is a ``_chown_to_zeloo_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_zeloo_uid_gid
# ---------------------------------------------------------------------------


class TestResolveZELOOUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("ZELOO_UID", "1000")
        monkeypatch.setenv("ZELOO_GID", "911")
        from zeloo_cli.config import _resolve_zeloo_uid_gid
...
class TestChownToZELOOUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_zeloo_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("ZELOO_UID", raising=False)
        monkeypatch.delenv("ZELOO_GID", raising=False)
        from zeloo_cli import config as cfg
