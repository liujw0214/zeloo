...
1. `Zeloo secrets bitwarden setup --help` works (dispatch path)
2. `Zeloo update --check` works (update path)
3. `Zeloo secrets bitwarden disable` works (handler execution)
4. `Zeloo secrets onepassword status` works (lazy backend loads on demand)
...
def _run_ZELOO(args: list[str], timeout: int = 30) -> subprocess.CompletedProcess[str]:
    """Run Zeloo CLI as a subprocess from repo root.
    ...
    connectivity — but ``Zeloo update --check`` really does ``git fetch``
    ...
    env.update(
        {
            ...
            "GIT_CONFIG_KEY_0": "url.file:///nonexistent-Zeloo-test-remote/.insteadOf",
            "GIT_CONFIG_VALUE_0": "https://",
            "GIT_CONFIG_KEY_1": "url.file:///nonexistent-Zeloo-test-remote/.insteadOf",
...
class TestSecretsDispatchE2E:
    ...
    def test_bitwarden_status(self) -> None:
        """`Zeloo secrets bitwarden status` must exit 0 (runs lazy backend)."""
        result = _run_ZELOO(["secrets", "bitwarden", "status"])
    ...
    def test_bitwarden_disable(self) -> None:
        """`Zeloo secrets bitwarden disable` must exit 0."""
        result = _run_ZELOO(["secrets", "bitwarden", "disable"])
        assert result.returncode == 0, (
    ...
    def test_onepassword_status(self) -> None:
        """`Zeloo secrets onepassword status` must exit 0 (1Password lazy backend)."""
        result = _run_ZELOO(["secrets", "onepassword", "status"])
        assert result.returncode in (0, 1), (
    ...
    def test_onepassword_setup_help(self) -> None:
        """`Zeloo secrets onepassword setup --help` must exit 0."""
        result = _run_ZELOO(["secrets", "onepassword", "setup", "--help"])
...
class TestUpdatePathE2E:
    ...
    These tests invoke the real `Zeloo update --check` path as a subprocess.
    ...
    def test_update_check_clean(self) -> None:
        """`Zeloo update --check` must not load cryptography._rust."""
        result = _run_ZELOO(["update", "--check"])
    ...
    def test_main_update_check_crypto_absent_in_sys_modules(self) -> None:
        """Decisive invariant: invoking main() with argv=['Zeloo','update','--check']
...
sys.argv = ['Zeloo', 'update', '--check']

import zeloo_cli.main as m
...
# resolves the zeloo_cli.main module's lazily re-exported attribute —
# so the patch must land on update_cmd._cmd_update_check.
with patch('zeloo_cli.update_cmd._cmd_update_check', capture_update_check):
    try:
        m.main()
