...
import zeloo_state
import zeloo_state_holders
import zeloo_state_schema
from zeloo_state import SessionDB
from zeloo_state_common import FTS_REBUILD_DEFERRAL_KEY, FTS_STALE_KEY, LEGACY_FTS_SQL, LEGACY_FTS_TRIGRAM_SQL, SCHEMA_SQL, _FTS_TRIGGERS
from zeloo_state_dbfile import _concrete_state_db_holder_pids, _is_inactive_orphan_desktop_holder
...
class TestRuntimeFtsRebuild:
    def test_reap_candidates_exclude_uninspectable_holder_suspicions(
        ...
        assert _concrete_state_db_holder_pids(
            db_path,
            [
                (222, "uninspectable holder: python -m zeloo_cli.main serve --port 0"),
    ...
    @pytest.mark.parametrize(
        "argv",
        (
            ("journalctl", "-u", "Zeloo-agent.service"),
            ("grep", "Zeloo-agent", "/var/log/syslog"),
            (
                "/usr/sbin/tailscaled",
                "be-child",
                "ssh",
                "--cmd=python -m zeloo_cli.main gateway",
            ),
            ("tmux", "new-session", "/opt/Zeloo-agent/.venv/bin/Zeloo gateway"),
            ("python3", "/opt/Zeloo-agent/tools/check_state.py"),
            ("Zeloo-monitor", "gateway"),
            ("ZELOOctl", "serve"),
            ("python3", "worker.py", "zeloo_cli.main"),
            ("python3", "-m", "other.module", "zeloo_cli.main"),
            ("python3", "-c", "zeloo_cli.main"),
            ("python3", "-Icprint('zeloo_cli.main')", "zeloo_cli/main.py"),
        ),
    )
    def test_uninspectable_non_zeloo_process_is_not_a_holder(self, argv):
        assert not zeloo_state_holders._looks_like_ZELOO(argv)

    @pytest.mark.parametrize(
        "argv",
        (
            ("/usr/local/bin/Zeloo", "gateway"),
            ("/usr/local/bin/Zeloo-agent", "serve"),
            ("/usr/local/bin/Zeloo-acp", "--stdio"),
            ("/usr/bin/python3", "-m", "zeloo_cli.main", "gateway"),
            ("/usr/bin/python3", "-m", "acp_adapter"),
            ("/usr/bin/python3", "-Im", "zeloo_cli.main", "gateway"),
            ("/usr/bin/python3", "-mzeloo_cli.main", "gateway"),
            ("/usr/bin/python3", "-W", "ignore", "-m", "zeloo_cli.main"),
            ("/usr/bin/python3", "-Xdev", "-m", "zeloo_cli.main"),
            (
                "/opt/Zeloo-agent/.venv/bin/python",
                "/opt/Zeloo-agent/zeloo_cli/main.py",
                "gateway",
            ),
            ("python.exe", "--", "zeloo_cli/main.py", "gateway"),
            ("python3", "/opt/Zeloo-agent/run_agent.py", "--query", "hello"),
        ),
    )
    def test_uninspectable_zeloo_process_remains_a_holder(self, argv):
        assert zeloo_state_holders._looks_like_ZELOO(argv)
    ...
    def test_foreign_holder_detection_proc_readlink_deleted_wal(
        ...
        monkeypatch.setattr(zeloo_state_holders.os, "getpid", lambda: 111)
        real_listdir = os.listdir
        def _listdir(path):
            if isinstance(path, str):
                path = path.replace("/proc", str(proc_root))
            return real_listdir(path)
        monkeypatch.setattr(zeloo_state_holders.os, "listdir", _listdir)
        real_readlink = os.readlink
        def _readlink(path):
            path = path.replace("/proc", str(proc_root))
            return real_readlink(path)
        monkeypatch.setattr(zeloo_state_holders.os, "readlink", _readlink)
        real_stat = os.stat
        ...
        monkeypatch.setattr(zeloo_state_holders.os, "stat", _stat)

        holders = zeloo_state_holders.foreign_state_db_holders(db_path)
    ...
    def test_foreign_holder_ignores_same_path_with_different_file_identity(
        ...
        assert zeloo_state_holders.foreign_state_db_holders(db_path) == []
    ...
    def test_foreign_holder_uninspectable_process_cmdline_fallback(
        ...
        flagged when /proc/<pid>/cmdline identifies it as a Zeloo process."""
        db_path = tmp_path / "state.db"
        ...
        # PID 222's cmdline is world-readable and looks like Zeloo
        cmdline_path = proc_root / "222" / "cmdline"
        cmdline_path.write_bytes(
            b"python3\x00-m\x00zeloo_cli.main\x00chat\x00"
        )
        ...
        def _fake_argv(pid):
            ...
            try:
                ...
                return None
        monkeypatch.setattr(zeloo_state_holders, "_read_proc_argv", _fake_argv)

        holders = zeloo_state_holders.foreign_state_db_holders(db_path)
        # Should include PID 222 with the cmdline info
        assert len(holders) == 1
        assert holders[0][0] == 222
        assert "zeloo_cli.main" in holders[0][1]
    ...
    def test_structural_corruption_propagates_without_live_fts_mutation(
        ...
        from zeloo_state import StateDbCorruptError
    ...
    def test_repeated_deferrals_reap_inactive_orphan_then_rebuild(
        ...
        monkeypatch.setattr(zeloo_state_schema.time, "time", lambda: 120.0)
    ...
    def test_same_holder_set_across_futile_window_names_holder_and_gateway_safe_remedy(
        ...
        monkeypatch.setattr(
            zeloo_state_schema, "_read_proc_argv", lambda pid: ["python", "-m", "zeloo_cli.main", "serve"], raising=False,
        )
        clock = [1000.0]
        monkeypatch.setattr(zeloo_state_schema.time, "time", lambda: clock[0])

        from zeloo_cli.doctor_state import _render_state_db_stats
        from zeloo_state_dbfile import collect_state_db_stats
        ...
        futile_attempts = getattr(zeloo_state_schema, "_FTS_HOLDER_FUTILE_ATTEMPTS", 10)
        futile_seconds = getattr(zeloo_state_schema, "_FTS_HOLDER_FUTILE_SECONDS", 1800.0)
        reopened = SessionDB(db_path=db_path)
        try:
            ...
            assert "pid 4242: python -m zeloo_cli.main serve" in msg
    ...
    def test_retry_backoff_resets_when_the_blocking_holder_set_changes(
        ...
        try:
            ...
            reopened._fts_stale_retry_after = time.monotonic() + zeloo_state_schema._FTS_STALE_RETRY_MAX_SECONDS
            reopened._fts_stale_retry_interval = zeloo_state_schema._FTS_STALE_RETRY_MAX_SECONDS
...
class TestPhysicalCorruptionAcceptance:
    ...
    def test_canonical_btree_corruption_fails_closed(
        ...
        try:
            caplog.clear()
            with caplog.at_level("WARNING", logger="zeloo_state"):
                with pytest.raises(sqlite3.DatabaseError) as caught:
