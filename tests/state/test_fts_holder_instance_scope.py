...
Zeloo instances: a main gateway (user ``ubuntu``,
ZELOO_HOME=/home/ubuntu/.Zeloo) and a demo gateway (user ``demo``,
ZELOO_HOME=/home/demo/.Zeloo).  The demo gateway runs as another user, so
its ``/proc/<pid>/fd`` table is unreadable from the main instance and the
holder scan falls back to ``/proc/<pid>/cmdline`` + ``_looks_like_ZELOO``.
The demo process's argv matches the Zeloo patterns exactly, so the fallback
...
rebuild in ``zeloo_state_schema._recover_stale_fts`` was deferred 42 times
...
Zeloo); a genuine second instance with a DIFFERENT ZELOO_HOME is still
misjudged on current main (issue #92401).

Behavior contract: an uninspectable holder identified only by argv must be
counted unless its own argv proves it is scoped to a *different* Zeloo
...
# token lives under /home/demo/.Zeloo, the binary name matches the Zeloo
# patterns, and nothing references the main instance's home or state.db.
DEMO_HOME_ARGV = [
    "/home/demo/.Zeloo/Zeloo-agent/Zeloo",
    "gateway",
    "run",
]

# Same, spelled through the venv interpreter + Zeloo launcher script.
DEMO_VENV_ARGV = [
    "/home/demo/.Zeloo/Zeloo-agent/venv/bin/python",
    "/home/demo/.Zeloo/Zeloo-agent/zeloo_cli/main.py",
    "gateway",
]

# A Zeloo-shaped argv with no absolute paths: cannot disprove that this
# process touches our state.db, so it must stay fail-closed.
AMBIGUOUS_ARGV = ["Zeloo", "gateway", "run"]
...
class TestUninspectableHolderInstanceScope:
    def test_other_instance_argv_is_not_a_holder_of_our_db(self, tmp_path, monkeypatch):
        ...
        DIFFERENT Zeloo home → not a holder of our state.db."""
    ...
    def test_argv_referencing_our_db_stays_flagged(self, tmp_path, monkeypatch):
        ...
        for argv in (
            ["Zeloo", f"--db={db_path}", "gateway"],
            ["Zeloo", "checkpoint", f"{db_path}-wal"],
            ["Zeloo", "--home", our_home, "gateway"],
        ):
            assert zeloo_state_holders._looks_like_ZELOO(argv) or argv[0] == "Zeloo"
            _install_fake_proc(monkeypatch, tmp_path, unreadable_pids=(222,))
