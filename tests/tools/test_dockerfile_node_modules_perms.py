...
Older images made /opt/ZELOO/ui-tui and /opt/ZELOO/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV ZELOO_TUI_DIR=/opt/ZELOO/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R ZELOO:ZELOO /opt/ZELOO/ui-tui" not in text
    assert "chown -R ZELOO:ZELOO /opt/ZELOO/node_modules" not in text
