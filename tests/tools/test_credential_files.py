...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        zeloo_home.mkdir()
        (zeloo_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"ZELOO_HOME": str(zeloo_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(zeloo_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.Zeloo/token.json"
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        zeloo_home = tmp_path / ".Zeloo"
        zeloo_home.mkdir()
        (zeloo_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.Zeloo/skills"


    def test_custom_container_base(self, tmp_path):
        zeloo_home = tmp_path / ".Zeloo"
        (zeloo_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"ZELOO_HOME": str(zeloo_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.Zeloo")

        assert mounts[0]["container_path"] == "/home/user/.Zeloo/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.Zeloo/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.Zeloo/skills/cat/myskill/scripts/run.sh" in paths
    ...
    def test_skips_excluded_bookkeeping_dirs(self, tmp_path):
        ...
        paths = {f["container_path"] for f in files}
        assert "/root/.Zeloo/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.Zeloo/skills/cat/myskill/references/api.md" in paths
        for excluded in (
            ".hub",
            ".archive",
            ".curator_backups",
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape ZELOO_HOME."""
        monkeypatch.setenv("ZELOO_HOME", str(tmp_path / ".Zeloo"))
        (tmp_path / ".Zeloo").mkdir()

        # Create a sensitive file one level above zeloo_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside zeloo_home
    ...
    def test_nested_subdir_inside_zeloo_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of ZELOO_HOME must be allowed."""
        zeloo_home = tmp_path / ".Zeloo"
        zeloo_home.mkdir()
        subdir = zeloo_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside ZELOO_HOME pointing outside must be rejected."""
        zeloo_home = tmp_path / ".Zeloo"
        zeloo_home.mkdir()
        monkeypatch.setenv("ZELOO_HOME", str(zeloo_home))

        # Create a sensitive file outside zeloo_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside zeloo_home pointing outside
        symlink = zeloo_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes ZELOO_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, zeloo_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape ZELOO_HOME."""
        zeloo_home = tmp_path / ".Zeloo"
        zeloo_home.mkdir()
        monkeypatch.setenv("ZELOO_HOME", str(zeloo_home))
        ...
        sensitive.write_text("{}")
        self._write_config(zeloo_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(zeloo_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside ZELOO_HOME via config must still mount."""
        zeloo_home = tmp_path / ".Zeloo"
        zeloo_home.mkdir()
        monkeypatch.setenv("ZELOO_HOME", str(zeloo_home))

        (zeloo_home / "oauth.json").write_text("{}")
        self._write_config(zeloo_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        zeloo_home = tmp_path / ".Zeloo"
        zeloo_home.mkdir()
        (zeloo_home / "cache" / "documents").mkdir(parents=True)
        (zeloo_home / "cache" / "audio").mkdir(parents=True)
        (zeloo_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("ZELOO_HOME", str(zeloo_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.Zeloo/cache/documents" in paths
        assert "/root/.Zeloo/cache/audio" in paths
        assert "/root/.Zeloo/cache/videos" in paths
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_zeloo_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        zeloo_home = tmp_path / ".Zeloo"
        zeloo_home.mkdir()
        # Use legacy dir name with content — get_zeloo_dir prefers
        # populated old over new.
        legacy_doc = zeloo_home / "document_cache"
        legacy_img = zeloo_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(zeloo_home / "document_cache") in host_paths
        assert str(zeloo_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.Zeloo/cache/documents" in container_paths
        assert "/root/.Zeloo/cache/images" in container_paths
    ...
    def test_empty_zeloo_home(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.Zeloo/attachments" in container_paths
        assert "/root/.Zeloo/images" in container_paths
        assert "/root/.Zeloo/cache/images" in container_paths
        for mount in mounts:
    ...
    def test_images_upload_dir_is_mounted(self, tmp_path, monkeypatch):
        ...
        Desktop / clipboard / PDF uploads land in ``ZELOO_HOME/images``, not
        ...
        """
        zeloo_home = tmp_path / ".Zeloo"
        (zeloo_home / "images").mkdir(parents=True)
        monkeypatch.setenv("ZELOO_HOME", str(zeloo_home))

        mounts = get_cache_directory_mounts()
        by_container = {m["container_path"]: m["host_path"] for m in mounts}
        assert "/root/.Zeloo/images" in by_container
        assert by_container["/root/.Zeloo/images"] == str(zeloo_home / "images")
    ...
    def test_images_upload_file_maps_into_container(self, tmp_path, monkeypatch):
        ...
        zeloo_home = tmp_path / ".Zeloo"
        (zeloo_home / "images").mkdir(parents=True)
        upload = zeloo_home / "images" / "upload_20260722_181019_1.png"
        upload.write_bytes(bytes.fromhex("89504e470d0a1a0a"))
        monkeypatch.setenv("ZELOO_HOME", str(zeloo_home))

        assert (
            map_cache_path_to_container(str(upload))
            == "/root/.Zeloo/images/upload_20260722_181019_1.png"
        )
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        zeloo_home = tmp_path / ".Zeloo"
        img_dir = zeloo_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.Zeloo/cache/images/generated.png"
        )
    ...
    def test_maps_path_even_when_cache_dir_missing(self, tmp_path, monkeypatch):
        ...
        mapped = map_cache_path_to_container(str(zeloo_home / "cache" / "images" / "x.png"))
        assert mapped == "/root/.Zeloo/cache/images/x.png"
        assert (zeloo_home / "cache" / "images").is_dir()


class TestToAgentVisiblePathPerBackend:
    """#76577 follow-up: translation covers every backend that relocates the
    Zeloo cache — not just docker — and skips the ones where the host path
    stays correct (local; singularity auto-binds the host home)."""

    def _staged(self, tmp_path, monkeypatch):
        zeloo_home = tmp_path / ".Zeloo"
        (zeloo_home / "attachments").mkdir(parents=True)
        monkeypatch.setenv("ZELOO_HOME", str(zeloo_home))
        return str(zeloo_home / "attachments" / "drop.zip")

    def test_docker_maps_to_root_ZELOO(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "docker")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "/root/.Zeloo/attachments/drop.zip"

    def test_ssh_maps_to_tilde_ZELOO(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "ssh")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "~/.Zeloo/attachments/drop.zip"
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        zeloo_home = tmp_path / ".Zeloo"
        doc_dir = zeloo_home / "cache" / "documents"
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — ZELOO_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping ZELOO_HOME, but every master
    ...
    def test_refused_entry_does_not_block_the_rest_of_the_batch(self, tmp_path):
        ...
        paths = [m["container_path"] for m in mounts]
        assert "/root/.Zeloo/google_token.json" in paths
        assert "/root/.Zeloo/.env" not in paths
