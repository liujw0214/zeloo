"""Tests for tools.wake_word — the "Hey Zeloo" hotword detector.
...
def test_config_defaults_and_clamping():
    ...
    assert ww.wake_phrase({"phrase": "hey Zeloo"}) == "hey Zeloo"
    assert ww.wake_phrase({}) == "hey Zeloo"
...
def test_looks_like_path():
    ...
    assert _looks_like_path("models/hey_ZELOO.onnx")
...
def test_load_wake_word_config_guards_non_dict(monkeypatch):
    monkeypatch.setattr(
        "zeloo_cli.config.load_config", lambda: {"wake_word": "oops"}
    )
...
def test_requirements_openwakeword_available(monkeypatch):
    ...
    r = ww.check_wake_word_requirements(
        {"provider": "openwakeword", "phrase": "hey Zeloo"}
    )
    assert r["available"] is True
    assert r["provider"] == "openwakeword"
    assert r["phrase"] == "hey Zeloo"
...
def _install_fake_openwakeword(monkeypatch):
    ...
    class _FakeModel:
        def __init__(self, wakeword_models, inference_framework="onnx"):
            ...
            self.models = {"hey_ZELOO": object()}

        def predict(self, frame):
            return {"hey_ZELOO": 0.0}
...
def test_openwakeword_ensures_base_models_for_custom_path(monkeypatch):
    ...
    eng = ww._OpenWakeWordEngine(
        {"provider": "openwakeword", "openwakeword": {"model": "/models/hey_ZELOO.onnx"}}
    )
    assert calls["download"] == [["/models/hey_ZELOO.onnx"]]
    assert eng._labels == ["hey_ZELOO"]


def test_bundled_hey_zeloo_model_ships_on_disk():
    # The "hey Zeloo" wake word works out of the box only if the model is
