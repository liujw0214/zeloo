...
class TestSnapshotEndToEnd:
    ...
    def test_exported_env_changes_persist_between_commands(self, tmp_path):
        ...
        try:
            first = env.execute(
                'export ZELOO_STICKY_ENV_PROBE="sticky"; '
                'export PATH="/tmp/Zeloo-session-bin:$PATH"; '
                'echo "first=$ZELOO_STICKY_ENV_PROBE"'
            )
            second = env.execute(
                'echo "second=$ZELOO_STICKY_ENV_PROBE"; echo "PATH=$PATH"'
            )
        finally:
            env.cleanup()
    ...
    def test_snapshot_picks_up_init_file_exports(self, tmp_path, monkeypatch):
        ...
        with patch(
            ...
            try:
                result = env.execute(
                    'echo "PROBE=$ZELOO_SHELL_INIT_PROBE"; echo "PATH=$PATH"'
                )
            finally:
                env.cleanup()
