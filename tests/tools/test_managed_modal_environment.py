...
def _restore_tool_and_agent_modules():
    ...
    original_modules = {
        ...
        if name in {"tools", "agent", "zeloo_cli"}
    ...
    try:
        ...
        _reset_modules(("tools", "agent", "zeloo_cli"))
...
def _install_fake_tools_package(*, credential_mounts=None):
    ...
    zeloo_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["zeloo_cli"] = zeloo_cli
    sys.modules["zeloo_cli.config"] = types.SimpleNamespace(
        get_zeloo_home=lambda: Path(tempfile.gettempdir()) / "Zeloo-home",
    )
...
def test_managed_modal_rejects_host_credential_passthrough():
    _install_fake_tools_package(
        credential_mounts=[{
            ...
            "container_path": "/root/.Zeloo/token.json",
        }]
    )
    managed_modal = _load_tool_module("tools.environments.managed_modal", "environments/managed_modal.py")
