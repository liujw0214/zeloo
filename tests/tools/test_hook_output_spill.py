...
class GetSpillConfigTests(unittest.TestCase):
    ...
    def test_load_config_exception_is_swallowed(self):
        with patch("zeloo_cli.config.load_config", side_effect=RuntimeError("bad")):
            cfg = hos.get_spill_config()
        self.assertEqual(cfg["max_chars"], hos.DEFAULT_MAX_CHARS)
        self.assertTrue(cfg["enabled"])
...
class SpillIfOversizedTests(unittest.TestCase):
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix="Zeloo-spill-test-")
    ...
    def test_default_directory_uses_zeloo_home(self):
        """When no directory override, spill under ZELOO_HOME/hook_outputs."""
        test_home = tempfile.mkdtemp(prefix="Zeloo-home-")
        try:
            with patch.dict(os.environ, {"ZELOO_HOME": test_home}):
                # Also patch get_zeloo_home to the env var to mirror production.
            ...
            # ~/.Zeloo/hook_outputs depending on get_zeloo_home behaviour.
            candidates = [
                Path(test_home) / "hook_outputs" / "sess",
                Path(os.path.expanduser("~/.Zeloo/hook_outputs/sess")),
            ]
