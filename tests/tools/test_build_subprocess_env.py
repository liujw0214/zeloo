...
def test_no_scrub_inherit_profile_home_bridges_context_override(tmp_path):
    ...
    token = set_zeloo_home_override(str(tmp_path))
    try:
        env = build_subprocess_env(
    ...
    assert env["ZELOO_HOME"] == str(tmp_path)
...
def test_e2e_child_sees_zeloo_home_and_no_planted_secret(tmp_path, monkeypatch):
    """A real child spawned with a factory-built env must see ZELOO_HOME
    ...
    code = (
        "import os, json; "
        "print(json.dumps({'home': os.environ.get('ZELOO_HOME'), "
    ...
    assert result["home"] == str(zeloo_home)
...
# ---------------------------------------------------------------------------
# E2E regression (#93082): cron/no_agent children keep bare `Zeloo` on PATH
# ---------------------------------------------------------------------------


def test_e2e_scrubbed_env_resolves_bare_zeloo_under_minimal_parent_path(monkeypatch):
    """Regression for #92998/#93082: a gateway launched by systemd/cron with a
    minimal PATH (no Zeloo console-script dir) must still hand cron job
    children an env whose PATH resolves bare ``Zeloo``.
    ...
    bin_dir = local_mod._resolve_zeloo_bin_dir()
    if not bin_dir or not os.path.isfile(
        os.path.join(bin_dir, "Zeloo.exe" if os.name == "nt" else "Zeloo")
    ):
        pytest.skip("no real Zeloo console-script install available")

    # Simulate the service-manager minimal PATH: Zeloo dir absent.
    minimal_path = os.pathsep.join(["/usr/bin", "/bin"])
    monkeypatch.setenv("PATH", minimal_path)
    assert shutil.which("Zeloo", path=minimal_path) is None

    env = build_subprocess_env(scrub_secrets=True)  # cron _run_job_script path

    resolved = shutil.which("Zeloo", path=env.get("PATH", ""))
    assert resolved is not None, (
        f"bare 'Zeloo' must resolve from the child PATH {env.get('PATH')!r}"
    )
