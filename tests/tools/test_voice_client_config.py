...
Real config files in a temp ZELOO_HOME, real resolution chains (no mocked
...
def voice_home(tmp_path, monkeypatch):
    """Isolated ZELOO_HOME + reloaded config modules; yields a config writer."""
    home = tmp_path / ".Zeloo"
    home.mkdir()
    ...
    for var in (
        ...
        "DEEPINFRA_API_KEY", "ZELOO_LOCAL_STT_LANGUAGE",
    ):
