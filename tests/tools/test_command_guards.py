...
def _mode_manual(monkeypatch):
    ...
    The test conftest redirects ZELOO_HOME to an empty tempdir, so the
...
def _clean_state():
    ...
    saved = {}
    for k in ("ZELOO_INTERACTIVE", "ZELOO_GATEWAY_SESSION", "ZELOO_EXEC_ASK", "ZELOO_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["ZELOO_INTERACTIVE"] = "1"
        session_key = os.getenv("ZELOO_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No ZELOO_INTERACTIVE or ZELOO_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
