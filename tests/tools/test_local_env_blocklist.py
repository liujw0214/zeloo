...
Verifies that Zeloo-managed provider, tool, and gateway env vars are
stripped from subprocess environments so external CLIs are not silently
misrouted or handed Zeloo secrets.

See: https://github.com/NousResearch/Zeloo-agent/issues/1002
See: https://github.com/NousResearch/Zeloo-agent/issues/1264
"""
...
from tools.environments.local_env_policy import (
    _ZELOO_PROVIDER_ENV_BLOCKLIST,
    _ZELOO_PROVIDER_ENV_FORCE_PREFIX,
)
...
class TestProviderEnvBlocklist:
    """Provider env vars loaded from ~/.Zeloo/.env must not leak."""
    ...
    def test_bedrock_bearer_token_is_stripped(self):
        """The Bedrock-specific bearer token is a Zeloo inference secret
        ...
        models`` run inside a Zeloo terminal enumerated the entire Bedrock
    ...
    def test_general_aws_credential_chain_is_preserved(self):
        ...
        refuses to re-allow anything in _ZELOO_PROVIDER_ENV_BLOCKLIST
        (GHSA-rhgp-j443-p4rf). Only the Bedrock inference bearer token is
        Zeloo-managed; the rest belongs to the user.
        """
        general_chain = {
    ...
    def test_tool_and_gateway_vars_are_stripped(self):
        ...
        leaked_vars = {
            ...
            "ZELOO_DASHBOARD_SESSION_TOKEN": "dashboard-session-secret",
    ...
    def test_bare_zeloo_resolves_from_sanitized_subprocess_path(self):
        """Cron children can resolve Zeloo even when the gateway PATH cannot."""
        from tools.environments.local import _sanitize_subprocess_env

        with patch(
            "tools.environments.local._resolve_zeloo_bin_dir",
    ...
    def test_bare_zeloo_path_does_not_duplicate_existing_install_dir(self):
...
class TestTerminalFirstPartyPlatformEnv:
    ...
    (and the other BUZZ_* vars) are stripped by _ZELOO_PROVIDER_ENV_BLOCKLIST
    ...
    children then get the BUZZ_* vars; execute_code, zeloo_subprocess_env,
    ...
    def test_buzz_vars_stripped_without_buzz_context(self, monkeypatch):
        ...
        monkeypatch.delenv("ZELOO_SESSION_PLATFORM", raising=False)
        buzz_vars = {
            "BUZZ_PRIVATE_KEY": "nsec1faketestkey",
            "BUZZ_AUTH_TAG": '["tag","data","kind","sig"]',
            "BUZZ_RELAY_URL": "https://mycommunity.communities.buzz.xyz",
    ...
    def test_buzz_vars_stay_in_blocklist(self):
        ...
        zeloo_subprocess_env, env_passthrough registration)."""
...
class TestTerminalFirstPartySnapshotIsolation:
    ...
    an exclusion, profile A's BUZZ_PRIVATE_KEY lands in Zeloo-snap-<id>.sh
...
class TestForceEnvOptIn:
    """Callers can opt in to passing a blocked var via _ZELOO_FORCE_ prefix."""

    def test_force_prefix_passes_blocked_var(self):
        """_ZELOO_FORCE_OPENAI_API_KEY in self.env should inject OPENAI_API_KEY."""
        result_env = _run_with_env(self_env={
            f"{_ZELOO_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY": "sk-explicit",
        })
        ...
        assert f"{_ZELOO_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY" not in result_env
    ...
    def test_force_prefix_overrides_os_environ_block(self):
        ...
        result_env = _run_with_env(
            ...
            self_env={f"{_ZELOO_PROVIDER_ENV_FORCE_PREFIX}OPENAI_BASE_URL": "http://intended/v1"},
        )
...
class TestActiveVenvMarkerStripping:
    ...
    into the Zeloo venv path instead of the project's own ``.venv`` —
    silently clobbering the Zeloo environment (and, when the other project
    pins a different Python, breaking the gateway outright). The Zeloo venv
    stays reachable via PATH, so stripping the markers is safe.
    """

    def test_virtualenv_marker_stripped_end_to_end(self):
        result_env = _run_with_env(extra_os_env={
            "VIRTUAL_ENV": "/home/user/.Zeloo/Zeloo-agent/venv",
        })
        assert "VIRTUAL_ENV" not in result_env

    def test_conda_prefix_marker_stripped_end_to_end(self):
        result_env = _run_with_env(extra_os_env={
            "CONDA_PREFIX": "/opt/conda/envs/Zeloo",
        })
...
def _physical_repo_root(tmp_path: Path) -> Path:
    ...
    physical_root = tmp_path / "physical-home" / "Zeloo-agent"
    physical_root.mkdir(parents=True)
    return physical_root


class TestPythonpathSelectiveStrip:
    """PYTHONPATH Zeloo-owned entry stripping (#74817).

    The Desktop Electron app injects the Zeloo repo root and the Zeloo
    ...
    ``_strip_zeloo_owned_pythonpath`` surgically removes only the
    entries Zeloo itself owns (repo root, own venv site-packages),
    ...
    def test_owned_entries_stripped_matrix(self):
        """Exact Zeloo-owned entries are removed; everything else survives
        verbatim (ordering, duplicates, empty components).

        Covers: the running venv's site-packages, the repo root (computed
        independently via parents[2] so an off-by-one in _zeloo_repo_root
        cannot silently pass), duplicate Zeloo entries, all-owned input
        (PYTHONPATH key removed), and mixed user/Zeloo ordering with an
        ...
        for input_entries, expected in cases:
            ...
            _strip_zeloo_owned_pythonpath(env)
            if expected is None:
    ...
    def test_non_owned_entries_preserved(self, user_pp):
        """Anything not proven Zeloo-owned is preserved byte-for-byte.
    ...
    def test_non_owned_runtime_shaped_entries_preserved(self):
        ...
        different interpreter version, a descendant of the Zeloo venv
        ...
        off-by-one in _zeloo_repo_root cannot silently pass; no launcher
    ...
    def test_windows_backslash_paths(self):
        """Windows-style backslash paths are handled for Zeloo-owned entries.
        ...
        Zeloo-owned — the critical invariant is that user Windows paths
        (including site-packages paths for another Python version) are
        never destroyed.  On a real Windows host, Path splits on
        backslashes and Zeloo venv site-packages entries are stripped
        by the same Zeloo-owned check (covered by the Windows-only test
        below).
        """
        from tools.environments.local_pythonpath import _strip_zeloo_owned_pythonpath
        import sys
        ...
        pyver = f"python{sys.version_info[0]}.{sys.version_info[1]}"
        zeloo_win = f"C:\\\\Users\\\\u\\\\.Zeloo\\\\Zeloo-agent\\\\venv\\\\lib\\\\{pyver}\\\\site-packages"
        user_win = "D:\\\\user\\\\lib"
        env = {
            "PYTHONPATH": ";".join([zeloo_win, user_win]),
        }
        ...
        # the Zeloo-owned check cannot match a backslash path here.
        assert zeloo_win in entries
        assert user_win in entries

    @pytest.mark.windows_only
    def test_windows_zeloo_owned_paths_stripped(self):
        """On Windows, a Zeloo venv site-packages entry written with
        backslashes is stripped by the same Zeloo-owned check, while a
        ...
        _strip_zeloo_owned_pythonpath(env)
        entries = env["PYTHONPATH"].split(";")
        assert zeloo_win not in entries
        assert user_win in entries
    ...
    def test_base_python_sanitizer_uses_validated_separate_runtime_venv(self, tmp_path, monkeypatch):
        ...
        This deliberately uses a synthetic Zeloo venv separate from the test
        runner: sys.prefix represents base Python, while validated VIRTUAL_ENV
        identifies ``<repo>/venv`` as the Zeloo runtime producer contract.
        """
        import tools.environments.local as local
        from tools.environments import local_pythonpath

        repo_root = tmp_path / "Zeloo-agent"
        ...
        monkeypatch.setattr(local, "_zeloo_repo_root_aliases", (repo_root,))
        monkeypatch.setattr(local, "_in_venv", False)
        monkeypatch.setattr(local, "_zeloo_site_packages", None)
    ...
    def test_unrelated_virtual_env_is_not_runtime_provenance(self, tmp_path, monkeypatch):
        ...
        local_pythonpath._strip_zeloo_owned_pythonpath(env)
    ...
    @pytest.mark.parametrize("builder", [
        ...
        "zeloo_subprocess_env",
    ])
    def test_builders_strip_zeloo_venv_pythonpath(self, builder):
        """Every subprocess env builder applies the same sanitation contract:
        Zeloo venv site-packages is stripped, user entries survive.
        """
        ...
        with patch.dict(os.environ, seed, clear=True):
            if builder == "_make_run_env":
                ...
                result = local_mod.zeloo_subprocess_env()
    ...
    def test_scrub_child_env_strips_zeloo_venv_pythonpath(self):
        """execute_code's _scrub_child_env path: after scrubbing, Zeloo venv
        site-packages entries should be stripped when
        _strip_zeloo_owned_pythonpath is applied (as the spawn path does),
        ...
        _strip_zeloo_owned_pythonpath(scrubbed)
    ...
    def test_execute_code_composition_strips_inherited_zeloo_entries(self, same_env):
        """Integration: execute_code's real spawn path composes a clean PYTHONPATH.

        Seeds a contaminated inherited PYTHONPATH (Zeloo repo root + Zeloo
        ...
        * inherited Zeloo venv site-packages never survive into the sandbox;
        ...
        * user entries survive after the controlled entries.
        ...
        zeloo_root = str(Path(cet.__file__).resolve().parents[1])
        venv_sp = str(_running_venv_site_packages())
        ...
        with patch("tools.code_execution_tool._load_config",
             ...
             patch("tools.code_execution_env._uses_zeloo_python_environment",
                   return_value=same_env), \
             patch("subprocess.Popen", side_effect=_fake_popen), \
             patch.dict(os.environ, {
                 "PYTHONPATH": os.pathsep.join(
                     [zeloo_root, venv_sp, user_a, user_b]),
             }):
        ...
        norm_root = os.path.normcase(zeloo_root)
        ...
        assert norm_venv not in norm_parts, \
            "inherited Zeloo venv site-packages must be stripped"
    ...
    def test_configured_home_alias_matches_launcher_output(self, tmp_path, monkeypatch):
        ...
        import tools.environments.local as local
        from tools.environments import local_pythonpath
        from zeloo_cli.gateway_windows import _preserve_zeloo_home_path
        ...
        monkeypatch.setenv("ZELOO_HOME", str(configured_home))

        launcher_entry = Path(_preserve_zeloo_home_path(physical_root))
        aliases = local_pythonpath._build_zeloo_repo_root_aliases(
            physical_root.resolve(),
            physical_root,
            configured_home,
        )

        assert launcher_entry == configured_home / "Zeloo-agent"
        assert launcher_entry in aliases

        monkeypatch.setattr(local, "_zeloo_repo_root_aliases", aliases)
    ...
    def test_profile_rehome_keeps_junction_lexical_alias(self, tmp_path, monkeypatch):
        ...
        The desktop/CLI spawn children with ZELOO_HOME and PYTHONPATH in the
        configured (junction) spelling, but --profile / sticky active_profile
        re-home ZELOO_HOME through resolve_profile_env() before the
        ...
        physical_home = tmp_path / "physical-home"
        physical_root = physical_home / "Zeloo-agent"
        physical_root.mkdir(parents=True)
        ...
        monkeypatch.setenv("ZELOO_HOME", str(configured_home))
        lexical_root = configured_home / "Zeloo-agent"
        ...
        # The sanitizer now runs under the re-homed (profile) ZELOO_HOME.
        aliases = local_pythonpath._build_zeloo_repo_root_aliases(
            physical_root.resolve(),
            physical_root,
    ...
    def test_repo_level_junction_recovers_lexical_alias(self, tmp_path, monkeypatch):
        ...
        (e.g. D:\\Zeloo\\Zeloo-agent -> C:\\...\\Zeloo-agent) while the
        ...
        # repo-level link: <configured-home>/Zeloo-agent -> physical repo
        try:
            _make_directory_link(configured_home / "Zeloo-agent", physical_root)
        except OSError as exc:
            pytest.skip(f"directory link unavailable on this host: {exc}")
    ...
    def test_same_named_non_owned_directories_preserved(self, tmp_path, monkeypatch):
        ...
        physical_root = _physical_repo_root(tmp_path)
        configured_home = tmp_path / "configured-home"
        (configured_home / "Zeloo-agent").mkdir(parents=True)
        unrelated = tmp_path / "user-tools" / "Zeloo-agent"
        ...
        for lookalike in (configured_home / "Zeloo-agent", unrelated):
    ...
    def test_profile_home_with_repo_level_junction(self, tmp_path, monkeypatch):
        ...
        is <root>/profiles/<name> while the repo is a link at <root>/Zeloo-agent.
        ...
        try:
            _make_directory_link(configured_root / "Zeloo-agent", physical_root)
        except OSError as exc:
            pytest.skip(f"directory link unavailable on this host: {exc}")

        configured_home = configured_root / "profiles" / "coder"
        lexical_root = configured_root / "Zeloo-agent"
        aliases = local_pythonpath._build_zeloo_repo_root_aliases(
            physical_root.resolve(),
            physical_root,
            configured_home,
        ...
        assert any(local_pythonpath._same_path(a, lexical_root) for a in aliases)
        assert not any(local_pythonpath._same_path(a, configured_home / "Zeloo-agent") for a in aliases)
    ...
    def test_validated_runtime_venv_lexical_after_repo_recovery(self, tmp_path, monkeypatch):
        ...
        local._zeloo_site_packages = None
...
class TestPythonhomeSanitized:
    """PYTHONHOME must not leak from the Zeloo runtime into subprocesses.
    ...
    inherits it redirects its stdlib search to the Zeloo venv and crashes
    ...
    def test_builders_strip_pythonhome(self, builder):
        ...
        with patch.dict(os.environ, seed, clear=True):
            if builder == "_make_run_env":
                ...
                result = local_mod._sanitize_subprocess_env(dict(os.environ))
            elif builder == "zeloo_subprocess_env":
                result = local_mod.zeloo_subprocess_env()
            else:
    ...
    def test_build_subprocess_env_no_scrub_preserves_pythonhome(self):
        ...
        base = {
            ...
            "HOME": "/home/user",
            "PYTHONHOME": "/opt/Zeloo-venv",
            "VIRTUAL_ENV": "/opt/Zeloo-venv",
            "SERVICE_TOKEN": "s3cr3t",
        }
        result = build_subprocess_env(base, scrub_secrets=False)
        assert result.get("PYTHONHOME") == "/opt/Zeloo-venv"
        assert result.get("VIRTUAL_ENV") == "/opt/Zeloo-venv"
...
class TestBlocklistCoverage:
    ...
    def test_issue_1002_offenders(self):
        ...
        assert must_block.issubset(_ZELOO_PROVIDER_ENV_BLOCKLIST)
    ...
    def test_registry_vars_are_in_blocklist(self):
        ...
        by the user's Claude Code install, not Zeloo (#55878).
        """
        from zeloo_cli.auth import PROVIDER_REGISTRY
        ...
        for pconfig in PROVIDER_REGISTRY.values():
            for var in pconfig.api_key_env_vars:
                if var in exempt:
                    continue
                assert var in _ZELOO_PROVIDER_ENV_BLOCKLIST, (
                    f"Registry var {var} (provider={pconfig.id}) missing from blocklist"
                )
            if pconfig.base_url_env_var:
                assert pconfig.base_url_env_var in _ZELOO_PROVIDER_ENV_BLOCKLIST, (
    ...
    def test_bedrock_bearer_token_is_in_blocklist(self):
        """auth_type='aws_sdk' providers contribute their Zeloo-managed
        ...
        assert "AWS_BEARER_TOKEN_BEDROCK" in _ZELOO_PROVIDER_ENV_BLOCKLIST
    ...
    def test_general_aws_chain_not_in_blocklist(self):
        ...
        operator shell (SECURITY.md §3.2), not to Zeloo, and blocklisting
        ...
        leaked_block = general_chain & _ZELOO_PROVIDER_ENV_BLOCKLIST
    ...
    def test_claude_code_oauth_token_is_inheritable(self):
        ...
        assert "CLAUDE_CODE_OAUTH_TOKEN" not in _ZELOO_PROVIDER_ENV_BLOCKLIST
    ...
    def test_optional_tool_and_messaging_vars_are_in_blocklist(self):
        ...
        from zeloo_cli.config import OPTIONAL_ENV_VARS

        for name, metadata in OPTIONAL_ENV_VARS.items():
            category = metadata.get("category")
            if category in {"tool", "messaging"}:
                assert name in _ZELOO_PROVIDER_ENV_BLOCKLIST, (
...
class TestSanePathIncludesHomebrew:
    ...
    def _disable_zeloo_bin_injection(self):
        """These tests assert the sane-path merge in isolation. Disable the
        Zeloo-install-dir prepend (a separate concern, covered by
        TestZELOOBinDirOnPath) so a real ``Zeloo`` on the test runner's PATH
        doesn't shift the asserted PATH layout."""
        from tools.environments import local as local_mod
        saved = local_mod._ZELOO_BIN_DIR
        local_mod._ZELOO_BIN_DIR = None  # resolved -> no dir to inject
        yield
        local_mod._ZELOO_BIN_DIR = saved
...
class TestZELOOBinDirOnPath:
    """The Zeloo install dir is reachable in the terminal subshell PATH.

    Plugins shelling out to bare ``Zeloo`` via the terminal tool must work
    even when the gateway was launched without the Zeloo install dir on
    PATH (systemd, service managers, cron). See the discussion that motivated
    _resolve_zeloo_bin_dir / _prepend_zeloo_bin_dir.
    """

    def _reset_cache(self):
        from tools.environments import local as local_mod
        local_mod._ZELOO_BIN_DIR = local_mod._SENTINEL

    def test_resolves_via_which(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        monkeypatch.setattr(local_mod.shutil, "which",
                            lambda name: "/opt/Zeloo/bin/Zeloo" if name == "Zeloo" else None)
        monkeypatch.setattr(local_mod.os.path, "isdir", lambda p: p == "/opt/Zeloo/bin")
        assert local_mod._resolve_zeloo_bin_dir() == "/opt/Zeloo/bin"


    def test_prepend_noop_when_unresolved(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        local_mod._ZELOO_BIN_DIR = None
        assert local_mod._prepend_zeloo_bin_dir("/usr/bin:/bin") == "/usr/bin:/bin"

    def test_make_run_env_injects_zeloo_bin_dir(self):
        """A gateway env missing the Zeloo dir gets it back in the subshell PATH.

        Platform-agnostic: ``_prepend_zeloo_bin_dir`` uses ``os.pathsep`` on
        every host, so no platform flag is faked here."""
        from tools.environments import local as local_mod
        from tools.environments.local import _make_run_env
        self._reset_cache()
        local_mod._ZELOO_BIN_DIR = "/opt/Zeloo/bin"
        with patch.dict(
            os.environ,
            {"PATH": os.pathsep.join(["/usr/bin", "/bin"])},
            clear=True,
        ...
        assert entries[0] == "/opt/Zeloo/bin"
        assert "/usr/bin" in entries


class TestZELOOInternalDynamicSecrets:
    """Dynamically-named Zeloo secrets injected at gateway/CLI startup must
    not leak into terminal subprocesses.

    The static ``_ZELOO_PROVIDER_ENV_BLOCKLIST`` is name-based and derived
    ...
    ``_is_zeloo_internal_secret`` is the single source of truth; every spawn
    path (``_sanitize_subprocess_env``, ``_make_run_env``,
    ``zeloo_subprocess_env``, Docker forward filter, ``env_passthrough``)
    consults it. These tests exercise the terminal execute path + predicate.
    """

    def test_predicate_matches_auxiliary_api_key(self):
        from tools.environments.local_env_policy import _is_zeloo_internal_secret
        assert _is_zeloo_internal_secret("AUXILIARY_VISION_API_KEY")
        assert _is_zeloo_internal_secret("AUXILIARY_WEB_EXTRACT_API_KEY")
        assert _is_zeloo_internal_secret("AUXILIARY_APPROVAL_API_KEY")
        # plugin-registered task names are covered by the pattern
        assert _is_zeloo_internal_secret("AUXILIARY_MY_PLUGIN_TASK_API_KEY")

    def test_predicate_matches_auxiliary_base_url(self):
        from tools.environments.local_env_policy import _is_zeloo_internal_secret
        assert _is_zeloo_internal_secret("AUXILIARY_VISION_BASE_URL")
        assert _is_zeloo_internal_secret("AUXILIARY_COMPRESSION_BASE_URL")

    def test_predicate_matches_gateway_relay_auth(self):
        from tools.environments.local_env_policy import _is_zeloo_internal_secret
        assert _is_zeloo_internal_secret("GATEWAY_RELAY_SECRET")
        assert _is_zeloo_internal_secret("GATEWAY_RELAY_DELIVERY_KEY")
        assert _is_zeloo_internal_secret("GATEWAY_RELAY_SESSION_TOKEN")
    ...
    def test_predicate_allows_auxiliary_non_secrets(self):
        ...
        from tools.environments.local_env_policy import _is_zeloo_internal_secret
        assert not _is_zeloo_internal_secret("AUXILIARY_VISION_PROVIDER")
        assert not _is_zeloo_internal_secret("AUXILIARY_VISION_MODEL")
        assert not _is_zeloo_internal_secret("GATEWAY_RELAY_URL")
        assert not _is_zeloo_internal_secret("GATEWAY_RELAY_PLATFORMS")
        assert not _is_zeloo_internal_secret("GATEWAY_RELAY_ID")  # not a secret suffix
        # unrelated vars pass through
        assert not _is_zeloo_internal_secret("PATH")
        assert not _is_zeloo_internal_secret("MY_APP_KEY")
    ...
    def test_gateway_relay_static_names_in_blocklist(self):
        ...
        assert "GATEWAY_RELAY_SECRET" in _ZELOO_PROVIDER_ENV_BLOCKLIST
        assert "GATEWAY_RELAY_DELIVERY_KEY" in _ZELOO_PROVIDER_ENV_BLOCKLIST
        assert "GATEWAY_RELAY_ID" in _ZELOO_PROVIDER_ENV_BLOCKLIST
