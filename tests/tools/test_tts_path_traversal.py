...
def test_output_path_rejects_zeloo_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    zeloo_home = tmp_path / "Zeloo-home"
    zeloo_home.mkdir()
    monkeypatch.setattr(file_safety, "_zeloo_home_path", lambda: zeloo_home)
    monkeypatch.setattr(file_safety, "_zeloo_root_path", lambda: zeloo_home)

    target = zeloo_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    zeloo_home = tmp_path / "Zeloo-home"
    token_dir = zeloo_home / "mcp-tokens"
