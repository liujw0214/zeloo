...
class TestWriteDenyExactPaths:
    ...
    def test_zeloo_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.Zeloo/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "zeloo_root"
        ...
        # Sanity check: ZELOO_HOME does point to the profile dir, not the root.
        from zeloo_constants import get_zeloo_home, get_default_zeloo_root
...
class TestWriteAllowed:
    ...
    def test_zeloo_control_files_requested_writable(self):
