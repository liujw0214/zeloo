...
was gated on ``[ -f "$ZELOO_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        f"{env_setup}"
        f'ZELOO_HOME="{home}"\n'
        # In tests we run unprivileged; as_ZELOO is a passthrough then.
        'as_ZELOO() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    Zeloo loads $ZELOO_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/Zeloo/.env.example -> $ZELOO_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $ZELOO_HOME/.env from it on first boot"
    )
