...
Covers:
- :class:`ZELOOTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.ZELOOMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _ZELOO_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# ZELOOTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = ZELOOTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ZELOO_HOME", str(tmp_path))
        storage = ZELOOTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (ZELOOMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: ZELOOTokenStorage, **context_attrs):
    ...
    provider = _ZELOO_PROVIDER_CLS.__new__(_ZELOO_PROVIDER_CLS)
    provider._zeloo_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ZELOO_HOME", str(tmp_path))
        storage = ZELOOTokenStorage("mgr-srv")
        ...
        with patch.object(
            _ZELOO_PROVIDER_CLS.__bases__[-1], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ZELOO_HOME", str(tmp_path))
        storage = ZELOOTokenStorage("flow-srv")
        ...
        with patch.object(
            _ZELOO_PROVIDER_CLS.__bases__[-1],
            "async_auth_flow",
