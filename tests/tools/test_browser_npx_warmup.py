...
warm_agent_browser_npx_cache() is the fire-and-forget helper `Zeloo update` /
`Zeloo doctor --fix` call to pre-fetch agent-browser via npx so the first real
...
`Zeloo update` — not only when a browser tool is actually used), must pass
...
def test_captures_stdout_and_stderr_instead_of_inheriting_parent_fds():
    """The npx registry fetch runs on every `Zeloo update` — its stdout/
...
def test_uses_credential_scrubbed_environment():
    ...
    os.environ with every provider/gateway credential Zeloo holds."""
...
def test_merges_extended_path_so_managed_only_npx_can_find_sibling_node():
    """If npx was resolved via the Zeloo-managed/extended search (not the
    ...
    with patch("tools.browser_tool_install._resolve_npx_bin", return_value="/opt/Zeloo/node/bin/npx"), \
         patch("tools.browser_tool._build_browser_env", return_value={"PATH": "/usr/bin"}), \
         patch(
             "tools.browser_tool_install._merge_browser_path",
             return_value="/opt/Zeloo/node/bin:/usr/bin",
         ) as mock_merge, \
         patch("subprocess.Popen", return_value=_mock_proc()) as mock_popen:
        warm_agent_browser_npx_cache()
    ...
    _args, kwargs = mock_popen.call_args
    assert kwargs["env"]["PATH"] == "/opt/Zeloo/node/bin:/usr/bin"
...
def test_timeout_cleanup_communicate_itself_raising_does_not_propagate():
    ...
    fire-and-forget caller (zeloo_cli/doctor.py calls this bare)."""
    proc = _mock_proc(
        communicate_side_effect=[
            subprocess.TimeoutExpired(cmd=["npx"], timeout=60.0),
...
def test_returns_false_instead_of_raising_on_unexpected_communicate_exception():
    """Fire-and-forget contract: zeloo_cli/doctor.py calls this bare (no
