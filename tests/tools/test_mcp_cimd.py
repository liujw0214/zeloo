...
Under CIMD the ``client_id`` is the HTTPS URL of a document Zeloo publishes,
...
the conditions under which Zeloo is allowed to present it.
...
def test_document_client_id_is_the_url_zeloo_sends():
    """A CIMD document is only valid when its client_id is its own URL."""
    assert _document()["client_id"] == _CIMD_CLIENT_METADATA_URL


def test_document_declares_every_callback_zeloo_can_build(tmp_path, monkeypatch):
...
def test_generated_redirect_uri_is_registered_in_the_document(tmp_path, monkeypatch):
    ...
    _configure_callback_port(cfg, ZELOOTokenStorage("srv"))
...
def test_eligible_flow_gets_a_pinned_port(tmp_path, monkeypatch, private_ports):
    ...
    result = _maybe_use_cimd({}, ZELOOTokenStorage("srv"))
...
def test_concurrent_servers_get_different_pinned_ports(
    ...
    ports = {
        _maybe_use_cimd({}, ZELOOTokenStorage(f"srv-{i}"))[1]
...
def test_self_hosted_document_url_overrides_the_default(
    ...
    result = _maybe_use_cimd(cfg, ZELOOTokenStorage("srv"))
...
def test_config_that_conflicts_with_the_document_falls_back_to_dcr(
    ...
    assert _maybe_use_cimd(dict(cfg), ZELOOTokenStorage("srv")) is None
...
def test_exhausted_port_range_falls_back_to_dcr(tmp_path, monkeypatch, private_ports):
    ...
    try:
        cfg: dict = {}
        port = _configure_callback_port(cfg, ZELOOTokenStorage("srv"))
    finally:
        for sock in squatters:
            sock.close()
...
def test_more_servers_than_pinned_ports_all_get_cimd(
    ...
    results = [
        _maybe_use_cimd({}, ZELOOTokenStorage(f"srv-{i}"))
...
def test_cached_metadata_decides_whether_to_pin(
    ...
    long after Zeloo fixes the redirect URI. Metadata cached by an earlier
...
def test_cached_pinned_port_is_not_handed_to_a_sibling_server(
    ...
    monkeypatch.setenv("ZELOO_HOME", str(tmp_path))
    settled = ZELOOTokenStorage("settled")
    ...
    fresh = _maybe_use_cimd({}, ZELOOTokenStorage("fresh"))
...
def test_sdk_chooses_cimd_only_when_the_server_advertises_it(
    ...
    """Closes the loop on the handoff: feed what Zeloo configured into the
...
def test_reauthorizing_clears_the_rejection(tmp_path, monkeypatch, private_ports):
    """`Zeloo mcp login` wipes stored state, so a fixed document is retried."""
    monkeypatch.setenv("ZELOO_HOME", str(tmp_path))
    storage = ZELOOTokenStorage("srv")
...
def test_rejected_dcr_client_leaves_cimd_available(
    ...
    assert provider.context.client_metadata_url == _CIMD_CLIENT_METADATA_URL
    assert not ZELOOTokenStorage("srv").cimd_rejected()
...
def test_timeout_on_a_cimd_flow_names_the_document_and_the_escape_hatch(monkeypatch):
    ...
    symptom Zeloo sees is the callback timing out."""
