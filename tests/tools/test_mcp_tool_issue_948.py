...
def test_resolve_stdio_command_falls_back_to_zeloo_node_bin(tmp_path):
    ...
    with patch("tools.mcp_tool_config.shutil.which", return_value=None), \
         patch.dict("os.environ", {"ZELOO_HOME": str(tmp_path)}, clear=False):
...
def test_resolve_stdio_command_falls_back_to_usr_local_bin():
    """When ``npx`` isn't on the filtered PATH and isn't under ``$ZELOO_HOME/node/bin``
    ...
    the upstream ``node:bookworm-slim`` image (which the Zeloo Docker image
    ...
    # the other candidates ($ZELOO_HOME/node/bin/npx and ~/.local/bin/npx)
