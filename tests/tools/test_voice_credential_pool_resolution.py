...
Keys added via ``Zeloo auth add <provider>`` live in the credential pool /
auth store and used to be invisible to the voice tools, which only read
``os.environ`` + ``~/.Zeloo/.env`` via ``get_env_value``. The shared
...
def _no_dotenv(monkeypatch):
    """Keep the developer's real ~/.Zeloo/.env out of these tests."""
