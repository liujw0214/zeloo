...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_env._scrub_child_env — broad ZELOO_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: ZELOO_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_zeloo_allowlist_and_secret_blocks():
    from tools.code_execution_env import _scrub_child_env

    env = {
        # operational allowlist → kept
        "ZELOO_HOME": "/h", "ZELOO_PROFILE": "p",
        "ZELOO_CONFIG": "/c.yaml", "ZELOO_ENV": "/e",
        "ZELOO_DELEGATED_CHILD_CONTEXT": "1",
        # other ZELOO_* → dropped (broad prefix removed)
        "ZELOO_BASE_URL": "https://x", "ZELOO_INTERACTIVE": "1",
    ...
    for kept in (
        ...
        "ZELOO_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "ZELOO_BASE_URL", "ZELOO_INTERACTIVE", "ZELOO_KANBAN_TASK",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped ZELOO_* vars."""
    import logging
