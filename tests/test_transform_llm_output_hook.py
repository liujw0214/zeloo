...
from zeloo_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(zeloo_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <zeloo_home>/plugins/<name> and opt it in."""
    plugin_dir = zeloo_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ZELOO_HOME", str(tmp_path / "zeloo_empty"))
