...
- ``Zeloo verify`` records into the evidence ledger (pass and fail),
- a passing run satisfies the verify-on-stop guard,
- the verify-on-stop nudge names ``Zeloo verify --json`` when the workspace
...
def _ledger_on(monkeypatch):
    ...
    monkeypatch.setenv("ZELOO_VERIFY_ON_STOP", "1")
...
def _workspace(tmp_path, *, scripts=None, manifest_recipe=None):
    ...
    if manifest_recipe is not None:
        zeloo_dir = project / ".Zeloo"
        zeloo_dir.mkdir()
        (zeloo_dir / "environment.json").write_text(
...
def test_record_verify_run_marks_workspace_passed(zeloo_home):
    project = _workspace(zeloo_home)
    ...
    assert status["evidence"]["canonical_command"] == "Zeloo verify"


def test_record_verify_run_records_failure(zeloo_home):
    project = _workspace(zeloo_home)
...
def test_cli_passing_run_writes_ledger_evidence(zeloo_home, capsys):
    project = _workspace(zeloo_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
def test_cli_failing_run_writes_failed_evidence(zeloo_home, capsys):
    project = _workspace(zeloo_home, manifest_recipe={"name": "Fake", "test": ["false"]})
    code = run_verify_command(make_args(project))
    assert code == 1
    status = verification_status(session_id=None, cwd=project)
    assert status["status"] == "failed"
...
def test_cli_partial_run_records_targeted_scope(zeloo_home, capsys):
...
def test_cli_run_uses_zeloo_session_id_env(zeloo_home, capsys, monkeypatch):
    monkeypatch.setenv("ZELOO_SESSION_ID", "sess-42")
    project = _workspace(zeloo_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
# ---------------------------------------------------------------------------
# closed loop: edit -> stop guard nudge -> Zeloo verify -> guard satisfied
# ---------------------------------------------------------------------------


def test_passing_verify_run_satisfies_stop_guard(zeloo_home, capsys):
    project = _workspace(zeloo_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
# ---------------------------------------------------------------------------
# nudge wording: recipe-aware `Zeloo verify --json` suggestion
# ---------------------------------------------------------------------------


def test_nudge_mentions_zeloo_verify_when_recipe_has_start(zeloo_home):
    project = _workspace(zeloo_home, scripts={"test": "vitest", "dev": "vite"})
    changed = str(project / "src" / "app.ts")
    ...
    assert "Zeloo verify --json" in nudge
    # The cheap verify commands are still listed first.
    assert "npm run test" in nudge


def test_nudge_mentions_zeloo_verify_when_manifest_exists(zeloo_home):
    # No start script, but a saved .Zeloo/environment.json qualifies.
    project = _workspace(
        zeloo_home,
        scripts={"test": "vitest"},
...
def test_nudge_keeps_plain_wording_without_recipe_start(zeloo_home):
    # Verify commands but no start script and no manifest: today's wording.
    project = _workspace(zeloo_home, scripts={"test": "vitest"})
    changed = str(project / "src" / "app.ts")
    mark_workspace_edited(session_id="s1", cwd=project, paths=[changed])
    nudge = build_verify_on_stop_nudge(session_id="s1", changed_paths=[changed])
    assert nudge is not None
    assert "Zeloo verify" not in nudge


def test_nudge_recipe_detection_failure_is_silent(zeloo_home, monkeypatch):
...
def test_detect_path_merges_project_facts_commands(zeloo_home, capsys):
    project = _workspace(zeloo_home)  # package.json with no scripts
...
def test_manifest_recipe_is_not_merged(zeloo_home, capsys):
...
def test_merge_skips_commands_recipe_already_has(zeloo_home, capsys):
    project = _workspace(zeloo_home, scripts={"test": "vitest"})
    code = run_verify_command(make_args(project, detect_only=True))
    assert code == 0
    payload = json.loads(capsys.readouterr().out)
