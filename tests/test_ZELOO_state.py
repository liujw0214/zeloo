"""Tests for zeloo_state.py — SessionDB SQLite CRUD, FTS5 search, export."""
...
import zeloo_state
import zeloo_state_wal
import zeloo_state_common
from agent.session_activity import ActivityProvenance, build_activity_snapshot
from zeloo_state import SessionDB
from zeloo_state_common import FTS_SQL, FTS_STORAGE_VERSION, SCHEMA_SQL, SCHEMA_VERSION
...
class TestConnectionLifecycle:
    def test_failed_writable_open_does_not_leak_tracked_connection(
        ...
        from zeloo_cli.sqlite_safe_read import has_live_connection

        db_path = tmp_path / "state.db"
        opened = []
        real_connect = zeloo_state._connect_tracked_db
        ...
        monkeypatch.setattr(zeloo_state, "_connect_tracked_db", capture_connect)
        monkeypatch.setattr(
            SessionDB,
            "_init_schema",
    ...
    def test_failed_wal_read_open_does_not_leak_tracked_connection(
        ...
        from zeloo_cli import sqlite_safe_read

        db_path = tmp_path / "state.db"
        db = SessionDB(db_path=db_path)
        opened = []
        real_connect = zeloo_state._connect_tracked_db
        real_pragmas = zeloo_state.apply_database_pragmas
        ...
        monkeypatch.setattr(zeloo_state, "_connect_tracked_db", capture_connect)
        monkeypatch.setattr(zeloo_state, "apply_database_pragmas", fail_pragmas)
        ...
        try:
            ...
            monkeypatch.setattr(
                zeloo_state, "apply_database_pragmas", real_pragmas
            )
            for conn in opened:
                try:
                    conn.close()
    ...
    def test_read_only_open_retries_transient_wal_ioerr(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(zeloo_state, "_connect_tracked_db", flaky_connect)
        # Keep the test fast: one backoff tick is enough; the retry budget
        # itself is exercised by the attempt count below.
        monkeypatch.setattr(zeloo_state, "_READ_ONLY_IOERR_RETRY_BACKOFF_S", 0.0)
    ...
    def test_read_only_open_exhausts_retry_budget_for_persistent_ioerr(
        ...
        monkeypatch.setattr(zeloo_state, "_connect_tracked_db", bad_connect)
        monkeypatch.setattr(zeloo_state, "_READ_ONLY_IOERR_RETRY_BACKOFF_S", 0.0)
        budget = zeloo_state._READ_ONLY_IOERR_RETRY_ATTEMPTS
...
class TestSessionLifecycle:
    ...
    def test_cjk_search_falls_back_to_like_when_trigram_unavailable(
        ...
        monkeypatch.setattr("zeloo_state.sqlite3.connect", connect_without_trigram)
        db = SessionDB(db_path=db_path)
        try:
            db.create_session(session_id="s1", source="cli")
...
class TestSessionTitleLineage:
    ...
    def test_projected_tip_inherits_root_title_when_untitled(self, db):
        ...
        `Zeloo peer dm` -> canonical "Bot Chat", #106165). A titled tip keeps its own title."""
        import time as _time
        self._make_compression_chain(db, _time.time() - 3600)
...
class TestSchemaInit:
    def test_wal_mode(self, db):
        ...
        from zeloo_state_wal import is_sqlite_wal_reset_vulnerable
    ...
    def test_schema_sql_is_source_of_truth(self, db):
        ...
        """
        from zeloo_state_common import SCHEMA_SQL
...
class TestReconcileColumnsErrorHandling:
    ...
    def test_duplicate_column_race_stays_quiet(self, tmp_path, caplog):
        ...
        try:
            ...
            with caplog.at_level(logging.WARNING, logger="zeloo_state"):
                stale._reconcile_columns(cursor)
        finally:
            conn.close()
        assert not [
...
class TestFtsRebuildLoopWithoutTrigram:
    ...
    interrupted ``Zeloo sessions optimize-storage`` silently lost its place
    ...
    def _trace(monkeypatch, statements, *, trigram):
        ...
        monkeypatch.setattr("zeloo_state.sqlite3.connect", connect)
    ...
    def test_fts_trigger_subsets_match_the_ddl(self):
        ...
        """
        from zeloo_state_common import (
            FTS_SQL,
            FTS_TRIGRAM_SQL,
            LEGACY_FTS_SQL,
        ...
        from zeloo_state_schema import _FTS_BASE_TRIGGERS, _FTS_TRIGRAM_TRIGGERS
...
class TestListSessionsRich:
    ...
    def test_rich_list_keeps_legacy_reset_children_visible(self, db, end_reason):
        from zeloo_state_common import _ephemeral_child_sql
    ...
    # tests/zeloo_state/test_resolve_resume_session_id.py
...
class TestVacuum:
    ...
    def test_auto_maintenance_freelist_ratio_exactly_at_threshold_skips(self, db, monkeypatch):
        ...
        from zeloo_state_common import AUTO_VACUUM_MIN_FREELIST_RATIO
...
class TestFTS5ToolCallMigration:
    ...
    def test_v10_to_v11_upgrade_backfills_tool_fields(self, tmp_path):
        ...
        try:
            ...
            # `Zeloo db optimize` performs the v23 transition; afterwards the
...
class TestFTSExternalContentMigration:
    ...
    def _simulate_pre_fix_demote_crash_window(self, db):
        ...
        """
        from zeloo_state_common import FTS_SQL, FTS_TRIGRAM_SQL
    ...
    def test_optimize_resume_after_demote_crash_window_restores_search(
        ...
        try:
            ...
            assert db.get_meta("fts_storage_version") == str(
                zeloo_state_common.FTS_STORAGE_VERSION
            )
            assert db._conn.execute(
    ...
    def test_optimize_heals_premature_stamp_with_empty_index(self, tmp_path):
        ...
        try:
            ...
            db._conn.execute(
                ...
                "('fts_storage_version', ?) "
                "ON CONFLICT(key) DO UPDATE SET value = excluded.value",
                (str(zeloo_state_common.FTS_STORAGE_VERSION),),
            )
            db._conn.commit()
...
class TestApplyWalProbe:
    ...
    def _assume_fixed_sqlite(self, monkeypatch):
        ...
        monkeypatch.setattr(
            zeloo_state_wal, "is_sqlite_wal_reset_vulnerable", lambda version_info=None: False
        )
...
def test_peer_fallback_never_adopts_a_sibling_profiles_row(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(zeloo_state, "DEFAULT_DB_PATH", zeloo_state._IMPORT_DEFAULT_DB_PATH)
...
def test_refresh_compression_lock_requires_holder_and_preserves_reclaimability(db, monkeypatch):
    ...
    monkeypatch.setattr(zeloo_state.time, "time", lambda: 1000.0)
    ...
    monkeypatch.setattr(zeloo_state.time, "time", lambda: 1005.0)
    ...
    monkeypatch.setattr(zeloo_state.time, "time", lambda: 1016.0)
...
def test_refresh_cannot_resurrect_a_lock_already_reclaimed(db, monkeypatch):
    ...
    monkeypatch.setattr(zeloo_state.time, "time", lambda: 1020.0)
    assert db.try_acquire_compression_lock("s1", "holder-b", ttl_seconds=10.0) is True
...
class TestGetMessagesPagination:
    ...
    def test_resume_safety_counts_active_rows_across_lineage(self, db):
        ...
        with pytest.raises(zeloo_state.SessionResumeTooLargeError) as exc_info:
    ...
    def test_resume_safety_tip_only_counts_the_tip_segment(self, db):
        ...
        with pytest.raises(zeloo_state.SessionResumeTooLargeError) as full:
        ...
        with pytest.raises(zeloo_state.SessionResumeTooLargeError) as tip:
    ...
    def test_export_safety_is_bounded_to_the_requested_active_segment(self, db):
        ...
        with pytest.raises(zeloo_state.SessionExportTooLargeError) as exc_info:
    ...
    def test_zero_limit_disables_resume_and_export_guards(self, db, monkeypatch):
        ...
        with pytest.raises(zeloo_state.SessionResumeTooLargeError):
            db.assert_resume_safe("big", max_messages=2)
        with pytest.raises(zeloo_state.SessionExportTooLargeError):
        ...
        # effect only).
        monkeypatch.setattr(zeloo_state, "resolved_max_resume_messages", lambda: 0)
        monkeypatch.setattr(zeloo_state, "resolved_max_export_messages", lambda: 0)
    ...
    def test_guard_limits_resolve_from_config_at_call_time(self, db, monkeypatch):
        ...
        monkeypatch.setattr(zeloo_state, "resolved_max_resume_messages", lambda: 3)
        monkeypatch.setattr(zeloo_state, "resolved_max_export_messages", lambda: 3)
        with pytest.raises(zeloo_state.SessionResumeTooLargeError) as resume_exc:
            db.assert_resume_safe("cfg")
        assert resume_exc.value.limit == 3
        with pytest.raises(zeloo_state.SessionExportTooLargeError) as export_exc:
...
class TestGatewayRoutingPkHeal:
    ...
    def test_legacy_pk_rebuilt_to_composite(self, tmp_path):
        db_path = self._make_legacy_db(
            tmp_path, rows=[("/home/u/.Zeloo/sessions", "agent:main:telegram:dm:1", "{}", 1.0)]
        )
        db = SessionDB(db_path=db_path)
        try:
            assert self._pk_cols(db) == ["scope", "session_key"]
            # Existing rows survive the rebuild.
            entries = db.load_gateway_routing_entries(scope="/home/u/.Zeloo/sessions")
...
class TestApplyDatabasePragmas:
    ...
    def _patch_cfg(monkeypatch, cfg):
        monkeypatch.setattr(
            "zeloo_cli.config.load_config_readonly",
            lambda: cfg,
        )

    def test_honors_wal_autocheckpoint_from_config(self, tmp_path, monkeypatch):
        import sqlite3
        from zeloo_state import apply_database_pragmas
...
class TestFtsRebuildFinishWithoutTrigram:
    ...
    def test_rebuild_finish_skips_trigram_when_unavailable(
        ...
        monkeypatch.setattr(
            "zeloo_state.sqlite3.connect", connect_without_trigram
        )
        db = SessionDB(db_path=db_path)
        try:
            assert db._trigram_available is False
...
class TestPerformancePragmasEndToEnd:
    ...
    def _fresh_home(self, tmp_path, monkeypatch, config_text=None):
        ...
        monkeypatch.setattr(
            zeloo_state_wal, "is_sqlite_wal_reset_vulnerable",
            lambda version_info=None: False,
        )
        home = tmp_path / "zeloo_home"
        home.mkdir()
