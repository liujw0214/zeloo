...
def test_root_session_no_compression(db):
    ...
    assert prov["currentZELOOSessionId"] == "root1"
    assert prov["rootZELOOSessionId"] == "root1"
    assert prov["parentZELOOSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_zeloo_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentZELOOSessionId"] == "old"
    assert prov["rootZELOOSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousZELOOSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootZELOOSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"Zeloo"}
    assert "sessionProvenance" in meta["Zeloo"]
    assert meta["Zeloo"]["sessionProvenance"]["currentZELOOSessionId"] == "root1"
