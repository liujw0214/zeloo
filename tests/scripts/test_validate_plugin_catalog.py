...
VALID_ENTRY = {
    "name": "example-plugin",
    "repo": "https://github.com/NousResearch/Zeloo-example-plugins",
...
def test_bad_requires_zeloo_spec_fails(tmp_path):
    _expect_error(tmp_path, {"requires_ZELOO": "banana"}, "requires_ZELOO")


def test_comma_separated_requires_zeloo_passes(tmp_path):
    entry = {**VALID_ENTRY, "requires_ZELOO": ">=0.19, <2.0"}
    path = write_entry(tmp_path, entry)
    result = run_validator(str(path))
    assert result.returncode == 0, result.stdout + result.stderr
