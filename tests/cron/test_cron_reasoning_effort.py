...
- Job store (cron/jobs.py): the field is validated at the storage choke
  point against the canonical Zeloo effort grammar (parse_reasoning_effort
  in zeloo_constants — the SAME parser every other effort surface uses).
...
class TestCronjobToolReasoningEffort:
    ...
    via `Zeloo cron create/edit --reasoning-effort` only."""
    ...
    def test_schema_does_not_expose_reasoning_effort(self):
        ...
        keeps the parameter for the CLI lane (zeloo_cli/cron.py), but the
