...
kind) on every recurring dispatch so `Zeloo cron list` and
`Zeloo cron status` can surface late catch-ups.
"""
...
def cron_store(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("cron.jobs._zeloo_now", lambda: FIXED_NOW)
    return tmp_path
...
class TestDueScanDispatchStamp:
    def test_catch_up_beyond_grace_stamped_and_persisted(self, cron_store):
        ...
        # Persisted, so a separate `Zeloo cron list` process can read it.
