...
Running a state-writing ``Zeloo cron`` CLI command as root (the default for
...
2. Zombie-ticker surfacing: a failing tick must persist the failure reason
   (``ticker_last_error``) where ``Zeloo cron status`` can show it, and a
...
class TestTickerLoopRecordsErrors:
    ...
    def test_failing_tick_persists_error(self, cron_store, monkeypatch):
        ...
        msg = jobs.get_ticker_last_error()
        assert msg is not None, (
            "a failing tick must persist its reason for `Zeloo cron status` "
            "to surface (#68483)"
        )
        assert "Permission denied" in msg
...
# =========================================================================
# 3. `Zeloo cron status` surfaces the failure reason
# =========================================================================
...
class TestCronStatusSurfacesError:
    def test_status_shows_last_error_and_permission_hint(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr("zeloo_cli.gateway.find_gateway_pids", lambda: [4321])
