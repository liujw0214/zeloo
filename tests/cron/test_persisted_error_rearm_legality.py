...
def cron_store(tmp_path, monkeypatch):
    ...
    (zeloo_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("ZELOO_HOME", str(zeloo_home))
    monkeypatch.setattr(J, "ZELOO_DIR", zeloo_home)
    monkeypatch.setattr(J, "CRON_DIR", zeloo_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", zeloo_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", zeloo_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_zeloo_now", lambda: now):
            due = J.get_due_jobs()
