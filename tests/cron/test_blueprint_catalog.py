...
the shared /blueprint command handler (zeloo_cli/blueprint_cmd.py), and
the docs generator. Uses an isolated ZELOO_HOME for anything that touches the
cron job store.
"""
...
class TestRenderers:
    ...
    def test_deeplink_shape(self):
        ...
        assert url.startswith("Zeloo://blueprint/morning-brief?")
