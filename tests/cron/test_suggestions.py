...
ZELOO_HOME so the real suggestions.json is never touched.
"""
...
def store(tmp_path, monkeypatch):
    """A cron.suggestions module bound to an isolated ZELOO_HOME."""
    home = tmp_path / ".Zeloo"
    home.mkdir()
...
class TestStore:
    ...
    def test_profile_override_routes_writes_to_current_home(self, tmp_path):
        ...
        import_token = set_zeloo_home_override(profile_a)
        try:
            importlib.reload(suggestions_mod)
        finally:
            reset_zeloo_home_override(import_token)

        runtime_token = set_zeloo_home_override(profile_b)
        try:
            _add(suggestions_mod, key="profile-b")
        finally:
            reset_zeloo_home_override(runtime_token)
