...
Field incident: an interrupted `Zeloo update` (pull done, restart never ran)
...
had no way to know the fix was one `Zeloo gateway restart`. The failure
...
def test_import_error_without_skew_stays_a_plain_import_message(monkeypatch):
    ...
    assert "Zeloo gateway restart" not in msg
