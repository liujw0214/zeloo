"""Regression for #86721 — a one-shot `Zeloo cron run` invocation's
...
def test_recover_interrupted_executions_reaps_the_stale_claim_from_a_dead_process(
    ...
    # exactly what a subsequent `Zeloo cron run` invocation would trigger
...
def test_try_dispatch_background_run_calls_recovery_before_claiming(monkeypatch):
    ...
    # caller ("Zeloo cron run", tests) early return documented just
