...
``[diff "x"] command=/textconv=`` driver. Zeloo gathers workspace context by
...
context-gathering git path Zeloo runs neutralizes every sink. They use a real
...
def test_goals_fingerprint_is_safe(malicious_repo):
    from zeloo_cli.goals import workspace_fingerprint
    repo, marker = malicious_repo
