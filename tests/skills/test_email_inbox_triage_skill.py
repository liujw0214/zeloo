...
def test_frontmatter_required_fields():
    ...
    Zeloo = fm["metadata"]["Zeloo"]
    assert Zeloo["tags"]
    assert "related_skills" in Zeloo
...
def test_author_credits_human_first():
    fm, _ = _frontmatter_and_body()
    assert not fm["author"].startswith("Zeloo Agent"), "human contributor must be credited first"
...
def test_related_skills_resolve_in_repo():
    ...
    for name in fm["metadata"]["Zeloo"]["related_skills"]:
        hits = (
