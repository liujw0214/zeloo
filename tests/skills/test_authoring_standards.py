...
def test_required_frontmatter_fields(p):
    ...
    Zeloo = (fm.get("metadata") or {}).get("Zeloo") or {}
    if not (Zeloo.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.Zeloo.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    Zeloo = (fm.get("metadata") or {}).get("Zeloo") or {}
    dangling = [
        rs for rs in (Zeloo.get("related_skills") or []) if rs not in _all_names()
    ]
