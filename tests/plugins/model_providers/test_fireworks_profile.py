...
class TestFireworksHeaders:
    def test_attribution_matches_canonical_zeloo_values(self, fireworks_profile):
        """Fireworks requests carry the same attribution identity Zeloo sends
        everywhere else.
    ...
    def test_user_agent_identifies_ZELOO(self, fireworks_profile):
        # Prefix, not the full string — the version moves every release.
        assert fireworks_profile.default_headers["User-Agent"].startswith("ZELOOAgent/")
