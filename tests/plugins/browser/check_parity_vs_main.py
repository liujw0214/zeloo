...
Run from the PR worktree:
    ...
    cd ~/.Zeloo/Zeloo-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.Zeloo/Zeloo-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.Zeloo/Zeloo-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a Zeloo-agent checkout"
)
...
# Isolated ZELOO_HOME for the config write.
