...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure ZELOO_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", zeloo_home=str(tmp_path / ".Zeloo"))
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ZELOO_HOME", str(zeloo_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", zeloo_home=str(zeloo_home))
        return p
