...
def _inject_fake_zeloo_cli(monkeypatch):
    """Inject fake zeloo_cli modules so yaml/curses aren't required."""
    fake_config_mod = types.ModuleType("zeloo_cli.config")
    fake_config_mod.save_config = lambda c: None

    fake_setup_mod = types.ModuleType("zeloo_cli.memory_setup")
    fake_setup_mod._curses_select = lambda *a, **kw: 0
    fake_setup_mod._prompt = lambda label, default=None, secret=False: default or ""

    fake_zeloo_cli = types.ModuleType("zeloo_cli")
    fake_zeloo_cli.config = fake_config_mod
    fake_zeloo_cli.memory_setup = fake_setup_mod

    monkeypatch.setitem(sys.modules, "zeloo_cli", fake_zeloo_cli)
    monkeypatch.setitem(sys.modules, "zeloo_cli.config", fake_config_mod)
    monkeypatch.setitem(sys.modules, "zeloo_cli.memory_setup", fake_setup_mod)
...
class TestPostSetup:
    ...
    def test_platform_flag_mode(self, tmp_path, monkeypatch):
        monkeypatch.setattr("sys.argv", ["Zeloo", "--mode", "platform", "--api-key", "sk-test"])
        monkeypatch.setattr("plugins.memory.mem0._setup.get_zeloo_home", lambda: tmp_path)
        _inject_fake_zeloo_cli(monkeypatch)
        config = {"memory": {}}
    ...
    def test_selfhosted_flag_mode(self, tmp_path, monkeypatch):
        ...
        assert mem0_json["user_id"] == "Zeloo-user"
...
def test_discovery_loaded_setup_module_exposes_post_setup(monkeypatch):
    """`Zeloo memory setup mem0` reaches the wizard when the package is first imported by plugin
