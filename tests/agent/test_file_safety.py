...
class TestCacheFileReadBlocking:
    """Internal Zeloo cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        zeloo_home = tmp_path / ".Zeloo"
        cache = zeloo_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._zeloo_home_path", return_value=zeloo_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Zeloo cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        zeloo_home = tmp_path / ".Zeloo"
        hub = zeloo_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_zeloo_home(self, tmp_path):
        """The env basename guard does not depend on ZELOO_HOME resolution."""
        zeloo_home = tmp_path / ".Zeloo"
        zeloo_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        zeloo_home = tmp_path / ".Zeloo"
        cache = zeloo_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
