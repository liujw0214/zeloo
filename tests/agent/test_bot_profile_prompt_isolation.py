...
thread that did not bind the ZELOO_HOME ContextVar.
...
active-profile line resolved the home via the ambient ``get_zeloo_home()``,
so an unbound build thread fell back to ``~/.Zeloo`` (default) and leaked
default's full skills index + "Active Zeloo profile: default" into a bot's
...
def test_skills_prompt_scoped_to_override_not_ambient_home(tmp_path, monkeypatch):
    """An explicit skills_dir_override wins over ambient ZELOO_HOME, on a
    ...
    def build():
        # No set_zeloo_home_override on THIS thread — ambient resolves to
...
def test_profile_name_correct_on_bound_profile_session(tmp_path, monkeypatch):
    ...
    ``get_zeloo_home()/profiles`` as the root would never match and every
    profile would misreport as \"default\". The name must derive from the
    Zeloo ROOT (get_default_zeloo_root)."""
    from agent import system_prompt

    bot_home = tmp_path / "profiles" / "mybot"
    bot_home.mkdir(parents=True)

    # Bound session: ZELOO_HOME env points at the profile dir itself.
    monkeypatch.setenv("ZELOO_HOME", str(bot_home))
