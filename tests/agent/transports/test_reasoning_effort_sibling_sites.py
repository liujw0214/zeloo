...
class TestKimiEffortVocabulary:
    def test_k3_maps_full_zeloo_ladder(self):
        expected = {
            "minimal": "low",
            "low": "low",
            "medium": "high",
        ...
        for zeloo_level, wire_level in expected.items():
            kw = _kimi_kwargs(
                "kimi-k3", {"enabled": True, "effort": zeloo_level}
            )
            assert kw["reasoning_effort"] == wire_level, zeloo_level
