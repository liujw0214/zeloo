...
path via ``Zeloo chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp ZELOO_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point ZELOO_HOME at a temp dir and turn moa.save_traces on."""
    zeloo_home = tmp_path / ".Zeloo"
    zeloo_home.mkdir()
    monkeypatch.setenv("ZELOO_HOME", str(zeloo_home))

    # save_moa_turn reads config via zeloo_cli.config.load_config; stub it to
