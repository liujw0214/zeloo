...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated ZELOO_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="zeloo_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".Zeloo"))
    prev_home = os.environ.get("ZELOO_HOME")
    os.environ["ZELOO_HOME"] = os.path.join(test_home, ".Zeloo")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("zeloo_"):
            del sys.modules[mod]
    ...
    try:
        ...
        # references into zeloo_cli.*/tools.* and their monkeypatches would otherwise
        ...
        if prev_home is None:
            ...
            os.environ["ZELOO_HOME"] = prev_home
