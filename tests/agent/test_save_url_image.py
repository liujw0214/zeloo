...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate ZELOO_HOME under tmp_path."""
    monkeypatch.setenv("ZELOO_HOME", str(tmp_path / ".Zeloo"))
    (tmp_path / ".Zeloo").mkdir()

    # Force the constants/image cache helpers to re-read ZELOO_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("zeloo_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_zeloo_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under ZELOO_HOME — gateway cleanup
