...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"Zeloo-agent-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "Zeloo-agent-dev", "declared": True},
        },
    )
...
# `Zeloo curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `Zeloo curator pin <umbrella>` in the rename block so
