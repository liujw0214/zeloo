"""Tests for ZELOO_HOME credential-file read blocking in file_safety.

Regression for https://github.com/NousResearch/Zeloo-agent/issues/17656 —
``read_file`` was previously only sandboxed against ``ZELOO_HOME`` itself,
...
for the credential stores while leaving arbitrary ``ZELOO_HOME`` files
...
def fake_home(tmp_path, monkeypatch):
    """Point ``_zeloo_home_path()`` at a tmp dir for isolated checks."""
    import agent.file_safety as fs

    home = tmp_path / "zeloo_home"
    home.mkdir()
    monkeypatch.setattr(fs, "_zeloo_home_path", lambda: home)
    return home
...
def test_arbitrary_zeloo_home_file_not_blocked(fake_home):
    """Non-credential files inside ZELOO_HOME stay readable."""
...
def test_identically_named_zeloo_files_outside_home_not_blocked(
    fake_home, tmp_path
):
    """Zeloo-specific filenames (``auth.json``, ``mcp-tokens/``, ``google_oauth.json``)
    outside ZELOO_HOME must remain readable — the gate is per-location for
    ...
    project.mkdir()
    # auth.json outside ZELOO_HOME — readable (per-location gate).
    p = project / "auth.json"
    p.write_text("not secret here", encoding="utf-8")
    assert get_read_block_error(str(p)) is None, (
        "auth.json outside ZELOO_HOME should NOT be blocked"
    )
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    """Under a profile, ZELOO_HOME = <root>/profiles/<name>, but
    ...
    monkeypatch.setattr(fs, "_zeloo_home_path", lambda: profile)
