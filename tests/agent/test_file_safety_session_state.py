...
manipulation. Zeloo keeps canonical conversation history in state.db and may
...
def fake_homes(tmp_path, monkeypatch):
    """Point ZELOO_HOME at a temp profile dir.

    Uses the real env-var resolution chain (get_zeloo_home /
    get_default_zeloo_root) instead of monkeypatching private helpers —
    ...
    ZELOO_HOME=<root>/profiles/<name> makes get_default_zeloo_root()
