...
A skill whose metadata.Zeloo.session_platforms names gateway channels is
...
def _conds(platforms):
    return extract_skill_conditions(
        {"metadata": {"Zeloo": {"session_platforms": platforms}}}
    )
...
class TestSessionPlatformGate:
    ...
    def test_teams_meeting_pipeline_carries_the_gate(self):
        ...
        assert fm["metadata"]["Zeloo"]["session_platforms"] == ["teams", "cron"]
