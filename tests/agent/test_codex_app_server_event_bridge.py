"""Regression tests for the codex_app_server → Zeloo UI event bridge.
...
class TestCodexItemToToolName:
    ...
    def test_zeloo_tools_mcp_server_emits_bare_tool_name(self):
        """The Zeloo-tools MCP server wraps Zeloo' own tools for codex;
        ...
        mcp.Zeloo-tools.* namespacing (from #26541 by @simpolism)."""
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "Zeloo-tools", "tool": "web_search"}
        ) == "web_search"
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "Zeloo-tools", "tool": "browser_navigate"}
        ) == "browser_navigate"
...
class TestToolProgressDispatch:
    ...
    def test_web_search_builtin_fires_started_and_completed(self):
        ...
        assert calls[0].args[2] == "Zeloo agent docs"
        assert calls[0].args[3] == {"query": "Zeloo agent docs"}
