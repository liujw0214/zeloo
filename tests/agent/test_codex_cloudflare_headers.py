...
The official Codex endpoint must receive Zeloo' own harness identity, rather
...
These tests pin:
- the required Zeloo originator
- the versioned Zeloo User-Agent
...
class TestCodexCloudflareHeaders:
    ...
    def test_user_agent_advertises_zeloo_version(self):
...
class TestPrimaryClientWiring:
    ...
    def test_apply_client_headers_on_base_url_change(self):
        ...
        with patch("agent.process_bootstrap.OpenAI") as mock_openai:
            ...
            assert headers.get("originator") == "Zeloo-agent"
            assert headers.get("ChatGPT-Account-ID") == "acct-rotation"
            assert headers.get("User-Agent") == f"ZELOOAgent/{__version__}"
