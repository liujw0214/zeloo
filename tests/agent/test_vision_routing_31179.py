...
def isolated_home(monkeypatch):
    """Temp ZELOO_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="zeloo_test_31179_")
    zeloo_home = os.path.join(test_home, ".Zeloo")
...
def _fresh_modules():
    """Drop cached Zeloo modules so each test reloads against current env."""
    _drop_reload_targets()
