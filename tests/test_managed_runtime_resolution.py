"""Guard: Zeloo-owned subprocesses must not resolve managed runtimes by bare PATH.

Zeloo installs runtimes for itself — ``uv`` at ``$ZELOO_HOME/bin/uv``, Node at
``$ZELOO_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Zeloo's own
...
* when a system copy also exists, the one Zeloo does not own wins — which is
...
``iter_zeloo_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes Zeloo provisions into ZELOO_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Zeloo-owned subprocess code: plugins ship their own
...
_ALLOWED: dict[tuple[str, str], str] = {
    ...
    ("zeloo_cli/update_cmd_deps.py", "uv"): (
    ...
    ("zeloo_cli/update_cmd_deps.py", "npm"): (
...
def _source_files() -> list[Path]:
    ...
    # (zeloo_agent-<version>/) gets created and deleted concurrently, and
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Zeloo-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$ZELOO_HOME/bin (uv) and $ZELOO_HOME/node are not on an "
        ...
        "  node/npm -> zeloo_constants.find_node_executable()\n"
        "  PATH env -> zeloo_constants.iter_zeloo_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(zeloo_constants, helper))


def test_managed_uv_helpers_exist():
    from zeloo_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
