...
def test_media_delivery_denies_encrypted_bitwarden_cache(tmp_path, monkeypatch):
    ...
    zeloo_home.mkdir()
    monkeypatch.setattr(base, "_ZELOO_HOME", zeloo_home)
    monkeypatch.setattr(base, "_ZELOO_ROOT", zeloo_home)
    path = zeloo_home / "cache" / "bws_cache.enc.json"
    path.parent.mkdir()
...
class TestMediaInsideSerializedJson:
    ...
    def test_media_in_embedded_serialized_reply_not_extracted(self):
        ...
        content = (
            '{"content":"previous reply MEDIA:/Users/ex/.Zeloo/media/'
...
class TestExtensionlessMediaDelivery:
    ...
    def _patch_allow_root(self, monkeypatch, root):
        ...
        monkeypatch.delenv("ZELOO_MEDIA_DELIVERY_STRICT", raising=False)
...
class TestMediaDeliveryPathValidation:
    def _patch_roots(self, monkeypatch, *roots):
        ...
        monkeypatch.setenv("ZELOO_MEDIA_DELIVERY_STRICT", "1")
        ...
        monkeypatch.setenv("ZELOO_MEDIA_TRUST_RECENT_FILES", "0")
    ...
    def test_allows_stale_kanban_attachment_but_not_neighboring_workspace(
        ...
        monkeypatch.setenv("ZELOO_KANBAN_HOME", str(tmp_path / "Zeloo"))
        monkeypatch.setenv("ZELOO_MEDIA_TRUST_RECENT_FILES", "0")
        board_root = tmp_path / "Zeloo" / "kanban" / "boards" / "research"
    ...
    def test_recency_trust_denies_system_paths_even_when_fresh(self, tmp_path, monkeypatch):
        ...
        """
        self._patch_roots(monkeypatch)
        monkeypatch.delenv("ZELOO_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("ZELOO_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("ZELOO_MEDIA_TRUST_RECENT_SECONDS", "600")
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_mcp_oauth_tokens(self, tmp_path, monkeypatch, rel):
        """Live MCP OAuth tokens/client creds under ~/.Zeloo/mcp-tokens/ must
        ...
        fake_home = tmp_path / "home"
        zeloo_dir = fake_home / ".Zeloo"
        (zeloo_dir / "mcp-tokens").mkdir(parents=True)
        secret = zeloo_dir / rel
        secret.write_text('{"access_token": "live-bearer-abc123"}')
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr(
            "gateway.platforms.base._ZELOO_HOME",
            zeloo_dir,
        )
        monkeypatch.setattr(
            "gateway.platforms.base._ZELOO_ROOT",
            zeloo_dir,
        )
    ...
    def test_denylist_blocks_google_token_default_mode(self, tmp_path, monkeypatch):
        """Integration credentials at the ZELOO_HOME root (google_token.json)
        ...
        fake_home = tmp_path / "home"
        zeloo_dir = fake_home / ".Zeloo"
        zeloo_dir.mkdir(parents=True)
        token = zeloo_dir / "google_token.json"
        token.write_text('{"access_token": "***", "refresh_token": "***"}')
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr("gateway.platforms.base._ZELOO_HOME", zeloo_dir)
        monkeypatch.setattr("gateway.platforms.base._ZELOO_ROOT", zeloo_dir)

        assert BasePlatformAdapter.validate_media_delivery_path(str(token)) is None


    def test_denylist_blocks_non_cache_file_under_zeloo_home(self, tmp_path, monkeypatch):
        """A non-credential file the agent wrote directly under ~/.Zeloo
        ...
        zeloo_dir = fake_home / ".Zeloo"
        zeloo_dir.mkdir(parents=True)
        artifact = zeloo_dir / "adhoc_report.pdf"
    ...
    def test_denylist_blocks_session_stores_but_not_neighbouring_artifacts(self, tmp_path, monkeypatch):
        ...
        pasted into a chat; ``MEDIA:~/.Zeloo/state.db`` must not exfiltrate them (#41071). Named
        ...
        monkeypatch.setattr("gateway.platforms.base._ZELOO_HOME", zeloo_dir)
        monkeypatch.setattr("gateway.platforms.base._ZELOO_ROOT", zeloo_dir)
        board = zeloo_dir / "kanban" / "boards" / "team-a"
        ...
        assert [rel for rel in denied if BasePlatformAdapter.validate_media_delivery_path(str(zeloo_dir / rel))] == []
        assert [rel for rel in allowed if not BasePlatformAdapter.validate_media_delivery_path(str(zeloo_dir / rel))] == []
    ...
    def test_denylist_covers_every_profile_home_not_just_the_launch_home(self, tmp_path, monkeypatch):
        ...
        turn is the launch (default) profile's or the secondary's own (ZELOO_HOME override), while
        ...
        self._patch_roots(monkeypatch)
        fake_home = tmp_path / "home"
        zeloo_root = fake_home / ".Zeloo"
        profile_b = zeloo_root / "profiles" / "beta"
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr("gateway.platforms.base._ZELOO_HOME", zeloo_root)
        monkeypatch.setattr("gateway.platforms.base._ZELOO_ROOT", zeloo_root)
        ...
        for scope in (None, profile_b):  # default profile's turn, then beta's own turn
            token = set_zeloo_home_override(scope)
            try:
    ...
    def test_strict_mode_envvar_restores_legacy_behavior(self, tmp_path, monkeypatch):
        """Setting ZELOO_MEDIA_DELIVERY_STRICT=1 reactivates the older
    ...
    def test_profile_scoped_cache_delivers_under_symlinked_root(self, tmp_path, monkeypatch):
        """Reopened #31733: a profile gateway whose ZELOO_HOME is symlinked
        under a denied prefix (e.g. /opt/data -> /root/.Zeloo) emits
        ...
        zeloo_root = denied_root / ".Zeloo"
        prof_cache = zeloo_root / "profiles" / "myprof" / "cache" / "images"
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._ZELOO_ROOT", zeloo_root
        )
...
class TestDockerContainerMediaPathTranslation:
    ...
    def test_cache_dir_container_path_translates_to_host_cache(self, tmp_path, monkeypatch):
        """MEDIA:/root/.Zeloo/cache/images/... (the agent_visible_image path
        under docker) must translate to the HOST cache file, not the sandbox
        home copy."""
        zeloo_home = tmp_path / ".Zeloo"
        cache = zeloo_home / "cache" / "images"
    ...
    def test_container_credential_path_never_translates_through_home(self, tmp_path, monkeypatch):
        """/root/.Zeloo/* outside a cache mount (the sandbox's credential
...
class TestGetHumanDelay:
    ...
    def test_natural_mode_ignores_malformed_custom_env_vars(self):
        env = {
            "ZELOO_HUMAN_DELAY_MODE": "natural",
            "ZELOO_HUMAN_DELAY_MIN_MS": "oops",
            "ZELOO_HUMAN_DELAY_MAX_MS": "still-bad",
        }
...
class TestMediaDeliveryDiagnosability:
    ...
    def test_rejected_path_appears_in_log(self, tmp_path, caplog):
        ...
        with patch.dict(os.environ, {"ZELOO_MEDIA_DELIVERY_STRICT": "1",
                                     "ZELOO_MEDIA_TRUST_RECENT_FILES": "0"}), \
    ...
    def test_crafted_null_path_does_not_abort_batch(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ZELOO_MEDIA_DELIVERY_STRICT", "0")
...
class _CapturingAdapter(BasePlatformAdapter):
    ...
    chat text. That argument is a host filesystem path inside the Zeloo
...
class TestMediaFallbackDoesNotLeakHostPath:
    ...
    a chat message like ``🎬 Video: /home/.../Zeloo/cache/video/abc.mp4``
    — a host filesystem path with no actionable information.
    """

    SENSITIVE_PATH = "/home/jayne/.Zeloo/cache/media/sensitive_host_path_abc123.bin"
...
class TestDockerProfileSandboxMediaTranslation:
    ...
    def test_home_credential_surface_still_refused(self, monkeypatch):
        """The /root/.Zeloo exclusion survives profile scoping: translating
        ...
        for task in ("default", f"session:{self.SESSION_KEY}"):
            secrets = self._sandbox_dir(task) / "home" / ".Zeloo"
            secrets.mkdir(parents=True, exist_ok=True)
            (secrets / "auth.json").write_text("{}")

        assert (
            BasePlatformAdapter.validate_media_delivery_path(
                "/root/.Zeloo/auth.json", session_key=self.SESSION_KEY
            )
            is None
        )
