...
``zeloo_cli.config`` (get_env_value / save_env_value / remove_env_value),
``zeloo_cli.cli_output`` (prompt / prompt_yes_no / print_*), and
``zeloo_cli.setup`` (prompt_choice); we patch each at its source module so
