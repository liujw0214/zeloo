...
class TestNtfyRequirements:
    ...
    def test_returns_false_when_httpx_unavailable(self, monkeypatch):
        monkeypatch.setenv("NTFY_TOPIC", "Zeloo-test")
...
class TestNtfyAdapterInit:
    ...
    def test_publish_topic_uses_extra_value(self):
        config = PlatformConfig(
            enabled=True,
            extra={"topic": "Zeloo-in", "publish_topic": "Zeloo-out"},
        )
        adapter = NtfyAdapter(config)
        assert adapter._publish_topic == "Zeloo-out"
...
class TestConnect:
    ...
    def test_connect_starts_stream_task(self, monkeypatch):
        ...
        config = PlatformConfig(enabled=True, extra={"topic": "Zeloo-test"})
        adapter = NtfyAdapter(config)
...
class TestSend:
    ...
    def _make_adapter(self, topic="Zeloo-in", publish_topic="", token="", markdown=False):
    ...
    def test_send_posts_to_publish_topic(self):
        adapter = self._make_adapter(topic="Zeloo-in", publish_topic="Zeloo-out")
        ...
        result = _run(adapter.send("Zeloo-in", "Hello ntfy!"))
        assert result.success is True
        assert result.message_id == "abc123"

        posted_url = mock_client.post.call_args[0][0]
        assert posted_url.endswith("/Zeloo-out")

    def test_send_falls_back_to_subscribe_topic(self):
        adapter = self._make_adapter(topic="Zeloo-in")
        ...
        result = _run(adapter.send("Zeloo-in", "Hello!"))
        assert result.success is True
        posted_url = mock_client.post.call_args[0][0]
        assert posted_url.endswith("/Zeloo-in")
    ...
    def test_send_uses_metadata_publish_topic(self):
        ...
        result = _run(adapter.send(
            "Zeloo-in", "Hi!", metadata={"publish_topic": "override-out"}
        ))
        assert result.success is True
        posted_url = mock_client.post.call_args[0][0]
    ...
    def test_get_chat_info_returns_dict(self):
        adapter = NtfyAdapter(PlatformConfig(enabled=True, extra={"topic": "t"}))
        info = _run(adapter.get_chat_info("Zeloo-in"))
        assert info["name"] == "Zeloo-in"
        assert info["type"] == "dm"
...
class TestOnMessage:
    ...
    def _make_adapter(self):
        return NtfyAdapter(PlatformConfig(enabled=True, extra={"topic": "Zeloo-in"}))
    ...
    def test_duplicate_message_skipped(self):
        ...
        adapter.set_message_handler(handler)
        event = {"id": "dup-1", "event": "message", "topic": "Zeloo-in", "message": "hi", "time": None}
        _run(adapter._on_message(event))
        _run(adapter._on_message(event))
        assert len(calls) == 1
...
class TestEnvEnablement:
    ...
    def test_markdown_truthy_values(self, monkeypatch):
        monkeypatch.setenv("NTFY_TOPIC", "Zeloo-in")
...
class TestStandaloneSend:
    ...
    def test_emits_echo_tag_header(self, monkeypatch):
        ...
        with patch.object(_ntfy, "httpx") as mock_httpx:
            ...
            _run(_standalone_send(pconfig, "Zeloo-in", "hi"))
