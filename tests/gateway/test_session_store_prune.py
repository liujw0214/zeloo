...
def test_session_store_default_db_uses_runtime_zeloo_home(tmp_path, monkeypatch):
    """SessionStore must honor runtime ZELOO_HOME when opening the default DB.

    Regression for the import-time DEFAULT_DB_PATH freeze: importing
    zeloo_state before a fixture redirected ZELOO_HOME used to pin every
    default SessionDB() at the developer's real ~/.Zeloo/state.db.
    """
    config = GatewayConfig()
    fake_home = tmp_path / "alt_zeloo_home"
    fake_home.mkdir()
    monkeypatch.setenv("ZELOO_HOME", str(fake_home))
...
class TestReadmeSentinel:
    ...
    def test_save_writes_readme_sentinel_first(self, tmp_path):
        ...
        assert "Zeloo sessions list" in raw["_README"]
