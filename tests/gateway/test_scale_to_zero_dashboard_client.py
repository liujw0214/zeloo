...
ZELOO_HOME, GatewayRunner._scale_to_zero_is_idle's composition, and
...
def test_heartbeat_path_lives_under_zeloo_home_state(zeloo_home):
    p = s2z.dashboard_client_heartbeat_path()
    assert p == zeloo_home / "state" / "dashboard_clients.heartbeat"


def test_last_seen_missing_marker_is_none_not_fail_awake(zeloo_home):
...
def test_touch_creates_state_dir_and_marker(zeloo_home):
...
def test_last_seen_returns_raw_mtime_without_staleness_cutoff(zeloo_home):
...
def test_last_seen_future_mtime_is_clamped_to_now(zeloo_home):
...
def test_last_seen_unreadable_marker_fails_awake(zeloo_home, monkeypatch):
    s2z.touch_dashboard_client_heartbeat()
...
def test_touch_never_raises(zeloo_home, monkeypatch):
...
def test_idle_without_dashboard_client_unchanged(zeloo_home, monkeypatch):
    r = _runner(monkeypatch, last_inbound_at=time.time() - 600)
    assert r._scale_to_zero_is_idle() is True


def test_attached_dashboard_client_blocks_idle(zeloo_home, monkeypatch):
    r = _runner(monkeypatch, last_inbound_at=time.time() - 600)
    s2z.touch_dashboard_client_heartbeat()
    assert r._scale_to_zero_is_idle() is False


def test_client_gets_the_same_idle_grace_as_a_message(zeloo_home, monkeypatch):
...
def test_client_gone_longer_than_idle_timeout_is_idle(zeloo_home, monkeypatch):
    r = _runner(monkeypatch, last_inbound_at=time.time() - 600)
    s2z.touch_dashboard_client_heartbeat()
    p = s2z.dashboard_client_heartbeat_path()
    old = time.time() - 121
...
def test_marker_predating_gateway_inbound_does_not_matter(zeloo_home, monkeypatch):
...
def test_dashboard_client_seen_recently_extends_inbound_clock(zeloo_home, monkeypatch):
...
def test_newer_gateway_inbound_wins_over_older_marker(zeloo_home, monkeypatch):
...
def test_unreadable_marker_keeps_gateway_awake(zeloo_home, monkeypatch):
    r = _runner(monkeypatch, last_inbound_at=time.time() - 600)
    s2z.touch_dashboard_client_heartbeat()
    monkeypatch.setattr(s2z.os, "stat", _stat_denying(s2z.dashboard_client_heartbeat_path()))
    assert r._scale_to_zero_is_idle() is False
...
def test_handle_ws_connect_touches_marker(zeloo_home, monkeypatch):
...
def test_handle_ws_inbound_frames_refresh_marker(zeloo_home, monkeypatch):
    from tui_gateway import server, ws as ws_mod
...
def test_note_activity_is_throttled(zeloo_home, monkeypatch):
