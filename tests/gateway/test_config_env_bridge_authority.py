...
Guards against the 60-vs-500 bug where a stale `.env ZELOO_MAX_ITERATIONS=60`
...
def _run_gateway_import(zeloo_home: Path, initial_env: dict[str, str]) -> dict[str, str]:
    ...
    script = textwrap.dedent(
        ...
        for k in (
            ...
            "ZELOO_AGENT_TIMEOUT",
            "ZELOO_AGENT_TIMEOUT_WARNING",
            "ZELOO_TURN_LEASE_TIMEOUT",
            "ZELOO_SESSION_STALL_TIMEOUT",
            "ZELOO_GATEWAY_BUSY_INPUT_MODE",
            "ZELOO_GATEWAY_BUSY_TEXT_MODE",
            "ZELOO_GATEWAY_PLATFORM_CONNECT_TIMEOUT",
            "ZELOO_TIMEZONE",
        ):
            v = os.environ.get(k)
            if v is not None:
    ...
    # otherwise intentionally clean so stale Zeloo settings cannot leak in.
    for k in (
        "PATH",
        "PYTHONPATH",
        "VIRTUAL_ENV",
...
@pytest.fixture
def zeloo_home(tmp_path: Path) -> Path:
    home = tmp_path / ".Zeloo"
    home.mkdir()
    return home


def test_config_gateway_timeout_wins_over_stale_env(zeloo_home: Path) -> None:
    """Every agent.* bridge key must be config-authoritative, not .env-authoritative."""
    _write_config(zeloo_home, agent_cfg={
        "gateway_timeout": 1800,
        "gateway_timeout_warning": 900,
        "session_stall_timeout": 300,
    })
    _write_env(zeloo_home, {
        "ZELOO_AGENT_TIMEOUT": "60",
        "ZELOO_AGENT_TIMEOUT_WARNING": "30",
        "ZELOO_SESSION_STALL_TIMEOUT": "15",
    })

    env = _run_gateway_import(zeloo_home, initial_env={})

    assert env.get("ZELOO_AGENT_TIMEOUT") == "1800"
    assert env.get("ZELOO_AGENT_TIMEOUT_WARNING") == "900"
    assert env.get("ZELOO_SESSION_STALL_TIMEOUT") == "300"


def test_config_turn_lease_timeout_wins_over_stale_env(zeloo_home: Path) -> None:
    ...
    _write_env(
        zeloo_home,
        {"ZELOO_TURN_LEASE_TIMEOUT": "60"},
    )

    env = _run_gateway_import(zeloo_home, initial_env={})

    assert env.get("ZELOO_TURN_LEASE_TIMEOUT") == "600"
...
def test_default_turn_lease_timeout_overrides_stale_env_when_key_is_omitted(
    ...
    assert env.get("ZELOO_TURN_LEASE_TIMEOUT") == "5"
...
def test_config_platform_connect_timeout_supplies_env_when_unset(zeloo_home: Path) -> None:
    ...
    _write_config(zeloo_home, gateway_cfg={"platform_connect_timeout": 90})

    env = _run_gateway_import(zeloo_home, initial_env={})

    assert env.get("ZELOO_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "90"


def test_env_platform_connect_timeout_wins_over_config(zeloo_home: Path) -> None:
    """Unlike the agent.*/display.*/timezone bridges (config-authoritative),
    ZELOO_GATEWAY_PLATFORM_CONNECT_TIMEOUT is the manual-override escape hatch:
    ...
    env = _run_gateway_import(
        zeloo_home,
        initial_env={"ZELOO_GATEWAY_PLATFORM_CONNECT_TIMEOUT": "120"},
    )

    assert env.get("ZELOO_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "120"
