...
The adapter is constructed and connected inside ``_profile_runtime_scope`` (ZELOO_HOME override +
...
def _observe(routed_home: Path) -> tuple:
    try:
        ...
        secret = "<unscoped>"
    return ("routed" if Path(get_zeloo_home()) == routed_home else "launch", secret)
...
def routed_scope(tmp_path):
    ...
    home_token = set_zeloo_home_override(str(routed_home))
