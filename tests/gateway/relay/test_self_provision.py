...
def _arm(monkeypatch, *, url="wss://connector.example/relay", token="nas-token"):
    ...
    monkeypatch.setattr("zeloo_cli.auth.resolve_nous_access_token", lambda: token)
...
def test_no_nas_token_is_non_fatal(monkeypatch):
    ...
    monkeypatch.setattr("zeloo_cli.auth.resolve_nous_access_token", _boom)
...
def test_relay_display_name_suppresses_stock_brand(monkeypatch):
    """The default 'Zeloo Agent' brand is identical on every install — forwarding
    ...
    class _Skin:
        def get_branding(self, key, fallback=""):
            return "Zeloo Agent" if key == "agent_name" else fallback

    monkeypatch.setattr("zeloo_cli.skin_engine.get_active_skin", lambda: _Skin())
