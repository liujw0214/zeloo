...
def test_cold_fronted_platforms_is_read_only(tmp_path, case):
    ...
    # pytest's imported modules. HOME and cwd are temporary as well as ZELOO_HOME.
    ...
    env.update({"HOME": str(tmp_path), "USERPROFILE": str(tmp_path),
                "ZELOO_HOME": str(home), "ZELOO_MANAGED_DIR": str(managed),
    ...
    code = textwrap.dedent("""
        ...
        if sys.argv[1] == "scoped":
            set_zeloo_home_override(sys.argv[2])
