...
class TestGatewayPidState:
    def test_write_pid_file_records_gateway_metadata(self, tmp_path, monkeypatch):
        ...
        assert payload["kind"] == "Zeloo-gateway"
    ...
    def test_get_running_pid_cached_invalidates_when_pid_file_changes(self, tmp_path, monkeypatch):
        ...
        def _write_record(pid: int, start_time: int) -> None:
            record = {
                "pid": pid,
                "kind": "Zeloo-gateway",
                "argv": ["python", "-m", "zeloo_cli.main", "gateway"],
    ...
    def test_gateway_identity_files_use_process_home_not_context_override(
        ...
        """Regression: pid/lock/state files must use process-level ZELOO_HOME.
        ...
        the process-level ZELOO_HOME, not the profile's directory.  See #56986.
        """
        ...
        monkeypatch.setenv("ZELOO_HOME", str(process_home))
...
class TestScopedGatewayPidQuery:
    ...
    def _write_scoped_profile(self, tmp_path):
        ...
        record = {
            "pid": 4242,
            "kind": "Zeloo-gateway",
            "argv": ["python", "-m", "zeloo_cli.main", "gateway", "--profile", "wiki"],
            "start_time": 123,
            "zeloo_home": str(profile_dir.resolve()),
        }
    ...
    def test_scoped_query_reports_live_foreign_profile_pid(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ZELOO_HOME", str(tmp_path / "default-home"))
        ...
        monkeypatch.setattr(
            ...
            lambda pid: "python -m zeloo_cli.main gateway --profile wiki",
        )
...
class TestGatewayRuntimeStatus:
    ...
    def test_runtime_status_running_pid_rejects_pid_reused_by_other_profile(self, monkeypatch):
        ...
        onto the live *default* gateway (``Zeloo gateway run``).  The recorded
        ...
        monkeypatch.setattr(
            status, "_read_process_cmdline", lambda pid: "Zeloo gateway run --replace"
        )
    ...
    def test_runtime_status_running_pid_accepts_matching_profile_cmdline(self, monkeypatch):
        ...
        for cmdline in (
            "Zeloo -p coder gateway run --replace",
            "/opt/Zeloo/.venv/bin/Zeloo --profile coder gateway run --replace",
            "zeloo_home=/opt/data/profiles/coder Zeloo gateway run --replace",
        ):
    ...
    def test_command_line_belongs_to_profile_normalizes_separators(self):
        """A Windows argv renders ZELOO_HOME with backslashes while the
        ...
        cmdline = r"zeloo_home=c:\opt\data\profiles\coder Zeloo gateway run --replace"
...
class TestScopedLocks:
    ...
    def test_acquire_scoped_lock_rejects_live_other_process(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ZELOO_GATEWAY_LOCK_DIR", str(tmp_path / "locks"))
    ...
    def test_acquire_scoped_lock_replaces_pid_reused_by_unrelated_process(self, tmp_path, monkeypatch):
        ...
        lock_path.write_text(json.dumps({
            ...
            "start_time": None,
            "kind": "Zeloo-gateway",
            "argv": ["/Users/user/.Zeloo/Zeloo-agent/zeloo_cli/main.py", "gateway", "run", "--replace"],
        }))
    ...
    def test_acquire_scoped_lock_self_reacquires_when_disk_start_time_null(
        ...
        lock_path.write_text(json.dumps({
            ...
            "start_time": None,
            "kind": "Zeloo-gateway",
            "argv": ["zeloo_cli/main.py", "--profile", "milena", "gateway", "run", "--replace"],
    ...
    def test_acquire_scoped_lock_self_reacquires_when_start_times_differ(
        ...
        lock_path.write_text(json.dumps({
            ...
            "start_time": 111,
            "kind": "Zeloo-gateway",
            "argv": ["zeloo_cli/main.py", "gateway", "run", "--replace"],
            "scope": "discord-bot-token",
        }))
    ...
    def test_acquire_scoped_lock_stamps_profile_label(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ZELOO_GATEWAY_LOCK_DIR", str(tmp_path / "locks"))
        monkeypatch.setenv("ZELOO_HOME", str(tmp_path / "profiles" / "lead-gen-outreach"))
    ...
    def test_acquire_scoped_lock_omits_profile_when_not_inferable(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ZELOO_GATEWAY_LOCK_DIR", str(tmp_path / "locks"))
        monkeypatch.setenv("ZELOO_HOME", str(tmp_path / "some-custom-dir"))
...
class TestScopedLockOwnerLabel:
    ...
    def test_profile_label_for_root_home_is_default(self, monkeypatch):
        monkeypatch.setenv("ZELOO_HOME", "/opt/data")
    ...
    def test_owner_label_prefers_explicit_profile_field(self):
        record = {"profile": "coder", "zeloo_home": "/opt/data/profiles/other"}
    ...
    def test_owner_label_ignores_invalid_profile_and_uses_home(self):
        ...
        # zeloo_home fallback — attribution degrades, never corrupts.
        record = {
            "profile": "bad; rm -rf /",
            "zeloo_home": "/opt/data/profiles/zerocool",
        }
        assert status.scoped_lock_owner_label(record) == "zerocool"

    def test_owner_label_falls_back_to_zeloo_home(self):
        # Locks written before the profile field existed still attribute.
        record = {"pid": 559, "zeloo_home": "/opt/data/profiles/zerocool"}
        assert status.scoped_lock_owner_label(record) == "zerocool"
...
class TestTakeoverMarker:
    ...
    def test_write_marker_records_replacer_zeloo_home(self, tmp_path, monkeypatch):
        """The marker stamps the replacer's ZELOO_HOME for cross-profile guard (#29092)."""
        monkeypatch.setenv("ZELOO_HOME", str(tmp_path))
        monkeypatch.setattr(status, "_get_process_start_time", lambda pid: 42)
        ...
        payload = json.loads((tmp_path / ".gateway-takeover.json").read_text())
        assert payload["replacer_zeloo_home"] == str(tmp_path)
    ...
    def test_consume_rejects_marker_from_different_profile(self, tmp_path, monkeypatch):
        ...
        ZELOO_HOME must be rejected even when PID + start_time coincidentally
        match — otherwise two profile services sharing a default ~/.Zeloo flap
        ...
        marker_path.write_text(json.dumps({
            ...
            "replacer_pid": 99999,
            "replacer_zeloo_home": str(tmp_path / "profiles" / "other"),
    ...
    def test_consume_accepts_legacy_marker_without_zeloo_home(self, tmp_path, monkeypatch):
        """Back-compat (#29092): markers written by older Zeloo versions have no
        ``replacer_zeloo_home`` field; an absent field is treated as same-home so
...
class TestScopedLockTakeover:
    ...
    def _owner_record(target_home: Path, *, pid: int = 4242, start_time: int = 123):
        ...
        record = {
            "pid": pid,
            "kind": "Zeloo-gateway",
            "argv": ["python", "-m", "zeloo_cli.main", "gateway", "run"],
            "start_time": start_time,
            "zeloo_home": str(target_home),
        }
    ...
    def test_verified_distinct_home_handoff_marks_target_before_sigterm(
        ...
        replacer_home.mkdir()
        monkeypatch.setenv("ZELOO_HOME", str(replacer_home))
        ...
        monkeypatch.setattr(
            ...
            lambda _pid: "python -m zeloo_cli.main gateway run",
        )
        calls = []
        ...
        def terminate(pid, *, force=False):
            ...
            assert payload["target_zeloo_home"] == str(target_home)
            assert payload["replacer_zeloo_home"] == str(replacer_home)
...
class TestPlannedStopMarker:
    ...
    def test_consume_returns_true_on_windows_when_start_time_unavailable(
        ...
        it would always return False — and ``Zeloo gateway stop`` would be
...
class TestReadProcessCmdlinePsFallback:
    ...
    def test_proc_cmdline_takes_priority_over_ps(self, monkeypatch):
        ...
        def fake_read_bytes(self):
            calls.append("proc")
            return b"python\x00zeloo_cli/main.py\x00gateway\x00"

        monkeypatch.setattr(status.Path, "read_bytes", fake_read_bytes)
        result = status._read_process_cmdline(12345)
        assert "zeloo_cli/main.py" in result
        assert calls == ["proc"]
...
class TestResolveGatewayLiveness:
    ...
    def test_profile_dir_scopes_every_read_to_that_profile(self, tmp_path):
        ...
        ignore the ZELOO_HOME contextvar override (#56986), so the profile
...
def test_strict_gateway_identity_rejects_reused_pid(tmp_path, monkeypatch):
    pid_path = tmp_path / "gateway.pid"
    lock_path = tmp_path / "gateway.lock"
    record = {"pid": 123, "start_time": 10.0, "kind": "Zeloo-gateway"}
