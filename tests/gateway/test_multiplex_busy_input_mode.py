...
async def test_secondary_profile_busy_mode_controls_priority_path(
    ...
    monkeypatch.setenv("ZELOO_TELEGRAM_FOLLOWUP_GRACE_SECONDS", "0")
    runner = _runner(default_mode="interrupt")
    adapter = await _load_profile_snapshot(
        runner,
        tmp_path / "research",
...
async def test_primary_adapter_busy_origin_uses_routed_privacy(
    ...
    monkeypatch.setattr("gateway.run._zeloo_home", home)
    monkeypatch.setenv("ZELOO_GATEWAY_BUSY_ACK_ENABLED", "false")
    ...
    ambient = get_zeloo_home_override()
    ...
    assert get_zeloo_home_override() == ambient
