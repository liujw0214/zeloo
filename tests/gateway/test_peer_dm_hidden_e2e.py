"""E2E: ``Zeloo peer dm`` against a REAL api_server gateway whose canonical
Bot Chat session is HIDDEN (issue #91583).

Two real Zeloo homes in spirit: the "peer" side is a real
:class:`APIServerAdapter` bound to a real loopback TCP socket over a real
SQLite ``state.db`` (its own tmp ZELOO_HOME) containing a hidden
``Bot Chat`` row — exactly what Bot Mode leaves behind. The "local" side is
the stock ``Zeloo peer dm`` client code (``zeloo_cli.subcommands.peer``),
...
def peer_gateway(tmp_path, monkeypatch):
    """A real api_server gateway (own ZELOO_HOME + state.db) on a real socket."""
