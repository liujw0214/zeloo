...
(/opt/Zeloo/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable ZELOO_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_zeloo_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable ZELOO_HOME."""
    ...
    monkeypatch.setattr(
        "zeloo_constants.get_zeloo_home", lambda: zeloo_home
    )
    ...
    expected = zeloo_home / "scripts" / "whatsapp-bridge"
