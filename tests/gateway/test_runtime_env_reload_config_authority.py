...
ZELOO_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env ZELOO_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``Zeloo config get terminal.backend`` still says local.
    """
    zeloo_home = tmp_path / ".Zeloo"
    zeloo_home.mkdir()
    (zeloo_home / "config.yaml").write_text(
    ...
    (zeloo_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
