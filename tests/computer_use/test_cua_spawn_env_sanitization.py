...
def _assert_path_preserved(env):
    """Original PATH entries survive sanitization; the Zeloo console-script
    dir may be prepended (see _sanitize_subprocess_env, issue #92998) so we
    assert the contract, not byte equality."""
    from tools.environments.local import _resolve_zeloo_bin_dir

    path_val = env.get("PATH", "")
    assert path_val.endswith("/usr/bin:/bin"), path_val
    zeloo_bin = _resolve_zeloo_bin_dir()
    if zeloo_bin and path_val != "/usr/bin:/bin":
        assert path_val.startswith(zeloo_bin + os.pathsep), path_val
...
def test_resolve_mcp_invocation_sanitizes_env(monkeypatch):
    ...
    monkeypatch.delenv("ZELOO_CUA_TELEMETRY", raising=False)
