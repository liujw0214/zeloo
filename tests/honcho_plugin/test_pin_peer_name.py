...
Under a gateway (Telegram, Discord, Slack, ...) Zeloo passes the
...
class TestPinPeerNameConfigParsing:
    ...
    def test_root_level_true(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ZELOO_HOME", str(tmp_path / "isolated"))
...
class TestCrossPlatformMemoryUnification:
    """The same physical user talking to Zeloo via Telegram AND Discord
...
class TestProfilePeerUniqueness:
    """Each Zeloo profile can pin to its own unique peerName.

    Profile cloning copies host blocks, but operators routinely diverge them
    afterwards (e.g. `Zeloo -p partner` pinned to a different person's peer).
