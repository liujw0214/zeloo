...
class TestCredentialModel:
    def test_roundtrip(self):
        ...
        assert block["clientId"] == "Zeloo-desktop"
...
class TestEnsureFreshToken:
    def test_no_oauth_credential_is_noop(self, tmp_path):
        ...
        token, refreshed = oauth.ensure_fresh_token(path, "Zeloo", now=0)
        assert token is None and refreshed is False

    def test_fresh_token_skips_refresh(self, tmp_path, monkeypatch):
        path = tmp_path / "honcho.json"
        _write(path, {"hosts": {"Zeloo": _host_block(expires_at=10_000)}})
        monkeypatch.setattr(
    ...
    def test_expired_token_refreshes_and_persists_rotation(self, tmp_path, monkeypatch):
        path = tmp_path / "honcho.json"
        _write(path, {"hosts": {"Zeloo": _host_block(expires_at=100)}})

        def fake_post(url, data, timeout):
            assert data["grant_type"] == "refresh_token"
            assert data["refresh_token"] == "hch-rt-old"
            assert data["client_id"] == "Zeloo-desktop"
            return 200, {
    ...
    def test_refresh_failure_fails_open(self, tmp_path, monkeypatch):
        ...
        assert token == "hch-at-old" and refreshed is False
        assert len(calls) == 2
        assert json.loads(path.read_text())["hosts"]["Zeloo"]["apiKey"] == "hch-at-old"
    ...
    def test_double_check_uses_disk_when_already_rotated(self, tmp_path, monkeypatch):
        ...
        path = tmp_path / "honcho.json"
        _write(path, {"hosts": {"Zeloo": _host_block(refresh="hch-rt-fresh", expires_at=10_000)}})
        stale_raw = {"hosts": {"Zeloo": _host_block(refresh="hch-rt-old", expires_at=100)}}
        stale_raw["hosts"]["Zeloo"]["apiKey"] = "hch-at-stale"
        monkeypatch.setattr(
            oauth, "_http_post_form_status",
            lambda *a, **k: pytest.fail("must not refresh; disk token is fresh"),
        )
        token, refreshed = oauth.ensure_fresh_token(path, "Zeloo", stale_raw, now=1000)
...
class TestInstallGrant:
    def test_deep_merges_config_and_preserves_other_hosts(self, tmp_path):
        ...
        _write(path, {
            ...
            "hosts": {
                ...
                "Zeloo": {"workspace": "Zeloo", "saveMessages": False},
            },
        })
        grant = {
        ...
        assert h["workspace"] == "Zeloo"  # pre-existing key preserved
    ...
    def test_rejects_grant_without_tokens(self, tmp_path):
        ...
        with pytest.raises(ValueError):
            oauth.install_grant(
                path, "Zeloo", {"access_token": "hch-at-x"},  # no refresh_token
