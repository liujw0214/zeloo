...
The ``Zeloo update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m zeloo_cli.main gateway run`` — image name
    ``pythonw``, not ``Zeloo.exe`` — so the ``taskkill /IM Zeloo.exe`` guard
    ...
    # The Zeloo.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM Zeloo.exe' in text
