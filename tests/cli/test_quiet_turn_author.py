"""``Zeloo chat -Q`` passes the dispatcher's ZELOO_TURN_AUTHOR to ``run_conversation`` as ``turn_author``.
...
def _plain_one_shot_env(monkeypatch):
    monkeypatch.delenv("ZELOO_KANBAN_GOAL_MODE", raising=False)
    monkeypatch.delenv("ZELOO_KANBAN_TASK", raising=False)


def _run(monkeypatch, env_value, run_conversation=None):
    """One quiet turn with ZELOO_TURN_AUTHOR set to ``env_value`` (unset for None); returns the recorded call kwargs."""
