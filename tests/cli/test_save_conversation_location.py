...
Regression: the old implementation wrote ``zeloo_conversation_<ts>.json``
to the current working directory (CWD). Users who ran /save expected the
file to be discoverable via ``Zeloo sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.Zeloo/sessions/saved/`` and prints
...
def zeloo_home(tmp_path, monkeypatch):
    ...
    # Clear any cached zeloo_home computation
    import zeloo_constants
    if hasattr(zeloo_constants, "_zeloo_home_cache"):
        zeloo_constants._zeloo_home_cache = None
    return home
...
def test_save_conversation_writes_under_zeloo_home(zeloo_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.Zeloo/sessions/saved/, not CWD."""
    # Change CWD to a different directory to prove the file does NOT go there.
    work = tmp_path / "somewhere-else"
    work.mkdir()
    monkeypatch.chdir(work)

    # Import fresh to pick up the ZELOO_HOME fixture
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "zeloo_constants"]:
    ...
    cli.ZELOOCLI.save_conversation(stub, "/save json")

    # File must NOT be in CWD
    cwd_leak = list(work.glob("zeloo_conversation_*.json"))
    assert not cwd_leak, f"snapshot leaked to CWD: {cwd_leak}"

    # File MUST be under ~/.Zeloo/sessions/saved/
    saved_dir = zeloo_home / "sessions" / "saved"
    assert saved_dir.is_dir(), "expected saved/ subdirectory to be created"
    files = list(saved_dir.glob("zeloo_conversation_*.json"))
    ...
    assert "Zeloo --resume 20260101_120000_abc123" in out, out


def test_save_conversation_empty_history_does_nothing(zeloo_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "zeloo_constants"]:
        sys.modules.pop(mod, None)
    import cli
...
def test_save_conversation_bare_shows_usage(zeloo_home, capsys):
    ...
    cli.ZELOOCLI.save_conversation(stub, "/save")
...
def test_save_conversation_bad_format_shows_usage(zeloo_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "zeloo_constants"]:
        sys.modules.pop(mod, None)
    import cli

    stub = _make_stub_cli([{"role": "user", "content": "hi"}])
    cli.ZELOOCLI.save_conversation(stub, "/save pdf")
