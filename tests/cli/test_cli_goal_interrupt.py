...
minimal ``ZELOOCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def zeloo_home(tmp_path, monkeypatch):
    """Isolated ZELOO_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".Zeloo"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("ZELOO_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves ZELOO_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal ZELOOCLI stub with an active goal wired in."""
...
class TestInterruptAutoPause:
    ...
    def test_interrupted_turn_is_resumable(self, zeloo_home):
        ...
        with patch("zeloo_cli.goals.judge_goal"):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, zeloo_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, zeloo_home):
        ...
        src = inspect.getsource(ZELOOCLI.chat)
