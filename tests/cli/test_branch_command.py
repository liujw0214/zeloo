...
def session_db(tmp_path):
    ...
    os.environ["ZELOO_HOME"] = str(tmp_path / ".Zeloo")
    os.makedirs(tmp_path / ".Zeloo", exist_ok=True)
    from zeloo_state import SessionDB
    db = SessionDB(db_path=tmp_path / ".Zeloo" / "test_sessions.db")
    yield db
    db.close()
...
def cli_instance(tmp_path, session_db):
    """Create a minimal ZELOOCLI-like object for testing _handle_branch_command."""
...
class TestBranchCommandCLI:
    ...
    def test_branch_creates_new_session(self, cli_instance, session_db):
        ...
        ZELOOCLI._handle_branch_command(cli_instance, "/branch")
    ...
    def test_branch_with_custom_name(self, cli_instance, session_db):
        ...
        ZELOOCLI._handle_branch_command(cli_instance, "/branch refactor approach")
