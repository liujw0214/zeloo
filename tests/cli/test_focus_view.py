...
class TestStatusBarSegment:
    ...
    def test_text_renderer_includes_the_badge_at_every_width_tier(self, width):
        ...
        host = ZELOOCLI.__new__(ZELOOCLI)
        host.model = "opus"
        ...
        with patch.object(ZELOOCLI, "_get_status_bar_snapshot", return_value=snapshot), \
             patch.object(ZELOOCLI, "_is_session_yolo_active", return_value=False):
            text = ZELOOCLI._build_status_bar_text(host, width=width)
