...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.Zeloo/Zeloo-agent/utils.py)
...
2. ``Zeloo update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``zeloo_cli/main.py`` (the ``Zeloo update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``zeloo_constants`` after the
