...
def _reset_configured_delete_override_warned_paths():
    ...
    zeloo_state_wal._delete_overridden_warned_paths.clear()
    yield
    zeloo_state_wal._delete_overridden_warned_paths.clear()
...
def test_wal_probe_unknown_never_emits_set_pragma(monkeypatch, tmp_path, caplog):
    ...
    zeloo_state_wal._wal_probe_unknown_paths.clear()
    ...
    try:
        ...
        monkeypatch.setattr("zeloo_state_wal._on_disk_journal_mode", lambda _conn: None)
        conn = sqlite3.connect(str(db_path), factory=_SpyConnection)
        try:
            with caplog.at_level(logging.WARNING, logger="zeloo_state_wal"):
...
def test_resolve_journal_mode_uses_real_database_config(monkeypatch, tmp_path):
    from zeloo_state_wal import resolve_journal_mode

    _configure_mode(monkeypatch, tmp_path, "DELETE")
    assert resolve_journal_mode() == "delete"


def test_new_nonsecret_zeloo_env_override_is_not_exposed(monkeypatch, tmp_path):
    from zeloo_state_wal import resolve_journal_mode

    _configure_mode(monkeypatch, tmp_path, "wal")
    monkeypatch.setenv("ZELOO_JOURNAL_MODE", "delete")
...
def test_real_db_openers_honor_configured_delete(monkeypatch, tmp_path):
    ...
    from zeloo_cli import kanban_db, projects_db
    ...
    discord = DiscordRecoveryStore(zeloo_home=tmp_path)
