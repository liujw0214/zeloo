"""Shared fixtures for the Zeloo-agent test suite.
...
2. **Isolated ZELOO_HOME.** ZELOO_HOME points to a per-test tempdir so
   code reading ``~/.Zeloo/*`` via ``get_zeloo_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".Zeloo"`` instead of the canonical
   ``get_zeloo_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No ZELOO_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox ZELOO_HOME before ANY test module is imported ──────────────────
# `zeloo_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_zeloo_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.Zeloo/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes ZELOO_HOME, but fixtures run
...
# the REAL Zeloo root — capture it BEFORE the sandbox rewires ZELOO_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.Zeloo (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("ZELOO_KANBAN_HOME", "").strip()
_PRE_SANDBOX_ZELOO_HOME = os.environ.get("ZELOO_HOME", "")


def _zeloo_home_points_at_production(value: str) -> bool:
    """True when a pre-set ZELOO_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    ZELOO_HOME=~/.Zeloo``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``zeloo_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.Zeloo`` — the escape vector that landed pytest
    ...
    ZELOO_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".Zeloo").resolve()
    except Exception:
        return True
...
if _zeloo_home_points_at_production(os.environ.get("ZELOO_HOME", "")):
    _SESSION_ZELOO_HOME = tempfile.mkdtemp(prefix="Zeloo-test-home-")
    os.environ["ZELOO_HOME"] = _SESSION_ZELOO_HOME
    atexit.register(shutil.rmtree, _SESSION_ZELOO_HOME, True)
...
# like a real CLI") — which used to disarm zeloo_state's live-DB guard in
# the child at the same moment the child lost the ZELOO_HOME redirect.
# ZELOO_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# zeloo_state_guard._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export ZELOO_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["ZELOO_TEST_ISOLATION"] = os.environ.get("ZELOO_HOME", "") or "1"

#: ZELOO_HOME as it stood when conftest was imported - i.e. before any test
...
ZELOO_HOME_AT_CONFTEST_IMPORT = os.environ.get("ZELOO_HOME", "")
...
# ZELOO_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_ZELOO_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``zeloo_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a Zeloo agent session inherits them and
    # zeloo_constants home-resolution helpers prefer them over monkeypatched
    ...
    "ZELOO_MAX_ITERATIONS",
    "ZELOO_SESSION_PLATFORM",
    "ZELOO_SESSION_CHAT_ID",
    "ZELOO_SESSION_CHAT_NAME",
    "ZELOO_SESSION_CHAT_TYPE",
    "ZELOO_SESSION_THREAD_ID",
    "ZELOO_SESSION_SOURCE",
    "ZELOO_SESSION_KEY",
    "ZELOO_GATEWAY_SESSION",
    "ZELOO_CRON_SESSION",
    "_ZELOO_GATEWAY",
    "ZELOO_PLATFORM",
    "ZELOO_MODEL",
    "ZELOO_INFERENCE_MODEL",
    "ZELOO_INFERENCE_PROVIDER",
    "ZELOO_TUI_PROVIDER",
    "ZELOO_MANAGED",
    "ZELOO_MANAGED_DIR",
    "ZELOO_DEV",
    "ZELOO_CONTAINER",
    "ZELOO_EPHEMERAL_SYSTEM_PROMPT",
    "ZELOO_TIMEZONE",
    "ZELOO_REDACT_SECRETS",
    "ZELOO_BACKGROUND_NOTIFICATIONS",
    "ZELOO_EXEC_ASK",
    "ZELOO_HOME_MODE",
    "ZELOO_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.Zeloo/kanban.db instead of the per-test ZELOO_HOME.
    "ZELOO_KANBAN_DB",
    "ZELOO_KANBAN_BOARD",
    "ZELOO_KANBAN_HOME",
    "ZELOO_KANBAN_WORKSPACES_ROOT",
    "ZELOO_KANBAN_LOGS_ROOT",
    "ZELOO_KANBAN_TASK",
    "ZELOO_KANBAN_WORKSPACE",
    "ZELOO_KANBAN_RUN_ID",
    "ZELOO_KANBAN_CLAIM_LOCK",
    "ZELOO_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    # otherwise-unrelated config tests away from the default "Zeloo" host.
    "ZELOO_HONCHO_HOST",
    ...
    "ZELOO_DASHBOARD_OAUTH_CLIENT_ID",
    "ZELOO_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and ZELOO_HOME to per-test tempdirs so code that
    reads ``~/.Zeloo/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral ZELOO_* vars that could change test semantics.
    for name in _ZELOO_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("ZELOO_HONCHO_HOST", "Zeloo")

    # 3. Redirect ZELOO_HOME to a per-test tempdir. Code that reads
    #    ``~/.Zeloo/*`` via ``get_zeloo_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.Zeloo/*`` via
    #    ``Path.home() / ".Zeloo"`` instead of ``get_zeloo_home()``
    #    is a bug to fix at the callsite.
    fake_zeloo_home = tmp_path / "zeloo_test"
    fake_zeloo_home.mkdir()
    (fake_zeloo_home / "sessions").mkdir()
    (fake_zeloo_home / "cron").mkdir()
    (fake_zeloo_home / "memories").mkdir()
    (fake_zeloo_home / "skills").mkdir()
    monkeypatch.setenv("ZELOO_HOME", str(fake_zeloo_home))
    ...
    # zeloo_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("ZELOO_TEST_ISOLATION", str(fake_zeloo_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("ZELOO_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. zeloo_state computes ``DEFAULT_DB_PATH = get_zeloo_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from zeloo_state import ...``) that
    ...
    zeloo_state_mod = sys.modules.get("zeloo_state")
    if zeloo_state_mod is not None and hasattr(zeloo_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            zeloo_state_mod, "DEFAULT_DB_PATH", fake_zeloo_home / "state.db"
        )
    ...
    monkeypatch.setenv("ZELOO_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.Zeloo/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import zeloo_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path ZELOO_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_zeloo_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from zeloo_cli import kanban_db_dispatch as _kbd_mod
    except Exception:
        return
...
# invocation), kanban writes silently pollute the real ~/.Zeloo. This autouse
...
# allow-list because test-level fixtures legitimately move ZELOO_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session ZELOO_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``ZELOO_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Zeloo root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_ZELOO_HOME and not _zeloo_home_points_at_production(
        _PRE_SANDBOX_ZELOO_HOME
    ):
        # ZELOO_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_zeloo_root().resolve()
    # No pre-existing ZELOO_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".Zeloo").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.Zeloo`` captured at import time. Hermetic tests that legitimately
    move ZELOO_HOME to sibling tempdirs are unaffected.

    Only patches when ``zeloo_cli.kanban_db_connect`` is *already imported*
    ...
    """
    _kdb = sys.modules.get("zeloo_cli.kanban_db")
    _kdbc = sys.modules.get("zeloo_cli.kanban_db_connect")
    if _kdb is None or _kdbc is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import zeloo_cli.kanban_db`
...
# ``zeloo_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Zeloo root. This fixture wires the
# test-side knobs:
...
#     installs where ZELOO_HOME is not ~/.Zeloo) into the guard's
...
# so subprocess children that import zeloo_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("zeloo_state")
    ...
    if _PRE_SANDBOX_ZELOO_HOME and not _zeloo_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_ZELOO_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Zeloo home override redirects every later
    # ``get_zeloo_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from zeloo_constants import get_zeloo_home_override, set_zeloo_home_override

        if get_zeloo_home_override() is not None:
            set_zeloo_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal Zeloo config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``Zeloo-gateway`` process
...
# correlated with a ``tests/zeloo_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> Zeloo-gateway``
...
def _wal_is_usable() -> bool:
    """True when Zeloo will actually put a database into WAL mode here.

    Zeloo refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Zeloo can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Zeloo managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``zeloo_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_zeloo_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_zeloo_home`` fixture
    redirects ``ZELOO_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.Zeloo/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``zeloo_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["ZELOO_VOICE_TTS"] = "1"``. The
...
#      ``zeloo_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``zeloo_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``zeloo_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``zeloo_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# Zeloo runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.Zeloo``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Zeloo falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Zeloo uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart Zeloo-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting Zeloo/python patterns are also blocked.
    """
    ...
    def _matches_zeloo_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _ZELOO_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_zeloo_gateway(cmd_str):
            return False
        try:
            tokens = _shlex.split(cmd_str)
        except ValueError:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch Zeloo-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m zeloo_cli.main".
                if (
                    "Zeloo" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live Zeloo-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting Zeloo/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `Zeloo update` (or the
        # equivalent `python -m zeloo_cli.main update`).  These commands
        ...
        if "update" in low and (
            # Zeloo update / Zeloo update --gateway / setsid bash -c ... Zeloo update
            ("Zeloo" in low and "update" in low.split())
            or
            # python -m zeloo_cli.main update --gateway
            ("zeloo_cli" in low and "update" in low.split())
            or
            # venv/bin/Zeloo update  (absolute path variant used in tests)
            (".venv/bin/Zeloo" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`Zeloo update` against the real checkout, fetching "
        ...
        # Block spawning a REAL gateway runtime (``python -m zeloo_cli.main
        # gateway run|start|restart``). ``_spawn_zeloo_action`` launches it
        # with start_new_session=True, so it outlives the pytest worker; the
        # child inherits the pytest-tmp ZELOO_HOME, resolves the DEVELOPER's
        # ``Zeloo-gateway`` systemd unit (a tmp home hashes to no profile
        ...
        # A gateway launched INSIDE a container (`docker exec … Zeloo gateway start`) cannot
        ...
        if (
            ...
            raise RuntimeError(
                ...
                "Zeloo gateway runtime that outlives the test (it is "
                "detached), restarts the developer's live gateway, and "
                "holds the webhook port. Patch the spawn seam where "
                "production reads it (zeloo_cli.web_server_gateway."
                "_spawn_zeloo_action), or mark with "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_ZELOO_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import zeloo_cli.voice as _voice
    except Exception:
