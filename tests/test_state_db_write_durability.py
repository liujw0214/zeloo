...
The defect: zeloo_state already knows macOS `fsync()` does not guarantee
...
import zeloo_state
from zeloo_state import repair_state_db_schema
...
def test_repair_path_has_no_bare_connects() -> None:
    ...
    """
    # The repair/probe helpers live in zeloo_state_repair; zeloo_state only
    # re-imports them.
    import zeloo_state_repair

    source = Path(zeloo_state_repair.__file__).read_text(encoding="utf-8")
    tree = ast.parse(source, filename=str(zeloo_state_repair.__file__))
