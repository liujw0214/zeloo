...
Uses a throwaway ZELOO_HOME so it never touches ~/.Zeloo. No real names.
"""
...
def _fresh_home():
    """Point ZELOO_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_zeloo_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["ZELOO_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER ZELOO_HOME is set.
...
# a different get_zeloo_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"ZELOO_HOME (throwaway): {HOME}")
