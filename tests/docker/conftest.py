...
Override the image with ``ZELOO_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("ZELOO_TEST_IMAGE", "Zeloo-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``ZELOO_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("ZELOO_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged Zeloo user
# ---------------------------------------------------------------------------
#
# Background: every Zeloo runtime path inside the container drops to UID
# 10000 (the ``Zeloo`` user) via ``s6-setuidgid Zeloo``. ``docker exec``
...
# fails (PermissionError swallowed) for Zeloo, so a test that ran as root
...
# helpers below run every probe under ``-u Zeloo`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: Zeloo).
