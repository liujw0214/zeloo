...
def test_container_sets_hosted_write_policy_env(built_image: str) -> None:
    script = (
        'test "$ZELOO_HOME" = "/opt/data" && '
        'test "$ZELOO_WRITE_SAFE_ROOT" = "/opt/data" && '
...
def test_zeloo_user_cannot_modify_install_but_can_write_data(built_image: str) -> None:
    script = textwrap.dedent(
        ...
        install_file = Path("/opt/Zeloo/agent/message_sanitization.py")
        try:
