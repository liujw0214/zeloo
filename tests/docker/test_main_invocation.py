...
def test_no_args_starts_ZELOO(built_image: str) -> None:
    """``docker run <image>`` should start Zeloo cleanly.
...
def test_chat_subcommand_passthrough(built_image: str) -> None:
    """``docker run <image> chat --help`` should exec ``Zeloo chat --help``.
...
def test_container_exit_code_matches_inner_exit(built_image: str) -> None:
    ...
    Critical for CI: ``docker run <image> Zeloo batch ...`` returns a
