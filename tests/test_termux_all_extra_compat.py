...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"Zeloo-agent[termux]"' in text
    assert '"Zeloo-agent[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"Zeloo-agent[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
