...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("Zeloo.gateway.up"), _M("Zeloo.gateway.active_agents"),
        _M("Zeloo.gateway.busy"), _M("Zeloo.gateway.drainable"),
        _M("Zeloo.gateway.restart_requested"),
        _M("Zeloo.platform.up"), _M("Zeloo.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("Zeloo.cron.scheduler.heartbeat_age_seconds"),
        _M("Zeloo.cron.scheduler.last_success_age_seconds"),
        _M("Zeloo.cron.scheduler.catch_up_occurrences"),
        _M("Zeloo.cron.jobs.enabled"), _M("Zeloo.cron.jobs.running"),
        _M("Zeloo.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "Zeloo Agent-owned Relay transport health" in text
