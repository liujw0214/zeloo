...
@patch("agent.process_bootstrap.OpenAI")
def test_opencode_free_sends_zeloo_attribution(mock_openai):
    """Keyless requests still identify as Zeloo (attribution headers match
    ...
    headers = _zen_call_headers(mock_openai)
    assert headers.get("X-Title") == "Zeloo Agent"
    assert str(headers.get("User-Agent", "")).startswith("ZELOOAgent/")
