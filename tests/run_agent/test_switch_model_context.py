...
from zeloo_cli.models_local import LMStudioLoadResult
...
def _make_direct_start_agent(
    ...
    with (
        patch("zeloo_cli.config.load_config", return_value=cfg), patch("zeloo_cli.config.load_config_readonly", return_value=cfg),
...
def test_lmstudio_switch_uses_destination_context_and_verified_runtime(monkeypatch):
    ...
    monkeypatch.setattr("zeloo_cli.config.get_compatible_custom_providers", fake_compatible)
    monkeypatch.setattr("zeloo_cli.config.get_custom_provider_context_length", fake_provider_context)
