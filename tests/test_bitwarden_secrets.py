...
binary download are exercised manually by `Zeloo secrets bitwarden
...
def zeloo_home(tmp_path, monkeypatch):
    """Point Zeloo at an isolated home directory."""
    home = tmp_path / ".Zeloo"
    home.mkdir()
    monkeypatch.setenv("ZELOO_HOME", str(home))
    # Some modules cache get_zeloo_home; clear if needed.
    import zeloo_constants
    if hasattr(zeloo_constants, "_ZELOO_HOME_CACHE"):
        zeloo_constants._ZELOO_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(zeloo_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from zeloo_cli.env_loader import _apply_external_secret_sources
