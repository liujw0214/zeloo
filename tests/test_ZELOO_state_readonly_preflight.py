...
- files inside the Zeloo home tree are repaired with ``chmod u+rw``
...
import zeloo_state
from zeloo_state import SessionDB, preflight_db_writability
...
def zeloo_home(tmp_path, monkeypatch):
    """Isolated ZELOO_HOME so the repair scope covers tmp DBs."""
    home = tmp_path / ".Zeloo"
    home.mkdir()
...
class TestRepairScope:
    def test_repairs_readonly_db_inside_home(self, zeloo_home):
        db = zeloo_home / "state.db"
        _make_db(db)
        os.chmod(db, 0o444)
    ...
    def test_repairs_readonly_sidecars(self, zeloo_home):
        db = zeloo_home / "state.db"
        _make_wal_db(db)
    ...
    def test_repairs_readonly_parent_directory(self, zeloo_home):
        sub = zeloo_home / "kanban"
        sub.mkdir()
        db = sub / "kanban.db"
        _make_db(db)
        os.chmod(sub, 0o555)
...
class TestRefusalOutsideScope:
    def test_actionable_error_names_file_and_chmod(self, zeloo_home, tmp_path):
    ...
    def test_wal_error_warns_against_deletion(self, zeloo_home, tmp_path):
        outside = tmp_path / "elsewhere"
        outside.mkdir()
        db = outside / "custom.db"
        _make_wal_db(db)
...
class TestSkips:
    ...
    def test_healthy_db_untouched(self, zeloo_home):
        db = zeloo_home / "state.db"
        _make_db(db)
...
class TestSessionDBIntegration:
    def test_sessiondb_selfheals_readonly_db_in_home(self, zeloo_home):
        db_path = zeloo_home / "state.db"
