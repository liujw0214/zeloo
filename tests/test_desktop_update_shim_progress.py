...
def progress(tmp_path):
    ...
    status = tmp_path / "Zeloo-update-status"
...
# Stands in for `Zeloo update`, and reports the stage that was on screen
...
n="$(cat "$ZELOO_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$ZELOO_TEST_CALLS"
for f in "$TMPDIR"/Zeloo-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$ZELOO_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$ZELOO_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "Zeloo-agent"
    (install_root / "venv" / "bin").mkdir(parents=True)
    Zeloo = install_root / "venv" / "bin" / "Zeloo"
    Zeloo.write_text(FAKE_ZELOO)
    Zeloo.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "ZELOO_TEST_CAPTURE": str(capture),
        "ZELOO_TEST_CALLS": str(calls),
        "ZELOO_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".Zeloo-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`Zeloo update` is the longest wait in the hand-off and the one the
