...
The slash worker that parses ``/heartbeat`` runs a ZELOOCLI whose watchdog queues the due prompt
...
@pytest.fixture()
def server(zeloo_home):
    with patch.dict("sys.modules", {"zeloo_cli.env_loader": MagicMock(), "zeloo_cli.banner": MagicMock()}):
...
def _arm_due(key: str):
    from zeloo_cli.heartbeat import HeartbeatManager, save_heartbeat
...
def test_notification_poller_fires_due_heartbeat_when_idle(server, session):
    ...
    from zeloo_cli.heartbeat import load_heartbeat
...
def test_heartbeat_tick_defers_when_busy_or_not_due(server, session, running, due):
    ...
    if not due:
        ...
        from zeloo_cli.heartbeat import save_heartbeat
...
def test_abandon_fire_never_overwrites_a_concurrent_pause(zeloo_home):
    from zeloo_cli.heartbeat import HeartbeatManager, load_heartbeat
