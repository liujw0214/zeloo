...
def test_session_operations_require_resolved_owner(tmp_path, monkeypatch, operation):
    ...
    monkeypatch.setattr(sessions, "_sessions", {
        "owned": {"flow": flow, "server_name": "reports", "zeloo_home": home, "httpd": None}
    })
    ...
    def invoke():
        ...
        return sessions.cancel_flow("owned", "reports", str(get_zeloo_home()))

    token = set_zeloo_home_override(tmp_path / "other")
    try:
        rejected = invoke()
    finally:
