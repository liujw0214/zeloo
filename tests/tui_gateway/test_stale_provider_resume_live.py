...
def live_home(monkeypatch):
    """A REAL isolated ZELOO_HOME with a config.yaml + state.db on disk."""
    tmp = Path(tempfile.mkdtemp(prefix="Zeloo-live-staleprov-"))
    home = tmp / ".Zeloo"
    ...
    # zeloo_constants caches the resolved home at first read — the env var
    ...
    import zeloo_cli.config as hconfig
    ...
    # conftest also re-pins zeloo_state.DEFAULT_DB_PATH per test; pin it to
    # THIS home so server._get_db() opens the same real state.db we seed.
    monkeypatch.setattr(zeloo_state, "DEFAULT_DB_PATH", home / "state.db")
    monkeypatch.setattr(server, "_db", None, raising=False)
    monkeypatch.setattr(server, "_db_error", None, raising=False)
    monkeypatch.setattr(server, "_zeloo_home", str(home), raising=False)
    yield home, server
