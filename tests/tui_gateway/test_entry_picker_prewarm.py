...
guard is covered in ``tests/zeloo_cli/test_picker_prewarm.py``):

- ``main()`` invokes ``zeloo_cli.model_switch_providers.prewarm_picker_cache_async``
...
import zeloo_cli.model_switch_providers
...
def _run_main(monkeypatch, events, *, prewarm=None):
    ...
    # entry.main() imports the helper lazily from zeloo_cli.model_switch,
