...
def zeloo_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves ZELOO_HOME.
...
@pytest.fixture()
def server(zeloo_home, monkeypatch):
    ...
    # Pin config resolution to the isolated ZELOO_HOME. Sibling test
    ...
    # module-level ``_zeloo_home = get_zeloo_home()`` snapshot freezes
    ...
    monkeypatch.setattr(mod, "_zeloo_home", zeloo_home)
...
def test_moa_bare_returns_usage(server, session, zeloo_home):
    _write_moa_config(zeloo_home, """
moa:
...
def test_goal_draft_uses_session_profile_without_blocking_rpc_reader(
    ...
    def draft(objective):
        observed.append(get_zeloo_home())
        started.set()
