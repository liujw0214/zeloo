...
child at the same moment the child lost the ``ZELOO_HOME`` redirect. That
pairing is exactly how fixture rows (dm:123 / chat-1 / wx-chat) landed in a
developer's production state.db.

``ZELOO_TEST_ISOLATION`` is Zeloo's own marker: the hermetic conftest
exports it (value = the isolation root) before any test module imports, it
inherits into children by default, and ``zeloo_state`` honors it as a
...
try:
    ...
    fired = True
print(json.dumps({{
    "armed": zeloo_state_guard._running_under_pytest(),
    "fired": fired,
}}))
"""
...
def test_conftest_exports_the_marker():
    ...
    assert os.environ.get("ZELOO_TEST_ISOLATION"), (
        "ZELOO_TEST_ISOLATION must be exported by tests/conftest.py so "
...
def test_marker_alone_reports_test_context(monkeypatch):
    ...
    monkeypatch.setenv("ZELOO_TEST_ISOLATION", "/tmp/some-isolation-root")
    assert zeloo_state_guard._running_under_pytest() is True
...
def test_no_signals_reports_production(monkeypatch):
    ...
    monkeypatch.delenv("PYTEST_CURRENT_TEST", raising=False)
    monkeypatch.delenv("PYTEST_VERSION", raising=False)
    monkeypatch.delenv("ZELOO_TEST_ISOLATION", raising=False)
    assert zeloo_state_guard._running_under_pytest() is False
...
def test_child_with_rebuilt_env_keeping_marker_refuses_production_db():
    ...
    stripped, ZELOO_HOME lost) but which keeps the marker must still refuse
    to open the production state.db."""
    env = _minimal_env(ZELOO_TEST_ISOLATION="/tmp/pytest-isolation-root")
...
def test_child_bypass_env_disarms_guard_even_with_marker():
    ...
    DB: ZELOO_STATE_DB_GUARD_BYPASS=1, not marker-stripping."""
    env = _minimal_env(
        ZELOO_TEST_ISOLATION="/tmp/pytest-isolation-root",
        ZELOO_STATE_DB_GUARD_BYPASS="1",
    )
    result = _spawn_probe(env)
