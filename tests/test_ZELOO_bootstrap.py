"""Tests for zeloo_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Zeloo entry point
(Zeloo, Zeloo-agent, Zeloo-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Zeloo entry point imports zeloo_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported zeloo_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("zeloo_bootstrap", None)
    import zeloo_bootstrap  # noqa: WPS433
    return zeloo_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Zeloo CLI
...
class TestEntryPointsImportBootstrap:
    """Every Zeloo entry point must import zeloo_bootstrap as its
    ...
    # Entry points that invoke Zeloo as a process.  Each one must
    # import zeloo_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "zeloo_cli/main.py",   # Zeloo CLI (console_script)
        "run_agent.py",          # Zeloo-agent (console_script)
        "acp_adapter/entry.py",  # Zeloo-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import zeloo_bootstrap' and that
        ...
        half-finished ``Zeloo update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``zeloo_bootstrap`` as a top-level module) leaves Zeloo
        ...
        """
        # Resolve relative to the Zeloo-agent repo root.  Tests live
        # at tests/test_zeloo_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # Zeloo start even when zeloo_bootstrap hasn't been
        ...
        assert first_import_name == "zeloo_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'zeloo_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import zeloo_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Zeloo's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("ZELOO_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["ZELOO_PYTHON_SRC_ROOT"] = env
            elif "ZELOO_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["ZELOO_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/Zeloo")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("ZELOO_PYTHON_SRC_ROOT", None)
            else:
                os.environ["ZELOO_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/Zeloo", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Zeloo root.  The guard must relocate Zeloo to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/Zeloo"])
        assert result[0] == "/opt/Zeloo"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Zeloo root.
        assert result.index("/opt/Zeloo") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["ZELOO_PYTHON_SRC_ROOT"] = "/env/Zeloo"
            hb.harden_import_path()
            assert sys.path[0] == "/env/Zeloo"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("ZELOO_PYTHON_SRC_ROOT", None)
