...
# ============================================================================
# Proves, on a real Windows machine, that a user who installs Zeloo the way
...
# actually touches:
#
#   INSTALL   - downloads the production Zeloo-Setup.exe from the website,
#               launches it HEADED, and AutoHotkey clicks Install, waits,
#               then clicks Launch. The real Electron Zeloo.exe must appear.
...
#   installer's `git clone` and `Zeloo update`'s `git fetch origin`
...
param(
    ...
    # open-app-update (from a desktop-installer install) and Zeloo-update /
    ...
    # they land.
    [ValidateSet("open-app-update", "Zeloo-desktop-app-update", "Zeloo-update", "desktop-installer@latest", "installer-script", "installer-script+desktop")]
    [string]$Route = "open-app-update",

    # The OLD version: the ref served as `main` while the installer runs,
    # i.e. what the user starts on. The published Zeloo-Setup.exe carries
    ...
    [string]$WorkRoot = $(if ($env:ZELOO_E2E_WORKROOT) { $env:ZELOO_E2E_WORKROOT } else { Join-Path $env:TEMP "Zeloo-desktop-gui-e2e" }),

    [string]$SetupExeUrl = "https://Zeloo-assets.nousresearch.com/Zeloo-Setup.exe",
...
$ZELOOHome  = Join-Path $WorkRoot "Zeloo-home"
$InstallDir  = Join-Path $ZELOOHome "Zeloo-agent"
...
function Invoke-Git([string[]]$GitArgs) {
    ...
    # if Zeloo ever adds a git command that calls something with ^ this will break, lol.
...
function Set-GitRedirect {
    # we redirect to our own repo so we can play around with what commit Zeloo thinks we're on.
...
function Get-DesktopExe {
    foreach ($c in @(
        (Join-Path $InstallDir "apps\desktop\release\win-unpacked\Zeloo.exe"),
        (Join-Path $InstallDir "apps\desktop\release\win-arm64-unpacked\Zeloo.exe")
    )) {
...
# Install-side state snapshot, taken BEFORE Test-ZELOORuns can throw: on
...
function Save-InstallSideState([string]$Label) {
    ...
    $logsDir = Join-Path $ZELOOHome "logs"
    if (Test-Path -LiteralPath $logsDir) {
        Copy-Item $logsDir (Join-Path $dest "Zeloo-logs") -Recurse -Force -ErrorAction SilentlyContinue
    }
    $resultFile = Join-Path $ZELOOHome ".Zeloo-update-result.json"
    ...
    Get-ChildItem -LiteralPath $ZELOOHome -ErrorAction SilentlyContinue |
        Select-Object Name, Length, LastWriteTime |
        Format-Table -AutoSize | Out-String |
        Set-Content (Join-Path $dest "Zeloo-home-ls.txt")
}

function Test-ZELOORuns([string]$Label) {
    Save-InstallSideState $Label
    $ZELOOExe = Join-Path $InstallDir "venv\Scripts\Zeloo.exe"
    Assert-True (Test-Path -LiteralPath $ZELOOExe) "$Label -- venv\Scripts\Zeloo.exe exists"
    & $ZELOOExe --version 2>&1 | ForEach-Object { Write-Host "    Zeloo --version| $_" }
    Assert-True ($LASTEXITCODE -eq 0) "$Label -- Zeloo --version exits 0"
}
...
function Invoke-RefInstaller {
    ...
    $flags = @("-SkipSetup", "-ZELOOHome", $ZELOOHome, "-InstallDir", $InstallDir)
...
function Invoke-ZELOOUpdate {
    # The venv updater. --yes reaches the update subcommand only in later
    # releases; ask the installed binary, never parse its source.
    $ZELOOExe = Join-Path $InstallDir "venv\Scripts\Zeloo.exe"
    $updateArgs = @("update")
    $prevEap = $ErrorActionPreference; $ErrorActionPreference = "Continue"
    $helpText = & $ZELOOExe update --help 2>&1 | Out-String
    if ($helpText -match '--yes') { $updateArgs += "--yes" }
    New-Item -ItemType Directory -Path (Join-Path $WorkRoot "logs") -Force | Out-Null
    $log = Join-Path $WorkRoot "logs\update.log"
    Push-Location $InstallDir
    try {
        & $ZELOOExe @updateArgs 2>&1 | Add-TsPrefix | Out-File -Encoding UTF8 $log
        $updateExit = $LASTEXITCODE
    } finally {
        Pop-Location
        $ErrorActionPreference = $prevEap
    }
    Write-LogGroup "Zeloo update transcript" $log
    Assert-True ($updateExit -eq 0) "Zeloo update exited $updateExit (expected 0)"
}

function Invoke-ZELOODesktopAppUpdate([string]$TargetSha) {
    # The Zeloo-desktop launch surface: `Zeloo desktop` runs its whole
    ...
    $prevCap = $env:ZELOO_E2E_CAPTURE_LAUNCH
    $env:PYTHONPATH = if ($prevPy) { "$capDir;$prevPy" } else { $capDir }
    $env:ZELOO_E2E_CAPTURE_LAUNCH = $spec
    $prevEap = $ErrorActionPreference; $ErrorActionPreference = "Continue"
    Push-Location $InstallDir
    try {
        & $ZELOOExe desktop 2>&1 | Add-TsPrefix | Out-File -Encoding UTF8 $log
        $capExit = $LASTEXITCODE
    } finally {
        Pop-Location
        $ErrorActionPreference = $prevEap
        $env:PYTHONPATH = $prevPy
        $env:ZELOO_E2E_CAPTURE_LAUNCH = $prevCap
    }
    Write-LogGroup "Zeloo desktop (launch capture) transcript" $log
    Assert-True ($capExit -eq 0) "Zeloo desktop exited 0 during launch capture"
    ...
    try {
        & $node "launch-from-spec.mjs" --spec $spec `
            --result (Join-Path $ZELOOHome ".Zeloo-update-result.json") `
    ...
    Assert-True ($driveExit -eq 0) "app driven via captured Zeloo desktop spec; update completed"
}
...
function Stop-ZELOOAppProcesses([string]$Label) {
    # Close the desktop app the blunt way between phases (a user quitting).
    # Only Zeloo.exe (Electron) -- never Zeloo.exe (the venv CLI shim).
    $procs = @(Get-Process -Name "Zeloo" -ErrorAction SilentlyContinue)
    foreach ($p in $procs) {
        try { Stop-Process -Id $p.Id -Force -ErrorAction SilentlyContinue } catch {}
    }
    if ($procs.Count -gt 0) {
        Write-Host "  [$Label] stopped $($procs.Count) Zeloo.exe process(es)"
        Start-Sleep -Seconds 3
    }
}

function Get-ManagedNode {
    # `Zeloo update`/desktop builds use the Zeloo-managed Node; use the same
    # one to run the Playwright driver so no system Node is required.
    $candidates = @(
        (Join-Path $ZELOOHome "node\node.exe"),
        (Join-Path $ZELOOHome "bin\node\node.exe"),
...
# ----------------------------------------------------------------------------
# Phase: install-gui -- website Zeloo-Setup.exe, headed, AHK-driven
# ----------------------------------------------------------------------------
function Invoke-PhaseInstallGui {
    ...
    Write-Step "$($Mode.ToUpper()) (GUI): Zeloo-Setup.exe from the website, headed, AHK clicks"
    ...
    $setupExe = Join-Path $WorkRoot "Zeloo-Setup.exe"
    if (-not (Test-Path -LiteralPath $setupExe)) {
        Write-Host "  downloading $SetupExeUrl"
        Invoke-WebRequest -Uri $SetupExeUrl -OutFile $setupExe
    }
    Assert-True ((Get-Item $setupExe).Length -gt 1MB) "Zeloo-Setup.exe downloaded ($([math]::Round((Get-Item $setupExe).Length / 1MB, 1)) MB)"
    ...
    Remove-Item Env:ZELOO_SETUP_DEV_REPO_ROOT -ErrorAction SilentlyContinue
    New-Item -ItemType Directory -Path $ZELOOHome -Force | Out-Null
    ...
    try {
        ...
        Write-Host "  Zeloo-Setup.exe launched (pid $($installer.Id))"

        # Drive it: Install click -> wait -> Launch click -> Zeloo.exe window.
        ...
        $ahk = Start-Process -FilePath $ahkExe `
            -ArgumentList (Join-Path $AhkDir "install-and-launch.ahk"), $ahkLog, "Zeloo-Setup.exe", (Join-Path $ZELOOHome "logs\bootstrap-installer.log") `
            -PassThru
        ...
        # actually be running.
        Assert-True ($null -ne (Get-Process -Name "Zeloo" -ErrorAction SilentlyContinue)) "Zeloo.exe process is running (installer Launch hand-off worked)"

        # Installer should have exited after Launch.
        if (-not $installer.HasExited) {
            Start-Sleep -Seconds 10
        }
        Assert-True $installer.HasExited "Zeloo-Setup.exe exited after Launch"
    }
    finally {
        Stop-DesktopRecorder $recorder (Join-Path $proof "desktop-frames")
        # Surface the installer's own log win or lose, full and folded.
        $bootLog = Join-Path $ZELOOHome "logs\bootstrap-installer.log"
    ...
    Stop-ZELOOAppProcesses "post-install"
    ...
    Test-ZELOORuns "post-$Mode-gui"
    Assert-True ($null -ne (Get-DesktopExe)) "packaged Desktop Zeloo.exe exists"
...
function Invoke-GuiUpdateDesktopRoute([string]$TargetSha) {
    ...
    Assert-True ($null -ne $desktopExe) "packaged Zeloo.exe present before update"
    ...
    try {
        ...
        # `Zeloo update` + desktop rebuild + relaunch. Which updater depends
        # on the installed checkout, and BOTH are production paths:
        #   * checkouts shipping scripts/desktop-update.ps1 -> that script,
        #     which writes .Zeloo-update-result.json on every exit;
        #   * older checkouts -> the staged Zeloo-setup.exe --update flow,
        #     which does NOT write the result JSON.
        # So: poll for COMPLETION = (result JSON) OR (checkout reached the
        # target sha AND the marker is gone). The sha/marker/Zeloo/relaunch
        ...
        $updateLog = Join-Path $ZELOOHome "logs\update.log"
        $updateLogPos = 0
        ...
        Test-ZELOORuns "post-update"
        Assert-True ($null -ne (Get-DesktopExe)) "Zeloo.exe still present after update"
        ...
        Write-Host "  waiting for the relaunched Zeloo.exe ..."
        $rDeadline = (Get-Date).AddMinutes(5)
        $relaunched = $null
        while ((Get-Date) -lt $rDeadline) {
            $relaunched = Get-Process -Name "Zeloo" -ErrorAction SilentlyContinue
            if ($relaunched) { break }
        ...
        # Foreground the relaunched Zeloo window so the proof screenshot
        ...
        try {
            $mainProc = Get-Process -Name "Zeloo" -ErrorAction SilentlyContinue |
...
'@ -ErrorAction SilentlyContinue
        ...
        Stop-DesktopRecorder $recorder (Join-Path $proof "desktop-frames")
        $handoffLog = Join-Path $ZELOOHome "logs\desktop-update-handoff.log"
        ...
        Stop-ZELOOAppProcesses "post-update"
    }
}

function Invoke-PhaseInstall {
    # Dispatch on the install axis. Each arm ends with the same contract:
    # checkout at OLD, Zeloo runs, and state carries how OLD landed so any
    ...
    switch ($InstallMethod) {
        ...
        "installer-script" {
            ...
            Test-ZELOORuns "post-install-script"
        }
        "installer-script+desktop" {
            ...
            Assert-True ((Get-InstalledHead) -eq $state.old) "installed checkout is at OLD"
            Test-ZELOORuns "post-install-script-desktop"
...
function Invoke-PhaseUpdate {
    ...
    New-Item -ItemType File -Path (Join-Path $ZELOOHome ".skip_upstream_prompt") -Force | Out-Null
    ...
    switch ($Route) {
        ...
        "Zeloo-desktop-app-update" {
            Invoke-ZELOODesktopAppUpdate $state.current
        }
        "Zeloo-update" {
            Invoke-ZELOOUpdate
        }
        ...
        "desktop-installer@latest" {
            # A user re-downloading Zeloo-Setup.exe and clicking Install over
...
function Invoke-CheckedPhaseUpdate {
    ...
    foreach ($oldLog in @((Join-Path $WorkRoot "logs\update.log"), (Join-Path $ZELOOHome "logs\desktop.log"))) {
