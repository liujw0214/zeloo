...
# (Zeloo-Setup.dmg from the website) can reach HEAD.
#
...
#              Zeloo-desktop-app-update  capture `Zeloo desktop`'s spawn,
#                                         launch the spec under Playwright,
#                                         click Update now
#              Zeloo-update              CLI update from the installed venv
...
#     --update-method open-app-update|Zeloo-desktop-app-update
...
UPDATE_METHOD=""
INSTALL_REF=""
DMG_URL="https://Zeloo-assets.nousresearch.com/Zeloo-Setup.dmg"
...
case "$UPDATE_METHOD" in
  open-app-update|Zeloo-desktop-app-update|Zeloo-update|installer-script|installer-script+desktop) ;;
...
WORK_ROOT="${ZELOO_E2E_WORKROOT:-${RUNNER_TEMP:-${TMPDIR:-/tmp}}/Zeloo-macos-desktop-e2e}"
...
EOF
  ...
  export INSTALL_DIR="$ZELOO_HOME/Zeloo-agent"
}
...
phase_stage() {
  ...
  arm_redirect
  mkdir -p "$ZELOO_HOME"
  touch "$ZELOO_HOME/.skip_upstream_prompt"
...
find_installed_app() {
  ...
  local cand
  for cand in \
    "$INSTALL_DIR/apps/desktop/release/mac-arm64/Zeloo.app" \
    "$INSTALL_DIR/apps/desktop/release/mac/Zeloo.app" \
    "/Applications/Zeloo.app"; do
...
phase_install() {
  ...
  . "$STATE"
  arm_redirect
  step "installing OLD ($OLD_REF) via the published Zeloo-Setup.dmg"

  local dmg="$WORK_ROOT/Zeloo-Setup.dmg"
  ...
  # "Install Zeloo" button with native input.
  local rc=0
  ...
  log_group "Zeloo-Setup (dmg bootstrap) transcript" "$LOG_DIR/bootstrap-install.log"
  ...
  "$Zeloo" --version 2>&1 | ts_prefix > "$LOG_DIR/version-old.log" || fail "Zeloo --version failed after install"
  ok "Zeloo --version works: $(head -c 120 "$LOG_DIR/version-old.log" | tr -d '\n')"
  find_installed_app >/dev/null || fail "no installed Zeloo.app after the dmg bootstrap"
...
phase_update() {
  ...
  case "$UPDATE_METHOD" in
    Zeloo-update)
      ...
      # update subcommand only in later releases; ask the installed Zeloo.
      local Zeloo="$INSTALL_DIR/venv/bin/Zeloo"
      local update_cmd=("$Zeloo" update)
      ...
      [ "$rc" -eq 0 ] || fail "Zeloo update exited $rc; transcript above"
      ;;
    installer-script)
    ...
    installer-script+desktop)
      ...
      for cand in \
        "$INSTALL_DIR/apps/desktop/release/mac-arm64/Zeloo.app" \
        "$INSTALL_DIR/apps/desktop/release/mac/Zeloo.app"; do
        [ -d "$cand" ] && { head_app="$cand"; break; }
      done
      [ -n "$head_app" ] || fail "no built Zeloo.app under the checkout after the +desktop update"
    ...
    open-app-update)
      ...
      # which is exactly what the app's children (git, Zeloo update) need.
      local app app_bin
...
PYEOF
    ...
    Zeloo-desktop-app-update)
      ...
      [ "$rc" -eq 0 ] || fail "Zeloo desktop exited $rc during launch capture"
      [ -f "$spec.captured" ] || fail "Zeloo desktop exited 0 but no launch was captured"
  ...
  mkdir -p "$ildest"
  cp -R "$HOME_SANDBOX/.Zeloo/logs" "$ildest/Zeloo-logs" 2>/dev/null || true
  local ud="$HOME_SANDBOX/Library/Application Support/Zeloo"
  ...
  "$INSTALL_DIR/venv/bin/Zeloo" --version 2>&1 | ts_prefix > "$LOG_DIR/version-head.log" \
    || fail "Zeloo --version failed after update"
  ok "Zeloo --version works post-update"
