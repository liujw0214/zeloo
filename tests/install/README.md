...
2. Install: run the old release's own installer script. Make sure that the checkout is at the old commit and that `Zeloo --version` works.
3. Desktop smoke: run `Zeloo desktop --build-only` from the installed CLI. This proves that the installed version can build the desktop app. If the installed version does not have this flag, the phase reports a skip and continues.
4. Update: move `main` to HEAD. Apply one update method. Make sure that the checkout is at HEAD and that `Zeloo --version` works.
5. Desktop smoke again, at HEAD.

The windows GUI driver replaces phases 2 and 4 when the install method is `desktop-installer@latest`. It downloads the published `Zeloo-Setup.exe`, clicks through the installer window with AutoHotkey, and clicks "Update now" in the running app with Playwright.
...
- `desktop-installer@latest`: the published GUI installer (`Zeloo-Setup.exe` on windows, `Zeloo-Setup.dmg` on macos), driven through the real user flow.
...
Running the drivers locally: don't, except in a disposable VM. The windows driver kills every process named Zeloo during teardown and the macos driver operates on `/Applications/Zeloo.app`; on a machine with a real Zeloo install they will interfere with it.
