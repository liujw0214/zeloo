...
; Drive the REAL Zeloo-Setup.exe (Tauri bootstrap installer) window:
; click Install, wait for the install to finish, click Launch, and wait for
; the real Zeloo.exe (Electron desktop) window to appear.
;
; Adapted from @ethernet8023's e2e/windows/install-Zeloo-desktop.ahk
...
setupExe := A_Args.Length >= 2 ? A_Args[2] : "Zeloo-Setup.exe"
...
Log("Launch clicked; waiting for the Zeloo desktop app window")
...
try {
    ...
    throw Error("Zeloo.exe real-sized window did not appear within 300s of clicking Launch")
}
