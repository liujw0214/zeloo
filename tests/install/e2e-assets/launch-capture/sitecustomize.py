"""Driver-side spawn interception for Zeloo desktop E2E legs.

The installed ``Zeloo`` is a venv console script, so its interpreter
imports ``sitecustomize`` at startup when this directory is on
``PYTHONPATH``. Behind an explicit env-var opt-in the module wraps
``subprocess.run`` so the FINAL electron launch call of ``Zeloo
...
Opt-in: ``ZELOO_E2E_CAPTURE_LAUNCH=<path>`` -- the spec is written
there, and the marker file ``<path>.captured`` distinguishes "Zeloo
...
_SPEC_PATH = os.environ.get("ZELOO_E2E_CAPTURE_LAUNCH")
...
if _SPEC_PATH:
    ...
    def _match_shape(argv: "list[str]") -> str:
        ...
        # apps/desktop/release/ (win-unpacked/Zeloo.exe, linux-unpacked/...,
        # mac*/Zeloo.app/Contents/MacOS/...).
