// drive-update.cjs — launch the INSTALLED Zeloo.exe (real Electron desktop
...
//
//   node <this file> <path-to-Zeloo.exe> <proof-dir>
//
...
if (!exePath || !proofDir) {
  console.error('usage: node drive-update.cjs <Zeloo.exe> <proof-dir>')
  process.exit(1)
}
...
async function main() {
  ...
  const app = await _electron.launch({
    ...
    // Inherit the driver's env: ZELOO_HOME (isolated install) and
  ...
  // The real backend (`Zeloo serve`) is booting underneath; give it time.
  ...
  // buttonless boot-progress card ("Starting Zeloo... 86%"), then the
  ...
  if (!visible) {
    ...
    const status = await page.evaluate(() =>
      window.ZELOODesktop?.updates?.check?.() ?? Promise.resolve('no updates.check bridge')
  ...
  // The "Updating Zeloo — this window will close" overlay should appear,
  ...
  // the staged binary), which claims ZELOO_HOME/.Zeloo-update-in-progress
  ...
  // proved the desktop had exited and `Zeloo update` was already running).
  //
  ...
  const ZELOOHome = process.env.ZELOO_HOME
  const markerPath = ZELOOHome ? path.join(ZELOOHome, '.Zeloo-update-in-progress') : null
  const resultPath = ZELOOHome ? path.join(ZELOOHome, '.Zeloo-update-result.json') : null
