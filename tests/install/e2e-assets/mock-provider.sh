...
#   mock_start <workroot>      start, write config into $ZELOO_HOME
#   mock_stop                  kill the background server
#
# Requires: ASSETS (e2e-assets dir), LOG_DIR, ZELOO_HOME, ok/fail helpers.
...
mock_start() {
  ...
  cat > "$ZELOO_HOME/config.yaml" <<EOF
model:
  default: mock-model
  provider: mock
providers:
...
EOF
  ...
  ok "provider 'mock' configured in $ZELOO_HOME (api $url/v1)"
}
