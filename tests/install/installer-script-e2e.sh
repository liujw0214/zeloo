...
#              install landed on OLD with a working `Zeloo`
#   update     advance served main to HEAD, apply ONE update method, assert
#              the checkout landed on HEAD with a working `Zeloo`
#
# Usage:
#   tests/install/installer-script-e2e.sh --update-method Zeloo-update|installer-script|installer-script+desktop
...
case "$UPDATE_METHOD" in
  Zeloo-update|installer-script|installer-script+desktop|Zeloo-desktop-app-update) ;;
  *) echo "error: --update-method must be Zeloo-update, installer-script, installer-script+desktop or Zeloo-desktop-app-update, got '$UPDATE_METHOD'" >&2; exit 1 ;;
esac
...
WORK_ROOT="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/Zeloo-installer-script-e2e"
LOG_DIR="${ZELOO_E2E_LOG_DIR:-$WORK_ROOT/logs}"
...
# Isolated HOME: the runner's real one may carry a preinstalled Zeloo or a
# developer config, and old installer scripts hardcode $HOME/.Zeloo (the
# ZELOO_HOME env override is newer than tags we sample). GIT_CONFIG_GLOBAL
# above keeps working -- an explicit path wins over $HOME/.gitconfig.
export HOME="$WORK_ROOT/home"
mkdir -p "$HOME/.local/bin"
export PATH="$HOME/.local/bin:$PATH"
export ZELOO_HOME="$HOME/.Zeloo"
mkdir -p "$ZELOO_HOME"
...
assert_desktop_artifact() {
  ...
  local cand
  for cand in \
    "$release_dir/linux-unpacked/Zeloo" \
    "$release_dir/linux-unpacked/Zeloo" \
    "$release_dir/mac-arm64/Zeloo.app" \
    "$release_dir/mac/Zeloo.app"; do
...
assert_checkout() {
  ...
  ok "checkout is $2 ($1)"
  local Zeloo="$INSTALL_DIR/venv/bin/Zeloo"
  [ -x "$Zeloo" ] || fail "no Zeloo console script at $Zeloo"
  "$Zeloo" --version 2>&1 | ts_prefix > "$LOG_DIR/version-$2.log" \
    || fail "Zeloo --version failed after $2; log in $LOG_DIR/version-$2.log"
  ok "Zeloo --version works: $(head -c 120 "$LOG_DIR/version-$2.log" | tr -d '\n')"
}

smoke_desktop() {
  # $1: label (old|head). Prove the installed CLI can produce the desktop
  # app: `Zeloo desktop --build-only` runs the full desktop pipeline
  # (workspace install, renderer build, stamp write) and stops before the
  # launch -- the same call `Zeloo update` itself makes. Probe the
  # INSTALLED Zeloo for the flag rather than assuming this checkout's
  # surface: sampled OLD releases may predate `Zeloo desktop` or
  # --build-only entirely, and for them the phase skips, loudly.
  local Zeloo="$INSTALL_DIR/venv/bin/Zeloo"
  if ! "$Zeloo" desktop --help 2>/dev/null | grep -qF -- --build-only; then
    ok "Zeloo desktop --build-only not supported at $1; skipping desktop smoke"
    return 0
  fi
  local rc=0
  (cd "$INSTALL_DIR" && "$Zeloo" desktop --build-only < /dev/null 2>&1 \
    | ts_prefix > "$LOG_DIR/desktop-smoke-$1.log") || rc=$?
  log_group "Zeloo desktop --build-only ($1) transcript" "$LOG_DIR/desktop-smoke-$1.log"
  [ "$rc" -eq 0 ] || fail "Zeloo desktop --build-only ($1) exited $rc; transcript above"
  ok "Zeloo desktop --build-only works at $1"
...
case "$UPDATE_METHOD" in
  Zeloo-update)
    ...
    # installed Zeloo; older ones read the prompt from stdin, so close it.
    Zeloo="$INSTALL_DIR/venv/bin/Zeloo"
    if "$Zeloo" update --help 2>&1 | grep -qF -- --yes; then
      update_cmd=("$Zeloo" update --yes)
    else
      update_cmd=("$Zeloo" update)
    fi
    rc=0
    (cd "$INSTALL_DIR" && "${update_cmd[@]}" < /dev/null 2>&1 | ts_prefix > "$LOG_DIR/update.log") || rc=$?
    log_group "Zeloo update transcript" "$LOG_DIR/update.log"
    [ "$rc" -eq 0 ] || fail "Zeloo update exited $rc; transcript above, log at $LOG_DIR/update.log"
    ;;
  installer-script)
  ...
  Zeloo-desktop-app-update)
    # The real user surface: `Zeloo desktop` launches the app, the user
    ...
    # suite's own) is configured into ZELOO_HOME exactly like the dev:mock
    ...
    step "capturing the Zeloo desktop launch spec (build runs for real)"
    rc=0
    (cd "$INSTALL_DIR" && \
      PYTHONPATH="$ASSETS/launch-capture${PYTHONPATH:+:$PYTHONPATH}" \
      ZELOO_E2E_CAPTURE_LAUNCH="$SPEC" \
      "$Zeloo" desktop < /dev/null 2>&1 | ts_prefix > "$LOG_DIR/desktop-launch-capture.log") || rc=$?
    log_group "Zeloo desktop (launch capture) transcript" "$LOG_DIR/desktop-launch-capture.log"
    [ "$rc" -eq 0 ] || fail "Zeloo desktop exited $rc during launch capture; transcript above"
    # Exit 0 without a capture means a version that never reached its
    # launch - that must fail loudly, not pass as a no-op.
    [ -f "$SPEC.captured" ] || fail "Zeloo desktop exited 0 but no launch was captured at $SPEC"
...
mkdir -p "$ildest"
cp -R "$ZELOO_HOME/logs" "$ildest/Zeloo-logs" 2>/dev/null || true
if [ -n "${XDG_DATA_HOME:-}" ]; then
  cp -R "$XDG_DATA_HOME/Zeloo/logs" "$ildest/desktop-userdata-logs" 2>/dev/null || true
fi
cp "$ZELOO_HOME/.Zeloo-update-result.json" "$ildest" 2>/dev/null || true
ls -la "$ZELOO_HOME" > "$ildest/Zeloo-home-ls.txt" 2>/dev/null || true
