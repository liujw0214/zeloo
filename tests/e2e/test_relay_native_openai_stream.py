...
Relay runs its finalizer as soon as the provider stream ends — concurrently with Zeloo'
...
def _stream_through_relay(tmp_path, monkeypatch, response_body: bytes, *, finalize_before):
    ...
    Returns ``(zeloo_result, relay_llm_end_event)``."""
    ...
    monkeypatch.setenv("ZELOO_STREAM_RETRIES", "0")
...
def test_openai_stream_final_tool_call_delta_reaches_relay_parent_event(tmp_path, monkeypatch):
    ...
    zeloo_call = result.choices[0].message.tool_calls[0]
    assert (zeloo_call.function.name, zeloo_call.function.arguments) == ("read_file", '{"path": "/tmp/x"}')
