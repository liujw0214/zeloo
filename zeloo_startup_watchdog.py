...
timeout. Config is env-only (``ZELOO_STARTUP_WATCHDOG=0``,
``ZELOO_STARTUP_WATCHDOG_TIMEOUT_S``) because config.yaml parsing is itself in scope.
...
# Module singleton: the arm sites (zeloo_cli.main / zeloo_cli.gateway /
...
def _process_zeloo_home() -> Path:
    """ZELOO_HOME for diagnostic files — stdlib-only replica of the zeloo_constants default."""
...
def get_startup_watchdog_dump_path(home: Optional[Path] = None) -> Path:
    """Return ``<ZELOO_HOME>/logs/gateway-startup-watchdog.log``."""
...
def startup_watchdog_disabled() -> bool:
    """True when ``ZELOO_STARTUP_WATCHDOG`` opts out explicitly."""
...
def arm_startup_watchdog(
    ...
    *,
    ...
    ``ZELOO_STARTUP_WATCHDOG=0`` or when the thread could not be started.
    """
    global _handle
    try:
