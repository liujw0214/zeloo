...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with Zeloo-agent)
2. User plugins: ``$ZELOO_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``zeloo_agent.plugins``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$ZELOO_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$ZELOO_HOME/plugins/model-providers/`` if it exists."""
    try:
        from zeloo_constants import get_zeloo_home

        d = get_zeloo_home() / "plugins" / "model-providers"
...
def _installed_plugins_dir() -> Path | None:
    """Return ``$ZELOO_HOME/plugins/`` if it exists.

    This is where ``Zeloo plugins install`` clones a plugin — flat, one
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple ZELOO_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_zeloo_user_provider_{safe_name}"
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``zeloo_agent.plugins``
    ...
    * **Provider targets only.** The ``zeloo_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$ZELOO_HOME``) keep their
    ...
    try:
        from zeloo_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$ZELOO_HOME/plugins/model-providers/<name>/``
      2b. Plugins installed by ``Zeloo plugins install`` at
          ``$ZELOO_HOME/plugins/<name>/`` that declare ``kind: model-provider``
    ...
    # 0. Pip-installed plugins — entry points in the ``zeloo_agent.plugins``
    ...
    #    filesystem steps means a bundled or ``$ZELOO_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with Zeloo-agent.
    ...
    # 2. User plugins — under $ZELOO_HOME/plugins/model-providers/<name>/.
    ...
    # 2b. Plugins installed by ``Zeloo plugins install`` / the plugin index.
    #     Those clone into $ZELOO_HOME/plugins/<name>/ — flat, NOT under
