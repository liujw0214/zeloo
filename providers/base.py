...
def _profile_user_agent() -> str:
    """Return a ``Zeloo-cli/<version>`` UA string, with a stable fallback.
    ...
    """
    try:
        from zeloo_cli import __version__ as _ver  # lazy: avoid layer cycle at import time
        return f"Zeloo-cli/{_ver}"
    except Exception:
        return "Zeloo-cli"
...
class ProviderProfile:
    ...
    # describe how to launch it; zeloo_cli/auth.py's
    ...
    def create_client(self, **client_kwargs: Any) -> Any | None:
        ...
        a profile registered from ``~/.Zeloo/plugins/model-providers/`` or a
    ...
    def fetch_models(
        ...
        *,
        ...
        # Zeloo-cli UA so the catalog endpoint is reachable.
