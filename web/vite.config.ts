...
const BACKEND = process.env.ZELOO_DASHBOARD_URL ?? "http://127.0.0.1:9119";

/**
 * In production the Python `Zeloo dashboard` server injects a one-shot
 * session token into `index.html` (see `zeloo_cli/web_server.py`). The
 * Vite dev server serves its own `index.html`, so unless we forward that
 * token, every protected `/api/*` call 401s.
 *
 * This plugin fetches the running dashboard's `index.html` on each dev page
 * load, scrapes the `window.__ZELOO_SESSION_TOKEN__` assignment, and
 * re-injects it into the dev HTML. No-op in production builds.
 */
function ZELOODevToken(): Plugin {
  const TOKEN_RE = /window\.__ZELOO_SESSION_TOKEN__\s*=\s*"([^"]+)"/;
  const EMBEDDED_RE =
    /window\.__ZELOO_DASHBOARD_EMBEDDED_CHAT__\s*=\s*(true|false)/;

  return {
    name: "Zeloo:dev-session-token",
    apply: "serve",
    async transformIndexHtml() {
      try {
        ...
        if (!match) {
          console.warn(
            `[Zeloo] Could not find session token in ${BACKEND} — ` +
              `is \`Zeloo dashboard\` running? /api calls will 401.`,
          );
          return;
        }
        ...
        return [
          {
            ...
            injectTo: "head",
            children:
              `window.__ZELOO_SESSION_TOKEN__="${match[1]}";` +
              `window.__ZELOO_DASHBOARD_EMBEDDED_CHAT__=${embeddedJs};`,
          },
        ];
      } catch (err) {
        console.warn(
          `[Zeloo] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`Zeloo dashboard\` or set ZELOO_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      "@Zeloo/shared": path.resolve(__dirname, "../apps/shared/src"),
    },
  ...
  server: {
    proxy: {
      ...
      // Same host as `Zeloo dashboard` must serve these; Vite has no
