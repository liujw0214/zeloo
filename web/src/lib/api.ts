import { buildZELOOWebSocketUrl } from "@Zeloo/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/Zeloo/). The Python backend
// injects ``window.__ZELOO_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__ZELOO_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const ZELOO_BASE_PATH = readBasePath();
const BASE = ZELOO_BASE_PATH;
...
declare global {
  interface Window {
    __ZELOO_SESSION_TOKEN__?: string;
    __ZELOO_BASE_PATH__?: string;
...
const SESSION_HEADER = "X-Zeloo-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__ZELOO_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``Zeloo update``, ``Zeloo gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__ZELOO_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__ZELOO_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__ZELOO_SESSION_TOKEN__`` is valid, so the next
 ...
 *
 * The dashboard's gated-mode WS auth (``zeloo_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__ZELOO_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Zeloo-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``zeloo_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__ZELOO_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  updateZELOO: () =>
    fetchJSON<ActionResponse>("/api/Zeloo/update", { method: "POST" }),
  checkZELOOUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/Zeloo/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** Zeloo-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``Zeloo update``. */
  can_update_ZELOO?: boolean;
  config_path: string;
  ...
  /** NS-656: disk-usage rollup for the ZELOO_HOME volume. Absent on
   * older gateways. */
 ...
 * sample of the ZELOO_HOME volume — no staleness dimension, so no
 * sampled_at. */
