/**
 * ChatPage — embeds `Zeloo --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (zeloo_cli/web_server.py)                          .
 *          ▼                                                              .
...
const PTY_ATTACH_TOKEN_KEY = "Zeloo.pty.token.chat";
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__ZELOO_SESSION_TOKEN__ &&
    !window.__ZELOO_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `Zeloo dashboard`, not directly."
      : null,
  );
  ...
  const [chatPanelCollapsed, setChatPanelCollapsed] = useState(
    () => localStorage.getItem("Zeloo-chat-panel-collapsed") === "1",
  );
  const toggleChatPanel = useCallback(() => {
    setChatPanelCollapsed((prev) => {
      const next = !prev;
      localStorage.setItem("Zeloo-chat-panel-collapsed", next ? "1" : "0");
      return next;
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const gated = !!window.__ZELOO_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Zeloo TUI has enabled xterm mouse-events
    ...
    //      payloads upload to ZELOO_HOME/images then drive `/image`.
    //
    ...
    // must upload browser bytes to ZELOO_HOME/images, then drive `/image`
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[Zeloo-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__ZELOO_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets ZELOO_HOME pointed at the
    ...
    // forwarding them into Zeloo. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="Zeloo-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
