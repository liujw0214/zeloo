...
const SIDEBAR_COLLAPSED_KEY = "Zeloo-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="Zeloo-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="Zeloo-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateZELOO = status?.can_update_ZELOO === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'Zeloo update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "Zeloo update";
    return (
      t.status.updateZELOOConfirmMessage ??
      `This will run 'Zeloo update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateZELOOConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateZELOO) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateZELOO,
      runningLabel: t.status.updatingZELOO,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Zeloo gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateZELOOConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateZELOOConfirmTitle ?? `${t.status.updateZELOO}?`}
    />
    </>
  );
}
