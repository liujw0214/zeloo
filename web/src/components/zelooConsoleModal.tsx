...
interface ZELOOConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function ZELOOConsoleModal({ open, onClose }: ZELOOConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("Zeloo> ");
  const inputPromptRef = useRef("Zeloo> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "Zeloo> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Zeloo Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="Zeloo-console-title"
    >
      <div
        className={cn(
          themedBody,
