/**
 * Zeloo Dashboard Plugin SDK — typed contract (SPIKE)
 * ====================================================
 *
 * This is the public type surface for ``window.__ZELOO_PLUGIN_SDK__`` and
 * ``window.__ZELOO_PLUGINS__``, the globals the dashboard host exposes to
 ...
 * window.__ZELOO_PLUGIN_SDK__`` because:
 ...
 *
 * Versioning: bump ``ZELOOPluginSDK["sdkVersion"]`` (and the
 ...
 *   - Ship as a published ``@Zeloo/dashboard-plugin-sdk`` types package, or
 ...
 * of calling ``fetch`` with a hand-read ``window.__ZELOO_SESSION_TOKEN__``.
 */
...
// ---------------------------------------------------------------------------
// Registry surface (window.__ZELOO_PLUGINS__)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SDK surface (window.__ZELOO_PLUGIN_SDK__)
// ---------------------------------------------------------------------------

export interface ZELOOPluginSDK {
...
declare global {
  interface Window {
    __ZELOO_PLUGIN_SDK__?: ZELOOPluginSDK;
    __ZELOO_PLUGINS__?: PluginRegistry;
  }
}
