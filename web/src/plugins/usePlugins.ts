...
import { api, ZELOO_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "Zeloo:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${ZELOO_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${ZELOO_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?zeloo_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-Zeloo-plugin", manifest.name);
