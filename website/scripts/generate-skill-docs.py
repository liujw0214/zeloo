...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/Zeloo-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    zeloo_meta = (fm.get("metadata") or {}).get("Zeloo") or {}
    tags = zeloo_meta.get("tags") or []
    related = zeloo_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `Zeloo skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Zeloo Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Zeloo ships with a large built-in skill library copied into `~/.Zeloo/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Zeloo also syncs bundled skills on `Zeloo update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.Zeloo/skills/` tree, it is still shipped with Zeloo; restore it with `Zeloo skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with Zeloo-agent — install via Zeloo skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with Zeloo-agent under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
        "Zeloo skills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "Zeloo skills install official/blockchain/solana",
        "Zeloo skills install official/mlops/flash-attention",
        "```",
        "",
        ...
        "",
        "```bash",
        "Zeloo skills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):
