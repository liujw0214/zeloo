...
2. The unified Zeloo Skills Index at ``website/static/api/skills-index.json``,
    ...
    for base_dir, source_label in LOCAL_SKILL_DIRS:
        ...
        for root, _dirs, files in os.walk(base_path):
            ...
            if isinstance(metadata, dict):
                zeloo_meta = metadata.get("Zeloo", {})
                if isinstance(zeloo_meta, dict):
                    tags = zeloo_meta.get("tags", [])
            if not tags:
                tags = fm.get("tags", [])
