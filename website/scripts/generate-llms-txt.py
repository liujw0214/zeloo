#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Zeloo docs site.
...
LLM reads to learn what Zeloo does.

Both publish at:
  https://Zeloo-agent.nousresearch.com/docs/llms.txt
  https://Zeloo-agent.nousresearch.com/docs/llms-full.txt
...
SITE_BASE = "https://Zeloo-agent.nousresearch.com/docs"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Zeloo as a Python Library", None),
        ("guides/use-mcp-with-Zeloo", "Use MCP with Zeloo", None),
        ("guides/use-voice-mode-with-Zeloo", "Use Voice Mode with Zeloo", None),
        ("guides/use-soul-with-Zeloo", "Use SOUL.md with Zeloo", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Zeloo"),
...
def emit_llms_index() -> str:
    ...
    lines.append(
        ...
        "Zeloo-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/NousResearch/Zeloo-agent")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Zeloo Agent — Full Documentation\n",
        (
            "This file is the entire Zeloo Agent documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using Zeloo, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://Zeloo-agent.nousresearch.com/docs\n",
        "Short index: https://Zeloo-agent.nousresearch.com/docs/llms.txt\n",
        "\n---\n\n",
    ]
