...
Zeloo runs shell commands through a pluggable set of **terminal backends**.
...
installed under `~/.Zeloo/plugins/`, registering a backend the user selects
...
| `Zeloo setup` backend picker | `display_name`, `description`, `setup_instructions()`, `post_setup()` |
| Dashboard terminal-backend picker (probe status) | `probe()` |
| `Zeloo status` / `Zeloo doctor` | `doctor_checks()` |
...
```python title="~/.Zeloo/plugins/acmebox/__init__.py"
...
class AcmeBoxProvider(TerminalEnvironmentProvider):
    ...
    def cache_path_base(self):
        return "~/.Zeloo"    # where synced cache files land, or None
...
```yaml title="~/.Zeloo/plugins/acmebox/plugin.yaml"
name: acmebox
version: 0.1.0
...
```bash
Zeloo plugins enable acmebox
Zeloo config set terminal.backend acmebox
```
