...
Plugins can launch and supervise fresh Zeloo child sessions without importing
...
active agent turn fails closed with `No active Zeloo parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Zeloo can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Zeloo's existing unsafe-tool block remains enforced.
