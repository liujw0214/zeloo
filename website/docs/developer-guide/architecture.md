...
sidebar_position: 1
title: "Architecture"
description: "Zeloo Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Zeloo Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ zeloo_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # ZELOOCLI facade — mixins in zeloo_cli/cli_*_mixin.py
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── zeloo_state.py           # SQLite session/state database facade (+ zeloo_state_*.py siblings)
├── zeloo_constants.py       # ZELOO_HOME, profile-aware paths
...
│
├── zeloo_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — `Zeloo` subcommands (parsers in subcommands/, main_*.py)
...
│   ├── skills_config.py      # Zeloo skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # Zeloo tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # Zeloo gateway start/stop
│
...
```text
User input → ZELOOCLI.process_input()
...
Three discovery sources: `~/.Zeloo/plugins/` (user), `.Zeloo/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `Zeloo plugins` or `config.yaml`.
...
Exposes Zeloo as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`Zeloo -p <name>`) gets its own ZELOO_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
