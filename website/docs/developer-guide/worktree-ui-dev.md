...
| Variable | Meaning |
|----------|---------|
| `ZELOO_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `ZELOO_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `ZELOO_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Zeloo itself — they're private to these helpers. The variables Zeloo *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `Zeloo --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_zeloo_root)" || { echo "htui: not in a Zeloo checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$ZELOO_MAIN_CHECKOUT/.venv/bin/python" -m zeloo_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `ZELOO_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_zeloo_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `ZELOO_TUI_DIR` are mutually exclusive
`ZELOO_TUI_DIR` points Zeloo at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `Zeloo --tui --dev` exits with an error. Run `unset ZELOO_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_zeloo_root)" || { echo "hgui: not in a Zeloo checkout" >&2; return 1; }
  deps="${ZELOO_GUI_DEPS_CHECKOUT:-$ZELOO_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _zeloo_link_deps "$desktop" "$deps/apps/desktop"
    _zeloo_link_deps "$root" "$deps"
  else
  ...
  trap '_zeloo_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    ZELOO_DESKTOP_ZELOO_ROOT="$root" \
    ZELOO_DESKTOP_PYTHON="$ZELOO_MAIN_CHECKOUT/.venv/bin/python" \
    ZELOO_DESKTOP_IGNORE_EXISTING=1 \
    ZELOO_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `ZELOO_DESKTOP_ZELOO_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `Zeloo`. |
| `ZELOO_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `ZELOO_DESKTOP_IGNORE_EXISTING` | Ignores any `Zeloo` on `PATH` so it can't shadow the worktree. |
| `ZELOO_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real Zeloo checkout.
_zeloo_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/zeloo_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_zeloo_gui_cleanup() {
  ...
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'zeloo_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `Zeloo --tui --dev` and the `ZELOO_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/NousResearch/Zeloo-agent/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `ZELOO_*` variable Zeloo reads
