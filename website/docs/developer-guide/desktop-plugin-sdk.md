...
title: "Desktop Plugin SDK (@Zeloo/plugin-sdk)"
description: "Extend the native Zeloo Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Zeloo Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `ZELOOPlugin`.
It imports one module — `@Zeloo/plugin-sdk` — and gets everything: the app's
...
`$ZELOO_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Zeloo. This page is the **native
desktop app** (`Zeloo desktop`) SDK — the `@Zeloo/plugin-sdk` module and
`$ZELOO_HOME/desktop-plugins/`. The **web dashboard** (`Zeloo dashboard`) has
its own, unrelated plugin system on `window.__ZELOO_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a Zeloo Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$ZELOO_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$ZELOO_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `ZELOOPlugin` contract, appear in **Capabilities → Plugins**,
...
[`Zeloo-example-plugins`](https://github.com/NousResearch/Zeloo-example-plugins)
repo.

## Quick start — your first plugin

Create `$ZELOO_HOME/desktop-plugins/hello/plugin.js` (that's `~/.Zeloo/...`
...
```javascript
// ~/.Zeloo/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@Zeloo/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Zeloo' }),
      jsx('div', {
...
The only importable specifiers are `@Zeloo/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `ZELOOPlugin`:

```ts
interface ZELOOPlugin {
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `Zeloo.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@Zeloo/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@Zeloo/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@Zeloo/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@Zeloo/plugin-sdk'
...
```javascript
import { Button, useTheme } from '@Zeloo/plugin-sdk'
...
```javascript
import { host, requestTheme } from '@Zeloo/plugin-sdk'
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@Zeloo/plugin-sdk'
...
`window.Zeloo.send('get-price eth')` (or a declarative
`<button data-Zeloo-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@Zeloo/plugin-sdk'
...
for requests; `targetProfile` is the backend Zeloo profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from Zeloo
...
ctx.os.notify({
  ...
  // Body click → focus Zeloo + navigate. Same vocabulary as OS deep links:
  activate: 'Zeloo://index-network/intent/1',
  ...
  actions: [
    { id: 'open', label: 'Open', activate: 'Zeloo://index-network/intent/1' },
    { id: 'dismiss', label: 'Dismiss', onAction: () => dismiss('1') },
  ],
})
```

`activate` is deeplink-compatible: `Zeloo://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `Zeloo://…` URL works as an OS deep link). Action buttons only render on
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@Zeloo/plugin-sdk'
...
```javascript
import { queryClient } from '@Zeloo/plugin-sdk'
...
copies that half into `$ZELOO_HOME/desktop-plugins/<id>/` beside a
`.Zeloo-package.json` marker, and the renderer loads it through the exact same
pipeline as the standalone disk door (hot reload included):

```
~/.Zeloo/plugins/<id>/           # ONE installable folder
...
is refreshed when the source `plugin.js` changes (`Zeloo plugins update`, or
...
a package into `~/.Zeloo/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.Zeloo/plugins` is not reachable as a filesystem —
...
the `Zeloo://` scheme — a plain anchor on your website or README:
...
`dashboard/` subfolder of a regular Zeloo plugin and declare it in a
`manifest.json`:
...
Zeloo-agent codebase directly (`zeloo_state`, `zeloo_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.Zeloo/`)
...
(`Zeloo.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `ZELOOPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@Zeloo/plugin-sdk` alias.
2. It's still lint-fenced to `@Zeloo/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@Zeloo/plugin-sdk`, `react`,
...
| Plugin contract | `ZELOOPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `ZELOOOpenTarget`, `Contribution` |
...
### Agents: the `Zeloo-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`Zeloo-desktop-plugins`** skill — it carries the same contract as this page in
...
`$ZELOO_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `Zeloo logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@Zeloo/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.Zeloo/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.Zeloo/logs/errors.log` for
