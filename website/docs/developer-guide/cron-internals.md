...
description: "How Zeloo stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `zeloo_cli/cron.py` | CLI `Zeloo cron` subcommands |
...
Jobs are stored in `~/.Zeloo/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
renderer (`Zeloo cron list`/`doctor`, the `cronjob` tool, the web dashboard
...
In CLI mode, cron jobs only fire when `Zeloo cron` commands are run or during active CLI sessions.
...
```python
# ~/.Zeloo/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.Zeloo/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `Zeloo cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `Zeloo cron` CLI provides direct job management:

```bash
Zeloo cron list                    # Show all jobs
Zeloo cron create                  # Interactive job creation (alias: add)
Zeloo cron edit <job_id>           # Edit job configuration
Zeloo cron pause <job_id>          # Pause a running job
Zeloo cron resume <job_id>         # Resume a paused job
Zeloo cron run <job_id>            # Trigger immediate execution
Zeloo cron remove <job_id>         # Delete a job
```
