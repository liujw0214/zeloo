...
Zeloo Agent uses a dual compression system and Anthropic prompt caching to
...
Configure via `Zeloo plugins` → Provider Plugins → Context Engine, or edit `config.yaml` directly.

For building a context engine plugin, see [Context Engine Plugins](/developer-guide/context-engine-plugin).

## Dual Compression System

Zeloo has two separate compression layers that operate independently:
...
value is kept per `model@host` in `~/.Zeloo/cache/image_token_costs.json` and
...
compression:
  ...
  codex_app_server_auto: native  # native|Zeloo|off for Codex app-server thread compaction
...
| `codex_app_server_auto` | `native` | `native`, `Zeloo`, `off` | Thread-compaction mode for Codex app-server sessions (see below) |
...
excluded because they already unlock the wider window), Zeloo raises the
trigger to **85%** (~231K) and shows a notice with the opt-out command. The
notice is shown once per profile — a marker under `$ZELOO_HOME`
...
```bash
Zeloo config set compression.codex_gpt55_autoraise false
```

To keep the 85% autoraise but hide only the one-time notice:

```bash
Zeloo config set compression.codex_gpt55_autoraise_notice false
```
...
for ChatGPT-subscription accounts (live-verified Aug 2026). Zeloo keeps the
...
`gpt-5.4-900k`). These are Zeloo-side aliases: the suffix is stripped before
...
thread context, so Zeloo' auxiliary summarizer cannot shrink it — rewriting the
...
- Automatic compaction is controlled by `compression.codex_app_server_auto`:
  the default `native` lets the app-server decide when to compact and Zeloo
  records the resulting compaction events (compression counters, session
  events). Set `Zeloo` to let Zeloo' compression threshold initiate
  app-server compaction, or `off` to disable Zeloo-initiated automatic
  compaction entirely (codex may still compact natively).

Zeloo' local transcript is never rewritten on this runtime — state.db records
the compaction boundary while the visible transcript stays intact. All other
routes (including Codex OAuth chat sessions) keep Zeloo' summary compressor.
...
encrypted `compaction` output item. Zeloo captures that item into the
...
Anthropic allows a maximum of 4 `cache_control` breakpoints per request. Zeloo
...
5. **Model identity is part of the cache key**: Provider-side caches are scoped
   ...
   Zeloo can avoid; user-facing docs for `/model`, fallback providers, and
