...
Zeloo Agent uses a SQLite database (`~/.Zeloo/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source files: `zeloo_state.py` (facade) plus the `zeloo_state_*.py` siblings (schema, fts, search, compression, portability, gateway, ...)

### Desktop profile isolation and compaction generations

Each named profile stores its transcript in its own `$ZELOO_HOME/state.db`,
including when one `Zeloo serve` process serves several profiles. In-session
...
splice boundary, Zeloo excludes only that leading item when it exactly matches
...
```
~/.Zeloo/state.db (SQLite, WAL mode)
...
`Zeloo sessions recover` copies the row-bearing tables above into the
...
Abridged — see `SCHEMA_SQL` in `zeloo_state_common.py` (applied by `zeloo_state_schema.py`) for the full current column list
...
indexed columns — see `SCHEMA_SQL` in `zeloo_state_common.py` for the exact SQL.
...
Multiple Zeloo processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.Zeloo/state.db
...
Default path: `~/.Zeloo/state.db`

This is derived from `zeloo_constants.get_zeloo_home()` which resolves to
`~/.Zeloo/` by default, or the value of `ZELOO_HOME` environment variable.
