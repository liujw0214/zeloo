...
title: "Git Worktrees"
description: "Run multiple Zeloo agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Zeloo Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Zeloo so each session has a clean, isolated working directory.

## Why Use Worktrees with Zeloo?

Zeloo treats the **current working directory** as the project root:

- CLI: the directory where you run `Zeloo` or `Zeloo chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.Zeloo/config.yaml`
...
Zeloo creates `.worktrees/my-experiment/` inside the repo (branch
`Zeloo/my-experiment`, based on the freshly-fetched remote tip unless
`worktree_sync: false`), and retargets the session's terminal and file tools
into it — no restart needed. Omit the name to get a random `Zeloo-<id>`
...
git worktree add ../repo-feature feature/Zeloo-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/Zeloo-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Zeloo there:

```bash
cd ../repo-feature

# Start Zeloo in the worktree
Zeloo
```
...
git worktree add ../repo-experiment-a feature/Zeloo-a
git worktree add ../repo-experiment-b feature/Zeloo-b
```
...
- Works on its own branch (`feature/Zeloo-a` vs `feature/Zeloo-b`).
...
- Zeloo checkpoint data under `~/.Zeloo/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Zeloo experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/Zeloo-checkpoints-docs`, `feature/Zeloo-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Zeloo from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `Zeloo -w` (Automatic Worktree Mode)

Zeloo has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `Zeloo/Zeloo-<hash>`).
...
```bash
Zeloo -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `Zeloo -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Zeloo session its own clean checkout.
