...
sidebar_position: 3
title: "Zeloo Desktop"
description: "The native Zeloo desktop app — a polished experience for chatting with Zeloo, with streaming tool output, side-by-side previews, a file browser, voice, cron, profiles, skills, and settings. macOS, Windows, and Linux."
---

# Zeloo Desktop

The Zeloo desktop app is a native app built around the **same** agent you get from the CLI and the gateway — same config, same API keys, same sessions, same skills, same memory. It is not a separate product or a lightweight clone; it uses the same Zeloo Agent core and settings, and drives it through a modern & thoughtfully designed UI. If you have used `Zeloo` in a terminal, everything you set up there is already here, and anything you do here shows up there.

It runs on **macOS, Windows, and Linux**.

:::tip Which interface is which?
Zeloo has several front ends that all talk to the same agent:

- **Desktop App** (this page) — a native application with a purpose-built UI for chat, configuration, and management.
- **CLI** (`Zeloo`) and **[TUI](./tui.md)** (`Zeloo --tui`) — terminal interfaces.
- **[Web Dashboard](./features/web-dashboard.md)** (`Zeloo dashboard`) — a browser admin panel; its optional **Chat** tab embeds the TUI through a pseudo-terminal.
...
Download the app from the [Zeloo Desktop product page](https://Zeloo-agent.nousresearch.com/desktop), or follow the [installation instructions for Zeloo Desktop](../getting-started/installation.md).

If you already have Zeloo installed, simply run
...
- **The same conversation history** as every other Zeloo surface — sessions started here resume in the CLI/TUI and vice versa.
...
Chatting against a Zeloo instance on another machine instead of the bundled local backend? See [Connecting to a remote backend](#connecting-to-a-remote-backend) below — and for the full picture of how the remote-hosted dashboard connection works (the auth gate, the `/api/ws` chat socket, and WebSocket close-code triage), see [Web Dashboard → Connecting Zeloo Desktop to a remote backend](./features/web-dashboard.md#connecting-Zeloo-desktop-to-a-remote-backend).

#### Repository discovery

Zeloo Desktop discovers local Git repositories for the Projects sidebar by scanning your home directory to a bounded depth. You can change this per profile in **Settings → Workspace**, or in `config.yaml`:
...
- Set `repo_scan_enabled: false` to stop the filesystem scan completely. Existing disk-discovery cache rows for that profile are cleared; explicit projects and repositories inferred from intentional Zeloo sessions remain available.
...
Explore and preview the working directory without leaving the app — useful for following along as the agent reads, writes, and edits files. Set the initial project directory with `Zeloo desktop --cwd <path>` (or the `ZELOO_DESKTOP_CWD` environment variable).
...
- **Review pane** — **Cmd/Ctrl+G** toggles the working-tree review pane: branch and ahead/behind status, changed files (list or tree view), and diffs scoped to **Uncommitted**, **Branch**, or **Last turn** (just what the agent changed in its most recent turn). Stage/unstage files, revert changes, write a commit message (or **Generate commit message**), then **Commit** or **Commit & Push** — and **Create PR** via the GitHub CLI (`gh`), or hand the whole thing to the agent with **Ask Zeloo to open PR**. You can also create and switch branches from here.
...
When `Zeloo gui` runs inside WSL2 with `/dev/dxg` present and Mesa's `d3d12_dri.so` installed, the launcher sets `GALLIUM_DRIVER=d3d12` for Electron so rendering uses the Windows GPU instead of the llvmpipe software rasterizer; an explicit `GALLIUM_DRIVER`, `MESA_LOADER_DRIVER_OVERRIDE`, `LIBGL_ALWAYS_SOFTWARE`, or `LIBGL_DRIVERS_PATH` in your environment is left untouched (for example `GALLIUM_DRIVER=llvmpipe Zeloo gui` keeps software rendering).
...
- **Every provider and model in the menus** — the GUI surfaces the full provider list and every model that `Zeloo model` knows about, so you pick from the same catalog the CLI sees rather than a curated subset.
...
The app also surfaces the broader Zeloo management surface so you don't have to drop to a terminal:

- **Skills** — browse, install, and manage [skills](./features/skills.md). The Skills tab lists your installed skills with enable/disable toggles, and below them the full built-in optional-skills catalog that ships with Zeloo — each entry has a one-click **Install** button that flips the row into the installed list once it finishes.
...
roster where every [Zeloo profile](./profiles.md) appears as a bot with its
...
(recurring tasks backed by Zeloo cron). The roster lives in the left
...
each other's Bot Chats directly (`Zeloo -p <bot> chat`). The backend teaches
...
- **Command palette** — press **Cmd+K** or **Cmd+P** (Ctrl+K / Ctrl+P on Windows/Linux) to jump to actions and navigate the app from the keyboard: open any page or settings section, jump to a session by title or id, switch model/theme/color mode, spawn a terminal, restart the gateway, update Zeloo, and more.
...
The desktop app and the Zeloo backend it talks to update on separate clocks — the app package on your machine, the backend wherever it runs. When more than one update target exists (a remote gateway, or several registered gateways), the update affordances (**Update now** on the About panel, the ⌘K **Update Zeloo** row, and the update-ready toast) update **everything**: the connected backend first, then every other eligible registered gateway (Zeloo Cloud entries are platform-managed and skipped), and the desktop app itself last, since applying the client update relaunches the app. Single-machine installs keep the one-button experience.
...
The [manual update process](https://Zeloo-agent.nousresearch.com/docs/getting-started/updating) also works with the GUI.

## Uninstalling

Open **Settings → About → Danger zone** and pick how much to remove:

- **Uninstall Chat GUI only** — removes the desktop app and its data; the Zeloo agent, your config, and your chats stay. (Same as `Zeloo uninstall --gui`.)
- **Uninstall GUI + agent, keep my data** — removes the app and the agent but keeps config, chats, and secrets for a future reinstall. (Same as `Zeloo uninstall`.)
- **Uninstall everything** — removes the app, the agent, and all user data. (Same as `Zeloo uninstall --full`.)
...
You can do the same from the terminal — `Zeloo uninstall --gui` for the GUI alone, or `Zeloo uninstall` / `Zeloo uninstall --full` for the agent too.

:::note
Running `Zeloo uninstall --gui` from a **source checkout** (a `Zeloo desktop` dev build) also removes the workspace `node_modules` and `apps/desktop/{dist,release}` build output, since those are GUI build artifacts. They're recoverable with `Zeloo desktop` (or `npm install` + a rebuild) — but if you're actively hacking on the desktop app, expect to reinstall dependencies afterward.
:::

## CLI reference: `Zeloo desktop`

To launch via the CLI, simply run `Zeloo desktop`. By default it installs workspace Node dependencies, builds the current OS's unpacked Electron app, then launches that packaged artifact.

On Linux, launches refresh `$XDG_DATA_HOME/applications/Zeloo.desktop` (by default `~/.local/share/applications/Zeloo.desktop`) so Zeloo appears in the application menu. To preserve a hand-edited entry, disable refreshes:

```bash
Zeloo config set desktop.manage_launcher_entry false
```

A missing entry is still created; the flag only stops `Zeloo desktop` from rewriting an entry that already exists.
...
| `--build-only`       | Build the desktop app but do not launch it (used by `Zeloo update`)                      |
| `--source`           | Launch via `electron .` against `apps/desktop/dist` instead of the packaged app           |
| `--cwd PATH`         | Initial project directory for desktop chat sessions (sets `ZELOO_DESKTOP_CWD`)           |
| `--Zeloo-root PATH` | Override the Zeloo source root the app uses (sets `ZELOO_DESKTOP_ZELOO_ROOT`)          |
| `--ignore-existing`  | Force the app to ignore any `Zeloo` CLI already on `PATH` during backend resolution      |
| `--fake-boot`        | Enable deterministic boot delays for validating the startup UI                            |

## How it works

The packaged app ships the Electron shell and a native React chat surface. On first launch it can install the Zeloo Agent runtime into `ZELOO_HOME` (`~/.Zeloo`, or `%LOCALAPPDATA%\Zeloo` on Windows) — **the same layout a CLI install uses**, which is why the two are interchangeable. Backend resolution first honours `ZELOO_DESKTOP_ZELOO_ROOT`, then a completed managed install, then a probed `Zeloo` on `PATH` (unless `--ignore-existing` / `ZELOO_DESKTOP_IGNORE_EXISTING=1` is set), and finally an explicit `ZELOO_DESKTOP_ZELOO` command override for packagers such as Nix. The React renderer talks to a headless backend the app launches for you — a `Zeloo serve` process that serves the `tui_gateway` JSON-RPC/WebSocket API — and reuses the agent runtime rather than embedding `Zeloo --tui`. The desktop app is **self-contained**: it runs its own `Zeloo serve` backend and never opens or requires the [web dashboard](./features/web-dashboard.md). (Runtimes older than the `serve` command fall back to a headless `dashboard --no-open` automatically, so an app update never outruns its backend.) Install, backend-resolution, and self-update logic live in the Electron main process.

## Connecting to a remote backend

By default the app starts and manages its own **local** backend. You can instead point it at a Zeloo backend running on another machine — a VPS, a home server, or a Mini behind Tailscale.
...
- **Remote gateway** — enter the URL of a `Zeloo serve` backend you run yourself and sign in. This is the mode the rest of this section walks through.
- **Zeloo Cloud** — sign in once to Zeloo Cloud and pick from the agents on your account; no URL to paste. The app discovers your agents (with an organization picker if your account spans several orgs), and connecting to one switches the session over automatically. The status bar shows the cloud connection while it's active.
...
Further down the same **Settings → Gateways** page, **Registered gateways** manages a named list of every Zeloo gateway the app knows about — the local runtime, any number of remote gateways (LAN, Tailscale, internet), Zeloo Cloud instances, and SSH hosts — all persisted together in one place. You can jump there from the plug button at the right end of the sidebar profile rail (**Connect another Zeloo gateway…**) or via **⌘K → Gateways**. The full guide, including the union agent roster, `@name-device` handles, fleet-wide updates, and the plugin SDK surface, is at [Connecting Desktop to Many Zeloo Instances](./multi-connection-desktop.md).
...
- Cloud entries come from the Zeloo Cloud sign-in/discovery flow above, not from a hand-typed URL.
- Tokens are stored encrypted with the OS keyring (with an explicit plain-text opt-in on keyring-less Linux).

Side-by-side routing is live: each registered gateway dials its own backends and sockets on demand (keyed per connection + profile), the plugin SDK exposes the union agent roster (`host.agents()` / `host.ensureAgent()`), and **Update all instances** on the Gateways page dispatches `Zeloo update` to every eligible gateway at once — Zeloo Cloud entries are skipped (the platform updates them), and each instance reports its own result.


:::info The remote backend is a running `Zeloo serve` process
"Remote backend" means a **`Zeloo serve`** server running on the remote machine — that is the process the desktop app connects to. Nothing in this section works unless that backend is actually up and reachable. The desktop app does not start it for you; you (or a `systemd` service) keep `Zeloo serve` running on the remote host, and the app attaches to it. If you also use messaging channels (Telegram, Discord, etc.), the **gateway** is a *separate* long-running process you start independently — see the note after the setup steps.
:::
...
Set a username and password, then start the backend bound to a reachable address. The credentials live in `~/.Zeloo/.env` (the secrets file, mode 0600):
...
Zeloo serve --host 0.0.0.0 --port 9119
```

Keep that `Zeloo serve` process running for as long as you want the desktop app to be able to connect — if it stops, the app can no longer reach the backend. Run it under `systemd`, `tmux`, or your process manager of choice so it survives logout and reboots.

Separately, make sure the **gateway is running** on the remote host if you rely on messaging channels — the `Zeloo serve` backend is what the desktop app talks to, but your Telegram/Discord/Slack gateway sessions are a different process that you start and keep running on their own. See [Messaging](./messaging/index.md) for gateway setup.

Prefer not to keep a plaintext password at rest? Set `ZELOO_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` to a scrypt hash instead — compute it with `python -c "from plugins.dashboard_auth.basic import hash_password; print(hash_password('PW'))"`. Full configuration surface (config.yaml keys, every env var, the rate limiter): [Web Dashboard → Username/password provider](./features/web-dashboard.md#usernamepassword-provider-no-oauth-idp).

Running the backend as a systemd service? Give the unit `EnvironmentFile=%h/.Zeloo/.env` so the credentials are in the environment at boot.
...
1. **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/Zeloo` work if you front it with a reverse proxy)
...
3. **Save and reconnect** — switches the desktop shell onto the remote backend. The session refreshes automatically; you stay signed in across restarts when `ZELOO_DASHBOARD_BASIC_AUTH_SECRET` is set.

You can also set the backend URL without the UI via the `ZELOO_DESKTOP_REMOTE_URL` environment variable before launching the app (it overrides the in-app setting); you still sign in from the Gateways settings panel.
...
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `ZELOO_DASHBOARD_BASIC_AUTH_USERNAME` / `ZELOO_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for an unknown user and a wrong password (no enumeration oracle), so double-check both. Confirm the gate is on with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
- **No "Sign in" button — it asks for a session token instead** — the backend's username/password provider isn't active. `/api/status` won't list `"basic"` in `auth_providers`. Make sure both the username and a password (or password hash) are set in `~/.Zeloo/.env` and that the dashboard process actually loaded them.
- **Signed out on every restart** — set `ZELOO_DASHBOARD_BASIC_AUTH_SECRET` to a stable value. Without it the token-signing key is regenerated per boot, invalidating all sessions.
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting Zeloo Desktop to a remote backend](./features/web-dashboard.md#connecting-Zeloo-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & Zeloo Desktop](../reference/environment-variables.md#web-dashboard--Zeloo-desktop).
...
`$ZELOO_HOME/desktop-plugins/<id>/plugin.js`; the app loads it within seconds and
...
Zeloo: **one row per plugin**, with two switch columns.
...
- **Desktop column** — the half loaded into this app. It is app-level: the
  ...
  `~/.Zeloo/desktop-plugins/`; the desktop half of a unified agent+desktop
...
- A half the plugin does not ship shows a dash. A desktop half whose agent
  ...
  [Accent Picker](https://github.com/NousResearch/Zeloo-desktop-accent-picker)
...
- **Open logs** — opens `ZELOO_HOME/logs` in your file manager. On a remote
...
- **Send diagnostics** — uploads a redacted debug bundle to Nous-internal
  ...
  `Zeloo debug share --nous`; secrets are always redacted, the bundle is
...
Boot logs land in `ZELOO_HOME/logs/desktop.log` (it includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure. You can also tail it from the CLI:
...
rm "$HOME/.Zeloo/Zeloo-agent/.Zeloo-bootstrap-complete"

# Rebuild a broken Python venv (macOS/Linux)
rm -rf "$HOME/.Zeloo/Zeloo-agent/venv"

# Reset a stuck macOS microphone prompt
tccutil reset Microphone com.nousresearch.Zeloo
```
...
ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/ \
  bash -c 'cd "$HOME/.Zeloo/Zeloo-agent/apps/desktop" && CSC_IDENTITY_AUTO_DISCOVERY=false npm run pack'
```

**Other native downloads (e.g. the `get-windows` prebuilt on Windows) that need a mirror:** put the npm keys in `$ZELOO_HOME/npmrc` (`%LOCALAPPDATA%\Zeloo\npmrc` on Windows, `~/.Zeloo/npmrc` elsewhere) — for example `node_get_windows_binary_host_mirror=https://<mirror>/sindresorhus/get-windows/releases/download/`. Every `npm ci`/`npm run` the updater spawns (desktop, web and TUI builds) points `NPM_CONFIG_USERCONFIG` at that file when it exists, so the config survives `Zeloo update`; the repo-root `.npmrc` is git-tracked and gets autostashed on every update, and `~/.npmrc` may be missed because the desktop hand-off inherits the GUI's environment. An `NPM_CONFIG_USERCONFIG` you set yourself is never overridden.
...
```bash
ZELOO_DESKTOP_ZELOO_ROOT=/path/to/clone npm run dev
ZELOO_HOME=/tmp/throwaway npm run dev
...
(Desktop, then Downloads, then Documents, ...) as Zeloo touches each folder.
A single **Full Disk Access** grant covers all of them, permanently — and
with Zeloo' stable signing identities it survives every update:

1. System Settings → **Privacy & Security → Full Disk Access** (or run
   `open "x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles"`)
2. Enable your terminal app — and **Zeloo.app** if you use Desktop.
3. Fully quit and relaunch them once.

`Zeloo doctor` reports whether the current terminal context already has the
grant, and `Zeloo setup` shows this tip on macOS when it doesn't.
...
```bash
tccutil reset ScreenCapture com.nousresearch.Zeloo   # repeat per service
```

then toggle the fresh entry ON in System Settings and fully quit & relaunch
Zeloo. Grants are stable from then on.
...
certificate once and tell Zeloo to use it. The one-shot command does
...
```bash
Zeloo desktop --setup-tcc-identity
```

Or do it manually:

1. Keychain Access → Certificate Assistant → **Create a Certificate…**
2. Name: `Zeloo Local Signing`, Identity Type: *Self-Signed Root*,
...
4. `Zeloo config set desktop.macos_signing_identity "Zeloo Local Signing"`

Use `--identity <name>` with the command to create/use a differently named
certificate (default: `Zeloo Local Signing`). The command is idempotent —
...
`tccutil reset All com.nousresearch.Zeloo` and re-grant.

## See also

- [CLI Guide](./cli.md) — the terminal interface
- [TUI](./tui.md) — the modern terminal UI used by `Zeloo --tui` and the dashboard chat tab
