...
| | |
|---|---|
| Source | Optional — install with `Zeloo skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research\darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Zeloo Agent |
| License | MIT |
...
import upstream classes into Zeloo itself.
...
```bash
mkdir -p ~/.Zeloo/cache/darwinian-evolver && cd ~/.Zeloo/cache/darwinian-evolver
...
```bash
cd ~/.Zeloo/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.Zeloo/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.Zeloo/skills/research/darwinian-evolver
DE_DIR=~/.Zeloo/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Zeloo core.
   Custom driver scripts under `~/.Zeloo/skills/...` are user-side and fine.
