...
| | |
|---|---|
| Source | Optional — install with `Zeloo skills install official/creative/pixel-art` |
...
import sys
import os
sys.path.insert(0, os.path.expanduser("~/.Zeloo/skills/creative/pixel-art/scripts"))
...
```bash
cd ~/.Zeloo/skills/creative/pixel-art/scripts
...
- ffmpeg on PATH (only needed for video — Zeloo installs package this)
