...
| | |
|---|---|
| Source | Optional — install with `Zeloo skills install official/creative/touchdesigner-mcp` |
...
```
Zeloo Agent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${ZELOO_HOME:-$HOME/.Zeloo}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to Zeloo config (if missing)
...
3. **Restart Zeloo session** to pick up the new MCP server
