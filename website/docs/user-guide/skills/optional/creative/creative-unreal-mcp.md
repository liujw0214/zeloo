...
| | |
|---|---|
| Source | Optional — install with `Zeloo skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Zeloo MCP catalog. The
...
Two halves, in this order: the editor side must be up before Zeloo connects.
...
### One-time, Zeloo side
    ...
    Zeloo mcp install unreal-engine
...
`~/.Zeloo/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Zeloo — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Zeloo connects
...
2. Start the Zeloo session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Zeloo
they appear as:
...
then happens at `Zeloo mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Zeloo runs batched calls
...
- **Start order matters.** Editor + server up first, then the Zeloo
...
- **Timeouts on long operations.** Zeloo' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.Zeloo/config.yaml` for
