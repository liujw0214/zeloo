---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into Zeloo"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into Zeloo"
---
...
Import an OpenClaw setup (memories, skills) into Zeloo.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `Zeloo skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration\openclaw-migration` |
| Version | `1.0.0` |
| Author | Zeloo Agent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Zeloo`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> Zeloo Migration

Use this skill when a user wants to move their OpenClaw setup into Zeloo Agent with minimal manual cleanup.
...
```bash
Zeloo claw migrate              # Full interactive migration
...
Zeloo claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `Zeloo setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_ZELOO.py` to:

- import `SOUL.md` into the Zeloo home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Zeloo memory entries
- merge OpenClaw command approval patterns into Zeloo `command_allowlist`
- migrate Zeloo-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Zeloo working-directory configuration
- copy OpenClaw skills into `~/.Zeloo/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Zeloo workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.Zeloo/tts/`
- archive non-secret docs that do not have a direct Zeloo destination
...
- `scripts/openclaw_to_ZELOO.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.Zeloo/skills/migration/openclaw-migration/scripts/openclaw_to_ZELOO.py`

Do not guess a shorter path like `~/.Zeloo/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.Zeloo/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Zeloo-compatible secrets, currently:
...
Zeloo CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Zeloo. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Zeloo workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Zeloo skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python ~/.Zeloo/skills/migration/openclaw-migration/scripts/openclaw_to_ZELOO.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python /home/USER/.Zeloo/skills/migration/openclaw-migration/scripts/openclaw_to_ZELOO.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python ~/.Zeloo/skills/migration/openclaw-migration/scripts/openclaw_to_ZELOO.py --preset user-data
```

Execute a user-data migration:

```bash
python ~/.Zeloo/skills/migration/openclaw-migration/scripts/openclaw_to_ZELOO.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python ~/.Zeloo/skills/migration/openclaw-migration/scripts/openclaw_to_ZELOO.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python ~/.Zeloo/skills/migration/openclaw-migration/scripts/openclaw_to_ZELOO.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Zeloo unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Zeloo targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Zeloo destination. Unsupported auth blobs must still be reported as skipped.
...
- Zeloo persona state imported
- Zeloo memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.Zeloo/skills/openclaw-imports/`
