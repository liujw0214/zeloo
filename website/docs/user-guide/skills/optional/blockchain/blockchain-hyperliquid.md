...
| | |
|---|---|
| Source | Optional — install with `Zeloo skills install official/blockchain/hyperliquid` |
| Path | `optional-skills/blockchain\hyperliquid` |
| Version | `0.1.0` |
| Author | Hugo Sequier (Hugo-SEQUIER), Zeloo Agent |
| License | MIT |
...
The script reads `${ZELOO_HOME:-~/.Zeloo}/.env` for two optional defaults:
...
Helper script: `~/.Zeloo/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
```bash
python ~/.Zeloo/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py <command> [args]
```
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${ZELOO_HOME:-~/.Zeloo}/.env`.
...
```bash
python ~/.Zeloo/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py dexs

python ~/.Zeloo/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py \
