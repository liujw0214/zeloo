...
| | |
|---|---|
| Source | Optional — install with `Zeloo skills install official/security/1password` |
| Path | `optional-skills/security\1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Zeloo Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Zeloo terminal calls (desktop app flow only)
...
### Service Account (recommended for Zeloo)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${ZELOO_HOME:-~/.Zeloo}/.env` (the skill will prompt for this on first load).
...
## Zeloo Execution Pattern (desktop app flow)

Zeloo terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/Zeloo-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/Zeloo-op.sock"
