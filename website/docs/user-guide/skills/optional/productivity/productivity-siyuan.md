...
| | |
|---|---|
| Source | Optional — install with `Zeloo skills install official/productivity/siyuan` |
...
3. Store it in `${ZELOO_HOME:-~/.Zeloo}/.env`:
   ```
...
```yaml
# In ~/.Zeloo/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
