...
| | |
|---|---|
| Source | Optional — install with `Zeloo skills install official/productivity/shopify` |
...
4. Save to `${ZELOO_HOME:-~/.Zeloo}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"Zeloo","productType":"Apparel","tags":["test"]}}'
```
