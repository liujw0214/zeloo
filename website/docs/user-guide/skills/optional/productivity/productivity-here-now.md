...
| | |
|---|---|
| Source | Optional — install with `Zeloo skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${ZELOO_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${ZELOO_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${ZELOO_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client Zeloo
```
...
```bash
PUBLISH="${ZELOO_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client Zeloo
```
...
```bash
DRIVE="${ZELOO_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `Zeloo`)    |
