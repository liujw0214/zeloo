...
| | |
|---|---|
| Source | Optional — install with `Zeloo skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Zeloo Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${ZELOO_HOME:-~/.Zeloo}/.env`:
...
```bash
CANVAS="python $ZELOO_HOME/skills/productivity/canvas/scripts/canvas_api.py"
