...
| | |
|---|---|
| Source | Optional — install with `Zeloo skills install official/mcp/fastmcp` |
...
| Related skills | [`Zeloo-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-Zeloo-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Zeloo or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Zeloo. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.Zeloo/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.Zeloo/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Zeloo integration, either:

- configure the server in `~/.Zeloo/config.yaml` using the `native-mcp` skill, or
...
### Zeloo cannot see the deployed server

The server-building part may be correct while the Zeloo config is not. Load the `native-mcp` skill and configure the server in `~/.Zeloo/config.yaml`, then restart Zeloo.
