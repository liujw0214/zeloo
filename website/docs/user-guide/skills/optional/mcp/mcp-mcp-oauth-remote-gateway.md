...
| | |
|---|---|
| Source | Optional — install with `Zeloo skills install official/mcp/mcp-oauth-remote-gateway` |
| Path | `optional-skills/mcp\mcp-oauth-remote-gateway` |
| Version | `1.0.0` |
| Author | Ben Barclay (benbarclay), Zeloo Agent |
| License | MIT |
| Platforms | linux, macos |
| Tags | `MCP`, `OAuth`, `PKCE`, `Remote-Deployment` |
| Related skills | [`Zeloo-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-Zeloo-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter), [`fastmcp`](/docs/user-guide/skills/optional/mcp/mcp-fastmcp) |
...
# MCP OAuth on a Remote Zeloo Gateway

## Overview

Zeloo' built-in MCP OAuth client runs a one-shot HTTP listener on `127.0.0.1:<port>`
inside the Zeloo process and registers that loopback address as the OAuth
`redirect_uri`. That works perfectly for a local CLI on the user's own machine.
It breaks completely when Zeloo runs as a remote gateway (container, VPS,
messaging bot), because the user's browser resolves `127.0.0.1` to the user's own
laptop, not the remote container — so the authorization code never reaches Zeloo.

This skill does the OAuth dance by hand and writes the resulting tokens into the
exact files Zeloo' token storage expects, so a subsequent `/reload-mcp` finds
...
2. Zeloo is running as a **remote gateway** (container, VPS, Docker, managed service) — NOT a local CLI on the user's laptop.
...
Do NOT use this for:
- **Local CLI Zeloo** — just set `auth: oauth` in `mcp_servers.<name>` and `/reload-mcp`. The built-in flow opens a browser and captures the callback on localhost. Works perfectly.
...
laptop and fails to connect. The callback never reaches the Zeloo process, the
flow times out, and `/reload-mcp` returns "No MCP tools available" with no detail.

Symptoms to recognize: `[xdg-open] <defunct>` processes under the Zeloo user, an
empty or missing tokens directory (`$ZELOO_HOME/mcp-tokens/`), and a reload that
...
already cover the deployment. When Zeloo detects a remote session it prints two
...
Both require an interactive terminal to the Zeloo host. The rest of this skill
is for when there is NO interactive TTY — Zeloo running purely as a messaging
gateway/bot where `/reload-mcp` triggers the flow with nobody at a prompt.

## Preferred Front Door: the Zeloo Dashboard (try this BEFORE manual token surgery)

A remote Zeloo gateway often also runs the **dashboard** web UI as a SEPARATE
process (e.g. `Zeloo dashboard --host 0.0.0.0 --port <port>`; check with
`ps aux | grep 'Zeloo dashboard'`). It exposes a connector/MCP console —
...
```bash
env | grep -iE "ZELOO_DASHBOARD_PUBLIC_URL|RAILWAY_PUBLIC_DOMAIN|RAILWAY_STATIC_URL|RAILWAY_SERVICE_.*_URL|PUBLIC_URL|BASE_URL|DOMAIN" \
  | sed -E 's/(TOKEN|SECRET|KEY|PASSWORD)=.*/\1=***REDACTED***/I'
```

`ZELOO_DASHBOARD_PUBLIC_URL` is authoritative when present. On Railway also check
...
across restarts) and anything reading credentials from `$ZELOO_HOME/.env`. Those
...
Zeloo' `ZELOOTokenStorage` would have written, so on `/reload-mcp` Zeloo finds
cached tokens and skips the browser flow entirely.
...
```bash
env | grep -iE "Zeloo|RAILWAY|CONTAINER"
echo "$DISPLAY $WAYLAND_DISPLAY $SSH_CLIENT"
```

No display + a remote indicator = remote gateway. `tools/mcp_oauth.py::_can_open_browser()`
uses these same env vars, so if Zeloo' own auto-detect says "headless", the
built-in flow won't work.

### 2. Find ZELOO_HOME and the config path

```bash
ZELOO_HOME=$(python3 -c 'from zeloo_constants import get_zeloo_home; print(get_zeloo_home())')
echo "config: $ZELOO_HOME/config.yaml"
echo "tokens: $ZELOO_HOME/mcp-tokens/"
```
...
```json
{
  "client_name": "Zeloo Agent (manual OAuth)",
...
### 8. Write tokens in Zeloo' exact schema

`tools/mcp_oauth.py::ZELOOTokenStorage` expects two files under
`$ZELOO_HOME/mcp-tokens/` (create dir with `0o700`, files with `0o600`):
...
body = json.dumps({
    ...
    "params": {
        ...
        "capabilities": {},
        "clientInfo": {"name": "Zeloo-debug", "version": "1.0"},
    },
}).encode()
...
UA** — Cloudflare will 403 you even though Zeloo (which uses httpx) will succeed.
`scripts/diagnose-oauth-mcp.py` automates this smoke test.

### 11. Tell the user to run `/reload-mcp`

On reload, Zeloo sees `auth: oauth`, calls `ZELOOTokenStorage.get_tokens()`,
...
1. **Do not assume "headless" means "OAuth impossible."** The built-in flow works fine for local CLI; the issue is strictly remote deployments where the user's browser and the Zeloo process are on different machines. Check the execution environment before claiming OAuth isn't an option.
...
11. **Client info file is NOT optional.** Zeloo needs `<server>.client.json` to know the `client_id` for refresh grants. Skipping it means the first refresh fails and the user has to re-auth — writing both files is the whole point of this skill.
...
- **Don't use `mcp-remote` as a fallback.** It runs an npx subprocess whose OAuth callback server ALSO sits on the remote container's localhost — same problem. `mcp-remote` only helps when the MCP client doesn't speak remote HTTP at all (Zeloo does natively).
...
- **Don't claim Zeloo doesn't support a feature without reading the source.** Grep the source tree before making capability claims.
...
- `native-mcp` — general guide to configuring MCP in Zeloo. Authoritative config reference lives there.
- `mcporter` — the external CLI bridge, for ad-hoc MCP calls outside of Zeloo' config.
