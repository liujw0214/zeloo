---
title: "Honcho — Configure and troubleshoot Honcho memory for Zeloo"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Zeloo"
---
...
Configure and troubleshoot Honcho memory for Zeloo.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `Zeloo skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Zeloo

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Zeloo profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/Zeloo#running-honcho-locally-with-Zeloo

### Verify

```bash
Zeloo honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Zeloo profile's AI peer
...
Honcho models conversations as interactions between **peers**. Zeloo creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Zeloo instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Zeloo run |
| `global` | Single session across all directories |

Manual override: `Zeloo honcho map my-project-name`
...
Each Zeloo profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block Zeloo.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `Zeloo.coder` host block in `honcho.json`
...
```bash
Zeloo honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Zeloo when Honcho memory is active.
...
Config file: `$ZELOO_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `Zeloo honcho setup`. Ensure `memory.provider: honcho` is in `~/.Zeloo/config.yaml`.

### Memory not persisting across sessions
Check `Zeloo honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `Zeloo profile create <name> --clone`. For existing profiles: `Zeloo honcho sync`.
...
| Command | Description |
|---------|-------------|
| `Zeloo honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `Zeloo honcho status` | Show resolved config, connection test, peer info for active profile |
| `Zeloo honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `Zeloo honcho disable` | Disable Honcho for the active profile |
| `Zeloo honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `Zeloo honcho peers` | Show peer identities across all profiles |
| `Zeloo honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `Zeloo honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `Zeloo honcho sessions` | List known directory-to-session-name mappings |
| `Zeloo honcho map <name>` | Map current working directory to a Honcho session name |
| `Zeloo honcho identity` | Seed AI peer identity or show both peer representations |
| `Zeloo honcho sync` | Create host blocks for all Zeloo profiles that don't have one yet |
| `Zeloo honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Zeloo + Honcho |
| `Zeloo memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `Zeloo memory status` | Show active memory provider and config |
| `Zeloo memory off` | Disable external memory provider |
