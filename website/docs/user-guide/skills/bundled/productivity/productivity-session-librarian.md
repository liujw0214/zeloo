...
| List/filter by metadata (age, source, cost, tokens, workspace) | `Zeloo sessions list` / `stats` via terminal |
| Rename | `Zeloo sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `Zeloo sessions archive <filters>` |
| Delete (destructive) | `Zeloo sessions delete` / `Zeloo sessions prune <filters>` |
| Export before deleting anything valuable | `Zeloo sessions export --session-id <id> --format md` |
...
`Zeloo sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `Zeloo sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `Zeloo sessions list`
