...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`Zeloo-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-Zeloo-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Zeloo terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Zeloo itself, `model.provider: openai-codex` uses Zeloo-managed Codex
OAuth from `~/.Zeloo/auth.json` after `Zeloo auth add openai-codex`. For the
...
## Zeloo Gateway Caveat

When invoking the Codex CLI from a Zeloo gateway/service context (for example,
