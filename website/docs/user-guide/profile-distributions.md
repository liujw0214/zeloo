...
A **profile distribution** packages a complete Zeloo agent — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## Two ways to share a profile

Zeloo has two sharing paths, and they answer different questions. Distributions are the durable one; export files are the quick one.

| | **Distribution** (git repo) | **Export file** (`.tar.gz`) |
|---|---|---|
| Ship it by | `Zeloo profile install <repo>` | Send a file — chat, AirDrop, USB, email |
| Recipient needs | git, and access to the repo | The file |
| Updates | `Zeloo profile update` pulls new versions | Re-send the file |
...
| Made with | `Zeloo profile install` / `update` | `/export` and `/import`, or `Zeloo profile export` / `import` |
...
Before distributions, sharing a Zeloo agent meant sending someone:
...
```bash
Zeloo profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `Zeloo profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
The tradeoff: recipients need git installed. On any machine running Zeloo in 2026, that's already true.
...
:::caution
**Zeloo does not control git.** The file exclusions described on this page are applied by the **installer** when someone runs `Zeloo profile install` or `Zeloo profile update`. They are **not** applied when you run `git add` or `git commit`.
:::
...
```bash
Zeloo profile create research-bot
research-bot setup                    # configure model, API keys
# Edit ~/.Zeloo/profiles/research-bot/SOUL.md
...
Create `~/.Zeloo/profiles/research-bot/distribution.yaml`:

```yaml
name: research-bot
version: 1.0.0
description: "Autonomous research assistant with arXiv and web tools"
zeloo_requires: ">=0.12.0"
author: "Your Name"
license: "MIT"
...
Create `~/.Zeloo/profiles/research-bot/.gitignore` with at minimum:
...
```bash
cd ~/.Zeloo/profiles/research-bot
git init
git add .
git commit -m "v1.0.0"
...
Recipients who run `Zeloo profile update research-bot` will pull the latest.
...
```bash
Zeloo profile install github.com/you/research-bot --alias
```
...
5. Copies distribution-owned files into `~/.Zeloo/profiles/research-bot/` (or wherever the manifest's `name` resolves). The [hard-excluded paths](#whats-not-in-a-distribution-ever) are stripped during this copy, even if the author accidentally left them in the repo.
...
```bash
# GitHub shorthand
Zeloo profile install github.com/you/research-bot

# Full HTTPS
Zeloo profile install https://github.com/you/research-bot.git

# SSH
Zeloo profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
Zeloo profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
Zeloo profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
Zeloo profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
Zeloo profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
Zeloo profile install github.com/acme/support-bot --name support-eu --alias
```
...
```
# Environment variables required by this Zeloo distribution.
...
```bash
cp ~/.Zeloo/profiles/research-bot/.env.EXAMPLE ~/.Zeloo/profiles/research-bot/.env
...
```bash
Zeloo profile info research-bot
```
...
`Zeloo profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
Zeloo profile update research-bot
```
...
```bash
Zeloo profile delete research-bot
```
...
Path:    ~/.Zeloo/profiles/research-bot
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `Zeloo profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
# Each engineer
Zeloo profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `Zeloo profile update pr-reviewer` and everyone's on the new version within minutes.
...
cd ~/.Zeloo/profiles/polymarket-trader
# Write a solid README.md at the repo root — GitHub shows it on the repo page
git init && git add . && git status   # confirm no secrets staged
git commit -m "v1.0"
git tag v1.0.0
# Publish to a public GitHub repo
git remote add origin https://github.com/you/Zeloo-polymarket-trader.git
git push -u origin main --tags

# Anyone
Zeloo profile install github.com/you/Zeloo-polymarket-trader --alias
```
...
You built Zeloo-on-top — maybe a compliance-monitoring harness, a customer-support stack, a domain-specific research platform. You want to distribute it as a product.
...
# Each on-call
Zeloo profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
Zeloo profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `Zeloo profile info <name>` and hold off on updates until you're ready.
:::
...
Zeloo profile info research-bot | grep Version
...
```yaml
# ~/.Zeloo/profiles/research-bot/local/my-overrides.yaml
...
Zeloo profile delete research-bot --yes
Zeloo profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
Zeloo profile update research-bot --force-config --yes
```
...
Zeloo profile install github.com/yourname/forked-research-bot --alias

# Iterate locally in ~/.Zeloo/profiles/forked-research-bot/
...
Zeloo profile install ~/.Zeloo/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
Zeloo profile delete research-bot-test --yes
Zeloo profile install ~/.Zeloo/profiles/research-bot --name research-bot-test
```
...
Without `-o`, the CLI and TUI place the archive in Zeloo's managed
`profile-exports/` directory under the default Zeloo home, not in the current
...
source file. If the Zeloo home itself lives inside a Git checkout (some
Docker/custom deployments), the archive goes to `~/.Zeloo-profile-exports/`
...
```bash
Zeloo profile export research-bot
Zeloo profile export research-bot -o ./research-bot.tar.gz
```
...
```bash
Zeloo profile import ./research-bot.tar.gz
Zeloo profile import ./research-bot.tar.gz --name research-bot-2
```

The profile name is inferred from the archive unless you pass `--name`. Importing over an existing profile is refused — rename or delete the old one first. A shell wrapper (`research-bot` → `Zeloo -p research-bot`) is created when the name doesn't collide with an existing command.
...
:::note
You cannot import as `default` — that name is the built-in root profile (`~/.Zeloo`). Pass `--name something-else`.
:::
...
**The default profile** (`~/.Zeloo`) is exported through an allow-list — only known Zeloo artifacts, so an unrelated file sitting in your home directory can't get swept in:
...
**A named profile** (`~/.Zeloo/profiles/<name>`) copies the whole directory minus `auth.json` / `.env`. That's broader — if the profile has `state.db`, logs, or caches, they go in the archive too, and the file gets big.
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `Zeloo -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `Zeloo profile` — not a parallel command tree.
...
- Reserved profile names (`Zeloo`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [`Zeloo profile export` / `import`](../reference/profile-commands.md#Zeloo-profile-export) — the CLI form of [export files](#export-and-import-a-profile-file)
- [Slash Commands reference](../reference/slash-commands.md) — `/export`, `/import`, and every other in-chat command
- [Using SOUL with Zeloo](../guides/use-soul-with-Zeloo.md) — authoring personalities
