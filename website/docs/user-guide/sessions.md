...
Zeloo Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.Zeloo/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Zeloo stores session history so it can resume conversations, but it does not
...
Zeloo explicitly injects for that turn.
...
For example, if a user sends an image and asks Zeloo to make a meme from it,
Zeloo may inspect that image once with vision and run an image-processing
...
`Zeloo sessions prune` only when you want to delete old ended sessions from
storage. If `state.db` has simply grown large, start with the non-destructive
option first: `Zeloo sessions optimize` merges FTS5 index segments and
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`Zeloo` or `Zeloo chat`) |
...
Zeloo -c "my project"   # → resumes "my project #3"
```
...
Zeloo --resume 20250305_091523_a1b2c3d4
Zeloo -r 20250305_091523_a1b2c3d4

# Resume by title
Zeloo --resume "refactoring auth"

# Resume the most recent session — same lookup as -c
Zeloo --resume latest

# Or with the chat subcommand
Zeloo chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `Zeloo sessions list`.
...
Zeloo --resume latest --in ./my-project

# Works with the TUI too
Zeloo --tui --resume latest --in ./my-project
```
...
```bash
Zeloo --resume 20250305_091523_a1b2c3 --no-restore-cwd
```
...
`Zeloo sessions list` accepts `--workspace <needle>` to show only sessions whose workspace key (git repo root, else cwd) matches — by path substring or exact directory basename:

```bash
Zeloo sessions list --workspace my-project
Zeloo sessions list --workspace ~/code/Zeloo-agent
```

### Conversation Recap on Resume

When you resume a session, Zeloo displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Zeloo session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.Zeloo/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `Zeloo -r "<title>"` from the shell) and pick up where the platform left off.
...
Zeloo automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `Zeloo sessions list` or `Zeloo sessions browse`.
...
```bash
Zeloo sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Zeloo creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`Zeloo -c "my project"`), it automatically picks the most recent session in the lineage.
...
Zeloo provides a full set of session management commands via `Zeloo sessions`:
...
# Filter by platform
Zeloo sessions list --source telegram

# Show more sessions
Zeloo sessions list --limit 50
```
...
`Zeloo sessions export` is one surface for every export format, selected with `--format`:
...
Zeloo sessions export backup.jsonl

# Export sessions from a specific platform
Zeloo sessions export telegram-history.jsonl --source telegram

# Export a single session
Zeloo sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4

# Redact API keys/tokens/credentials from the exported content
Zeloo sessions export backup.jsonl --redact
```
...
Zeloo sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
Zeloo sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
Zeloo sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
Zeloo sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
`--format trace` emits Claude Code JSONL — the transcript shape the Hugging Face Hub auto-detects for its [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces). Write it locally, or add `--upload` to push it to your own private `Zeloo-traces` dataset (reads `HF_TOKEN`):

```bash
# Trace of the most recent session, to stdout
Zeloo sessions export --format trace

# One session to a local trace file
Zeloo sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
Zeloo sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.Zeloo/session-exports`).

```bash
# Export one session to Markdown
Zeloo sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
Zeloo sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
Zeloo sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
Zeloo sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
Zeloo sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
Zeloo sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
Zeloo sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
Zeloo sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
Zeloo sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
Zeloo sessions pin 20250305_091523_a1b2c3d4
Zeloo sessions pin 20250305 20250306

# Remove the pin
Zeloo sessions unpin 20250305_091523_a1b2c3d4

# List pinned sessions
Zeloo sessions pinned

# Machine-readable output, e.g. for a nightly backup of your pin set
Zeloo sessions pinned --json > pinned-sessions.json
```
...
Zeloo sessions prune

# Custom age threshold — bare numbers are days
Zeloo sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
Zeloo sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
Zeloo sessions prune --newer-than 5h

# Explicit window with absolute timestamps
Zeloo sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
Zeloo sessions prune --source telegram
Zeloo sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
Zeloo sessions prune --newer-than 5h --title "smoke test"   # title substring
Zeloo sessions prune --older-than 30 --max-messages 3        # tiny sessions
Zeloo sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
Zeloo sessions prune --model gpt-5 --older-than 1w           # by model (substring)
Zeloo sessions prune --provider openrouter --older-than 60   # by billing provider
Zeloo sessions prune --branch feature/old-experiment         # by git branch
Zeloo sessions prune --user 12345678 --chat-type group       # by messaging origin
Zeloo sessions prune --max-tokens 500 --older-than 7         # by token usage
Zeloo sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
Zeloo sessions prune --newer-than 5h --dry-run

# Skip confirmation
Zeloo sessions prune --older-than 30 --yes
```
...
the implicit 90-day default, so `Zeloo sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `Zeloo sessions prune` keeps the 90-day cutoff. Every
...
`Zeloo sessions archive` takes the same filters as `prune` but soft-hides
...
Zeloo sessions archive --newer-than 5h

# Archive by title substring, preview first
Zeloo sessions archive --title "dry run" --dry-run
Zeloo sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `Zeloo sessions archive` refuses to
archive your entire history. Archived sessions are hidden from
`Zeloo sessions list` and `/resume` but remain in the database and can be
unarchived from the Desktop/Dashboard session list.

### Session Statistics

```bash
Zeloo sessions stats
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`Zeloo insights`](/reference/cli-commands#Zeloo-insights).
...
`Zeloo sessions repair-routing` finds message-bearing session rows with no
...
Zeloo sessions repair-routing --apply

# Widen/narrow the contiguity window (default 900 seconds)
Zeloo sessions repair-routing --max-gap-seconds 300
```
...
(`cp ~/.Zeloo/state.db ~/.Zeloo/state.db.bak`).


## Importing Sessions from Claude Code and Codex CLI

Started a conversation in another agent CLI? You can pull it into Zeloo and
continue it here. Zeloo reads Claude Code's session logs
...
Zeloo sessions import

# Limit to one tool, or point at a specific file
Zeloo sessions import --from claude
Zeloo sessions import --from codex ~/.codex/sessions/2026/08/15/rollout-....jsonl
...
When Zeloo cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Zeloo uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Zeloo in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.Zeloo/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.Zeloo/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `gateway_routing` table in `~/.Zeloo/state.db` | Maps session keys to active session IDs (origin metadata, expiry flags) |
| Legacy routing mirror | `~/.Zeloo/sessions/sessions.json` | Backward-compat mirror of the routing index, written when `gateway.write_sessions_json: true` (the default) |
...
`state.db`; `~/.Zeloo/sessions/sessions.json` is a **legacy mirror** of it,
...
`Zeloo sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.Zeloo/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `Zeloo sessions list`, the cause is
`state.db` not receiving them — run `Zeloo sessions repair` and watch for a
...
`*.jsonl` files in `~/.Zeloo/sessions/`. They are no longer written or
read by Zeloo. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Zeloo process in the same `ZELOO_HOME`

Without pruning, `state.db` grows without bound — multi-GB files within weeks were reported on gateway + cron installs. If you would rather keep every ended session forever (the pre-#54189 behavior), turn it off in `~/.Zeloo/config.yaml`:
...
`in its tip segment`). `Zeloo sessions export` still works for such sessions.
...
:::tip
Auto-prune is **on by default**: ended sessions that have been inactive for `sessions.retention_days` (default 90) are removed at startup, and active sessions are never touched (see [Automatic Cleanup](#automatic-cleanup) above). Session history powers `session_search` recall across past conversations, so if you want to keep every ended session forever, set `sessions.auto_prune: false` in `config.yaml`, or raise `retention_days`. With auto-prune off, `Zeloo sessions prune` remains available for one-off cleanup (observed failure mode without any pruning: a 384 MB `state.db` with ~1000 sessions slowing down FTS5 inserts and `/resume` listing).
:::
