...
sidebar_position: 30
title: "Zeloo Relay"
description: "Connect Zeloo to messaging platforms through a relay connector that owns the platform credentials — enrollment, capabilities, config, and troubleshooting"
---

# Zeloo Relay (Connector)
...
Zeloo Relay is not a chat platform itself — it is a **connector system** that
...
1. Resolves a fresh Nous Portal access token from your existing login
   (`~/.Zeloo/auth.json`) — this proves which Nous org (tenant) you own. If
...
3. The connector verifies the token (signature, single-use, tenant match),
   ...
   once.
4. Persists the credentials into `~/.Zeloo/.env`:
...
`Zeloo gateway enroll` refuses to run in managed/hosted installs — there the
...
`Zeloo gateway enroll` remains available while runtime relay is disabled.

| Setting | Where | Meaning |
|---------|-------|---------|
| `GATEWAY_RELAY_URL` | env (`~/.Zeloo/.env`) | Connector relay WebSocket URL. Enables relay unless explicitly disabled in platform configuration. |
...
token. Re-login with `Zeloo auth add nous` (or `Zeloo setup`) and retry.
...
startup. Restart it (`Zeloo gateway restart`).
