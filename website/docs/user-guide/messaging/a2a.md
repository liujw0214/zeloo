...
[A2A](https://a2a-protocol.org) is the open Agent2Agent protocol (v1.0, stewarded by the Linux Foundation) for communication between independent AI agents. The Zeloo A2A plugin works in **both directions**: your agent can call other A2A agents as tools, and other agents can send tasks to your Zeloo over HTTP.

It interoperates with any A2A-compliant peer — another Zeloo, LangChain, CrewAI, Google ADK agents, or anything built on the official `a2a-sdk`.

## When to use A2A

- **Zeloo ↔ Zeloo across machines** — let your desktop agent hand tasks to a Zeloo on a server, or vice versa, each with its own memory, tools, and credentials.
...
- **Being a callable service** — expose your Zeloo so other frameworks' agents can send it tasks.
...
```bash
Zeloo tools enable a2a --platform cli        # CLI/TUI sessions
Zeloo tools enable a2a --platform telegram   # or any messaging platform
Zeloo tools enable a2a --platform a2a        # let inbound A2A tasks call peers (agent chaining)
```
...
With the platform enabled, Zeloo serves:
...
- **Audit log** — every exchange appends to `~/.Zeloo/a2a_audit.jsonl`.
