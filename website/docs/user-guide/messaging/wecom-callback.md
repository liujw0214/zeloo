...
Connect Zeloo to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Zeloo supports two WeCom integration modes:
...
> Run `Zeloo gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Zeloo decrypts the message, queues it for the agent
...
(Use `Zeloo gateway start` only after `Zeloo gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Zeloo and the token the
...
in `~/.Zeloo/.env` values around `=` will also break signature checks. After
fixing, restart `Zeloo gateway run`.
...
Check `Zeloo gateway run` logs for the bound host/port. If the adapter bound to
