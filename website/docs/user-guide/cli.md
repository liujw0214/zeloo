...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Zeloo Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Zeloo Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `Zeloo setup --portal` — and you're ready to `Zeloo chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
Zeloo also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `Zeloo --tui` — see the [TUI](tui.md) guide.
:::
...
Zeloo chat --query-file prompt.txt
Zeloo chat --query-file - < prompt.txt

# With a specific model
Zeloo chat --model "anthropic/claude-sonnet-4"

# With a specific provider
Zeloo chat --provider nous        # Use Nous Portal
Zeloo chat --provider openrouter  # Force OpenRouter
...
Zeloo chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
Zeloo --continue             # Resume the most recent CLI session (-c)
Zeloo --resume <session_id>  # Resume a specific session by ID (-r)
Zeloo --resume latest        # Resume the most recent session (same as -c)
Zeloo --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
Zeloo chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
Zeloo -w                         # Interactive mode in worktree
Zeloo -w -z "Fix issue #123"     # Single query in worktree
```

### Worktree cleanup

`Zeloo -w` sessions create disposable worktrees under `<repo>/.worktrees/`.
...
```bash
Zeloo worktree list              # audit: age, size, verdict, reason per tree
Zeloo worktree prune             # remove safe trees + delete merged branches
Zeloo worktree prune --dry-run   # show the plan without changing anything
Zeloo worktree prune --trees-only     # leave local branches alone
Zeloo worktree prune --branches-only  # leave worktrees alone
```
...
- Trees **in use by a running Zeloo session** are never touched.
- **Untracked-only scratch** (PR body drafts, notes) is archived to
  `~/.Zeloo/archive/worktree-prune/` before its tree is removed — never
  destroyed.
...
launches `Zeloo -w` for days — no longer accumulate merged scratch trees
...
The `Zeloo plugins` commands manage native Zeloo plugins and portable Agent
...
Zeloo plugins disable <plugin-name>
Zeloo plugins update <plugin-name>
Zeloo plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. Zeloo currently
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Zeloo CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Zeloo CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `ZELOO_YOLO_MODE` is on (either `Zeloo --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Zeloo to background (Unix only). Run `fg` in the shell to resume. |
...
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.Zeloo/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. Zeloo supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Zeloo replies |
...
```bash
Zeloo -s Zeloo-agent-dev,github-auth
Zeloo chat -s github-pr-workflow -s github-auth
```

Zeloo loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.Zeloo/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.Zeloo/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Zeloo..."
  # Add your own!
```
...
`Ctrl+J` and backslash continuation are enabled by default, matching Claude Code / Codex / OpenCode multiline shortcuts. On supported terminals such as iTerm2, Zeloo also requests extended key reporting so `Shift+Enter` arrives as a distinct newline key. If your terminal sends LF for plain `Enter` and you need the legacy `Ctrl+J`-as-submit fallback, opt out:
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Zeloo recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work by default. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Zeloo — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while Zeloo is working, Zeloo prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Zeloo to the background — just like any terminal process. The shell prints a confirmation:

```
Zeloo Agent has been suspended. Run `fg` to bring Zeloo Agent back.
```
...
Resume this session with:
  Zeloo --resume 20260225_143052_a1b2c3
...
```bash
Zeloo --continue                          # Resume the most recent CLI session
Zeloo -c                                  # Short form
Zeloo -c "my project"                     # Resume a named session (latest in lineage)
Zeloo --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
Zeloo --resume "refactoring auth"         # Resume by title
Zeloo --resume latest                     # Resume the most recent session (same as -c)
Zeloo --resume latest --in ./my-project   # Latest session for ./my-project's workspace
Zeloo -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `Zeloo sessions rename <id> <title>` from the command line. Use `Zeloo sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Zeloo's SQLite state database under `~/.Zeloo/state.db`. The database keeps:
...
Zeloo immediately confirms the task and gives you back the prompt:
