...
fills the password through Zeloo. The tool result it sees is
...
- **Authenticator key saved with the login** (the "setup key" or `otpauth://`
  ...
  hold a TOTP seed count too): Zeloo generates the current code and enters
  it. Nobody is asked. Add the key in **Settings → Passwords & Logins → Add**
  or `Zeloo vault add`; the item shows a *2FA auto* badge.
- **Code sent to your phone or email**: a small prompt appears in your
  surface ("Verification code for github.com"), you type the code, Zeloo
...
in, Zeloo picks it up automatically and its website logins become fillable
...
per session, 30 minutes idle). Zeloo hands the master password to the manager's
...
Prefer not to use a detected manager? `Zeloo vault sources --disable bitwarden`,
...
Passwords & Logins → Add**, or `Zeloo vault add`), bound to the checkout site,
...
- **CLI**: `Zeloo vault list`, `Zeloo vault add`, `Zeloo vault rm <handle>`,
  `Zeloo vault sources`.

Items live encrypted under `~/.Zeloo/vault/` (Fernet key + vault file, both
...
Cron jobs, webhooks, the API server and `Zeloo chat -q` have nobody to answer a
...
**Does:** the password never enters the model's context through Zeloo: not in
