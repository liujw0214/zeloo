...
chat completions — use your Zeloo-managed provider subscription as their
...
| Use case | "Use Zeloo as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Zeloo stores
the refresh token in `~/.Zeloo/auth.json` — the same place all Zeloo
...
Model:      Zeloo-4-70B    # or Zeloo-4.3-36B, Zeloo-4-405B
```
...
interface in `zeloo_cli/proxy/adapters/`.
...
If you see `not logged in`, run `Zeloo portal`. If you see
...
`zeloo_cli/proxy/adapters/<provider>.py` and registering it in
