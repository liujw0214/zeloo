...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Zeloo's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Zeloo instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
Zeloo memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `Zeloo` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Zeloo syncs them back at session init.
...
The `Zeloo honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `Zeloo memory setup honcho` (or run `Zeloo memory setup` and pick it from the list); the `Zeloo honcho` subcommand then appears on the next invocation.

```bash
Zeloo memory setup honcho    # Configure Honcho directly (works before activation)
Zeloo honcho status          # Connection status, config, and key settings
Zeloo honcho setup           # Redirects to `Zeloo memory setup` (post-activation alias)
Zeloo honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
Zeloo honcho peer            # Show or update peer names + dialectic reasoning level
Zeloo honcho mode            # Show or set recall mode (hybrid/context/tools)
Zeloo honcho tokens          # Show or set token budget for context and dialectic
Zeloo honcho identity        # Seed or show the AI peer's Honcho identity
Zeloo honcho sync            # Sync Honcho config to all existing profiles
Zeloo honcho peers           # Show peer identities across all profiles
Zeloo honcho sessions        # List known Honcho session mappings
Zeloo honcho map             # Map current directory to a Honcho session name
Zeloo honcho enable          # Enable Honcho for the active profile
Zeloo honcho disable         # Disable Honcho for the active profile
Zeloo honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `Zeloo honcho`

If you previously used the standalone `Zeloo honcho setup`:
...
No re-login or re-setup needed. Run `Zeloo memory setup` and select "honcho" — the wizard detects your existing config.
