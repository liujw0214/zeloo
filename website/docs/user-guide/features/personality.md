...
description: "Customize Zeloo Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Zeloo Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `ZELOO_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Zeloo is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Zeloo now seeds a default `SOUL.md` automatically in:

```text
~/.Zeloo/SOUL.md
```

More precisely, it uses the current instance's `ZELOO_HOME`, so if you run Zeloo with a custom home directory, it will use:
...
- Zeloo creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Zeloo loads `SOUL.md` only from `ZELOO_HOME`
- Zeloo does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Zeloo falls back to a built-in default identity
...
If Zeloo loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `ZELOO_HOME`, the personality belongs to the Zeloo instance itself.

That also makes it easier to teach users:
- "Edit `~/.Zeloo/SOUL.md` to change Zeloo' default personality."
...
- how Zeloo should handle uncertainty, disagreement, or ambiguity
...
## What Zeloo injects into the prompt
...
Zeloo ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Zeloo, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Zeloo its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

Built-in personalities are always available on every surface (CLI, messaging platforms, TUI, and the desktop app). You can add your own — or override a built-in by reusing its name — in `~/.Zeloo/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.Zeloo/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-Zeloo)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Zeloo speaks
- `display.skin` and `/skin` affect how Zeloo looks in the terminal
