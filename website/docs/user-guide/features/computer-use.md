...
Zeloo Agent can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended Zeloo integration. It
...
**Fresh installs already have the driver.** The Zeloo installer
...
- **`Zeloo tools`** → pick `🖱️  Computer Use` — installs the driver
...
```
Zeloo computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `Zeloo computer-use
status` to verify the install.

Already have cua-driver? Zeloo reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `Zeloo update`, and the first
`computer_use` call of a session, Zeloo checks the local version and
...
selected with `ZELOO_CUA_DRIVER_CMD` stays
under your control, so Zeloo reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. Zeloo autodetection is a
planned cua-driver follow-up, so currently point Zeloo at that directory or
...
interface. The built-in toolset provides Zeloo actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `Zeloo computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the Zeloo host identity. |
...
```
Zeloo -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.Zeloo/config.yaml`.

## Permission modes and logged-in browser profiles

Zeloo maps its existing approval UX onto cua-driver's immutable runtime
modes. Permission mode and capability manifest approval are launch settings.
They cannot change after the runtime starts:

| Zeloo session | cua-driver mode | Human intervention |
|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Zeloo approvals; Cua stops at its protected boundary |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Zeloo risk acceptance; no runtime Cua prompts |
...
computer_use:
  ...
  capability_manifest: ~/.Zeloo/cua-manifest.yaml
```
...
for the format). Zeloo launches a private runtime with
...
identity instead of resetting with every Zeloo build), and Zeloo verifies
...
resetting or closing the Zeloo session, cancellation cleanup, or process exit
closes that transport session. Zeloo also stops private runtimes that it
launched for bounded or unrestricted access. One Zeloo
conversation cannot change another runtime's mode or grants. Bounded and
unrestricted modes use a private
embedded service under the Zeloo host identity.
...
## `Zeloo computer-use doctor` — your first triage stop

`Zeloo computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ Zeloo computer-use doctor
...
Zeloo run declares a public cua-driver **session name** (something like
`Zeloo-3a7b9c14d2e8`). The name labels cursor identity and related state, so
...
without it. Zeloo disables it automatically where it is a known failure
...
Zeloo keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Zeloo-side `computer_use` workflow and action vocabulary. For
...
The command installs the pack under `~/.cua-driver/skills/cua-driver`. Zeloo
autodetection is a planned cua-driver follow-up, so currently point Zeloo at
...
These are **platform deep dives, not duplicates of the Zeloo skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Zeloo
...
image capture also saves a bounded, shareable copy under Zeloo' image cache
...
Zeloo applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.Zeloo/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Zeloo applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Zeloo agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Zeloo agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Zeloo warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Zeloo from inside the RDP / console
...
On Linux, native Wayland support remains an explicit opt-in. Zeloo passes the
...
```
ZELOO_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
ZELOO_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Zeloo disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Zeloo sets
...
When it's on, `Zeloo computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Zeloo at a binary you built from source instead
of the published release. Zeloo resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`ZELOO_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Zeloo testing (see notes).
...
### Option B — point Zeloo straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`ZELOO_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
ZELOO_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
ZELOO_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Zeloo is using your build

- `Zeloo computer-use status` prints the resolved binary path and
  version.
- `Zeloo computer-use doctor` confirms the binary is reachable and
...
- **Zeloo spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  Zeloo YOLO, Zeloo instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Zeloo
  change needed either way.
- **Local builds skip the version check.** Zeloo warns when the
...
**First action when anything's off: run `Zeloo computer-use doctor`.**
...
Run `Zeloo computer-use install` to fetch the cua-driver binary, or
run `Zeloo tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `Zeloo computer-use doctor` will
...
- **Zeloo-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  Zeloo `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. Zeloo autodetection is a
  planned follow-up; currently point Zeloo at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Zeloo among them)
