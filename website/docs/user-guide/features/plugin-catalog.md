...
description: "Browse and install reviewed, SHA-pinned Zeloo plugins from the curated catalog"
---

# Plugin Catalog

The plugin catalog is a curated, human-reviewed directory of Zeloo plugins you
can install by name with a single command:

```bash
Zeloo plugins install <name>
```
...
[`plugin-catalog/`](https://github.com/NousResearch/Zeloo-agent/tree/main/plugin-catalog)
directory of the Zeloo-agent repository, declaring:

| Field | Meaning |
|---|---|
| `name` | The catalog key you pass to `Zeloo plugins install` |
...
| `requires_ZELOO` | Minimum Zeloo version, e.g. `>=0.19` (optional) |
...
`Zeloo plugins update <name>` never runs `git pull` for catalog installs —
...
enabled/disabled state is preserved. `Zeloo plugins list` shows catalog
...
(`https://Zeloo-agent.nousresearch.com/docs/api/plugin-catalog.json`).
`search`/`install`/`update` fetch it at most every six hours and cache it under
`~/.Zeloo/cache/`, so new entries and removals reach installed clients without
updating Zeloo. Offline, the copy shipped with your checkout is used. Removals
from the in-tree list and the live list are always both enforced.

### Custom git URLs are different

`Zeloo plugins install <git-url>` still works for any repository, but it
...
[plugin-catalog README](https://github.com/NousResearch/Zeloo-agent/tree/main/plugin-catalog);
...
- [Plugins](plugins.md) — the plugin system itself: manifest format, enabling,
  configuration
- [Built-in Plugins](built-in-plugins.md) — plugins that ship with Zeloo
- [Build a Zeloo Plugin](/developer-guide/plugins) — write your own
