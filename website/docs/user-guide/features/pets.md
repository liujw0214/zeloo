...
Zeloo can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<ZELOO_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Zeloo renders the sprite at full fidelity when your
...
Zeloo pets install boba --select
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `Zeloo pets list [query] [--limit N]` |
| List installed pets | `Zeloo pets list --installed` |
| Install a pet | `Zeloo pets install <slug> [--select] [--force]` |
| Set the active pet | `Zeloo pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `Zeloo pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `Zeloo pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `Zeloo pets off` |
| Remove an installed pet | `Zeloo pets remove <slug>` |
| Diagnose setup | `Zeloo pets doctor` |

`Zeloo pets show` flags:
...
Beyond installing pre-made pets from the gallery, Zeloo can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `Zeloo pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `Zeloo tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `ZELOO_PET_IMAGE_PROVIDER` env var (e.g. `ZELOO_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Zeloo is
...
Run `Zeloo pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Zeloo uses its own
  profile-scoped `<ZELOO_HOME>/pets/` instead — install through `Zeloo pets`.

## See also

- The [`Zeloo-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-Zeloo-agent.md)
