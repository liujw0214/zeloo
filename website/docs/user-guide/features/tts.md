...
Zeloo Agent supports both text-to-speech output and voice message transcription across all messaging platforms.

:::tip Nous Subscribers
If you have a paid [Nous Portal](https://portal.nousresearch.com) subscription, OpenAI TTS is available through the **[Tool Gateway](tool-gateway.md)** without a separate OpenAI API key. New installs can run `Zeloo setup --portal` to log in and turn on every gateway tool at once; existing installs can pick **Nous Subscription** for just TTS via `Zeloo model` or `Zeloo tools`.
:::
...
| CLI | Saved to `~/.Zeloo/audio_cache/` | MP3 |
...
tts:
  provider: "edge"              # "edge" | "elevenlabs" | "openai" | "minimax" | "mistral" | "gemini" | "xai" | "deepinfra" | "neutts" | "kittentts" | "piper" — or "nous" for the managed Tool Gateway (written when you pick Nous Subscription in `Zeloo tools`)
  ...
  piper:
    ...
    # voices_dir: ''                            # default: ~/.Zeloo/cache/piper-voices/
...
- If `region` is omitted, `MINIMAX_API_KEY` keeps precedence for backward compatibility. If only `MINIMAX_CN_API_KEY` is configured, Zeloo selects `cn`.
- An explicitly selected region must have its matching credential. Zeloo never borrows the other region's key. A `base_url` override does not change the selected credential, and an override pointing at the other region's official endpoint is rejected.
...
If the file contains `{transcript}` or `{{ transcript }}`, Zeloo replaces that placeholder with the live TTS text. Otherwise, Zeloo appends a labeled `TRANSCRIPT` section automatically. The persona prompt stays local and is not shown in the chat reply.
...
Each provider has a documented per-request input-character cap. Zeloo splits longer replies into ordered, sentence-aware chunks before calling the provider, so the full normalized text is preserved instead of silently truncated:
...
**Install via `Zeloo tools`** → Voice & TTS → Piper — Zeloo runs `pip install piper-tts` for you. Or install manually: `pip install piper-tts`.
...
On the first TTS call for a voice that isn't cached locally, Zeloo runs `python -m piper.download_voices <name>` and downloads the model (~20-90MB depending on quality tier) into `~/.Zeloo/cache/piper-voices/`. Subsequent calls reuse the cached model.
...
Local engines (Piper, KittenTTS) load their model lazily, so without help the *first* spoken reply after you turn speech on pays the whole model load — and on a fresh install the voice download — as silence before the first word. Zeloo treats the speech-output toggles as the signal that TTS is about to be needed:
...
**Supported `output_format` values:** `mp3` (default), `wav`, `ogg`, `flac`, `m4a`, `aac`, `amr`, `opus`. Your command must actually produce that format (e.g. via `ffmpeg`); Zeloo only validates the declared value and names the output file accordingly. An unknown value falls back to `mp3`. The chosen format is also exposed to the command as the `{format}` placeholder.

**Subprocess environment:** command providers (TTS and STT) run with Zeloo secrets scrubbed from the child environment — gateway bot tokens, LLM provider API keys, and internal relay credentials are removed; `PATH`, `HOME`, locale, and other normal variables are kept. If your command template needs its own API key from the environment (e.g. a `curl` one-liner), list the variable names under `env_passthrough` in the provider config:
...
Credentials come from your shell environment (`VOLCENGINE_APP_ID` / `VOLCENGINE_ACCESS_TOKEN`) or `~/.doubao-speech/config.yaml`. Pick a voice by adding `--voice zh-female-warm` (or any other alias from `doubao-speech list-voices`) to the command. `doubao-speech` also bundles streaming ASR — see the [STT section below](#example-doubao--volcengine-asr) for Zeloo integration. Source and full docs: [github.com/Hypnus-Yuan/doubao-speech](https://github.com/Hypnus-Yuan/doubao-speech).

#### Placeholders

Your command template can reference these placeholders. Zeloo substitutes them at render time and shell-quotes each value for the surrounding context (bare / single-quoted / double-quoted), so paths with spaces and other shell-sensitive characters are safe.

| Placeholder      | Meaning                                              |
|------------------|------------------------------------------------------|
| `{input_path}`   | Path to the temp UTF-8 text file Zeloo wrote        |
...
| `output_format`    | `mp3`   | One of `mp3` / `wav` / `ogg` / `flac`. Auto-inferred from the output extension if Zeloo picks a path.      |
| `voice_compatible` | `false` | When `true`, Zeloo converts MP3/WAV output to Opus/OGG via ffmpeg so Telegram renders a voice bubble.      |
...
Command-type providers run whatever shell command you configure, with your user's permissions. Zeloo quotes placeholder values and enforces the configured timeout, but the command template itself is trusted local input — treat it the same way you would a shell script on your PATH.
...
| A voice-listing API used by `Zeloo setup` | **Plugin** (override `list_voices()`) |
...
Drop this in `~/.Zeloo/plugins/my-tts/`:
...
Enable it (`Zeloo plugins enable my-tts`), point `tts.provider` at it (`tts.provider: my-tts` in `config.yaml`), and the `text_to_speech` tool will route through your plugin.

#### Optional hooks

Override these on your provider class for richer integration:

- `list_voices()` → list of `{id, display, language, gender, preview_url}` dicts shown in `Zeloo tools`.
- `list_models()` → list of `{id, display, languages, max_text_length}` dicts.
- `get_setup_schema()` → return `{name, badge, tag, env_vars: [{key, prompt, url}]}` to power the picker row in `Zeloo tools` / `Zeloo setup`. Without this, the plugin still works but its row in the picker is minimal.
...
:::info Zero Config
Local transcription works out of the box when `faster-whisper` is installed. If that's unavailable, Zeloo can also use a local `whisper` CLI from common install locations (like `/opt/homebrew/bin`) or a custom command via `ZELOO_LOCAL_STT_COMMAND`.
:::
...
stt:
  ...
  local:
    ...
    language: ""              # optional ISO-639-1 hint; blank = use ZELOO_LOCAL_STT_LANGUAGE if set, else auto-detect
  groq:
    language: ""              # optional ISO-639-1 hint; blank = use ZELOO_LOCAL_STT_LANGUAGE if set, else auto-detect
  openai:
  ...
  xai:
    ...
    language: ""              # optional ISO-639-1 hint; blank = use ZELOO_LOCAL_STT_LANGUAGE if set, else "en"
```
...
**Groq API** — Requires `GROQ_API_KEY`. Good cloud fallback when you want a free hosted STT option. Set `stt.groq.language` (or the global `ZELOO_LOCAL_STT_LANGUAGE` env var) to skip Whisper's auto-detect and reduce latency on known-language audio.
...
**Mistral API (Voxtral Transcribe)** — Requires `MISTRAL_API_KEY`. Uses Mistral's [Voxtral Transcribe](https://docs.mistral.ai/capabilities/audio/speech_to_text/) models. Supports 13 languages, speaker diarization, and word-level timestamps. Install with `cd ~/.Zeloo/Zeloo-agent && uv pip install -e ".[mistral]"`.
...
**Custom local CLI fallback** — Set `ZELOO_LOCAL_STT_COMMAND` if you want Zeloo to call a local transcription command directly. The command template supports `{input_path}`, `{output_dir}`, `{language}`, and `{model}` placeholders. Zeloo tokenizes the rendered template into an argument list and executes it without a shell, so operators such as `|`, `>`, `&&`, and `;` are passed as literal arguments. Your command must write a `.txt` transcript somewhere under `{output_dir}`.
...
export VOLCENGINE_APP_ID="your-app-id"
export VOLCENGINE_ACCESS_TOKEN="your-access-token"
export ZELOO_LOCAL_STT_COMMAND='doubao-speech transcribe {input_path} --out {output_dir}/transcript.txt'
```
...
```bash
export ZELOO_LOCAL_STT_COMMAND='sh -c '\''whisper "$1" --output_format txt --output_dir "$2" | tee "$2/whisper.log"'\'' _ {input_path} {output_dir}'
```
...
Zeloo writes the incoming voice message to `{input_path}`, runs the command, and reads the `.txt` file produced under `{output_dir}`. Language is auto-detected by the Volcengine bigmodel endpoint.

### Fallback Behavior

An **explicit** `stt.provider` selection (written in `config.yaml`, e.g. via `Zeloo tools`) is honored strictly — if that provider can't run, transcription fails with a clear error (`stt is configured to use <provider> (set via Zeloo tools), but <failure>. Run 'Zeloo tools' to change it.`) instead of silently switching engines. Note that `stt.provider: local` written in your config counts as an explicit selection.

When **no provider has ever been selected**, Zeloo auto-detects from what's available:
- **Local faster-whisper unavailable** → Tries a local `whisper` CLI or `ZELOO_LOCAL_STT_COMMAND` before cloud providers
...
1. If `{output_path}` exists and is non-empty → Zeloo reads it as UTF-8 text.
2. Otherwise, if the command wrote to stdout → Zeloo uses that.
...
For `format: json` / `srt` / `vtt`, Zeloo returns the raw file content as the `transcript` field. Extracting `.text` from JSON is out of scope for the runner — either configure `format: txt`, or post-process JSON downstream.
...
The shell command runs under the same user as Zeloo with full filesystem access — same trust model as `tts.providers.<name>: type: command` and `ZELOO_LOCAL_STT_COMMAND`. Only declare command providers from sources you trust.
...
| Only the legacy single-command escape hatch is wanted        | `ZELOO_LOCAL_STT_COMMAND` env var (argv-tokenized; no implicit shell) |
...
Drop this in `~/.Zeloo/plugins/my-stt/`:
...
Enable it (`Zeloo plugins enable my-stt`), set `stt.provider: my-stt` in `config.yaml`, and voice-message transcription will route through your plugin.
...
- `get_setup_schema()` → return `{name, badge, tag, env_vars: [{key, prompt, url}]}` to power picker rows in `Zeloo tools` / `Zeloo setup` (the picker category for STT is not yet shipped — this metadata is available to plugins for forward compatibility).
