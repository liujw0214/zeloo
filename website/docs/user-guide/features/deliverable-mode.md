...
When Zeloo Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.Zeloo/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.Zeloo/config.yaml` (switchable per session
via `/personality`).
...
If you use Zeloo' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.Zeloo/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Zeloo Agent's deliverable
