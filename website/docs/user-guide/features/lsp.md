...
Zeloo runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Zeloo
...
1. Zeloo captures a baseline of current diagnostics for the file.
...
…). Zeloo auto-detects the binary on PATH or in
`<ZELOO_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Zeloo at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<ZELOO_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`Zeloo lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Zeloo installs both packages together when you
run `Zeloo lsp install typescript` or auto-install fires on first
use.

## CLI

```
Zeloo lsp status          # service state + per-server install status
Zeloo lsp list            # registry, optionally --installed-only
Zeloo lsp install <id>    # eagerly install one server
Zeloo lsp install-all     # try every server with a known recipe
Zeloo lsp restart         # tear down running clients
Zeloo lsp which <id>      # print resolved binary path
```

`Zeloo lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <ZELOO_HOME>/lsp/bin
...
When `install_strategy: auto`, Zeloo installs binaries into
`<ZELOO_HOME>/lsp/bin/`. NPM packages land in
`<ZELOO_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Zeloo-owned and is
...
**single process** per Zeloo process: the first Python project spawns
...
**`Zeloo lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<ZELOO_HOME>/lsp/bin/`. Run
`Zeloo lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `Zeloo lsp status`**
...
When `Zeloo lsp status` shows a `Backend warnings` section, install
...
`~/.Zeloo/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.Zeloo/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `Zeloo lsp restart` to clear the set;
