...
title: "Plugins"
description: "Extend Zeloo with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

Zeloo has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/developer-guide/adding-tools) page is for built-in Zeloo
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Zeloo Plugin](/developer-guide/plugins)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.Zeloo/plugins/` with a `plugin.yaml` and Python code:

```
~/.Zeloo/plugins/my-plugin/
...
Start Zeloo — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.Zeloo/plugins/hello-world/plugin.yaml`**
...
**`~/.Zeloo/plugins/hello-world/__init__.py`**

```python
"""Minimal Zeloo plugin — registers a tool and a hook."""
...
Drop both files into `~/.Zeloo/plugins/hello-world/`, restart Zeloo, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

The model-facing tool description belongs in `schema["description"]`. The optional `ctx.register_tool(description=...)` value is separate `ToolEntry` registry metadata: when omitted, it defaults to the schema description, but Zeloo does not copy it back into a schema that lacks `description`. Prefer defining the text once in the schema. If you provide both values, keep them synchronized; the model sees the schema value.

Project-local plugins under `./.Zeloo/plugins/` are disabled by default. Enable them only for trusted repositories by setting `ZELOO_ENABLE_PROJECT_PLUGINS=true` before starting Zeloo.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `Zeloo <plugin> <subcommand>` |
...
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `Zeloo plugins install` |
| Distribute via pip | `[project.entry-points."zeloo_agent.plugins"]` |
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with Zeloo — see [Built-in Plugins](/user-guide/features/built-in-plugins) |
| User | `~/.Zeloo/plugins/` | Personal plugins |
| Project | `.Zeloo/plugins/` | Project-specific plugins (requires `ZELOO_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `zeloo_agent.plugins` entry_points | Distributed packages |
| Nix | `services.Zeloo-agent.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/getting-started/nix-setup#plugins) |

Later sources override earlier ones on name collision, so a user plugin with the same name as a bundled plugin replaces it.

### Plugin sub-categories

Within each source, Zeloo also recognizes sub-category directories that route plugins to specialized discovery systems:
...
User plugins at `~/.Zeloo/plugins/model-providers/<name>/` override bundled model providers of the same name (last-writer-wins in `register_provider()`), so you can replace a built-in provider profile without any repo edits. Memory providers resolve the other way round: for `~/.Zeloo/plugins/memory/<name>/` the **bundled** provider wins on a name collision (bundled, then user, then project, then entry points; first seen wins), so a user memory provider needs its own unique name.

## Plugins are opt-in (with a few exceptions)

**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `Zeloo plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.Zeloo/config.yaml`. This stops third-party code from running without your explicit consent.
...
```bash
Zeloo plugins                    # interactive toggle (space to check/uncheck)
Zeloo plugins enable <name>      # add to allow-list
Zeloo plugins disable <name>     # remove from allow-list + add to disabled
```

After `Zeloo plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

For a reproducible install, pin a full immutable commit (tags, branches, and
abbreviated SHAs are not accepted):

```bash
Zeloo plugins install owner/repo --ref 0123456789abcdef0123456789abcdef01234567
```

Zeloo checks out the commit detached, verifies that `HEAD` exactly matches the
requested SHA, and records the canonical source, installed revision, and pin
status in the current profile. `Zeloo plugins update` refuses to move a pinned
plugin; choose a new exact commit explicitly with
`Zeloo plugins install <source> --force --ref <new-commit>`. The
profile-local install metadata contains no config values, environment values,
secrets, or capability grants.

The same pin is available in Zeloo Desktop: **Skills → Plugins → Install from
...
can confirm everyone is running the same commit. `Zeloo plugins list` prints
...
`Zeloo plugins install` clones non-interactively (it never prompts for a
username or password), so a private repo needs a credential Zeloo can find on
...
before. The same resolution applies to `Zeloo plugins update`, catalog MCP
installs from git, and profile distributions fetched from a git URL.

### What the allow-list does NOT gate

Several categories of plugin bypass `plugins.enabled` — they're part of Zeloo' built-in surface and would break basic functionality if gated off by default:
...
| **User-installed platforms** (under `~/.Zeloo/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |

In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.Zeloo/plugins/`.

### Approval transports

An approval transport changes **where a human sees and answers** an existing
Zeloo tool-approval request. It does not decide whether a command needs
...
`present` may be synchronous or async. Zeloo runs it on a bounded worker and
...
`transport_fallback: builtin`. Without that exact opt-in, Zeloo never
materializes the prompt on another surface.

Zeloo still owns hardline blocks, sudo-stdin protection, user deny rules,
...
When you upgrade to a version of Zeloo that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.Zeloo/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled standalone plugins are NOT grandfathered — even existing users have to opt in explicitly. (Bundled platform/backend plugins never needed grandfathering because they were never gated.)

## Available hooks

Plugins can register the 26 lifecycle events currently accepted by `zeloo_cli.plugins.VALID_HOOKS`. The **[Event Hooks catalog](/user-guide/features/hooks#shipped-plugin-hook-catalog)** is canonical for exact timing, return handling, payload fields, and privacy notes.
...
Zeloo has four kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.Zeloo/plugins/` |
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and Zeloo also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Zeloo Plugin](/developer-guide/plugins) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a Zeloo Plugin](/developer-guide/plugins) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Zeloo Plugin](/developer-guide/plugins) · [Extending the CLI](/developer-guide/extending-the-cli) |
| A **subcommand** for `Zeloo <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/developer-guide/extending-the-cli) |
...
| An **STT backend** (any CLI — whisper.cpp, custom whisper binary, local ASR CLI) | Config-driven (recommended) — declare under `stt.providers.<name>` with `type: command` in `config.yaml`, or set `ZELOO_LOCAL_STT_COMMAND` for the legacy single-command escape hatch. OR Python backend plugin — `ctx.register_transcription_provider()` for Python-SDK engines (OpenRouter, SenseAudio, Gemini-STT, etc.). | [STT Setup](/user-guide/features/tts#stt-custom-command-providers) · [Python plugin guide](/user-guide/features/tts#python-plugin-providers-stt) |
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Zeloo auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `Zeloo skills tap add <repo>` | [Skills Hub](/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.Zeloo/hooks/<name>/` | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `Zeloo plugins install` needed. See the **[Nix Setup guide](/getting-started/nix-setup#plugins)** for full details.
...
```bash
Zeloo plugins                               # unified interactive UI
Zeloo plugins list                          # table: enabled / disabled / not enabled
Zeloo plugins search <term>                 # search the Zeloo plugin catalog
Zeloo plugins install <name>                # install a catalog entry (repo @ reviewed pinned SHA)
Zeloo plugins install user/repo             # install from Git, then prompt Enable? [y/N]
Zeloo plugins install user/repo --enable    # install AND enable (no prompt)
Zeloo plugins install user/repo --no-enable # install but leave disabled (no prompt)
Zeloo plugins update my-plugin              # pull latest (local edits are autostashed and re-applied)
Zeloo plugins remove my-plugin              # uninstall
Zeloo plugins enable my-plugin              # add to allow-list
Zeloo plugins disable my-plugin             # remove from allow-list + add to disabled
Zeloo plugins capabilities [my-plugin]      # declared vs granted capabilities
```

### One-click install links (Desktop)

Zeloo Desktop registers the `Zeloo://` URL scheme, so a website, README, or
chat message can link straight to a plugin install:

```
Zeloo://plugin/install?repo=owner/repo            # main install link
Zeloo://plugin/install?repo=owner/repo&enable=1   # enable the agent plugin after install
Zeloo://plugin/install?repo=owner/repo&force=1    # replace an existing install
```

Clicking one opens Zeloo and shows a **confirmation dialog** — the repo id,
...
Plugins → Install from Git**. Legacy `Zeloo://plugin-agent/…` and
`Zeloo://plugin-desktop/…` URLs route into the same dialog. In dev builds
(`npm run dev`) the scheme is `Zeloo-dev://`.

Websites need no SDK — a normal anchor works:

```html
<a href="Zeloo://plugin/install?repo=owner/repo&enable=1">Install in Zeloo</a>
```

MCP servers have the equivalent link form — see
[Add to Zeloo link](/reference/mcp-config-reference#add-to-Zeloo-link).
...
When a plugin declares capabilities, `Zeloo plugins install` (and
`Zeloo plugins enable`) shows the list with one-line risk descriptions and
...
granted, `Zeloo plugins update` surfaces the additions and asks again. New
...
`Zeloo plugins enable <id>` interactively to grant them later.

Inspect the state at any time:

```bash
Zeloo plugins capabilities             # all plugins with declared/granted capabilities
Zeloo plugins capabilities my-plugin   # one plugin, declared vs granted
```
...
not a code audit, and Zeloo has not reviewed the plugin's code. Only install
...
### Discovering plugins — the Zeloo plugin catalog

`Zeloo plugins search <term>` searches the **Zeloo plugin catalog** — the
curated, SHA-pinned catalog maintained in the Zeloo-agent repository
(`plugin-catalog/`). Matching covers entry names, descriptions, and declared
tools:

```bash
Zeloo plugins search telegram    # search the catalog
Zeloo plugins browse             # browse every entry
Zeloo plugins info <name>        # full details for one entry
```
...
recorded so `Zeloo plugins update` can re-pin when the catalog moves:

```bash
Zeloo plugins install <catalog-name>
```
...
A **plugin pack** is a declarative, shareable YAML file (`Zeloo-pack.yaml`)
...
version: 1.0.0
plugins:
  - name: Zeloo-telegram-business       # bare plugin-catalog name…
...
```bash
Zeloo plugins pack show ./Zeloo-pack.yaml     # dry-run review
Zeloo plugins pack install ./Zeloo-pack.yaml  # review → confirm → install
Zeloo plugins pack export > Zeloo-pack.yaml   # snapshot the current install
Zeloo plugins pack export --enabled-only       # only plugins.enabled
```
...
same pinned install path as `Zeloo plugins install --ref <sha>` and record
...
capability-consent prompt — identical to a single `Zeloo plugins install`.
...
provenance (installed via `Zeloo plugins install`). Local-only plugins are
...
auto-installed — install those manually for now (`Zeloo skills`). Wiring
skill-hub ids into pack install is a documented follow-up seam.

### Install-time security scanning

Every `Zeloo plugins install` and `Zeloo plugins update` runs a static
...
On `Zeloo plugins update`, a dangerous verdict on the updated tree
...
Running `Zeloo plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `Zeloo plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
...
- Zeloo reuses that session's stored platform, chat, thread, profile, and conversation history. Plugins cannot supply a new chat route through this API.
- Zeloo rechecks the stored route against the gateway's current authorisation rules before dispatch.
- Routes that relied only on an adapter-time or upstream authorisation decision are rejected unless Zeloo can revalidate them from current core allowlists, pairing, or explicit allow-all configuration.
...
- The route and conversation are pinned while dispatch is pending. Zeloo drops the request if topic recovery changes the route or the session rotates before handling starts.
...
:::warning
Only grant gateway injection to plugins you trust. Zeloo checks this host API permission and restricts it to existing session routes, but Python plugins run in-process and this setting is not a sandbox.
:::
...
`ctx.call_mcp()` lets a plugin call a tool on one of the user's configured MCP servers — synchronously, from any hook or tool handler — routing through Zeloo' existing native MCP client (same connections, trust-tier gates, circuit breaker, and reconnect logic as model-invoked MCP tools; never a parallel client).
