...
When using [OpenRouter](https://openrouter.ai) as your LLM provider, Zeloo Agent supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
:::note
[Nous Portal](/integrations/nous-portal) decides routing centrally per model and does not accept caller-supplied provider preferences; Zeloo never sends the `provider` object to Portal, so `provider_routing` is simply ignored there.
:::

## Configuration

Add a `provider_routing` section to your `~/.Zeloo/config.yaml`:
...
own pins. Edit `config.yaml` directly for these keys: model ids contain dots, which `Zeloo config set`
...
- **CLI mode** — configured in `~/.Zeloo/config.yaml`, loaded at startup
