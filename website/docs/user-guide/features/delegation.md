...
Top-level model calls run in the background automatically. Zeloo returns a handle immediately so the conversation can continue, then posts the result back as a new message. An orchestrator subagent waits for its own workers so it can synthesize their results before returning.
...
One exception: when the parent has a resolved workspace directory, every subagent's system prompt embeds that workspace's **project context files** (`.Zeloo.md` > AGENTS.md chain > CLAUDE.md > `.cursorrules` — the same discovery, priority, and size caps as the main agent's system prompt; SOUL.md is excluded). Subagents working in a repo operate under the repo's own conventions without having to rediscover them.
...
When a top-level agent provides a `tasks` array, Zeloo returns one background handle and runs the subagents in parallel. By default the call returns **one** consolidated message once every task has finished. Results are delivered only between the parent's turns: the parent should finish anything that does not depend on the children, then end its turn rather than polling transcripts, artifacts, or CI while it waits.
...
When a background delegation finishes, Zeloo stores its completion event in
the active profile's `state.db` before publishing it to the normal fresh-turn
queue. If Zeloo restarts after completion but before delivery, the pending
...
Zeloo cannot prove whether its external side effects happened. Pending and
...
```
<zeloo_home>/cache/delegation/live/<delegation_id>/task-<n>.log
```
...
```bash
tail -f ~/.Zeloo/cache/delegation/live/deleg_ab12cd34/task-0.log
```
...
Top-level model-facing `delegate_task` calls run in the background automatically where the session supports later delivery. Zeloo returns a handle immediately, and the result re-enters the conversation after the child or batch finishes. Orchestrator subagents wait for their workers in the current turn because they must synthesize those results before returning. Stateless request/response endpoints fall back to synchronous execution when they cannot deliver a detached result later.
...
- A Zeloo process restart does **not** resume a running child. Its attempt becomes `unknown` because Zeloo cannot prove which side effects happened.
...
- Each child starts its terminal in `<repo>/.worktrees/subagent-<id>` on its
  own branch `Zeloo-subagent/subagent-<id>`, and its goal message tells it to
