...
Zeloo can run open models entirely on your own machine. It downloads and
...
sizes, GPU layers, or quantization. You pick a model; Zeloo does the rest.
...
2. Click **Install runtime**. Zeloo downloads the official llama.cpp
...
That's the whole flow. The server starts and stops with Zeloo, restarts
survive app restarts, and switching back to a cloud provider is one click
in the model picker.

## How Zeloo picks what to download
...
Models ship in several quality grades (quantizations). Zeloo picks the
...
guarantees. Below 4-bit the quality loss is too severe, so Zeloo never
...
Local models live or die by memory placement, so Zeloo manages it
...
- **Every recommended model gets at least a 64K context window.** When a
  model is larger than your GPU's memory, Zeloo deliberately places the
...
- **Memory fit includes the launch configuration**, not just the model file:
  ...
  count. For multi-token prediction (MTP), Zeloo uses smaller batches when
...
- **Conversation compression follows a growth check.** If a larger window
  ...
  Zeloo compresses instead of claiming a window the server did not receive.
...
- Results show download counts and a per-file fit check sized to your
  ...
  your GPU.
- Anything you download behaves exactly like a catalog model — Zeloo
...
If a llama-server is already running on your machine, Zeloo detects it
...
[Run Zeloo Locally with Ollama](/guides/local-ollama-setup) and
...
```yaml
local_runtime:
  enabled: false     # true = start the managed server with Zeloo.
                     # The desktop "Use" button sets this automatically.
  backend: auto      # auto | cuda | metal | vulkan | hip | cpu
  tag: b10362        # pinned llama.cpp release; Zeloo updates it with
                     # each release after re-validation
```

Models and runtime builds live under the Zeloo home directory
