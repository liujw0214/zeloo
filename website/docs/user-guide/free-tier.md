...
description: "What Zeloo gives you before you add a key or sign in, how the free tier coexists with your own API key, how to sign in, and how to turn it off."
---
...
unless the process was started with `ZELOO_GUEST_ONBOARDING=1` in its environment; without it a
fresh install behaves exactly as before (the provider picker on first run). This note goes away
when the rollout completes.
:::

A fresh Zeloo install works before you paste an API key or sign in anywhere. When Zeloo starts
...
`Zeloo setup` is still there when you want it; it is never forced.
...
While the free tier carries inference, the banner and `Zeloo auth status` read
`Nous · free tier · nous/welcome`, and `Zeloo model` lists a **Nous · free tier** row with that
...
names `Zeloo auth upgrade` in the terminal); the turn continues without it.

If `model.default` in `config.yaml` names something other than `nous/welcome` while the free tier
is doing inference, Zeloo uses `nous/welcome` anyway and says so in one line. The free tier
...
On an install that already has a provider, Zeloo still sets the free tier up once at start so
...
You can pick the free tier explicitly from `Zeloo model` (or `/model`) like any other provider.

## Signing in from a chat or terminal

### From a chat

Run `/login` in a Zeloo DM on Telegram, Discord, or another supported messaging platform (on
Slack use `/Zeloo login`), or in a CLI chat session. It must be a paired direct message:
elsewhere Zeloo replies `Sign in from a direct message with Zeloo.` Broadcast-shaped platforms
...
chatting while Zeloo waits, and the result is pushed into the same DM. Running `/login` again
...
`/login` binds this whole Zeloo install to the account that approves the code: its inference, its
...
1. Zeloo prints a URL and a short code, and opens the browser unless you pass `--no-browser`
...
Inference moves to your account's model catalog, paid tools unlock, and `Zeloo auth status`
...
model for its plan (the same one a fresh `Zeloo model` pick would suggest), and a default model
you chose yourself is left alone. If no recommendation is available at that moment, no default is
set and Zeloo tells you to run `Zeloo model`.

`/login` in a chat, or `Zeloo auth upgrade` in a terminal, is offered wherever the free tier is
...
`Zeloo auth add nous --type oauth` also signs you in, but it replaces the free tier outright and
does not carry your connectors over. Use `/login`, or `Zeloo auth upgrade` in a terminal, when
...
| Where | What you see |
|---|---|
| First launch | A ready screen: "Zeloo is ready." with the default model `nous/welcome`, a Free tier badge, and **Begin**. "Sign in with a Nous account instead" and "Other providers" sit under it. The screen shows once. |
...
```bash
Zeloo config set nous.guest false
```
...
| Free-tier row in `Zeloo model` | Shown | Hidden |
| Fresh install with nothing configured | Chats immediately | Offered `Zeloo setup` |
...
## What `Zeloo logout` does

| Situation | Result |
|---|---|
| Only the free tier is present | Nothing is cleared. Zeloo prints: `You're not signed in. Free inference and connectors are always on. Run Zeloo auth to sign in with a Nous account.` |
...
|---|---|---|
| First command prints `It looks like Zeloo isn't configured yet` and offers `Zeloo setup` | The free tier could not be set up within a few seconds: you are offline, or the free tier is not open on the portal Zeloo is pointed at, or it is rate limited. | Come back online and run the command again, or run `Zeloo setup` and add a provider of your own. Nothing is left half-configured. |
| `Nous free tier is not open on this portal.` | The portal Zeloo is pointed at is not offering the free tier right now. If you set `ZELOO_PORTAL_BASE_URL`, that portal may not have it at all. | Sign in with an account, unset a portal override you no longer need, or add your own key with `Zeloo setup`. |
| `Nous free tier is rate limited; try again shortly.` | The portal is throttling new free-tier setups at the moment. | Wait a few minutes and retry, or add your own key with `Zeloo setup`. |
| `This needs a Nous account.` | You called a paid Tool Gateway tool on the free tier. | `/login` in a chat, `Zeloo auth upgrade` in a terminal, or configure that tool with your own key in `Zeloo tools`. |
...
| The free tier stopped working after two weeks away | The free-tier identity expired (see below) and is replaced at the next start, or the next time a turn or connector finds it retired. | Nothing; start Zeloo again. Connectors linked before the gap need to be linked again unless you had signed in. |

## Privacy

To make the free tier work, Zeloo creates an identity on the Nous portal the first time it
needs one and stores the credential in your Zeloo directory, shared across the profiles under
...
14 days without use, at which point Zeloo transparently creates a new one the next time you run
a command. Signing in (`/login`, or `Zeloo auth upgrade` in a terminal) moves what that identity
