...
# Connecting Desktop to Many Zeloo Instances

Register every Zeloo backend you own — the local runtime, remote gateways on
your LAN or VPS, SSH hosts, and Zeloo Cloud instances — in one desktop app,
...
- **The sidebar profile rail** — the plug button at the right end of the rail
  (tooltip: **"Connect another Zeloo gateway…"**) deep-links straight to
...
list of Zeloo gateways. Its intro says it plainly: *"Manage this device and
every Zeloo gateway it can reach through remote, SSH, or Cloud connections."*
Each entry is a *connection*:

| Kind | What it is | Auth |
|---|---|---|
| **Local** | "The Zeloo runtime managed by this app." | automatic |
| **Remote gateway** | "A Zeloo gateway reachable over HTTP(S) — LAN, Tailscale, or the internet." | session token or OAuth |
| **SSH** | "A Zeloo install reached over SSH." The app opens the tunnel and starts the dashboard for you | SSH key + adopted token |
| **Zeloo Cloud** | "A hosted instance discovered through your Zeloo Cloud account." | portal sign-in |
...
- **Duplicates are rejected when you save**: there is only ever one **local**
  ...
  **SSH** entries are deduplicated on the normalized `user@host:port` plus
  remote profile.
- Cloud entries normally come from the Zeloo Cloud sign-in/discovery flow at
  the top of the Gateways page — the **Zeloo Cloud** kind in the add-connection
...
The Zeloo Cloud panel also lists **Saved Cloud gateways** when portal discovery
...
3. Pick the kind: **Local**, **Zeloo Cloud**, **Remote gateway**, or **SSH**.
   (**Local** is disabled while the app-managed local entry exists — which is
   almost always; **Zeloo Cloud** directs you to the cloud sign-in/discovery
   flow above.)
4. Fill the fields:
   - **Name** — required, unique; the "device name" shown everywhere this
     instance appears (placeholder: `Homelab`). Max 64 characters.
   - *Remote gateway only:*
     - **Gateway URL** — the base URL of a running `Zeloo serve` backend,
...
Gateways are machines or hosted backends; profiles are isolated Zeloo agents
...
connection is registered) dispatches `Zeloo update` to every eligible
...
- **Zeloo Cloud** instances are skipped with a *"Managed by Zeloo Cloud"*
...
panel, ⌘K **Update Zeloo**, the update-ready toast) run the same fan-out
...
- **"Connection test failed"** — the backend isn't reachable at that URL from
  this machine. Check that `Zeloo serve` is running on the remote host, the
...
- **"Update Zeloo Desktop to chat with agents on other connections"** — the
