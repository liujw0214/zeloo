...
`~/.Zeloo/config.yaml`, `~/.Zeloo/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/Zeloo`:

```text
/etc/Zeloo/
├── config.yaml     # managed config layer (wins over ~/.Zeloo/config.yaml)
└── .env            # managed env layer (wins over ~/.Zeloo/.env + shell)
```
...
The location can be relocated with the `ZELOO_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `ZELOO_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Zeloo.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export ZELOO_MANAGED_DIR=/opt/org/Zeloo-policy
```

:::warning
A user who can set `ZELOO_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `Zeloo doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/Zeloo/config.yaml` (managed) | `/etc/Zeloo/.env` (managed) |
| 2 | `~/.Zeloo/config.yaml` (user) | `~/.Zeloo/.env` (user) |
...
```bash
Zeloo config        # shows a header naming the managed source + the pinned keys
Zeloo doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Zeloo refuses and names the source:

```bash
$ Zeloo config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/Zeloo/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `Zeloo config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/Zeloo

# Pin some config values for every user on this machine
sudo tee /etc/Zeloo/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/Zeloo/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/Zeloo
sudo chmod 0644 /etc/Zeloo/config.yaml /etc/Zeloo/.env
```

Changes take effect on the next Zeloo start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`Zeloo doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Zeloo as `root`), managed scope is advisory.
