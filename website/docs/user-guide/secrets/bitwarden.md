...
Pull API keys from [Bitwarden Secrets Manager](https://bitwarden.com/products/secrets-manager/) at process startup instead of storing them in plaintext inside `~/.Zeloo/.env`. One bootstrap secret (a machine-account access token) replaces N per-provider keys, and rotating a credential becomes a single change in the Bitwarden web app.
...
2. Zeloo stores that single token in `~/.Zeloo/.env` as `BWS_ACCESS_TOKEN`.
3. Every time `Zeloo` (or the gateway, or a cron job) starts, after `~/.Zeloo/.env` has loaded, Zeloo calls `bws secret list <project_id>` and sets the returned keys into `os.environ`.
4. By default Zeloo **overrides** values already in your environment, so Bitwarden is the source of truth — rotate a key once in the web app and every Zeloo process picks it up on next start. Flip `override_existing: false` in config if you want `.env` to win instead.

The `bws` binary is auto-downloaded into `~/.Zeloo/bin/` on first use — no `apt`, no `brew`, no `sudo`.
...
2. Create or pick a **Project** (e.g. "Zeloo keys").
...
4. **Machine accounts → New machine account → My Zeloo machine** → **Projects** tab → grant Read access to your project.
...
```bash
Zeloo secrets bitwarden setup
```

It will:

1. Download and verify `bws v2.0.0` into `~/.Zeloo/bin/bws`.
2. Prompt you for the access token (input is hidden). Stored in `~/.Zeloo/.env` as `BWS_ACCESS_TOKEN`.
...
```bash
Zeloo secrets bitwarden status
```

From now on, every `Zeloo` invocation pulls fresh secrets at startup. You'll see a one-line summary in stderr the first time secrets are applied in a process.

## CLI

| Command | What it does |
|---|---|
| `Zeloo secrets bitwarden setup` | Interactive wizard (install binary, prompt for token, pick project, test fetch) |
| `Zeloo secrets bitwarden status` | Show config + binary version + token presence/validation |
| `Zeloo secrets bitwarden token` | Rotate the access token: validate the new token against Bitwarden, then store it in `.env` |
| `Zeloo secrets bitwarden sync` | Dry-run: pull secrets now and show what would be applied |
| `Zeloo secrets bitwarden sync --apply` | Pull and export into the current shell's environment |
| `Zeloo secrets bitwarden install` | Just download the pinned `bws` binary (no auth required) |
| `Zeloo secrets bitwarden disable` | Flip `enabled: false`; leaves token + project id in place |
...
Bitwarden Secrets Manager: → Run `Zeloo secrets bitwarden token` to paste a fresh access token ...
```

Fix it without re-running the whole wizard:

```bash
Zeloo secrets bitwarden token                     # masked prompt
Zeloo secrets bitwarden token --access-token 0.…  # non-interactive
```
...
| `encrypted_cache.enabled` | `false` | Store the last successful fetch in an AES-GCM encrypted cache at `~/.Zeloo/cache/bws_cache.enc.json`. |
...
| `auto_install` | `true` | When true, `bws` is auto-downloaded into `~/.Zeloo/bin/` on first use. |

## Failure modes

Bitwarden never blocks Zeloo startup. If anything goes wrong, you'll see a one-line warning in stderr and Zeloo continues with whatever credentials `.env` already had:

| Symptom | Cause | Fix |
|---|---|---|
| `BWS_ACCESS_TOKEN is not set` | Enabled in config but token cleared from `.env` | Re-run `Zeloo secrets bitwarden setup` |
| `Bitwarden rejected the machine-account access token … invalid_client` | Token revoked, expired, machine account deleted — or the token belongs to another region (e.g. EU token hitting the US identity endpoint) | Run `Zeloo secrets bitwarden token` to paste a fresh token; for region mismatches re-run setup and pick EU/self-hosted (or set `secrets.bitwarden.server_url`) |
| `bws exited 1: invalid access token` | Token revoked or wrong | Run `Zeloo secrets bitwarden token` with a new token |
...
- Zeloo will refuse to let Bitwarden overwrite the bootstrap token itself, even with `override_existing: true`. If you store `BWS_ACCESS_TOKEN` as a secret inside the project, it's silently skipped during apply.
...
- The pinned version (`bws v2.0.0` at time of writing) is updated through PRs to this repo — Zeloo does not auto-upgrade `bws` to "latest" because upstream release shapes can change.

## When NOT to use this

- **Single-machine personal setups** where `~/.Zeloo/.env` is fine. You're trading one credential for another and adding a network dependency at startup.
...
The good case for this is multi-machine fleets, shared dev boxes, gateway VPSes, or any setup where you want centralized rotation and revocation across multiple Zeloo installations.
