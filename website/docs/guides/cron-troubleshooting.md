...
description: "Diagnose and fix common Zeloo cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`Zeloo gateway` for foreground, or `Zeloo gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `Zeloo cron tick`.
...
```bash
date
Zeloo cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.Zeloo/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.Zeloo/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.Zeloo/.env` |
...
| `local` | Write access to `~/.Zeloo/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `Zeloo cron list` for updated `last_error` field (if available).
...
### Check 5: Relay-fronted platforms (Zeloo Cloud / Team Gateway)
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `Zeloo skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `Zeloo skills list`.
...
2. `~/.Zeloo/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `Zeloo cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Zeloo config directory). Verify:
```bash
ls ~/.Zeloo/scripts/your-script.py   # Must exist
Zeloo cron edit <job_id> --script ~/.Zeloo/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `Zeloo skills install <skill-name>`.
...
Jobs are stored in `~/.Zeloo/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.Zeloo/cron/jobs.json
chmod 600 ~/.Zeloo/cron/jobs.json   # Your user should own it
```
...
```bash
Zeloo cron list                    # Show all jobs, states, next_run times
Zeloo cron run <job_id>            # Schedule for next tick (for testing)
Zeloo cron edit <job_id>           # Fix configuration issues
Zeloo logs                         # View recent Zeloo logs
Zeloo skills list                  # Verify installed skills
```
...
1. Run the job with `Zeloo cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.Zeloo/logs/agent.log` for scheduler messages and `~/.Zeloo/logs/errors.log` for warnings
3. Open an issue at [github.com/NousResearch/Zeloo-agent](https://github.com/NousResearch/Zeloo-agent) with:
