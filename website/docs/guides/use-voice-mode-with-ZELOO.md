---
sidebar_position: 8
title: "Use Voice Mode with Zeloo"
description: "A practical guide to setting up and using Zeloo voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Zeloo
...
- you want Zeloo sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Zeloo.
...
## Step 1: make sure normal Zeloo works first
...
Zeloo supports both local and cloud speech stacks.
...
### If you use `Zeloo setup`

If you choose NeuTTS in the setup wizard, Zeloo checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Zeloo transcribes and responds
...
- asking Zeloo to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Zeloo loop.

## Tuning CLI behavior

### Silence threshold

If Zeloo starts/stops too aggressively, tune:
...
Zeloo stays a normal chat bot, but can speak replies.
...
- you want Zeloo to function like a portable research or ops assistant
...
Zeloo joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Zeloo detects speech boundaries
- transcripts are posted in the associated text channel
- Zeloo responds in text and audio
...
1. get text Zeloo working
2. run `Zeloo setup tts` to enable voice support
