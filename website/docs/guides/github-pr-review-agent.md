...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Zeloo Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Zeloo instantly when PRs are opened or updated.
:::
...
- **Zeloo Agent installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  Zeloo gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.Zeloo/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Zeloo can access GitHub. Start a chat:
...
```
Run: gh pr list --repo NousResearch/Zeloo-agent --state open --limit 3
```
...
Still in the chat, ask Zeloo to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo NousResearch/Zeloo-agent
```
...
A skill gives Zeloo consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.Zeloo/skills/code-review
```

Create `~/.Zeloo/skills/code-review/SKILL.md`:
...
Verify it loaded — start `Zeloo` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Zeloo your team's standards:
...
2. Teach Zeloo your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
Zeloo gateway status    # Is the gateway running?
Zeloo cron list         # Is the job enabled?
```
