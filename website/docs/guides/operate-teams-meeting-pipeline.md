...
```bash
Zeloo teams-pipeline token-health
Zeloo teams-pipeline token-health --force-refresh
```
...
```bash
Zeloo teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
Zeloo teams-pipeline maintain-subscriptions
Zeloo teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Zeloo cron (recommended if you already run the Zeloo gateway)

Zeloo ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.Zeloo/scripts/`. First create the script:

```bash
mkdir -p ~/.Zeloo/scripts
cat > ~/.Zeloo/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec Zeloo teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.Zeloo/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
Zeloo cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/Zeloo-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Zeloo Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=Zeloo
EnvironmentFile=/etc/Zeloo/env
ExecStart=/usr/local/bin/Zeloo teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/Zeloo-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Zeloo Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now Zeloo-teams-pipeline-maintain.timer
systemctl list-timers Zeloo-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/Zeloo teams-pipeline maintain-subscriptions >> /var/log/Zeloo/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.Zeloo/.env` at the top of a wrapper script that crontab calls.
...
```bash
Zeloo teams-pipeline subscriptions   # should show expirationDateTime advanced
Zeloo teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
Zeloo teams-pipeline list
Zeloo teams-pipeline list --status failed
Zeloo teams-pipeline show <job-id>
```

### Replay a stored job

```bash
Zeloo teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
Zeloo teams-pipeline fetch --meeting-id <meeting-id>
Zeloo teams-pipeline fetch --join-web-url "<join-url>"
Zeloo teams-pipeline fetch --join-web-url "<join-url>" --organizer-user-id <entra-user-id>
```
...
- run `Zeloo teams-pipeline maintain-subscriptions --dry-run`
- inspect `Zeloo teams-pipeline list --status failed`
...
- run `Zeloo teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `Zeloo teams-pipeline run`
...
- [ ] `Zeloo teams-pipeline validate` returns an OK snapshot
- [ ] `Zeloo teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Zeloo cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
