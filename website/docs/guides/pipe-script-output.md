...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `Zeloo send`."
---

# Pipe Script Output to Messaging Platforms

`Zeloo send` is a small, scriptable CLI that pushes a message to any
messaging platform Zeloo is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | Zeloo send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `Zeloo
...
Zeloo send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
Zeloo send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
Zeloo send --list

# Filter by platform
Zeloo send --list telegram
```
...
Any platform Zeloo ships adapters for works as a target:
...
`Zeloo send` resolves the message body in this order:

1. **Positional argument** — `Zeloo send --to telegram "hi"`
2. **`--file PATH`** — `Zeloo send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | Zeloo send --to telegram`

When stdin is a TTY (no pipe), Zeloo does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  Zeloo send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `Zeloo send` reuses your Zeloo config, the same script works on
any host where Zeloo is installed — no need to export bot tokens into
...
`Zeloo send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  Zeloo send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | Zeloo send \
...
```bash
./train.py --epochs 200 && \
  Zeloo send --to telegram "training done" || \
  Zeloo send --to telegram "training failed (exit $?)"
```
...
Zeloo send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(Zeloo send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `Zeloo send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `Zeloo send` calls
the platform's REST endpoint directly using credentials from
`~/.Zeloo/.env` and `~/.Zeloo/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `Zeloo gateway start` and retry.
...
# Machine-readable
Zeloo send --list --json
```

The listing is built from `~/.Zeloo/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`Zeloo gateway
...
| Approach | Multi-platform | Reuses Zeloo creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `Zeloo send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`Zeloo send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `Zeloo send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `Zeloo send` shares with cron delivery.
