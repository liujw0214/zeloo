---
sidebar_position: 7
title: "Use SOUL.md with Zeloo"
description: "How to use SOUL.md to shape Zeloo Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Zeloo

`SOUL.md` is the **primary identity** for your Zeloo instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Zeloo to feel like the same assistant every time you talk to it — or if you want to replace the Zeloo persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Zeloo should be
- what Zeloo should avoid stylistically
- how Zeloo should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Zeloo is and how Zeloo speaks
...
Zeloo now uses only the global SOUL file for the current instance:

```text
~/.Zeloo/SOUL.md
```

If you run Zeloo with a custom home directory, it becomes:
...
Zeloo automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Zeloo does not overwrite it
- if the file exists but is empty, Zeloo adds nothing from it to the prompt

## How Zeloo uses it

When Zeloo starts a session, it reads `SOUL.md` from `ZELOO_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Zeloo feels.
...
Zeloo already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Zeloo should sound.

# Avoid
What Zeloo should not do.

# Defaults
How Zeloo should behave when ambiguity appears.
```
...
Then restart Zeloo or start a new session.
...
### I edited SOUL.md but Zeloo still sounds the same

Check:
- you edited `~/.Zeloo/SOUL.md` or `$ZELOO_HOME/SOUL.md`
...
### Zeloo is ignoring parts of my SOUL.md
