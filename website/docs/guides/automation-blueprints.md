...
Copy-paste blueprints for common automation patterns. Each blueprint uses Zeloo's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`Zeloo webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `Zeloo webhook subscribe`) |
...
```bash
Zeloo cron create "0 2 * * *" \
  "You are a project manager triaging the NousResearch/Zeloo-agent GitHub repo.

1. Run: gh issue list --repo NousResearch/Zeloo-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
Zeloo webhook subscribe github-pr-review \
  --events "pull_request" \
...
Zeloo cron create "0 9 * * 1" \
  "Scan the NousResearch/Zeloo-agent repo for documentation drift.

1. Run: gh pr list --repo NousResearch/Zeloo-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (zeloo_cli/commands.py, zeloo_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (zeloo_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
Zeloo cron create "0 6 * * *" \
  "Run a dependency security audit on the Zeloo-agent project.

1. cd ~/.Zeloo/Zeloo-agent && source .venv/bin/activate
...
```bash
Zeloo webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
Zeloo webhook subscribe alert-triage \
...
```python title="~/.Zeloo/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Zeloo-Monitor/1.0"})
...
Zeloo cron create "every 30m" \
  ...
  --script ~/.Zeloo/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
Zeloo cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Zeloo Agent development." \
...
```bash
Zeloo webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
Zeloo webhook subscribe stripe-payments \
...
```bash
Zeloo cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the Zeloo-agent codebase.
