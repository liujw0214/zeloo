---
sidebar_position: 6
title: "Use MCP with Zeloo"
description: "A practical guide to connecting MCP servers to Zeloo Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Zeloo

This guide shows how to actually use MCP with Zeloo Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Zeloo tool
- you want Zeloo to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Zeloo to internal APIs, databases, or company systems without modifying Zeloo core

Do not use MCP when:
- a built-in Zeloo tool already solves the job well
...
- Zeloo remains the agent
- MCP servers contribute tools
- Zeloo discovers those tools at startup or reload time
...
If you installed Zeloo with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Zeloo banner/status should show MCP integration when configured
- ask Zeloo what tools it has available
...
## WSL2: bridge Zeloo in WSL to Windows Chrome

This is the practical setup when:

- Zeloo runs inside WSL2
...
In this setup, Zeloo does **not** connect to Chrome directly. Instead:

- Zeloo runs in WSL
- Zeloo starts a local stdio MCP server
...
```text
Zeloo (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Zeloo stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Zeloo core browser transport
...
```bash
Zeloo mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
Zeloo mcp test chrome-devtools-win
```

Then start a fresh Zeloo session or run:
...
Once loaded, Zeloo can use the MCP-prefixed browser tools directly. For example:
...
If Zeloo runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Zeloo from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Zeloo from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Zeloo:
...
2. Zeloo-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Zeloo will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Zeloo to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want Zeloo to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. Zeloo remains the agent; Open Scaffold remains the repo-local record.

Add the server for one scaffolded repository:

```bash
Zeloo mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
Zeloo mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `Zeloo mcp add` prompt, or edit `config.yaml` afterward:
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want Zeloo to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize Zeloo to merge, publish, deploy, or spawn runtimes.
...
Now Zeloo can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Zeloo core.
...
Because Zeloo now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
