...
This page covers all commands related to [Zeloo profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `Zeloo profile` without a subcommand shows help.
...
```bash
Zeloo profile use <name>
```

Sets `<name>` as the active profile. All subsequent `Zeloo` commands (without `-p`) will use this profile.
...
Zeloo profile use default
```

## `Zeloo profile create`

```bash
Zeloo profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `Zeloo profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `Zeloo update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.Zeloo`), use `Zeloo skills opt-out` / `Zeloo skills opt-in`. |
...
Zeloo profile create work2 --clone-from work

# Clone everything from a specific profile
Zeloo profile create work2-backup --clone-from work --clone-all
```

## `Zeloo profile describe`

```bash
Zeloo profile describe [<name>] [options]
```
...
Zeloo profile describe researcher

# Set it explicitly
Zeloo profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
Zeloo profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
Zeloo profile describe --all --auto
```

## `Zeloo profile delete`

```bash
Zeloo profile delete <name> [options]
```
...
```bash
Zeloo profile delete mybot
Zeloo profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.Zeloo`) cannot be deleted — use `Zeloo uninstall` to remove everything.
:::
...
This shows the profile's Zeloo home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ Zeloo profile show work
Profile: work
Path:    ~/.Zeloo/profiles/work
...
```bash
Zeloo profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Zeloo installation.
...
```bash
Zeloo profile alias work
# Creates/updates ~/.local/bin/work

Zeloo profile alias work --name mywork
# Creates ~/.local/bin/mywork

Zeloo profile alias work --remove
# Removes the wrapper script
```

## `Zeloo profile rename`

```bash
Zeloo profile rename <old-name> <new-name>
```
...
```bash
Zeloo profile rename mybot assistant
# ~/.Zeloo/profiles/mybot → ~/.Zeloo/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `Zeloo profile export`

```bash
Zeloo profile export <name> [options]
```
...
```bash
Zeloo profile export work
# Creates work.tar.gz in the current directory

Zeloo profile export work -o ./work-2026-03-29.tar.gz
```
...
## `Zeloo profile import`

```bash
Zeloo profile import <archive> [options]
```
...
```bash
Zeloo profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

Zeloo profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `Zeloo -p telemetry`). |
...
Zeloo profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
Zeloo profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
Zeloo profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
Zeloo profile install ./telemetry/
```

### `Zeloo profile update`

```bash
Zeloo profile update <name> [--force-config] [--yes]
```
...
```bash
Zeloo profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Zeloo version, author, env var requirements, the source URL/path, and
...
`Zeloo profile list` also shows the distribution name and version in a
`Distribution` column, and `Zeloo profile show <name>` / `delete <name>`
...
Zeloo profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
Zeloo profile install https://github.com/your-org/internal-assistant.git
```
...
`zeloo_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Zeloo version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Zeloo can clone from.
3. Tell recipients to run `Zeloo profile install <your-repo-url>`.
...
## `Zeloo -p` / `Zeloo --profile`

```bash
Zeloo -p <name> <command> [options]
Zeloo --profile <name> <command> [options]
```

Global flag to run any Zeloo command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
Zeloo -p work chat -q "Check the server status"
Zeloo --profile dev gateway start
Zeloo -p personal skills list
Zeloo -p work config edit
```

## `Zeloo completion`

```bash
Zeloo completion <shell>
```
...
- `Zeloo profile <TAB>` — subcommands (list, use, create, etc.)
- `Zeloo profile use <TAB>` — profile names
- `Zeloo -p <TAB>` — profile names
