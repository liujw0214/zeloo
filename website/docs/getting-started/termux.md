...
description: "Run Zeloo Agent directly on an Android phone with Termux"
---

# Zeloo on Android with Termux
...
Zeloo Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Zeloo from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
APT-managed Zeloo installs are marked with install method `apt`. Zeloo therefore does not run its Git self-updater against package-owned files; use the package manager instead:

```bash
pkg update
pkg upgrade Zeloo-agent
```

Packaging/repository/signing problems for this option should be reported to the community packaging repositories above. Zeloo runtime bugs can still be reported here, keeping in mind that Android/Termux support is best-effort.

---

## Option 1: One-line installer

Zeloo now ships a Termux-aware installer path:
...
- links `Zeloo` into `$PREFIX/bin` so it stays on your Termux PATH
...
Zeloo requires **Python >=3.11,&lt;3.14**. Current Termux ships `python`
...
### 5. Put `Zeloo` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/Zeloo" "$PREFIX/bin/Zeloo"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `Zeloo` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.Zeloo/.env`.
...
### `Zeloo doctor` says ripgrep or Node is missing
